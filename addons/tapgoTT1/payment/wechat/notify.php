...
if(strpos($get['out_trade_no'],'RC') !== false){ 
    $get['attach']="8:1";
}else{
    $get['attach']="8:0";
}
...
// $gets='{"bank_type":"ABC_DEBIT","cash_fee":"100","charset":"UTF-8","coupon_fee":"0","fee_type":"CNY","mch_id":"QRA393486610002","mdiscount":"0","nonce_str":"1642951697022","openid":"omCqSs16hnn9PGgV1_3mjreQeBVw","out_trade_no":"SH20220123232802286622","out_transaction_id":"4200001403202201234955110740","pay_result":"0","result_code":"0","sign":"7F2575E624223B969D4DD72F84B6A687","sign_type":"MD5","status":"0","time_end":"20220123232816","total_fee":"100","trade_type":"pay.weixin.jspay","transaction_id":"95516000851013200403734366090231","version":"2.0","attach":"8:0"}';

// $get=json_decode($gets,true);

$myfile = fopen("/www/wwwroot/w.itapgo.com/data/newfile.txt", "w") or die("Unable to open file!");
$txt =json_encode($get);
fwrite($myfile, $txt);
fclose($myfile);
//$get = json_decode('{
...
require IA_ROOT.'/addons/tapgo_ttv2/defines.php';
require IA_ROOT.'/addons/tapgo_ttv2/core/inc/functions.php';
require IA_ROOT.'/addons/tapgo_ttv2/core/inc/plugin_model.php';
require IA_ROOT.'/addons/tapgo_ttv2/core/inc/com_model.php';
...
class EweiShopWechatPay
{
    ...
    public function __construct($get)
    {
        ...
        if($this->get['fee_type']=="CNY"){
            $out_trade_no=$this->get['out_trade_no'];
            $this->get['out_trade_no'] = substr($out_trade_no,0,strlen($out_trade_no)-5);
        }
        
        $tid3= substr($this->get['out_trade_no'], 0, 22);
        ...
        $order = pdo_fetch("SELECT * FROM ".tablename('tapgo_tt_order')." WHERE ordersn = :ordersn ",array(':ordersn'=>$tid3));
        $ordersn = $tid = $this->get['out_trade_no'];
         
        
        $GLOBALS['_W']['uniacid'] = $order['uniacid'];
    ...
    {
        $myfile = fopen("/www/wwwroot/w.itapgo.com/data/newfile5.txt", "w") or die("Unable to open file!");
        $txt =$this->type ;
        fwrite($myfile, $txt);
        fclose($myfile);
        if ($this->type == '0'){
    ...
    public function order()
    {
        ...
        if (strexists($tid, 'GJ')) {
            ...
            if($ordersn2 >= 100){
                pdo_update('tapgo_tt_order', array('ordersn2' =>$ordersn2), array('ordersn' => $tid, 'openid' => $openid));
            }
        }
        $ispeerpay = 0;

        $myfile = fopen("/www/wwwroot/w.itapgo.com/data/newfile8.txt", "w") or die("Unable to open file!");
        $txt =$tid;
        fwrite($myfile, $txt);
        fclose($myfile);
        ...
        $order = pdo_fetch("SELECT * FROM ".tablename('tapgo_tt_order')." WHERE ordersn = :ordersn ",array(':ordersn'=>$tid));
        
       

        $sql = 'SELECT * FROM ' . tablename('core_paylog') . ' WHERE `module`=:module AND `tid`=:tid  limit 1';
        $params = array();
        pdo_update('tapgo_tt_order',array('is_wxapp'=>intval($this->is_wxapp)),array('id'=>$order['id']));
        $params[':tid'] = $tid;
        $params[':module'] = 'tapgo_ttv2';
        ...
        if (!empty($log) && ($log['status'] == '0' || $ispeerpay)  && ($log['fee']==$this->total_fee || $ispeerpay) ) {
            ...
            if ($count_ordersn == 2) {
                pdo_update('tapgo_tt_order', array('tradepaytype' => 21,'isborrow'=>$isborrow,'borrowopenid'=>$borrowopenid,'apppay'=>$this->isapp?1:0,'transid'=>$transaction_id), array('ordersn_trade' => $log['tid'],'uniacid'=>$log['uniacid']));
            } else {
                pdo_update('tapgo_tt_order', array('paytype' => 21,'isborrow'=>$isborrow,'borrowopenid'=>$borrowopenid,'apppay'=>$this->isapp?1:0,'transid'=>$transaction_id), array('ordersn' => $log['tid'],'uniacid'=>$log['uniacid']));
            }

            $site = WeUtility::createModuleSite($log['module']);
            m('order')->setOrderPayType($order['id'], $paytype);
            
            $myfile = fopen("/www/wwwroot/w.itapgo.com/data/newfile11.txt", "w") or die("Unable to open file!");
            $txt =$paytype;
            fwrite($myfile, $txt);
            fclose($myfile);
            ...
            if (!empty($ispeerpay)) {
                ...
                $myfile = fopen("/www/wwwroot/w.itapgo.com/data/newfile9.txt", "w") or die("Unable to open file!");
                $txt =json_encode($ispeerpay);
                fwrite($myfile, $txt);
                fclose($myfile);
                ...
                if (!empty($ispeerpay)){//是代付
                    ...
                    m('order')->peerStatus(array('pid' => $ispeerpay['id'], 'uid' => $member['id'], 'uname' => $member['nickname'], 'usay' => '支持一下，么么哒!', 'price' => $this->total_fee, 'createtime' => time(), 'openid' => $openid, 'headimg' => $member['avatar'], ));
                    m('order')->setOrderPayType($order['id'], $paytype);
                    //高并发代付时支付完成清除
                    ...
                    $peerpay_info = (float)pdo_fetchcolumn("select SUM(price) from " . tablename('tapgo_tt_order_peerpay_payinfo') . ' where pid=:pid limit 1', array(':pid' => $ispeerpay['id']));
    ...
    public function recharge()
    {
        ...
        $log = pdo_fetch('SELECT * FROM ' . tablename('tapgo_tt_member_log') . ' WHERE  `logno`=:logno limit 1', array(':logno' => $logno));
        $OK= !empty($log) && empty($log['status']) && $log['money']==$this->total_fee;
        
        $GLOBALS['_W']['uniacid'] = $log['uniacid'];
        $_W['uniacid'] =  $log['uniacid'];
        
        $myfile = fopen("/www/wwwroot/w.itapgo.com/data/newfile1.txt", "w") or die("Unable to open file!");
        $txt =json_encode($log);
        fwrite($myfile, $txt);
        fclose($myfile);
        ...
        if ($OK) {
            //充值状态
            pdo_update('tapgo_tt_member_log', array('status' => 1, 'rechargetype' => 'wechat','isborrow'=>$isborrow,'borrowopenid'=>$borrowopenid, 'apppay'=>$this->isapp?1:0), array('id' => $log['id']));
            //增加会员余额
    ...
    public function creditShop()
    {
        ...
        $logno = str_replace('_borrow','',$logno);
        /*$log = pdo_fetch('SELECT * FROM ' . tablename('tapgo_tt_creditshop_log') . ' WHERE `logno`=:logno and `uniacid`=:uniacid  limit 1', array(':uniacid' => $_W['uniacid'], ':logno' => $logno));
        if (!empty($log) && empty($log['status'])) {
            $goods = pdo_fetch("SELECT * FROM" .tablename("tapgo_tt_creditshop_goods") . "WHERE id=:id and uniacid=:uniacid limit 1 ",array(":id"=>$log['goodsid'], ":uniacid"=>$_W['uniacid']));
            if(!empty($goods) && $this->total_fee==$goods['money']){
                pdo_update('tapgo_tt_creditshop_log', array('paystatus' => 1, 'paytype' => 1), array('id' => $log['id']));
            }
        }*/
    ...
    public function creditShopFreight()
    {
        ...
        $log = pdo_fetch('SELECT * FROM ' . tablename('tapgo_tt_creditshop_log') . ' WHERE `logno`=:dispatchno and `uniacid`=:uniacid  limit 1', array(':uniacid' => $_W['uniacid'], ':dispatchno' => $dispatchno));

        if (!empty($log) && $log['dispatchstatus'] <= 0) {
            pdo_update('tapgo_tt_creditshop_log', array('dispatchstatus' => 1,'status' => 2,'paytype'=>1,'paystatus'=>1), array('logno' => $dispatchno));
        }
    }
    ...
    public function coupon()
    {
        ...
        $coupon = pdo_fetchcolumn('select money from ' . tablename('tapgo_tt_coupon') . ' where id=:id limit 1', array(':id' => $log['couponid']));
 ...
 */
    ...
    public function groups()
    {
        ...
        if ($this->is_jie){
            pdo_update('tapgo_tt_groups_order', array('isborrow'=>'1','borrowopenid'=>$this->get['openid']), array('orderno'=>$orderno, 'uniacid' => $_W['uniacid']));
        }
        if(p('groups')){
    ...
    public function threen()
    {
        ...
        if ($this->is_jie){
            pdo_update('tapgo_tt_threen_log', array('isborrow'=>'1','borrowopenid'=>$this->get['openid']), array('logno'=>$orderno, 'uniacid' => $_W['uniacid']));
        }
        if(p('threen')){
    ...
    {
        global $_W;
        $setting = pdo_fetch("select * from ".tablename('tapgo_tt_system_grant_setting')." where id = 1 limit 1 ");
        if ($setting['weixin']>0) {
            ksort($this->get);
            $string1 = '';
            ...
            if ( $this->sign ==  $this->get['sign']) {
                $order = pdo_fetch("select * from ".tablename('tapgo_tt_system_grant_order')." where logno = '".$this->get['out_trade_no']."'");
                pdo_update('tapgo_tt_system_grant_order', array('paytime'=>time(),'paystatus'=>1), array('logno' => $this->get['out_trade_no']));
                ...
                foreach ($plugind as $key => $value){
                    $plugin = pdo_fetch("select `identity` from ".tablename('tapgo_tt_plugin')." where id = ".$value." ");
                    $data['identity'] = $plugin['identity'];
                    $data['pluginid'] = $value;
                    pdo_insert('tapgo_tt_system_grant_log', $data);
                    $id = pdo_insertid();
                    if(m('grant')){
    ...
    {
        global $_W;
        $setting = pdo_fetch("select * from ".tablename('tapgo_tt_system_plugingrant_setting')." where 1 = 1 limit 1 ");
        if ($setting['weixin']>0) {
            ksort($this->get);
            $string1 = '';
            foreach ($this->get as $k => $v) {
            ...
            $this->sign = strtoupper(md5($string1 . "key={$setting['apikey']}"));
            if ( $this->sign ==  $this->get['sign']) {
                $order = pdo_fetch("select * from ".tablename('tapgo_tt_system_plugingrant_order')." where logno = '".$this->get['out_trade_no']."'");
                pdo_update('tapgo_tt_system_plugingrant_order', array('paytime'=>time(),'paystatus'=>1), array('logno' => $this->get['out_trade_no']));
                $plugind = explode(",",$order['pluginid']);
                $data = array(
                    'logno'=>$order['logno'],
                    'uniacid'=>$order['uniacid'],
                ...
                foreach ($plugind as $key => $value){
                    ...
                    $data['identity'] = $plugin['identity'];
                    $data['pluginid'] = $value;
                    pdo_query('update ' . tablename('tapgo_tt_system_plugingrant_plugin') . " set sales = sales + 1 where pluginid = ".$value." ");
                    pdo_insert('tapgo_tt_system_plugingrant_log', $data);
                    $id = pdo_insertid();
                    if(p('grant')){
    ...
    public function mr()
    {
        ...
        if(p('mr')){
            $price = pdo_fetchcolumn('select payprice from ' . tablename('tapgo_tt_mr_order') . ' where ordersn=:ordersn limit 1', array(':ordersn' =>$ordersn));
            if($price==$this->total_fee){
                if($isborrow==1){
                    pdo_update('tapgo_tt_order', array('isborrow'=>$isborrow,'borrowopenid'=>$borrowopenid), array('ordersn' => $ordersn));
                }
        ...
        if (!empty($log) && $log['status'] == '0'  && $log['fee']==$this->total_fee ) {
            ...
            if (!is_error($site)) {
                ...
                if (method_exists($site, $method)) {
                    ...
                    $ret['fee'] = $log['fee'];
                    $ret['tag'] = $log['tag'];
                    pdo_update('tapgo_tt_order', array('paytype' => 21,'apppay'=>2,'transid'=>$transaction_id), array('ordersn' => $log['tid'],'uniacid'=>$log['uniacid']));
                    $result = $site->$method($ret);
                    if ($result) {
                        $log['tag'] = iunserializer($log['tag']);
                        $log['tag']['transaction_id'] = $this->get['transaction_id'];
    ...
    public function wxapp_recharge()
    {
        ...
        $log = pdo_fetch('SELECT * FROM ' . tablename('tapgo_tt_member_log') . ' WHERE `uniacid`=:uniacid and `logno`=:logno limit 1', array(':uniacid' => $_W['uniacid'], ':logno' => $logno));
        $OK= !empty($log) && empty($log['status']) && $log['money']==$this->total_fee;
        if ($OK) {
            //充值状态
            pdo_update('tapgo_tt_member_log', array('status' => 1, 'rechargetype' => 'wechat', 'apppay'=>2), array('id' => $log['id']));
            //增加会员余额
            $shopset = m('common')->getSysset('shop');
            m('member')->setCredit($log['openid'], 'credit2', $log['money'], array(0, $shopset['name'].'会员充值:微信充值:余额:' . $log['money']));
            //充值积分
            ...
            pdo_update('tapgo_tt_member_log', array('rechargetype' => 'wechat', 'apppay'=>2), array('id' => $log['id']));
        }
    }
    ...
    public function wxapp_groups()
    {
        ...
        $sql = 'SELECT * FROM ' . tablename('tapgo_tt_groups_paylog') . ' WHERE `tid`=:orderno  limit 1';
        $params = array();
        $params[':orderno'] = $orderno;
        $log = pdo_fetch($sql, $params);
        if (!empty($log) && $log['status'] == '0'  && $log['fee']==$this->total_fee ) {
            if(p('groups')){
                pdo_update('tapgo_tt_groups_paylog', array('status' => '1'), array('id' => $log['id']));
    ...
    public function membercard()
    {
        ...
        if ($this->is_jie){
            pdo_update('tapgo_tt_member_card_order', array('isborrow'=>'1','borrowopenid'=>$this->get['openid']), array('orderno'=>$orderno, 'uniacid' => $_W['uniacid']));
        }
        if(p('membercard')){
