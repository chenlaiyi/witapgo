...
class aliApy
{
        ...
        if (!empty($log) && $log['status'] == '0') {
            ...
            if (!is_error($site)) {
                ...
                if (method_exists($site, $method)) {
                    ...
                    pdo_update('tapgo_tt_order', array('paytype' => 22),array('uniacid'=>$log['uniacid'],'ordersn' => $log['tid']));
                    ...
                    if ($result) {
                        ...
                        pdo_update('tapgo_tt_order', array('paytype' => 22, 'apppay'=>$this->isapp?1:0,'transid'=>$this->post['trade_no']), array('ordersn' => $log['tid'],'uniacid'=>$log['uniacid']));
                        exit('success');
                    }
                }
            }
    ...
    public function threen()
    {
        ...
        $log = pdo_fetch('SELECT * FROM ' . tablename('tapgo_tt_threen_log') . ' WHERE `uniacid`=:uniacid and logno = :logno limit 1', array(':uniacid' => $_W['uniacid'], ':logno' => $logno));
        if(!$this->isapp && $this->post['sign_type']=="RSA"){
    ...
    public function recharge()
    {
        ...
        if (!empty($log) && empty($log['status'])) {
            pdo_update('tapgo_tt_member_log', array('status' => 1, 'rechargetype' => 'alipay', 'apppay'=>$this->isapp?1:0,'transid'=>$this->post['trade_no']), array('id' => $log['id']));
    ...
    public function batch_trans_notify_cp($id,$money)
    {
        ...
        $apply = pdo_fetch('select * from ' . tablename('tapgo_tt_commission_apply') . ' where id=:id limit 1'
            , array(':id' => $id));
        ...
        $list = pdo_fetchall("select id,agentid, ordersn,price,goodsprice, dispatchprice,createtime, paytype from " . tablename('tapgo_tt_order') . " where  id in ( " . implode(",", $ids) . " );");
        $totalcommission = 0;
        $totalpay = 0;
        $totalmoney = 0;
        foreach ($list as &$row) {
    ...
    {
        $log = pdo_fetch('select * from ' . tablename('tapgo_tt_member_log') . ' where id=:id limit 1', array(
            ':id' => $id
        ));
        ...
        pdo_update('tapgo_tt_member_log', array('status' => 1), array('id' => $id, 'uniacid' => $_W['uniacid']));
        m('notice')->sendMemberLogMessage($log['id']);
        $member = m('member')->getMember($log['openid']);
    ...
    {
        $item = pdo_fetch("SELECT * FROM " . tablename('tapgo_tt_order') . " WHERE id = :id limit 1", array(':id' => $id));
        if (empty($item)) {
            exit;
        }
        ...
        $goods = pdo_fetchall("SELECT g.id,g.credit, o.total,o.realprice FROM " . tablename('tapgo_tt_order_goods') .
            " o left join " . tablename('tapgo_tt_goods') . " g on o.goodsid=g.id " . " WHERE o.orderid=:orderid and o.uniacid=:uniacid", array(':orderid' => $item['id'], ':uniacid' => $item['uniacid']));
        ...
        pdo_update('tapgo_tt_order', array('refundstate' => 0, 'status' => -1, 'refundtime' => $time), array('id' => $item['id'], 'uniacid' => $item['uniacid']));
    ...
    public function batch_refund_notify_rc($id,$money)
    {
        ...
        pdo_update('tapgo_tt_member_log', array('status' => 3), array('id' => $id, 'uniacid' => $_W['uniacid']));
