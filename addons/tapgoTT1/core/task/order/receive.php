...
require("../../../../../addons/tapgo_ttv2/core/inc/plugin_model.php");
global $_W;
global $_GPC;
ignore_user_abort();
set_time_limit(0);
...
foreach( $sets as $set ) 
{
    ...
    $p = p("commission");
    $pcoupon = com("coupon");
    $orders = pdo_fetchall("select id,status,couponid,openid,isparent,sendtime,price,merchid,isverify,addressid,isvirtualsend,`virtual`,dispatchtype,city_express_state, wxapp_allow_subscribe from " . tablename("tapgo_tt_order") . " where uniacid=" . $_W["uniacid"] . " and (status=2 or  (status = 1 and `isverify` = 1 and `verifyendtime` <= unix_timestamp() and `verifyendtime` > 0))", array(  ), "id");
    if( !empty($orders) ) 
    {
        foreach( $orders as $orderid => $order ) 
        {
            ...
            $time = time();
            pdo_query("update " . tablename("tapgo_tt_order") . " set status=3,finishtime=:time where id=:orderid", array( ":time" => $time, ":orderid" => $orderid ));
            if( $order["isparent"] == 1 ) 
            {
                continue;
            }
            ...
            if( !empty($order["wxapp_allow_subscribe"]) ) 
            {
                ...
                {
                    if( strexists("sns_wa_", $order["openid"]) ) 
                    {
                        $openid = $order["openid"];
                    }
                    else
                    ...
                    $goods = pdo_fetchall("select og.goodsid,og.price,g.title,g.thumb,og.total,g.credit,og.optionid,og.optionname as optiontitle,g.isverify,g.storeids, og.realprice from " . tablename("tapgo_tt_order_goods") . " og " . " left join " . tablename("tapgo_tt_goods") . " g on g.id=og.goodsid " . " where og.orderid=:orderid ", array( ":orderid" => $orderid ));
                    $title = "";
                    foreach( $goods as $og ) 
                    {
                        if( !empty($title) ) 
...
function goodsReceive($order, $sysday = 0)
{
    ...
    if( $order["merchid"] == 0 ) 
    {
        $goods = pdo_fetchall("select og.goodsid, g.autoreceive from" . tablename("tapgo_tt_order_goods") . " og left join " . tablename("tapgo_tt_goods") . " g on g.id=og.goodsid where og.orderid=" . $order["id"]);
