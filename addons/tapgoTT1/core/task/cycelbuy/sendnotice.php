...
if (!(empty($order))) 
{
	foreach ($order as $k => $v ) 
	{
		...
		$cycel = pdo_fetchall('select * from ' . tablename('tapgo_tt_cycelbuy_periods') . ' where  orderid = ' . $v['id'] . ' and status = 0 and uniacid = ' . $_W['uniacid'] . ' order by receipttime asc limit 1');
