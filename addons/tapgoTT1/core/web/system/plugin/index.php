...
class Index_EweiShopV2Page extends SystemPage
{
	public function main()
	{
		...
		if ($_W['ispost']) {
			if (!empty($_GPC['displayorder'])) {
				foreach ($_GPC['displayorder'] as $id => $displayorder) {
					pdo_update('tapgo_tt_plugin', array('status' => $_GPC['status'][$id], 'displayorder' => $displayorder, 'name' => $_GPC['name'][$id], 'thumb' => $_GPC['thumb'][$id], 'desc' => $_GPC['desc'][$id]), array('id' => $id));
				}
		...
		$list = pdo_fetchall('select * from ' . tablename('tapgo_tt_plugin') . (' where 1 ' . $condition . ' order by displayorder asc'), $params);
		$bucket = array();
