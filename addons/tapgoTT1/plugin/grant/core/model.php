...
class GrantModel extends PluginModel
{
    public function checkplugin($identity)
    {
        ...
        if (false === in_array($identity,$plugins)  && false === in_array($identity,$coms)) {
            $plugin = pdo_fetch("SELECT * FROM " . tablename('tapgo_tt_system_plugingrant_log') . " WHERE uniacid = " . $uniacid . " and `identity` = '" . $identity . "'order by permendtime desc ");
    ...
    {
        $item = pdo_fetch("SELECT uniacid,pluginid,`month` FROM " . tablename('tapgo_tt_system_plugingrant_log') . " WHERE id = " . $id . " and isperm = 0  ");
        if (empty($item)) {
        ...
        pdo_update('tapgo_tt_system_plugingrant_log', $data, array('id' => $id));
    }
...
//		'seller_id'=>'yilianhudong@163.com',
            ...
            'notify_url' => $_W['siteroot'] . 'addons/tapgo_ttv2/payment/alipay/pluginverify.php',
