...
class MembercardModel extends PluginModel
{
	...
	public function get_Allcard($page = 1, $psize = 20)
	{
		...
		$sql = 'select * from ' . tablename('tapgo_tt_member_card') . (' where  ' . $condition . ' ORDER BY sort_order DESC,create_time DESC LIMIT ' . $limit);
		$list = pdo_fetchall($sql, $params);
		$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('tapgo_tt_member_card') . ('where 1 and ' . $condition), $params);
		...
		if (!empty($goodsids) && $goodsids != 'all') {
			foreach ($goodsids as $value) {
				...
				$temp_list = pdo_fetchall('SELECT c.*,h.openid,h.expire_time
                                FROM ' . tablename('tapgo_tt_member_card_history') . ' as h
                                left join ' . tablename('tapgo_tt_member_card') . (' as c on c.id = h.member_card_id
	...
	public function check_Hasget($cardid = 0, $openid = '')
	{
		...
		$record = pdo_fetch('SELECT * FROM ' . tablename('tapgo_tt_member_card_history') . ' 
	...
	public function check_month_point($cardid = 0, $openid = '')
	{
		...
		$params = array(':uniacid' => $_W['uniacid'], ':openid' => $openid, ':cardid' => $cardid);
		$card = pdo_fetch('SELECT id,validate FROM ' . tablename('tapgo_tt_member_card') . ' 
		...
		$month_point = pdo_fetch('SELECT * FROM ' . tablename('tapgo_tt_member_card_monthsend') . (' 
	...
	public function querycoupon($couponid = array())
	{
		...
		foreach ($couponid as $ck => $cv) {
			...
			$cpsql = 'SELECT id,catid,couponname,bgcolor,enough,coupontype,timestart,timeend,discount,deduct,backtype,backmoney,backcredit,backredpack,createtime,total,limitgoodcatetype,limitgoodtype,limitgoodids,limitgoodcatetype,limitgoodcateids FROM ' . tablename('tapgo_tt_coupon') . $where;
			...
			if ($row['limitgoodcatetype'] == 1 && $row['limitgoodcateids']) {
				...
				$cateid = $cateids[0];
				$catename = pdo_fetchcolumn('select `name`  from ' . tablename('tapgo_tt_category') . ' where uniacid=:uniacid  and id=:id ', array(':uniacid' => $_W['uniacid'], ':id' => $cateid));
			...
			if ($row['limitgoodtype'] == 1 && $row['limitgoodids']) {
				...
				$goodid = $goodids[0];
				$goodsname = pdo_fetchcolumn('select `title`  from ' . tablename('tapgo_tt_goods') . ' where uniacid=:uniacid  and id=:id ', array(':uniacid' => $_W['uniacid'], ':id' => $goodid));
	...
	public function getMemberCard($id)
	{
		global $_W;
		global $_GPC;
		$sql = 'SELECT * FROM ' . tablename('tapgo_tt_member_card') . ' WHERE uniacid = :uniacid AND id = :id ';
	...
	protected function buycard_send($cardid = 0, $openid = '')
	{
		...
		if ($card['is_card_points'] && $card['card_points']) {
			...
			pdo_insert('tapgo_tt_member_card_buysend', $send_log);
		}
	...
	public function payResult($orderno, $type, $app = false)
	{
		...
		$params = array(':tid' => $orderno, ':module' => 'tapgo_ttv2', ':uniacid' => $uniacid);
		$log = pdo_fetch($sql, $params);
		$order = pdo_fetch('select * from ' . tablename('tapgo_tt_member_card_order') . ' where  orderno =:orderno and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':orderno' => $orderno));
		...
		pdo_update('tapgo_tt_member_card_order', $record, array('orderno' => $orderno));
	...
	{
		global $_W;
		$order = pdo_fetch('SELECT * FROM  ' . tablename('tapgo_tt_member_card_order') . '  where id=:id and uniacid=:uniacid and status=1', array(':uniacid' => $_W['uniacid'], ':id' => $id));
		return $order;
	}
	...
	public function getMembercard_order_history($card_id)
	{
		global $_W;
		$time = time();
		$card_history = pdo_fetch($sql = 'SELECT * FROM  ' . tablename('tapgo_tt_member_card_history') . ' where member_card_id=:cardid and uniacid=:uniacid and openid=:openid and isdelete=0 and (expire_time=-1 or expire_time >' . $time . ') limit 1', array(':cardid' => $card_id, ':uniacid' => $_W['uniacid'], ':openid' => $_W['openid']));
		return $card_history;
	}
	...
	public function getUserMembercardOrderHistory($card_id, $openid)
	{
		global $_W;
		$time = time();
		$card_history = pdo_fetch($sql = 'SELECT * FROM  ' . tablename('tapgo_tt_member_card_history') . ' where member_card_id=:cardid and uniacid=:uniacid and openid=:openid and isdelete=0 and (expire_time=-1 or expire_time >' . $time . ') limit 1', array(':cardid' => $card_id, ':uniacid' => $_W['uniacid'], ':openid' => $openid));
		return $card_history;
	}
	...
	{
		global $_W;
		$card_history = pdo_fetch('SELECT * FROM  ' . tablename('tapgo_tt_member_card_history') . ' where member_card_id=:cardid and uniacid=:uniacid and openid=:openid and isdelete=0 and (expire_time=-1 or expire_time <' . time() . ')', array(':uniacid' => $_W['uniacid'], ':cardid' => $card_id, ':openid' => $_W['openid']));
		return $card_history;
	}
	...
	public function addMembercard($order_id)
	{
		...
		if ($order) {
			...
			if ($check['errno'] == 1 || $check['using'] == -1) {
				...
				pdo_insert('tapgo_tt_member_card_history', $data);
				pdo_query('UPDATE ' . tablename('tapgo_tt_member_card') . ' SET `sale_count` = `sale_count` + 1,`stock` = `stock` - 1 WHERE id=' . $order['member_card_id'] . '');
			}
			else {
				if ($check['using'] == 1) {
					...
					if ($card) {
						...
						pdo_update('tapgo_tt_member_card_history', $updata, array('id' => $card['id']));
						pdo_query('UPDATE ' . tablename('tapgo_tt_member_card') . ' SET `sale_count` = `sale_count` + 1,`stock` = `stock` - 1 WHERE id=' . $order['member_card_id'] . '');
					}
				}
			}
	...
	{
		global $_W;
		$buysend = pdo_fetch('SELECT * FROM ' . tablename('tapgo_tt_member_card_buysend') . (' WHERE  ' . $condition . ' limit 1'), $params);
		return $buysend;
	}
	...
	public function member_card_use_record($orderid, $cardid, $dec_price, $openid = '', $card_free_dispatch = false)
	{
		...
		$order = pdo_fetch('SELECT id,price FROM  ' . tablename('tapgo_tt_order') . '  where id=:id and uniacid=:uniacid', array(':uniacid' => $_W['uniacid'], ':id' => $orderid));
		...
		pdo_insert('tapgo_tt_member_card_uselog', $log_data);
	}
	...
	public function if_order_use_membercard($orderid)
	{
		...
		$use_log = pdo_fetch('SELECT * FROM  ' . tablename('tapgo_tt_member_card_uselog') . '  where order_id=:order_id and uniacid=:uniacid', array(':uniacid' => $_W['uniacid'], ':order_id' => $orderid));
