...
class DiypageModel extends PluginModel
{
    ...
    public function getPageList($type = "allpage", $condition = NULL, $page = 0)
    {
        ...
        $list = pdo_fetchall("select id, `name`, `type`, createtime, lastedittime, keyword,`data` from " . tablename("tapgo_tt_diypage") . " where merch=:merch " . $c . " and uniacid=:uniacid order by `type` desc, id desc " . $limit, array( ":merch" => intval($_W["merchid"]), ":uniacid" => $_W["uniacid"] ));
        $total = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename("tapgo_tt_diypage") . " where merch=:merch and uniacid=:uniacid " . $c, array( ":merch" => intval($_W["merchid"]), ":uniacid" => $_W["uniacid"] ));
    ...
    public function getPage($id, $mobile = false)
    {
        ...
        $page = pdo_fetch("select * from " . tablename("tapgo_tt_diypage") . $where . " limit 1 ", $params);
        if( !empty($page) ) 
        {
                        ...
                        if( $item["params"]["goodsdata"] == "0" ) 
                        {
                            if( !empty($item["data"]) && is_array($item["data"]) ) 
                            {
                                ...
                                if( !empty($goodsids) && is_array($goodsids) ) 
                                {
                                    ...
                                    if( $creditshop ) 
                                    {
                                        $goods = pdo_fetchall("select id, title, thumb, price as productprice, money as minprice, credit, total, showgroups, `type`, goodstype from " . tablename("tapgo_tt_creditshop_goods") . " where id in( " . $newgoodsids . " ) and status=1 and deleted=0 and uniacid=:uniacid order by displayorder desc ", array( ":uniacid" => $_W["uniacid"] ));
                                    }
                                    else
                                    {
                                        $goods = pdo_fetchall("select id,`presellend`,`preselltimeend`,isdiscount, isdiscount_time, title, subtitle, thumb, productprice, minprice, total,`type`,showlevels, showgroups,hascommission,nocommission,commission,commission1_rate,marketprice,commission1_pay,maxprice, bargain, merchid, sales, salesreal,ispresell,presellprice from " . tablename("tapgo_tt_goods") . " where id in( " . $newgoodsids . " ) and status=1 and deleted=0 and checked=0 and uniacid=:uniacid order by displayorder desc ", array( ":uniacid" => $_W["uniacid"] ));
                                    ...
                                    if( !empty($goods) && is_array($goods) ) 
                                    {
                                        ...
                                        foreach( $goods as $key => $value ) 
                                        {
                                            if( $value["nocommission"] == 0 ) 
                                            {
                                                ...
                                                {
                                                    $pricemax = array(  );
                                                    $options = pdo_fetchall("select * from " . tablename("tapgo_tt_goods_option") . " where goodsid=:goodsid and uniacid=:uniacid order by displayorder asc", array( ":goodsid" => $value["id"], ":uniacid" => $_W["uniacid"] ));
                                                ...
                                                if( $good["id"] == $goodsid ) 
                                                {
                                                    if( (empty($good["isdiscount"]) || $good["isdiscount_time"] < time()) && !$creditshop ) 
                                                    {
                                                        if( isset($options) && 0 < count($options) && $good["hasoption"] ) 
                                                        {
                                                            ...
                                                            $sql = "update " . tablename("tapgo_tt_goods") . " g set\n        g.minprice = (select min(marketprice) from " . tablename("tapgo_tt_goods_option") . " where goodsid = " . $good["id"] . "),\n        g.maxprice = (select max(marketprice) from " . tablename("tapgo_tt_goods_option") . " where goodsid = " . $good["id"] . ")\n        where g.id = " . $good["id"] . " and g.hasoption=1";
                                                            pdo_query($sql);
                                                        }
                                                        else
                                                        {
                                                            $sql = "update " . tablename("tapgo_tt_goods") . " set minprice = marketprice,maxprice = marketprice where id = " . $good["id"] . " and hasoption=0";
                                                            pdo_query($sql);
                                                        }

                                                        $goods_price = pdo_fetch("select title,minprice,maxprice,ispresell,presellend,preselltimeend,presellprice from " . tablename("tapgo_tt_goods") . " where id=:id and uniacid=:uniacid limit 1", array( ":id" => $good["id"], ":uniacid" => $_W["uniacid"] ));
                        ...
                        else
                        {
                            if( $item["params"]["goodsdata"] == "1" ) 
                            {
                                if( !empty($item["params"]["cateid"]) ) 
                                {
                                    ...
                                    {
                                        $category = pdo_fetch("select id,`name`, enabled from " . tablename("tapgo_tt_category") . " where id=:id and uniacid=:uniacid limit 1 ", array( ":id" => $item["params"]["cateid"], ":uniacid" => $_W["uniacid"] ));
                                    }
                                    else
                                    {
                                        $category = pdo_fetch("select id,`name`, enabled from " . tablename("tapgo_tt_creditshop_category") . " where id=:id and uniacid=:uniacid limit 1 ", array( ":id" => $item["params"]["cateid"], ":uniacid" => $_W["uniacid"] ));
                                    }
                            ...
                            else
                            {
                                ...
                                {
                                    $group = pdo_fetch("select id, `name` from " . tablename("tapgo_tt_goods_group") . " where id=:id and uniacid=:uniacid limit 1 ", array( ":id" => $item["params"]["groupid"], ":uniacid" => $_W["uniacid"] ));
                                    if( !empty($group) ) 
                                    {
                            ...
                            if( $item["id"] == "merchgroup" && p("merch") ) 
                            {
                                if( $item["params"]["merchdata"] == "0" ) 
                                {
                                    ...
                                    if( !empty($merchids) && is_array($merchids) ) 
                                    {
                                        ...
                                        $merchs = pdo_fetchall("select id, merchname, logo, status, `desc` from " . tablename("tapgo_tt_merch_user") . " where id in( " . $newmerchids . " ) and status=1 and uniacid=:uniacid order by  field (id," . $newmerchids . ")", array( ":uniacid" => $_W["uniacid"] ));
                                ...
                                else
                                {
                                    if( $item["params"]["merchdata"] == "1" ) 
                                    {
                                        if( !empty($item["params"]["cateid"]) ) 
                                        {
                                            $category = pdo_fetch("select id, `catename`, status from " . tablename("tapgo_tt_merch_category") . " where id=:id and uniacid=:uniacid limit 1 ", array( ":id" => $item["params"]["cateid"], ":uniacid" => $_W["uniacid"] ));
                                        }
                                        ...
                                        {
                                            $merchs = pdo_fetchall("select id, merchname, logo, status, `desc` from " . tablename("tapgo_tt_merch_user") . " where cateid=:cateid and status=1 and uniacid=:uniacid order by isrecommand desc ", array( ":uniacid" => $_W["uniacid"], ":cateid" => $item["params"]["cateid"] ));
                                            if( !empty($merchs) && is_array($merchs) ) 
                                            {
                                                $item["data"] = array(  );
                                                foreach( $merchs as $index => $merch ) 
                                    ...
                                    else
                                    {
                                        if( $item["params"]["merchdata"] == "2" ) 
                                        {
                                            ...
                                            {
                                                $group = pdo_fetch("select id, groupname, status from " . tablename("tapgo_tt_merch_group") . " where id=:id and uniacid=:uniacid limit 1 ", array( ":id" => $item["params"]["groupid"], ":uniacid" => $_W["uniacid"] ));
                                                if( !empty($group) ) 
                                                {
                                            ...
                                            {
                                                $merchs = pdo_fetchall("select id, merchname, logo, status, `desc` from " . tablename("tapgo_tt_merch_user") . " where groupid=:groupid and status=1 and uniacid=:uniacid order by isrecommand desc ", array( ":uniacid" => $_W["uniacid"], ":groupid" => $item["params"]["groupid"] ));
                                                if( !empty($merchs) && is_array($merchs) ) 
                                                {
                                                    $item["data"] = array(  );
                                                    foreach( $merchs as $index => $merch ) 
                                        ...
                                        else
                                        {
                                            ...
                                            {
                                                $merchs = pdo_fetchall("select id, merchname, logo, status, `desc` from " . tablename("tapgo_tt_merch_user") . " where isrecommand=1 and status=1 and uniacid=:uniacid order by isrecommand desc ", array( ":uniacid" => $_W["uniacid"] ));
                                                if( !empty($merchs) && is_array($merchs) ) 
                                                {
                                                    $item["data"] = array(  );
                                                        ...
                                                        if( $item["id"] == "coupon" ) 
                                                        {
                                                            if( !empty($item["data"]) && is_array($item["data"]) ) 
                                                            {
                                                                ...
                                                                if( !empty($couponids) && is_array($couponids) ) 
                                                                {
                                                                    ...
                                                                    $coupons = pdo_fetchall("select * from " . tablename("tapgo_tt_coupon") . " where id in( " . $newcouponids . " ) and uniacid=:uniacid", array( ":uniacid" => $_W["uniacid"] ), "id");
                ...
                foreach( $page["data"]["items"] as $itemid => &$item ) 
                {
                        ...
                        if( $item["params"]["goodsdata"] == "0" ) 
                        {
                            if( !empty($item["data"]) && is_array($item["data"]) ) 
                            {
                                ...
                                if( !empty($goodsids) && is_array($goodsids) ) 
                                {
                                    ...
                                    if( $creditshop ) 
                                    {
                                        $goods = pdo_fetchall("select id, showlevels, showgroups from " . tablename("tapgo_tt_creditshop_goods") . " where id in( " . $newgoodsids . " ) and status=1 and deleted=0 and uniacid=:uniacid order by displayorder desc ", array( ":uniacid" => $_W["uniacid"] ));
                                    }
                                    else
                                    {
                                        $goods = pdo_fetchall("select id, showlevels,hascommission,nocommission,commission,commission1_rate,marketprice,commission1_pay,maxprice,showgroups from " . tablename("tapgo_tt_goods") . " where id in( " . $newgoodsids . " ) and status=1 and deleted=0 and checked=0 and uniacid=:uniacid order by displayorder desc ", array( ":uniacid" => $_W["uniacid"] ));
                                    }
                        ...
                        else
                        {
                            if( $item["params"]["goodsdata"] == "1" ) 
                            {
                                ...
                                if( !empty($cateid) ) 
                                {
                                    ...
                                    {
                                        $goods = pdo_fetchall("select id, title, thumb, price as productprice, money as minprice, credit, total, showlevels, showgroups, `type`, goodstype from " . tablename("tapgo_tt_creditshop_goods") . " where cate=:cate and status=1 and deleted=0 and uniacid=:uniacid order by " . $orderby . " limit " . $limit, array( ":cate" => $cateid, ":uniacid" => $_W["uniacid"] ));
                                    }
                            ...
                            else
                            {
                                if( $item["params"]["goodsdata"] == "2" && empty($item["params"]["goodstype"]) ) 
                                {
                                    ...
                                    if( !empty($groupid) ) 
                                    {
                                        $group = pdo_fetch("SELECT * FROM " . tablename("tapgo_tt_goods_group") . " WHERE id=:id and uniacid=:uniacid limit 1 ", array( ":id" => $groupid, ":uniacid" => $_W["uniacid"] ));
                                    }
                                ...
                                else
                                {
                                    if( 2 < $item["params"]["goodsdata"] ) 
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            $goods = pdo_fetchall("select id, title, thumb, price as productprice, money as minprice, credit, total, showlevels,showgroups, `type`, goodstype from " . tablename("tapgo_tt_creditshop_goods") . " where 1 " . $condition . " order by " . $args["order"] . " limit " . $args["pagesize"], $params);
                                        }
                    ...
                    else
                    {
                        if( $item["id"] == "groupsgoods" ) 
                        {
                            ...
                            $params = array( ":uniacid" => $_W["uniacid"] );
                            $sql = "SELECT c.*,g.* FROM " . tablename("tapgo_tt_groups_goods") . " AS g\n                                LEFT JOIN " . tablename("tapgo_tt_groups_category") . " AS c ON g.category = c.id\n                                where  1 = 1 and " . $condition . " ORDER BY g.displayorder DESC,g.id DESC LIMIT " . $limit;
                        ...
                        else
                        {
                            if( $item["id"] == "notice" ) 
                            {
                                if( $item["params"]["noticedata"] == "0" ) 
                                {
                                    ...
                                    if( !empty($page["merch"]) ) 
                                    {
                                        $notices = pdo_fetchall("select id, title, link, thumb from " . tablename("tapgo_tt_merch_notice") . " where uniacid=:uniacid and merchid=:merchid and status=1 order by displayorder desc limit " . $limit, array( ":uniacid" => $_W["uniacid"], ":merchid" => intval($page["merch"]) ));
                                    }
                                    else
                                    {
                                        $notices = pdo_fetchall("select id, title, link, thumb from " . tablename("tapgo_tt_notice") . " where uniacid=:uniacid and iswxapp=0 and status=1 order by displayorder desc limit " . $limit, array( ":uniacid" => $_W["uniacid"] ));
                                    }
    ...
    public function savePage($id, $data, $update = true)
    {
        ...
        if( !empty($keyword) && $update ) 
        {
            ...
            if( !empty($result) ) 
            {
                if( $result["name"] != "tapgo_ttv2:diypage:" . $id ) 
                {
                    show_json(0, "关键字已存在！");
                }
        ...
        if( !empty($id) ) 
        {
            ...
            pdo_update("tapgo_tt_diypage", $diypage, array( "id" => $id, "uniacid" => $_W["uniacid"] ));
            $savetype = "update";
        }
        else
        {
            ...
            pdo_insert("tapgo_tt_diypage", $diypage);
            $id = pdo_insertid();
            $savetype = "insert";
        }
        ...
        if( !empty($keyword) && $update ) 
        {
            $rule = pdo_fetch("select * from " . tablename("rule") . " where uniacid=:uniacid and module=:module and name=:name  limit 1", array( ":uniacid" => $_W["uniacid"], ":module" => "tapgo_ttv2", ":name" => "tapgo_ttv2:diypage:" . $id ));
            if( !empty($rule) ) 
            {
            ...
            {
                $rule_data = array( "uniacid" => $_W["uniacid"], "name" => "tapgo_ttv2:diypage:" . $id, "module" => "tapgo_ttv2", "displayorder" => 0, "status" => 1 );
                pdo_insert("rule", $rule_data);
                $rid = pdo_insertid();
                $keyword_data = array( "uniacid" => $_W["uniacid"], "rid" => $rid, "module" => "tapgo_ttv2", "content" => $keyword, "type" => 1, "displayorder" => 0, "status" => 1 );
        ...
        if( $update ) 
        {
            $item = pdo_fetch("select id, type, name, keyword from " . tablename("tapgo_tt_diypage") . " where id=:id and uniacid=:uniacid limit 1", array( ":uniacid" => $_W["uniacid"], ":id" => $id ));
            if( $savetype == "update" ) 
            {
                if( $item["type"] == 1 ) 
                {
    ...
    public function delPage($id)
    {
        ...
        $items = pdo_fetchall("SELECT id,`name`,`type`,keyword FROM " . tablename("tapgo_tt_diypage") . " WHERE id in( " . $id . " ) and uniacid=:uniacid ", array( ":uniacid" => $_W["uniacid"] ));
        foreach( $items as $item ) 
        {
            pdo_delete("tapgo_tt_diypage", array( "id" => $item["id"], "uniacid" => $_W["uniacid"] ));
            if( $item["type"] == 1 ) 
            {
            ...
            $keyword = pdo_fetch("SELECT * FROM " . tablename("rule_keyword") . " WHERE content=:content and module=:module and uniacid=:uniacid limit 1 ", array( ":content" => $item["keyword"], ":module" => "tapgo_ttv2", ":uniacid" => $_W["uniacid"] ));
            if( !empty($keyword) ) 
            {
        ...
        $str = preg_replace_callback("/\"imgurl\"\\:\"([^\\'\" ]+)\"/", function($matches)
{
    ...
    if( !strexists($preg, "../addons/tapgo_ttv2") ) 
    {
...
, $str);
    ...
    public function saveTemp($temp)
    {
        ...
        pdo_insert("tapgo_tt_diypage_template", $temp);
        if( $temp["type"] == 1 ) 
        {
    ...
    public function verify($do, $pagetype)
    {
        ...
        if( $do == "add" ) 
        {
            ...
            if( !empty($tid) ) 
            {
                $template = pdo_fetch("select * from " . tablename("tapgo_tt_diypage_template") . " where id=:id and (uniacid=:uniacid or uniacid=0) limit 1", array( ":uniacid" => $_W["uniacid"], ":id" => $tid ));
    ...
    public function calculate($str = NULL, $pagetype = false)
    {
        ...
        if( strexists($str, "r=commission") ) 
        {
            ...
            if( $plugin_commissiom ) 
            {
                ...
                {
                    $this->commission["applycount"] = pdo_fetchcolumn("select count(id) from " . tablename("tapgo_tt_commission_apply") . " where mid=:mid and uniacid=:uniacid limit 1", array( ":uniacid" => $_W["uniacid"], ":mid" => $member["id"] ));
                }

                if( strexists($str, "r=commission.down") ) 
                {
                    $level1 = $level2 = $level3 = 0;
                    $level1 = pdo_fetchcolumn("select count(*) from " . tablename("tapgo_tt_member") . " where agentid=:agentid and uniacid=:uniacid limit 1", array( ":agentid" => $member["id"], ":uniacid" => $_W["uniacid"] ));
                    if( 2 <= $plugin_commissiom_set["level"] && 0 < count($member["level1_agentids"]) ) 
                    {
                        $level2 = pdo_fetchcolumn("select count(*) from " . tablename("tapgo_tt_member") . " where agentid in( " . implode(",", array_keys($member["level1_agentids"])) . ") and uniacid=:uniacid limit 1", array( ":uniacid" => $_W["uniacid"] ));
                    }

                    if( 3 <= $plugin_commissiom_set["level"] && 0 < count($member["level2_agentids"]) ) 
                    {
                        $level3 = pdo_fetchcolumn("select count(*) from " . tablename("tapgo_tt_member") . " where agentid in( " . implode(",", array_keys($member["level2_agentids"])) . ") and uniacid=:uniacid limit 1", array( ":uniacid" => $_W["uniacid"] ));
                    }
        ...
        if( strexists($str, "r=order") || strexists($str, "r=member.cart") || strexists($str, "r=coupon.my") ) 
        {
            ...
            if( strexists($str, "status=0") ) 
            {
                ...
                $ordernum["status0"] = pdo_fetchcolumn("select count(*) from " . tablename("tapgo_tt_order") . " where " . $condition . " and openid=:openid and status=0 and (isparent=1 or (isparent=0 and parentid=0)) and uniacid=:uniacid and istrade=0 and userdeleted=0 limit 1", $params);
            }
            ...
            if( strexists($str, "status=1") ) 
            {
                ...
                $ordernum["status1"] = pdo_fetchcolumn("select count(*) from " . tablename("tapgo_tt_order") . " where " . $condition . " and openid=:openid and uniacid=:uniacid limit 1", $params);
            }
            ...
            if( strexists($str, "status=2") ) 
            {
                ...
                $ordernum["status2"] = pdo_fetchcolumn("select count(*) from " . tablename("tapgo_tt_order") . " where " . $condition . " and openid=:openid and uniacid=:uniacid limit 1", $params);
            }
            ...
            if( strexists($str, "status=4") ) 
            {
                ...
                $ordernum["status4"] = pdo_fetchcolumn("select count(*) from " . tablename("tapgo_tt_order") . " where " . $condition . " and openid=:openid and uniacid=:uniacid limit 1", $params);
            }
            ...
            if( strexists($str, "r=member.cart") ) 
            {
                ...
                $ordernum["cartnum"] = pdo_fetchcolumn("select ifnull(sum(total),0) from " . tablename("tapgo_tt_member_cart") . " where " . $condition . " and uniacid=:uniacid and openid=:openid ", $params);
            }
            ...
            if( strexists($str, "r=member.favorite") ) 
            {
                ...
                $ordernum["favorite"] = pdo_fetchcolumn("select count(*) from " . tablename("tapgo_tt_member_favorite") . " where " . $condition . " and uniacid=:uniacid and openid=:openid ", $params);
            }

            if( strexists($str, "r=sale.coupon.my") ) 
            {
                $time = time();
                $sql = "select count(*) from " . tablename("tapgo_tt_coupon_data") . " d";
    ...
    public function url($str)
    {
        ...
        if( !empty($_W["openid"]) ) 
        {
            ...
            if( !empty($myid) ) 
            {
                $member = pdo_fetch("select id,isagent,status from" . tablename("tapgo_tt_member") . "where id=" . $myid);
...
, $str);
    ...
    {
        global $_W;
        $adv = pdo_fetch("SELECT * FROM" . tablename("tapgo_tt_diypage_plu") . "WHERE id=:id AND status=1 AND `type`=1 AND uniacid=:uniacid", array( ":id" => $id, ":uniacid" => $_W["uniacid"] ));
        if( empty($adv) ) 
        {
            return NULL;
        }
