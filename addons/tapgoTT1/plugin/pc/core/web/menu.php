...
class MenuController extends PluginWebPage
{
	...
	public function post()
	{
		...
		if ($isEdit) {
			$item = pdo_get('tapgo_tt_pc_menu', array('id' => $id));
		}
		...
		if ($_W['ispost']) {
			...
			if ($isEdit) {
				$execResult = pdo_update('tapgo_tt_pc_menu', $data, array('id' => $id));
			}
			else {
				$execResult = pdo_insert('tapgo_tt_pc_menu', $data);
			}
	...
	public function switchChange()
	{
		...
		$status = $_GPC['status'];
		$id = $_GPC['id'];
		pdo_update('tapgo_tt_pc_menu', array('status' => $status), array('id' => $id));
		show_json(1);
	}
	...
	public function delete()
	{
		...
		$r = pdo_query('delete from ' . tablename('tapgo_tt_pc_menu') . (' where id in (' . $ids . ')'));
	...
	public function getList($type)
	{
		...
		$list = pdo_fetchall('select *  from ' . tablename('tapgo_tt_pc_menu') . (' where 1 ' . $condition . ' ' . $searchCondition . ' order by create_time desc '));
		$total = pdo_fetchcolumn('select count(1) from ' . tablename('tapgo_tt_pc_menu') . (' where 1 ' . $condition . ' ' . $searchCondition));
