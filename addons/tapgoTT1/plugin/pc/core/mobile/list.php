...
class ListController extends PluginMobilePage
{
	public function main()
	{
		...
		if (isset($_GPC['group_id'])) {
			$args['ispc'] = 1;
			$goodsGroup = pdo_get('tapgo_tt_pc_goods', array('id' => $_GPC['group_id']));
			...
			if ($goodsGroup['goods_type'] == 0) {
				...
				if ($goodsGroup['goods_type'] == 2) {
					$list = pdo_get('tapgo_tt_goods_group', array('id' => $goodsGroup['goods_info']));
		...
		$cateinfo = pdo_get('tapgo_tt_category', array('id' => $_GPC['cate'], 'uniacid' => $_W['uniacid'], 'enabled' => 1));
		...
		if ($cateinfo) {
			...
			$category['downcate'] = pdo_getall('tapgo_tt_category', array('level' => 1, 'uniacid' => $_W['uniacid'], 'enabled' => 1));
		}
	...
	public function getallcate($info, $shoplevel)
	{
		...
		if ($shoplevel == 1) {
			$cate['upcate'][] = $info;
			$cate['downcate'] = pdo_getall('tapgo_tt_category', array('level' => $shoplevel, 'uniacid' => $_W['uniacid'], 'enabled' => 1), array('id', 'name', 'parentid'));
		}

		if ($shoplevel == 2) {
			if ($info['level'] == 1) {
				$cate['upcate'][] = $info;
				$cate['downcate'] = pdo_getall('tapgo_tt_category', array('parentid' => $info['id']), array('id', 'name', 'parentid'));
			}
			else if ($info['level'] == 2) {
				$cate['upcate'][] = pdo_get('tapgo_tt_category', array('id' => $info['parentid']), array('id', 'name', 'parentid'));
				$cate['upcate'][] = $info;
				$cate['downcate'] = pdo_getall('tapgo_tt_category', array('parentid' => $info['id'], 'enabled' => 1), array('id', 'name', 'parentid'));
			}
			else {
				$partent = pdo_get('tapgo_tt_category', array('id' => $info['parentid']), array('id', 'name', 'parentid'));

				if (0 < $partent['parentid']) {
					$cate['upcate'][] = pdo_get('tapgo_tt_category', array('id' => $partent['parentid']), array('id', 'name', 'parentid'));
				}

				$cate['upcate'][] = $partent;
				$cate['upcate'][] = $info;
				$cate['downcate'] = pdo_getall('tapgo_tt_category', array('parentid' => $info['id'], 'uniacid' => $_W['uniacid'], 'enabled' => 1), array('id', 'name', 'parentid'));
			}
		}
