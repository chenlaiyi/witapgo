...
class MemberController extends PluginMobilePage
{
	...
	public function memberData($tag = 'member')
	{
		...
		if (0 < $userinfo['level']) {
			$levelname = pdo_get('tapgo_tt_member_level', array('id' => $userinfo['level'], 'enabled' => 1), 'levelname');
	...
	public function getOrderList()
	{
		global $_W;
		p('pc')->checkLogin();
		$orders = pdo_fetchall('select o.price,o.ordersn,o.paytime,o.createtime,o.paytype,o.addressid,o.isverify,o.sendtype,o.status,o.id,og.goodsid,g.thumb from ' . tablename('tapgo_tt_order') . ' as o
				left join ' . tablename('tapgo_tt_order_goods') . ' as og on og.orderid = o.id
				left join ' . tablename('tapgo_tt_goods') . ' as g on g.id = og.goodsid
	...
	public function getMyFavorite()
	{
		...
		$sql = 'SELECT COUNT(*) FROM ' . tablename('tapgo_tt_member_favorite') . (' f where 1 ' . $condition);
		$total = pdo_fetchcolumn($sql, $params);
		$list = array();

		if (!empty($total)) {
			$sql = 'SELECT f.id,f.goodsid,g.title,g.thumb,g.marketprice,g.productprice,g.merchid,g.minprice,g.maxprice FROM ' . tablename('tapgo_tt_member_favorite') . ' f ' . ' left join ' . tablename('tapgo_tt_goods') . ' g on f.goodsid = g.id ' . ' where 1 ' . $condition . ' ORDER BY `id` DESC LIMIT 3 ';
	...
	public function getMyHistory()
	{
		...
		$condition = ' and f.uniacid = :uniacid and f.openid=:openid and f.deleted=0';
		$params = array(':uniacid' => $_W['uniacid'], ':openid' => $_W['openid']);
		$sql = 'SELECT COUNT(*) FROM ' . tablename('tapgo_tt_member_history') . (' f where 1 ' . $condition);
		$total = pdo_fetchcolumn($sql, $params);
		$sql = 'SELECT f.id,f.goodsid,g.title,g.thumb,g.marketprice,g.productprice,f.createtime,g.merchid FROM ' . tablename('tapgo_tt_member_history') . ' f ' . ' left join ' . tablename('tapgo_tt_goods') . ' g on f.goodsid = g.id ' . ' where 1 ' . $condition . ' ORDER BY `id` DESC LIMIT 3 ';
		$list = pdo_fetchall($sql, $params);
	...
	public function loginin()
	{
		global $_W;
		global $_GPC;
		if (is_weixin() || !empty($_GPC['__tapgo_ttv2_member_session_' . $_W['uniacid']])) {
		...
		if ($_W['ispost']) {
			...
			$pwd = trim($_GPC['pwd']);
			$member = pdo_fetch('select id,openid,mobile,pwd,salt from ' . tablename('tapgo_tt_member') . ' where mobile=:mobile and mobileverify=1 and uniacid=:uniacid limit 1', array(':mobile' => $mobile, ':uniacid' => $_W['uniacid']));
	...
	protected function rf($type)
	{
		...
		if ($_W['ispost']) {
			...
			$key = '__tapgo_ttv2_member_verifycodesession_' . $_W['uniacid'] . '_' . $mobile;
	...
	public function cartSelect()
	{
		...
		foreach ($id as $value) {
			if (!empty($value)) {
				$data = pdo_fetch('select id,goodsid,optionid, total from ' . tablename('tapgo_tt_member_cart') . ' ' . ' where id=:id and uniacid=:uniacid and openid=:openid limit 1 ', array(':id' => $value, ':uniacid' => $_W['uniacid'], ':openid' => $_W['openid']));

				if (!empty($data)) {
					pdo_update('tapgo_tt_member_cart', array('selected' => $select), array('id' => $value, 'uniacid' => $_W['uniacid']));
				}
			}
			else {
