...
class Index_EweiShopV2Page extends PluginWebPage {
	...
	function main() {
        ...
        $list = pdo_fetchall("SELECT * FROM " . tablename('tapgo_tt_lottery') . " WHERE 1 {$condition} ORDER BY addtime desc LIMIT " . ($pindex - 1) * $psize . ',' . $psize, $params);

        $total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('tapgo_tt_lottery') . " where 1 {$condition} ", $params);
        $pager = pagination2($total, $pindex, $psize);
        foreach ($list as $key=>$value){
            $member_total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('tapgo_tt_lottery_join') . " where lottery_id=:lottery_id ", array(":lottery_id"=>$value['lottery_id']));
    ...
    protected function post() {
        ...
        if($id){
            $item = pdo_fetch("SELECT * FROM " . tablename('tapgo_tt_lottery') . " WHERE lottery_id =:lottery_id and uniacid=:uniacid limit 1", array(':lottery_id' => $id, ':uniacid' => $_W['uniacid']));
        ...
        if ($_W['ispost']) {
            ...
            if($id){
                //更新
                $res = pdo_update('tapgo_tt_lottery',$data,array('lottery_id' => $id, 'uniacid' => $_W['uniacid']));
                ...
                //添加
                $res = pdo_insert('tapgo_tt_lottery',$data);
                $id = pdo_insertid();
                if($res){
    ...
    function delete() {
        ...
        $posters = pdo_fetchall("SELECT lottery_id,lottery_title FROM " . tablename('tapgo_tt_lottery') . " WHERE lottery_id in ( $id ) and uniacid=" . $_W['uniacid']);
        foreach ($posters as $poster) {
            pdo_update('tapgo_tt_lottery',array('is_delete'=>1),array('lottery_id' => $poster['lottery_id'], 'uniacid' => $_W['uniacid']));
    ...
    function taskTypeIsExist($task_type, $id) {
        global $_W;
        $sql = "SELECT COUNT(1) FROM ".tablename('tapgo_tt_lottery')." WHERE uniacid = :uniacid AND task_type = :task_type AND is_delete = 0";
        $res = pdo_fetchcolumn($sql, array(':uniacid'=>$_W['uniacid'], ':task_type'=>$task_type));
        $res = intval($res);
        if($res === 1){
            $sql = "SELECT COUNT(1) FROM ".tablename('tapgo_tt_lottery')." WHERE uniacid = :uniacid AND task_type = :task_type AND lottery_id = :id AND is_delete = 0";
