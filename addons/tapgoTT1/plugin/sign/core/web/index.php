...
class Index_EweiShopV2Page extends PluginWebPage {
    ...
    public function rule() {
        ...
        if ($_W['ispost']){
            ...
            if(empty($set)){
                ...
                pdo_insert('tapgo_tt_sign_set', $data);
            }else{
                pdo_update('tapgo_tt_sign_set', $data, array('id'=>$set['id'], 'uniacid'=>$_W['uniacid']));
            }
    ...
    public function records() {
        ...
        $list = pdo_fetchall('select r.*, m.nickname, m.id as mid, m.avatar from ' . tablename('tapgo_tt_sign_records') . "r left join " . tablename('tapgo_tt_member'). "m on r.openid=m.openid " .  $condition .' order by r.time desc, r.id desc limit ' . ($pindex - 1) * $psize . ',' . $psize, array(':uniacid' => $_W['uniacid']));
        $total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('tapgo_tt_sign_records') . "r left join " .tablename('tapgo_tt_member'). "m on r.openid=m.openid " . $condition);
    ...
    public function set() {
        ...
        if($_W['ispost']){
            ...
            if(!empty($keyword)){
                if($keyword['name']!='tapgo_ttv2:sign'){
                    show_json(0, '关键字已存在!');
                }
            }
            // 处理 关键字
            $rule = pdo_fetch("select * from " . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'cover', ':name' => "tapgo_ttv2:sign"));
            if(!empty($rule)){
            ...
            // 处理 rule 表
            $rule_data = array('uniacid' => $_W['uniacid'],'name' => 'tapgo_ttv2:sign','module' => 'cover','displayorder' => 0,'status' => 1);
            if(empty($rule)){
            ...
            // 处理 cover_reply 表
            $cover_data = array('uniacid' => $_W['uniacid'],'rid'=>$rid,'module' => 'tapgo_ttv2','title' => $data['title'],'description' =>$data['desc'],'thumb' => $data['thumb'],'url' => mobileUrl('sign'));
            if(empty($cover)){
