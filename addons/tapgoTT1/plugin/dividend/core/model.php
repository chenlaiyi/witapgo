...
class DividendModel extends PluginModel
{
    ...
    public function getInfo($openid, $options = NULL)
    {
        ...
        $groupscount = 0;
        $groupscounts = pdo_fetch("select count(id) as counts from " . tablename("tapgo_tt_member") . " where headsid = :headsid and uniacid = :uniacid", array( ":headsid" => $member["id"], ":uniacid" => $_W["uniacid"] ));
        ...
        {
            $order_all = pdo_fetchall("select id,headsid,price,deductcredit2,dispatchprice,dividend from " . tablename("tapgo_tt_order") . " where 1 and headsid = :headsid and status >= 0 and uniacid = :uniacid", array( ":headsid" => $member["id"], ":uniacid" => $_W["uniacid"] ));
            if( !empty($order_all) ) 
            {
        ...
        {
            $divedend_order_ok = pdo_fetchall("select id,headsid,price,deductcredit2,dispatchprice,dividend,finishtime from " . tablename("tapgo_tt_order") . " where 1 and headsid = :headsid and status >= 3 and dividend_status=0 and (" . $time . "-finishtime > " . $day_times . ") and uniacid = :uniacid", array( ":headsid" => $member["id"], ":uniacid" => $_W["uniacid"] ));
        ...
        {
            $check_apply = pdo_fetchall("select id,orderids,dividend from " . tablename("tapgo_tt_dividend_apply") . " where mid = :mid and status = 2 and uniacid = :uniacid", array( ":mid" => $member["id"], ":uniacid" => $_W["uniacid"] ));
        ...
        if( in_array("check", $options) ) 
        {
            $check_apply = pdo_fetchall("select id,orderids,dividend from " . tablename("tapgo_tt_dividend_apply") . " where mid = :mid and status = 1 and uniacid = :uniacid", array( ":mid" => $member["id"], ":uniacid" => $_W["uniacid"] ));
            if( !empty($check_apply) ) 
            {
                foreach( $check_apply as $k => $v ) 
                {
        ...
        {
            $check_apply_pay = pdo_fetchall("select id,orderids,dividend from " . tablename("tapgo_tt_dividend_apply") . " where mid = :mid and status = 3 and uniacid = :uniacid", array( ":mid" => $member["id"], ":uniacid" => $_W["uniacid"] ));
        ...
        {
            $check_apply_lock = pdo_fetchall("select id,headsid,price,deductcredit2,dispatchprice,dividend,finishtime from " . tablename("tapgo_tt_order") . " where 1 and headsid = :headsid and status >= 3 and dividend_status=0 and (" . $time . "-finishtime <= " . $day_times . ") and uniacid = :uniacid", array( ":headsid" => $member["id"], ":uniacid" => $_W["uniacid"] ));
        ...
        {
            $divedend_order_wait = pdo_fetchall("select id,headsid,price,deductcredit2,dispatchprice,dividend,finishtime from " . tablename("tapgo_tt_order") . " where 1 and headsid = :headsid and status = 2 and (" . $time . "-finishtime > " . $day_times . ") and uniacid = :uniacid", array( ":headsid" => $member["id"], ":uniacid" => $_W["uniacid"] ));
        ...
        {
            $check_apply_fail = pdo_fetchall("select id,orderids,dividend from " . tablename("tapgo_tt_dividend_apply") . " where mid = :mid and status = -1 and uniacid = :uniacid", array( ":mid" => $member["id"], ":uniacid" => $_W["uniacid"] ));
        ...
        {
            $order0 = pdo_fetch("select sum(price) as ordermoney,count(distinct id) as ordercount from " . tablename("tapgo_tt_order") . " where headsid = :headsid and status >=0 and uniacid = :uniacid", array( ":headsid" => $member["id"], ":uniacid" => $_W["uniacid"] ));
            if( !empty($order0) ) 
            {
        ...
        {
            $order = pdo_fetch("select sum(price) as ordermoney,count(distinct id) as ordercount from " . tablename("tapgo_tt_order") . " where headsid = :headsid and status >=1 and uniacid = :uniacid", array( ":headsid" => $member["id"], ":uniacid" => $_W["uniacid"] ));
            if( !empty($order) ) 
            {
        ...
        {
            $order3 = pdo_fetch("select sum(price) as ordermoney,count(distinct id) as ordercount from " . tablename("tapgo_tt_order") . " where headsid = :headsid and status >=3 and uniacid = :uniacid", array( ":headsid" => $member["id"], ":uniacid" => $_W["uniacid"] ));
            if( !empty($order3) ) 
            {
    ...
    public function getHeadsDownNum($openid = NULL)
    {
        ...
        $itemscount = 0;
        $count = (int) pdo_fetchcolumn("select count(id) from " . tablename("tapgo_tt_member") . " where headsid=:agentid and headsstatus = 0 and  and uniacid=:uniacid limit 1", array( ":headsid" => $member["id"], ":uniacid" => $_W["uniacid"] ));
    ...
    public function checkOrderConfirm($orderid = "0")
    {
        ...
        $order = pdo_fetch("select id,openid,ordersn,goodsprice,agentid,paytime,officcode,dispatchprice,deductcredit2,price,deductcredit2 from " . tablename("tapgo_tt_order") . " where id=:id and status>=0 and uniacid=:uniacid limit 1", array( ":id" => $orderid, ":uniacid" => $_W["uniacid"] ));
        if( empty($order) ) 
        {
            return NULL;
        }
        ...
        pdo_update("tapgo_tt_order", array( "headsid" => $member["headsid"], "dividend" => iserializer($divedend) ), array( "id" => $orderid ));
        return true;
    }
    ...
    public function checkOrderPay($orderid = "0")
    {
        ...
        $order = pdo_fetch("select id,openid,ordersn,goodsprice,agentid,paytime from " . tablename("tapgo_tt_order") . " where id=:id and status>=1 and uniacid=:uniacid limit 1", array( ":id" => $orderid, ":uniacid" => $_W["uniacid"] ));
        if( empty($order) ) 
        {
            return NULL;
        }
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    if( $message_type == TM_DIVIDEND_DOWNLINE_BECOME && empty($usernotice["dividend_downline_become"]) ) 
                                    {
                                        ...
                                        $heads = pdo_fetch("select id,openid from " . tablename("tapgo_tt_member") . " where id=:id", array( ":id" => $member["headsid"] ));
    ...
    public function update_headsid($memberid, $agentid)
    {
        global $_W;
        global $_GPC;
        $agent = pdo_fetch("select id,isheads,headsid,headsstatus from " . tablename("tapgo_tt_member") . " where id = :id", array( ":id" => $agentid ));
        if( !empty($agent["isheads"]) && !empty($agent["headsstatus"]) ) 
        {
            pdo_update("tapgo_tt_member", array( "headsid" => $agentid ), array( "id" => $memberid, "uniacid" => $_W["uniacid"] ));
        }
        else
        {
            if( empty($agent["isheads"]) && !empty($agent["headsid"]) ) 
            {
                pdo_update("tapgo_tt_member", array( "headsid" => $agent["headsid"] ), array( "id" => $memberid, "uniacid" => $_W["uniacid"] ));
            }
