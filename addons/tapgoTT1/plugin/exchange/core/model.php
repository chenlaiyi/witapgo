	...
	public function sendMessage($coupon, $send_total, $member, $account = NULL)
	{
		...
		if (!empty($articles)) {
			...
			if (is_error($resp)) {
				...
				if (is_error($ret)) {
					$info = pdo_fetch('SELECT coupon_templateid FROM ' . tablename('tapgo_tt_exchange_setting') . ' WHERE uniacid = :uniacid ', array(':uniacid' => $_W['uniacid']));
...
' . $_W['shopset']['shop']['name'] . '感谢您的支持，如有疑问请联系在线客服。', 'color' => '#000000')
	...
	public function sendRedpacket($key)
	{
		...
		if (!is_error($result)) {
			pdo_update('tapgo_tt_exchange_code', array('status' => 2), array('key' => $key, 'uniacid' => $_W['uniacid'], 'status' => 1));
	...
	{
		global $_W;
		$rule = pdo_fetch('select * from ' . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'tapgo_ttv2', ':name' => 'tapgo_ttv2:exchange:' . $id));
		$keyword = m('common')->keyExist($koulingstart);
		if (!empty($keyword) && $keyword['name'] != 'tapgo_ttv2:exchange:' . $id) {
			show_json(0, '关键字已存在!');
		}
		else {
			if (!empty($rule)) {
			...
			$rule_data = array('uniacid' => $_W['uniacid'], 'name' => 'tapgo_ttv2:exchange:' . $id, 'module' => 'tapgo_ttv2', 'displayorder' => 0, 'status' => $status);
			pdo_insert('rule', $rule_data);
			$rid = pdo_insertid();
			$keyword_data = array('uniacid' => $_W['uniacid'], 'rid' => $rid, 'module' => 'tapgo_ttv2', 'content' => $koulingstart, 'type' => 1, 'displayorder' => 0, 'status' => $status);
	...
	public function redKeyword($key)
	{
		...
		$key = pdo_fetch('SELECT * FROM ' . tablename('tapgo_tt_exchange_group') . ' WHERE uniacid = :uniacid AND koulingstart = :koulingstart AND kouling = 1', array(':koulingstart' => $key, ':uniacid' => $_W['uniacid']));
	...
	{
		global $_W;
		$sql = 'UPDATE ' . tablename('tapgo_tt_exchange_code') . ' SET repeatcount = repeatcount - 1 WHERE repeatcount >1 AND uniacid = :uniacid AND `key` = :key';
	...
	{
		global $_W;
		$logsql = 'SELECT COUNT(*) FROM ' . tablename('tapgo_tt_exchange_record') . ' WHERE `key` = :code AND uniacid = :uniacid AND openid = :openid';
	...
	{
		global $_W;
		$no_qrimg = pdo_fetchcolumn('select no_qrimg from ' . tablename('tapgo_tt_exchange_setting') . ' where uniacid = :uniacid', array(':uniacid' => $_W['uniacid']));
