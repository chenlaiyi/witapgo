...
class GoodscircleModel extends PluginModel
{
    ...
    private function getOrder($openid = '',$order_id = 0,$update = false){
        ...
        $sql = "select {$field} from " . tablename('tapgo_tt_order') . " where {$condition} limit 1";
        ...
        $ordergoods_sql = "select og.orderid,og.optionname,og.goodsid,og.total,og.realprice,og.price,og.oldprice,og.optionid,og.expresscom,og.expresssn,og.express,og.sendtime,g.thumb,g.title,g.description,g.subtitle,g.marketprice,g.productprice,g.cates from " . tablename('tapgo_tt_order_goods') . " as og 
            left join ". tablename('tapgo_tt_goods') ." as g on g.id=og.goodsid and g.uniacid=og.uniacid
            where {$og_condition}";
    ...
    private function getHistoryOrders($openid = ''){
        ...
        $log_sql = "select * from " . tablename('tapgo_tt_goodscircle_log') . " where {$log_condition} limit 1";
        ...
        $sql = "select {$field} from ".tablename('tapgo_tt_order')." where {$condition} order by createtime desc limit 0,10";
        ...
        $ordergoods_sql = "select og.orderid,og.optionname,og.goodsid,og.total,og.realprice,og.price,og.oldprice,og.optionid,g.thumb,g.title,g.description,g.subtitle,g.marketprice,g.productprice,g.cates from " . tablename('tapgo_tt_order_goods') . " as og 
            left join ". tablename('tapgo_tt_goods') ." as g on g.id=og.goodsid and g.uniacid=og.uniacid
            where {$og_condition}";
    ...
    private function getCartGoods($openid,$cart_id,$goods_id){
        ...
        $sql = "select {$field} from " . tablename('tapgo_tt_member_cart') . " as cart
            left join " . tablename('tapgo_tt_goods') . " as g on cart.goodsid=g.id
            left join " . tablename('tapgo_tt_goods_option') . " as op on cart.optionid=op.id
    ...
    public function getShopGoods($goods_id,$detail = false){
        ...
        $sql = "select {$field} from " . tablename('tapgo_tt_goods') . " where {$condition} limit 1";
        $goods = pdo_fetch($sql, $params);
        if(empty($goods)){
            return array();
        }
    ...
    private function getGoodsCate($cate_ids = ''){
        ...
        foreach ($cate_ids as $key=>$cid){
            $cate_name = pdo_fetchcolumn('select `name` from ' . tablename('tapgo_tt_category') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $cid, ':uniacid' => $_W['uniacid']));
            if($cate_name){
    ...
    private function wlog($response,$openid = '',$op_type,$op_id = 0){
        ...
        return pdo_insert('tapgo_tt_goodscircle_log', $data);
    }
    ...
    private function rlog($openid = '',$op_type,$op_id = 0){
        ...
        $sql = "select * from " . tablename('tapgo_tt_goodscircle_log') . " where {$condition} limit 1";
