...
class QuickModel extends PluginModel
{
    ...
    public function update($data, $page = array()) {
        ...
        if(!empty($goodsids)){
            ...
            $allGoods = pdo_fetchall("SELECT id, title, subtitle, minprice, total, sales FROM". tablename("tapgo_tt_goods")." WHERE uniacid=:uniacid AND id in({$goodsids}) AND `deleted`=0 AND `status`=1", array(":uniacid"=>$_W['uniacid']), "id");
        }
        // 读取分类数据
        $cateids = array_filter($cateids);
        if(!empty($cateids)){
            $cateids = implode(",", $cateids);
            $allCates = pdo_fetchall("SELECT * FROM". tablename("tapgo_tt_category")." WHERE uniacid=:uniacid AND id in({$cateids}) AND enabled=1", array(':uniacid'=>$_W['uniacid']), "id");
        }
        // 读取分组数据
        $groupids = array_filter($groupids);
        if(!empty($groupids)){
            $groupids = implode(",", $groupids);
            $allGroups = pdo_fetchall("SELECT * FROM". tablename("tapgo_tt_goods_group")." WHERE uniacid=:uniacid AND id in({$groupids}) AND enabled=1", array(':uniacid'=>$_W['uniacid']), "id");
        }
        foreach ($data['datas'] as $index=>&$item){
            if($item['datatype']==0){
                if(!empty($item['data'])){
    ...
    public function mobile($data, $merchid=0){
        ...
        if($data['template']==0){
            ...
            if ($data['showadv']==1){
                if($merchid>0){
                    $returnData['advs'] = pdo_fetchall("SELECT * FROM".tablename("tapgo_tt_quick_adv")."WHERE uniacid=:uniacid AND merchid=:merchid AND enabled=1", array(":uniacid"=>$_W['uniacid'], ":merchid"=>$merchid));
                }else{
                    $returnData['advs'] = pdo_fetchall("SELECT * FROM".tablename("tapgo_tt_quick_adv")."WHERE uniacid=:uniacid AND enabled=1", array(":uniacid"=>$_W['uniacid']));
                }
            }
            ...
            if(!empty($data['datas'])){
                foreach ($data['datas'] as $index=>$d){
                    ...
                    if($d['datatype']==2 && !empty($d['groupid'])){
                        $group = pdo_fetch('select * from ' . tablename('tapgo_tt_goods_group') . ' where id=:id and uniacid=:uniacid and enabled=1 limit 1 ', array(':id'=>$d['groupid'], ':uniacid' => $_W['uniacid']));
            ...
            if($returnData['style']['notice']==1){
                ...
                if($merchid>0){
                    $returnData['notices'] = pdo_fetchall("SELECT id, title FROM".tablename("tapgo_tt_merch_notice")."WHERE uniacid=:uniacid AND status=1 AND merchid=:merchid LIMIT ".$limit, array(":uniacid"=>$_W['uniacid'], ":merchid"=>$merchid));
                }else{
                    $returnData['notices'] = pdo_fetchall("SELECT id, title FROM".tablename("tapgo_tt_notice")."WHERE uniacid=:uniacid AND status=1 AND iswxapp=0 LIMIT ".$limit, array(":uniacid"=>$_W['uniacid']));
                }
            }
    ...
    public function getQuick($id) {
        ...
        return pdo_fetch("SELECT * FROM".tablename("tapgo_tt_quick")."WHERE id=:id AND uniacid=:uniacid AND status=1 LIMIT 1", array(":id"=>$id, ":uniacid"=>$_W['uniacid']));
    }
    ...
    public function getPageList($merch=false, $type=0, $condition = '', $params = array()) {
        ...
        return pdo_fetchall("SELECT id, title, status FROM". tablename("tapgo_tt_quick")."WHERE ".$condition." ORDER BY createtime DESC", $params);
    }
    ...
    public function getList($args){
        ...
        if(!empty($list) && is_array($list)){
            foreach ($list as $i=>&$g){
                ...
                //总购买量
                if ($g['usermaxbuy'] > 0) {
                    $order_goodscount = pdo_fetchcolumn('select ifnull(sum(og.total),0)  from ' . tablename('tapgo_tt_order_goods') . ' og ' . ' left join ' . tablename('tapgo_tt_order') . ' o on og.orderid=o.id '
...
//               $g['cannotbuy'] = empty($total) ?'该商品已售罄':'';
                ...
                if (p('bargain') && !empty($g['bargain']) && empty($g['gotodetail'])) {
                    $bargain = pdo_fetch("SELECT * FROM " . tablename('tapgo_tt_bargain_goods') . " WHERE id = :id AND unix_timestamp(start_time)<" . time() . " AND unix_timestamp(end_time)>" . time() . " AND status = 0", array(':id' => $g['bargain']));
                    if($bargain){
                        $g['gotodetail'] = 1;
                        $g['isbargain'] = 1;
                    }
    ...
    public function getCart($pageid, $json=true) {
        ...
        $tablename = empty($pageid)?"tapgo_tt_member_cart":"tapgo_tt_quick_cart";
        if(!empty($pageid)){
