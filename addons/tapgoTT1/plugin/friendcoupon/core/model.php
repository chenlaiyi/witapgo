...
class FriendcouponModel extends PluginModel
{
    ...
    {
        global $_W;
        return pdo_fetch("select * from " . tablename("tapgo_tt_coupon_data") . " where uniacid = :uniacid and openid = :openid and friendcouponid = :friendcoupoinid", array(":uniacid" => $_W["uniacid"], ":openid" => $openid, ":friendcouponid" => $friendcouponid));
    }
    /**
     * 获取活动列表
     */
    ...
    {
        global $_W;
        return pdo_fetchall("select * from " . tablename("tapgo_tt_friendcoupon") . " where uniacid = :uniacid and deleted = :deleted order by create_time desc", array(":uniacid" => $_W["uniacid"], ":deleted" => 0));
    }
    ...
    {
        global $_W;
        return pdo_fetch("select * from " . tablename("tapgo_tt_friendcoupon_data") . " where uniacid = :uniacid and openid = :openid and activity_id = :activity_id", array(":openid" => $openid, ":uniacid" => $_W["uniacid"], ":activity_id" => $activity_id));
    }
    /**
     * 获取当前用户参加的活动的信息
     * @param $openid
    ...
    {
        global $_W;
        return pdo_fetchall("select * from " . tablename("tapgo_tt_friendcoupon_data") . " where uniacid = :uniacid and activity_id = :activity_id and headerid = :headerid", array(":headerid" => $headerid, ":activity_id" => $activity_id, ":uniacid" => $_W["uniacid"]));
    }
    /**
     * 获取活动,不传入id返回所有活动
     * @param int $id
    ...
    {
        global $_W;
        $table = tablename("tapgo_tt_friendcoupon");
        if (is_null($id)) {
    ...
    {
        global $_W;
        return pdo_fetchall("select * from " . tablename("tapgo_tt_friendcoupon_data") . " where uniacid = :uniacid and headerid = :headerid and activity_id = :activity_id and openid <> ''", array(":uniacid" => $_W["uniacid"], ":headerid" => $headerid, ":activity_id" => $activity_id));
    }
    ...
    {
        global $_W;
        if (strstr($openid, "sns_wa_")) {
            $openid = substr($openid, "7");
            return pdo_fetch("select * from " . tablename("tapgo_tt_member") . " where uniacid = :uniacid and openid_wa = :openid", array(":uniacid" => $_W["uniacid"], ":openid" => $openid));
        }
        return pdo_fetch("select * from " . tablename("tapgo_tt_member") . " where uniacid = :uniacid and openid = :openid", array(":uniacid" => $_W["uniacid"], ":openid" => $openid));
    }
    public function getmemberwx($openid_wx)
    {
        global $_W;
        return pdo_fetch("select * from " . tablename("tapgo_tt_member") . " where uniacid = :uniacid and openid_wa = :openid_wa", array(":uniacid" => $_W["uniacid"], ":openid_wa" => $openid_wx));
    }
    public function getTakePartInPeopleData($activity_id, $headerid)
    {
        global $_W;
        $r = pdo_fetchall("select * from " . tablename("tapgo_tt_friendcoupon_data") . " where uniacid = :uniacid and headerid = :headerid and activity_id = :activity_id", array(":uniacid" => $_W["uniacid"], ":headerid" => $headerid, ":activity_id" => $activity_id));
    }
    /**
     * 引入随机优惠券算法
     */
    ...
    {
        $path = IA_ROOT . "/addons/tapgo_ttv2/plugin/friendcoupon/core/redPackBuilder.php";
        require_once $path;
    }
    /**
    ...
    public function getTemplateMessage($type)
    {
        ...
        if ($data["messageId"] == "default") {
            return pdo_fetch("select * from " . tablename("tapgo_tt_member_wxapp_message_template_default") . " where uniacid = :uniacid and typecode = :typecode", array(":uniacid" => $_W["uniacid"], ":typecode" => $typecode));
        }
        return pdo_fetch("select * from " . tablename("tapgo_tt_wxapp_tmessage") . " where id = :id", array("id" => $data["messageId"]));
    }
    /**
     * 获取公众号或者小程序token
     * @param bool $wxapp
