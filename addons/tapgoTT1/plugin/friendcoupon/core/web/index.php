...
class Index_EweiShopV2Page extends PluginWebPage
{
    ...
    public function activity_list()
    {
        ...
        $psize = 20;
        $list = pdo_fetchall("select * from " . tablename("tapgo_tt_friendcoupon") . " where 1 " . $condition . " and uniacid = :uniacid and deleted = :deleted order by create_time desc limit " . ($pindex - 1) * $psize . "," . $psize, array(":uniacid" => $_W["uniacid"], ":deleted" => 0));
        $time = time();
        foreach ($list as &$item) {
            ...
            if ($item["status"] != $initStatus) {
                pdo_update("tapgo_tt_friendcoupon", array("status" => $item["status"]), array("id" => $item["id"]));
            }
            $item["codeUrl"] = mobileUrl("friendcoupon", array("id" => $item["id"]), true);
        }
        unset($item);
        $total = pdo_fetchcolumn("select count(1) from " . tablename("tapgo_tt_friendcoupon") . " where 1 " . $condition . " and uniacid = :uniacid and deleted = 0", array(":uniacid" => $_W["uniacid"]));
    ...
    public function post()
    {
        ...
        if ($_W["ispost"]) {
            ...
            $titleExists = pdo_fetchcolumn("select count(1) from " . tablename("tapgo_tt_friendcoupon") . " where uniacid = :uniacid and deleted = :deleted and title = :title and id <> :id", array(":uniacid" => $_W["uniacid"], ":title" => $data["title"], ":deleted" => 0, ":id" => $id));
            if ($titleExists) {
                show_json(0, "标题不允许重复");
            }
            ...
            if (!$id) {
                ...
                pdo_insert("tapgo_tt_friendcoupon", $data);
                $insert_id = pdo_insertid();
                show_json(1, array("url" => webUrl("friendcoupon.post", array("id" => $insert_id))));
            } else {
                pdo_update("tapgo_tt_friendcoupon", $data, array("id" => $id));
                show_json(1);
            }
        }
        ...
        if (($activity["limitgoodtype"] == 1 || $activity["limitgoodtype"] == 2) && $activity["limitgoodids"]) {
            $goods = pdo_fetchall("SELECT id,title,thumb FROM " . tablename("tapgo_tt_goods") . " WHERE uniacid = :uniacid and id in (" . $activity["limitgoodids"] . ") ", array(":uniacid" => $_W["uniacid"]));
        }
        if (!empty($goods)) {
    ...
    public function delete()
    {
        global $_W;
        global $_GPC;
        pdo_update("tapgo_tt_friendcoupon", array("deleted" => 1), array("id" => $_GPC["id"]));
        show_json(1, "删除成功!");
    }
    /**
    ...
    public function stop()
    {
        ...
        global $_GPC;
        $activity_id = $_GPC["id"];
        pdo_update("tapgo_tt_friendcoupon", array("status" => -1, "stop_time" => time()), array("id" => $activity_id, "uniacid" => $_W["uniacid"]));
        pdo_update("tapgo_tt_friendcoupon_data", array("status" => -1), array("uniacid" => $_W["uniacid"], "id" => $activity_id));
        show_json(1, "操作成功");
    }
