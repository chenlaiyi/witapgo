...
if ($maxgoods > 0) {
    $sql = 'SELECT COUNT(1) FROM ' . tablename('tapgo_tt_goods') . ' where uniacid=:uniacid and merchid=:merchid';
...
$item = pdo_fetch("SELECT * FROM " . tablename('tapgo_tt_goods') . " WHERE id = :id and uniacid=:uniacid and merchid=:merchid", array(':id' => $id, ':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid']));
...
if ($_W['ispost']) {
    if ($_W['merch_user']['can_edit'] != 1 && $item['import_id'] > 0)
    {
        return show_json(0,'您没有权限修改商品信息');
        exit();
    }
    if (empty($id)) {
...
//    }
    ...
    $data = array(
        ...
        'merchdisplayorder' => intval($_GPC['displayorder']),
    ...
    //如果这些字段进行了修改，商户又不是商品免审核，则需要重新审核
    $needcheck = false;
    ...
    if (empty($id)) {
        if (empty($_W['merch_user']['goodschecked'])) $data['checked'] = 1;
...
if (!empty($id)) {
    ...
    if (!empty($item['storeids'])) {
        $stores = pdo_fetchall('select id,storename from ' . tablename('tapgo_tt_merch_store') . ' where id in (' . $item['storeids'] . ' ) and uniacid=' . $_W['uniacid'] . ' and merchid=' . $_W['merchid']);
    }
    //通知人
...
//查询快递模板
$dispatch_data = pdo_fetchall('select * from ' . tablename('tapgo_tt_dispatch') . ' where uniacid=:uniacid and merchid=:merchid and enabled=1 order by displayorder desc', array(':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid']));

if ($com_virtual) {
    $virtual_types = pdo_fetchall("select * from " . tablename('tapgo_tt_virtual_type') . " where uniacid=:uniacid and merchid=:merchid order by id asc", array(":uniacid" => $_W['uniacid'], ':merchid' => $_W['merchid']));
}
