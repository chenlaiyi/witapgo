...
class Index_EweiShopV2Page extends MerchWebPage 
{
	...
	public function main() 
	{
		...
		$sql = 'SELECT * FROM ' . tablename('tapgo_tt_coupon') . ' ' . ' where  1 and ' . $condition . ' ORDER BY displayorder DESC,id DESC LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize;
		$list = pdo_fetchall($sql, $params);
		foreach ($list as &$row ) 
		{
			$url = mobileUrl('sale/coupon/detail', array('id' => $row['id']), true);
			$row['qrcode'] = m('qrcode')->createQrcode($url);
			$row['gettotal'] = pdo_fetchcolumn('select count(*) from ' . tablename('tapgo_tt_coupon_data') . ' where couponid=:couponid and uniacid=:uniacid and merchid=:merchid limit 1', array(':couponid' => $row['id'], ':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid']));
			$row['usetotal'] = pdo_fetchcolumn('select count(*) from ' . tablename('tapgo_tt_coupon_data') . ' where used = 1 and couponid=:couponid and uniacid=:uniacid and merchid=:merchid limit 1', array(':couponid' => $row['id'], ':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid']));
			$row['pwdjoins'] = pdo_fetchcolumn('select count(*) from ' . tablename('tapgo_tt_coupon_guess') . ' where couponid=:couponid and uniacid=:uniacid and merchid=:merchid limit 1', array(':couponid' => $row['id'], ':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid']));
			$row['pwdoks'] = pdo_fetchcolumn('select count(*) from ' . tablename('tapgo_tt_coupon_guess') . ' where couponid=:couponid and uniacid=:uniacid and merchid=:merchid and ok=1 limit 1', array(':couponid' => $row['id'], ':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid']));
		}
		unset($row);
		$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('tapgo_tt_coupon') . ' where 1 and ' . $condition, $params);
		$pager = pagination($total, $pindex, $psize);
		$category = pdo_fetchall('select * from ' . tablename('tapgo_tt_coupon_category') . ' where uniacid=:uniacid and merchid=:merchid order by id desc', array(':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid']), 'id');
	...
	protected function post() 
	{
		...
		$item = pdo_fetch('SELECT * FROM ' . tablename('tapgo_tt_coupon') . ' WHERE id =:id and uniacid=:uniacid and merchid=:merchid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $id, ':merchid' => $_W['merchid']));
		if (empty($item)) 
		{
			$starttime = time();
	...
	public function delete() 
	{
		...
		$items = pdo_fetchall('SELECT id,couponname FROM ' . tablename('tapgo_tt_coupon') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid'] . ' AND merchid=' . $_W['merchid']);
		foreach ($items as $item ) 
		{
			pdo_delete('tapgo_tt_coupon', array('id' => $item['id'], 'uniacid' => $_W['uniacid'], 'merchid' => $_W['merchid']));
			pdo_delete('tapgo_tt_coupon_data', array('couponid' => $item['id'], 'uniacid' => $_W['uniacid'], 'merchid' => $_W['merchid']));
