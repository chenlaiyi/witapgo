...
class Index_EweiShopV2Page extends MerchWebPage 
{
	public function main() 
	{
		...
		if (p('cashier')) 
		{
			$sql = 'SELECT * FROM ' . tablename('tapgo_tt_cashier_user') . ' WHERE uniacid=:uniacid AND merchid=:merchid AND deleted=0 AND status=1';
			$res = pdo_fetch($sql, array(':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid']));
			if (!(empty($res)) && (time() < $res['lifetimeend'])) 
			{
				$cashier = $res;
				$auth_code = base64_encode(authcode($cashier['username'] . '|' . $cashier['password'] . '|' . $cashier['salt'], 'ENCODE', 'tapgo_ttv2_cashier'));
