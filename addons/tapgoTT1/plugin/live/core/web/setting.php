...
class Setting_EweiShopV2Page extends PluginWebPage
{
	public function main()
	{
		...
		$data = pdo_fetch('select * from ' . tablename('tapgo_tt_live_setting') . ' where uniacid = ' . $uniacid . ' ');
		...
		if ($_W['ispost']) {
			...
			if (!empty($data)) {
				pdo_update('tapgo_tt_live_setting', $set_data, array('uniacid' => $uniacid));
			}
			else {
				$set_data['uniacid'] = $uniacid;
				pdo_insert('tapgo_tt_live_setting', $set_data);
			}
