...
class GroupsModel extends PluginModel {
    ...
    /*public function exhelper(){
        $plugins = pdo_fetch("select status from ".tablename('tapgo_tt_plugin')." where identity = :identity ",array(':identity'=>'exhelper'));
        if($plugins['status']==1){
            $exhelper = 1;
        }else{
            $exhelper = 0;
    ...
    public function orderstest(){
        ...
        $uniacid = $_W['uniacid'];
        //检测未付款订单
        $sql= "SELECT * FROM".tablename('tapgo_tt_groups_order')."where uniacid = :uniacid and status = 0 ";
        ...
        if($allorders){
            ...
            foreach($allorders as $key => $value){
                ...
                if($lasttime2 < 0){
                    pdo_update('tapgo_tt_groups_order', array('status'=>-1), array('id' => $value['id']));
                }
            }
        }
        //检测拼团中的订单是否过期
        $sql1 = "SELECT * FROM".tablename('tapgo_tt_groups_order')."where uniacid = :uniacid and heads = 1 and status = 1 and success = 0 ";
        $allteam = pdo_fetchall($sql1, $params);
        if($allteam){
            foreach($allteam as $key => $value){
                $total = pdo_fetchcolumn('select count(1) from ' . tablename('tapgo_tt_groups_order') . "  where uniacid = :uniacid and teamid = :teamid and heads = :heads and status = :status and success = :success ",
                    array(':uniacid'=>$uniacid,':heads'=>1,':teamid'=>$value['teamid'],':status'=>1,':success'=>0));
                if($value['groupnum'] == $total){
                    //是否满足拼团人数
                    pdo_update('tapgo_tt_groups_order', array('success'=>1), array('teamid' => $value['teamid']));
                }else{
                    //检测拼团结束时间
                    $hours = $value['endtime'];
                    $time = time();
                    ...
                    if($lasttime2 < 0){
                        pdo_update('tapgo_tt_groups_order', array('success'=>-1,'canceltime'=>strtotime($endtime)), array('teamid' => $value['teamid']));
                    }
                }
            }
        }
    ...
    public function payResult($orderno,$type, $app=false) {
        ...
        $log = pdo_fetch('SELECT * FROM ' . tablename('tapgo_tt_groups_paylog') . '
		 WHERE `uniacid`=:uniacid AND `module`=:module AND `tid`=:tid limit 1', array(':uniacid' => $uniacid, ':module' => 'groups', ':tid' => $orderno));

        $order = pdo_fetch('select * from ' . tablename('tapgo_tt_groups_order') . ' where  orderno =:orderno and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':orderno' => $orderno));
        if($order['status']>0 || $log['status'] > 0){
            return true;
        }

        $openid = $order['openid'];
        $order_goods = pdo_fetch('select * from  '. tablename('tapgo_tt_groups_goods') . '
...
//        }
        ...
        pdo_update('tapgo_tt_groups_order', array('pay_type' => $type,'status'=>1,'paytime'=>TIMESTAMP,'starttime'=>TIMESTAMP, 'apppay'=>$app?1:0), array('orderno' => $orderno));
        pdo_update( 'tapgo_tt_groups_paylog' , array( 'status' => 1 ) , array( 'tid' => $orderno ) );

        //模板消息
        $this->sendTeamMessage($order['id']);
        if(!empty($order['is_team'])){
            $total = pdo_fetchcolumn('select count(1) from ' . tablename('tapgo_tt_groups_order') . " as o where status = :status and teamid = :teamid and uniacid = :uniacid and success = :success ", $params);
            if($order['groupnum']==$total){
                pdo_update('tapgo_tt_groups_order', array('success'=>1), array('teamid' => $order['teamid'],'status'=>1,'uniacid' => $uniacid));
                pdo_update('tapgo_tt_groups_order', array('success'=>-1,'status'=>-1,'canceltime'=>time()), array('teamid' => $order['teamid'],'status'=>0,'uniacid' => $uniacid));
                $this->sendTeamMessage($order['id']);
            }
        }

        if( $order_goods['more_spec'] == '1' ){
            $order_goods_S = pdo_get( 'tapgo_tt_groups_order_goods' , array( 'groups_order_id' => $order['id'] ) );
            $goods_option = pdo_get( 'tapgo_tt_groups_goods_option' , array( 'goods_option_id' => $order_goods_S['groups_goods_option_id'] ) );
            pdo_update( 'tapgo_tt_groups_goods_option' , array( 'stock' => $goods_option['stock']-1 ) , array( 'id' => $goods_option['id'] ) );
...
//            $teamnum = intval($order_goods['teamnum']) + 1;
        ...
        pdo_update('tapgo_tt_groups_goods', array('stock' => $stock,'sales'=>$sales), array('id' => $order_goods['id']));
        return true;
    }
    /*
     *订单tab列表数量
    ...
    public function getTotals() {
        ...
        $totals['all'] = pdo_fetchcolumn(
            'SELECT COUNT(1) FROM ' . tablename('tapgo_tt_groups_order') . " o
        ...
        $totals['refund1'] = pdo_fetchcolumn(
            'SELECT COUNT(1) FROM ' . tablename('tapgo_tt_groups_order_refund') . " as ore
			left join ".tablename('tapgo_tt_groups_order')." as o on o.id = ore.orderid
			right join ".tablename('tapgo_tt_groups_goods')." as g on g.id = o.goodid
			right join " . tablename('tapgo_tt_member') . " m on m.openid=o.openid and m.uniacid =  o.uniacid
			left join " . tablename('tapgo_tt_member_address') . " a on a.id=ore.refundaddressid
			right join ".tablename('tapgo_tt_groups_category')." as c on c.id = g.category
        ...
        $totals['verify1'] = pdo_fetchcolumn(
            'SELECT COUNT(1) FROM ' . tablename('tapgo_tt_groups_order') . " as o
    ...
    public function groupsShare(){
        global $_W;
        $uniacid = $_W['uniacid'];
        $share = pdo_fetch("select share_title,share_icon,share_desc,share_url from " . tablename('tapgo_tt_groups_set') . ' where uniacid=:uniacid ', array(':uniacid' => $uniacid));
    ...
    public function sendTeamMessage($orderid = '0', $delRefund = false)
    {
        ...
        $order = pdo_fetch('select * from ' . tablename('tapgo_tt_groups_order') . ' where uniacid = :uniacid and id=:id limit 1', array(':uniacid'=>$uniacid,':id' => $orderid));
        if (empty($order)) {
            return;
        }
        $openid = $order['openid'];
        ...
        $order_goods = pdo_fetch('select * from ' . tablename('tapgo_tt_groups_goods') . ' where uniacid=:uniacid and id=:id ', array(':uniacid' => $_W['uniacid'], ':id' => intval($order['goodid'])));
        ...
        // 拼团成功时间
        $successTime = pdo_fetchcolumn("select paytime from " . tablename('tapgo_tt_groups_order') . " where uniacid=:uniacid and teamid=:teamid order by paytime desc", array(
            ':uniacid'  => $uniacid,
            ':teamid'   => $order['teamid']
        ));
        ...
        if($delRefund==true){
            //买家退款通知
            $order_refund = pdo_fetch('select * from ' . tablename('tapgo_tt_groups_order_refund') . ' where uniacid=:uniacid and id=:id ', array(':uniacid' => $_W['uniacid'], ':id' => intval($order['refundid'])));
            $refundtype = '';
            ...
            if ($order['status'] == 1) {
                ...
                if($order['success'] == 1){
                    //拼团成功通知
                    $order = pdo_fetchall('select * from ' . tablename('tapgo_tt_groups_order') . ' where teamid = :teamid and success = 1 and status = 1 ', array(':teamid' => $order['teamid']));
    ...
    public function sendGroupsNotice(array $params) {
        ...
        if ($tm['is_advanced']) {
            ...
            //高级模式
            if (!empty($templateid)) {
                $advanced_template = pdo_fetch('select * from ' . tablename('tapgo_tt_member_message_template') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $templateid, ':uniacid' => $_W['uniacid']));
    ...
    public function allow($orderid, $times = 0,$verifycode = '',$openid = '') {
        ...
        $saler = pdo_fetch('select * from ' . tablename('tapgo_tt_saler') . ' where openid=:openid and uniacid=:uniacid limit 1', array(
        ...
        /*if (empty($saler) && $merch_plugin) {
            $saler = pdo_fetch('select * from ' . tablename('tapgo_tt_merch_saler') . ' where openid=:openid and uniacid=:uniacid limit 1', array(
                ':uniacid' => $_W['uniacid'], ':openid' => $openid
            ));
        }*/
        ...
        $order = pdo_fetch('select * from ' . tablename('tapgo_tt_groups_order') . ' where id=:id and uniacid=:uniacid  limit 1', array(':id' => $orderid, ':uniacid' => $uniacid));
        if (empty($order)) {
        ...
        $goods = pdo_fetch("select * from " . tablename('tapgo_tt_groups_goods') . "
        ...
        $option_title = '';
        $order_goods = pdo_fetch("select id,option_name from " . tablename('tapgo_tt_groups_order_goods') . "
        ...
        if ($order['isverify']) {
            ...
            if ($order['verifytype'] == 0) {
                //按订单核销
                $verifynum = pdo_fetchcolumn("select COUNT(1) from ".tablename('tapgo_tt_groups_verify')." where uniacid = :uniacid and orderid = :orderid ",
            ...
            if (!empty($saler['storeid'])) {
                if ($merchid > 0) {
                    $store = pdo_fetch('select * from ' . tablename('tapgo_tt_merch_store') . ' where id=:id and uniacid=:uniacid and merchid = :merchid limit 1', array(':id' => $saler['storeid'], ':uniacid' => $_W['uniacid'], ':merchid' => $merchid));

                } else {
                    $store = pdo_fetch('select * from ' . tablename('tapgo_tt_store') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $saler['storeid'], ':uniacid' => $_W['uniacid']));
                }
            }
        }
...
//        核销清除维权状态
        if (!empty($order['refundid'])) {
            $refund = pdo_fetch('select * from ' . tablename('tapgo_tt_groups_order_refund') . ' where id=:id limit 1', array(':id' => $order['refundid']));
            if (!empty($refund)) {
                $time = time();
                pdo_update('tapgo_tt_groups_order_refund', array('status' => -1, 'endtime' => $time), array('id' => $order['refundid']));
                pdo_update('tapgo_tt_groups_order', array('refundstate' => 0), array('id' => $order['id']));
            }
        }
        if ($order['isverify']) {
            if ($order['verifytype'] == 0) {
                pdo_update('tapgo_tt_groups_order',array('status'=>3,'finishtime'=>time(),'sendtime' => $current_time),array('id' => $order['id']));
                $data = array(
                    'uniacid'=>$uniacid,
                    'openid'=>$order['openid'],
                    'orderid'=>$orderid,
                ...
                pdo_insert('tapgo_tt_groups_verify', $data);
            } else if ($order['verifytype'] == 1) {
                //按次核销
                if ($order['status'] != 3) {
                    pdo_update('tapgo_tt_groups_order',array('status'=>3,'finishtime'=>time(),'sendtime' => $current_time),array('id' => $order['id']));
    ...
    public function setDefault($id,$type){
        global $_W;
        $item = pdo_fetch("SELECT id,expressname,type FROM " . tablename('tapgo_tt_exhelper_express') . " WHERE id=:id and type=:type AND uniacid=:uniacid" ,array(":id"=>$id, ':type'=>$type,":uniacid"=>$_W['uniacid']));
        if (!empty($item)) {
            pdo_update('tapgo_tt_exhelper_express', array('isdefault'=>0), array('type'=>$type,'uniacid'=>$_W['uniacid']));
            pdo_update('tapgo_tt_exhelper_express',array('isdefault'=>1),array('id'=>$id));
            if($type==1){
    ...
    public function tempDelete($id,$type){
        global $_W;
        $items = pdo_fetchall("SELECT id,expressname FROM " . tablename('tapgo_tt_exhelper_express') . " WHERE id in( $id ) and type=:type and uniacid=:uniacid ", array(':type'=>$type, ':uniacid'=>$_W['uniacid']));
        foreach ($items as $item) {
            pdo_delete('tapgo_tt_exhelper_express', array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
    ...
    public function getTemp(){
        ...
        // 查询发件人模板
        $temp_sender = pdo_fetchall("SELECT id,isdefault,sendername,sendertel FROM " . tablename('tapgo_tt_exhelper_senduser') . " WHERE uniacid=:uniacid order by isdefault desc ", array(':uniacid' => $_W['uniacid']));
        // 查询 快递单模板
        $temp_express = pdo_fetchall("SELECT id,type,isdefault,expressname FROM " . tablename('tapgo_tt_exhelper_express') . " WHERE type=1 and uniacid=:uniacid order by isdefault desc ", array(':uniacid' => $_W['uniacid']));
        // 查询发货单模板
        $temp_invoice = pdo_fetchall("SELECT id,type,isdefault,expressname FROM " . tablename('tapgo_tt_exhelper_express') . " WHERE type=2 and uniacid=:uniacid order by isdefault desc ", array(':uniacid' => $_W['uniacid']));

        // 查询电子面单模板
        $temp_esheet = pdo_fetchall("SELECT id,isdefault,esheetname FROM " . tablename('tapgo_tt_exhelper_esheet_temp') . " WHERE uniacid=:uniacid and merchid=0 order by isdefault desc ", array(':uniacid' => $_W['uniacid']));
        return array(
