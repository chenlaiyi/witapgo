...
class CycelbuyModel extends PluginModel
{
    public function cycelbuy_periodic($orderid)
    {
        ...
        global $_W;
        $openid = $_W["openid"];
        $sql = "SELECT id,ordersn,status,cycelbuy_periodic,cycelbuy_predict_time,iscycelbuy,addressid,dispatchprice,dispatchtype FROM " . tablename("tapgo_tt_order") . " WHERE id = :id and iscycelbuy=1 and uniacid =:uniacid";
        ...
        if( $order["status"] == 1 ) 
        {
            ...
            $addressSql = "SELECT * FROM " . tablename("tapgo_tt_member_address") . " WHERE id=:id and uniacid=:uniacid";
            ...
            for( $i = 0; $i < $cycelbuy_num; $i++ ) 
            {
                ...
                pdo_insert("tapgo_tt_cycelbuy_periods", $data);
            }
        }
