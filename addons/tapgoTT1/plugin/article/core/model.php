...
if (!class_exists('ArticleModel')) {
    ...
    class ArticleModel extends PluginModel {
        ...
        public function doShare($aid, $shareid, $myid) {
            ...
            $my_click = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename('tapgo_tt_article_share') . " WHERE aid=:aid and click_user=:click_user and uniacid=:uniacid ", array(':aid' => $article['id'], ':click_user' => $myid, ':uniacid' => $_W['uniacid']));
            if (!empty($my_click)) {
            ...
            $clicktime = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename('tapgo_tt_article_share') . " WHERE aid=:aid and share_user=:share_user and click_user=:click_user and uniacid=:uniacid ", array(':aid' => $article['id'], ':share_user' => $shareid, ':click_user' => $myid, ':uniacid' => $_W['uniacid']));
            if ($clicktime >= $readtime) {
                return;
            }

            $all_click = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename('tapgo_tt_article_share') . " WHERE aid=:aid and share_user=:share_user and uniacid=:uniacid ", array(':aid' => $article['id'], ':share_user' => $shareid, ':uniacid' => $_W['uniacid']));
            if ($all_click >= $article['article_rule_allnum']) {
                ...
                $day_click = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename('tapgo_tt_article_share') . " WHERE aid=:aid and share_user=:share_user and click_date>:day_start and click_date<:day_end and uniacid=:uniacid ", array(':aid' => $article['id'], ':share_user' => $shareid, ':day_start' => $day_start, ':day_end' => $day_end, ':uniacid' => $_W['uniacid']));
            ...
            $toto = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename('tapgo_tt_article_share') . " WHERE aid=:aid and share_user=:click_user and click_user=:share_user and uniacid=:uniacid ", array(':aid' => $article['id'], ':share_user' => $shareid, ':click_user' => $myid, ':uniacid' => $_W['uniacid']));
            if (!empty($toto)) {
                return;
            }
            ...
            if ($article['article_rule_credittotal'] > 0 || $article['article_rule_moneytotal'] > 0) {
                $creditlast = 0; 
                $moneylast = 0; 
                $firstreads = pdo_fetchcolumn('select count(distinct click_user) from ' . tablename('tapgo_tt_article_share') . ' where aid=:aid and uniacid=:uniacid limit 1', array(':aid' => $article['id'], ':uniacid' => $_W['uniacid']));
                $allreads = pdo_fetchcolumn('select count(*) from ' . tablename('tapgo_tt_article_share') . ' where aid=:aid and uniacid=:uniacid limit 1', array(':aid' => $article['id'], ':uniacid' => $_W['uniacid']));
                ...
                if ($article['article_rule_credittotal'] > 0) {
                    if (!empty($article['article_advance'])) {
                        ...
                        $creditout = pdo_fetchcolumn('select sum(add_credit) from ' . tablename('tapgo_tt_article_share') . ' where aid=:aid and uniacid=:uniacid limit 1', array(':aid' => $article['id'], ':uniacid' => $_W['uniacid']));
                ...
                if ($article['article_rule_moneytotal'] > 0) {
                    if (!empty($article['article_advance'])) {
                        ...
                        $moneyout = pdo_fetchcolumn('select sum(add_money) from ' . tablename('tapgo_tt_article_share') . ' where aid=:aid and uniacid=:uniacid limit 1', array(':aid' => $article['id'], ':uniacid' => $_W['uniacid']));
            ...
            $insert = array('aid' => $article['id'], 'share_user' => $shareid, 'click_user' => $myid, 'click_date' => time(),
                ...
                'add_money' => $givemoney,
                'uniacid' => $_W['uniacid']);
            pdo_insert('tapgo_tt_article_share', $insert);
            if ($givecredit > 0) {
        ...
        function href_replace($lnk) {
            global $_GPC;
            $newlnk = $lnk;
            if (strexists($lnk, 'tapgo_tt') && !strexists($lnk, '&mid')) {
                if (strexists($lnk, '?')) {
