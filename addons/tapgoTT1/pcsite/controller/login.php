			...
			if (!(empty($record))) {
				...
				if (!(empty($_GPC['__uniacid']))) {
					if ($role === false) {
						...
						$result_url = '../web/index.php?c=site&a=entry&m=tapgo_ttv2&do=web&r=sysset.account';
					}
					 else {
						$result_url = '../web/index.php?c=site&a=entry&m=tapgo_ttv2&do=web&r=shop';
					}
				}
				 else {
					load()->model('account');
