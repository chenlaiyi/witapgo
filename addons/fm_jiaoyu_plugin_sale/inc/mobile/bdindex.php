...
if ($operation == 'display') {
    ...
    // $allstu  = pdo_fetchcolumn("select COUNT(*) FROM ".tablename('wx_school_students')." WHERE weid = '{$weid}' AND schoolid = '{$schoolid}'   ");
    //学生总数
    ...
    // //老师的年龄
    // $tea_age = pdo_fetch("SELECT count(birthdate) AS total FROM " . tablename('wx_school_teachers') . " where weid = :weid AND schoolid = '{$schoolid}'", array(':weid' => $weid)); //所有总数
    // $tea_age1 = pdo_fetch("SELECT count(birthdate) AS total FROM " . tablename('wx_school_teachers') . " where weid = :weid AND schoolid = '{$schoolid}' AND (birthdate between $year1 AND $year)", array(':weid' => $weid)); //小于30岁的
    // $tea_age2 = pdo_fetch("SELECT count(birthdate) AS total FROM " . tablename('wx_school_teachers') . " where weid = :weid AND schoolid = '{$schoolid}' AND (birthdate between $year2 AND $year1)", array(':weid' => $weid)); //30-40
    // $tea_age3 = pdo_fetch("SELECT count(birthdate) AS total FROM " . tablename('wx_school_teachers') . " where weid = :weid AND schoolid = '{$schoolid}' AND (birthdate between $year3 AND $year2)", array(':weid' => $weid));//40-50
    // $tea_age4 = pdo_fetch("SELECT count(birthdate) AS total FROM " . tablename('wx_school_teachers') . " where weid = :weid AND schoolid = '{$schoolid}' AND (birthdate between $year4 AND $year3)", array(':weid' => $weid));//50-60
    // $tea_age5 = pdo_fetch("SELECT count(birthdate) AS total FROM " . tablename('wx_school_teachers') . " where weid = :weid AND schoolid = '{$schoolid}' AND (birthdate <$year4)", array(':weid' => $weid));//60以上
    // $tea_age_data = array(
    //     'xaixs' => array(
    //         0=>'30岁以下',
    //         1=>'30岁到40岁',
    ...
    if($keep_MC){
        /**************************************体温检测数据***************************************/
        //当日正常体温
        $zc = pdo_fetchcolumn("SELECT count(distinct sid) FROM " . GetTableName('morningcheck') . " WHERE schoolid = '{$schoolid}' AND createdate = '{$nowday}' AND ( (tiwen BETWEEN '35.5' AND '37.5') or tiwem < '1') ");
        //昨日正常体温
        $yzc = pdo_fetchcolumn("SELECT count(distinct sid) FROM " . GetTableName('morningcheck') . " WHERE schoolid = '{$schoolid}' AND createdate = '{$yesterday}' AND ( (tiwen BETWEEN '35.5' AND '37.5') or tiwem < '1')");
