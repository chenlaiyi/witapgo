...
if ($operation == 'display') {
   ...
   foreach ($stu_nj_list as $key=>$value){
    ...
    if($keep_MC){
        /**************************************体温检测数据***************************************/
        //当日正常体温
        $zc = pdo_fetchcolumn("SELECT count(distinct sid) FROM " . GetTableName('morningcheck') . " WHERE schoolid = '{$schoolid}' AND createdate = '{$nowday}' AND (tiwen BETWEEN '35.5' AND '37.5')");
        //昨日正常体温
        ...
        //获取当前学生学生总人数
        $AllStu = pdo_fetchcolumn("SELECT count(distinct sid) FROM " . GetTableName('students') . " WHERE schoolid = '{$schoolid}'");
        //获取今日已经检测的学员
