...
if($op == 'fensi'){
	...
	if(!empty($list)){
		foreach($list as $k=>$v){
			$cservice = pdo_fetch("SELECT id FROM ".tablename(BEST_CSERVICE)." WHERE weid = {$_W['uniacid']} AND ctype = 1 AND content = '{$v['kefuopenid']}'");
			if(!empty($cservice)){
				$html .= '<div class="item flex">
							<div class="item-d">
								<img src="'.$v['fansavatar'].'">
								<div class="kefuname textellipsis1 flex">'.$v['fansnickname'].'</div>
							</div>
							
							<div class="flex1 flex tod">
								<div class="flex1"></div>
								<a class="flex" href="'.$this->createMobileUrl('qdadmin',array('fkid'=>$v['id'],'op'=>'kefuchatdetail')).'">详情</a>
								<div class="flex1"></div>
							</div>
							
							<div class="item-d">
								<img src="'.$v['kefuavatar'].'">
								<div class="kefuname textellipsis1 flex">'.$v['kefunickname'].'</div>
							</div>
						</div>';
			}
		}
	...
	if(empty($fansandkefu)){
		$message = '参数错误！';
		include $this->template('error');
		exit;
	}
	include_once ROOT_PATH.'qqface.php';
	$openid = $fansandkefu['kefuopenid'];
	...
	foreach($chatcon as $k=>$v){
		if($v['openid'] != $openid){
			...
			$chatcon[$k]['avatar'] = $fansandkefu['fansavatar'];
		}else{
			$chatcon[$k]['class'] = 'right';
			$chatcon[$k]['avatar'] = $fansandkefu['kefuavatar'];
		}
