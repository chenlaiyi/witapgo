...
if ($operation == 'display') {
	...
	$isck = intval($_GPC['isck']);
	if($isck == 1){
		$condition .= " AND cangzh = 1";
	}
	...
	if($_GPC['groupid']){
		...
		$condition .= " AND (groupid = '{$_GPC['groupid']}' OR id in (".implode(',',$kefuidsarr)."))";
	}
	$cservicelist = pdo_fetchall("SELECT * FROM ".tablename(BEST_CSERVICE)." WHERE ".$condition." ORDER BY displayorder ASC,id ASC");
	$qrcodeRoot = (defined('ROOT_PATH') && file_exists(ROOT_PATH.'qrcode.class.php'))
		? ROOT_PATH
		: IA_ROOT . '/addons/cy163_customerservice/';
	include $qrcodeRoot.'qrcode.class.php';
	foreach($cservicelist as $k=>$v){
		if($v['ctype'] == 1){
			...
			$cservicelist[$k]['lyurl'] = $_W['siteroot'].'app/'.str_replace('./','',$this->createMobileUrl('chat',array('toopenid'=>$v['content'],'lyon'=>1)));
		}
		if($v['ctype'] == 2){
		...
		$zuname = '';
		$kfgroups = pdo_fetchall("SELECT groupid FROM ".tablename(BEST_KEFUANDGROUP)." WHERE kefuid = {$v['id']}");
		if(!empty($kfgroups)){
			foreach($kfgroups as $kk=>$vv){
				$kfgroup = pdo_fetch("SELECT name FROM ".tablename(BEST_CSERVICEGROUP)." WHERE id = {$vv['groupid']}");
				$zuname .= $kfgroup['name'].",";
			}
			$zuname = substr($zuname,0,-1);
		}
		$cservicelist[$k]['zuname'] = $zuname;
	}
	$this->addauth();
	...
	include $this->template('web/cservice');
}elseif($operation == 'liuyan') {
	$cservicelist = pdo_fetchall("SELECT * FROM " . tablename(BEST_CSERVICE) . " WHERE weid = {$_W['uniacid']} AND ctype = 1 ORDER BY displayorder ASC");
	
	$content = trim($_GPC['content']);
	$pindex = max(1, intval($_GPC['page']));
	$psize = 10;
	$total = pdo_fetchcolumn("SELECT count(*) FROM ".tablename(BEST_LIUYAN)." WHERE kefuopenid = '{$content}'");
	$limit = ' LIMIT ' . ($pindex - 1) * $psize . ',' . $psize;
	$liuyans = pdo_fetchall("SELECT * FROM ".tablename(BEST_LIUYAN)." WHERE kefuopenid = '{$content}' ORDER BY readstatus ASC,time DESC".$limit);
	foreach($liuyans as $k=>$v){
		$others = unserialize($v['autotext']);
		if(!empty($others)){
			foreach($others as $kk=>$vv){
				if(strstr($vv['val'],'images/')){
					$others[$kk]['val'] = tomedia($vv['val']);
					$others[$kk]['isimg'] = 1;
				}
			}
			$liuyans[$k]['others'] = $others;
		}else{
			$liuyans[$k]['others'] = [];
		}
	}
	...
	$id = intval($_GPC['id']);
	$kuaijie = pdo_fetch("SELECT * FROM " . tablename(BEST_KUAIJIE) . " WHERE id = {$id}");
	if (empty($kuaijie)) {
		message('抱歉，该快捷消息不存在或是已经被删除！', referer(), 'error');
	}
	if (checksubmit('submit')) {
		...
		pdo_update(BEST_KUAIJIE,$data,array('id'=>$id));
	...
	if (checksubmit('submit')) {
		...
		$data = array(
			...
			'nowjdnumbasic' => intval($_GPC['nowjdnumbasic']),
			...
			'lyon' => intval($_GPC['lyon']),
			'lytitle' => $_GPC['lytitle'],
			'jhtext' => $_GPC['jhtext'],
		...
		if (!empty($id)) {
			...
			$fieldopenid = $data['content'];
		}
		//处理字段
		pdo_delete(BEST_LYFIELD,array('openid'=>$fieldopenid,'weid'=>$_W['uniacid']));
		
		$totalstocks = 0;
		$field_displayorders = $_POST['field_displayorder'];
		$field_names = $_POST['field_name'];
		$field_fnames = $_POST['field_fname'];
		$field_yushes = $_POST['field_yushe'];
		$field_isneeds = $_POST['field_isneed'];
		$field_isimgs = $_POST['field_isimg'];
		$field_neizhis = $_POST['field_neizhi'];
		
		$len = count($field_names);
		for ($k = 0; $k < $len; $k++) {
			if($field_names[$k] != "" && $field_fnames[$k] != "" ){
				$a = array(
					"name" => $field_names[$k],
					"fname" => $field_fnames[$k],
					"yushe" => $field_yushes[$k],
					"isneed" => $field_isneeds[$k],
					"isimg" => $field_isimgs[$k],
					"neizhi" => $field_neizhis[$k],
					"displayorder" => $field_displayorders[$k],
					"openid" => $fieldopenid,
					"weid" => $_W['uniacid'],
					"ftype"=>"text",
				);
				pdo_insert(BEST_LYFIELD, $a);
			}
		}
		message('操作成功！', $this->createWebUrl('cservice', array('op' => 'display')), 'success');
	}
	$rule_keyword = pdo_fetch("SELECT * FROM " . tablename('rule_keyword') . " WHERE uniacid = {$_W['uniacid']} AND module = 'cy163_customerservice' AND status = 1 AND type = 3");
	include $this->template('web/cservice');
