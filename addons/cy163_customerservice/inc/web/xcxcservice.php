...
if ($operation == 'display') {
	...
	$id = intval($_GPC['id']);
	$cservice = pdo_fetch("SELECT id,content FROM ".tablename(BEST_XCXCSERVICE)." WHERE id = {$id} AND weid = {$_W['uniacid']}");
	if (empty($cservice)) {
		message('抱歉，小程序客服不存在或是已经被删除！');
	}
	pdo_delete(BEST_XCXCSERVICE, array('id' => $id));
	pdo_delete(BEST_XCXFANSKEFU, array('kefuopenid' => $cservice['content']));
	message('小程序客服删除成功！', referer(), 'success');
