...
if (!function_exists('send_sys_msg_admin'))
{
	...
	function send_sys_msg_admin($phone, $type = 0)
	{
		...
		if ($res) {
			$curr_date = $_W['slwl']['datetime']['now']; // 变量最长只能15个字符
			...
			if ($_W['slwl']['set']['msg_settings']) {
				...
				if ($messages['type']=='Alidayu') {
					...
					$return_data = sendsms($mobile, $content, $data);
					...
					$return_data = sendsms_new($mobile, $content, $data);
				} else {
					$return_data = array(
						'errcode'=>'2',
						'errmsg'=>'没有找到短信服务',
...
if (!function_exists('send_user_msg'))
{
	function send_user_msg($phone, $money)
	{
		...
		$msg = pdo_fetch("SELECT * FROM " . sl_table_name('settings',true) . ' WHERE 1 ' . $condition, $params);
		...
		if ($msg) {
			...
			if ($messages['type']=='Alidayu') {
				...
				$return_data = sendsms($phone, $content, $data);
			} else if ($messages['type']=='Alidysmslite') {
				$content = array(
					'time'=>$curr_date,
					'phone' => $phone,
				...
				$return_data = sendsms_new($phone, $content, $data);
			} else {
				$return_data = array(
					'errcode'=>'2',
					'errmsg'=>'没有找到短信服务',
