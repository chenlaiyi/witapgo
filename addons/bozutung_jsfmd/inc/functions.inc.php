...
function wxappUrl($segment='', $params = array(), $noredirect = TRUE, $addhost = TRUE)
{
	...
	$array_tmp = [
		'a' => 'wxapp',
		'm' => strtolower($_W['current_module']['name']),
	];

	$array_new = array_merge($array_tmp, $params);

	return murl('entry/wxapp/'.$segment, $array_new, $noredirect, $addhost);
}
 ...
 */
function sl_file_write($fileAddress, $data) {
	$fp = fopen($fileAddress, "w");
	...
	$s .= "
		<div class='box'>
			...
			<div class='input-group' style='margin-top:.5em;'>
				...
				<em class='close' style='position:absolute; top: 0px; right: -14px;' title='删除' onclick='deleteUserImage(this)'>×</em>
			</div>
		</div>
	";
	return $s;
...
// 选择-平台用户
function slwl_tpl_form_show_mp_user($name, $value=[],$default='')
{
	if (empty($default)) {
		$default = MODULE_URL.'template/public/image/nopic.jpg';
	}
	...
	$s = '';
	if (!defined('SLWL_TPL_FORM_SHOW_MP_USER')) {
		$s = '
		<script type="text/javascript">
			function showLinkMpUser(elm) {
				myutil.myShowMpUserSelect(function(e) {
					let _this = $(elm);
					let _this_parent = _this.closest(".box");
					_this_parent.find(".mpuid").val(e.uid);
					_this_parent.find(".username").val(e.username);
					_this_parent.find(".avatar").attr("src", e.avatar);
				});
			}
			function deleteUserImage(elm){
		...
		define('SLWL_TPL_FORM_SHOW_MP_USER', TRUE);
	}
	$tmp_onerror = "onerror=\"this.src='{$default}'; this.title='图片未找到'\"";
	$s .= "
		<div class='box'>
			<div class='input-group'>
				<input type='hidden' name='{$name}' value='{$value['id']}' class='mpuid'>
				<input type='text' value='{$value['username']}' class='layui-input username' readonly>
				<span class='input-group-btn'>
					<button class='btn btn-default' type='button' onclick='showLinkMpUser(this);'>选择用户</button>
				</span>
			</div>
			<div class='input-group' style='margin-top:.5em;'>
				<img src='{$val}' {$tmp_onerror} class='img-responsive img-thumbnail avatar' width='150' />
