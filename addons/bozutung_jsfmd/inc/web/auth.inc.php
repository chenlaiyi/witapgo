...
if ($operation == 'display') {
	...
	if ($_W['ispost']) {
		...
		if ($_W['slwl']['set']['auth_settings']) {
			...
			$rst = pdo_update('bozutung_jsfmd_settings', $data, $where);
		} else {
			$data['uniacid'] = '0';
			$data['setting_name'] = 'auth_settings';
			$data['addtime'] = $_W['slwl']['datetime']['now'];
			$rst = pdo_insert('bozutung_jsfmd_settings', $data);
		}
	...
	if ($_W['ispost']) {
		...
		$sql_data = "CREATE TABLE IF NOT EXISTS `{$tablepre}bozutung_jsfmd_settings` (
