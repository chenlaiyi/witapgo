...
if ($operation == 'display') {
	...
	$sql = "SELECT * FROM " . tablename('bozutung_jsfmd_users'). ' WHERE 1 '
	...
	if ($list) {
		...
		$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('bozutung_jsfmd_users') . ' WHERE 1 ' . $condition, $params);
	...
	if ($_W['ispost']) {
		...
		if ($id) {
			pdo_update('bozutung_jsfmd_users', $data, array('id' => $id));
		}
		iajax(0, '保存成功');
		exit;
	}
	...
	$one = pdo_fetch("SELECT * FROM " . tablename('bozutung_jsfmd_users') . " where 1 " . $condition, $params);
...
include $this->template('web/user/user');
