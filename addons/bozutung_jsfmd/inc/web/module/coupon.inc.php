...
if ($operation == 'display') {
	...
	$sql = "SELECT * FROM " . sl_table_name('coupon',true). ' WHERE 1 '
	...
	if ($list) {
		...
		$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . sl_table_name('coupon',true) . ' WHERE 1 ' . $condition, $params);
	...
	if ($_W['ispost']) {
		...
		if ($id) {
			pdo_update(sl_table_name('coupon'), $data, array('id' => $id));
		} else {
			$data['addtime'] = $_W['slwl']['datetime']['now'];
			pdo_insert(sl_table_name('coupon'), $data);
			$id = pdo_insertid();
		}
		sl_ajax(0, '保存成功');
	}
	...
	$one = pdo_fetch('SELECT * FROM ' . sl_table_name('coupon',true) . ' WHERE 1 ' . $condition, $params);
	...
	$rst = @pdo_delete(sl_table_name('coupon'), $where);
