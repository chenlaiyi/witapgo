...
if ($_W['ispost']) {
	...
	if ($ws_data_history_update == '1')
	{
		...
		$sql_act_news = "SELECT * FROM " . tablename('bozutung_jsfmd_website_act_news'). ' WHERE 1 ' . $condition_act_news;
		...
		if ($list_act_news) {
			foreach ($list_act_news as $k => $v) {
				if ((isset($v['newsname'])) && ($v['newsname'] != '')) {
					...
					@pdo_update('bozutung_jsfmd_website_act_news', $data_act_news, array('id'=>$v['id']));
				}
			}
		}
	}
	...
	if ($dy_data_history_update == '1')
	{
		$condition_act_news = " AND uniacid=:uniacid AND title='' ";
		$params_act_news = array(':uniacid' => $_W['uniacid']);
		$sql_act_news = "SELECT * FROM " . tablename('bozutung_jsfmd_dynamic_act'). ' WHERE 1 ' . $condition_act_news;
		$list_act_news = pdo_fetchall($sql_act_news, $params_act_news);
		...
		if ($list_act_news) {
			foreach ($list_act_news as $k => $v) {
				if ((isset($v['dy_title'])) && ($v['dy_title'] != '')) {
					...
					@pdo_update('bozutung_jsfmd_dynamic_act', $data_act_news, array('id'=>$v['id']));
				}
			}
		}
	}
	...
	if ($ai_data_history_update == '1')
	{
		...
		$sql_sync = "SELECT openid AS user_id,mobile FROM " . tablename('bozutung_jsfmd_users'). ' WHERE 1 ' . $condition_sync;
	...
	if ($store_class_transfer == '1')
	{
		...
		$sql_old_category = "SELECT * FROM " . tablename('bozutung_jsfmd_shop_category'). ' WHERE 1 '
		...
		$sql_child_category = "SELECT * FROM " . tablename('bozutung_jsfmd_shop_category'). ' WHERE 1 '
		...
		if ($list_old_category) {
			foreach ($list_old_category as $k => $v) {
				...
				$rst = pdo_insert('bozutung_jsfmd_store_category', $data_old_category);
				$parent_id_category = pdo_insertid();
				if ($rst === false) {
					iajax(1, '失败-商品-旧版转到新版-父类');
					break;
				}
				...
				if ($list_child_category) {
					foreach ($list_child_category as $key => $value) {
						if ($value['parentid'] == $v['id']) {
							...
							$rst_child = pdo_insert('bozutung_jsfmd_store_category', $data_child);
							if ($rst_child === false) {
								iajax(1, '失败-商品-旧版转到新版-子类');
								break;
							}
							break;
						}
	...
	if ($store_goods_transfer == '1')
	{
		...
		$sql_old_goods = "SELECT * FROM " . tablename('bozutung_jsfmd_shop_goods'). ' WHERE 1 '
		...
		$psize_old_spec = 1000;
		$sql_old_spec = "SELECT * FROM " . tablename('bozutung_jsfmd_shop_spec'). ' WHERE 1 '
		...
		if ($list_old_goods) {
			foreach ($list_old_goods as $k => $v) {
				...
				$rst = pdo_insert('bozutung_jsfmd_store_goods', $data_goods);
				$parent_id = pdo_insertid();
				if ($rst === false) {
					iajax(1, '失败-商城模块-商品-旧版转到新版');
					break;
				}
			}
		}
	...
	if ($sys_users_clear == '1') {
		$rst_users = pdo_delete('bozutung_jsfmd_users', array('uniacid' => $_W['uniacid']));
		if ($rst_users === false) {
			iajax(1, '清除客户数据表失败');
		}
	}
	...
	if ($sys_mycard_clear == '1') {
		$rst_mycard = pdo_delete('bozutung_jsfmd_mycard', array('uniacid' => $_W['uniacid']));
		if ($rst_mycard === false) {
			iajax(1, '清除客户数据表失败');
		}
	}
	...
	if ($sys_formid_clear == '1') {
		$rst_formid = pdo_delete('bozutung_jsfmd_formid', array('uniacid' => $_W['uniacid']));
		if ($rst_formid === false) {
			iajax(1, '清除客户数据表失败');
		}
	}
	...
	if ($sys_oplog_clear == '1') {
		$rst_oplog = pdo_delete('bozutung_jsfmd_oplog', array('uniacid' => $_W['uniacid']));
		if ($rst_oplog === false) {
			iajax(1, '清除客户数据表失败');
		}
	}
