...
if ($operation == 'display') {
	...
	$sql = "SELECT * FROM " . tablename('bozutung_jsfmd_adact'). ' WHERE 1 '
	...
	if ($list) {
		foreach ($list as $k => $v) {
			$list[$k]['out_thumb_url'] = tomedia($v['out_thumb']);
			$list[$k]['enabled_format'] = $v['enabled'] ? '启用':'禁用';
		}
		$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('bozutung_jsfmd_adact') . ' WHERE 1 ' . $condition, $params);
	...
	if ($_W['ispost']) {
		...
		if ($id) {
			pdo_update('bozutung_jsfmd_adact', $data, array('id' => $id));
		} else {
			$data['addtime'] = $_W['slwl']['datetime']['now'];
			pdo_insert('bozutung_jsfmd_adact', $data);
			$id = pdo_insertid();
		}
		iajax(0, '保存成功！');
	}
	...
	$one = pdo_fetch('SELECT * FROM ' . tablename('bozutung_jsfmd_adact') . ' WHERE 1 ' . $condition, $params);
	...
	$rst = @pdo_delete('bozutung_jsfmd_adact', $where);
