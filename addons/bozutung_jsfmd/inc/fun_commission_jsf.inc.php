 ...
 */
function sl_jsf_commission_post($id_me, $id_invite)
{
	global $_GPC, $_W;
 ...
 * @return json                  返回结果，成功code=0，失败code=1，msg=信息
 */
function sl_jsf_commission_rebate($id_order_user,$id_order,$money,$type)
{
	global $_GPC, $_W;
	...
	$one_user = pdo_get(sl_table_name('user'),$where_order_user);
...
// 健身房-结算
function sl_jsf_commission_settle()
{
	global $_GPC, $_W;
			...
			if ($value['rebate']) {
				...
				if ($value['rebate']['set']['level'] > 0) {
					...
					if ($brokerage_com_1 == 0) {
						...
						// 更新用户余额
						$rst_1 = pdo_update(sl_table_name('user'),
				...
				if ($value['rebate']['set']['level'] > 1) {
					...
					if ($brokerage_com_2 == 0) {
						...
						// 更新用户余额
						$rst_2 = pdo_update(sl_table_name('user'),
				...
				if ($value['rebate']['set']['level'] > 2) {
					...
					if ($brokerage_com_1 == 0) {
						...
						// 更新用户余额
						$rst_3 = pdo_update(sl_table_name('user'),
...
// 分销中心
function sl_jsf_commission_center()
{
	global $_GPC, $_W;
	...
	$one = [];
	$nickname = is_json($user['nickname'])?json_decode($user['nickname']):$user['nickname'];
	$one['name'] = $user['real_name']?$user['real_name']:$nickname;
	$one['id'] = $uid;
	...
	if ($one_com && $one_com['id_com_1'] > 0) {
		...
		$one_user = pdo_get(sl_table_name('user'), $where_user);

		$nickname_superior = json_decode($one_user['nickname']);
		$name_superior = $one_user['real_name']?$one_user['real_name']:$nickname_superior;
		$one['superior'] = [
			'name'   => $name_superior,
			'tel'    => $one_user['tel'],
...
// 分销佣金-汇总
function sl_jsf_commission_brokerage()
{
	global $_GPC, $_W;
...
// 分销佣金-明细
function sl_jsf_commission_brokerage_log()
{
	global $_GPC, $_W;
	...
	$tb_order = sl_table_name('order', TRUE);
	...
	$field .= " o.id AS id_order,o.sn,o.paid_money,o.order_info,o.status ";
	...
	if ($list) {
		...
		foreach ($list as $key => $value) {
			$list[$key]['order_info'] = json_decode($value['order_info'],TRUE);
...
// 提现-明细
function sl_jsf_commission_withdraw_log()
{
	global $_GPC, $_W;
...
// 我的下线列表
function sl_jsf_commission_my_downline()
{
	global $_GPC, $_W;
	...
	if ($list_com) {
		...
		$field = ['nickname','avatar','real_name','tel'];
		$list_user = pdo_getall(sl_table_name('user'), $where_user, $field, '', $order_by_user);

		if ($list_user) {
			foreach ($list_user as $key => $value) {
				$nickname = is_json($value['nickname'])?json_decode($value['nickname']):$value['real_name'];
				$list_user[$key]['nickname'] = $nickname;
			}
	...
	sl_ajax(0, $list_my_downline);
}

// 提现-post
function sl_jsf_withdraw_post()
{
	global $_GPC, $_W;
