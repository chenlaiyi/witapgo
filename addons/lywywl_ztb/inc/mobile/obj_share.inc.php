...
$act = trim($_GPC["act"]);
$allow_acts = array("index", "draw", "register", "article", "award");
...
if ($plug_appad) {
    $plug_appad_act = getPluginConfig($_W["uniacid"], "lywywl_ztb_plugin_appad", "is_open_share");
}
$plug_vaptcha = getPluginStatus($_W["uniacid"], "lywywl_ztb_plugin_vaptcha");
$is_vaptcha_share = 0;
if ($plug_vaptcha) {
    $is_vaptcha_share = getPluginConfig($_W["uniacid"], "lywywl_ztb_plugin_vaptcha", "is_vaptcha_share");
...
$object = pdo_get(ztbNopreTable("obj_share"), array("deltime" => 0, "activity_id" => $activity["id"]));
...
if ($act == "index") {
    ...
    if ($object["is_show_ads"] == 1) {
        if (empty($_GPC["ad_share_" . $object["id"]])) {
            $object["is_show_ads"] = 1;
            isetcookie("ad_share_" . $object["id"], "ad", 3600 * 24 * 365);
        } else {
    ...
    if ($object["models"] == 1) {
        $isShare = 0;
        $userShare = pdo_getall(ztbNopreTable("user_share"), array("deltime" => 0, "activity_id" => $object["activity_id"], "openid" => $openid), array(), '', "id desc", 1)[0];
        if ($userShare) {
            $isShare = 1;
            $shareViewCount = pdo_fetchcolumn("SELECT COUNT(*) FROM " . ztbTable("obj_share_view") . " where `deltime`=0 and is_effective=0 and activity_id=:activity_id and openid=:openid", array(":activity_id" => $object["activity_id"], ":openid" => $userShare["openid"]));
            $userDrawCount = pdo_fetchcolumn("SELECT COUNT(*) FROM " . ztbTable("user_draw") . " where `deltime`=0  and activity_id=:activity_id and openid=:openid", array("activity_id" => $object["activity_id"], "openid" => $userShare["openid"]));
            if ($userDrawCount > 0) {
                $isShare = 2;
            }
        }
    ...
    $_share["link"] = __MURL("obj_share", array("act" => "article", "token" => $token, "origin_id" => $origin_id, "i_openid" => $openid), false, TRUE);
...
if ($act == "article") {
    ...
    if (empty($i_openid)) {
        tip_redirect("对不起，您浏览的文章不存在！");
    }
    $userShare = pdo_getall(ztbNopreTable("user_share"), array("deltime" => 0, "activity_id" => $object["activity_id"], "openid" => $i_openid), array(), '', "note desc", 1)[0];
    if (empty($userShare)) {
        tip_redirect("对不起，活动分享信息不存在！");
    }
    $Whethe_to_award = false;
    setCache("i_openid_share_" . $object["id"] . $openid, $i_openid, 3600 * 24);
    if ($userShare["openid"] != $openid) {
        if ($object["limit_clicks"]) {
            $viewOpenIdCliks = pdo_fetchcolumn("SELECT COUNT(*) FROM " . ztbTable("obj_share_view") . " where `deltime`=0 and `is_effective`=0 and activity_id=:activity_id and view_openid=:view_openid", array(":activity_id" => $object["activity_id"], ":view_openid" => $openid));
            if ($viewOpenIdCliks < $object["limit_clicks"]) {
                $result = pdo_get(ztbNopreTable("obj_share_view"), array("deltime" => 0, "activity_id" => $object["activity_id"], "openid" => $userShare["openid"], "view_openid" => $openid));
                if (empty($result)) {
                    $data = array("uniacid" => $userShare["uniacid"], "store_id" => $userShare["store_id"], "activity_types" => $userShare["activity_types"], "activity_id" => $userShare["activity_id"], "origin_team_id" => $originModel["team_id"], "origin_id" => $origin_id, "openid" => $userShare["openid"], "view_openid" => $openid, "createtime" => TIMESTAMP, "is_effective" => 0);
                    $result = pdo_insert(ztbNopreTable("obj_share_view"), $data);
                    $share_view_id = pdo_insertid();
                    if (!empty($object["area_limit"]) && intval($object["vsit_types"]) == 1) {
                        pdo_update(ztbNopreTable("obj_share_view"), array("is_effective" => 1), array("id" => $share_view_id));
                    }
                    if (!empty($origin_id)) {
                        pdo_update(ztbNopreTable("marketing_user"), array("share_view_num +=" => 1), array("id" => $origin_id));
                    }
                    $Whethe_to_award = true;
                }
            }
        } else {
            $result = pdo_get(ztbNopreTable("obj_share_view"), array("deltime" => 0, "activity_id" => $object["activity_id"], "openid" => $userShare["openid"], "view_openid" => $openid));
    ...
    $shareViewCount = pdo_fetchcolumn("SELECT COUNT(*) FROM " . ztbTable("obj_share_view") . " where `deltime`=0  and `is_effective`=0 and activity_id=:activity_id and openid=:openid ", array(":activity_id" => $object["activity_id"], ":openid" => $userShare["openid"]));
    if ($activity["start_time"] <= TIMESTAMP && $activity["end_time"] >= TIMESTAMP) {
        if ($object["models"] == 1) {
            $userDrawCount = pdo_fetchcolumn("SELECT COUNT(*) FROM " . ztbTable("user_draw") . " where `deltime`=0 and activity_id=:activity_id and openid=:openid", array("activity_id" => $object["activity_id"], "openid" => $userShare["openid"]));
            if ($userDrawCount <= 0 && $shareViewCount >= $object["winer_num"] && $object["recurring_status"] == 0 || $shareViewCount > 0 && $shareViewCount % $object["winer_num"] == 0 && $object["recurring_status"] == 1 && $Whethe_to_award) {
                $shareUserModel = pdo_get(ztbTable("user_account", false), array("uniacid" => $uniacid, "store_id" => $store_id, "openid" => $userShare["openid"]));
                ...
                if (!empty($prizeList)) {
                            ...
                            if (!(intval($val["odds"]) <= 0)) {
                                if (!(intval($val["limittypes"]) == 0 && intval($val["limitnum"]) > 0)) {
                                    if (!(intval($val["limittypes"]) == 1 && intval($val["limitnum"]) > 0)) {
                                        ...
                                        $count = pdo_fetchcolumn("SELECT COUNT(*) FROM " . ztbTable("user_draw") . " WHERE  prize_id = :prize_id AND openid = :openid AND FROM_UNIXTIME(createtime, '%Y%m%d') = DATE_FORMAT(NOW(),'%Y%m%d')", array(":prize_id" => $val["id"], ":openid" => $i_openid));
                                    ...
                                    $count = pdo_getcolumn(ztbNopreTable("user_draw"), array("prize_id" => $val["id"], "openid" => $i_openid), "count(*)");
                                    if (!($count >= intval($val["limitnum"]))) {
                                        if (!(intval($val["limittypes"]) == 1 && intval($val["limitnum"]) > 0)) {
                                            $arr[$key] = $val["odds"];
                                        } else {
                    ...
                    if (!empty($prizeModel)) {
                        $share_draw_data = array("uniacid" => $uniacid, "store_id" => $store_id, "activity_types" => $activity_types, "activity_id" => $object["activity_id"], "origin_team_id" => $originModel["team_id"], "origin_id" => $origin_id, "prize_id" => $prizeModel["id"], "types" => $prizeModel["types"], "name" => $prizeModel["name"], "prize_pic_url" => $prizeModel["picurl"], "writeoff_types" => $prizeModel["writeoff_types"], "openid" => $shareUserModel["openid"], "nickname" => $shareUserModel["nickname"], "headurl" => $shareUserModel["headurl"], "register_data" => $userShare["note"], "updatetime" => TIMESTAMP, "createtime" => TIMESTAMP);
                        $result = pdo_insert(ztbNopreTable("user_draw", false), $share_draw_data);
                        $writecode = '';
                        if (!empty($result)) {
                            $draw_id = pdo_insertid();
                            $hashids = Hashids::instance(6, "lywyztb", '');
                            $encode_id = $hashids->encode($draw_id);
                            $share_draw_data = array("writecode" => $encode_id);
                            pdo_update(ztbNopreTable("user_draw", false), $share_draw_data, array("id" => $draw_id));
                            $writecode = $encode_id;
                        }
                        if (intval($prizeModel["types"]) == 1) {
                            ...
                            $note = "中奖获得：" . timeToStr(TIMESTAMP) . " 会员【" . $shareUserModel["nickname"] . "】参与活动：" . $activity["title"] . "，获得积分：" . $prizeModel["score"];
                            pdo_update(ztbNopreTable("user_account"), array("score +=" => $prizeModel["score"]), array("id" => $shareUserModel["id"]));
                            $userScoreModel = array();
                            $userScoreModel["uniacid"] = $uniacid;
                            $userScoreModel["store_id"] = $store_id;
                            $userScoreModel["openid"] = $shareUserModel["openid"];
                            $userScoreModel["nickname"] = $shareUserModel["nickname"];
                            $userScoreModel["headurl"] = $shareUserModel["headurl"];
                            ...
                            if (intval($prizeModel["types"]) == 2) {
                                ...
                                $note = "中奖获得：" . timeToStr(TIMESTAMP) . " 会员【" . $shareUserModel["nickname"] . "】参与活动：" . $activity["title"] . "，获得金额：" . $prizeModel["sys"] . "元";
                                $storeModel = pdo_get(ztbNopreTable("store_account"), array("uniacid" => $uniacid, "id" => $store_id));
                                if ($storeModel["money"] >= $prizeModel["sys"]) {
                                    pdo_update(ztbNopreTable("user_account"), array("money +=" => $prizeModel["sys"]), array("id" => $shareUserModel["id"]));
                                    $userBillModel = array();
                                    $userBillModel["uniacid"] = $uniacid;
                                    $userBillModel["store_id"] = $store_id;
                                    $userBillModel["openid"] = $shareUserModel["openid"];
                                    $userBillModel["nickname"] = $shareUserModel["nickname"];
                                    $userBillModel["headurl"] = $shareUserModel["headurl"];
                                    $userBillModel["types"] = 3;
                                    $userBillModel["detail_id"] = $draw_id;
                                    $userBillModel["money"] = $prizeModel["sys"];
                                    $userBillModel["balance"] = pdo_getcolumn(ztbNopreTable("user_account"), array("store_id" => $store_id, "openid" => $shareUserModel["openid"]), "money");
                                    ...
                                    $sysReissueModel["openid"] = $shareUserModel["openid"];
                                    $sysReissueModel["nickname"] = $shareUserModel["nickname"];
                                    $sysReissueModel["headurl"] = $shareUserModel["headurl"];
                                ...
                                if (intval($prizeModel["types"]) == 3) {
                                    ...
                                    $note = "中奖获得：" . timeToStr(TIMESTAMP) . " 会员【" . $shareUserModel["nickname"] . "】参与活动：" . $activity["title"] . "，获得红包：" . $prizeModel["money"] . "元";
                                    $storeModel = pdo_get(ztbNopreTable("store_account"), array("uniacid" => $uniacid, "id" => $store_id));
                                    if ($storeModel["money"] >= $prizeModel["money"]) {
                                        ...
                                        $storeBillModel["createtime"] = TIMESTAMP;
                                        pdo_insert(ztbNopreTable("store_bill"), $storeBillModel);
                                        $result = sendWeixinMchPay($shareUserModel["openid"], floatval($prizeModel["money"]) * 100, $prizeModel["name"], true, $uniacid, $config);
                                        if (!($result === true)) {
                                            ...
                                            if ($is_mchpayfail_usermoney == 0) {
                                                ...
                                                pdo_update(ztbNopreTable("user_account"), array("money +=" => $prizeModel["money"]), array("store_id" => $store_id, "openid" => $shareUserModel["openid"]));
                                                $userBillModel = array();
                                                $userBillModel["uniacid"] = $uniacid;
                                                $userBillModel["store_id"] = $store_id;
                                                $userBillModel["openid"] = $shareUserModel["openid"];
                                        ...
                                        $sysReissueModel["openid"] = $shareUserModel["openid"];
                                        $sysReissueModel["nickname"] = $shareUserModel["nickname"];
                                        $sysReissueModel["headurl"] = $shareUserModel["headimgurl"];
                                        $sysReissueModel["status"] = 0;
                                        $sysReissueModel["money"] = $prizeModel["money"];
                                        $sysReissueModel["desc"] = $prizeModel["name"];
                                        $sysReissueModel["is_store_sell"] = 0;
                        ...
                        if ($prizeModel["is_sms"] == 1 && $storeModel["sms"] > 0) {
                            if (!empty($prizeModel["sms_tmp"])) {
                                if (!empty($shareUserModel["mobile"])) {
                                    $sms_uid = $config["sms_uid"];
                                    $sms_key = $config["sms_key"];
                                    $mobile = $shareUserModel["mobile"];
                                    $sms_content = $prizeModel["sms_tmp"];
                                    $sms_content = str_replace("{NICKNAME}", $shareUserModel["nickname"], $sms_content);
    ...
    if (intval($object["vsit_types"]) == 1) {
        header("Location: " . $object["vsit_url"]);
        exit;
    }
    $_share["title"] = replaceShareInfo($object["share_title"]) . (empty($origin_name) ? '' : "【" . $origin_name . "】");
    $_share["imgUrl"] = $object["share_thumb"] ? tomedia($object["share_thumb"]) : getMemberAvatar();
    $_share["desc"] = replaceShareInfo($object["share_desc"]);
    $_share["link"] = __MURL("obj_share", array("act" => "article", "token" => $token, "origin_id" => $origin_id, "i_openid" => $i_openid), false, TRUE);
    $_share["link"] = replaceDieDomain($config, $_share["link"], $user["id"], $object["activity_id"], $user["id"]);
    $object["share_title"] = replaceShareInfo($object["share_title"], $i_openid);
    $template = pdo_get(ztbNopreTable("sys_tmp"), array("deltime" => 0, "id" => $activity["tmp_id"]));
    $resource = $template["resource"];
    $tmpDir = "tmp/" . $resource;
    include $this->template("{$tmpDir}/article");
    exit;
}
if ($act == "draw" && $request_method == "post") {
    ...
    if ($object["models"] == 2) {
        if (intval($object["get_types"]) == 0) {
            $count = pdo_fetchcolumn("SELECT COUNT(*) FROM " . ztbTable("user_draw") . " WHERE deltime = 0 AND uniacid = :uniacid AND activity_id = :activity_id AND openid = :openid", array(":uniacid" => $uniacid, ":activity_id" => $object["activity_id"], ":openid" => $openid));
        ...
        if (intval($object["get_types"]) == 1) {
            $count = pdo_fetchcolumn("SELECT COUNT(*) FROM " . ztbTable("user_draw") . " WHERE deltime = 0 AND uniacid = :uniacid AND activity_id = :activity_id AND FROM_UNIXTIME(createtime, '%Y%m%d') = DATE_FORMAT(NOW(),'%Y%m%d') AND openid = :openid", array(":uniacid" => $uniacid, ":activity_id" => $object["activity_id"], ":openid" => $openid));
            if ($count >= intval($object["limit_num"])) {
                thread_unlock($openid);
    ...
    if ($object["area_limit"]) {
        ...
        if (empty($longitude) || empty($latitude)) {
            thread_unlock($openid);
            exit(json_encode(array("status" => 0, "tip" => "area", "msg" => "亲，请同意授权获取地理位置后，再进行抽奖！"), JSON_UNESCAPED_UNICODE));
        }
        ...
        if (empty($content)) {
            ...
            if (empty($content)) {
                thread_unlock($openid);
                exit(json_encode(array("status" => 0, "msg" => "对不起，本次活动限制" . $object["area_limit"] . "区域内参加！系统本次没有获取到地理位置,请刷新后重试"), JSON_UNESCAPED_UNICODE));
            }
        }
        $is_in_area = false;
        ...
        if ($is_in_area === false) {
            thread_unlock($openid);
            exit(json_encode(array("status" => 0, "msg" => "亲，您所在的区域不在本次活动范围,本次活动限制" . $object["area_limit"] . "区域内参加!"), JSON_UNESCAPED_UNICODE));
        }
        $i_openid = getCache("i_openid_share_" . $object["id"] . $openid);
        $share_view = pdo_get(ztbNopreTable("obj_share_view"), array("deltime" => 0, "is_effective" => 1, "activity_id" => $object["activity_id"], "view_openid" => $openid, "openid" => $i_openid));
        if ($share_view) {
            if ($share_view["is_effective"] == 1) {
                pdo_update(ztbNopreTable("obj_share_view"), array("is_effective" => 0), array("id" => $share_view["id"]));
            }
        }
    }
    if (intval($object["is_register"]) == 1) {
        ...
        if (empty($register_data)) {
            thread_unlock($openid);
            exit(json_encode(array("status" => 0, "tip" => "register", "msg" => "请先登记后再抽奖"), JSON_UNESCAPED_UNICODE));
        }
    }
    $prizeList = pdo_getall(ztbNopreTable("obj_prize"), array("deltime" => 0, "activity_id" => $object["activity_id"], "status" => 1), array(), '', "sort asc");
    if (!empty($prizeList)) {
        ...
        thread_unlock($openid);
        exit(json_encode(array("status" => 0, "msg" => "活动未设置抽奖奖项。"), JSON_UNESCAPED_UNICODE));
    }
    if (!empty($_GPC["valid"])) {
        thread_unlock($openid);
        exit(json_encode(array("status" => 1, "msg" => "验证通过"), JSON_UNESCAPED_UNICODE));
    }
    if ($object["models"] == 2 && $plug_vaptcha && $is_vaptcha_share == 1) {
        $verify_token = trim($_GPC["verify_token"]);
        if (empty($verify_token)) {
            thread_unlock($openid);
            exit(json_encode(array("status" => 0, "msg" => "请通过人机验证后参与活动！"), JSON_UNESCAPED_UNICODE));
        }
        if (!vaptcha_verify($verify_token, 1)) {
            thread_unlock($openid);
            exit(json_encode(array("status" => 0, "msg" => "请通过人机验证后参与活动！"), JSON_UNESCAPED_UNICODE));
        }
    }
    if ($object["models"] == 1) {
        $userDrawCount = pdo_fetchcolumn("SELECT COUNT(*) FROM " . ztbTable("user_draw") . " where `deltime`=0  and activity_id=:activity_id and openid=:openid", array("activity_id" => $object["activity_id"], "openid" => $openid));
        if ($userDrawCount > 0) {
            thread_unlock($openid);
            exit(json_encode(array("status" => 0, "msg" => "恭喜您，转发成功！"), JSON_UNESCAPED_UNICODE));
        } else {
            thread_unlock($openid);
            exit(json_encode(array("status" => 0, "tip" => "viewshare", "msg" => "恭喜您，转发成功，达到点击量后自动获得奖品！"), JSON_UNESCAPED_UNICODE));
        }
    }
    ...
    if (intval($object["is_register"]) == 1) {
        $draw_data["register_data"] = base64_decode($register_data);
    }
    ...
    $result_data = array("status" => 1, "draw_id" => $draw_id, "prize_id" => $drawModel["prize_id"], "prize_types" => $drawModel["types"], "prize_name" => $drawModel["name"], "prize_picurl" => tomedia($drawModel["prize_pic_url"]), "score" => $drawModel["score"], "sys" => $drawModel["sys"], "money" => $drawModel["money"], "card_id" => $drawModel["card_id"], "is_register" => $object["is_register"], "writecode" => $drawModel["writecode"], "writeoff_types" => $drawModel["writeoff_types"]);
    thread_unlock($openid);
    exit(json_encode($result_data, JSON_UNESCAPED_UNICODE));
}
if ($act == "register" && $request_method == "post") {
    if (!$_W["isajax"]) {
        exit(json_encode(array("status" => 0, "msg" => "非法请求"), JSON_UNESCAPED_UNICODE));
    }
    $registerFields = unserialize($object["register_field"]);
    foreach ($registerFields as &$item) {
    ...
    unset($item);
    isetcookie("reg_objshare_" . $object["id"], base64_encode(serialize($registerFields)), 3600 * 24 * 365);
    exit(json_encode(array("status" => 1, "msg" => "登记成功！"), JSON_UNESCAPED_UNICODE));
}
if ($act == "award" && $request_method == "post") {
    if (!thread_lock($openid)) {
        exit(json_encode(array("status" => 0, "msg" => "当前请求速度过快，请稍后访问！"), JSON_UNESCAPED_UNICODE));
    }
    if ($object["area_limit"]) {
        ...
        if (empty($longitude) || empty($latitude)) {
            thread_unlock($openid);
            exit(json_encode(array("status" => 0, "msg" => "亲，请同意授权获取地理位置！"), JSON_UNESCAPED_UNICODE));
        }
        $content = getCityNameByLongitudeAndLatitude($longitude, $latitude);
        if (empty($content)) {
            $content = getCityNameByLongitudeAndLatitude($longitude, $latitude);
            if (empty($content)) {
                thread_unlock($openid);
                exit(json_encode(array("status" => 0, "msg" => "对不起，活动限制" . $object["area_limit"] . "区域内参加！系统没有获取到地理位置,请刷新后重试"), JSON_UNESCAPED_UNICODE));
            }
        }
        $is_in_area = false;
        foreach ($areaArr as $area) {
        ...
        if ($is_in_area === false) {
            thread_unlock($openid);
            exit(json_encode(array("status" => 0, "msg" => "对不起，您不在活动区域内， 本次活动限制" . $object["area_limit"] . "区域内参加！"), JSON_UNESCAPED_UNICODE));
        }
    } else {
        thread_unlock($openid);
        exit(json_encode(array("status" => 1, "msg" => "非法请求！"), JSON_UNESCAPED_UNICODE));
    }
    $i_openid = getCache("i_openid_share_" . $object["id"] . $openid);
    $share_view = pdo_get(ztbNopreTable("obj_share_view"), array("deltime" => 0, "is_effective" => 1, "activity_id" => $object["activity_id"], "view_openid" => $openid, "openid" => $i_openid));
    $Whethe_to_award = false;
    if ($share_view) {
        ...
        thread_unlock($openid);
        exit(json_encode(array("status" => 1, "msg" => "请求成功!"), JSON_UNESCAPED_UNICODE));
    }
    $shareViewCount = pdo_fetchcolumn("SELECT COUNT(*) FROM " . ztbTable("obj_share_view") . " where `deltime`=0 and is_effective=0 and activity_id=:activity_id and openid=:openid", array(":activity_id" => $object["activity_id"], ":openid" => $i_openid));
    $userDrawCount = pdo_fetchcolumn("SELECT COUNT(*) FROM " . ztbTable("user_draw") . " where `deltime`=0  and activity_id=:activity_id and openid=:openid", array("activity_id" => $object["activity_id"], "openid" => $i_openid));
    if ($userDrawCount <= 0) {
        if ($activity["start_time"] <= TIMESTAMP && $activity["end_time"] >= TIMESTAMP) {
            if ($object["models"] == 1) {
                if ($userDrawCount <= 0 && $shareViewCount == $object["winer_num"] && $object["recurring_status"] == 0 || $shareViewCount > 0 && $shareViewCount % $object["winer_num"] == 0 && $object["recurring_status"] == 1 && $Whethe_to_award) {
                    $shareUserModel = pdo_get(ztbTable("user_account", false), array("uniacid" => $uniacid, "store_id" => $store_id, "openid" => $i_openid));
                    $prizeList = pdo_getall(ztbNopreTable("obj_prize"), array("deltime" => 0, "activity_id" => $object["activity_id"], "status" => 1), array(), '', "sort asc");
                    if (!empty($prizeList)) {
                        $prizeModel = null;
                        foreach ($prizeList as $key => $val) {
