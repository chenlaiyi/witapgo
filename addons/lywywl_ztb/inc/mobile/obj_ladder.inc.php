...
$act = trim($_GPC["act"]);
$allow_acts = array("index", "join", "share", "register", "joinPay", "joinWxPay", "getJoinPayNumber", "checkJoinPay", "pay", "wxPay", "getPayNumber", "checkPay", "detail");
...
if ($plug_appad) {
    $plug_appad_act = getPluginConfig($_W["uniacid"], "lywywl_ztb_plugin_appad", "is_open_ladder");
}
...
if ($act == "index") {
    ...
    if ($show_join_id) {
        $show_join_model = pdo_get("lywywl_ztb_obj_ladder_join", array("uniacid" => $uniacid, "store_id" => $store_id, "deltime" => 0, "activity_id" => $activity["id"], "id" => $show_join_id));
    }
    if (!empty($_GET["join_id"])) {
        $join_id = $_GET["join_id"];
        isetcookie("join_id_ladder_" . $object["id"], $_GET["join_id"], 3600 * 24);
    } else {
        $join_id = $_GPC["join_id_ladder_" . $object["id"]];
    }
    if ($object["is_show_ads"] == 1) {
        if (empty($_GPC["ad_ladder_" . $object["id"]])) {
            $object["is_show_ads"] = 1;
            isetcookie("ad_ladder_" . $object["id"], "ad", 3600 * 24);
        } else {
    ...
    unset($item);
    $ladder_price_list = unserialize($object["ladder_price"]);
    $group_min_num = max(0, reset($ladder_price_list)["num"]);
    $group_max_num = max(0, end($ladder_price_list)["num"]);
    $group_min_price = max(0, end($ladder_price_list)["price"]);
    $group_max_price = max(0, reset($ladder_price_list)["price"]);
    $object["group_price"] = $object["cost_price"];
    $share_id = 0;
    $join_model = pdo_get("lywywl_ztb_obj_ladder_join", array("uniacid" => $uniacid, "store_id" => $store_id, "deltime" => 0, "activity_id" => $activity["id"], "openid" => $openid));
    if ($join_model) {
        $share_id = $join_model["id"];
        $_share["link"] = __MURL("scan", array("token" => $token, "origin_id" => $origin_id, "join_id" => $join_model["id"]), false, TRUE);
        $current_group_count = pdo_count(ztbNopreTable("obj_ladder_join"), array("uniacid" => $uniacid, "store_id" => $store_id, "activity_id" => $activity["id"], "pid" => $join_model["pid"], "deltime" => 0));
        foreach ($ladder_price_list as $item) {
            if ($current_group_count >= $item["num"]) {
                $object["group_price"] = $item["price"];
            }
        }
        if ($join_model["is_heads"] == 1) {
            $object["group_price"] = round($object["group_price"] * $object["heads_discount"], 2);
        }
    } else {
    ...
    $groupCount = pdo_count(ztbNopreTable("obj_ladder_join"), array("uniacid" => $uniacid, "activity_id" => $activity["id"], "is_heads" => 1, "deltime" => 0));
    $joinGroupCount = pdo_count(ztbNopreTable("obj_ladder_join"), array("uniacid" => $uniacid, "activity_id" => $activity["id"], "deltime" => 0));
    $joinSuccessCount = pdo_count(ztbNopreTable("obj_ladder_join"), array("uniacid" => $uniacid, "activity_id" => $activity["id"], "status" => 1, "deltime" => 0));
    $surplusCount = $object["open_num"] * $group_max_num - $joinGroupCount;
    if (!empty($join_model)) {
        $invite_id = $join_model["pid"];
        $joinGroupList = pdo_getall(ztbNopreTable("obj_ladder_join"), array("pid" => $join_model["pid"], "deltime" => 0), '', '', "id asc");
    } else {
        if (!empty($join_id) && intval($join_id) > 0) {
            $pIdArr = pdo_get(ztbNopreTable("obj_ladder_join"), array("uniacid" => $uniacid, "activity_id" => $activity["id"], "id" => $join_id, "deltime" => 0), "pid");
            if (!empty($pIdArr)) {
                $invite_id = $join_id;
                $joinGroupList = pdo_getall(ztbNopreTable("obj_ladder_join"), array("pid" => $pIdArr["pid"], "deltime" => 0), '', '', "id asc");
            } else {
                $groupIdArr = pdo_getall(ztbNopreTable("obj_ladder_join"), array("uniacid" => $uniacid, "activity_id" => $activity["id"], "is_heads" => 1, "status" => 0, "deltime" => 0), "id", '', " id asc ", 1);
                if (!empty($groupIdArr)) {
                    $invite_id = $groupIdArr[0]["id"];
                    $joinGroupList = pdo_getall(ztbNopreTable("obj_ladder_join"), array("pid" => $groupIdArr[0]["id"], "deltime" => 0), '', '', "id asc");
                }
            }
        } else {
            $groupIdArr = pdo_getall(ztbNopreTable("obj_ladder_join"), array("uniacid" => $uniacid, "activity_id" => $activity["id"], "is_heads" => 1, "status" => 0, "deltime" => 0), "id", '', " id asc ", 1);
    ...
    $joinGroupPrice = $object["cost_price"];
    $joinGroupNum = 0;
    if (!empty($joinGroupList)) {
        foreach ($ladder_price_list as $item) {
            if (count($joinGroupList) >= $item["num"]) {
                $joinGroupPrice = $item["price"] + $object["front_money"];
                $joinGroupNum = $item["num"];
            }
        }
    }
    $invite_model = pdo_get(ztbNopreTable("obj_ladder_join"), array("uniacid" => $uniacid, "store_id" => $store_id, "activity_id" => $activity["id"], "id" => $invite_id, "deltime" => 0));
    $current_group_id = max(0, $invite_model["pid"]);
    $current_group_list = pdo_getall(ztbNopreTable("obj_ladder_join"), array("uniacid" => $uniacid, "store_id" => $store_id, "activity_id" => $activity["id"], "is_heads" => 1, "id <>" => $current_group_id, "deltime" => 0), '', '', "id asc");
    foreach ($current_group_list as $key => $value) {
        $current_group_list[$key]["user"] = pdo_getall(ztbNopreTable("obj_ladder_join"), array("pid" => $value["id"], "deltime" => 0), '', '', "id asc");
    }
    ...
    $buyUserList = pdo_getall(ztbTable("user_draw", false), array("uniacid" => $uniacid, "store_id" => $store_id, "activity_id" => $activity["id"], "types" => -1, "deltime" => 0), array("nickname", "headurl", "createtime", "paymoney", "mymoney"), '', "id desc", array(20));
    if (count($buyUserList) < 20 && intval($activity["bogus_buy_num"]) > 0) {
        ...
        foreach ($userList as $user) {
            ...
            if ($is_insert) {
                $insertUser = array("nickname" => $user["nickname"], "headurl" => $user["avatar"], "createtime" => $activity["start_time"], "paymoney" => $group_min_price, "mymoney" => 0);
    ...
    unset($d_user);
    $joinUserList = pdo_getall(ztbTable("obj_ladder_join", false), array("uniacid" => $uniacid, "store_id" => $store_id, "activity_id" => $activity["id"], "deltime" => 0), array("nickname", "headurl", "createtime"), '', "id desc", array(20));
    if (count($joinUserList) < 20 && intval($activity["bogus_join_num"]) > 0) {
        ...
        foreach ($userList as $user) {
            $is_insert = true;
            foreach ($joinUserList as $buyUser) {
                if (!($buyUser["nickname"] == $user["nickname"])) {
                    if ($user["nickname"] == $userinfo["nickname"]) {
                        $is_insert = false;
                    }
    ...
    unset($d_user);
    $topUserList = pdo_getall(ztbTable("obj_ladder_join", false), array("uniacid" => $uniacid, "store_id" => $store_id, "deltime" => 0, "activity_id" => $activity["id"]), array("nickname", "headurl", "joins", "bogus_joins", "money", "bogus_money"), '', "joins`+`bogus_joins desc,money`+`bogus_money desc", array(10));
...
if ($act == "join" && $request_method == "post") {
    ...
    if ($invite_id > 0) {
        ...
        $current_group_count = pdo_count(ztbNopreTable("obj_ladder_join"), array("uniacid" => $uniacid, "store_id" => $store_id, "activity_id" => $activity["id"], "pid" => $invite_model["pid"], "deltime" => 0));
        $ladder_price_list = unserialize($object["ladder_price"]);
        $group_max_num = max(0, end($ladder_price_list)["num"]);
        if ($current_group_count >= $group_max_num) {
            exit(json_encode(array("status" => 0, "msg" => "对不起，参团人数已满，请重新参团或开团！"), JSON_UNESCAPED_UNICODE));
        }
    } else {
        if ($object["open_num"] > 0) {
            $countOpenNum = pdo_count(ztbNopreTable("obj_ladder_join"), array("uniacid" => $uniacid, "activity_types" => $activity_types, "activity_id" => $activity["id"], "is_heads" => 1, "deltime" => 0));
            if ($countOpenNum >= $object["open_num"]) {
                exit(json_encode(array("status" => 0, "msg" => "对不起，活动可开团数已满，请点击参团购买！"), JSON_UNESCAPED_UNICODE));
            }
        }
    }
    $joinCount = pdo_count(ztbNopreTable("obj_ladder_join"), array("uniacid" => $uniacid, "store_id" => $store_id, "activity_id" => $activity["id"], "openid" => $openid, "deltime" => 0));
    if ($joinCount > 0) {
    ...
    isetcookie("join_objladder_" . $object["id"], base64_encode(serialize($registerFields)), 3600 * 24);
...
if ($act == "share" && $request_method == "post") {
    ...
    $join_model = pdo_get("lywywl_ztb_obj_ladder_join", array("uniacid" => $uniacid, "store_id" => $store_id, "activity_id" => $activity["id"], "openid" => $openid, "deltime" => 0));
    if (empty($join_model)) {
    ...
    if (empty($join_model["qrcode_url"])) {
        ...
        if (!empty($originModel)) {
            if ($originModel["openid"] == $openid) {
                $scanurl = __MURL("scan", array("token" => $token, "origin_id" => $originModel["id"], "join_id" => $join_model["id"]), false, TRUE);
            } else {
        ...
        if ($qr !== false) {
            pdo_update(ztbTable("obj_ladder_join", false), array("qrcode_url" => $file_path), array("id" => $join_model["id"]));
...
if ($act == "register" && $request_method == "post") {
    ...
    if ($join_model) {
        ...
        exit(json_encode(array("status" => 0, "msg" => "对不起，请先参团再来购买更优惠哦！"), JSON_UNESCAPED_UNICODE));
    }
    $drawCount = pdo_count(ztbNopreTable("user_draw"), array("uniacid" => $uniacid, "store_id" => $store_id, "activity_id" => $activity["id"], "openid" => $openid, "types" => -1, "deltime" => 0));
    if ($drawCount > 0) {
        exit(json_encode(array("status" => 0, "msg" => "对不起，您已经购买过了！"), JSON_UNESCAPED_UNICODE));
    }
    isetcookie("reg_objladder_" . $object["id"], base64_encode(serialize($registerFields)), 3600 * 24);
    exit(json_encode(array("status" => 1, "msg" => "登记成功！"), JSON_UNESCAPED_UNICODE));
}
if ($act == "joinPay") {
    ...
    $register_data = $_GPC["join_objladder_" . $object["id"]];
...
if ($act == "getJoinPayNumber" && $request_method == "post") {
    ...
    if ($is_pay) {
        ...
        if (!empty($join_id)) {
            $joinModel = pdo_get(ztbTable("obj_ladder_join", false), array("uniacid" => $uniacid, "id" => $join_id));
        }
        $payModel = array();
        $payModel["uniacid"] = $uniacid;
        $payModel["store_id"] = $store_id;
        ...
        pdo_insert(ztbNopreTable("obj_ladder_join"), $joinPayModel);
        $obj_join_id = pdo_insertid();
        if ($pid == 0) {
            $pid = pdo_insertid();
            pdo_update(ztbNopreTable("obj_ladder_join"), array("pid" => $pid), array("id" => $pid));
        }
        $ladder_price_list = unserialize($object["ladder_price"]);
        $group_min_num = max(0, reset($ladder_price_list)["num"]);
        isetcookie("join_objladder_" . $object["id"], '', -3600 * 24);
        $joinCount = pdo_count(ztbNopreTable("obj_ladder_join"), array("uniacid" => $uniacid, "pid" => $pid, "deltime" => 0));
        if ($joinCount >= $group_min_num) {
            pdo_update(ztbNopreTable("obj_ladder_join"), array("status" => 1), array("pid" => $pid));
        }
        ...
        if ($joinCount >= $group_min_num && $object["rebate_node"] == 1) {
            $join_invite_sql = "SELECT invite_id,openid,nickname,headurl FROM " . ztbTable("obj_ladder_join") . " where deltime=0 and activity_id=:activity_id and uniacid=:uniacid and pid=:pid ";
            ...
            foreach ($join_invite_list as $join_user) {
                if (!empty($join_user["invite_id"])) {
                    ...
                    if (!empty($join_id)) {
                        $join_invite_model = pdo_get(ztbTable("obj_ladder_join", false), array("id" => $join_id));
                        if ($joinModel["openid"] != $openid) {
                            $params = ["uniacid" => $uniacid, "store_id" => $store_id, "activity_types" => $activity_types, "origin_id" => $origin_id, "object" => $object, "originModel" => $originModel, "userModel" => $join_user, "activity" => isset($activity) ? $activity : $object, "objJoinTableName" => "obj_ladder_join"];
                            ...
                            if (!($count >= 1)) {
                                pdo_update(ztbTable("obj_ladder_join", false), array("joins +=" => 1), array("id" => $join_id));
                                ...
                                if ($plug_rebate2) {
                                    if (isset($object["or_open_rebate2"]) && $object["or_open_rebate2"] == 1) {
                                        ...
                                        if (!empty($draw2Model)) {
                                            $join2Model = pdo_get(ztbTable("obj_ladder_join", false), array("uniacid" => $uniacid, "activity_id" => $activity["id"], "openid" => $draw2Model["openid"]));
                                            if (!empty($join2Model)) {
                                                if ($join2Model["openid"] != $openid && $join2Model["openid"] != $join_invite_model["openid"]) {
                                                    ...
                                                    pdo_update(ztbTable("obj_ladder_join", false), array("joins +=" => 1), array("id" => $join2Model["id"]));
            ...
            if ($object["rebate_node"] == 0) {
                if (!empty($join_id)) {
                    if ($joinModel["openid"] != $openid) {
                        $params = ["uniacid" => $uniacid, "store_id" => $store_id, "activity_types" => $activity_types, "origin_id" => $origin_id, "object" => $object, "originModel" => $originModel, "userModel" => $userModel, "activity" => isset($activity) ? $activity : $object, "objJoinTableName" => "obj_ladder_join"];
                        $join_invite_model = pdo_get(ztbTable("obj_ladder_join", false), array("id" => $join_id));
                        pdo_update(ztbTable("obj_ladder_join", false), array("joins +=" => 1), array("id" => $join_id));
...
if ($act == "checkJoinPay") {
    ...
    header("Location: " . __MURL("obj_ladder", array("act" => "index", "pay_number" => $pay_number, "token" => $activity["token"], "origin_id" => $origin_id, "incoming" => "pay")));
    exit;
}
if ($act == "pay") {
    $join_id = $_GPC["join_id"];
    ...
    $register_data = $_GPC["reg_objladder_" . $object["id"]];
    ...
    if ($join_model["is_heads"] == 1) {
        $object["money"] = round($object["money"] * $object["heads_discount"], 2);
    }
    $drawCount = pdo_count(ztbNopreTable("user_draw"), array("uniacid" => $uniacid, "store_id" => $store_id, "activity_id" => $activity["id"], "openid" => $openid, "types" => -1, "deltime" => 0));
    if ($drawCount > 0) {
        tip_redirect("对不起，您已经购买过了！");
...
if ($act == "getPayNumber" && $request_method == "post") {
    ...
    if ($is_pay) {
        ...
        $payModel["terminal"] = 1;
        $payModel["types"] = 3;
        $payModel["data"] = "{$token},{$join_id},0,{$origin_id}";
        $payModel["openid"] = $openid;
        $payModel["nickname"] = $userinfo["nickname"];
        $payModel["headurl"] = $userinfo["headimgurl"];
        $payModel["paynumber"] = getOrderNumber();
        ...
        isetcookie("reg_objladder_" . $object["id"], '', -3600 * 24);
        ...
        if ($config["orderpushtmp"] && $storeModel["openid"]) {
            if (isFollow($storeModel["openid"], $uniacid)) {
                ...
                $url = replaceDieDomain($config, __MURL("store.obj_ladder_pay", array("act" => "index", "aid" => $activity["id"]), true, true), 0, 0, 0);
...
if ($act == "checkPay") {
    ...
    header("Location: " . __MURL("obj_ladder", array("act" => "detail", "pay_number" => $pay_number, "token" => $activity["token"], "origin_id" => $origin_id, "incoming" => "pay")));
    exit;
}
if ($act == "detail") {
    $pay_number = $_GPC["pay_number"];
