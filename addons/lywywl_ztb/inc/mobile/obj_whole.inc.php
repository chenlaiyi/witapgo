...
$act = trim($_GPC["act"]);
$allow_acts = array("index", "join", "share", "register", "pay", "wxPay", "getPayNumber", "checkPay", "detail", "success");
...
if ($plug_appad) {
    $plug_appad_act = getPluginConfig($_W["uniacid"], "lywywl_ztb_plugin_appad", "is_open_whole");
}
...
if ($act == "index") {
    ...
    if ($show_join_id) {
        $show_join_model = pdo_get("lywywl_ztb_obj_whole_join", array("uniacid" => $uniacid, "store_id" => $store_id, "deltime" => 0, "activity_id" => $activity["id"], "id" => $show_join_id));
    }
    if (!empty($_GET["join_id"])) {
        $join_id = $_GET["join_id"];
        isetcookie("join_id_whole_" . $object["id"], $_GET["join_id"], 3600 * 24);
    } else {
        $join_id = $_GPC["join_id_whole_" . $object["id"]];
    }
    if ($object["is_show_ads"] == 1) {
        if (empty($_GPC["ad_whole_" . $object["id"]])) {
            $object["is_show_ads"] = 1;
            isetcookie("ad_whole_" . $object["id"], "ad", 3600 * 24);
        } else {
    ...
    unset($item);
    $share_id = 0;
    $join_model = pdo_get("lywywl_ztb_obj_whole_join", array("uniacid" => $uniacid, "store_id" => $store_id, "activity_id" => $activity["id"], "openid" => $openid, "deltime" => 0));
    if ($join_model) {
        ...
        if ($object["join_is_register"] <= 0) {
            $insert_join_model = array("uniacid" => $uniacid, "store_id" => $store_id, "activity_types" => $activity_types, "activity_id" => $activity["id"], "origin_team_id" => $originModel["team_id"], "origin_id" => $origin_id, "openid" => $openid, "nickname" => $userinfo["nickname"], "headurl" => $userinfo["headimgurl"], "qrcode_url" => '', "joins" => 0, "money" => 0, "register_data" => '', "status" => 0, "createtime" => TIMESTAMP);
            $join_group_model = pdo_get(ztbNopreTable("obj_whole_group"), array("uniacid" => $uniacid, "activity_id" => $activity["id"], "openid" => $openid, "deltime" => 0));
            if ($join_group_model) {
                $insert_join_model["status"] = 1;
            }
            $result = pdo_insert("lywywl_ztb_obj_whole_join", $insert_join_model);
            if (!empty($result)) {
    ...
    $groupCount = pdo_count(ztbNopreTable("obj_whole_group"), array("uniacid" => $uniacid, "activity_id" => $activity["id"], "is_heads" => 1, "deltime" => 0));
    $joinGroupCount = pdo_count(ztbNopreTable("obj_whole_group"), array("uniacid" => $uniacid, "activity_id" => $activity["id"], "deltime" => 0));
    $joinSuccessCount = pdo_count(ztbNopreTable("obj_whole_group"), array("uniacid" => $uniacid, "activity_id" => $activity["id"], "status" => 1, "deltime" => 0));
    $noGroupCount = pdo_count(ztbNopreTable("obj_whole_group"), array("uniacid" => $uniacid, "activity_id" => $activity["id"], "is_heads" => 1, "status" => 0, "deltime" => 0));
    $noSuccessCount = pdo_count(ztbNopreTable("obj_whole_group"), array("uniacid" => $uniacid, "activity_id" => $activity["id"], "status" => 0, "deltime" => 0));
    $surplusCount = max(0, ($object["open_num"] - $groupCount) * $object["group_num"]) + max(0, $noGroupCount * $object["group_num"] - $noSuccessCount);
    $group_model = pdo_get(ztbNopreTable("obj_whole_group"), array("uniacid" => $uniacid, "store_id" => $store_id, "activity_id" => $activity["id"], "openid" => $openid, "deltime" => 0));
    if ($group_model) {
        $joinGroupList = pdo_getall(ztbNopreTable("obj_whole_group"), array("pid" => $group_model["pid"], "deltime" => 0), '', '', "id asc");
    } else {
        $join_id = intval($join_id);
        $joinModel = pdo_get(ztbTable("obj_whole_join", false), array("uniacid" => $uniacid, "activity_id" => $activity["id"], "id" => $join_id, "deltime" => 0));
        if (!empty($joinModel)) {
            if ($joinModel["status"] == 1) {
                $joinGroupModel = pdo_get(ztbNopreTable("obj_whole_group"), array("uniacid" => $uniacid, "store_id" => $store_id, "activity_id" => $activity["id"], "openid" => $joinModel["openid"], "deltime" => 0));
                $joinGroupList = pdo_getall(ztbNopreTable("obj_whole_group"), array("pid" => $joinGroupModel["pid"], "deltime" => 0), '', '', "id asc");
            } else {
                $joinGroupList = null;
            }
        }
    }
    $current_group_list = pdo_getall(ztbNopreTable("obj_whole_group"), array("uniacid" => $uniacid, "store_id" => $store_id, "activity_id" => $activity["id"], "is_heads" => 1, "status" => 0, "deltime" => 0), '', '', "id asc");
    foreach ($current_group_list as $key => $value) {
        $current_group_list[$key]["user"] = pdo_getall(ztbNopreTable("obj_whole_group"), array("pid" => $value["id"], "deltime" => 0), '', '', "id asc");
    }
    ...
    $joinUserList = pdo_fetchall("SELECT nickname,headurl FROM " . ztbTable("obj_whole_join") . "where `uniacid`=:uniacid and `store_id`=:store_id and `activity_id`=:activity_id and `deltime`=0  order by id desc LIMIT 14 ", array(":uniacid" => $uniacid, ":store_id" => $store_id, ":activity_id" => $activity["id"]));
    ...
    if (count($buyUserList) < 20 && intval($activity["bogus_buy_num"]) > 0) {
        ...
        foreach ($userList as $user) {
            ...
            if ($is_insert) {
                $insertUser = array("nickname" => $user["nickname"], "headurl" => $user["avatar"], "createtime" => $activity["start_time"], "paymoney" => $object["front_money"], "mymoney" => 0);
    ...
    unset($d_user);
    $topUserList = pdo_getall(ztbTable("obj_whole_join", false), array("uniacid" => $uniacid, "store_id" => $store_id, "deltime" => 0, "activity_id" => $activity["id"]), array("nickname", "headurl", "joins", "bogus_joins", "money", "bogus_money"), '', "joins`+`bogus_joins desc,money`+`bogus_money desc", array(10));
...
if ($act == "join" && $request_method == "post") {
    ...
    $join_model = pdo_get("lywywl_ztb_obj_whole_join", array("uniacid" => $uniacid, "store_id" => $store_id, "activity_id" => $activity["id"], "openid" => $openid, "deltime" => 0));
    if ($join_model) {
        exit(json_encode(array("status" => 1, "msg" => "参与成功，邀请好友参团可获得奖励哦！"), JSON_UNESCAPED_UNICODE));
    }
    $registerFields = serialize($registerFields);
    $insert_join_model = array("uniacid" => $uniacid, "store_id" => $store_id, "activity_types" => $activity_types, "activity_id" => $activity["id"], "origin_team_id" => $originModel["team_id"], "origin_id" => $origin_id, "openid" => $openid, "nickname" => $userinfo["nickname"], "headurl" => $userinfo["headimgurl"], "qrcode_url" => '', "register_data" => $registerFields, "status" => 0, "joins" => 0, "money" => 0, "createtime" => TIMESTAMP);
    $join_group_model = pdo_get(ztbNopreTable("obj_whole_group"), array("uniacid" => $uniacid, "activity_id" => $activity["id"], "openid" => $openid, "deltime" => 0));
    if ($join_group_model) {
        $insert_join_model["status"] = 1;
    }
...
if ($act == "share" && $request_method == "post") {
    ...
    if (empty($join_model["qrcode_url"])) {
        ...
        if ($qr !== false) {
            pdo_update(ztbTable("obj_whole_join", false), array("qrcode_url" => $file_path), array("id" => $join_model["id"]));
...
if ($act == "register" && $request_method == "post") {
    ...
    unset($item);
    $group_model = pdo_get("lywywl_ztb_obj_whole_group", array("uniacid" => $uniacid, "store_id" => $store_id, "activity_id" => $activity["id"], "openid" => $openid, "deltime" => 0));
    if ($group_model) {
        if ($group_model["status"] == 0) {
            exit(json_encode(array("status" => 0, "msg" => "对不起，您已经参团！"), JSON_UNESCAPED_UNICODE));
        } else {
            exit(json_encode(array("status" => 0, "msg" => "对不起，您已经成团！"), JSON_UNESCAPED_UNICODE));
        }
    }
    ...
    $pid = max(0, intval($_GPC["pid"]));
    if ($pid > 0) {
        $p_group_model = pdo_get(ztbNopreTable("obj_whole_group"), array("uniacid" => $uniacid, "store_id" => $store_id, "activity_id" => $activity["id"], "id" => $pid, "deltime" => 0));
        if (empty($p_group_model)) {
            exit(json_encode(array("status" => 0, "msg" => "对不起，参团信息不存在或已删除，请重新参团或开团！"), JSON_UNESCAPED_UNICODE));
        }
        if ($p_group_model["status"] == 1) {
            exit(json_encode(array("status" => 0, "msg" => "对不起，参团人数已满，请重新参团或开团！"), JSON_UNESCAPED_UNICODE));
        }
    } else {
        if ($object["open_num"] > 0) {
            $countOpenNum = pdo_count(ztbNopreTable("obj_whole_group"), array("uniacid" => $uniacid, "activity_types" => $activity_types, "activity_id" => $activity["id"], "is_heads" => 1, "deltime" => 0));
            if ($countOpenNum >= $object["open_num"]) {
                exit(json_encode(array("status" => 0, "msg" => "对不起，活动可开团数已满，请点击参团购买！"), JSON_UNESCAPED_UNICODE));
            }
        }
    }
    isetcookie("reg_objwhole_" . $object["id"], base64_encode(serialize($registerFields)), 3600 * 24);
    exit(json_encode(array("status" => 1, "msg" => "登记成功！", "pid" => $pid), JSON_UNESCAPED_UNICODE));
}
if ($act == "pay") {
    ...
    $register_data = $_GPC["reg_objwhole_" . $object["id"]];
...
if ($act == "getPayNumber" && $request_method == "post") {
    ...
    if ($is_pay) {
        ...
        $payModel["terminal"] = 1;
        $payModel["types"] = 3;
        $payModel["data"] = "{$token},{$join_id},{$pid},{$origin_id}";
        $payModel["openid"] = $userModel["openid"];
        $payModel["nickname"] = $userModel["nickname"];
        $payModel["headurl"] = $userModel["headurl"];
        ...
        $payModel["activity_id"] = $activity["id"];
        pdo_insert(ztbTable("sys_pay", false), $payModel);
        isetcookie("reg_objwhole_" . $object["id"], '', -3600 * 24);
        $group_data = array("uniacid" => $uniacid, "store_id" => $store_id, "activity_types" => $activity_types, "activity_id" => $activity["id"], "origin_team_id" => $originModel["team_id"], "origin_id" => $origin_id, "invite_id" => $joinModel["id"], "pid" => $pid, "openid" => $userModel["openid"], "nickname" => $userModel["nickname"], "headurl" => $userModel["headurl"], "is_heads" => $pid == 0 ? 1 : 0, "front_money" => $object["money"], "pay_number" => $payModel["paynumber"], "register_data" => $object["buy_is_register"] == 1 ? base64_decode($register_data) : '', "status" => 0, "is_settlement" => $config["store_settlement_types"] == 1 ? 0 : 1, "endmoney" => $payModel["endmoney"], "createtime" => TIMESTAMP);
        if ($object["models"] == 3) {
            $group_data["is_settlement"] = 1;
        }
        pdo_insert(ztbNopreTable("obj_whole_group"), $group_data);
        $obj_group_id = pdo_insertid();
        if ($pid == 0) {
            $pid = pdo_insertid();
            pdo_update(ztbNopreTable("obj_whole_group"), array("pid" => $pid), array("id" => $pid));
        }
        ...
        if ($join_model) {
            pdo_update(ztbNopreTable("obj_whole_join"), array("status" => 1), array("id" => $join_model["id"]));
        } else {
            $insert_join_model = array("uniacid" => $uniacid, "store_id" => $store_id, "activity_types" => $activity_types, "activity_id" => $activity["id"], "origin_team_id" => $originModel["team_id"], "origin_id" => $origin_id, "openid" => $userModel["openid"], "nickname" => $userModel["nickname"], "headurl" => $userModel["headurl"], "qrcode_url" => '', "register_data" => $object["buy_is_register"] == 1 ? base64_decode($register_data) : '', "status" => 1, "joins" => 0, "money" => 0, "createtime" => TIMESTAMP);
            pdo_insert("lywywl_ztb_obj_whole_join", $insert_join_model);
            pdo_update(ztbTable("obj_activity", false), array("join_num +=" => 1), array("id" => $activity["id"]));
            if (!empty($origin_id)) {
                pdo_update(ztbNopreTable("marketing_user"), array("join_num +=" => 1), array("id" => $origin_id));
            }
        }
        if ($group_data["is_settlement"] == 1) {
            pdo_update(ztbNopreTable("store_account"), array("money +=" => floatval($payModel["endmoney"])), array("id" => $store_id));
            $note = "参与活动：" . date("Y-m-d H:i:s", TIMESTAMP) . " 会员【" . $userModel["nickname"] . "】参与活动：" . $activity["title"] . "，拼团支付：" . floatval($payModel["paymoney"] + $payModel["mymoney"]) . "元";
            $storeBillModel = array();
            $storeBillModel["uniacid"] = $uniacid;
            $storeBillModel["store_id"] = $store_id;
            $storeBillModel["types"] = 12;
            $storeBillModel["detail_id"] = $obj_group_id;
        ...
        $note = "参与活动：" . date("Y-m-d H:i:s", TIMESTAMP) . " 会员【" . $userModel["nickname"] . "】参与活动：" . $activity["title"] . "，拼团支付：" . floatval($object["money"]) . "元";
        $userBillModel = array();
        ...
        $userBillModel["detail_id"] = $obj_group_id;
        ...
        $groupList = pdo_getall(ztbNopreTable("obj_whole_group"), array("uniacid" => $uniacid, "pid" => $pid, "deltime" => 0), '', '', "is_heads desc");
        if (intval($object["buy_is_push"]) == 1 && $config["grouppushtmp"]) {
            if (isFollow($userModel["openid"], $uniacid)) {
                $postdata = array("first" => array("value" => "{$userModel["nickname"]}，您参与了新的拼团活动", "color" => "#173177"), "keyword1" => array("value" => $activity["title"], "color" => "#173177"), "keyword2" => array("value" => date("Y-m-d H:i:s", TIMESTAMP), "color" => "#173177"), "keyword3" => array("value" => $object["group_num"], "color" => "#173177"), "keyword4" => array("value" => count($groupList), "color" => "#173177"), "remark" => array("value" => "您已经成功参团，" . (count($groupList) >= intval($object["group_num"]) ? "拼团成功！" : "还差" . ($object["group_num"] - count($groupList)) . "人成团！快来喊朋友们一批拼吧!!"), "color" => "#173177"));
                $url = replaceDieDomain($config, __MURL("scan", array("token" => $activity["token"]), true, true), 0, 0, 0);
                $template_id = $config["grouppushtmp"];
                $result = sendTplNotice($uniacid, $userModel["openid"], $postdata, $template_id, $url);
            }
            if (!empty($groupList)) {
                if ($object["group_num"] > 2 && $object["group_num"] > count($groupList) && count($groupList) > 1) {
                    if (isFollow($groupList[0]["openid"], $uniacid)) {
                        $postdata = array("first" => array("value" => "{$groupList[0]["nickname"]}，您发起的拼团活动有新用户参团", "color" => "#173177"), "keyword1" => array("value" => $activity["title"], "color" => "#173177"), "keyword2" => array("value" => date("Y-m-d H:i:s", TIMESTAMP), "color" => "#173177"), "keyword3" => array("value" => $object["group_num"], "color" => "#173177"), "keyword4" => array("value" => count($groupList), "color" => "#173177"), "remark" => array("value" => "好友[" . $userModel["nickname"] . "]成功参团，" . (count($groupList) >= intval($object["group_num"]) ? "拼团成功！" : "还差" . ($object["group_num"] - count($groupList)) . "人成团！"), "color" => "#173177"));
                        $url = replaceDieDomain($config, __MURL("scan", array("token" => $activity["token"], "origin_id" => $groupList[0]["origin_id"]), true, true), 0, 0, 0);
                        $template_id = $config["grouppushtmp"];
                        $result = sendTplNotice($uniacid, $groupList[0]["openid"], $postdata, $template_id, $url);
                    }
                }
            }
        }
        if (intval($object["buy_is_push"]) == 1 && $config["grouppushtmp_sub"] && !empty($groupList)) {
            if ($object["group_num"] > 2 && $object["group_num"] > count($groupList) && count($groupList) > 1) {
                $touser = $groupList[0]["openid"];
                $template_id = $config["grouppushtmp_sub"];
                $postdata = array("thing5" => array("value" => $activity["title"]), "thing3" => array("value" => "新用户参团"), "character_string7" => array("value" => count($groupList) . "/" . $object["group_num"]), "thing4" => array("value" => "好友[" . $userModel["nickname"] . "]成功参团！"));
                $url = replaceDieDomain($config, __MURL("scan", array("token" => $activity["token"], "origin_id" => $groupList[0]["origin_id"]), true, true), 0, 0, 0);
                $result = sendWeixinTemplate($touser, $template_id, $postdata, $url);
            }
        }
        if (count($groupList) >= intval($object["group_num"])) {
            foreach ($groupList as $item) {
                if ($item["status"] == 0) {
                    pdo_update(ztbNopreTable("obj_whole_group"), array("status" => 1, "is_settlement" => 1), array("id" => $item["id"]));
                    $origin_buy_types = 0;
                    if (!empty($item["invite_id"])) {
                        $groupJoinModel = pdo_get(ztbNopreTable("obj_whole_join"), array("id" => $item["invite_id"]));
                        if (!empty($item["origin_id"])) {
                            $groupOriginModel = pdo_get(ztbNopreTable("marketing_user"), array("id" => $item["origin_id"]));
                            if ($groupJoinModel["openid"] == $groupOriginModel["openid"]) {
                                $origin_buy_types = 1;
                                pdo_update(ztbNopreTable("marketing_user"), array("buy_num +=" => 1), array("id" => $item["origin_id"]));
                            }
                        }
                    }
                    $groupPayModel = pdo_get(ztbNopreTable("sys_pay"), array("uniacid" => $uniacid, "paynumber" => $item["pay_number"]));
                    $draw_data = array("uniacid" => $uniacid, "store_id" => $store_id, "activity_types" => $activity_types, "activity_id" => $activity["id"], "origin_team_id" => $item["origin_team_id"], "origin_id" => $item["origin_id"], "origin_buy_types" => $origin_buy_types, "prize_id" => 0, "types" => -1, "name" => $activity["title"], "pay_openid" => $groupJoinModel["openid"], "pay_nickname" => $groupJoinModel["nickname"], "pay_headurl" => $groupJoinModel["headurl"], "pay_number" => $item["pay_number"], "paymoney" => $groupPayModel["paymoney"], "mymoney" => $groupPayModel["mymoney"], "endmoney" => $groupPayModel["endmoney"], "is_settlement" => $item["is_settlement"], "prize_pic_url" => $activity["pic_url"], "writeoff_types" => $object["is_fedex"], "openid" => $item["openid"], "nickname" => $item["nickname"], "headurl" => $item["headurl"], "register_data" => $item["register_data"], "updatetime" => TIMESTAMP, "createtime" => TIMESTAMP);
                    if ($object["models"] != 3) {
                    ...
                    if ($object["models"] == 2) {
                        $models_qr_content = pdo_getcolumn(ztbNopreTable("obj_whole"), array("id" => $object["id"]), "models_qr_content");
                        $models_qr_content = unserialize($models_qr_content);
                        if ($object["models_qr_types"] == 0) {
                            foreach ($models_qr_content as $key => $val) {
                                if ($val["Count"] == -1 || $val["Count"] > $val["Use"]) {
                                    ...
                                    pdo_update(ztbNopreTable("obj_whole"), array("models_qr_content" => iserializer($models_qr_content)), array("id" => $object["id"]));
                                }
                            }
                        } else {
                    ...
                    if (!empty($item["origin_id"])) {
                        pdo_update(ztbNopreTable("marketing_user"), array("marketing_num +=" => 1), array("id" => $item["origin_id"]));
                    }
                    $groupUserModel = pdo_get(ztbNopreTable("user_account"), array("uniacid" => $uniacid, "store_id" => $store_id, "deltime" => 0, "openid" => $item["openid"]));
                    $groupOriginModel = pdo_get(ztbNopreTable("marketing_user"), array("uniacid" => $uniacid, "store_id" => $store_id, "deltime" => 0, "id" => $item["origin_id"]));
                    ...
                    if (intval($object["buy_is_sms"]) == 1 && $storeModel["sms"] > 0) {
                        if (!empty($object["buy_sms_tmp"])) {
                            if (!empty($groupUserModel["mobile"])) {
                                $sms_uid = $config["sms_uid"];
                                $sms_key = $config["sms_key"];
                                $mobile = $groupUserModel["mobile"];
                                $sms_content = $object["buy_sms_tmp"];
                                $sms_content = str_replace("{NICKNAME}", $groupUserModel["nickname"], $sms_content);
                    ...
                    if (intval($object["buy_is_push"]) == 1 && $config["grouppushtmp"] && $item["openid"]) {
                        if (isFollow($item["openid"], $uniacid)) {
                            $postdata = array("first" => array("value" => "尊贵的会员，您的亲友团组团成功", "color" => "#173177"), "keyword1" => array("value" => $activity["title"], "color" => "#173177"), "keyword2" => array("value" => date("Y-m-d H:i:s", $item["createtime"]), "color" => "#173177"), "keyword3" => array("value" => $object["group_num"], "color" => "#173177"), "keyword4" => array("value" => $object["group_num"], "color" => "#173177"), "remark" => array("value" => "点击查看订单详情", "color" => "#173177"));
                            $url = replaceDieDomain($config, __MURL("obj_whole", array("act" => "detail", "pay_number" => $item["pay_number"], "token" => $activity["token"], "origin_id" => $item["origin_id"]), true, true), 0, 0, 0);
                            $template_id = $config["grouppushtmp"];
                            $result = sendTplNotice($uniacid, $item["openid"], $postdata, $template_id, $url);
                        }
                    }
                    if (intval($object["buy_is_push"]) == 1 && $config["grouppushtmp_sub"] && $item["openid"]) {
                        $touser = $item["openid"];
                        $template_id = $config["grouppushtmp_sub"];
                        $postdata = array("thing5" => array("value" => $activity["title"]), "thing3" => array("value" => "拼团成功"), "character_string7" => array("value" => $object["group_num"] . "/" . $object["group_num"]), "thing4" => array("value" => "恭喜，您参与的活动已拼团成功！"));
                        $url = replaceDieDomain($config, __MURL("obj_whole", array("act" => "detail", "pay_number" => $item["pay_number"], "token" => $activity["token"], "origin_id" => $item["origin_id"]), true, true), 0, 0, 0);
                        $result = sendWeixinTemplate($touser, $template_id, $postdata, $url);
                    }
                    if ($config["orderpushtmp"] && $storeModel["openid"]) {
                        if (isFollow($storeModel["openid"], $uniacid)) {
                            $postdata = array("first" => array("value" => "{$storeModel["name"]}，您的活动有新的订单", "color" => "#173177"), "keyword1" => array("value" => $item["pay_number"], "color" => "#173177"), "keyword2" => array("value" => $activity["title"], "color" => "#173177"), "keyword3" => array("value" => $draw_data["paymoney"] + $draw_data["mymoney"] . "元", "color" => "#173177"), "keyword4" => array("value" => "已支付", "color" => "#173177"), "keyword5" => array("value" => date("Y-m-d H:i:s", $item["createtime"]), "color" => "#173177"), "remark" => array("value" => "【{$groupUserModel["nickname"]}】" . $groupUserModel["mobile"], "color" => "#173177"));
                            $url = replaceDieDomain($config, __MURL("store.obj_whole_pay", array("act" => "index", "aid" => $activity["id"]), true, true), 0, 0, 0);
                    ...
                    if ($config["orderpushtmp_sub"] && $storeModel["openid"]) {
                        ...
                        $postdata = array("thing13" => array("value" => $activity["title"]), "character_string10" => array("value" => $item["pay_number"]), "amount1" => array("value" => $draw_data["paymoney"] + $draw_data["mymoney"]), "time12" => array("value" => date("Y-m-d H:i:s", $item["createtime"])), "thing9" => array("value" => "购买用户：{$groupUserModel["nickname"]}"));
                        $url = replaceDieDomain($config, __MURL("store.obj_whole_pay", array("act" => "index", "aid" => $activity["id"]), true, true), 0, 0, 0);
                        $result = sendWeixinTemplate($touser, $template_id, $postdata, $url);
                    }
                    if ($config["originpushtmp"] && !empty($item["origin_id"])) {
                        if (isFollow($groupOriginModel["openid"], $uniacid)) {
                            $postdata = array("first" => array("value" => "{$groupOriginModel["name"]}，您参与的溯源活动有新的订单", "color" => "#173177"), "keyword1" => array("value" => $draw_data["paymoney"] + $draw_data["mymoney"] . "元", "color" => "#173177"), "keyword2" => array("value" => $activity["title"], "color" => "#173177"), "keyword3" => array("value" => $item["pay_number"], "color" => "#173177"), "keyword4" => array("value" => "【{$groupUserModel["nickname"]}】" . $groupUserModel["mobile"], "color" => "#173177"), "remark" => array("value" => "点击查看溯源统计数据", "color" => "#173177"));
                            $url = replaceDieDomain($config, __MURL("user.admins_marketing", array("store_id" => $groupOriginModel["store_id"], "activity_id" => $groupOriginModel["activity_id"]), true, true), 0, 0, 0);
                            $template_id = $config["originpushtmp"];
                            $result = sendTplNotice($uniacid, $groupOriginModel["openid"], $postdata, $template_id, $url);
                        }
                    }
                    if ($config["orderpushtmp_sub"] && !empty($item["origin_id"])) {
                        $touser = $item["origin_id"];
                        $template_id = $config["orderpushtmp_sub"];
                        $postdata = array("thing13" => array("value" => $activity["title"]), "character_string10" => array("value" => $item["pay_number"]), "amount1" => array("value" => $draw_data["paymoney"] + $draw_data["mymoney"]), "time12" => array("value" => date("Y-m-d H:i:s", $item["createtime"])), "thing9" => array("value" => "溯源用户：{$groupUserModel["nickname"]}"));
                        $url = replaceDieDomain($config, __MURL("user.admins_marketing", array("store_id" => $groupOriginModel["store_id"], "activity_id" => $groupOriginModel["activity_id"]), true, true), 0, 0, 0);
                        $result = sendWeixinTemplate($touser, $template_id, $postdata, $url);
                    }
                }
        ...
        if (!empty($joinModel)) {
            if ($joinModel["openid"] != $userModel["openid"]) {
                $params = ["uniacid" => $uniacid, "store_id" => $store_id, "activity_types" => $activity_types, "origin_id" => $origin_id, "object" => $object, "originModel" => $originModel, "userModel" => $userModel, "activity" => isset($activity) ? $activity : $object, "objJoinTableName" => "obj_whole_join"];
                pdo_update(ztbTable("obj_whole_join", false), array("joins +=" => 1), array("id" => $joinModel["id"]));
                $joinUserModel = pdo_get(ztbTable("user_account", false), array("uniacid" => $uniacid, "store_id" => $object["store_id"], "deltime" => 0, "openid" => $joinModel["openid"]));
                ...
                if ($plug_ladder) {
                    if (isset($object["or_open_ladder"]) && $object["or_open_ladder"] == 1) {
                        $joinInvite = intval($joinModel["joins"]) + 1;
                ...
                if ($plug_rebate2) {
                    if (isset($object["or_open_rebate2"]) && $object["or_open_rebate2"] == 1) {
                        $group2Model = pdo_get(ztbNopreTable("obj_whole_group"), array("deltime" => 0, "uniacid" => $uniacid, "activity_id" => $activity["id"], "openid" => $joinModel["openid"]));
                        if (!empty($group2Model)) {
                            $join2Model = pdo_get(ztbTable("obj_whole_join", false), array("uniacid" => $uniacid, "activity_id" => $activity["id"], "id" => $group2Model["invite_id"]));
                            if (!empty($join2Model)) {
                                if ($join2Model["openid"] != $userModel["openid"] && $join2Model["openid"] != $joinModel["openid"]) {
                                    pdo_update(ztbTable("obj_whole_join", false), array("joins +=" => 1), array("id" => $join2Model["id"]));
        ...
        thread_unlock($openid);
        resultMsg(["status" => 1, "ispay" => 0, "pay_number" => $group_data["pay_number"], "pay_price" => $object["money"], "msg" => "请求成功！"]);
    }
}
if ($act == "checkPay") {
    $pay_number = $_GPC["pay_number"];
        ...
        if (intval($payModel["status"]) == 0) {
            ...
            if ($success && intval(strval($payModel["paymoney"] * 100)) == $paymoney) {
                ...
                if ($payToken == $token) {
                    ...
                    if (!empty($join_id)) {
                        $joinModel = pdo_get(ztbTable("obj_whole_join", false), array("uniacid" => $uniacid, "id" => $join_id, "deltime" => 0));
                    }
                    isetcookie("reg_objwhole_" . $object["id"], '', -3600 * 24);
                    ...
                    if (intval($object["buy_is_push"]) == 1 && $config["grouppushtmp"]) {
                        if (isFollow($userModel["openid"], $uniacid)) {
                            $postdata = array("first" => array("value" => "{$userModel["nickname"]}，您参与了新的拼团活动", "color" => "#173177"), "keyword1" => array("value" => $activity["title"], "color" => "#173177"), "keyword2" => array("value" => date("Y-m-d H:i:s", TIMESTAMP), "color" => "#173177"), "keyword3" => array("value" => $object["group_num"], "color" => "#173177"), "keyword4" => array("value" => count($groupList), "color" => "#173177"), "remark" => array("value" => "您已经成功参团，" . (count($groupList) >= intval($object["group_num"]) ? "拼团成功！" : "还差" . ($object["group_num"] - count($groupList)) . "人成团！快来喊朋友们一批拼吧!!"), "color" => "#173177"));
                            $url = replaceDieDomain($config, __MURL("scan", array("obj_token" => $token), false, TRUE));
                            $template_id = $config["grouppushtmp"];
                            $result = sendTplNotice($uniacid, $userModel["openid"], $postdata, $template_id, $url);
                        }
                        if (!empty($groupList)) {
    ...
    header("Location: " . __MURL("obj_whole", array("act" => "detail", "pay_number" => $pay_number, "token" => $activity["token"], "origin_id" => $origin_id, "incoming" => "pay")));
    exit;
}
if ($act == "detail") {
    $pay_number = $_GPC["pay_number"];
    ...
    if (empty($drawModel)) {
        ...
        if (!empty($group_model)) {
            header("Location: " . __MURL("obj_whole", array("act" => "success", "token" => $activity["token"], "origin_id" => $origin_id, "incoming" => "pay")));
            exit;
        }
...
if ($act == "success") {
    ...
    $_share["link"] = replaceDieDomain($config, $_share["link"], $user["id"], $activity["id"], $user["id"]);
    include $this->template("tmp/" . $tmp["resource"] . "/success");
    exit;
}