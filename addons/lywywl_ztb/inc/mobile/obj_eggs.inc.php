...
if ($plug_appad) {
    $plug_appad_act = getPluginConfig($_W["uniacid"], "lywywl_ztb_plugin_appad", "is_open_eggs");
}
if (empty($token)) {
...
$object = pdo_get(ztbNopreTable("obj_eggs"), array("deltime" => 0, "activity_id" => $activity["id"]));
...
if ($act == "index") {
    ...
    if ($object["is_show_ads"] == 1) {
        if (empty($_GPC["ad_eggs_" . $object["id"]])) {
            $object["is_show_ads"] = 1;
            isetcookie("ad_eggs_" . $object["id"], "ad", 3600 * 24);
        } else {
    ...
    if (intval($object["is_register"]) > 0) {
        ...
        unset($item);
        $register_data = $_GPC["reg_objeggs_" . $object["id"]];
    ...
    if (intval($object["models"]) == 0 && intval($object["is_invite_friend"]) > 0) {
        ...
        if (!empty($_GET["i_openid"])) {
            ...
            isetcookie("i_openid_eggs_" . $object["id"], $_GET["i_openid"], 3600 * 24);
        } else {
            $i_openid = $_GPC["i_openid_eggs_" . $object["id"]];
        }
    }
...
if ($act == "register" && $request_method == "post") {
    ...
    unset($item);
    isetcookie("reg_objeggs_" . $object["id"], base64_encode(serialize($registerFields)), 3600 * 24 * 365);
