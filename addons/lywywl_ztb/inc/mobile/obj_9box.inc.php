...
if ($plug_appad) {
    $plug_appad_act = getPluginConfig($_W["uniacid"], "lywywl_ztb_plugin_appad", "is_open_9box");
}
if (empty($token)) {
...
$object = pdo_get(ztbNopreTable("obj_9box"), array("deltime" => 0, "activity_id" => $activity["id"]));
...
if ($act == "index") {
    ...
    if ($object["is_show_ads"] == 1) {
        if (empty($_GPC["ad_9box_" . $object["id"]])) {
            $object["is_show_ads"] = 1;
            isetcookie("ad_9box_" . $object["id"], "ad", 3600 * 24);
        } else {
    ...
    if (intval($object["is_register"]) > 0) {
        ...
        unset($item);
        $register_data = $_GPC["reg_obj9box_" . $object["id"]];
    ...
    if (intval($object["models"]) == 0 && intval($object["is_invite_friend"]) > 0) {
        ...
        if (!empty($_GET["i_openid"])) {
            ...
            isetcookie("i_openid_9box_" . $object["id"], $_GET["i_openid"], 3600 * 24);
        } else {
            $i_openid = $_GPC["i_openid_9box_" . $object["id"]];
        }
    }
...
if ($act == "draw" && $request_method == "post") {
    ...
    if ($object["area_limit"]) {
        ...
        if ($is_in_area === false) {
            thread_unlock($openid);
            exit(json_encode(array("status" => 0, "msg" => "亲，您所在的区域不在本次活动范围！", "code" => $config["is_open_area_black"]), JSON_UNESCAPED_UNICODE));
        }
    }
...
if ($act == "register" && $request_method == "post") {
    ...
    unset($item);
    isetcookie("reg_obj9box_" . $object["id"], base64_encode(serialize($registerFields)), 3600 * 24 * 365);
