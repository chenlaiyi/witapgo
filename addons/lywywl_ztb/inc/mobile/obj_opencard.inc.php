...
$act = trim($_GPC["act"]);
$allow_acts = array("index", "draw", "register");
...
if ($plug_appad) {
    $plug_appad_act = getPluginConfig($_W["uniacid"], "lywywl_ztb_plugin_appad", "is_open_opencard");
}
if (empty($token)) {
...
$object = pdo_get(ztbNopreTable("obj_opencard"), array("deltime" => 0, "activity_id" => $activity["id"]));
...
if ($act == "index") {
    ...
    if ($object["is_show_ads"] == 1) {
        if (empty($_GPC["ad_opencard_" . $object["id"]])) {
            $object["is_show_ads"] = 1;
            isetcookie("ad_opencard_" . $object["id"], "ad", 3600 * 24);
        } else {
    ...
    if (intval($object["is_register"]) > 0) {
        ...
        unset($item);
        $register_data = $_GPC["reg_objopencard_" . $object["id"]];
    ...
    if (intval($object["models"]) == 0 && intval($object["is_invite_friend"]) > 0) {
        ...
        if (!empty($_GET["i_openid"])) {
            ...
            isetcookie("i_openid_opencard_" . $object["id"], $_GET["i_openid"], 3600 * 24);
        } else {
            $i_openid = $_GPC["i_openid_opencard_" . $object["id"]];
        }
    }
...
if ($act == "draw" && $request_method == "post") {
    ...
    $prize_icon = $prizeList[$rid]["icon"];
    unset($prizeList[$rid]);
    shuffle($prizeList);
    $prizePicList = array();
    foreach ($prizeList as $key => $val) {
        $prizePicList[$key] = tomedia($val["icon"]);
    }
    $result_data = array("status" => 1, "draw_id" => $draw_id, "prize_id" => $prizeModel["id"], "prize_types" => $prizeModel["types"], "prize_name" => $prizeModel["name"], "prize_picurl" => tomedia($prizeModel["picurl"]), "prize_icon" => tomedia($prize_icon), "prize_other" => implode(",", $prizePicList), "score" => $prizeModel["score"], "sys" => $prizeModel["sys"], "money" => $prizeModel["money"], "card_id" => $prizeModel["card_id"], "is_register" => $object["is_register"], "writecode" => $writecode, "writeoff_types" => $prizeModel["writeoff_types"]);
    thread_unlock($openid);
...
if ($act == "register" && $request_method == "post") {
    ...
    unset($item);
    isetcookie("reg_objopencard_" . $object["id"], base64_encode(serialize($registerFields)), 3600 * 24 * 365);
