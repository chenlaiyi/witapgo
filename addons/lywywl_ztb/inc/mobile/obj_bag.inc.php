...
$act = trim($_GPC["act"]);
$allow_acts = array("index", "draw", "register", "bag");
...
if ($plug_appad) {
    $plug_appad_act = getPluginConfig($_W["uniacid"], "lywywl_ztb_plugin_appad", "is_open_bag");
}
if (empty($token)) {
...
$object = pdo_get(ztbNopreTable("obj_bag"), array("deltime" => 0, "activity_id" => $activity["id"]));
...
if ($act == "index") {
    ...
    if ($object["is_show_ads"] == 1) {
        if (empty($_GPC["ad_bag_" . $object["id"]])) {
            $object["is_show_ads"] = 1;
            isetcookie("ad_bag_" . $object["id"], "ad", 3600 * 24);
        } else {
    ...
    if (intval($object["models"]) == 0 && intval($object["is_invite_friend"]) > 0) {
        if (intval($object["get_types"]) == 0) {
            $inviteCount = pdo_fetchcolumn("SELECT COUNT(*) FROM " . ztbTable("obj_invite") . " WHERE activity_id = :activity_id AND openid = :openid", array(":activity_id" => $object["activity_id"], ":openid" => $openid));
            if (intval($object["invite_limit_num"]) > 0) {
                $inviteCount = min(intval($object["invite_limit_num"]), $inviteCount);
            }
        } else {
            $inviteCount = pdo_fetchcolumn("SELECT COUNT(*) FROM " . ztbTable("obj_invite") . " WHERE activity_id = :activity_id AND FROM_UNIXTIME(createtime, '%Y%m%d') = DATE_FORMAT(NOW(),'%Y%m%d') AND openid = :openid", array(":activity_id" => $object["activity_id"], ":openid" => $openid));
            if (intval($object["invite_limit_num"]) > 0) {
                $inviteCount = min(intval($object["invite_limit_num"]), $inviteCount);
            }
        }
    ...
    if (intval($object["get_types"]) == 0) {
        $count = pdo_fetchcolumn("SELECT COUNT(*) FROM " . ztbTable("user_draw") . " WHERE deltime = 0 AND activity_id = :activity_id AND openid = :openid", array(":activity_id" => $object["activity_id"], ":openid" => $openid));
        $surplusCount = $count >= $object["limit_num"] + $inviteCount ? 0 : $object["limit_num"] + $inviteCount - $count;
    }
    if (intval($object["get_types"]) == 1) {
        $count = pdo_fetchcolumn("SELECT COUNT(*) FROM " . ztbTable("user_draw") . " WHERE deltime = 0 AND activity_id = :activity_id  AND FROM_UNIXTIME(createtime, '%Y%m%d') = DATE_FORMAT(NOW(),'%Y%m%d') AND openid = :openid", array(":activity_id" => $object["activity_id"], ":openid" => $openid));
        $surplusCount = $count >= $object["limit_num"] + $inviteCount ? 0 : $object["limit_num"] + $inviteCount - $count;
    }
    $registerTipShow = 0;
    if (intval($object["is_register"]) > 0) {
        ...
        unset($item);
        $register_data = $_GPC["reg_objbag_" . $object["id"]];
    ...
    if ($object["is_show_winners"] > 0) {
        ...
        $prizeList = pdo_getall(ztbNopreTable("obj_prize"), array("deltime" => 0, "activity_id" => $object["activity_id"], "status" => 1, "types >" => 0), array("name", "odds"), '', "sort asc");
    ...
    if (intval($object["models"]) == 0 && intval($object["is_invite_friend"]) > 0) {
        ...
        if (!empty($_GET["i_openid"])) {
            ...
            isetcookie("i_openid_bag_" . $object["id"], $_GET["i_openid"], 3600 * 24);
        } else {
            $i_openid = $_GPC["i_openid_bag_" . $object["id"]];
        }
    }
    ...
    $drawLink = replaceDieDomain($config, $this->createMobileUrl("obj_bag", array("act" => "bag", "token" => $token, "origin_id" => $origin_id, "i_openid" => $i_openid)), $user["id"], $object["activity_id"], $user["id"]);
...
if ($act == "bag") {
    ...
    $_share["link"] = replaceDieDomain($config, $_share["link"], $user["id"], $object["activity_id"], $user["id"]);
    $activityLink = replaceDieDomain($config, $this->createMobileUrl("obj_bag", array("act" => "index", "token" => $token, "origin_id" => $origin_id, "i_openid" => $i_openid)), $user["id"], $object["activity_id"], $user["id"]);
    if (!empty($object["audio_url"]) && strpos($object["audio_url"], "/addons/lywywl_ztb/") !== false) {
        $object["audio_url"] = substr($object["audio_url"], strpos($object["audio_url"], "/addons/lywywl_ztb/"));
    }
    $template = pdo_get(ztbNopreTable("sys_tmp"), array("deltime" => 0, "id" => $activity["tmp_id"]));
    $resource = $template["resource"];
    $tmpDir = "tmp/" . $resource;
    include $this->template("{$tmpDir}/bag");
    exit;
}
if ($act == "draw" && $request_method == "post") {
    ...
    if (intval($object["models"]) == 0 && intval($object["is_invite_friend"]) > 0) {
            ...
            if ($count >= intval($object["limit_num"]) + $inviteCount) {
                ...
                if (intval($object["invite_limit_num"]) > 0) {
                    if (intval($object["invite_limit_num"]) <= $inviteCount) {
                        exit(json_encode(array("status" => 0, "msg" => "亲，您本次活动的抽奖次数已用完！"), JSON_UNESCAPED_UNICODE));
                    } else {
                        exit(json_encode(array("status" => 0, "tip" => "join", "msg" => "亲，您参与本次活动已达到抽奖上限，分享朋友圈邀请好友参与抽奖可获得一次抽奖机会！"), JSON_UNESCAPED_UNICODE));
                    }
                } else {
                    exit(json_encode(array("status" => 0, "tip" => "join", "msg" => "亲，您参与本次活动已达到抽奖上限，分享朋友圈邀请好友参与抽奖可获得一次抽奖机会！"), JSON_UNESCAPED_UNICODE));
                }
        ...
        if (intval($object["get_types"]) == 1) {
            ...
            if ($count >= intval($object["limit_num"]) + $inviteCount) {
                ...
                if (intval($object["invite_limit_num"]) > 0) {
                    if (intval($object["invite_limit_num"]) <= $inviteCount) {
                        exit(json_encode(array("status" => 0, "msg" => "亲，您今天的抽奖次数已用完！"), JSON_UNESCAPED_UNICODE));
                    } else {
                        exit(json_encode(array("status" => 0, "tip" => "join", "msg" => "亲，系统每日限制抽奖" . $object["limit_num"] . "次，分享朋友圈邀请好友参与抽奖可获得一次抽奖机会！"), JSON_UNESCAPED_UNICODE));
                    }
                } else {
                    exit(json_encode(array("status" => 0, "tip" => "join", "msg" => "亲，系统每日限制抽奖" . $object["limit_num"] . "次，分享朋友圈邀请好友参与抽奖可获得一次抽奖机会！"), JSON_UNESCAPED_UNICODE));
                }
        ...
        if (intval($object["models"]) == 1) {
            if (intval($userAccount["score"]) < intval($object["cost_score"])) {
                thread_unlock($openid);
                exit(json_encode(array("status" => 0, "msg" => "您的账户积分不足,无法进行抽奖"), JSON_UNESCAPED_UNICODE));
            }
        } else {
            if (intval($object["get_types"]) == 0) {
                $count = pdo_fetchcolumn("SELECT COUNT(*) FROM " . ztbTable("user_draw") . " WHERE deltime = 0 AND activity_id = :activity_id AND openid = :openid", array(":activity_id" => $object["activity_id"], ":openid" => $openid));
    ...
    if ($is_invate_draw) {
        if (intval($object["get_types"]) == 0) {
            $inviteModel = pdo_get(ztbNopreTable("obj_invite"), array("activity_id" => $object["activity_id"], "openid" => $openid, "status" => 0), array("id"));
        }
        if (intval($object["get_types"]) == 1) {
            $inviteModel = pdo_get(ztbNopreTable("obj_invite"), array("activity_id" => $object["activity_id"], "openid" => $openid, "status" => 0, "createtime >" => strtotime(date("Y-m-d"))), array("id"));
        }
        if (!empty($inviteModel)) {
            pdo_update(ztbNopreTable("obj_invite"), array("status" => 1), array("id" => $inviteModel["id"]));
        }
    }
    if (intval($object["models"]) == 1) {
        $note = "抽奖支付：" . timeToStr(TIMESTAMP) . " 会员【" . $userinfo["nickname"] . "】参与活动：" . $activity["title"] . "，支付积分：" . $object["cost_score"];
    ...
    if (intval($object["models"]) == 0 && intval($object["is_invite_friend"]) > 0) {
        ...
        if (!empty($i_openid) && $i_openid != $openid) {
            $objInvite = pdo_get(ztbNopreTable("obj_invite"), array("uniacid" => $uniacid, "activity_id" => $object["activity_id"], "openid" => $i_openid, "invite_openid" => $openid));
            if (empty($objInvite)) {
                $data = array("uniacid" => $uniacid, "store_id" => $store_id, "activity_types" => $activity_types, "activity_id" => $object["activity_id"], "openid" => $i_openid, "invite_openid" => $openid, "draw_id" => $draw_id, "status" => 0, "createtime" => TIMESTAMP);
                pdo_insert(ztbNopreTable("obj_invite"), $data);
            }
        }
    }
    $storeAccount = pdo_get(ztbNopreTable("store_account"), array("id" => $store_id));
...
if ($act == "register" && $request_method == "post") {
    ...
    unset($item);
    isetcookie("reg_objbag_" . $object["id"], base64_encode(serialize($registerFields)), 3600 * 24 * 365);
    exit(json_encode(array("status" => 1, "msg" => "登记成功,开始抽奖吧！"), JSON_UNESCAPED_UNICODE));
}