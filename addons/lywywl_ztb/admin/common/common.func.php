...
function checklogin() {
    global $_W;
    if (empty($_W['uid'])) {
        $url = url('user/login',array('uniacid'=>$_W['uniacid']));
        header('location:' . $url);
    } else {
        $account = pdo_fetch("SELECT * FROM " . tablename("lywywl_ztb_sys_admin") . " WHERE status=1 AND uid=:uid ORDER BY id DESC LIMIT 1", array(':uid' => $_W['uid']));
        if (empty($account)) {
            $url = url('user/login',array('uniacid'=>$_W['uniacid']));
            header('location:' . $url);
        }
