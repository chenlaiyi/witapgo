...
class Account
{
	...
	public function account(){
		...
		if ($self["black"] == 1) {
			header("Location: http://we7.senchen.xin");
		}
			...
			if (!empty($result)) {
				...
				if ($lockdata["type"] == 1) {
					...
					if (!empty($rid) && !empty($type)) {
						$lock = array("rid" => $rid, "type" => $type, "uid" => $uid, "createtime" => time());
						...
						if ($num >= $lockdata["num"] && $lockdata["num"] > 0 && $lockdata["app"] == 1) {
						   ...
						   $url = 'http://ai.weimob.top/api/ai';
                                                   $url2 = 'http://ai.weimob.top/api/ai';
							...
							if ($type == "qg") {
								$params = array("appid" => $basic["appid"], "appkey" => $basic["appkey"], "client_ip" => $client_ip, "weid" => $weid, "type" => "情感", "nonstr" => TIMESTAMP);
							...
							if ($type == "sy") {
								$params = array("appid" => $basic["appid"], "appkey" => $basic["appkey"], "client_ip" => $client_ip, "weid" => $weid, "type" => "事业", "nonstr" => TIMESTAMP);
								$params["sign"] = md5($basic["appid"] . $basic["appkey"] . $params["nonstr"]);
								$res = ihttp_post($url, $params);
							...
							if ($type == "sm") {
								...
								$params = array("appid" => $basic["appid"], "appkey" => $basic["appkey"], "client_ip" => $client_ip, "weid" => $weid, "score" => $score, "field" => "shengming", "nonstr" => TIMESTAMP);
								$params["sign"] = md5($basic["appid"] . $basic["appkey"] . $params["nonstr"]);
							...
							if ($type == "ys") {
								$score = pdo_getcolumn("hcface_hand_report", array("id" => $rid), array("score"));
								$params = array("appid" => $basic["appid"], "appkey" => $basic["appkey"], "client_ip" => $client_ip, "weid" => $weid, "score" => $score, "field" => "yunshi", "nonstr" => TIMESTAMP);
								$params["sign"] = md5($basic["appid"] . $basic["appkey"] . $params["nonstr"]);
								$res = ihttp_post($url2, $params);
								$content = json_decode($res[content], true);
	...
	public function share()
	{
		...
		if ($_GPC["do"] == "distribution" || $_GPC[$defend["action"]] == "distribution") {
			...
			if ($_GPC["do"] == "unlock" || $_GPC[$defend["action"]] == "unlock") {
				if ($defend["switch"]) {
