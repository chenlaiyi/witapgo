...
//FAST客户支付配置
/*define("FAST_APPKEY", "");//你的appkey
define("SECRET_KEY", "");//你的秘钥*/
define('PAY_DESC',"xx平台给你汇款");//汇款备注
define('PAY_RETURN_URL',"http://www.baidu.com");//支付后成功后返回的页面,
define('PAY_NOTIFY_URL',"");//异步回调页面,不填写默认就是会员中心的

define('PAY_OPENID',0);//是否开启代汇款接口,0,关闭,1,为开启,如果页面不能进入或者有错误,请关闭


/**----汇款接口获取openid接口的域名---------**/
define("API_OPENID", 'http://xxx.cn/code.php?v='.PAY_VERSION);//如果需要汇款接口,请联系客服索取域名开通
...
//获取下单地址
function fastpay_order($paydata,$type="https"){
if(!is_array($paydata)){
exit("data错误");
}
$data = array(
...
$url=($type=='https') ? "/addons/hc_face/fastpay/fpay/pays.php?{$url_quer}" : "/addons/hc_face/fastpay/fpay/pay.php?{$url_quer}" ;
return $url;
}
...
function get_openid($scope="snsapi_base"){
  ...
  if(stripos(PAY_OPENID,"xxx.cn")!==false){
