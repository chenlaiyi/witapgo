...
// footer
function goodsList(tabId, tabitem, tabDiv, tabDivs, sort,img_bg_face,img_bg_hand,img_bottom_bg_face,img_bottom_bg_hand) {
	tab.apply(this, arguments);
	this.sort = sort
	console.log(sort)
	this.tabDiv = document.getElementsByClassName(tabDiv);
	this.tabDivs = document.getElementsByClassName(tabDivs);
	this.img_bg_face = img_bg_face;
	this.img_bg_hand = img_bg_hand;
	this.img_bottom_bg_face = img_bottom_bg_face;
	this.img_bottom_bg_hand = img_bottom_bg_hand;
}
...
goodsList.prototype.show = function (i) {
	...
	if (i == 1) {
		$(".firstDiv").css({
			"background": "url('"+this.img_bg_hand+"') no-repeat",
			"background-size": "100% 100%"
		})
		$(".upload_photo").css({
			"background": "url('"+this.img_bottom_bg_hand+"') no-repeat",
			"background-size": "100% 100%"
		})
	} else {
		
		$(".firstDiv").css({
			"background": "url('"+this.img_bg_face+"') no-repeat",
			"background-size": "100% 100%"
		})
		$(".upload_photo").css({
			"background": "url('"+this.img_bottom_bg_face+"') no-repeat",
			"background-size": "100% 100%"
		})
	}
	$(this.tabDiv[i]).show().siblings().hide()
