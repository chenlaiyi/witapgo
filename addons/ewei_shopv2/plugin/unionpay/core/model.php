...
class UnionpayModel extends PluginModel
{
    ...
    public function getSub($id){
        $res = pdo_fetchall("SELECT * FROM ".tablename('ewei_shop_unionpay_agent')." where upagentid = ".$id);
        $uplevel1 = count($res);
        if ($res) {
            $upids = '';
            foreach ($res as $k => $v) {
    ...
    public function getSubIds($id,$keys=0){
        if($keys){
            $res = pdo_fetchall("SELECT * FROM ".tablename('ewei_shop_unionpay_agent')." where upagentid in (".$id.")");
        }else{
            $res = pdo_fetchall("SELECT * FROM ".tablename('ewei_shop_unionpay_agent')." where upagentid = ".$id);
        }
        if ($res) {
                ...
                if( empty($become_child) ) 
                {
                    if( empty($member["fixagentid"]) ) 
                    {
                        ...
                        else
                        {
                            ...
                            {
                                pdo_update("ewei_shop_member", array( "agentid" => $parent["id"], "childtime" => $time, "fixagentid" => 1 ), array( "uniacid" => $_W["uniacid"], "id" => $member["id"] ));
                            }
                ...
                else
                {
                    ...
                    else
                    {
                        if( $set["become"] == 4 ) 
                        {
                            ...
                            $goodscount = pdo_fetchcolumn("select count(*) from " . tablename("ewei_shop_order_goods") . " og " . "  left join " . tablename("ewei_shop_order") . " o on o.id = og.orderid" . " where og.goodsid IN (" . implode(",", $set["become_goodsid"]) . ") and o.openid=:openid and o.status>=" . $order_status . "  limit 1", array( ":openid" => $openid ));
                            if( 0 < $goodscount ) 
                            {
                                $to_check_agent = true;
                            }
