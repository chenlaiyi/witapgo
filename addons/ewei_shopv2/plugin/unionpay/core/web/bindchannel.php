...
class Bindchannel_EweiShopV2Page extends WebPage {
    ...
    function main() {
        ...
        $profile_levelname  =   pdo_get('ewei_shop_commission_level', array('uniacid' => $_W['uniacid'], 'id' => $profile['agentlevel']), array('levelname'));
        $upinfo =  pdo_fetch('select * from ' . tablename('ewei_shop_unionpay_agent') . ' where upid=:upid and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':upid' => $id));
        $uplevel    =   pdo_getall('ewei_shop_unionpay_level', array('uniacid' => $_W['uniacid']), array('upagentlevel', 'levelname'));
        if(!empty($upinfo)){
        ...
        if ($_W['ispost']) {
            //查询已绑定的机构编号
            $check_channelId = pdo_fetchall("SELECT hnapay_channelId, newland_channelId FROM ".tablename('ewei_shop_unionpay_agent'). ' where upid<>:upid and uniacid=:uniacid', array(':uniacid' => $_W['uniacid'], ':upid' => $id));
            ...
            if(!empty($upinfo)){
                //更新数据
                pdo_update('ewei_shop_unionpay_agent', $data, array('upid' => $id));
                $logupid    =   $id;
            }else{
                //插入新记录
                $data['upupdatetime']   =   date("Y-m-d H:i:s" ,time());
                pdo_insert('ewei_shop_unionpay_agent', $data);
