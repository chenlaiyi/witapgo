...
//        }
        ...
        //获取分润等级列表
        $uplevellist    =   pdo_getall('ewei_shop_unionpay_level', array('uniacid' => $_W['uniacid']), array('upagentlevel', 'levelname'));

        //根据拼接好的条件进行查询
        //用户表的             昵称          头像 分销等级表会员等级名称 分销商等级
        $sql = "select dm.*,dm.nickname,dm.avatar,l.levelname,p.agentlevel as parentagentlevel, p.mobile as parentmobile,p.id as pid,p.nickname as parentname,p.avatar as parentavatar,f.follow as followed,p.realname as parentrealname, f.unfollowtime,u.* from " . tablename('ewei_shop_member') . " dm "
            ...
            . " left join " . tablename('ewei_shop_unionpay_agent') . " u on dm.id = u.upid"//银联结构表uid和用户表ID
