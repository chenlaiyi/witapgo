...
class Store_EweiShopV2Page extends AppMobilePage
{
    ...
    public function selector()
    {
        ...
        if (0 < $merchid) {
            ...
            $list = pdo_fetchall("select * from " . tablename("ewei_shop_merch_store") . " where  uniacid=:uniacid and merchid=:merchid and status=1 " . $condition . " order by displayorder desc,id desc", $params);
        } else {
            $list = pdo_fetchall("select * from " . tablename("ewei_shop_store") . " where  uniacid=:uniacid and status=1 " . $condition . " order by displayorder desc,id desc", $params);
        }
