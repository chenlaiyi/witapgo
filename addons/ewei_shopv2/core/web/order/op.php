        ...
        if ($_W['ispost']) {
            ...
            if($item['city_express_state']==0){
                ...
                if($cityexpress['express_type']==1){
                    $ret = pdo_fetchall('select goodsid , total , optionid from ' . tablename('ewei_shop_order_goods') . ' where orderid = :orderid and uniacid = :uniacid',array(':uniacid'=>$_W['uniacid'],':orderid'=>$item['id']));
                    $weight = 0;
                foreach ($ret as $key=>$val)
                {
                    if ($val['optionid'] > 0)
                    {
                        $weight += pdo_fetchcolumn(' select weight*' . $val['total'] . ' from ' . tablename('ewei_shop_goods_option') . ' where goodsid=:id and id = :optionid',array(':id'=>$val['goodsid'],':optionid'=>$val['optionid']));
                    }else
                    {
                        $weight += (pdo_fetchcolumn("select weight* ".$val['total']." from " . tablename('ewei_shop_goods'). ' where id=:id',array(':id'=>$val['goodsid'])));
                    }
                }
                    if ($weight == 0)
                    {
