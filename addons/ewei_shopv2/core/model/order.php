...
//                }
    ...
    {//查询是否是代付订单,如果不是返回false,如果是返回代付订单内容
        global $_W;
        $sql = "SELECT p.*,o.openid FROM " . tablename('ewei_shop_order_peerpay') . " AS p JOIN " . tablename('ewei_shop_order') . " AS o ON p.orderid = o.id WHERE p.orderid = :orderid  AND (p.status = 0 OR p.status=1) AND o.status >= 0 LIMIT 1";
