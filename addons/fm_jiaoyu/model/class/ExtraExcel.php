...
class ExtraExcel
{
    ...
    public function DDExtraExcelWriter($params, $filename = '')
    {
        ...
        if($MONTH == '12'){
            $TYEAR = $YEAR + 1;
            $TNE = 1;
            $endTime = strtotime($TYEAR.'-'.$TNE.'-01 00:00') - 1;
        }
        $dateCount = date("d", $endTime);
        // 考勤记录
        $checklist = pdo_fetchall("SELECT * FROM ".GetTableName('checklog')." WHERE schoolid = '{$schoolid}' and weid = '{$weid}' and createtime >= '{$startTime}' and createtime <= '{$endTime}' and sid = 0 and tid != 0 ORDER BY createtime ASC ");
        //请假记录
        ...
        $schoolset = pdo_fetch("SELECT dd_repet_time,dd_limit_time FROM ".GetTableName('schoolset')." WHERE weid = '{$weid}' and schoolid = '{$schoolid}' ");
        $repT = $schoolset['dd_repet_time'] * 60;
        $limitTime = $schoolset['dd_limit_time']*60 - 1;
        //组装考勤数据
        foreach ($checklist as $vc) {
            ...
            if (!empty($teacherlist[$vc['tid']])) {
                if (!empty($teacherlist[$vc['tid']]['log']['all'][$date]['content'])) {
                    $coT = count($teacherlist[$vc['tid']]['log']['all'][$date]['content']) - 1;
                    $lastC = $teacherlist[$vc['tid']]['log']['all'][$date]['content'][$coT];
                    if (($temp['type'] == 2 && $lastC['type'] == 1)  || ($lastC['type'] == $temp['type']  &&  $temp['time'] - $lastC['time'] < $repT)) { //处理限定时间重复刷卡不算作无匹配的刷卡
                        $temp['orange'] = 0;
                        $CotA = count($teacherlist[$vc['tid']]['log']['am'][$date]['content']) - 1;
                        $lastCA = $teacherlist[$vc['tid']]['log']['am'][$date]['content'][$CotA];
                        $CotP = count($teacherlist[$vc['tid']]['log']['pm'][$date]['content']) - 1;
                        $lastCP = $teacherlist[$vc['tid']]['log']['pm'][$date]['content'][$CotP];
                        if($lastCA['time'] == $lastC['time']){
                            $teacherlist[$vc['tid']]['log']['am'][$date]['content'][$CotA]['orange'] = 0;
                        }
                        if($lastCP['time'] == $lastC['time']){
                            $teacherlist[$vc['tid']]['log']['pm'][$date]['content'][$CotP]['orange'] = 0;
                        }
                    }
                    if ($limitTime > 0) {
                        if ($temp['type'] == 1 && $lastC['type'] == 2 && $lastC['spical'] == 1 && $temp['spical'] == 1) { //先出后进且原本的类型为异常
                            if ($temp['time'] - $lastC['time'] < $limitTime) {
                                $temp['spical'] = 0;
                                $CotA = count($teacherlist[$vc['tid']]['log']['am'][$date]['content']) - 1;
                                $lastCA = $teacherlist[$vc['tid']]['log']['am'][$date]['content'][$CotA];
                                $CotP = count($teacherlist[$vc['tid']]['log']['pm'][$date]['content']) - 1;
                                $lastCP = $teacherlist[$vc['tid']]['log']['pm'][$date]['content'][$CotP];
                                if ($lastCA['time'] == $lastC['time']) {
                                    $teacherlist[$vc['tid']]['log']['am'][$date]['content'][$CotA]['spical'] = 0;
                                }
                                if ($lastCP['time'] == $lastC['time']) {
                                    $teacherlist[$vc['tid']]['log']['pm'][$date]['content'][$CotP]['spical'] = 0;
                                }
                            }
                        }
                    }
                ...
                $teacherlist[$vc['tid']]['log']['all'][$date]['content'][] = $temp;
            }
            // spical
            ...
            for ($i=1;$i<=$dateCount;$i++) { //填充星期和日期
                ...
                if (!empty($vt['log']['am'][$i])) { //如果当天上午有数据
                    ...
                    foreach ($didata as $vdd) { //构造单元格内的文字
                        ...
                        if ($vdd['orange'] == 1) {
                            $spicallist[$kv]['spdata']['spical']['3']['am']++; //计算异常
                        }
                    }
                ...
                if (!empty($vt['log']['pm'][$i])) { //如果当天下午有数据
                    ...
                    foreach ($didata as $vdd) { //构造单元格内的文字
                        ...
                        if ($vdd['orange'] == 1) {
                            $spicallist[$kv]['spdata']['spical']['3']['pm']++; //计算异常
                        }
                    }
                    if ($vt['log']['pm'][$i]['type'] == 'leave') { //如果当前天是请假
                        $spicallist[$kv]['spdata']['leave']['pm']++; //计算请假
        ...
        foreach ($sheet0data['data'] as $kv=>$vt) {
            ...
            for ($i=1;$i<=$dateCount;$i++) { //填充星期和日期
                ...
                if (!empty($vt['log']['am'][$i])) { //如果当天上午有数据
                    ...
                    $objRichText = new RichText();
                    foreach ($didata as $vdd) { //构造单元格内的文字
                        if ($vdd['spical'] == 1 || $vdd['leave'] == 1 || $vdd['orange'] == 1) {
                            $objPayable = $objRichText->createTextRun($vdd['word']."\r\n");
                            $objPayable->getFont()->setSize(10);
                            $objPayable->getFont()->setName('微软雅黑');
                            if ($vdd['spical'] == 1){
