...
function getMyAllWelfare($openid,$schoolid,$type){
    $welfare = pdo_fetchall("SELECT wf.id,w.cose,w.maxrange FROM ".GetTableName('welfarelog')." as wf LEFT JOIN " . GetTableName('welfare') . " as w ON wf.welfareid = w.id AND w.type = '{$type}' LEFT JOIN " . GetTableName('special') . " as s ON s.shareval = w.id WHERE wf.openid = '{$openid}' AND wf.schoolid = '{$schoolid}' AND wf.status = 1 AND wf.time <= (w.day * 86400 + unix_timestamp(now())) AND s.start <= unix_timestamp(now()) ");
    return $welfare;
}
