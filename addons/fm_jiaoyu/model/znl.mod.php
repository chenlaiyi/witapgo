 ...
 * @LastEditTime: 2020-11-25 14:58:51
 */
...
function znlGetReportNew($schoolid,$sid = 0,$param = []){
    $sendIunfo = array(
        'deptNo' => $schoolid,
        'page' => $param['page'],
        'rows' => $param['rows'],
    ...
    switch ($type) {
        case 1:
            $ReportType = 'month';
            $ReportTitle = '月度报告';
            $sendIunfo['currentYear'] = ''.$param['year'];
            $sendIunfo['currentMonth'] = ''.$param['month'];
            break;
        case 2:
            $ReportType = 'semester';
            $xqinfo = pdo_fetch("SELECT cn_yearid,sname FROM ".GetTableName('classify')." WHERE sid = '{$param['xq_id']}' ");
            $ReportTitle = "{$xqinfo['sname']}报告";
            if(!empty($xqinfo['cn_yearid'])){
                $sendIunfo['cnbotsSemesterId'] = ''.$xqinfo['cn_yearid'];
            }
            else {
                return false;
            }
    ...
    // dd($sendIunfo);
  $res = json_decode(znl_http_get_json('/cnbots/rest/kindergarten/openapi/user/studentHealthReport/'.$ReportType, $sendIunfo, $schoolid), true);
  if($res['resultCode'] == 0){
      foreach ($res['result']['results'] as $value) {
          $weid = pdo_fetch("SELECT weid FROM ".GetTableName('index')." WHERE id = '{$value['deptNo']}'")['weid'];
          $hasreport = pdo_fetch("SELECT id FROM ".GetTableName('mcreportlist')." WHERE schoolid = '{$value['deptNo']}' AND sid = '{$value['studentNo']}' AND gettime = '{$value['createTime']}' AND type = '{$type}' ");
          if(empty($hasreport['id'])){
              $value['temperatureHigher'] = $value['temperatureHigh'];
              $reportData = array(
                  'weid' => $weid,
                  'schoolid' => $value['deptNo'],
                  'sid' => $value['studentNo'],
