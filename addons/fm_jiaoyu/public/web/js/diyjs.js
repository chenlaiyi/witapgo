	...
	function showImageDialogmfs_new(elm, opts, options) {
		require(["util"], function(util){
			var btn = $(elm);
			var ipt = btn.parent().prev();
			var val = ipt.val();
			...
			util.image(val, function(url){
				if(url.url){
					...
					ipt.attr("filename",url.filename);
					ipt.attr("url",url.url);
					$("#"+pp).find("div").css("background",'url(' + url.url+ ') no-repeat');
					$("#"+pp).find("div").css("background-size",'100% 100%');
				}
				if(url.media_id){
					if(img.length > 0){
						img.get(0).src = "";
					...
					ipt.val(url.media_id);
					$("#"+pp).find("div").css("background",'url(' + url.media_id+ ') no-repeat');
					$("#"+pp).find("div").css("background-size",'100% 100%');
				}
			}, null, options);
		});
