...
function Scroll_load(param) {
    ...
    this.limit = $(ScrollDiv).find(param.li_item).eq($(ScrollDiv).find(param.li_item).length-1).attr('time');
    this.limit_org = $(ScrollDiv).find(param.li_item).eq($(ScrollDiv).find(param.li_item).length-1).attr('time');
...
Scroll_load.prototype.load_init = function () {
    ...
    if ($('.has_show_over').length == 0) {
        $(self.ul_box).after('<div class="has_show_over" style="clear:both;height:45px;line-height:45px;display: none"><div class="jzz_div"><div class="jzz jzz_over"><div class="pir"><img src="../addons/fm_jiaoyu/public/mobile/img/p_jzz.png" /></div><div class="jzz_text"></div></div></div></div>');
    }
    ...
    if (($(ScrollDiv).find(self.li_item).length % self.pageSize != 0) || $(ScrollDiv).find(self.li_item).length == 0) {
...
//    console.log(`winHeight:${parseInt(winHeight)}`);
