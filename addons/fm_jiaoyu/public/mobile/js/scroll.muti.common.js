...
function Scroll_load(param) {
    ...
    this.limit_org = $(param.li_item).eq($(param.li_item).length-1).attr('time');
...
Scroll_load.prototype.load_init = function () {
    ...
    if(typeof(ActUl) === 'string'){
        TrueUl = ActUl
    }
    if ($('.has_show_over').length == 0) {
        $(TrueUl).after('<div class="has_show_over" style="clear:both;height:45px;line-height:45px;display: none"><div class="jzz_div"><div class="jzz jzz_over"><div class="pir"><img src="../addons/fm_jiaoyu/public/mobile/img/p_jzz.png" /></div><div class="jzz_text"></div></div></div></div>');
    }
...
function scroll_fun() {
    ...
    if (parseInt(winHeight) + parseInt(scrollTop) + 5 > parseInt(documentHeight)) {
        ...
        // if (self.ajax_switch) {
            ...
            var datanumb = $(TrueUl).children('li').length;
            if(datanumb >= 1){
                $('.has_show_over').show();
            ...
            for(let item of self.param){
                ...
                GetLiData[item] = $(TrueUl).children(`${self.li_item}`).eq($(TrueUl).children(`${self.li_item}`).length-1).attr(`${item}`) || -1 ;
            }
           // this.LiData = GetLiData;
            var post_data = {
                limit: $(TrueUl).children('li').eq($(TrueUl).children('li').length-1).attr('time'),
