 ...
 * @author 高贵血迹
 */global $_W, $_GPC;
   $operation = in_array ( $_GPC ['op'], array ('default','reciveConfirm', 'delmallorder', 'mallorder','sigeup','deleteclass','creatorder','xuefeiidcard','xufeiob','buyvod','xgks','chongzhi','buy_charge','make_kcorder','del_ordertid','get_payinfo_byorderid','CheckBuyManual','CheckWelfare','yuepayRemaster','check_orderpay','check_wxorderpay') ) ? $_GPC ['op'] : 'default';
...
if ($qty['qty'] < $NumOfGood ) {
	...
	if ($operation == 'check_wxorderpay'){
		$orderid = intval($_GPC['orderid']);
		//$complete = $_GPC['result'];
		//CommonWriteLog($complete,'complete');
		$result['result'] = false;
		$order =  pdo_fetch("SELECT id,weid,payweid,wxpayid,pay_type,status,cose FROM " . GetTableName('order') . " WHERE :id = id", array(':id' => $orderid));
		if($order['status'] != 2){
			$core_paylog = pdo_fetch("SELECT plid,uniontid,tag,type,uniacid,openid FROM " . tablename('core_paylog') . " where tid = :tid And uniontid > 0 And module = 'fm_jiaoyu' ", array(':tid' => $order['wxpayid']));
			if(!empty($core_paylog)){
				...
				$query = json_decode(json_encode($wxPay->orderquery($payweid,$core_paylog['uniontid'])),true);
				if($query['trade_state'] == 'SUCCESS'){  //查询微信接口本订单是否支付成功 成功模拟走支付完成处理流程
					pdo_update(GetTableName('order',false), array('uniontid' => $query['transaction_id'],'pay_type' => 'wechat'), array('id' => $orderid));
					$site = WeUtility::createModuleSite('fm_jiaoyu');
					if(!is_error($site)) {
						$site->weid = $order['weid'];
						$site->uniacid = $order['weid'];
						$site->inMobile = true;
						$method = 'payResult';
						if (method_exists($site, $method)) {
							$ret = array();
							...
							$ret['type'] = 'wechat';
							$ret['from'] = 'notify';
							$ret['tid'] = $order['wxpayid'];
							$ret['transaction_id'] = $query['transaction_id'];
							$ret['uniontid'] = $core_paylog['uniontid'];
							$ret['user'] = $core_paylog['openid'];
							$ret['fee'] = $order['cose'];
							$ret['paytime'] = empty($query['time_end'])?time():strtotime($query['time_end']);
							$ret['returnurl'] = 'returnurl';
							$ret['weid'] = $order['weid'];
							$ret['uniacid'] = $order['weid'];
							$ret['acid'] = $order['weid'];
							$ret['is_usecard'] = '';
							$ret['card_type'] = '';
							$ret['card_fee'] = '';
