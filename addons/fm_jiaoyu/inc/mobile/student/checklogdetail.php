 ...
 */
        ...
        if(!empty($it)){
			...
			// 新增log视频抓拍修改链接地址
			if($log['videoid'] && !$log['videourl']){
				mload()->model('ali');
				$aliyun = GetAliApp($weid,$schoolid);
				$appid = $aliyun['alivodappid'];
				$key = $aliyun['alivodkey'];
				$videourl = GetAliVideoUrl($appid,$key,$log['videoid'])['PlayURL'];
				$videocover = GetAliVideoCover($appid,$key,$log['videoid'])['CoverURL'];
				if($videourl){
					$videoarr = array(
						'videourl' => $videourl,
						'videocover' => $videocover,
					);
					pdo_update(GetTableName('checklog',false),$videoarr,array('id'=>$log['id']));
				}
			}
