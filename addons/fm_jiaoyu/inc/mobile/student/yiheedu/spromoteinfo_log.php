...
if(!empty($it)){
    ...
    //推广信息
    $promoteinfo = pdo_fetch("SELECT count(id) as promote_num, IFNULL (SUM(CASE WHEN status = 1 then 1 else 0 end),0) as buy_num,SUM(score) as score FROM ".GetTableName('yiheedu_share_log')." WHERE schoolid = '{$schoolid}' AND pu_id = '{$_GPC['myuserid']}' AND kc_id = '{$_GPC['kc_id']}' ");
    if($op == 'getData'){
        ...
        $promote_log = pdo_fetchAll("SELECT id,fans_openid,score,from_unixtime(createtime,'%Y-%m-%d') as createtime,status FROM ".GetTableName('yiheedu_share_log')." WHERE schoolid = '{$schoolid}' AND pu_id = '{$_GPC['myuserid']}' AND kc_id = '{$_GPC['kc_id']}' $condition ORDER BY createtime DESC LIMIT 0,6");
        $logList = [];
        foreach ($promote_log as $key => $value) {
            $fans = mc_fansinfo($value['fans_openid'],0,$weid);
            $logList[$key]['id'] = $value['id'];
            $logList[$key]['icon'] = $fans['avatar'];
            $logList[$key]['nickname'] = $fans['nickname'];
            $logList[$key]['time'] = $value['createtime'];
            $logList[$key]['action'] = $value['status'] == 1 ? 'buy' : 'click';
            $logList[$key]['point'] = $value['score'];
        }
	...
	include $this->template('students/yiheedu/spromoteinfo_log');
}else{
	session_destroy();
