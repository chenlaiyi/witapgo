 ...
 */
		...
		if ($od1){
			...
			$wxpay = pdo_fetch("SELECT * FROM " . GetTableName('wxpay') . " WHERE weid = '{$weid}' And schoolid = '{$schoolid}' And od1 = '{$od1}'");
			if (empty($wxpay)) {
			...
			// );	
			// $paylog = pdo_fetch("SELECT id FROM " . tablename('core_paylog') . " WHERE uniacid = '{$uniacid}' And module = 'fm_jiaoyu' And tid = '{$wxpay_id}'");
			// if (empty($paylog)) {
				// $paylog = array(
					// 'uniacid' => $uniacid,
					// 'acid' => $uniacid,
