 ...
 */
        ...
        if(!empty($it)){
			...
			foreach($list as $key => $row){
				...
				$yhprice = 0;
				if($row['couponlogid']){
					$coupon = getMyWelfare($row['couponlogid'],2);
					$list[$key]['yhq'] = $coupon;
					$yhprice += $coupon['cose'];
				}
				if($row['redpacketlogid']){
					$redpacket = getMyWelfare($row['redpacketlogid'],2);
					$list[$key]['yhq'] = $redpacket;
					$yhprice += $redpacket['cose'];
				}

				$list[$key]['yhcose'] = round(max(0,$row['cose'] - $yhprice),2);
