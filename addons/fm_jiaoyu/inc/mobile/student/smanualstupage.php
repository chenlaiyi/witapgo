...
$bjstu =pdo_fetchall("SELECT * FROM " . tablename($this->table_students) . " WHERE weid = '{$weid}' AND schoolid = '{$schoolid}' And bj_id = '{$bj_id}' AND id = '{$_GPC['sid']}' ");
...
// 获取个人相册
$bjstuVideo =pdo_fetchall("SELECT * FROM " . tablename($this->table_students) . " WHERE weid = '{$weid}' AND schoolid = '{$schoolid}' And bj_id = '{$bj_id}' AND id = '{$_GPC['sid']}' ");
