...
if(!empty($it)){
    ...
    if($op == 'display'){
        ...
        foreach($list as $k=>$v) {
            $AllowShare = pdo_fetch("SELECT id,pdffile,is_ok_stu,isallok FROM ".GetTableName('growuppage')." WHERE sid = '{$it['sid']}' AND gid = '{$v['id']}' AND schoolid = '{$schoolid}'");
            $list[$k]['thumb'] =  !empty($v['thumb']) ? tomedia($v['thumb']) : tomedia($school['logo']);
            $list[$k]['createtime'] =  date("Y-m-d",$v['createtime']);
            $list[$k]['AllowShare'] = $AllowShare['isallok'] == 1 ? true : false;
            $list[$k]['is_ok_stu'] = $AllowShare['is_ok_stu'];
            if($AllowShare['isallok'] == 1){
