...
if ($operation == 'ChangeCardSpic') {
    ...
    if (empty($card['id'])) {
        $rand_idcard = '';
        $kongka = pdo_fetch("SELECT id,idcard FROM " . tablename($this->table_idcard) . " WHERE schoolid = :schoolid AND tid = 0 AND sid = 0 AND pard = 0 ORDER BY id DESC", array(':schoolid' => $schoolid));
        $rAndStr = str_shuffle('0123456789');
        do{
            // $rand_idcard  = time();
            $rand_idcard = substr($rAndStr, 0, 10);
...
if($operation == 'sendManual'){
    ...
    // }
    if($_GPC['type'] == 'stu'){
        pdo_update(GetTableName('growuppage',false),array('is_ok_stu'=>1),array('gid'=>$_GPC['gid'],'sid'=>$_GPC['sid']));
    }else{
        pdo_update(GetTableName('growuppage',false),array('is_send'=>1),array('gid'=>$_GPC['gid'],'sid'=>$_GPC['sid']));
        $this->sendMobileManual($_GPC['sid'],$_GPC['gid'], $_GPC['schoolid'], $_GPC['weid']);
    }
    // 修改发送状态 
    $result['result'] = true;
    $result['msg'] = '操作成功';
...
if($operation == 'SavePage'){
    ...
    $PageList = json_decode(htmlspecialchars_decode($_GPC['PageList']),true);
...
if ($operation == 'getTempCate') {
    if (empty($_GPC ['schoolid'])) {
        ...
        $tempCateList = pdo_fetchAll("SELECT id,name FROM " . GetTableName('template_library_category') . " where weid=:weid", array(':weid' => $_GPC['weid']));
        $tempList = pdo_fetchall("SELECT * FROM ".GetTableName('template_library')." WHERE weid = '{$_GPC['weid']}' AND status = 2");
        include $this->template('comtool/templibrary');
        die;
    }
  }
if ($operation == 'getTempLibrary') {
    if (empty($_GPC ['schoolid'])) {
        ...
        $tempList = pdo_fetchall("SELECT id,content,name FROM ".GetTableName('template_library')." WHERE weid = '{$_GPC['weid']}' AND status = 2 $condition");
