 ...
 */
        ...
        $CheckTea = pdo_fetch("SELECT status FROM ".GetTableName('teachers')." WHERE id = '{$tid_global}' ");
        if($CheckTea['status'] != 2){
          $condition .= " AND tid = '{$tid_global}' ";
          $condition2 .= " AND d.tid = '{$tid_global}' ";
        }

        $BjList = pdo_fetchall("SELECT cls.sid,cls.sname FROM ".GetTableName('druglog')." as d LEFT JOIN ".GetTableName('students')." as s ON d.sid = s.id LEFT JOIN ".GetTableName('classify')." as cls ON cls.sid = s.bj_id  WHERE d.weid = '{$weid}' and d.schoolid = '{$schoolid}' {$condition2} GROUP BY cls.sid ORDER BY cls.ssort DESC ");
        ...
        $CheckTea = pdo_fetch("SELECT status FROM ".GetTableName('teachers')." WHERE id = '{$tid_global}' ");
        if($CheckTea['status'] != 2){
          $conditionF .= " AND d.tid = '{$tid_global}' ";
          $condition .= " AND d.tid = '{$tid_global}' ";
        }
				if(!empty($_GPC['starttime']) && !empty($_GPC['endtime']) ) {
					$starttime = strtotime($_GPC['starttime']);
					$endtime = strtotime($_GPC['endtime']) + 86399;
          $condition .= " AND (d.datetime >= '{$starttime}' AND d.datetime <= '{$endtime}')";
          $conditionF .= " AND (d.datetime >= '{$starttime}' AND d.datetime <= '{$endtime}')";;
				}
				if($ctype == '1'){
          $condition1 .= " AND status = '{$ctype}'";
          $conditionF .= " AND d.status = '{$ctype}' ";
				}elseif($ctype == '2'){
          $condition1 .= " AND status = '0'";
          $conditionF .= " AND d.status = '0' ";
        }
        
        if($_GPC['BjId'] != 0){
          $condition .= " AND s.bj_id = '{$_GPC['BjId']}'";
          $conditionF .= " AND s.bj_id = '{$_GPC['BjId']}'";
        }
        
				$total_1 = pdo_fetchcolumn("SELECT count(*) FROM ".GetTableName('druglog')." as d LEFT JOIN ".GetTableName('students')." as s ON d.sid = s.id LEFT JOIN ".GetTableName('classify')." as cls ON cls.sid = s.bj_id LEFT JOIN ".GetTableName('classify')." as grd ON grd.sid = s.xq_id  WHERE d.schoolid = '{$schoolid}'  {$condition}  AND d.status = '1' ORDER BY d.datetime ASC   ");
				$total_2 = pdo_fetchcolumn("SELECT count(*) FROM ".GetTableName('druglog')." as d LEFT JOIN ".GetTableName('students')." as s ON d.sid = s.id LEFT JOIN ".GetTableName('classify')." as cls ON cls.sid = s.bj_id LEFT JOIN ".GetTableName('classify')." as grd ON grd.sid = s.xq_id  WHERE d.schoolid = '{$schoolid}'  {$condition}  AND d.status = '0' ORDER BY d.datetime ASC   ");
        ...
        $drugloglist = pdo_fetchall("SELECT d.* FROM ".GetTableName('druglog')." as d LEFT JOIN ".GetTableName('students')." as s ON d.sid = s.id LEFT JOIN ".GetTableName('classify')." as cls ON cls.sid = s.bj_id LEFT JOIN ".GetTableName('classify')." as grd ON grd.sid = s.xq_id  WHERE d.schoolid = '{$schoolid}'  {$conditionF} ORDER BY d.datetime ASC LIMIT {$limit_start},6    ");
				foreach($drugloglist as $key => $value){
					$student = pdo_fetch("SELECT s_name,bj_id,xq_id,icon FROM ".GetTableName('students')." WHERE schoolid = '{$schoolid}' and id = '{$value['sid']}' ");
					$bjname = pdo_fetch("SELECT sname FROM ".GetTableName('classify')." WHERE schoolid = '{$schoolid}' and sid = '{$student['bj_id']}'  ");
					$njname = pdo_fetch("SELECT sname FROM ".GetTableName('classify')." WHERE schoolid = '{$schoolid}' and sid = '{$student['xq_id']}'  ");
