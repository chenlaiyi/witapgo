...
if (!empty($it)) {
    ...
    $master = pdo_fetchall("SELECT t.tname ,t.thumb,t.mobile,t.id ,t.status,t.userid,
    CASE
      WHEN fz.sname != NULL THEN fz.sname
      ELSE '校长'
    END as Ttitle
    FROM ".GetTableName('teachers')." as t LEFT JOIN ".GetTableName('classify')." as fz ON t.fz_id = fz.sid  WHERE t.weid = :weid and t.schoolid = :schoolid and t.status = :status and t.is_show = :is_show ORDER BY CONVERT(t.tname USING gbk) ASC ", array( ':weid' => $weid, ':schoolid' => $schoolid, ':status' => 2, ':is_show' => 0 ));
		$masterCount = count($master);
		//所有年级主任组
