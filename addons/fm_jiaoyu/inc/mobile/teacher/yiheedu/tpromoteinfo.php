...
if(!empty($it)){
    ...
    //查询出当前老师的唯一身份
    $school = pdo_fetch("SELECT title FROM ".GetTableName('index')." WHERE id = '{$schoolid}'");
    // $userinfo = pdo_fetch("SELECT * FROM ".GetTableName('yiheedu_promote_user')." WHERE schoolid = '{$schoolid}' AND id = '{$_GPC['myuserid']}' ");
    //课程基本信息
    ...
    $i = 0;
    $level1 = pdo_fetch("SELECT f_id FROM ".GetTableName('yiheedu_promote_relation')." WHERE pu_id = '{$_GPC['myuserid']}' AND kc_id = '{$_GPC['kc_id']}' AND f_id != 0 ");
    if($level1){
        $i++;
    }
    $level2 = pdo_fetch("SELECT f_id FROM ".GetTableName('yiheedu_promote_relation')." WHERE pu_id = '{$level1['f_id']}' AND kc_id = '{$_GPC['kc_id']}' AND f_id != 0 ");
    if($level2){
        $i++;
    }
    $level3 = pdo_fetch("SELECT f_id FROM ".GetTableName('yiheedu_promote_relation')." WHERE pu_id = '{$level2['f_id']}' AND kc_id = '{$_GPC['kc_id']}' AND f_id != 0 ");
    if($level3){
        $i++;
    }
    $is_promote_level = false;
    if($promote_level > $i){
        $is_promote_level= true;
    }

	include $this->template('teacher/yiheedu/tpromoteinfo');
}else{
	session_destroy();
