...
$yearList = pdo_fetchall("SELECT DISTINCT(year) as name FROM ".GetTableName('mcreportlist')." WHERE schoolid = '{$schoolid}' AND year != '' ORDER BY year");

if(!empty($it)){
    $teacher = pdo_fetch("SELECT * FROM ".GetTableName("teachers")." WHERE id = '{$tid_global}' ");
    if($operation == 'display'){
        $year = $yearList[0]['name'];
        $xq = '上学期';
        ...
        mload()->model('znl');
        $param = array(
            'type' => $_GPC['__input']['cate_id'],
            'year' => date("Y"),
            'month' => max(date("n")-1,1),
            'page' => 1,
            'rows' => 20,
        );
        $StudentList = znlGetReportNew($schoolid,$_GPC['__input']['id'],$param);
        $result['status'] =true;
        $result['msg'] = "获取学生报告成功";
        $result['data'] = $StudentList;
        die(json_encode($result));
