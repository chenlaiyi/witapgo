<?php
    ...
    if ($operation == 'addShareRecord') { //增加分享记录
        if (!$_GPC ['schoolid']) {
    ...
    if ($operation == 'addForm'){ //创建表单
        $item = pdo_fetch("SELECT content,tidstr FROM " . GetTableName('special') . " where :id = id", array(':id' => $_GPC['id']));
        $hasform = pdo_fetch("SELECT id FROM ".GetTableName('saleform')." WHERE openid = '{$_GPC['openid']}' AND spid = '{$_GPC['id']}' ");
        if($hasform){
            $result['result'] = false; 
            $result['msg'] = '您已提交过,请等待处理结果!';
            die (json_encode($result));
        }
        ...
        $this->sendMobileYxb($insertId,$_GPC ['schoolid'], $_GPC ['weid']);
        $result['result'] = true; 
