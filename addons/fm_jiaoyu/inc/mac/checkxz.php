...
//noteFace : 讯贞新增的
$operation = in_array($_GPC['op'], array('default', 'login', 'classinfo', 'check', 'gps', 'banner', 'video', 'checknew', 'checkold', 'all', 'banpai', 'allcard', 'getleave', 'onlineflow', 'busgps', 'offlineflow', 'cardinfo', 'checkforks', 'getallroom', 'syncmoney', 'changecard','visitorlog','checkwithurl','testop','noteFace')) ? $_GPC['op'] : 'default';
$weid = $_GPC['i'];
$schoolid = $_GPC['schoolid'];
$macid = empty($_GPC['macid']) ? $_GPC['terminalId'] : $_GPC['macid'];

$oldop = $operation;
if($operation == 'noteFace'){
  $_DDD  = json_decode(file_get_contents("php://input"),true);
  $temp = [];
  $macid = $_DDD['deviceId'];
  $operation = 'checkold';
  $temp['signTime'] = $_DDD['noteTime'];
  $temp['notePic'] = $_DDD['noteImg'];
  $temp['signId'] = $_DDD['employeeIc'];
  $temp['signTemp'] = $_DDD['humTemp'];
  $temp['notePass'] = $_DDD['notePass'];
  $temp['noteWay'] = $_DDD['noteWay'];
  $temp['notePity'] = $_DDD['notePity'];
  $temp['rest'] = $_DDD;
  $datasss[] = $temp;
}
            ...
            if(!empty($class)){
                foreach ($class as $key => $row) {
                    ...
                    if ($cls['type'] != 'jsfz') {
                        if ($ckmac['apid'] == 0) {
                            if ($row['s_type'] != 2) {
                                $class[$key]['timeset'] = array(array('start' => '00:00', 'end' => '23:59','s_type'=>0,'out_in'=>0));
                            } elseif ($row['s_type'] == 2) {
...
if ($operation == 'login') {
    if (!empty($ckmac)) {
        ...
        $result['data']['noteface']   = urlencode($_W['siteroot'] . 'app/index.php?i=' . $weid . '&c=entry&schoolid=' . $schoolid . '&do=checkxz&op=noteFace&m=fm_jiaoyu');
...
if ($operation == 'classinfo') {
    ...
    if ($isfz['type'] == 'theclass') { //如果是获取的班级下的信息
        ...
        $timeSetArr = [];
        foreach ($todaytimeset as $kt => $vt) {
          $timeSetArr[] = $vt;
        }
        $result['data']['todaytype'] = $todaytype;
        $result['data']['todaytimeset'] = $timeSetArr;
        if (!empty($ckmac)) {
            ...
            $fclass = [];
            foreach ($class as $vcla) {
              $fclass[] = $vcla;
            }
            $result['data']['childs'] = $fclass;
            $result['code'] = 1000;
            $result['msg'] = "success";
            $result['ServerTime'] = date('Y-m-d H:i:s', time());
            echo json_encode($result);
    ...
    if (empty($checkthisdata)) {
        if (!empty($ckuser) || $_GPC['signId'] == '9999999999') {
            ...
            if ($ckuser['cardtype'] == 1) {
                //个人卡处理
                if (!empty($ckuser['sid'])) {
                    $bj = pdo_fetch("SELECT bj_id,roomid,isopen,s_type FROM " . tablename($this->table_students) . " WHERE id = :id ", array(':id' => $ckuser['sid']));
                            ...
                            if ($school['send_overtime'] >= 1) {
                                ...
                                if ($overtime >= $timecha) {
                                    if (is_showyl()) {
                                        ...
                                        $allow = hasSendTz($schoolid,$xk_type['is_limit_send'],$xk_type['is_repet'],$bj['bj_id'],$signTime,$leixing,$bj['s_type'],$_GPC['signId'],$ckmac['id']);
                                        if($allow){
    ...
    if (!empty($datasss)) {
        ...
        foreach ($datasss as $k => $v) {
          $signTime = strtotime($v['signTime']);
            ...
            CommonWriteLog('接收数据','check-'.date("H",$signTime),'XunZhen/'.date("Y",$signTime)."/".date("m",$signTime)."/".date("d",$signTime),$IsLog);
       
            CommonWriteLog($v['rest'],'check-'.date("H",$signTime),'XunZhen/'.date("Y",$signTime)."/".date("m",$signTime)."/".date("d",$signTime),$IsLog);
            $fstype = false;
            ...
            if (empty($checkthisdata)) {
                if (!empty($ckuser) || $v['signId'] == '9999999999') {
                    ...
                    if($oldop == 'noteFace'){
                        ...
                        $path = "images/fm_jiaoyu/check/" . date('Y/m/d/');
                        $picurl = $path . random(30) . ".jpg";
                        $base64_string= explode(',', $v['notePic']);
                        $dataaa= base64_decode(str_replace("", "+", $base64_string[1]));
                        file_write($picurl, $dataaa); //写入文件并保存
                        if (!empty($_W['setting']['remote']['type'])) {
                          $remotestatus = file_remote_upload($picurl);
                        }
                        $pic = $picurl;
                    ...
                    if ($v['signId'] == '9999999999') {
                        ...
                        pdo_insert($this->table_checklog, $data);
                        CommonWriteLog('写入数据库','check-'.date("H",$signTime),'XunZhen/'.date("Y",$signTime)."/".date("m",$signTime)."/".date("d",$signTime),$IsLog);
                        $fstype = true;
                    }
                    if ($ckuser['cardtype'] == 1) {
                        if (!empty($ckuser['sid'])) {
                            ...
                            if ($school['is_cardpay'] == 1) {
                                if ($ckuser['severend'] > $times) {
                                    ...
                                    if (!empty($ckmac['apid'])) {
                                        ...
                                        if($oldop == 'noteFace'){
                                          $data['note_pity'] = $v['notePity'];
                                          $data['note_way'] = $v['noteWay'];
                                          $data['note_pass'] = $v['notePass'];
                                        }
                                    }
                                    ...
                                    if ($school['send_overtime'] >= 1) {
                                        ...
                                        if ($overtime >= $timecha) {
                                            if (is_showyl()) {
                                                ...
                                                $allow = hasSendTz($schoolid,$xk_type['is_limit_send'],$xk_type['is_repet'],$bj['bj_id'],$signTime,$leixing,$bj['s_type'],$v['signId'],$ckmac['id']);
                                                if($allow){
                                                    $this->sendMobileJxlxtz($schoolid, $weid, $bj['bj_id'], $ckuser['sid'], $type, $leixing, $checkid, $ckuser['pard']);
                                                }
                                            }
                                            ...
                                            $result['info'] = "延迟发送之数据将不推送刷卡提示";
                                            CommonWriteLog('延迟发送之数据将不推送刷卡提示','check-'.date("H",$signTime),'XunZhen/'.date("Y",$signTime)."/".date("m",$signTime)."/".date("d",$signTime),$IsLog);
                                        }
                                    } else {
                                        if (is_showyl()) {
                                            $this->sendMobileJxlxtz_yl($schoolid, $weid, $ckuser['sid'], $checkid, $ckmac['id']);
                                    ...
                                    $result['info'] = "本卡已失效,请联系学校管理员";
                                    CommonWriteLog('本卡已失效,请联系学校管理员','check-'.date("H",$signTime),'XunZhen/'.date("Y",$signTime)."/".date("m",$signTime)."/".date("d",$signTime),$IsLog);
                                }
                                $fstype = true;
                            } else {
                                if (!empty($ckmac['apid'])) {
                    ...
                    $fstype = true;
                    $result['info'] = "本卡未绑定任何学生或老师";
                    CommonWriteLog('本卡未绑定任何学生或老师','check-'.date("H",$signTime),'XunZhen/'.date("Y",$signTime)."/".date("m",$signTime)."/".date("d",$signTime),$IsLog);

                }
            } else {
                $fstype = true;
                $result['info'] = "不可重复相同刷卡数据";
                CommonWriteLog('不可重复相同刷卡数据','check-'.date("H",$signTime),'XunZhen/'.date("Y",$signTime)."/".date("m",$signTime)."/".date("d",$signTime),$IsLog);

            }
            if ($fstype == true) {
              if($oldop == 'noteFace'){
                $resultNP['msg'] = "success";
                $resultNP['success'] = 0;
                echo json_encode($resultNP);
                exit;
            }
                $result['data'] = "";
                $result['code'] = 1000;
        ...
        CommonWriteLog('未获取到刷卡数据','check-'.date("H",$signTime),'XunZhen/'.date("Y",$signTime)."/".date("m",$signTime)."/".date("d",$signTime),$IsLog);
...
if ($operation == 'gps') {
    ...
    $bj = pdo_fetch("SELECT bj_id,roomid,isopen,s_type FROM " . tablename($this->table_students) . " WHERE id = '{$ckuser['sid']}' ");
                        ...
                        if ($school['send_overtime'] >= 1) {
                            ...
                            if ($overtime >= $timecha) {
                                if (is_showyl()) {
                                    ...
                                    $allow = hasSendTz($schoolid,$xk_type['is_limit_send'],$xk_type['is_repet'],$bj['bj_id'],$signTime,$leixing,$bj['s_type'],$rfId,$ckmac['id']);
                                    if($allow){
                                        $this->sendMobileJxlxtz($schoolid, $weid, $bj['bj_id'], $ckuser['sid'], $type, $leixing, $checkid, $ckuser['pard']);
                                    }
                                }
                    ...
                    if ($num > 1) {
                        foreach ($card as $k => $r) {
                            if (!empty($r['idcard'])) {
                                ...
                                $class[$key]['card2icon'][$r['idcard']] = $r['usertype'] == '1' ? tomedia($thumb['thumb'], false, true) : tomedia($r['spic'], false, true);
                            }
                        }
                    } else {
                        $class[$key]['rfid'] = $card['0']['idcard'];
                        ...
                        $card2icon_key = $card['0']['idcard'] ? $card['0']['idcard'] : 0;
                        $class[$key]['card2icon'][$card2icon_key] = $card['0']['usertype'] == '1' ? tomedia($thumb['thumb'], false, true) : tomedia($card['0']['spic'], false, true);
                    }
                    $class[$key]['rfid'] = ltrim($class[$key]['rfid'], "#");
...
function getClassTime($schoolid,$datesetid,$time){
    $checkdateset = pdo_fetch("SELECT * FROM ".GetTableName('checkdateset')." WHERE schoolid = '{$schoolid}' AND id = '{$datesetid}' ");
    $nowdate = strtotime(date("Ymd",$time));
    $datatemp = [];
    $date = date("Y-m-j",$nowdate);
    // 取出当天数据是否为特殊设置
    $list = pdo_fetchall("SELECT * FROM ".GetTableName('checktimeset')." WHERE schoolid = '{$schoolid}' AND date = '{$date}' AND checkdatesetid = '{$datesetid}'");
    //如果有特殊设置，根据特殊设置的来
    if($list){
        foreach ($list as $k => $v) {
            if($v['s_type'] == 0){
                ...
                if($v['out_in'] == 0){
                    $datatemp[$v['s_type']][1][] = array(
                        'start' => $v['start'],
                        'end' => $v['end'],
                    );
                    $datatemp[$v['s_type']][2][] = array(
                        'start' => $v['start'],
                        'end' => $v['end'],
                    );
                }else{
                    $datatemp[$v['s_type']][$v['out_in']][] = array(
                        'start' => $v['start'],
                        'end' => $v['end'],
                    ); 
                }
        ...
        $holiday = pdo_fetch("SELECT id FROM ".GetTableName('checkdatedetail')." WHERE schoolid = '{$schoolid}' AND checkdatesetid = '{$datesetid}' AND ((unix_timestamp(sum_start) <= '{$nowdate}' AND unix_timestamp(sum_end) >= '{$nowdate}') OR (unix_timestamp(win_start) <= '{$nowdate}' AND unix_timestamp(win_end) >= '{$nowdate}'))");
        if($holiday){ //在假期范围内，则为全天
            ...
            if($nowday == 5){ //周五
                if($checkdateset['friday'] == 1){ //如果启用了周五单独设置
                    $speciallist = pdo_fetchall("SELECT * FROM ".GetTableName('checktimeset')." WHERE schoolid = '{$schoolid}' AND type = 2 AND checkdatesetid = '{$datesetid}'");
                }else{
                    $speciallist = pdo_fetchall("SELECT * FROM ".GetTableName('checktimeset')." WHERE schoolid = '{$schoolid}' AND type = '1' AND checkdatesetid = '{$datesetid}'");
                }
            }elseif($nowday == 6){ //周六
                if($checkdateset['saturday'] == 1){ //如果启用了周六单独设置
                    $speciallist = pdo_fetchall("SELECT * FROM ".GetTableName('checktimeset')." WHERE schoolid = '{$schoolid}' AND type = 3 AND checkdatesetid = '{$datesetid}'");
                }
            }elseif($nowday == 0){
                if($checkdateset['sunday'] == 1){ //如果启用了周日单独设置
                    $speciallist = pdo_fetchall("SELECT * FROM ".GetTableName('checktimeset')." WHERE schoolid = '{$schoolid}' AND type = 4 AND checkdatesetid = '{$datesetid}'");
                }
            }else{ //周一至周四
                $speciallist = pdo_fetchall("SELECT * FROM ".GetTableName('checktimeset')." WHERE schoolid = '{$schoolid}' AND type = 1 AND checkdatesetid = '{$datesetid}'");
            }
            ...
            if($speciallist){ //查出来了就按照查出来的
                foreach ($speciallist as $v1) {
                    if($v1['s_type'] == 0){
                        ...
                        if($v1['out_in'] == 0){
                            $datatemp[$v1['s_type']][1][] = array(
                                'start' => $v1['start'],
                                'end' => $v1['end'],
                            );
                            $datatemp[$v1['s_type']][2][] = array(
                                'start' => $v1['start'],
                                'end' => $v1['end'],
                            );
                        }else{
                            $datatemp[$v1['s_type']][$v1['out_in']][] = array(
                                'start' => $v1['start'],
                                'end' => $v1['end'],
                            ); 
                        }
 ...
 * @return bool
 */
function hasSendTz($schoolid,$is_limit_send,$is_repet,$bj_id,$signTime,$leixing,$s_type,$signId,$macid){
    $allow = true;
    if($is_limit_send){
        $datesetid = pdo_fetch("SELECT datesetid FROM ".GetTableName('classify')." WHERE sid = '{$bj_id}' ")['datesetid'];
        $checkoutinlist = getClassTime($schoolid,$datesetid,$signTime);
        $tempList = $checkoutinlist[$s_type][$leixing];
        if(empty($tempList)){
            $tempList = array(
                array(
                    'start'=>'00:00',
        ...
        foreach ($tempList as $value) {
            $start = strtotime(date("Y-m-d",$signTime).' '.$value['start']);
            $end = strtotime(date("Y-m-d",$signTime).' '.$value['end']);
    ...
    if($is_repet){
        //最新签到的一天时间
        $macGroup = pdo_fetch("SELECT time,macid FROM " . GetTableName('xzmacgroup') . " WHERE schoolid = '{$schoolid}' AND FIND_IN_SET('{$macid}',macid) ");
        $condition = '';
        if($macGroup){
            $condition = " AND FIND_IN_SET(macid,'{$macGroup['macid']}')";
        }
        $newchecktime = pdo_fetch("SELECT createtime FROM " . GetTableName('checklog') . " WHERE cardid = :cardid And schoolid = :schoolid AND leixing = :leixing $condition ORDER BY id DESC LIMIT 1,1", array(':cardid' => $signId, ':schoolid' => $schoolid, ':leixing' => $leixing))['createtime'];
        //  CommonWriteLog($signTime,'checkRepet','XunZhen');
        if(($newchecktime + 60*$macGroup['time']) >= $signTime){
            $allow = false;
        }
    }
    return $allow;
