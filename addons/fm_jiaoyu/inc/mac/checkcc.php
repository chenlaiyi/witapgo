...
if ($operation == 'student') {
	if(!empty($ckmac)) {
		...
		foreach($students as $key=>$row) {
			...
			$students[$key]['faceUrl']=tomedia($brcard['spic']);
			foreach($family as $f=>$v) {
				if($v['pard']!='1') {
					...
					$relation[$f]['faceUrl'] =tomedia($fcard['spic']) ;
				}
			}
...
if ($operation == 'teacher') {
	if(!empty($ckmac)) {
		...
		foreach($bj_t as $key=>$row) {
			...
			$teachers[$key]['faceUrl']=tomedia($tcard['spic']);
...
if ($operation == 'praise') {
	...
	foreach($praise as $m=>$p) {
		...
		foreach($list as $key=>$row) {
			...
			$student[$i]['faceUrl']=tomedia($fcard['spic']);
