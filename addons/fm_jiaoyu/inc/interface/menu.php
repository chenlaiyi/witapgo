...
defined('IN_IA') or exit('Access Denied');
  // require  'core.php';
// require 'pages/rootpage.php';
include_once  'pages/rootpage.php';
$PathRootMenu = dirname(dirname(__FILE__)); //指向INC文件夹


class MenuCore extends RootPage
{
    public function getNaveMenuCore($schoolid, $action, $IsDep = false)
    {
        global $_W, $_GPC;
        $do = $_GPC['do'];
        $op = $_GPC['op'];
        ...
        if ($IsDep == false) {
            ...
            if ($_W['schooltype']) {
                if (keep_yiheedu()) {
                    if ($_W['isfounder'] || $_W['role'] == 'owner') {
                        $navemenu[0]['items'][] = array(
                            'title' => '属性管理',
                            'url' => $do != 'attribute' ? $this->createWebUrl('attribute', array('op' => 'teadisplay', 'schoolid' => $schoolid)) : '#',
                            'active' => $action == 'attribute' ? ' active' : '',
                            'append' => array(
                                'title' => '<i style="color:#d9534f;" class="fa fa-cubes"></i>',
                            ),
                        );
            ...
            if ($_W['isfounder'] || $_W['role'] == 'owner' || strstr($qxarr, '100060') || strstr($qxarr, '100070') || strstr($qxarr, '1004101')   || strstr($qxarr, '100080') || strstr($qxarr, '10009') || strstr($qxarr, '100100') || strstr($qxarr, '10011')) {
                ...
                if (keep_yiheedu() && $_W['schooltype']) {
                    if ($_W['isfounder'] || $_W['role'] == 'owner') {
                        $navemenu[$tag]['items'][] = array(
                            'title' => '推广管理',
                            'url' => $do != 'promotefans' ? $this->createWebUrl('promotefans', array('op' => 'display', 'schoolid' => $schoolid)) : '#',
                            'active' => $action == 'promotefans' ? ' active' : '',
                            'append' => array(
                                'title' => '<i style="color:#7228b5;" class="fa fa-bomb"></i>',
                            ),
                        );
    ...
    if (is_file($PathRootMenu.'/interface/extra/extend.class.php')) {
        include_once $PathRootMenu.'/interface/extra/extend.class.php';
    } else {
        class connectClass extends MenuCore { }
    }

    class Menu extends connectClass
    {
        public function getNaveMenu($schoolid, $action, $IsDep = false)
        {
            return $this->getNaveMenuCore($schoolid, $action, $IsDep);
        }
    }
