...
class Core extends WeModuleSite {
    ...
    public function sendMobileXsqj($leave_id, $schoolid, $weid, $tid, $schooltype = false) { //学生请假通知 发送给老师 统一平台已处理 check
		...
		$shentx = get_weidset($weid,'bjqshtz');
        $sms_set = get_school_sms_set($schoolid);
        if($sms_set['xsqingjia'] == 1 || !empty($smsset['xsqingjia']) || !empty($shentx['bjqshtz'])) {
			...
			$datas=array(
                ...
                'keyword1'=>array('value'=>'学生请假申请','color'=>'#1587CD'),
                'keyword2'=>array('value'=>$student['s_name'],'color'=>'#FF9E05'),
                'keyword3'=>array('value'=>'待审核','color'=>'#1587CD'),
            ...
            if (!empty($shentx['bjqshtz'])) {
                $this->sendtempmsg($shentx['bjqshtz'], $url, $data, '#FF0000', $teacher['openid']);
            }
        }
    }
    ...
    public function sendMobileXsqjToParent($leave_id, $schoolid, $weid, $id) { //学生请假通知家长审核
        ...
        if($sms_set['xsqingjia'] == 1 || !empty($smsset['xsqingjia']) || !empty($shentx['bjqshtz'])) {
			...
			$datas=array(
                ...
                'keyword1'=>array('value'=>'学生请假申请','color'=>'#1587CD'),
                'keyword2'=>array('value'=>$student['s_name'],'color'=>'#FF9E05'),
                'keyword3'=>array('value'=>$status,'color'=>'#1587CD'),
                'keyword4'=>array('value'=>$time,'color'=>'#1587CD'),
                'remark'=> array('value'=>$body,'color'=>'#FF9E05')
            );
            $data=json_encode($datas); //发送的消息模板数据
            ...
            if (!empty($shentx['bjqshtz'])) {
                $this->sendtempmsg($shentx['bjqshtz'], $url, $data, '#FF0000', $parent['openid']);
            }
        }
    }
    ...
    public function sendMobileXsqjsh($leaveid, $schoolid, $weid, $tname) { //学生请假审核结果 发送给学生 统一平台已处理 check
		...
		$shentx = get_weidset($weid,'bjqshjg');
        $sms_set = get_school_sms_set($schoolid);
        if($sms_set['xsqjsh'] == 1 || !empty($smsset['xsqjsh']) || !empty($shentx['bjqshjg'])) {
			...
			$datas=array(
                ...
                'keyword3'=>array('value'=>$ctime,'color'=>'#1587CD'),
                'remark'=> array('value'=>$body,'color'=>'#FF9E05')
            );
            $data=json_encode($datas); //发送的消息模板数据
            ...
            if (!empty($shentx['bjqshjg'])) {
                $this->sendtempmsg($shentx['bjqshjg'], $url, $data, '#FF0000', $leave['openid']);
            }
        }
    }
        ...
        if($sms_set['xsqjsh'] == 1 || !empty($smsset['xsqjsh']) || !empty($shentx['bjqshjg'])) {
            ...
            foreach($tidArr as $t){
				...
				$datas=array(
					...
					'keyword1'=>array('value'=>'学生请假','color'=>'#1587CD'),
					'keyword2'=>array('value'=>$jieguo,'color'=>'#2D6A90'),
					'keyword3'=>array('value'=>$ctime,'color'=>'#1587CD'),
					'remark'=> array('value'=>$body,'color'=>'#FF9E05')
				);
                ...
                if (!empty($shentx['bjqshjg'])) {
                    $this->sendtempmsg($shentx['bjqshjg'], $url, $data, '#FF0000', $door['openid']);
                }
            }
    ...
    public function sendMobileYzxxhf($mailid, $schoolid, $weid) { // 校长信箱回复 统一平台已处理 check
        ...
        if (!empty($smsset['liuyanhf'])) {
            $this->sendtempmsg($smsset['liuyanhf'], $url, $data, '#FF0000', $stu['openid']);
        }
    }
    ...
    public function sendMobileLyhf($leave_id, $schoolid, $weid) { //通讯录私聊  统一平台已处理 check
        global $_GPC,$_W;
        $smsset = get_weidset($weid,'liuyan');
		$hfsset = get_weidset($weid,'liuyanhf');
        $sms_set = get_school_sms_set($schoolid);
        if($sms_set['liuyan'] == 1 || !empty($smsset['liuyan']) || !empty($hfsset['liuyanhf'])) {
			...
			if($user['sid'] > 0 && $touser['tid'] >0){
				$msgtotype = 1;//家长对老师
			}
			if($user['sid'] > 0 && $touser['sid'] >0){
				$msgtotype = 1;//家长对家长
			}
			if($user['tid'] > 0 && $touser['sid'] >0){
				$msgtotype = 2;//老师对家长
			}
			if($user['tid'] > 0 && $touser['tid'] >0){
				$msgtotype = 2;//老师对老师
			}
			...
			if($msgtotype == 2){
				if (!empty($hfsset['liuyanhf'])) {
					$this->sendtempmsg($hfsset['liuyanhf'], $url, $data, '#FF0000', $touser['openid']);
				}
			}
        }
    }
    ...
    public function sendMobileJsqj($leave_id, $schoolid, $weid, $openid) { //教师请假 发送给校长或主任 统一平台已处理 check
		...
		$shentx = get_weidset($weid,'bjqshtz');
        $sms_set = get_school_sms_set($schoolid);
        if($sms_set['jsqingjia'] == 1 || !empty($smsset['jsqingjia']) || !empty($shentx['bjqshtz'])) {
            $leave = pdo_fetch("SELECT * FROM ".tablename($this->table_leave)." WHERE :weid = weid AND :id = id AND :schoolid = schoolid", array(':weid' => $weid, ':id' => $leave_id, ':schoolid' => $schoolid));
			...
			$datas=array(
                ...
                'keyword1'=>array('value'=>'职工请假申请','color'=>'#1587CD'),
                'keyword2'=>array('value'=>$teacher['tname'],'color'=>'#FF9E05'),
                'keyword3'=>array('value'=>'待审核','color'=>'#1587CD'),
                'keyword4'=>array('value'=>$time,'color'=>'#1587CD'),
                'remark'=> array('value'=>$body,'color'=>'#FF9E05')
            );
            ...
            if (!empty($shentx['bjqshtz'])) {
                $this->sendtempmsg($shentx['bjqshtz'], $url, $data, '#FF0000', $openid);
            }
        }
    }
    ...
    public function sendMobileJsqjsh($leaveid, $schoolid, $weid, $shname) { //教师审核结果 发送给请假教师 统一平台已处理 check
		...
		$shentx = get_weidset($weid,'bjqshjg');
        $sms_set = get_school_sms_set($schoolid);
        if($sms_set['jsqjsh'] == 1 || !empty($smsset['jsqjsh']) || !empty($shentx['bjqshjg'])) {
    ...
    public function sendMobileJxlxtz($schoolid, $weid, $bj_id, $sid, $type, $leixing, $id, $pard) { //学生进校离校通知 统一平台已处理 check
        ...
        if($sms_set['jxlxtx'] == 1 || !empty($smsset['jxlxtx'])) {
            $student = pdo_fetch("SELECT s_name,mobile,isopen FROM " . GetTableName('students') . " where id = :id ", array(':id' => $sid));
            ...
            /**结束 和校园 消息发送判断 */
			if(!empty($userinfo) && $student['isopen'] == 1){
				foreach ($userinfo as $key => $value) {
    ...
    public function sendMobileYxb($id,$schoolid, $weid){ //营销宝通知
        global $_GPC,$_W;
        $smsset = get_weidset($weid,'xxtongzhi');
        $sms_set = get_school_sms_set($schoolid);
        if($sms_set['xxtongzhi'] == 1 || !empty($smsset['xxtongzhi'])) {
            $spid = pdo_fetch("SELECT spid FROM ".GetTableName('saleform')." WHERE id = '{$id}' ")['spid'];
            $special = pdo_fetch("SELECT title,tidstr,sharedescription FROM ".GetTableName('special')." WHERE id = '{$spid}' ");
            $teachers = pdo_fetchAll("SELECT t.id,t.tname,u.openid FROM ".GetTableName('teachers')." as t LEFT JOIN ".GetTableName('user')." as u ON u.tid = t.id WHERE t.schoolid = '{$schoolid}' AND FIND_IN_SET(t.id,'{$special['tidstr']}') ");
            $schoolTitle = pdo_fetch("SELECT title FROM ".GetTableName('index')." WHERE id = '{$schoolid}' ")['title'];
            foreach($teachers as $k => $v){
                $openid = $v['openid'];
                $title = "【{$v['tname']}】老师，您收到一条营销宝表单通知";
                ...
                $datas=array(
                    ...
                    'keyword2'=>array('value'=>$name,'color'=>'#2D6A90'),
                    'keyword3'=>array('value'=>$ttime,'color'=>'#1587CD'),
                    'keyword4'=>array('value'=>$special['sharedescription'],'color'=>'#1587CD'),
                    'remark'=> array('value'=>$body,'color'=>'#FF9E05')
                );
                $data = json_encode($datas); //发送的消息模板数据
                $url =  $_W['siteroot'] .'app/'.$this->createMobileUrl('tspecialform', array('schoolid' => $schoolid,'id' => $id));
                if(!empty($smsset['xxtongzhi'])){
                    $this->sendtempmsg($smsset['xxtongzhi'], $url, $data, '#FF0000', $v['openid']);
                }
            }
        }
    }
