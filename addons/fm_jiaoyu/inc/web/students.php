...
if($operation == 'post'){
	...
	if(!empty($_GPC['kc_id'])){
		...
		// $allbuysid = pdo_fetchall("SELECT distinct sid FROM ".tablename($this->table_order)." where kcid = '{$_GPC['kc_id']}' And type = 1 And status = 2 And sid != 0 ");
		$allbuysid = pdo_fetchall("SELECT distinct sid FROM " . GetTableName('coursebuy') . " WHERE kcid = '{$_GPC['kc_id']}' And is_change != 1 ");
		$sidlist = '';
	...
	pdo_delete(GetTableName('coursebuy',false),array('sid'=>$id));
	//校智付删除学生操作
	setXzfExtra($id,2);
