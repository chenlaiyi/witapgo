...
// 期号
$qh    = pdo_fetchall("SELECT sid,sname FROM " . tablename($this->table_classify) . " where weid = '{$weid}' AND schoolid ={$schoolid} And type = 'xq_score'  AND is_show_qh = 1 ORDER BY ssort DESC");
...
if($operation == 'display'){
    ...
    $yearList = array(
        intval(date("Y")),intval(date("Y")) - 1, intval(date("Y")) - 2,
    );
    ...
    $total = pdo_fetchcolumn("SELECT count(m.id) FROM ".GetTableName('mcreportlist')." as m  WHERE m.schoolid = '{$schoolid}' {$condition} ");
    $pager = pagination($total, $pindex, $psize);
}elseif($operation == 'getReportIsSync'){
    $sync = pdo_fetch("SELECT synctime,synctime_month,synctime_xq FROM ".GetTableName('schoolset')." WHERE schoolid = '{$schoolid}'");
    $data['result'] = false;
    if($sync['synctime_month'] && $_GPC['type'] == 1){
        $data['msg'] = '当前月度报告正在执行!';
        $data['result'] = true;
    }
    if($sync['synctime_xq'] && $_GPC['type'] == 2){
        $data['msg'] = '当前学期报告正在执行!';
        $data['result'] = true;
    }
    if($sync['synctime'] && $_GPC['type'] == 3){
        $data['msg'] = '当前年度报告正在执行!';
        $data['result'] = true;
    }
    die(json_encode($data));
    ...
    $param = array(
        'type' => $_GPC['reportType'],
        'year' => $_GPC['reportYear'],
        'month' => $_GPC['reportMonth'],
        'xq_id' => $_GPC['reportXq'],
        'page' => 1,
        'rows' => 20,
    );
