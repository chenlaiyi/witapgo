...
if($operation == 'display'){
	...
	$alljsfz = pdo_fetchall("SELECT sid,sname FROM " . tablename($this->table_classify) . " WHERE weid = '{$weid}' AND schoolid = '{$schoolid}' AND weid = '{$weid}' AND type = 'jsfz' ORDER BY ssort DESC");
    ...
    foreach($list as $index => $row){
		...
		if($row['videoid'] && !$row['videourl']){
			...
			$videourl = GetAliVideoUrl($appid,$key,$row['videoid'])['PlayURL'];
			$videocover = GetAliVideoCover($appid,$key,$row['videoid'])['CoverURL'];
			if($videourl){
				$videoarr = array(
					'videourl' => $videourl,
					'videocover' => $videocover,
				);
				pdo_update(GetTableName('checklog',false),$videoarr,array('id'=>$row['id']));
			}
		}
    }
    ...
    if($_GPC['out_put'] == 'output'){
		...
		if(!empty($jsfz)){
			...
			$fzname = pdo_fetch("SELECT sname FROM " . tablename($this->table_classify) . " WHERE sid = '{$jsfz}' ");
			$condition = " AND createtime > '{$start}' AND createtime < '{$end}' AND tid != 0 ";
		}
        ...
        foreach($list as $index => $row){
            $student             = pdo_fetch("SELECT s_name FROM " . tablename($this->table_students) . " WHERE id = '{$row['sid']}' ");
            $teacher             = pdo_fetch("SELECT tname,fz_id FROM " . tablename($this->table_teachers) . " WHERE id = '{$row['tid']}' ");
			if(!empty($jsfz) && $teacher['fz_id'] != $jsfz){
				continue;
			}
		...
		$filename = '考勤记录';
		if(!empty($jsfz)){
			$filename = $_GPC['start'].'至'.$_GPC['end'].$fzname['sname'].'考勤记录';
		}
        $this->exportexcel($arr,  $excel_title , $filename);
        exit();
    }
    ////////////////////////////////
