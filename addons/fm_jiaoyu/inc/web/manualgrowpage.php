 ...
 * @LastEditTime: 2020-12-03 10:51:06
 */
...
if($_GPC['op'] == 'display'){
    ...
    CommonWriteLog("pageList",'muban',"ChengZhangShouCe");
    CommonWriteLog($pageList,'muban',"ChengZhangShouCe");
    CommonWriteLog("DelArrId",'muban',"ChengZhangShouCe");
    CommonWriteLog($_GPC['DelArrId'],'muban',"ChengZhangShouCe");
    foreach ($PageList as $key => $value) {
        
        if($value['id']){ //修改或删除数据
            $data = array(
                'content_data' => json_encode($value['data']['pageData']) != null ? json_encode($value['data']['pageData'])  : ""  ,
                'auth' => $value['auth'],
                'ssort' => $value['order'],
            );
