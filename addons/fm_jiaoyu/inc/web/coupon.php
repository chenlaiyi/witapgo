...
if($operation == 'display'){
    ...
    $list = pdo_fetchall("SELECT * FROM " . GetTableName('welfare') . " WHERE weid = '{$weid}' AND schoolid = '{$schoolid}' AND type = 2 $conditions ORDER BY id DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize);
    $total = pdo_fetchcolumn("SELECT count(*) FROM " . GetTableName('welfare') . " WHERE weid = '{$weid}' AND schoolid = '{$schoolid}' AND type = 2 ");
    $pager = pagination($total, $pindex, $psize);
}elseif($operation == 'delete'){
    $id     = intval($_GPC['id']);
    $item = pdo_fetch("SELECT id  FROM " . GetTableName('welfare') . " WHERE id = '{$id}'");
    if(empty($item)){
        $this->imessage('抱歉，优惠券不存在或是已经被删除！', referer(), 'error');
    }
    ...
    $pindex    = max(1, intval($_GPC['page']));
    $psize     = 15;
    $list = pdo_fetchall("SELECT wl.id,wl.openid,wl.userid,wl.time,wl.usetime,wl.status,w.title as welfaretitle,s.title as sptitle,stu.s_name as usename FROM ".GetTableName('welfarelog')." as wl LEFT JOIN ".GetTableName('welfare')." as w ON wl.welfareid = w.id LEFT JOIN ".GetTableName('special')." as s ON wl.spid = s.id LEFT JOIN ".GetTableName('students')." as stu ON wl.sid = stu.id WHERE wl.schoolid = '{$schoolid}' AND wl.weid = '{$weid}' AND s.marketingtype = 2 ORDER BY wl.id DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize); 
...
include $this->template('web/welfare/coupon');
