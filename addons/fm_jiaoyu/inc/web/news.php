...
if($operation == 'display'){
    ...
    $list  = pdo_fetchall("SELECT n.*,i.title as name,( SELECT count(id) FROM " . GetTableName('articledz') . " WHERE a_id = n.id) as dianzan FROM " . tablename($this->table_news) . " as n LEFT JOIN " . tablename($this->table_index) . " as i ON i.id = n.schoolid WHERE n.weid = '{$weid}' And (n.schoolid = '{$schoolid}' OR FIND_IN_SET('{$schoolid}',n.schoolidstr)) And n.type = 'news' $condition ORDER BY n.displayorder DESC, n.id DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize, $params);
