 ...
 * @LastEditTime: 2020-12-12 11:35:13
 */
global $_GPC, $_W;
$weid              = $_W['uniacid'];
$action            = 'manual';
...
if($operation == 'display'){ //显示校长填写列表
    ...
    if($_GPC['mbid']){
        pdo_update(GetTableName('muban',false),$data,array('id'=>$_GPC['mbid']));
    }else{
    ...
    $result['data'] = $item;
    die(json_encode($result));
}elseif($operation == 'CORSFile'){
    $CORSFileName = $_GPC['fileName'];
    $txt = file_get_contents($CORSFileName);
    $localFileName =  strstr(basename($CORSFileName),'?') ? substr(basename($CORSFileName),0,strrpos(basename($CORSFileName),"?")) : basename($CORSFileName) ;
    //创建随机文件
    $fileDir = $_GPC['sid'].'-'.$_GPC['gid'];
    $dir = IA_ROOT . '/attachment/images/manual/'.$fileDir;
    if(!is_dir($dir)){
        mkdir(iconv("UTF-8", "GBK", $dir),0777,true);
    }
    $path = "images/manual/".$fileDir.'/'.$localFileName;
    file_write($path, $txt);
    $result['fileName'] = tomedia($path);
    $result['dir'] = $dir;
    die(json_encode($result));
    ...
    $data = json_decode(htmlspecialchars_decode($_GPC['SinglePageItemList']),true);
    foreach($data as $val){
        $key_arrays[]=$val['zindex'];
    }
    array_multisort($key_arrays,SORT_ASC,SORT_NUMERIC,$data);
    CommonWriteLog($data,'1','2');
    $pic = setComPoster(tomedia($bgimg),$data);
    //创建随机文件
    $fileDir = $_GPC['sid'].'-'.$_GPC['gid'];
    $pdfdir = IA_ROOT . '/attachment/images/manual2/'.$fileDir;
    if(!is_dir($pdfdir)){
        mkdir(iconv("UTF-8", "GBK", $pdfdir),0777,true);
    }
    $qrcode_name = $pdfdir.'/'.random(30).'.png';
    $filename =  $qrcode_name;
