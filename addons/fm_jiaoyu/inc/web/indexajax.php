...
if ($operation == 'DelTemplate') {
	if (empty($_GPC['weid'])) {
		...
		mload()->model('sms');
		$access_token = $this->getAccessToken2();
		$postarr = '{"template_id": "' . $_GPC['template_id'] . '"}';
		$res = ihttp_post('https://api.weixin.qq.com/cgi-bin/template/del_private_template?access_token=' . $access_token, $postarr);
		$content = @json_decode($res['content'], true);
		if ($content['errcode'] == 0) {
			$result['template_id'] = $content['template_id'];
			$result['msg'] = '修改成功';
			...
			$result['result'] = false;
			$result['msg'] = '清理失败,原因1公众号是否认证服务号,2公众号对接是否正常，3已添加的模板库无此模板ID';
		}
		$result['template_id'] = $_GPC['template_id'];
	}
	die(json_encode($result));
}
...
if ($operation == 'xugou_kc') {
	...
	foreach ($sidarr as $value) {
		$student = pdo_fetch("SELECT s_name FROM " . tablename($this->table_students) . " where schoolid = '{$schoolid}' And id = '{$value}'");
		$check = pdo_fetch("SELECT id FROM " . GetTableName('coursebuy') . " where schoolid = '{$schoolid}' And kcid = '{$kcid}' And is_change != 1 And sid = '{$value}'");
...
if ($operation == 'delBj') {
	if (!$_GPC['schoolid']) {
		...
		if ($hasclass) {
			pdo_update(GetTableName('coursebuy',false),array('bjid'=>0),array('bjid'=>$_GPC['id']));
...
if($operation == 'getTemplibrary'){
	if (! $_GPC ['schoolid']) {
		...
		$condition = '';
		if($_GPC['cate_id']){
			$condition = " AND cate_id = '{$_GPC['cate_id']}' ";
		}
		$list = pdo_fetchall("SELECT * FROM ".GetTableName('template_library')." WHERE weid = '{$_GPC['weid']}' AND status = 1 $condition");
		include $this->template('web/notice/templibrary');
		die;
	}
}
//获取模板库中的模板信息
if($operation == 'getTempInfo'){
	if (! $_GPC ['schoolid']) {
		die ( json_encode ( array (
			'result' => false,
			'msg' => '非法请求！'
		...
		$temp = pdo_fetch("SELECT l.*,c.name as catename FROM ".GetTableName('template_library')." as l LEFT JOIN ".GetTableName('template_library_category')." as c ON c.id = l.cate_id WHERE l.status = 1 AND l.weid = '{$_GPC['weid']}' AND l.id = '{$_GPC['id']}'");
