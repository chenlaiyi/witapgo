...
if ($operation == 'post') {
	...
	$id = intval($_GPC['id']);
	$order = pdo_fetch("SELECT id,payweid,wqorderid,wxpayid,pay_type FROM " . GetTableName('order') . " WHERE id = '{$id}'");
	...
	pdo_delete('core_paylog', array('uniontid' => $order['payweid'],'status' => 0,'uniontid' => '','module' => 'fm_jiaoyu'));
	if(!empty($core_paylog)){
		if($order['pay_type'] == 'wechat' || $order['pay_type'] == 'wxapp'){
			...
			$uniontid = !empty($order['wqorderid'])?$order['wqorderid']:$core_paylog['uniontid'];
			$wxPay = new WxpayService($payweid,$order['pay_type']);
			$query = json_decode(json_encode($wxPay->orderquery($payweid,$uniontid)),true);
			$result['query'] = $query;
			...
			$result['core_paylog'] = $core_paylog;
		}else{
