...
function getoauthurl(){
	$oauthurl = "manger.weimeizhan.com";
	return $oauthurl;
}
...
function check_sales($sale){
	...
	$data = array(
		'sale'   => $sale,
		'checkver'   => 'checkver',
		'oauthurl' => "manger.weimeizhan.com"//getoauthurl()
	);
	$postdata = ihttp_post(urldecode($url), $data);
	if($postdata['code'] ==200){
		$respoed = json_decode($postdata['content'],true);
 ...
 */
 //TODO: 新增制定bj_id
function GetDatesetWithBj($schoolid,$weid,$bjid = 0){
	global $_W;
	...
	$condition = '';
	if($bjid){
		$condition = " AND sid = '{$bjid}'";
	}
	$bjlist = pdo_fetchall("SELECT sid,datesetid,sname  FROM " . tablename('wx_school_classify') . " WHERE weid = '{$weid}' And schoolid = '{$schoolid}' and type='theclass' $condition");
	$result = array();
		...
		if(!empty($isfz['datesetid'])){
			...
			if(!empty($checktime)){
				...
				if ((strtotime($nowdate) >= strtotime($checkdateset_holi['win_start']) && strtotime($nowdate) <= strtotime($checkdateset_holi['win_end'])) || (strtotime($nowdate) >= strtotime($checkdateset_holi['sum_start']) && strtotime($nowdate) <= strtotime($checkdateset_holi['sum_end']))){
					...
					$timeset_work = pdo_fetchall("SELECT start,end,s_type,out_in FROM " . tablename('wx_school_checktimeset') . " WHERE weid = '{$weid}' And schoolid = {$schoolid} and  checkdatesetid = '{$isfz['datesetid']}' and type=1 ORDER BY id ASC ");
					if(!$timeset_work){
						$timeset_work = array(
							array(
								's_type'=>'0',
					...
					if($nowweek == 5){
						...
						if($checkdateset['friday'] == 1){
							$timeset_fri = pdo_fetchall("SELECT start,end,s_type,out_in FROM " . tablename('wx_school_checktimeset') . " WHERE weid = '{$weid}' And schoolid = {$schoolid} and  checkdatesetid = '{$isfz['datesetid']}' and type=2 ORDER BY id ASC ");
							if(!empty($timeset_fri[0]['start'])){
					...
					//星期六
						...
						if($checkdateset['saturday'] == 1){
							$timeset_sat = pdo_fetchall("SELECT start,end,s_type,out_in FROM " . tablename('wx_school_checktimeset') . " WHERE weid = '{$weid}' And schoolid = {$schoolid} and  checkdatesetid = '{$isfz['datesetid']}' and type=3 ORDER BY id ASC ");
							$todaytype = 2;
							if($timeset_sat){
					...
					//星期天
						...
						if($checkdateset['sunday'] == 1){
							$timeset_sun = pdo_fetchall("SELECT start,end,s_type,out_in FROM " . tablename('wx_school_checktimeset') . " WHERE weid = '{$weid}' And schoolid = {$schoolid} and  checkdatesetid = '{$isfz['datesetid']}' and type=4 ORDER BY id ASC ");
							$todaytype    = 2;
							if($timeset_sun){
...
function createComPoster($config=array(),$filename=""){
	...
	// $color = imagecolorallocate($imageRes, 0, 0, 0); //原始颜色
    // $color = imagecolorallocatealpha($imageRes, 123, 123, 123,127);
    $color = imagecolorexact($imageRes, 255, 255, 255);
	// imageColorTransparent($imageRes, $color);  //颜色透明
  imagefill($imageRes, 0, 0, $color);
    ...
    if(!empty($config['image_up'])){
		foreach ($config['image_up'] as $key => $val) {
            ...
            //建立画板 ，缩放图片至指定尺寸
            $canvas=imagecreatetruecolor(750*3,530*3);
            $color123 = imagecolorallocatealpha($canvas, 123, 123, 123,127);
            // $color123 = imagecolorallocate($canvas,254,254,254);
	          imageColorTransparent($canvas, $color123);  //颜色透明

            imagefill($canvas, 0, 0, $color123);
            //关键函数，参数（目标资源，源，目标资源的开始坐标x,y, 源资源的开始坐标x,y,目标资源的宽高w,h,源资源的宽高w,h）
            imagecopyresampled($canvas, $res, 0, 0, 0, 0, $val['width']*3, $val['height']*3,$resWidth,$resHeight);
            $val['left'] = $val['left']<0?$backgroundWidth- abs($val['left']) - $val['width']:$val['left'];
            $val['top'] = $val['top']<0?$backgroundHeight- abs($val['top']) - $val['height']:$val['top'];
    ...
    //生成图片
    if(!empty($filename)){
        $res = imagepng ($imageRes,$filename,9); //保存到本地
        imagedestroy($imageRes);
        if(!$res) return false;
        return $filename;
    }else{
  ...
  global $_GPC,$_W;
	...
	if(in_array($action,array('update','delete_card','delete_stu','delete_tea','add_card'))){
        ...
        $timeSet = array(
          ...
          $GetDatesetWithBj = GetDatesetWithBj($schoolid,$_W['uniacid'],$groupNo);
          $timeSet = $GetDatesetWithBj[$groupNo]['timeset']['todaytimeset'];

				}
        $TempTimeSet = [];
        foreach($timeSet as $vt){
          $TempTimeSet[] = $vt;
        }
        		//TODO:查出这个学生对应班级当天的timeSet
...
function xzTriggerDevice($action,$macid,$schoolid){
	if($action == 'bind_device'){
		$porturl = '/bind_device.do';
        $sendData['mac'] = $macid;
        $sendData['checkDeviceUrl'] = "https://mac.weimeizhan.com//pad/user/verify/";
...
function CommonWriteLog($GetData,$fileName="CommonLog",$fileDir='',$IsLog = true){
  if($IsLog == false){
    return;
  }
