...
class Fm_jiaoyuModuleSite extends Menu {
	...
	public function doWebTemplateLibrary() {
		include_once 'inc/web/templatelibrary.php';
	}
	...
	// 营销宝模板库
	public function doWebSpecialTemp() {
		$this->getLogic ( __FUNCTION__, 'web' );
	}
	// 营销宝关联表单
	public function doWebSpecialForm() {
		$this->getLogic ( __FUNCTION__, 'web' );
	}
    ...
    public function doMobileSpecialForm() {
		$this->getLogicmc ( __FUNCTION__, 'mobile', true );
	}
  ...
  public function doMobileTongxunlu_new() {
		$this->getLogicmt ( __FUNCTION__, 'mobile', true );
	}
	...
	//教师处理营销宝表单
	public function doMobileTSpecialform(){
        $this->getLogicmt ( __FUNCTION__, 'mobile', true  );
	}
	...
	public function payResult($params) {
		...
		if ($params['result'] == 'success' && $params['from'] == 'notify') {
			...
			$uniontid = !empty($params['transaction_id'])?$params['transaction_id']:$tag['transaction_id'];
			pdo_update($this->table_wxpay, array('status' => 2), array('id' => $orderid));
			pdo_update($this->table_order, array('status' => 2, 'uniontid' => $uniontid,'wqorderid'=> $params['uniontid'],'paytime' => $params['paytime'], 'paytype' => 1, 'pay_type' => $params['type']), array('id' => $wxpay['od1']));
