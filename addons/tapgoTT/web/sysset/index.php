...
'), '', trim($data['desc']));
	...
	public function settemplateid()
	{
		...
		$templatetype = pdo_fetch('select `name`,templatecode,content,templatename  from ' . tablename('tapgo_tt_member_message_template_type') . ' where typecode=:typecode  limit 1', array(':typecode' => $tag));
...
', ' '), '', $templatetype['content']);
		...
		$issnoet = true;
		$defaulttemp = pdo_fetch('select *  from ' . tablename('tapgo_tt_member_message_template_default') . ' where typecode=:typecode and uniacid=:uniacid  limit 1', array(':typecode' => $tag, ':uniacid' => $_W['uniacid']));
		...
		if (in_array($templatetype['templatename'], $templateTiele)) {
			...
			if (empty($defaulttemp)) {
				pdo_insert('tapgo_tt_member_message_template_default', array('typecode' => $tag, 'uniacid' => $_W['uniacid'], 'templateid' => $template_list[$found_key]['template_id']));
			}
			else {
				pdo_update('tapgo_tt_member_message_template_default', array('templateid' => $template_list[$found_key]['template_id']), array('typecode' => $tag, 'uniacid' => $_W['uniacid']));
			}
		}
		else {
			if (empty($defaulttemp) || !in_array($defaulttemp['templateid'], $templateIds)) {
				...
				if (!empty($result['errcode'])) {
					...
					if (empty($defaulttemp)) {
						pdo_insert('tapgo_tt_member_message_template_default', array('typecode' => $tag, 'uniacid' => $_W['uniacid'], 'templateid' => $result['template_id']));
					}
					else {
						pdo_update('tapgo_tt_member_message_template_default', array('templateid' => $result['template_id']), array('typecode' => $tag, 'uniacid' => $_W['uniacid']));
					}
				}
			}
		}
	...
	public function notice()
	{
		...
		if (isset($data['openid2'])) {
			if (!empty($data['openid2'])) {
				...
				$salers2 = pdo_fetchall('select id,nickname,avatar,openid from ' . tablename('tapgo_tt_member') . ' where openid in (' . implode(',', $openids2) . (') and uniacid=' . $_W['uniacid']));
			}
		}
		...
		if (isset($data['openid3'])) {
			if (!empty($data['openid3'])) {
				...
				$salers3 = pdo_fetchall('select id,nickname,avatar,openid from ' . tablename('tapgo_tt_member') . ' where openid in (' . implode(',', $openids3) . (') and uniacid=' . $_W['uniacid']));
			}
		}
		...
		if (isset($data['openid4'])) {
			if (!empty($data['openid4'])) {
				...
				$salers4 = pdo_fetchall('select id,nickname,avatar,openid from ' . tablename('tapgo_tt_member') . ' where openid in (' . implode(',', $openids4) . (') and uniacid=' . $_W['uniacid']));
			}
		}
	...
	{
		global $_W;
		$path = IA_ROOT . '/addons/tapgo_ttv2/cert';
		load()->func('file');
		mkdirs($path);
	...
	public function payset()
	{
		...
		if ($_W['ispost']) {
			...
			pdo_update('tapgo_tt_sysset', array('sec' => iserializer($sec)), array('uniacid' => $_W['uniacid']));
		...
		$payments = pdo_fetchall('SELECT id,title FROM ' . tablename('tapgo_tt_payment') . ' WHERE uniacid=:uniacid and paytype = 0 ', array(':uniacid' => $_W['uniacid']));
		$paymentalis = pdo_fetchall('SELECT id,title FROM ' . tablename('tapgo_tt_payment') . ' WHERE uniacid=:uniacid and paytype = 1 ', array(':uniacid' => $_W['uniacid']));
		...
		if (empty($payments)) {
			...
			if (!empty($payment['wechat']['mchid'])) {
				...
				$payments[] = $default;
				pdo_insert('tapgo_tt_payment', $default);
		...
		$url = $_W['siteroot'] . 'addons/tapgo_ttv2/payment/wechat/notify.php';
		load()->func('communication');
		$resp = ihttp_get($url);
		include $this->template();
		exit();
	...
	public function templat()
	{
		...
		$styles = array();
		$dir = IA_ROOT . '/addons/tapgo_ttv2/template/mobile/';
	...
	public function wap()
	{
		...
		$styles = array();
		$dir = IA_ROOT . '/addons/tapgo_ttv2/template/account/';
	...
	public function funbar()
	{
		...
		if ($_W['ispost']) {
			$data = pdo_fetch('select * from ' . tablename('tapgo_tt_funbar') . ' where uid=:uid and uniacid=:uniacid limit 1', array(':uid' => $_W['uid'], ':uniacid' => $_W['uniacid']));
			$funbardata = is_array($_GPC['funbardata']) ? $_GPC['funbardata'] : array();
			$funbardata = serialize($funbardata);

			if (empty($data)) {
				pdo_insert('tapgo_tt_funbar', array('uid' => $_W['uid'], 'datas' => $funbardata, 'uniacid' => $_W['uniacid']));
			}
			else {
				pdo_update('tapgo_tt_funbar', array('datas' => $funbardata), array('uid' => $data['uid'], 'uniacid' => $_W['uniacid']));
			}
		...
		if ($_W['ispost']) {
			...
			if (empty($data) || empty($data['new_area'])) {
				...
				if (!empty($array['new_area'])) {
					...
					$change_data['city'] = '';
					$change_data['area'] = '';
					pdo_update('tapgo_tt_member', $change_data, array('uniacid' => $uniacid));
					pdo_update('tapgo_tt_member_address', $change_data, array('uniacid' => $uniacid));
				}
				else {
			...
			if (empty($data)) {
				...
				pdo_insert('tapgo_tt_area_config', $array);
			}
			else {
				if (!empty($array)) {
					pdo_update('tapgo_tt_area_config', $array, array('id' => $data['id'], 'uniacid' => $uniacid));
				}
			}
