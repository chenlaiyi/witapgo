...
class Index_TapgoTtPage extends MobileLoginPage
{
	...
	public function detail()
	{
		...
		foreach ($goods as &$g) {
			if ($g['isfullback']) {
				$fullbackgoods = pdo_fetch('SELECT * FROM ' . tablename('tapgo_tt_fullback_goods') . ' WHERE uniacid = ' . $uniacid . ' and goodsid = ' . $g['goodsid'] . ' limit 1 ');

				if ($g['optionid']) {
					$option = pdo_fetch("select `day`,allfullbackprice,fullbackprice,allfullbackratio,fullbackratio,isfullback\r\n                      from " . tablename('tapgo_tt_goods_option') . ' where id = ' . $g['optionid'] . ' and uniacid = ' . $uniacid . ' ');
