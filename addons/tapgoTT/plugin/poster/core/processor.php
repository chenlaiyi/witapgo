...
require_once TAPGO_TT_INC . 'plugin_processor.php';
...
class PosterProcessor extends PluginProcessor {
	...
	private function responseText($obj) {
        ...
        if (is_file(TAPGO_TT_CORE.'queue.pid')){
	...
	private function responseScan($obj) {
		...
		$poster = pdo_fetch('select * from ' . tablename('tapgo_tt_poster') . ' where type=4 and isdefault=1 and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid']));
		if (empty($poster)) {
			return $this->responseDefault($obj);
		}
        ...
        if($poster['ismembergroup']>0){
            if(empty($member['groupid']) && $poster['membergroupid']>0){
                pdo_update('tapgo_tt_member',array('groupid'=>$poster['membergroupid']),array('openid'=>$openid,'uniacid'=>$_W['uniacid']));
            }
        }
	...
	private function responseSubscribe($obj) {
		...
		if ($member['isnew']) {
			pdo_update('tapgo_tt_poster', array('follows' => $poster['follows'] + 1), array('id' => $poster['id']));
		}
		$qrmember = m('member')->getMember($qr['openid']);
		$log = pdo_fetch('select * from ' . tablename('tapgo_tt_poster_log') . ' where openid=:openid and posterid=:posterid and uniacid=:uniacid limit 1', array(':openid' => $openid, ':posterid' => $poster['id'], ':uniacid' => $_W['uniacid']));
		if (empty($log) && $openid != $qr['openid']) {
			...
			pdo_insert('tapgo_tt_poster_log', $log);
			$log['id'] = pdo_insertid();
            if($poster['ismembergroup']>0){
                if(empty($member['groupid']) && $poster['membergroupid']>0){
                    pdo_update('tapgo_tt_member',array('groupid'=>$poster['membergroupid']),array('openid'=>$openid,'uniacid'=>$_W['uniacid']));
			...
			if(!empty($upgrade)){
				pdo_update('tapgo_tt_poster_log',$upgrade,array('id'=>$log['id']));
			}
							...
							if (empty($member['agentid'])) {
								if(empty($member['fixagentid'])){
									...
									if ($author) {
										...
										pdo_update('tapgo_tt_member', array('agentid' => $qrmember['id'], 'childtime' => $time,'authorid'=>$authorid), array('id' => $member['id']));
									}else{
										...
										if(!empty($author_set['become']) && ($author_set['become']=='2' || $author_set['become']=='5')){
											...
											if($can_author){
												...
												if (empty($member['authorblack'])) { 
													pdo_update('tapgo_tt_member', array('authorstatus' => $become_check, 'isauthor' => 1, 'authortime' => $time), array('uniacid' => $_W['uniacid'], 'id' => $qrmember['id']));
													if ($become_check == 1) {
