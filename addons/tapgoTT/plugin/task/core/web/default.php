...
class Default_TapgoTtPage extends PluginWebPage {
    ...
    private function post($data){
        ...
        $defaultcount = pdo_fetchcolumn('select COUNT(*) from ' . tablename('tapgo_tt_task_default') . ' where uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid']));
        ...
        if($defaultcount>0){
            ...
            $result = pdo_update('tapgo_tt_task_default', $update_data, array('uniacid' => $_W['uniacid']));
            if ($result!==false) {
            ...
            $result = pdo_insert('tapgo_tt_task_default', $insert_data);
            if (!empty($result)) {
                show_json(1, array('url' => webUrl('task/default')));
            }else{
                show_json(0, '更新失败');
    ...
    public function setstart()
    {
        ...
        if ($_W['ispost']) {
            ...
            if ($defaultcount > 0) {
                ...
                if (!empty($keyword)) {
                    if ($keyword['name'] != 'tapgo_ttv2:task') {
                        show_json(0, '关键字已存在!');
                    }
                }
                $rule = pdo_fetch("select * from " . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'cover', ':name' => "tapgo_ttv2:task"));
                if (!empty($rule)) {
                ...
                $rule_data = array('uniacid' => $_W['uniacid'], 'name' => 'tapgo_ttv2:task', 'module' => 'cover', 'displayorder' => 0, 'status' => 1);
                if (empty($rule)) {
                ...
                $cover_data = array('uniacid' => $_W['uniacid'], 'rid' => $rid, 'module' => 'tapgo_ttv2', 'title' => $default['title'], 'description' => $default['desc'], 'thumb' => $default['thumb'], 'url' => mobileUrl('task'));
                if (empty($cover)) {
                ...
                pdo_update('tapgo_tt_task_default', $update_data, array('uniacid' => $_W['uniacid']));
                ...
                pdo_insert('tapgo_tt_task_default', $insert_data);
            }
        }
        $set = unserialize($default);
        $bgimg = pdo_get('tapgo_tt_task_default',array('uniacid'=>$_W['uniacid']),array('bgimg','open'));
