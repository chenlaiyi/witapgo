...
class build_TapgoTtPage extends PluginPfMobilePage {
	...
	function main() {
		...
		//查找二维码
		$poster = pdo_fetch('select * from ' . tablename('tapgo_tt_task_poster') . ' where keyword=:keyword and uniacid=:uniacid and `status`=1 and `is_delete`=0 limit 1', array(':keyword' => $content, ':uniacid' => $_W['uniacid']));
        if (empty($poster)) {
        ...
        $is_waiting = false;
        //查找此类型任务是否正在参加
        $task_count = pdo_fetchcolumn('select COUNT(*) from ' . tablename('tapgo_tt_task_join') . ' where uniacid=:uniacid and join_user=:join_user and task_type='.$poster['poster_type'].' and is_reward=0 and failtime>'.time(), array(':uniacid' => $_W['uniacid'],':join_user'=>$member['openid']));
        if($task_count){
            //有
            $task_info = pdo_fetch('select `needcount`,`completecount`,`is_reward`,`failtime` from ' . tablename('tapgo_tt_task_join') . ' where uniacid=:uniacid and join_user=:join_user and task_id=:task_id and task_type=:task_type and  failtime>'.time().' order by `addtime` DESC limit 1', array(':uniacid' => $_W['uniacid'],':join_user'=>$member['openid'],':task_id'=>$poster['id'],':task_type'=>$poster['poster_type']));
            //判断是否是当前任务
            if($task_info){
            ...
            $other_task_count = pdo_fetchcolumn('select COUNT(*) from ' . tablename('tapgo_tt_task_join') . ' where uniacid=:uniacid and join_user=:join_user and task_type='.$poster_type.' and failtime>'.time(), array(':uniacid' => $_W['uniacid'],':join_user'=>$member['openid']));
            if($other_task_count){
                ...
                if($default){
                    ...
                    if($default['is_posterall']==1){
                        //判断是否有已结束的此类型任务
                        $end_task_count = pdo_fetchcolumn('select COUNT(*) from ' . tablename('tapgo_tt_task_join') . ' where uniacid=:uniacid and join_user=:join_user and task_type='.$poster['poster_type'].' and failtime<'.time(), array(':uniacid' => $_W['uniacid'],':join_user'=>$member['openid']));
                        if($end_task_count){
                            //是
                            $end_task_info = pdo_fetch('select `needcount`,`completecount`,`failtime` from ' . tablename('tapgo_tt_task_join') . ' where uniacid=:uniacid and join_user=:join_user and task_id=:task_id and task_type=:task_type and failtime<'.time().' order by `addtime` DESC limit 1', array(':uniacid' => $_W['uniacid'],':join_user'=>$member['openid'],':task_id'=>$poster['id'],':task_type'=>$poster['poster_type']));
                            //判断是否是当前任务
                            if($end_task_info){
                                //是
                                if($poster['is_repeat']){
                ...
                //判断是否有已结束的此类型任务
                $end_task_count = pdo_fetchcolumn('select COUNT(*) from ' . tablename('tapgo_tt_task_join') . ' where uniacid=:uniacid and join_user=:join_user and task_type='.$poster['poster_type'].' and (is_reward=1 or failtime<'.time().')', array(':uniacid' => $_W['uniacid'],':join_user'=>$member['openid']));
                if($end_task_count){
                    //是
	...
	private function join_task($member,$poster){
        ...
        pdo_insert('tapgo_tt_task_join', $task_join);
        $id = pdo_insertid();
        $img = '';
        if($id){
            //获取二维码图片
