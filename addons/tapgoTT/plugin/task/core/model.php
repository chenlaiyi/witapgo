...
class TaskModel extends PluginModel
{
    ...
    {
        global $_W;
        $sql = "select count(*) from " . tablename('tapgo_tt_task_reward') . " where openid = :openid and uniacid = :uniacid and `get` = 1 and `sent` = 0 and `read` = 0";
        return pdo_fetchcolumn($sql, array(':openid' => $_W['openid'], ':uniacid' => $_W['uniacid']));
    }
    public function set_read()
    {
        global $_W;
        pdo_update('tapgo_tt_task_reward', array('read' => 1), array('openid' => $_W['openid'], 'uniacid' => $_W['uniacid'],'get'=>1,'sent'=>0,'read'=>0));
    }

    public function isnew()
    {
        global $_W;
        $uniacid = pdo_fetchcolumn("select uniacid from " . tablename('tapgo_tt_task_set') . " where uniacid = {$_W['uniacid']}");
        if (empty($uniacid)) pdo_insert('tapgo_tt_task_set', array('uniacid' => $_W['uniacid']));
        if (is_mobile()) {
            $this->set_read();
        }
        return pdo_fetchcolumn('select isnew from ' . tablename('tapgo_tt_task_set') . " where uniacid = {$_W['uniacid']}");
    }
    ...
    public function getQR($poster, $member)
    {
        ...
        $qr = pdo_fetch('select * from ' . tablename('tapgo_tt_task_poster_qr') . ' where openid=:openid and acid=:acid and posterid=:posterid limit 1', array(':openid' => $member['openid'], ':acid' => $acid, ':posterid' => $poster['id']));
        if (empty($qr)) {
            $qr['current_qrimg'] = '';
            ...
            pdo_insert('tapgo_tt_task_poster_qr', $qr);
            $qr['id'] = pdo_insertid();
        } else {
            ...
            if ($time > $qr['endtime']) {
                ...
                pdo_update('tapgo_tt_task_poster_qr', array('ticket' => $ticket, 'qrimg' => $qrimg, 'url' => $result['url'], 'endtime' => $posterendtime), array('id' => $qr['id']));
                $qr['ticket'] = $ticket;
    ...
    public function createPoster($poster, $member, $qr, $upload = true)
    {
        ...
        global $_W;
        $path = IA_ROOT . "/addons/tapgo_ttv2/data/task/poster/" . $_W['uniacid'] . "/";
        ...
        $img = $_W['siteroot'] . "addons/tapgo_ttv2/data/task/poster/" . $_W['uniacid'] . "/" . $file;
        ...
        if ($qr['qrimg'] != $qr['current_qrimg'] || empty($qr['mediaid']) || empty($qr['createtime']) || $qr['createtime'] + 3600 * 24 * 3 - 7200 < time() || $is_new) {
            ...
            $qr['mediaid'] = $mediaid;
            $qr['img'] = $mediaid;
            pdo_update('tapgo_tt_task_poster_qr', array('mediaid' => $mediaid, 'createtime' => time()), array('id' => $qr['id']));
        }
    ...
    public function getQRByTicket($ticket = '')
    {
        ...
        $qrs = pdo_fetchall('select * from ' . tablename('tapgo_tt_task_poster_qr') . ' where ticket=:ticket and acid=:acid limit 1', array(':ticket' => $ticket, ':acid' => $_W['acid']));
        $count = count($qrs);
        if ($count <= 0) {
            return false;
        }
    ...
    public function checkMember($openid = '', $acc = '')
    {
        ...
        if (empty($member)) {
            ...
            pdo_insert('tapgo_tt_member', $member);
            ...
            pdo_update('tapgo_tt_member', $member, array('id' => $member['id']));
            $member['isnew'] = false;
        }
        return $member;
    }
    ...
    public function responseUnsubscribe($param = '')
    {
        ...
        if (isset($param['openid']) && !empty($param['openid'])) {
            ...
            $task_info = pdo_fetch('SELECT join_user FROM ' . tablename('tapgo_tt_task_join') . 'WHERE failtime>' . time() . ' and is_reward=0 and join_id in (SELECT join_id from ' . tablename('tapgo_tt_task_joiner') . ' where uniacid=:uniacid and joiner_id=:joiner_id and join_status=1)', array(':uniacid' => $_W['uniacid'], ':joiner_id' => $openid));
            if ($task_info) {
                $member = $this->checkMember($openid);
                pdo_update('tapgo_tt_task_joiner', array('join_status' => 0), $where);
                $updatesql = 'UPDATE ' . tablename('tapgo_tt_task_join') . ' SET completecount = completecount-1 WHERE failtime>' . time() . ' and is_reward=0 and join_id in (SELECT join_id from ' . tablename('tapgo_tt_task_joiner') . ' where uniacid=:uniacid and joiner_id=:joiner_id and join_status=1)';
    ...
    public function notice_complain($templete, $member, $poster, $scaner = '', $type = 1)
    {
        ...
        if ($templete) {
            ...
            $db_data = pdo_fetchcolumn('select `data` from ' . tablename('tapgo_tt_task_default') . ' where uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid']));
            $res = '';
            if (!empty($db_data)) {
    ...
    public function getdefault($key)
    {
        global $_W;
        if ($key) {
            $default = pdo_fetchcolumn('select `data` from ' . tablename('tapgo_tt_task_default') . ' where uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid']));
    ...
    public function getGoods($param = '')
    {
        ...
        global $_W;
        $search_sql = 'SELECT * FROM ' . tablename('tapgo_tt_task_join') . ' WHERE join_user= :openid AND uniacid = :uniacid AND `join_id`=:join_id  AND is_reward=1';
        $data = array(
            ':uniacid' => $_W['uniacid'],
            ':openid' => $param['openid'],
        ...
        if (isset($param['goods_num']) && !empty($param['goods_num'])) {
                    ...
                    if (isset($rec_reward['goods'][$goods_id]) && !empty($rec_reward['goods'][$goods_id])) {
                        ...
                        if ($goods_spec > 0) {
                            ...
                            if ($rec_reward['goods'][$goods_id]['spec'][$goods_spec]['total'] < 0) {
                                ...
                                $res = pdo_update('tapgo_tt_task_join', $update_data, $update_where);
                                if ($res) {
                                    return true;
                                } else {
                                    return false;
            ...
            if ($join_info['task_type'] == 1) {
                ...
                if (!empty($rec_reward)) {
                    $goods_id = intval($param['goods_id']);
                    if (isset($rec_reward['goods'][$goods_id]) && !empty($rec_reward['goods'][$goods_id])) {
                        $createtime_sql = 'SELECT `createtime` FROM ' . tablename('tapgo_tt_task_log') . ' WHERE openid= :openid AND uniacid = :uniacid AND `join_id`=:join_id  AND (recdata IS NOT NULL AND recdata !="") ';
                        ...
                        $rewardday_sql = 'SELECT `reward_days`,`is_goods` FROM ' . tablename('tapgo_tt_task_poster') . ' WHERE  uniacid = :uniacid AND `id`=:id  AND poster_type=:poster_type ';
    ...
    public function reward($member_info, $poster, $join_info, $qr, $openid, $qrmember)
    {
        ...
        if ($join_info['needcount'] == $count && $join_info['is_reward'] == 0) {
            ...
            pdo_update('tapgo_tt_task_join', array('completecount' => $count, 'is_reward' => 1, 'reward_data' => $reward), array('uniacid' => $_W['uniacid'], 'join_id' => $join_info['join_id'], 'join_user' => $qr['openid'], 'task_id' => $poster['id'], 'task_type' => 1));
            pdo_insert('tapgo_tt_task_log', $reward_log);
            $log_id = pdo_insertid();
            $scaner = array(
                'uniacid' => $_W['uniacid'],
                'task_user' => $qr['openid'],
            ...
            pdo_insert('tapgo_tt_task_joiner', $scaner);
                ...
                if ($key == 'rec') {
                    ...
                    if (isset($val['bribery']) && $val['bribery'] > 0) {
                        ...
                        if (!is_error($err)) {
                            ...
                            pdo_update('tapgo_tt_task_log', $upgrade, array('id' => $log_id));
                        }
                    }
            ...
            $default_text = pdo_fetchcolumn("SELECT `data` FROM " . tablename('tapgo_tt_task_default') . " WHERE uniacid=:uniacid limit 1", array(':uniacid' => $_W['uniacid']));
            ...
            pdo_update('tapgo_tt_task_join', array('completecount' => $count), array('uniacid' => $_W['uniacid'], 'join_user' => $qr['openid'], 'task_id' => $poster['id'], 'task_type' => 1));
            pdo_insert('tapgo_tt_task_log', $reward_log);
            $log_id = pdo_insertid();
            $scaner = array(
                'uniacid' => $_W['uniacid'],
            ...
            if ($val['needcount'] == $count) {
                if ($is_reward == 0) {
                    ...
                    if (!isset($val['is_reward']) || empty($val['is_reward'])) {
                        ...
                        pdo_update('tapgo_tt_task_join', array('reward_data' => $rec_data, 'is_reward' => 1), array('uniacid' => $_W['uniacid'], 'join_id' => $join_info['join_id'], 'join_user' => $qr['openid'], 'task_id' => $poster['id'], 'task_type' => 2));
                    } else {
    ...
    protected function reward_both($count, $reward_data, $qr, $join_info, $openid, $qrmember, $member_info, $poster)
    {
        ...
        pdo_update('tapgo_tt_task_join', array('completecount' => $count), array('uniacid' => $_W['uniacid'], 'join_id' => $join_info['join_id'], 'join_user' => $qr['openid'], 'task_id' => $poster['id'], 'task_type' => 2));
        pdo_insert('tapgo_tt_task_log', $reward_log);
        $log_id = pdo_insertid();
        $scaner = array(
            'uniacid' => $_W['uniacid'],
    ...
    protected function reward_scan($count, $reward_data, $qr, $join_info, $openid, $qrmember, $member_info, $poster)
    {
        ...
        pdo_update('tapgo_tt_task_join', array('completecount' => $count), array('uniacid' => $_W['uniacid'], 'join_user' => $qr['openid'], 'task_id' => $poster['id'], 'task_type' => 2));
        pdo_insert('tapgo_tt_task_log', $reward_log);
        $log_id = pdo_insertid();
        $scaner = array(
            'uniacid' => $_W['uniacid'],
    ...
    function checkTaskReward($taskclass = '', $num = 1, $openid = '')
    {
        ...
        $sql = "SELECT * FROM " . tablename('tapgo_tt_task_extension_join') .
        ...
        foreach ($allTask as $tk => $tv) {
            ...
            pdo_update('tapgo_tt_task_extension_join', array('progress_data' => $progress_data), array('uniacid' => $_W['uniacid'], 'id' => $tv['id']));
            ...
            if ($isreward) {
                pdo_update('tapgo_tt_task_extension_join', array('completetime' => time()), array('uniacid' => $_W['uniacid'], 'id' => $tv['id']));
    ...
    function sendReward($reward_data = array(), $btn = 0, $openid = null, $rewardid = 0)
    {
        ...
        if (!empty($reward_data['coupon']) && is_array($reward_data['coupon'])) {
            foreach ($reward_data['coupon'] as $k => $v) {
                ...
                pdo_insert('tapgo_tt_coupon_data', $data);
            }
        }
        ...
        pdo_update('tapgo_tt_task_extension_join', array('rewarded' => $rewarded), array('id' => $rewardid, 'uniacid' => $_W['uniacid']));
    }
    ...
    function getNewTask($id)
    {
        ...
        $nowtime = time();
        $sql = "SELECT * FROM " . tablename("tapgo_tt_task") . " WHERE id = :id AND status = 1 AND starttime < {$nowtime} AND endtime >{$nowtime} AND uniacid = :uniacid";
        ...
        $data['logo'] = $task['logo'];
        pdo_insert("tapgo_tt_task_extension_join", $data);
    ...
    function getTaskLixt($action, $page)
    {
        ...
        $psize = 20;
        $pstart = ($page - 1) * $psize;
        $sql = "SELECT id,title,starttime,endtime,status FROM " . tablename("tapgo_tt_task") . " WHERE `type` = :type AND uniacid = :uniacid ORDER BY endtime DESC LIMIT {$pstart},{$psize}";
    ...
    function taskFilter($task)
    {
        ...
        switch ($type) {
            case 1:
                $sql = "SELECT COUNT(*) FROM " . tablename('tapgo_tt_task_extension_join') . " WHERE taskid = :taskid AND openid = :openid AND uniacid = :uniacid";
                $all = pdo_fetchcolumn($sql, array(':taskid' => $task['id'], ':uniacid' => $_W['uniacid'], ':openid' => $_W['openid']));
                if (!empty($all)) return '已参加过';
                break;
            case 2:
                $sql = "SELECT COUNT(*) FROM " . tablename('tapgo_tt_task_extension_join') . " WHERE taskid = :taskid AND openid = :openid AND completetime = 0 AND uniacid = :uniacid";
                $res = pdo_fetchcolumn($sql, array(':taskid' => $task['id'], ':uniacid' => $_W['uniacid'], ':openid' => $_W['openid']));
                if (!empty($res)) {
                    return '任务未完成不能继续领';
                }
                $sql1 = "SELECT completetime FROM " . tablename('tapgo_tt_task_extension_join') . " WHERE taskid = :taskid AND openid = :openid AND uniacid = :uniacid ORDER BY completetime DESC";
                ...
                $sql2 = "SELECT COUNT(*) FROM " . tablename('tapgo_tt_task_extension_join') . " WHERE taskid = :taskid AND uniacid = :uniacid AND openid = :openid AND completetime > {$hourl} AND completetime < {$hourr} AND completetime != 0";
    ...
    function getRecordsList($page, $taskid)
    {
        ...
        $psize = 20;
        $pstart = ($page - 1) * $psize;
        $sql = "SELECT * FROM " . tablename("tapgo_tt_task_log") . " WHERE taskid = :taskid AND uniacid = :uniacid ORDER BY id DESC LIMIT {$pstart},{$psize}";
    ...
    function getUserTaskList($type)
    {
        ...
        $sql = "SELECT * FROM " . tablename('tapgo_tt_task') . " WHERE status = 1 {$condition} AND starttime < {$time} AND endtime > {$time} AND uniacid = :uniacid";
    ...
    function getMyTaskList($condition = '=')
    {
        ...
        $sql = "SELECT a.* FROM " . tablename("tapgo_tt_task_extension_join") . " a JOIN " . tablename('tapgo_tt_task') . " b ON a.taskid = b.id WHERE a.openid = :openid AND a.completetime {$condition} 0 {$condition2} AND a.uniacid = :uniacid";
    ...
    {
        global $_W;
        $sql = "SELECT a.* FROM " . tablename("tapgo_tt_task_extension_join") . " a JOIN " . tablename('tapgo_tt_task') . " b ON a.taskid = b.id WHERE a.openid = :openid AND a.completetime = 0 AND a.endtime < " . time() . " AND a.uniacid = :uniacid";
        return pdo_fetchall($sql, array(':uniacid' => $_W['uniacid'], ':openid' => $_W['openid']));
    }
    ...
    function returnName($taskclass)
    {
        ...
        $sql = "SELECT taskname FROM " . tablename('tapgo_tt_task_extension') . " WHERE taskclass = :taskclass";
    ...
    {
        global $_W;
        $sql = "SELECT title FROM " . tablename('tapgo_tt_goods') . " WHERE id = :id AND uniacid = :uniacid";
    ...
    function sendmessage($data)
    {
        ...
        if (strexists($url, '/addons/tapgo_ttv2/')) {
            $url = str_replace("/addons/tapgo_ttv2/", '/', $url);
        }
    ...
    public $tapgo_tt_task_list = array(
    ...
    public $tapgo_tt_task_set = array('uniacid', 'entrance', 'keyword', 'cover_title', 'cover_img', 'cover_desc', 'msg_pick', 'msg_progress', 'msg_finish', 'msg_follow', 'isnew', 'bg_img', 'top_notice');
    public $tapgo_tt_task_record = array('id', 'uniacid', 'taskid', 'tasktitle', 'tasktype', 'task_demand', 'openid', 'nickname', 'picktime', 'stoptime', 'finishtime', 'task_progress', 'reward_data', 'followreward_data', 'taskimage', 'design_data', 'design_bg', 'require_goods', 'level1', 'level2', 'reward_data1', 'reward_data2', 'member_group', 'auto_pick');

    function __construct($name = '')
    {
        parent::__construct($name);
        $this->taskType = pdo_getall('tapgo_tt_task_type');
    ...
    function getAllTask(&$page, $psize = 15)
    {
        ...
        $sql = "select {$field} from " . tablename('tapgo_tt_task_list') . " where uniacid = :uniacid {$condition}  order by displayorder desc,id desc limit {$pstart} , {$psize}";
    ...
    public function deleteTask($ids)
    {
        ...
        $tasks = pdo_fetchall("select `id`,`type`,`we7_rule_keyword_id` from ". tablename('tapgo_tt_task_list') . " where id in ({$idString})");
        ...
        if ($isArr) {
            ...
            if ($hasKeywordTask) {
                ...
                pdo_query("delete from ". tablename('rule_keyword') . " where id in ({$hasKeywordTaskIds})");
                    ...
                    pdo_query("delete from " . tablename('tapgo_tt_task_list') . " where {$condition}");
            return pdo_query("delete from " . tablename('tapgo_tt_task_record') . " where {$condition_record}");
        }
        if (!$isArr && !$isNum) {
            return false;
        }
    ...
    public function getThisTask($id)
    {
        ...
        return pdo_get('tapgo_tt_task_list', array('id' => $id, 'uniacid' => $_W['uniacid']));
    }
    ...
    public function getAllRecords(&$page, $psize = 20)
    {
        ...
        $field = '*';
        $sql = "select {$field} from " . tablename('tapgo_tt_task_record') . "where uniacid = :uniacid {$condition} order by id desc limit {$pstart} , {$psize}";
        $return = pdo_fetchall($sql, $params);
        $countsql = substr($sql, 0, strpos($sql, 'order by'));
        $countsql = str_replace($field, 'count(*)', $countsql);
        $count = pdo_fetchcolumn($countsql, $params);
    ...
    public function getAllRewards(&$page, $psize = 20)
    {
        ...
        $field = '*';
        $sql = "select {$field} from " . tablename('tapgo_tt_task_reward') . " where uniacid = :uniacid {$condition} and `get` = 1 order by id desc limit {$pstart} , {$psize}";
        $return = pdo_fetchall($sql, $params);
        $countsql = substr($sql, 0, strpos($sql, 'order by'));
        $countsql = str_replace($field, 'count(*)', $countsql);
        $count = pdo_fetchcolumn($countsql, $params);
    ...
    public function getGoods_new($keyword = '', $page = 1)
    {
        ...
        $sql = "select {$field} from " . tablename('tapgo_tt_goods') . "where uniacid = :uniacid and status = 1 and deleted = 0{$like} limit {$pstart} , $psize";
    ...
    public function getCoupon($keyword = '', $page = 1)
    {
        ...
        $field = 'id,couponname';
        $like = $keyword === '' ? $keyword : " and title like %{$keyword}%";
        $sql = "select {$field} from " . tablename('tapgo_tt_coupon') . "where uniacid = :uniacid {$like} limit {$pstart} , $psize";
        $countsql = substr($sql, 0, strpos($sql, 'limit'));
        $countsql = str_replace($field, 'count(*)', $countsql);
        $params = array(':uniacid' => $_W['uniacid']);
        $count = pdo_fetchcolumn($countsql, $params);
    ...
    public function send2wechat($recordid, $openid = '')
    {
        ...
        $posterInfo = pdo_fetch("select * from ".tablename('tapgo_tt_task_qr')." where openid=:openid and recordid=:recordid", array(':openid'=>$openid, ':recordid'=>$recordid));
        $taskRecord = pdo_fetch("select * from ". tablename('tapgo_tt_task_record'). ' where openid=:openid and id=:recordid', array(':openid'=>$openid, ':recordid'=>$recordid));
        $taskInfo = pdo_fetch("select * from ". tablename('tapgo_tt_task_list') . " where id=:id", array(':id'=>$taskRecord['taskid']));
        if($posterInfo['poster_version'] != $taskInfo['poster_version']) {
            ...
            pdo_update('tapgo_tt_task_qr', array(
                'mediaid' => $mediaid,
                'poster_version'=> $taskInfo['poster_version']
            ), array(
                'id'=>$posterInfo['id']
    ...
    protected function checkCanPick($task, $openid)
    {
        ...
        if ($task_start > $time) {
            ...
            $sql = "select * from " . tablename('tapgo_tt_task_record') . " where openid = :openid and taskid = :taskid and uniacid = :uniacid order by id desc";
    ...
    protected function createPoster2($poster, $member = 0, $upload = true, $update = false)
    {
        ...
        if (empty($qr['mediaid'])) {
            ...
            $qr['mediaid'] = $mediaid;
            $qr['img'] = $mediaid;
            pdo_update('tapgo_tt_task_qr', array('mediaid' => $mediaid), array('id' => $qr['id']));
        }
    ...
    protected function getQR2($poster, $member)
    {
        ...
        $this->createRule();
        $qr = pdo_fetch('select * from ' . tablename('tapgo_tt_task_qr') . ' where openid = :openid and uniacid = :uniacid and recordid = :recordid limit 1', array(':openid' => $member['openid'], ':uniacid' => $_W['uniacid'], ':recordid' => $poster['id']));
        if (empty($qr)) {
            ...
            $ims_qrcode = array(
                ...
                "model" => 0,
                "name" => "TAPGO_TT_TASKNEW_POSTER",
                "keyword" => 'TAPGO_TT_TASKNEW',
                "expire" => $expire,
                "createtime" => time(),
                "status" => 1,
                'url' => $result['url'],
            ...
            pdo_insert('tapgo_tt_task_qr', $qr);
            $qr['id'] = pdo_insertid();
        }
        return $qr;
    }
    ...
    {
        global $_W;
        $ruleSql = "select id from " . tablename('rule') . " where `name` = 'tapgo_ttv2:task' and `module` = 'tapgo_ttv2' and uniacid = {$_W['uniacid']}";
        $ruleCount = pdo_fetchcolumn($ruleSql);

        if (empty($ruleCount)) {
            $rule = array('uniacid' => $_W['uniacid'], 'name' => 'tapgo_ttv2:task', 'module' => 'tapgo_ttv2', 'status' => 1);
        ...
        $keywordSql = "select COUNT(*) from " . tablename('rule_keyword') . " where `content` = 'TAPGO_TT_TASKNEW' and uniacid = {$_W['uniacid']}";
        $keywordCount = pdo_fetchcolumn($keywordSql);
        if (empty($keywordCount)) {
            $keyword = array('rid' => $ruleCount, 'uniacid' => $_W['uniacid'], 'module' => 'tapgo_ttv2', 'content' => 'TAPGO_TT_TASKNEW', 'type' => 1, 'status' => 1);
    ...
    public function checkTaskProgress($num, $typeKey, $recordid = 0, $openid = '', $goodsid = 0)
    {
        ...
        $sqlPassive = "select * from " . tablename('tapgo_tt_task_list') . " where uniacid = :uniacid and picktype = 1 and starttime < :starttime and endtime > :endtime";
        ...
        if (!empty($passiveTask))
            foreach ($passiveTask as $task) {
                if (!pdo_fetchcolumn("select count(*) from " . tablename('tapgo_tt_task_record') . " where taskid = {$task['id']} and uniacid = {$_W['uniacid']}"))
        ...
        $sql = "select * from " . tablename('tapgo_tt_task_record') . " where uniacid = :uniacid {$condtion} and openid = :openid and tasktype = :tasktype and (stoptime = '0000-00-00 00:00:00' or stoptime >'{$time}') and finishtime = '0000-00-00 00:00:00'";
        ...
        if (!empty($allRecord))
            foreach ($allRecord as $record) {
                ...
                if ($record['task_progress'] >= $record['task_demand']) {
                    ...
                    $ret = pdo_update('tapgo_tt_task_record', $update_arr, array('id' => $record['id']));
                    if ($ret) {
                    ...
                    if ($type['once'] != 1) {
                        pdo_update('tapgo_tt_task_record', array('task_progress' => $record['task_progress']), array('id' => $record['id']));
    ...
    public function sentReward($recordid, $openid, $level = 0)
    {
        global $_W;
        $time = date('Y-m-d H:i:s');
        $sql = "select * from " . tablename('tapgo_tt_task_reward') . " where recordid = :recordid and openid = :openid and uniacid = :uniacid and `get` = 0 and sent = 0 and `level` = {$level} ";
            ...
            foreach ($rewards as $k => $reward) {
                switch ($reward['reward_type']) {
                    case 'credit':
                        ...
                        pdo_update('tapgo_tt_task_reward', array('sent' => 1, 'get' => 1, 'gettime' => $time, 'senttime' => $time), array('id' => $reward['id']));
                        break;
                    case 'balance':
                        ...
                        break;
                    case 'redpacket':
                        pdo_update('tapgo_tt_task_reward', array('get' => 1, 'gettime' => $time), array('id' => $reward['id']));
                        break;
                    case 'coupon':
                        $data = array(
                            'uniacid' => $_W['uniacid'],
    ...
    public function followReward($recordid)
    {
        ...
        $record = pdo_fetch("select * from " . tablename('tapgo_tt_task_record') . " where id = :id and uniacid = :uniacid", array(':id' => $recordid, ':uniacid' => $_W['uniacid']));
            ...
            switch ($k) {
                case 'credit':
                    if ($reward > 0) {
                        ...
                        pdo_insert('tapgo_tt_task_reward', array('uniacid' => $_W['uniacid'], 'taskid' => $record['taskid'], 'tasktitle' => $record['tasktitle'], 'tasktype' => $record['tasktype'], 'taskowner' => $record['openid'], 'ownernickname' => $record['nickname'], 'recordid' => $record['id'], 'nickname' => $info['nickname'], 'headimg' => $info['avatar'], 'openid' => $_W['openid'], 'reward_type' => 'credit', 'reward_title' => $reward . '积分', 'reward_data' => $reward, 'sent' => 1, 'get' => 1, 'gettime' => $time, 'senttime' => $time, 'isjoiner' => 1));
                    }
    ...
    public function getTaskRewardGoodsInfo($taskrewardgoodsid = 0)
    {
        ...
        $reward = pdo_get('tapgo_tt_task_reward', array('id' => $taskrewardgoodsid, 'openid' => $_W['openid'], 'uniacid' => $_W['uniacid'], 'get' => 1, 'sent' => 0, 'reward_type' => 'goods'));
        if (empty($reward)) {
            return false;
        }
        return $reward;
    ...
    public function setTaskRewardGoodsSent($taskrewardgoodsid = 0)
    {
        ...
        pdo_update('tapgo_tt_task_reward', array('sent' => 1, 'senttime' => date('Y-m-d H:i:s')), array('id' => $taskrewardgoodsid));
    ...
    {
        global $_W;
        $sql = "select entrance from " . tablename('tapgo_tt_task_set') . " where uniacid = {$_W['uniacid']}";
    ...
    {
        global $_W;
        $sql = "select top_notice from " . tablename('tapgo_tt_task_set') . " where uniacid = {$_W['uniacid']}";
        return pdo_fetchcolumn($sql);
    }
    ...
    public function getMessageSet($field = array('msg_pick', 'msg_progress', 'msg_finish', 'msg_follow'))
    {
        ...
        $msg = pdo_get('tapgo_tt_task_set', array('uniacid' => $_W['uniacid']), $field2);
        if ($isString) {
            return $msg[$field];
        }
        return $msg;
    ...
    public function checkGoodsock($task){
       ...
       if(!empty($task['requiregoods'])){
           ...
           foreach($goodsids as $key=>$val){
               $goods = pdo_fetch('select id,hasoption,total,title from ' . tablename('tapgo_tt_goods') . ' where uniacid=:uniacid and id=:id limit 1', array(':uniacid' => $_W['uniacid'], ':id' =>$val));
               if($goods['hasoption']>0){
                   $goodsoption = pdo_fetchall('select * from ' . tablename('tapgo_tt_goods_option') . ' where uniacid=:uniacid and goodsid=:id ', array(':uniacid' => $_W['uniacid'], ':id' =>$val));
    ...
    public function checkRewardStock($task){
        ...
        if(!empty($reward['goods'])){
                foreach($reward['goods'] as $key=>$val){
                    $goods = pdo_fetch('select id,hasoption,total,title from ' . tablename('tapgo_tt_goods') . ' where uniacid=:uniacid and id=:id limit 1', array(':uniacid' => $_W['uniacid'], ':id' =>$val['id']));
                    if($goods['hasoption']>0){
                        $goodsoption = pdo_fetchall('select * from ' . tablename('tapgo_tt_goods_option') . ' where uniacid=:uniacid and goodsid=:id ', array(':uniacid' => $_W['uniacid'], ':id' =>$val['id']));
                        foreach($goodsoption as $k =>$v){
                            if($v['stock']>0||$v['stock'] == -1 ){
                                $enough = true;
                            }else{
        ...
        if(!empty($reward['goods'])){
            foreach($reward['goods'] as $key=>$val){
                ...
                if($goods['hasoption']>0){
                    ...
                    $goodsoption = pdo_fetchall('select id,stock from ' . tablename('tapgo_tt_goods_option') . ' where uniacid=:uniacid and goodsid=:id ', array(':uniacid' => $_W['uniacid'], ':id' =>$val['id']));
