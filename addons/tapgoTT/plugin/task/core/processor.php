...
require(TAPGO_TT_INC . "plugin_processor.php");
...
class TaskProcessor extends PluginProcessor
{
    ...
    private function responseText_new($obj)
    {
        ...
        $id = pdo_fetchcolumn("select id from " . tablename("tapgo_tt_task_list") . " where keyword_pick = :keyword_pick and uniacid = :uniacid", array( ":uniacid" => $_W["uniacid"], ":keyword_pick" => $content ));
        $timeout = 4;
    ...
    public function responseSubscribe_new($obj)
    {
        ...
        $recordid = pdo_fetchcolumn("select recordid from " . tablename("tapgo_tt_task_qr") . " where uniacid = :uniacid and sceneid = :sceneid", array( ":uniacid" => $_W["uniacid"], ":sceneid" => $eventkey ));
        $record = pdo_fetch("select openid,member_group,tasktitle,auto_pick,taskid from " . tablename("tapgo_tt_task_record") . " where uniacid = :uniacid and id = :recordid", array( ":uniacid" => $_W["uniacid"], ":recordid" => $recordid ));
        $list = pdo_fetch("select member_level,followreward from " . tablename("tapgo_tt_task_list") . " where id = :id and uniacid = :uniacid ", array( ":id" => $record["taskid"], ":uniacid" => $_W["uniacid"] ));
            ...
            if( $p ) 
            {
                ...
                if( $member["isagent"] != 1 ) 
                {
                    ...
                    if( $agent["isagent"] ) 
                    {
                        ...
                        {
                            $a = pdo_update("tapgo_tt_member", array( "isagent" => 1, "agentid" => $agent["id"], "status" => $become_check, "childtime" => time(), "agenttime" => time() ), array( "id" => $member["id"] ));
                            if( $become_check == 1 ) 
                            {
                        ...
                        {
                            pdo_update("tapgo_tt_member", array( "agentid" => $agent["id"], "childtime" => time() ), array( "id" => $member["id"] ));
                        }
                    ...
                    else
                    {
                        if( !empty($beagent["beagent"]) ) 
                        {
                            pdo_update("tapgo_tt_member", array( "isagent" => 1, "agentid" => $agent["id"], "status" => $become_check, "childtime" => time(), "agenttime" => time() ), array( "id" => $member["id"] ));
                            if( $become_check == 1 ) 
                            {
                                $p->sendMessage($member["openid"], array( "nickname" => $member["nickname"], "agenttime" => time() ), TM_COMMISSION_BECOME);
                                $p->upgradeLevelByAgent($agent["id"]);
    ...
    public function saleVirtual($obj = NULL)
    {
        ...
        if( $open_redis ) 
        {
            ...
            if( !is_error($redis) ) 
            {
                ...
                {
                    $totalagent = pdo_fetchcolumn("select count(*) from" . tablename("tapgo_tt_member") . " where uniacid =" . $_W["uniacid"] . " and isagent =1");
                    $totalmember = pdo_fetchcolumn("select count(*) from" . tablename("tapgo_tt_member") . " where uniacid =" . $_W["uniacid"]);
    ...
    private function responseScan($obj)
    {
        ...
        $poster = pdo_fetch("select * from " . tablename("tapgo_tt_task_poster") . " where id=:id and uniacid=:uniacid limit 1", array( ":id" => $qr["posterid"], ":uniacid" => $_W["uniacid"] ));
        if( empty($poster) ) 
        {
            return $this->responseDefault($obj);
        }
        ...
        else
        {
            ...
            $scan_count = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename("tapgo_tt_task_joiner") . " where uniacid=:uniacid and task_user=:task_user and joiner_id=:joiner_id and join_id=:join_id ", $params);
    ...
    private function commission($poster, $member, $qrmember)
    {
        ...
        if( $p ) 
        {
            ...
            if( !empty($cset) && $member["isagent"] != 1 && $qrmember["isagent"] == 1 && $qrmember["status"] == 1 ) 
            {
                ...
                {
                    pdo_update("tapgo_tt_member", array( "agentid" => $qrmember["id"], "childtime" => $time ), array( "id" => $member["id"] ));
                ...
                {
                    $become_check = intval($cset["become_check"]);
                    pdo_update("tapgo_tt_member", array( "isagent" => 1, "status" => $become_check, "agenttime" => $time ), array( "id" => $member["id"] ));
                    if( $become_check == 1 ) 
                    {
                        $p->sendMessage($member["openid"], array( "nickname" => $member["nickname"], "agenttime" => $time ), TM_COMMISSION_BECOME);
                        $p->upgradeLevelByAgent($qrmember["id"]);
