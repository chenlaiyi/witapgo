...
class Apply_TapgoTtPage extends PluginWebPage {
    ...
    function main() {
        ...
        $sql = "select a.*, m.nickname,m.avatar,m.realname,m.mobile,m.agentlevel,l.levelname,l.levelname,a.realname as applyrealname,a.sendmoney from " . tablename('tapgo_tt_commission_apply') . " a "
            . " left join " . tablename('tapgo_tt_member') . " m on m.id = a.mid"
            . " left join " . tablename('tapgo_tt_commission_level') . " l on l.id = m.agentlevel"
        ...
        if ($status == 3){
            $realmoney_total = (float)pdo_fetchcolumn("select sum(a.realmoney) from " . tablename('tapgo_tt_commission_apply') . " a "
                . " left join " . tablename('tapgo_tt_member') . " m on m.id = a.mid"
                . " left join " . tablename('tapgo_tt_commission_level') . " l on l.id = m.agentlevel"
        ...
        if ($_GPC['export'] == '1') {
            ...
            foreach ($list as &$row) {
                ...
                $orders = pdo_fetchall("select id,agentid, ordersn,price,goodsprice,dispatchprice,createtime, paytype from " . tablename('tapgo_tt_order') . " where  id in ( " . implode(",", $ids) . " );");
                $totalcommission = 0;
                $totalpay = 0;
                $passmoney = 0;
                $ordergoodscount = 0;
                foreach ($orders as &$order) {
                    ...
                    $goods = pdo_fetchall("SELECT og.id,g.thumb,og.price,og.realprice, og.total,g.title,o.paytype,og.optionname,og.commission1,og.commission2,og.commission3,og.commissions,og.status1,og.status2,og.status3,og.content1,og.content2,og.content3 from " . tablename('tapgo_tt_order_goods') . " og"
        ...
        $total = pdo_fetchcolumn("select count(a.id) from" . tablename('tapgo_tt_commission_apply') . " a "
            . " left join " . tablename('tapgo_tt_member') . " m on m.uid = a.mid"
            . " left join " . tablename('tapgo_tt_commission_level') . " l on l.id = m.agentlevel"
            . " where 1 {$condition}", $params);
    ...
    protected  function applyData(){
        ...
        $apply = pdo_fetch('select * from ' . tablename('tapgo_tt_commission_apply') . ' where uniacid=:uniacid and id=:id limit 1'
        ...
        $listpage = pdo_fetchall("select id,agentid, ordersn,price,goodsprice, dispatchprice,createtime, paytype from " . tablename('tapgo_tt_order') . " where  id in ( " . implode(",", $ids) . " ) order by id desc LIMIT ".($pindex - 1) * $psize .",".$psize."");
         $list = pdo_fetchall("select id,agentid, ordersn,price,goodsprice, dispatchprice,createtime, paytype from " . tablename('tapgo_tt_order') . " where  id in ( " . implode(",", $ids) . " ) order by id desc ");
        $count = pdo_fetchcolumn('select count(*) from ' . tablename('tapgo_tt_order') . " where id in(".implode(",",$ids).")");
        $totalcommission = 0;
        $totalpay = 0;
        foreach ($list as &$row) {
            //判断是几级订单
...
//            dump($goods);exit;
        ...
        unset($row);
        $totalcount = $total = pdo_fetchcolumn("select count(*) from " . tablename('tapgo_tt_order') . ' o '
            . " left join " . tablename('tapgo_tt_member') . " m on o.openid = m.openid "
    ...
    function get_list($page=0) {
        ...
        $listpage = pdo_fetchall("select id,agentid, ordersn,price,goodsprice, dispatchprice,createtime, paytype from " . tablename('tapgo_tt_order') . " where  id in ( " . implode(",", $ids) . " ) order by id desc ".$limit."" );
...
//            dump($goods);exit;
    ...
    function check() {
        ...
        $ogids = array();
        foreach ($list as $row) {
            $goods = pdo_fetchall("SELECT id from " . tablename('tapgo_tt_order_goods') . " where uniacid = :uniacid and orderid=:orderid and nocommission=0", array(':uniacid' => $_W['uniacid'], ':orderid' => $row['id']));
            foreach ($goods as $g) {
                $ogids[] = $g['id'];
            }
        }
        ...
        foreach ($ogids as $ogid) {
            $g = pdo_fetch("SELECT total, commission1,commission2,commission3,commissions from " . tablename('tapgo_tt_order_goods') . "  "
            ...
            if (!empty($update)) {
                pdo_update('tapgo_tt_order_goods', $update, array('id' => $ogid));
            }
        }

        if ($isAllUncheck) {
            pdo_update('tapgo_tt_commission_apply', array('status' => -1, 'invalidtime' => $time), array('id' => $id, 'uniacid' => $_W['uniacid']));
        } else {
            pdo_update('tapgo_tt_commission_apply', array('status' => 2, 'checktime' => $time), array('id' => $id, 'uniacid' => $_W['uniacid']));
    ...
    function check_ajax() {
        ...
        foreach ($ogids as $ogid) {
            $g = pdo_fetch("SELECT total, orderid,commission1,commission2,commission3,commissions from " . tablename('tapgo_tt_order_goods') . "  "
                . "where id=:id and uniacid = :uniacid limit 1", array(':uniacid' => $_W['uniacid'], ':id' => $ogid));
            if (empty($g)) {
                continue;
            }
    ...
    function cancel() {
        ...
        foreach ($list as $row) {
            ...
            foreach ($row['goods'] as $g) {
                ...
                if (!empty($update)) {
                    pdo_update('tapgo_tt_order_goods', $update, array('id' => $g['id']));
                }
            }
        }
        pdo_update('tapgo_tt_commission_apply', array('status' => 1, 'checktime' => 0, 'invalidtime' => 0), array('id' => $id, 'uniacid' => $_W['uniacid']));
    ...
    function refuse() {
        ...
        pdo_update('tapgo_tt_commission_apply', array('status' => -2, 'checktime' => 0, 'invalidtime' => time(),'refusetime'=>time()), array('id' => $id, 'uniacid' => $_W['uniacid']));
...
//        $pay = $totalpay;
        ...
        if ($apply['type'] < 2) {
            ...
            //pdo_update('tapgo_tt_commission_apply', $updateno, array('id' => $apply['id']));
            ...
            if (!empty($data['paytype']['commission']) && $apply['type']== 1){
                if(strpos($member['openid'],'sns_wa_')===false){
                    ...
                    pdo_update('tapgo_tt_commission_apply',array(
            ...
            if (is_error($result)) {
                ...
                if(strexists($err_msg, '更换商户单号')){
                    ...
                    pdo_update('tapgo_tt_commission_apply', array('applyno'=>$applyno), array('id' => $apply['id']));
                    $err_msg = $err_msg.',请重试!';
                }
                show_json(0,$err_msg);
            }
        ...
        pdo_update('tapgo_tt_commission_apply', array('status' => 3, 'paytime' => $time, 'commission_pay' => $totalpay, 'realmoney' => $realmoney, 'deductionmoney' => $deductionmoney), array('id' => $id, 'uniacid' => $_W['uniacid']));
        ...
        pdo_insert('tapgo_tt_commission_log', $log);
    ...
    function changecommission() {
        ...
        $order = pdo_fetch('select * from ' . tablename('tapgo_tt_order') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
        if (empty($order)) {
            if ($_W['ispost']) {
        ...
        if ($_W['ispost']) {
            ...
            foreach ($order_goods_change as $og) {
                ...
                pdo_update('tapgo_tt_order_goods', array('commissions' => iserializer($commissions)), array('id' => $og['id']));
            }
