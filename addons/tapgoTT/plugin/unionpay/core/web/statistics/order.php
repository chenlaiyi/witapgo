...
class Order_TapgoTtPage extends WebPage {
    ...
    protected  function orderData($status, $st){
        ...
        if (!empty($_GPC['searchfield']) && !empty($_GPC['keyword'])) {
            ...
            if ($searchfield == 'ordersn') {
                ...
                $condition .= " AND o.openid in (select openid from " . tablename('tapgo_tt_member') . " where realname like :keyword or mobile=:keyword or nickname like :keyword)";
                ...
                $condition .= " AND verifyopenid in (select openid from " . tablename('tapgo_tt_member') . " where realname like :keyword or mobile=:keyword or nickname like :keyword)";
        ...
        if ($condition != ' o.uniacid = :uniacid and o.ismr=0 and o.deleted=0 and o.isparent=0 and o.agentid>0' || !empty($sqlcondition)){
            ...
            $sql = "select o.*  from " . tablename('tapgo_tt_order') . " o"
                . " $sqlcondition where $condition $statuscondition GROUP BY o.id ORDER BY o.createtime DESC  ";
            ...
            if (!empty($list)){
                ...
                $verifyopenid_merch_array = pdo_fetchall("SELECT sm.id as salerid,sm.nickname as salernickname,sm.openid,s.salername FROM " .tablename('tapgo_tt_merch_saler')." s LEFT JOIN ".tablename('tapgo_tt_member')." sm ON sm.openid = s.openid and sm.uniacid=s.uniacid WHERE s.openid IN ({$verifyopenid})",array(),'openid');
                foreach ($list as $key=>&$value){
            ...
            $sql = "select * from " . tablename('tapgo_tt_order') . " where uniacid = :uniacid and ismr=0 and deleted=0 and isparent=0 and agentid > 0 {$status_condition} GROUP BY id ORDER BY createtime DESC  ";
            if (empty($_GPC['export'])) {
                $sql.="LIMIT " . ($pindex - 1) * $psize . ',' . $psize;
            }
            $list = pdo_fetchall($sql, $paras);
