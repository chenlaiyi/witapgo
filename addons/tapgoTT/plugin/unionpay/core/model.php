...
if( !class_exists("UnionpayModel") )
{

class UnionpayModel extends PluginModel
{
    /**
     * 获取银联下级机构upid、统计直属下级机构数、统计所有下级机构数
     * @param $id String 待查找的upid
     * @return array
     **/
    public function getSub($id){
        $res = pdo_fetchall("SELECT * FROM ".tablename('tapgo_tt_unionpay_agent')." where upagentid = ".$id);
        $uplevel1 = count($res);
        if ($res) {
            $upids = '';
            foreach ($res as $k => $v) {
        ...
        $ids =$this->getSubIds($id);
        if ($ids){
            $alldown    =   count(explode(',',$ids));
        }else{
            $alldown    =   0 ;
        }
        $downcount['uplevel1'] = $uplevel1;//直接下级机构数
        $downcount['alldown'] = $alldown;//所有下级机构数
        $downcount['upids'] = $upids;//直接下级机构upid
        $downcount['ids'] = $ids;   //所有下级机构upID
        $this->getSub = $downcount;
        return $this->getSub;
    }
    /**
     * 获取所有银联下级机构upid
     ...
     **/
    public function getSubIds($id,$keys=0){
        if($keys){
            $res = pdo_fetchall("SELECT * FROM ".tablename('tapgo_tt_unionpay_agent')." where upagentid in (".$id.")");
        }else{
            $res = pdo_fetchall("SELECT * FROM ".tablename('tapgo_tt_unionpay_agent')." where upagentid = ".$id);
        }
        if ($res) {
    ...
    public function getSet($uniacid = 0)
    {
        ...
        $set["texts"] = array( "agent" => (empty($set["texts"]["agent"]) ? "分销商" : $set["texts"]["agent"]), "shop" => (empty($set["texts"]["shop"]) ? "小店" : $set["texts"]["shop"]), "myshop" => (empty($set["texts"]["myshop"]) ? "我的小店" : $set["texts"]["myshop"]), "center" => (empty($set["texts"]["center"]) ? "分销中心" : $set["texts"]["center"]), "become" => (empty($set["texts"]["become"]) ? "成为分销商" : $set["texts"]["become"]), "withdraw" => (empty($set["texts"]["withdraw"]) ? "提现" : $set["texts"]["withdraw"]), "commission" => (empty($set["texts"]["commission"]) ? "佣金" : $set["texts"]["commission"]), "commission1" => (empty($set["texts"]["commission1"]) ? "分销佣金" : $set["texts"]["commission1"]), "commission_total" => (empty($set["texts"]["commission_total"]) ? "累计佣金" : $set["texts"]["commission_total"]), "commission_ok" => (empty($set["texts"]["commission_ok"]) ? "可提现佣金" : $set["texts"]["commission_ok"]), "commission_apply" => (empty($set["texts"]["commission_apply"]) ? "已申请佣金" : $set["texts"]["commission_apply"]), "commission_check" => (empty($set["texts"]["commission_check"]) ? "待打款佣金" : $set["texts"]["commission_check"]), "commission_lock" => (empty($set["texts"]["commission_lock"]) ? "未结算佣金" : $set["texts"]["commission_lock"]), "commission_detail" => (empty($set["texts"]["commission_detail"]) ? "提现明细" : ($set["texts"]["commission_detail"] == "佣金明细" ? "提现明细" : $set["texts"]["commission_detail"])), "commission_pay" => (empty($set["texts"]["commission_pay"]) ? "成功提现佣金" : $set["texts"]["commission_pay"]), "commission_wait" => (empty($set["texts"]["commission_wait"]) ? "待收货佣金" : $set["texts"]["commission_wait"]), "commission_fail" => (empty($set["texts"]["commission_fail"]) ? "无效佣金" : $set["texts"]["commission_fail"]), "commission_charge" => (empty($set["texts"]["commission_charge"]) ? "扣除提现手续费" : $set["texts"]["commission_charge"]), "order" => (empty($set["texts"]["order"]) ? "分销订单" : $set["texts"]["order"]), "c1" => (empty($set["texts"]["c1"]) ? "一级" : $set["texts"]["c1"]), "c2" => (empty($set["texts"]["c2"]) ? "二级" : $set["texts"]["c2"]), "c3" => (empty($set["texts"]["c3"]) ? "三级" : $set["texts"]["c3"]), "mydown" => (empty($set["texts"]["mydown"]) ? "我的下线" : $set["texts"]["mydown"]), "down" => (empty($set["texts"]["down"]) ? "下线" : $set["texts"]["down"]), "up" => (empty($set["texts"]["up"]) ? "推荐人" : $set["texts"]["up"]), "yuan" => (empty($set["texts"]["yuan"]) ? "元" : $set["texts"]["yuan"]), "icode" => (empty($set["texts"]["icode"]) ? "邀请码" : $set["texts"]["icode"]), "orderlevel" => (empty($set["texts"]["orderlevel"]) ? "分销等级" : $set["texts"]["orderlevel"]), "moneytext" => $moneytext );
        return $set;
    }
    ...
    public function calculate($orderid = 0, $update = true, $order_agentid = NULL)
    {
        ...
        $order = pdo_fetch("select agentid,price,goodsprice,deductcredit2,discountprice,isdiscountprice,dispatchprice,changeprice,ispackage,packageid,couponprice,buyagainprice,deductprice,deductenough,merchdeductenough,grprice from " . tablename("tapgo_tt_order") . " where id=:id limit 1", array( ":id" => $orderid ));
        ...
        {
            $parent_sql = "select id from " . tablename("tapgo_tt_order") . "where parentid=" . $orderid;
            $condition = " WHERE  og.uniacid=:uniacid";
            $param[":uniacid"] = $_W["uniacid"];
            $condition .= " AND og.orderid in(" . $parent_sql . ")";
            $goods_sql = "select og.id,og.realprice,og.total,g.hasoption,og.goodsid,og.optionid,g.hascommission,g.nocommission, g.commission1_rate,g.commission1_pay,g.commission2_rate,g.commission2_pay,\n          g.commission3_rate,g.commission3_pay,g.commission,og.commissions,og.seckill,og.seckill_taskid,og.seckill_timeid from " . tablename("tapgo_tt_order_goods") . "  og " . " left join " . tablename("tapgo_tt_goods") . " g on g.id = og.goodsid " . $condition;
            $goods = pdo_fetchall($goods_sql, $param);
        }
        else
        {
            $goods = pdo_fetchall("select og.id,og.realprice,og.total,g.hasoption,og.goodsid,og.optionid,g.hascommission,g.nocommission, g.commission1_rate,g.commission1_pay,g.commission2_rate,g.commission2_pay,\n                  g.commission3_rate,g.commission3_pay,g.commission,og.commissions,og.seckill,og.seckill_taskid,og.seckill_timeid from " . tablename("tapgo_tt_order_goods") . "  og " . " left join " . tablename("tapgo_tt_goods") . " g on g.id = og.goodsid" . " where og.orderid=:orderid and og.uniacid=:uniacid", array( ":orderid" => $orderid, ":uniacid" => $_W["uniacid"] ));
        }
        ...
        if( 0 < $set["level"] ) 
        {
            foreach( $goods as &$cinfo ) 
            {
                if( $real_pay_price ) 
                {
                    $order_discount = $order["deductprice"] + $order["deductenough"] + $order["merchdeductenough"] + $order["couponprice"];
                ...
                else
                {
                    ...
                    if( empty($cinfo["nocommission"]) ) 
                    {
                        ...
                        if( 0 < $order["ispackage"] ) 
                        {
                            ...
                            {
                                $packoption = pdo_fetch("select commission1,commission2,commission3 from " . tablename("tapgo_tt_package_goods_option") . "\n                                            where uniacid = " . $_W["uniacid"] . " and pid = " . $order["packageid"] . " and optionid = " . $cinfo["optionid"] . " ");
                            }
                            else
                            {
                                $packoption = pdo_fetch("select commission1,commission2,commission3 from " . tablename("tapgo_tt_package_goods") . "\n                                            where uniacid = " . $_W["uniacid"] . " and pid = " . $order["packageid"] . " and goodsid = " . $cinfo["goodsid"] . " ");
                            }
                ...
                if( $update ) 
                {
                    ...
                    pdo_update("tapgo_tt_order_goods", array( "commission1" => iserializer($cinfo["commission1"]), "commission2" => iserializer($cinfo["commission2"]), "commission3" => iserializer($cinfo["commission3"]), "commissions" => iserializer($commissions), "nocommission" => $cinfo["nocommission"] ), array( "id" => $cinfo["id"] ));
                }
        ...
        if( !$hascommission ) 
        {
            pdo_update("tapgo_tt_order", array( "agentid" => 0 ), array( "id" => $orderid ));
        }
    ...
    public function getOrderCommissions($orderid = 0, $ogid = 0)
    {
        global $_W;
        $set = $this->getSet();
        $agentid = pdo_fetchcolumn("select agentid from " . tablename("tapgo_tt_order") . " where id=:id limit 1", array( ":id" => $orderid ));
        $goods = pdo_fetch("select commission1,commission2,commission3 from " . tablename("tapgo_tt_order_goods") . " where id=:id and orderid=:orderid and uniacid=:uniacid and nocommission=0 limit 1", array( ":id" => $ogid, ":orderid" => $orderid, ":uniacid" => $_W["uniacid"] ));
        $commissions = array( "level1" => 0, "level2" => 0, "level3" => 0 );
        if( 0 < $set["level"] ) 
        {
    ...
    public function getInfo($openid, $options = NULL)
    {
        ...
        if( 1 <= $level ) 
        {
            ...
            {
                $level1_ordercount = pdo_fetch("select sum(og.realprice) as ordermoney,count(distinct o.id) as ordercount from " . tablename("tapgo_tt_order") . " o " . " left join  " . tablename("tapgo_tt_order_goods") . " og on og.orderid=o.id " . " where o.agentid=:agentid and o.status>=0 and og.status1>=0 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0 " . $where_time . " limit 1", array( ":uniacid" => $_W["uniacid"], ":agentid" => $member["id"] ));
            ...
            {
                $level1_ordercount = pdo_fetch("select sum(og.realprice) as ordermoney,count(distinct o.id) as ordercount from " . tablename("tapgo_tt_order") . " o " . " left join  " . tablename("tapgo_tt_order_goods") . " og on og.orderid=o.id " . " where o.agentid=:agentid and o.status>=1 and og.status1>=0 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0 " . $where_time . " limit 1", array( ":uniacid" => $_W["uniacid"], ":agentid" => $member["id"] ));
            ...
            {
                $level1_ordercount3 = pdo_fetch("select sum(og.realprice) as ordermoney,count(distinct o.id) as ordercount from " . tablename("tapgo_tt_order") . " o " . " left join  " . tablename("tapgo_tt_order_goods") . " og on og.orderid=o.id " . " where o.agentid=:agentid and o.status>=3 and og.status1>=0 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0 " . $where_time . " limit 1", array( ":uniacid" => $_W["uniacid"], ":agentid" => $member["id"] ));
            ...
            {
                $level1_commissions = pdo_fetchall("select og.commission1,og.commissions  from " . tablename("tapgo_tt_order_goods") . " og " . " left join  " . tablename("tapgo_tt_order") . " o on o.id = og.orderid" . " where o.agentid=:agentid and o.status>=1 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0", array( ":uniacid" => $_W["uniacid"], ":agentid" => $member["id"] ));
            ...
            {
                $level1_commissions = pdo_fetchall("select og.commission1,og.commissions  from " . tablename("tapgo_tt_order_goods") . " og " . " left join  " . tablename("tapgo_tt_order") . " o on o.id = og.orderid" . " where o.agentid=:agentid and o.status>=3 and og.nocommission=0 and (" . $time . " - o.finishtime > " . $day_times . ") and og.status1=0  and o.uniacid=:uniacid and o.isparent=0", array( ":uniacid" => $_W["uniacid"], ":agentid" => $member["id"] ));
                foreach( $level1_commissions as $c ) 
                {
                    $commissions = iunserializer($c["commissions"]);
                    $commission = iunserializer($c["commission1"]);
            ...
            {
                $level1_commissions1 = pdo_fetchall("select og.commission1,og.commissions  from " . tablename("tapgo_tt_order_goods") . " og " . " left join  " . tablename("tapgo_tt_order") . " o on o.id = og.orderid" . " where o.agentid=:agentid and o.status>=3 and og.nocommission=0 and (" . $time . " - o.finishtime <= " . $day_times . ")  and og.status1=0  and o.uniacid=:uniacid and o.isparent=0", array( ":uniacid" => $_W["uniacid"], ":agentid" => $member["id"] ));
            ...
            {
                $level1_commissions2 = pdo_fetchall("select og.commission1,og.commissions  from " . tablename("tapgo_tt_order_goods") . " og " . " left join  " . tablename("tapgo_tt_order") . " o on o.id = og.orderid" . " where o.agentid=:agentid and o.status>=3 and og.status1=1 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0", array( ":uniacid" => $_W["uniacid"], ":agentid" => $member["id"] ));
            ...
            {
                $level1_commissions2 = pdo_fetchall("select og.commission1,og.commissions  from " . tablename("tapgo_tt_order_goods") . " og " . " left join  " . tablename("tapgo_tt_order") . " o on o.id = og.orderid" . " where o.agentid=:agentid and o.status>=3 and og.status1=2 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0", array( ":uniacid" => $_W["uniacid"], ":agentid" => $member["id"] ));
                foreach( $level1_commissions2 as $c ) 
                {
                    $commissions = iunserializer($c["commissions"]);
                    $commission = iunserializer($c["commission1"]);
            ...
            {
                $level1_commissions2 = pdo_fetchall("select og.commission1,og.commissions  from " . tablename("tapgo_tt_order_goods") . " og " . " left join  " . tablename("tapgo_tt_order") . " o on o.id = og.orderid" . " where o.agentid=:agentid and o.status>=3 and og.status1=3 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0", array( ":uniacid" => $_W["uniacid"], ":agentid" => $member["id"] ));
                foreach( $level1_commissions2 as $c ) 
                {
                    $commissions = iunserializer($c["commissions"]);
                    $commission = iunserializer($c["commission1"]);
            ...
            {
                $level1_commissions2 = pdo_fetchall("select og.commission1,og.commissions  from " . tablename("tapgo_tt_order_goods") . " og " . " left join  " . tablename("tapgo_tt_order") . " o on o.id = og.orderid" . " where o.agentid=:agentid and (o.status=2 or o.status=1) and og.status1=0  and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0", array( ":uniacid" => $_W["uniacid"], ":agentid" => $member["id"] ));
                foreach( $level1_commissions2 as $c ) 
                {
                    $commissions = iunserializer($c["commissions"]);
                    $commission = iunserializer($c["commission1"]);
            ...
            $level1_agentids = pdo_fetchall("select id from " . tablename("tapgo_tt_member") . " where agentid=:agentid and isagent=1 and status=1 and uniacid=:uniacid ", array( ":uniacid" => $_W["uniacid"], ":agentid" => $member["id"] ), "id");
        ...
        if( 2 <= $level && 0 < $level1 ) 
        {
            if( in_array("ordercount0", $options) ) 
            {
                $level2_ordercount = pdo_fetch("select sum(og.realprice) as ordermoney,count(distinct o.id) as ordercount from " . tablename("tapgo_tt_order") . " o " . " left join  " . tablename("tapgo_tt_order_goods") . " og on og.orderid=o.id " . " where o.agentid in( " . implode(",", array_keys($level1_agentids)) . ")  and o.status>=0 and og.status2>=0 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0 " . $where_time . " limit 1", array( ":uniacid" => $_W["uniacid"] ));
            ...
            if( in_array("ordercount3", $options) ) 
            {
                $level2_ordercount3 = pdo_fetch("select sum(og.realprice) as ordermoney,count(distinct o.id) as ordercount from " . tablename("tapgo_tt_order") . " o " . " left join  " . tablename("tapgo_tt_order_goods") . " og on og.orderid=o.id " . " where o.agentid in( " . implode(",", array_keys($level1_agentids)) . ")  and o.status>=3 and og.status2>=0 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0 " . $where_time . " limit 1", array( ":uniacid" => $_W["uniacid"] ));
            ...
            if( in_array("total", $options) ) 
            {
                $level2_commissions = pdo_fetchall("select og.commission2,og.commissions from " . tablename("tapgo_tt_order_goods") . " og " . " left join  " . tablename("tapgo_tt_order") . " o on o.id = og.orderid " . " where o.agentid in( " . implode(",", array_keys($level1_agentids)) . ")  and o.status>=1 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0", array( ":uniacid" => $_W["uniacid"] ));
            ...
            if( in_array("lock", $options) ) 
            {
                $level2_commissions1 = pdo_fetchall("select og.commission2,og.commissions  from " . tablename("tapgo_tt_order_goods") . " og " . " left join  " . tablename("tapgo_tt_order") . " o on o.id = og.orderid " . " where o.agentid in( " . implode(",", array_keys($level1_agentids)) . ")  and (" . $time . " - o.finishtime <= " . $day_times . ") and og.status2=0 and o.status>=3 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0", array( ":uniacid" => $_W["uniacid"] ));
            ...
            if( in_array("apply", $options) ) 
            {
                $level2_commissions2 = pdo_fetchall("select og.commission2,og.commissions  from " . tablename("tapgo_tt_order_goods") . " og " . " left join  " . tablename("tapgo_tt_order") . " o on o.id = og.orderid " . " where o.agentid in( " . implode(",", array_keys($level1_agentids)) . ")  and o.status>=3 and og.status2=1 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0", array( ":uniacid" => $_W["uniacid"] ));
            ...
            if( in_array("check", $options) ) 
            {
                $level2_commissions3 = pdo_fetchall("select og.commission2,og.commissions  from " . tablename("tapgo_tt_order_goods") . " og " . " left join  " . tablename("tapgo_tt_order") . " o on o.id = og.orderid " . " where o.agentid in( " . implode(",", array_keys($level1_agentids)) . ")  and o.status>=3 and og.status2=2 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0", array( ":uniacid" => $_W["uniacid"] ));
            ...
            $level2_agentids = pdo_fetchall("select id from " . tablename("tapgo_tt_member") . " where agentid in( " . implode(",", array_keys($level1_agentids)) . ") and isagent=1 and status=1 and uniacid=:uniacid", array( ":uniacid" => $_W["uniacid"] ), "id");
        ...
        if( 3 <= $level && 0 < $level2 ) 
        {
            if( in_array("ordercount0", $options) ) 
            {
                $level3_ordercount = pdo_fetch("select sum(og.realprice) as ordermoney,count(distinct og.orderid) as ordercount from " . tablename("tapgo_tt_order") . " o " . " left join  " . tablename("tapgo_tt_order_goods") . " og on og.orderid=o.id " . " where o.agentid in( " . implode(",", array_keys($level2_agentids)) . ")  and o.status>=0 and og.status3>=0 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0 " . $where_time . " limit 1", array( ":uniacid" => $_W["uniacid"] ));
            ...
            if( in_array("ordercount3", $options) ) 
            {
                $level3_ordercount3 = pdo_fetch("select sum(og.realprice) as ordermoney,count(distinct og.orderid) as ordercount from " . tablename("tapgo_tt_order") . " o " . " left join  " . tablename("tapgo_tt_order_goods") . " og on og.orderid=o.id " . " where o.agentid in( " . implode(",", array_keys($level2_agentids)) . ")  and o.status>=3 and og.status3>=0 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0 " . $where_time . " limit 1", array( ":uniacid" => $_W["uniacid"] ));
            ...
            if( in_array("total", $options) ) 
            {
                $level3_commissions = pdo_fetchall("select og.commission3,og.commissions  from " . tablename("tapgo_tt_order_goods") . " og " . " left join  " . tablename("tapgo_tt_order") . " o on o.id = og.orderid" . " where o.agentid in( " . implode(",", array_keys($level2_agentids)) . ")  and o.status>=1 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0", array( ":uniacid" => $_W["uniacid"] ));
            ...
            $level3_agentids = pdo_fetchall("select id from " . tablename("tapgo_tt_member") . " where uniacid=:uniacid and agentid in( " . implode(",", array_keys($level2_agentids)) . ") and isagent=1 and status=1", array( ":uniacid" => $_W["uniacid"] ), "id");
    ...
    public function getAgents($orderid = 0)
    {
        ...
        global $_GPC;
        $agents = array(  );
        $order = pdo_fetch("select id,agentid,openid from " . tablename("tapgo_tt_order") . " where id=:id and uniacid=:uniacid limit 1", array( ":id" => $orderid, ":uniacid" => $_W["uniacid"] ));
        if( empty($order) ) 
        {
            return $agents;
        }
    ...
    public function getAgentsDownNum($openid = NULL)
    {
        ...
        $level1 = (int) pdo_fetchcolumn("select count(*) from " . tablename("tapgo_tt_member") . " where agentid=:agentid and uniacid=:uniacid limit 1", array( ":agentid" => $member["id"], ":uniacid" => $_W["uniacid"] ));
        if( 2 <= $set["level"] && 0 < $levelcount1 ) 
        {
            $level2 = (int) pdo_fetchcolumn("select count(*) from " . tablename("tapgo_tt_member") . " where agentid in( " . implode(",", array_keys($member["level1_agentids"])) . ") and uniacid=:uniacid limit 1", array( ":uniacid" => $_W["uniacid"] ));
        }

        if( 3 <= $set["level"] && 0 < $levelcount2 ) 
        {
            $level3 = (int) pdo_fetchcolumn("select count(*) from " . tablename("tapgo_tt_member") . " where agentid in( " . implode(",", array_keys($member["level2_agentids"])) . ") and uniacid=:uniacid limit 1", array( ":uniacid" => $_W["uniacid"] ));
        }
    ...
    {
        global $_W;
        $path = IA_ROOT . "/addons/tapgo_ttv2/data/qrcode/" . $_W["uniacid"];
        if( !is_dir($path) ) 
        {
            load()->func("file");
            mkdirs($path);
        ...
        return $_W["siteroot"] . "addons/tapgo_ttv2/data/qrcode/" . $_W["uniacid"] . "/" . $file;
    }
    ...
    public function createGoodsImage($goods)
    {
        ...
        $path = IA_ROOT . "/addons/tapgo_ttv2/data/poster/" . $_W["uniacid"] . "/";
        if( !is_dir($path) ) 
        {
            load()->func("file");
            mkdirs($path);
        ...
        {
            set_time_limit(0);
            $font = IA_ROOT . "/addons/tapgo_ttv2/static/fonts/msyh.ttf";
            $target = imagecreatetruecolor(640, 1225);
            $bg = imagecreatefromjpeg(IA_ROOT . "/addons/tapgo_ttv2/plugin/commission/static/images/poster.jpg");
        ...
        return $_W["siteroot"] . "addons/tapgo_ttv2/data/poster/" . $_W["uniacid"] . "/" . $file;
    }
    ...
    public function checkAgent($openid = "")
    {
        ...
        {
            $clickcount = pdo_fetchcolumn("select count(*) from " . tablename("tapgo_tt_commission_clickcount") . " where uniacid=:uniacid and openid=:openid and from_openid=:from_openid limit 1", array( ":uniacid" => $_W["uniacid"], ":openid" => $openid, ":from_openid" => $parent["openid"] ));
            if( $clickcount <= 0 ) 
            {
                $click = array( "uniacid" => $_W["uniacid"], "openid" => $openid, "from_openid" => $parent["openid"], "clicktime" => time() );
                pdo_insert("tapgo_tt_commission_clickcount", $click);
                pdo_update("tapgo_tt_member", array( "clickcount" => $parent["clickcount"] + 1 ), array( "uniacid" => $_W["uniacid"], "id" => $parent["id"] ));
            }
        ...
        if( $open_redis ) 
        {
            ...
            if( !$first ) 
            {
                $first = pdo_fetchcolumn("select count(*) from " . tablename("tapgo_tt_member") . " where uniacid=:uniacid limit 1", array( ":uniacid" => $_W["uniacid"] ));
        ...
        {
            $first = 0;
            $member1 = pdo_fetch("select id,openid from " . tablename("tapgo_tt_member") . " where uniacid=:uniacid limit 1", array( ":uniacid" => $_W["uniacid"] ));
            if( $member1 ) 
            {
                $first = 1;
                $member2 = pdo_fetch("select id,openid from " . tablename("tapgo_tt_member") . " where uniacid=:uniacid and id<>:fid limit 1", array( ":uniacid" => $_W["uniacid"], ":fid" => $member1["id"] ));
                if( $member2 ) 
                {
                    $first = 2;
                }
        ...
        if( $first <= 1 ) 
        {
            pdo_update("tapgo_tt_member", array( "isagent" => 1, "status" => 1, "agenttime" => time(), "agentblack" => 0 ), array( "uniacid" => $_W["uniacid"], "id" => $member["id"] ));
        }
        else
        {
            $time = time();
            ...
            if( $parent_is_agent && empty($member["agentid"]) && $member["id"] != $parent["id"] ) 
            {
                if( empty($become_child) ) 
                {
                    if( empty($member["fixagentid"]) ) 
                    {
                        ...
                        if( $author ) 
                        {
                            ...
                            pdo_update("tapgo_tt_member", array( "agentid" => $parent["id"], "childtime" => $time, "authorid" => $authorid ), array( "uniacid" => $_W["uniacid"], "id" => $member["id"] ));
                        }
                        else
                        {
                            if( intval($set["become_child"]) == 0 ) 
                            {
                                pdo_update("tapgo_tt_member", array( "agentid" => $parent["id"], "childtime" => $time, "fixagentid" => 1 ), array( "uniacid" => $_W["uniacid"], "id" => $member["id"] ));
                            }
                        ...
                        if( $author ) 
                        {
                            ...
                            if( !empty($author_set["become"]) && ($author_set["become"] == "2" || $author_set["become"] == "5") ) 
                            {
                                ...
                                if( $can_author ) 
                                {
                                    ...
                                    {
                                        pdo_update("tapgo_tt_member", array( "authorstatus" => $become_check, "isauthor" => 1, "authortime" => $time ), array( "uniacid" => $_W["uniacid"], "id" => $parent["id"] ));
                                        if( $become_check == 1 ) 
                                        {
                ...
                {
                    pdo_update("tapgo_tt_member", array( "inviter" => $parent["id"] ), array( "uniacid" => $_W["uniacid"], "id" => $member["id"] ));
                }
            ...
            else
            {
                if( $set["become"] == 2 ) 
                {
                    $ordercount = pdo_fetchcolumn("select count(*) from " . tablename("tapgo_tt_order") . " where uniacid=:uniacid and openid=:openid and status>=" . $order_status . " limit 1", array( ":uniacid" => $_W["uniacid"], ":openid" => $openid ));
                ...
                else
                {
                    if( $set["become"] == 3 ) 
                    {
                        $moneycount = pdo_fetchcolumn("select sum(price) from " . tablename("tapgo_tt_order") . " where uniacid=:uniacid and openid=:openid and status>=" . $order_status . " limit 1", array( ":uniacid" => $_W["uniacid"], ":openid" => $openid ));
                    ...
                    else
                    {
                        if( $set["become"] == 4 ) 
                        {
                            ...
                            $goods = pdo_fetch("select id,title,thumb,marketprice from" . tablename("tapgo_tt_goods") . " where id=:id and uniacid=:uniacid limit 1", array( ":id" => $set["become_goodsid"], ":uniacid" => $_W["uniacid"] ));
                            $goodscount = pdo_fetchcolumn("select count(*) from " . tablename("tapgo_tt_order_goods") . " og " . "  left join " . tablename("tapgo_tt_order") . " o on o.id = og.orderid" . " where og.goodsid IN (" . implode(",", $set["become_goodsid"]) . ") and o.openid=:openid and o.status>=" . $order_status . "  limit 1", array( ":openid" => $openid ));
                            if( 0 < $goodscount ) 
                            {
                                $to_check_agent = true;
                            }
            ...
            if( $to_check_agent ) 
            {
                pdo_update("tapgo_tt_member", array( "isagent" => 1, "status" => $become_check, "agenttime" => ($become_check == 1 ? $time : 0), "applyagenttime" => 0 ), array( "uniacid" => $_W["uniacid"], "id" => $member["id"] ));
                if( $become_check == 1 ) 
                {
    ...
    public function checkOrderConfirm($orderid = "0")
    {
        ...
        $order = pdo_fetch("select id,openid,ordersn,goodsprice,agentid,paytime,officcode from " . tablename("tapgo_tt_order") . " where id=:id and status>=0 and uniacid=:uniacid limit 1", array( ":id" => $orderid, ":uniacid" => $_W["uniacid"] ));
        if( empty($order) ) 
        {
            return NULL;
        }
        ...
        else
        {
            ...
            {
                $parent = pdo_fetch("select * from " . tablename("tapgo_tt_member") . " where mobile = :mobile and uniacid = :uniacid limit 1 ", array( ":mobile" => trim($order["officcode"]), ":uniacid" => intval($_W["uniacid"]) ));
            }
            else
            {
        ...
        if( $parent_is_agent && $become_child == 1 && empty($member["agentid"]) && $member["id"] != $parent["id"] && empty($member["fixagentid"]) ) 
        {
            ...
            {
                pdo_update("tapgo_tt_member", array( "agentid" => $parent["id"], "childtime" => $time ), array( "uniacid" => $_W["uniacid"], "id" => $member["id"] ));
            }
        ...
        {
            $agentid = $parent["id"];
            pdo_update("tapgo_tt_member", array( "agentid" => $parent["id"] ), array( "id" => $member["id"] ));
        }

        if( !empty($agentid) ) 
        {
            $res = pdo_update("tapgo_tt_order", array( "agentid" => $agentid ), array( "id" => $orderid ));
    ...
    public function checkOrderPay($orderid = "0")
    {
        ...
        $order = pdo_fetch("select id,isparent,parentid,openid,ordersn,goodsprice,agentid,paytime,uniacid from " . tablename("tapgo_tt_order") . " where id=:id and status>=1 and uniacid=:uniacid limit 1", array( ":id" => $orderid, ":uniacid" => $_W["uniacid"] ));
        if( empty($order) ) 
        {
            return NULL;
        }
        ...
        if( $parent_is_agent && $become_child == 2 && empty($member["agentid"]) && $member["id"] != $parent["id"] && empty($member["fixagentid"]) ) 
        {
            ...
            if( empty($order["agentid"]) ) 
            {
                ...
                if( $order["isparent"] && $order["parentid"] == 0 ) 
                {
                    $merchSql = "SELECT id,merchid FROM " . tablename("tapgo_tt_order") . " WHERE uniacid = " . intval($order["uniacid"]) . " AND parentid = " . intval($orderid);
                    $merchData = pdo_fetchall($merchSql);
                    foreach( $merchData as $mk => $mv ) 
                    {
                        pdo_update("tapgo_tt_order", array( "agentid" => $parent["id"] ), array( "id" => $mv["id"] ));
                    }
                }

                pdo_update("tapgo_tt_order", array( "agentid" => $parent["id"] ), array( "id" => $orderid ));
        ...
        if( !$isagent ) 
        {
            if( intval($set["become"]) == 4 && !empty($set["become_goodsid"]) ) 
            {
                ...
                {
                    $set["become_goodsid"] = iunserializer($set["become_goodsid"]);
                    $order_goods = pdo_fetchcolumn("select count(*)from " . tablename("tapgo_tt_order_goods") . " where orderid=:orderid and uniacid=:uniacid and goodsid IN (" . implode(",", $set["become_goodsid"]) . ") ", array( ":uniacid" => $_W["uniacid"], ":orderid" => $order["id"] ));
                    if( 0 < $order_goods && empty($member["agentblack"]) ) 
                    {
                        pdo_update("tapgo_tt_member", array( "status" => $become_check, "isagent" => 1, "agenttime" => ($become_check == 1 ? $time : 0), "applyagenttime" => 0 ), array( "uniacid" => $_W["uniacid"], "id" => $member["id"] ));
                        if( $become_check == 1 ) 
                        {
                            $this->sendMessage($openid, array( "nickname" => $member["nickname"], "agenttime" => $time ), TM_COMMISSION_BECOME);
                            if( !empty($parent) ) 
            ...
            else
            {
                if( ($set["become"] == 2 || $set["become"] == 3) && empty($set["become_order"]) ) 
                {
                    ...
                    if( $set["become"] == "2" ) 
                    {
                        $ordercount = pdo_fetchcolumn("select count(*) from " . tablename("tapgo_tt_order") . " where openid=:openid and status>=1 and uniacid=:uniacid limit 1", array( ":uniacid" => $_W["uniacid"], ":openid" => $openid ));
                    ...
                    else
                    {
                        if( $set["become"] == "3" ) 
                        {
                            $moneycount = pdo_fetchcolumn("select sum(og.realprice) from " . tablename("tapgo_tt_order_goods") . " og left join " . tablename("tapgo_tt_order") . " o on og.orderid=o.id  where o.openid=:openid and o.status>=1 and o.uniacid=:uniacid limit 1", array( ":uniacid" => $_W["uniacid"], ":openid" => $openid ));
                    ...
                    {
                        pdo_update("tapgo_tt_member", array( "status" => $become_check, "isagent" => 1, "agenttime" => $time ), array( "uniacid" => $_W["uniacid"], "id" => $member["id"] ));
                        if( $become_check == 1 ) 
                        {
                            $this->sendMessage($openid, array( "nickname" => $member["nickname"], "agenttime" => $time ), TM_COMMISSION_BECOME);
                            if( $parentisagent ) 
        ...
        if( !empty($member["agentid"]) ) 
        {
            ...
            if( !empty($parent) && $parent["isagent"] == 1 && $parent["status"] == 1 ) 
            {
                $order_goods = pdo_fetchall("select g.id,g.title,og.total,og.price,og.realprice, og.optionname as optiontitle,g.noticeopenid,g.noticetype,og.commission1,og.commissions  from " . tablename("tapgo_tt_order_goods") . " og " . " left join " . tablename("tapgo_tt_goods") . " g on g.id=og.goodsid " . " where og.uniacid=:uniacid and og.orderid=:orderid ", array( ":uniacid" => $_W["uniacid"], ":orderid" => $order["id"] ));
                $goods = "";
                $commission_total1 = 0;
                $commission_total2 = 0;
                $commission_total3 = 0;
        ...
        if( $isagent ) 
        {
            ...
            if( intval($set["become"]) == 4 && !empty($set["become_goodsid"]) ) 
            {
                if( empty($set["become_order"]) ) 
                {
                    $order_goods = pdo_fetchall("select goodsid from " . tablename("tapgo_tt_order_goods") . " where orderid=:orderid and uniacid=:uniacid  ", array( ":uniacid" => $_W["uniacid"], ":orderid" => $order["id"] ), "goodsid");
                    if( in_array($set["become_goodsid"], array_keys($order_goods)) && empty($member["partnerblack"]) ) 
                    {
                        pdo_update("tapgo_tt_member", array( "partnerstatus" => $become_check, "ispartner" => 1, "partnertime" => ($become_check == 1 ? $time : 0) ), array( "uniacid" => $_W["uniacid"], "id" => $member["id"] ));
                        if( $become_check == 1 ) 
                        {
            ...
            else
            {
                if( ($set["become"] == 2 || $set["become"] == 3) && empty($set["become_order"]) ) 
                {
                    ...
                    {
                        pdo_update("tapgo_tt_member", array( "partnerstatus" => $become_check, "ispartner" => 1, "partnertime" => $time ), array( "uniacid" => $_W["uniacid"], "id" => $member["id"] ));
                        if( $become_check == 1 ) 
                        {
                            $plugin_globonus->sendMessage($openid, array( "nickname" => $member["nickname"], "partnertime" => $time ), TM_GLOBONUS_BECOME);
                        }
    ...
    public function checkOrderFinish($orderid = "")
    {
        ...
        $order = pdo_fetch("select id,openid, ordersn,goodsprice,agentid,finishtime from " . tablename("tapgo_tt_order") . " where id=:id and status>=3 and uniacid=:uniacid limit 1", array( ":id" => $orderid, ":uniacid" => $_W["uniacid"] ));
        if( empty($order) ) 
        {
            return NULL;
        }
        ...
        if( !$isagent && $set["become_order"] == "1" ) 
        {
            if( $set["become"] == "4" && !empty($set["become_goodsid"]) ) 
            {
                ...
                if( 0 < $order_goods && empty($member["agentblack"]) ) 
                {
                    pdo_update("tapgo_tt_member", array( "status" => $become_check, "isagent" => 1, "agenttime" => ($become_check == 1 ? $time : 0) ), array( "uniacid" => $_W["uniacid"], "id" => $member["id"] ));
                    if( $become_check == 1 ) 
                    {
                        $this->sendMessage($openid, array( "nickname" => $member["nickname"], "agenttime" => $time ), TM_COMMISSION_BECOME);
                        if( $parentisagent ) 
            ...
            else
            {
                if( $set["become"] == 2 || $set["become"] == 3 ) 
                {
                    ...
                    if( $set["become"] == "2" ) 
                    {
                        $ordercount = pdo_fetchcolumn("select count(*) from " . tablename("tapgo_tt_order") . " where openid=:openid and status>=3 and uniacid=:uniacid limit 1", array( ":uniacid" => $_W["uniacid"], ":openid" => $openid ));
                        $can = intval($set["become_ordercount"]) <= $ordercount;
                    }
                    else
                    {
                        if( $set["become"] == "3" ) 
                        {
                            $moneycount = pdo_fetchcolumn("select sum(goodsprice) from " . tablename("tapgo_tt_order") . " where openid=:openid and status>=3 and uniacid=:uniacid limit 1", array( ":uniacid" => $_W["uniacid"], ":openid" => $openid ));
                            $can = floatval($set["become_moneycount"]) <= $moneycount;
                        }
        ...
        if( !empty($member["agentid"]) ) 
        {
            ...
            if( !empty($parent) && $parent["isagent"] == 1 && $parent["status"] == 1 ) 
            {
                $order_goods = pdo_fetchall("select g.id,g.title,og.total,og.realprice,og.price,og.optionname as optiontitle,g.noticeopenid,g.noticetype,og.commission1,og.commissions from " . tablename("tapgo_tt_order_goods") . " og " . " left join " . tablename("tapgo_tt_goods") . " g on g.id=og.goodsid " . " where og.uniacid=:uniacid and og.orderid=:orderid ", array( ":uniacid" => $_W["uniacid"], ":orderid" => $order["id"] ));
                $goods = "";
                $commission_total1 = 0;
                $commission_total2 = 0;
                $commission_total3 = 0;
                ...
                if( !empty($set["open"]) ) 
                {
                    ...
                    else
                    {
                        ...
                        if( $set["become_order"] == "1" ) 
                        {
                            ...
                            {
                                pdo_update("tapgo_tt_member", array( "authorstatus" => $become_check, "isauthor" => 1, "authortime" => $time ), array( "uniacid" => $_W["uniacid"], "id" => $member["id"] ));
                                if( $become_check == 1 ) 
                                {
    ...
    public function orderFinishTask($order, $self_buy = false, $member)
    {
        ...
        $order_goods_list = pdo_fetchall("SELECT commissions FROM " . tablename("tapgo_tt_order_goods") . " WHERE orderid = :order_id AND uniacid = :uniacid AND nocommission = 0", array( ":order_id" => $order_id, ":uniacid" => $_W["uniacid"] ));
    ...
    public function getShop($m)
    {
        ...
        $shop = pdo_fetch("select * from " . tablename("tapgo_tt_commission_shop") . " where uniacid=:uniacid and mid=:mid limit 1", array( ":uniacid" => $_W["uniacid"], ":mid" => $member["id"] ));
    ...
    public function getLevels($all = true, $default = false)
    {
        ...
        if( $all ) 
        {
            $levels = pdo_fetchall("select * from " . tablename("tapgo_tt_commission_level") . " where uniacid=:uniacid order by commission1 asc", array( ":uniacid" => $_W["uniacid"] ));
        }
        else
        {
            $levels = pdo_fetchall("select * from " . tablename("tapgo_tt_commission_level") . " where uniacid=:uniacid and (ordermoney>0 or commissionmoney>0) order by commission1 asc", array( ":uniacid" => $_W["uniacid"] ));
        }
    ...
    public function getLevel($openid)
    {
        ...
        $level = pdo_fetch("select * from " . tablename("tapgo_tt_commission_level") . " where uniacid=:uniacid and id=:id limit 1", array( ":uniacid" => $_W["uniacid"], ":id" => $member["agentlevel"] ));
        return $level;
    }
    ...
    public function upgradeLevelByOrder($openid)
    {
        ...
        if( $leveltype == 4 || $leveltype == 5 ) 
        {
            ...
            $orders = pdo_fetch("select sum(og.realprice) as ordermoney,count(distinct og.orderid) as ordercount from " . tablename("tapgo_tt_order") . " o " . " left join  " . tablename("tapgo_tt_order_goods") . " og on og.orderid=o.id " . " where o.openid=:openid and o.status>=3 and o.uniacid=:uniacid limit 1", array( ":uniacid" => $_W["uniacid"], ":openid" => $openid ));
            $ordermoney = $orders["ordermoney"];
            $ordercount = $orders["ordercount"];
            if( $leveltype == 4 ) 
            {
                $newlevel = pdo_fetch("select * from " . tablename("tapgo_tt_commission_level") . " where uniacid=:uniacid  and " . $ordermoney . " >= ordermoney and ordermoney>0  order by ordermoney desc limit 1", array( ":uniacid" => $_W["uniacid"] ));
                if( empty($newlevel) ) 
                {
                    return NULL;
                }
            ...
            else
            {
                if( $leveltype == 5 ) 
                {
                    $newlevel = pdo_fetch("select * from " . tablename("tapgo_tt_commission_level") . " where uniacid=:uniacid  and " . $ordercount . " >= ordercount and ordercount>0  order by ordercount desc limit 1", array( ":uniacid" => $_W["uniacid"] ));
                    if( empty($newlevel) ) 
                    {
                        return NULL;
                    }
            ...
            pdo_update("tapgo_tt_member", array( "agentlevel" => $newlevel["id"] ), array( "id" => $m["id"] ));
        ...
        else
        {
            if( 0 <= $leveltype && $leveltype <= 3 ) 
            {
                ...
                foreach( $agents as $agent ) 
                {
                    ...
                    pdo_update("tapgo_tt_member", array( "agentlevel" => $newlevel["id"] ), array( "id" => $agent["id"] ));
    ...
    public function upgradeLevelByAgent($openid)
    {
        ...
        if( $leveltype == 6 || $leveltype == 8 ) 
        {
            ...
            foreach( $agents as $agent ) 
            {
                ...
                if( $leveltype == 6 ) 
                {
                    $downs1 = pdo_fetchall("select id from " . tablename("tapgo_tt_member") . " where agentid=:agentid and uniacid=:uniacid ", array( ":agentid" => $m["id"], ":uniacid" => $_W["uniacid"] ), "id");
                    $downcount += count($downs1);
                    if( !empty($downs1) ) 
                    {
                        $downs2 = pdo_fetchall("select id from " . tablename("tapgo_tt_member") . " where agentid in( " . implode(",", array_keys($downs1)) . ") and uniacid=:uniacid", array( ":uniacid" => $_W["uniacid"] ), "id");
                        $downcount += count($downs2);
                        if( !empty($downs2) ) 
                        {
                            $downs3 = pdo_fetchall("select id from " . tablename("tapgo_tt_member") . " where agentid in( " . implode(",", array_keys($downs2)) . ") and uniacid=:uniacid", array( ":uniacid" => $_W["uniacid"] ), "id");
                            $downcount += count($downs3);
                        }

                    }

                    $newlevel = pdo_fetch("select * from " . tablename("tapgo_tt_commission_level") . " where uniacid=:uniacid  and " . $downcount . " >= downcount and downcount>0  order by downcount desc limit 1", array( ":uniacid" => $_W["uniacid"] ));
                }
                else
                {
                    if( $leveltype == 8 ) 
        ...
        else
        {
            ...
            if( $leveltype == 7 ) 
            {
                $downcount = pdo_fetchcolumn("select count(*) from " . tablename("tapgo_tt_member") . " where agentid=:agentid and uniacid=:uniacid ", array( ":agentid" => $m["id"], ":uniacid" => $_W["uniacid"] ));
                $newlevel = pdo_fetch("select * from " . tablename("tapgo_tt_commission_level") . " where uniacid=:uniacid  and " . $downcount . " >= downcount and downcount>0  order by downcount desc limit 1", array( ":uniacid" => $_W["uniacid"] ));
            }
            else
            {
    ...
    public function upgradeLevelByCommissionOK($openid)
    {
        ...
        $newlevel = pdo_fetch("select * from " . tablename("tapgo_tt_commission_level") . " where uniacid=:uniacid  and " . $commissionmoney . " >= commissionmoney and commissionmoney>0  order by commissionmoney desc limit 1", array( ":uniacid" => $_W["uniacid"] ));
        if( empty($newlevel) ) 
        {
            return NULL;
        }
    ...
    public function upgradeLevelByGoods($openid, $orderid)
    {
        ...
        $orders = pdo_fetchall("select og.goodsid from " . tablename("tapgo_tt_order") . " o " . " left join  " . tablename("tapgo_tt_order_goods") . " og on og.orderid=o.id " . " where o.openid=:openid and o.status>=3 and o.uniacid=:uniacid and og.orderid=:orderid", array( ":uniacid" => $_W["uniacid"], ":openid" => $openid, "orderid" => $orderid ));
        if( $leveltype == 11 ) 
        {
            $newlevel = pdo_fetchall("select * from " . tablename("tapgo_tt_commission_level") . " where uniacid=:uniacid  ", array( ":uniacid" => $_W["uniacid"] ), "level");
    ...
    {
        global $_W;
        return array( "total1" => pdo_fetchcolumn("select count(*) from " . tablename("tapgo_tt_commission_apply") . " where status=1 and uniacid=:uniacid limit 1", array( ":uniacid" => $_W["uniacid"] )), "total2" => pdo_fetchcolumn("select count(*) from " . tablename("tapgo_tt_commission_apply") . " where status=2 and uniacid=:uniacid limit 1", array( ":uniacid" => $_W["uniacid"] )), "total3" => pdo_fetchcolumn("select count(*) from " . tablename("tapgo_tt_commission_apply") . " where status=3 and uniacid=:uniacid limit 1", array( ":uniacid" => $_W["uniacid"] )), "total_1" => pdo_fetchcolumn("select count(*) from " . tablename("tapgo_tt_commission_apply") . " where status=-1 and uniacid=:uniacid limit 1", array( ":uniacid" => $_W["uniacid"] )) );
    }
    ...
    protected function sendNotice($touser, $tm, $tag, $datas, $member, $msg)
    {
        ...
        {
            $advanced_template = pdo_fetch("select * from " . tablename("tapgo_tt_member_message_template") . " where id=:id and uniacid=:uniacid limit 1", array( ":id" => $tm[$tag], ":uniacid" => $_W["uniacid"] ));
    ...
    public function getLastApply($mid, $type = -1)
    {
        ...
        $sql = "select type,alipay,bankname,bankcard,realname from " . tablename("tapgo_tt_commission_apply") . " where mid=:mid and uniacid=:uniacid";
        if( -1 < $type ) 
        {
            $sql .= " and type=:type";
            $params[":type"] = $type;
    ...
    public function getRepurchase($openid, array $time)
    {
        ...
        foreach( $time as $value ) 
        {
            ...
            {
                $order_price = (double) pdo_fetchcolumn("SELECT SUM(price) as price FROM " . tablename("tapgo_tt_order") . " WHERE `uniacid`=:uniacid AND `openid`=:openid AND `status`>2 AND `createtime` BETWEEN :time1 AND :time2", array( ":uniacid" => $_W["uniacid"], ":openid" => $openid, ":time1" => $time1, ":time2" => $time2 ));
                $year_month = explode("-", $value);
                $year_month[0] = (int) $year_month[0];
                $year_month[1] = (int) $year_month[1];
                $residue_price = (double) pdo_fetchcolumn("SELECT SUM(repurchase) FROM " . tablename("tapgo_tt_commission_repurchase") . " WHERE `uniacid`=:uniacid AND `openid`=:openid AND `year`=:year AND `month`=:month", array( ":uniacid" => $_W["uniacid"], ":openid" => $openid, ":year" => $year_month[0], ":month" => $year_month[1] ));
    ...
    public function getAllDown($openid)
    {
        ...
        if( $uid == 0 ) 
        {
            $info = pdo_fetch("select id,openid,uniacid,agentid,isagent,status from " . tablename("tapgo_tt_member") . " where  openid=:openid and uniacid=:uniacid limit 1", array( ":uniacid" => $_W["uniacid"], ":openid" => $openid ));
            if( empty($info) ) 
            {
                return false;
            }
        ...
        $agents = pdo_fetchall("select id,openid,uniacid,agentid,isagent,nickname,agenttime,createtime,avatar,status,realname,mobile,weixin from " . tablename("tapgo_tt_member") . " where uniacid=:uniacid and agentid=:agentid", array( ":uniacid" => $_W["uniacid"], ":agentid" => $uid ));
        $ids = array(  );
        $openids = array(  );
        $users = array(  );
    ...
    public function getAllDownOrder($openid, $start = 0, $end = 0)
    {
        ...
        {
            $order = pdo_fetchall("SELECT * FROM " . tablename("tapgo_tt_order") . " WHERE uniacid=:uniacid AND openid IN ('" . implode("','", $down["openids"]) . "') AND createtime BETWEEN :time1 AND :time2 AND ccard>0", array( ":uniacid" => $_W["uniacid"], ":time1" => $start, ":time2" => $end ));
            if( $order ) 
            {
    ...
    public function saveRelation($id, $pid, $level = 1, $later = array(  ))
    {
        ...
        if( !empty($later) ) 
        {
            ...
            {
                $member = pdo_fetch("select * from " . tablename("tapgo_tt_member") . " where id = :id", array( ":id" => $pid ));
                return $member;
            }
        ...
        if( $level == 1 && 0 < $pid ) 
        {
            pdo_insert("tapgo_tt_commission_relation", array( "id" => $id, "pid" => $pid, "level" => $level ), true);
        }

        $childs = pdo_fetchall("select * from " . tablename("tapgo_tt_commission_relation") . " where id = :id", array( ":id" => $pid ));
        if( !empty($childs) ) 
        {
            foreach( $childs as $child ) 
            {
                ...
                pdo_insert("tapgo_tt_commission_relation", array( "id" => $id, "pid" => $child["pid"], "level" => (int) ($child["level"] + $level) ), true);
            }
        }

        $parent = pdo_fetch("select id,uniacid,agentid from " . tablename("tapgo_tt_member") . " where id = :id", array( ":id" => $pid ));
        if( !empty($parent) ) 
        {
            ...
            {
                $member = pdo_fetch("select * from " . tablename("tapgo_tt_member") . " where id = :id", array( ":id" => $id ));
                return $member;
            }

            pdo_insert("tapgo_tt_commission_relation", array( "id" => $id, "pid" => $parent["agentid"], "level" => ++$level ), true);
    ...
    {
        $parent = pdo_fetchall("select * from " . tablename("tapgo_tt_commission_relation") . " where id =:id", array( ":id" => $id ));
        if( !empty($parent) ) 
        {
            foreach( $parent as $p ) 
            {
                pdo_query("DELETE FROM " . tablename("tapgo_tt_commission_relation") . " WHERE id = " . $p["id"] . " and level >=" . $p["level"]);
            }
        }

        pdo_delete("tapgo_tt_commission_relation", array( "id" => $id ));
    }
    ...
    public function modify($id, $pid = NULL)
    {
        ...
        $parent = pdo_fetchall("select * from " . tablename("tapgo_tt_commission_relation") . " where pid =:pid", array( ":pid" => $id ));
        foreach( $parent as $item ) 
        {
            pdo_query("DELETE FROM " . tablename("tapgo_tt_commission_relation") . " WHERE id = " . $item["id"] . " and level >" . $item["level"]);
