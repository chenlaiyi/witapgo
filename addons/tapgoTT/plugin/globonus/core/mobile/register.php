...
require TAPGO_TT_PLUGIN . 'globonus/core/page_login_mobile.php';
class Register_TapgoTtPage extends GlobonusMobileLoginPage
{
	public function main()
	{
		...
		if ($set['become'] == '2') {
			...
			$status = 1;
			$moneycount = pdo_fetchcolumn('select sum(goodsprice) from ' . tablename('tapgo_tt_order') . ' where uniacid=:uniacid and openid=:openid and status>=' . $order_status . ' limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
			...
			if ($set['become'] == 4) {
				...
				$goodscount = pdo_fetchcolumn('select count(*) from ' . tablename('tapgo_tt_order_goods') . ' og ' . '  left join ' . tablename('tapgo_tt_order') . ' o on o.id = og.orderid' . ' where og.goodsid=:goodsid and o.openid=:openid and o.status>=' . $order_status . '  limit 1', array(':goodsid' => $set['become_goodsid'], ':openid' => $openid));
