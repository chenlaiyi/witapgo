...
//            $result = json_decode('{"errcode":0,"errmsg":"ok","room_info":[{"name":"测试直播","cover_img":"http:\/\/mmbiz.qpic.cn\/mmbiz_png\/7xibEKVHnApB8D3MmD1ia2J4tmtTWRp4L3X2JXlU30E0lBRLjibcMnZibzsq1CEEUyD7xUuF9VZcU91AdnibXZOibdLw\/0?wx_fmt=png","start_time":1583157600,"end_time":1583164800,"anchor_name":"李大妮","anchor_img":"http:\/\/mmbiz.qpic.cn\/mmbiz_png\/7xibEKVHnApB8D3MmD1ia2J4tmtTWRp4L3X2JXlU30E0lBRLjibcMnZibzsq1CEEUyD7xUuF9VZcU91AdnibXZOibdLw\/0?wx_fmt=png","roomid":2,"goods":[],"live_status":102}],"total":1,"live_replay":[]}', true);
            ...
            foreach ($result['room_info'] as $room) {
                ...
                // 查询当前直播间是否已经存在
                $wxlive = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('tapgo_tt_wxlive') . " WHERE `room_id`=:room_id AND `uniacid`=:uniacid", array(':uniacid' => $uniacid, ':room_id' => $roomId));
                ...
                if (empty($wxlive)) {
                    ...
                    // 插入数据
                    pdo_insert('tapgo_tt_wxlive', $insertData);
                    continue;
                }

                // 如果数据库中已经存在走更新
                pdo_update('tapgo_tt_wxlive', $updateData, array(
                    'room_id' => $room['roomid'],
                    'uniacid' => $uniacid,
                ));
            }
        ...
        // 批量删除没操作的直播间
        pdo_query("DELETE FROM " . tablename('tapgo_tt_wxlive') . " where room_id not in ( " . implode(',', $roomIds) . ") AND uniacid=:uniacid", array(':uniacid' => $uniacid));
    ...
    function flushLiveStatus($uniacid)
    {
        ...
        // 未开播更新为开播
        pdo_query('UPDATE ' . tablename('tapgo_tt_wxlive') . " SET `local_live_status`=1 WHERE `local_live_status` = 0 AND `uniacid` = :uniacid AND `start_time` < :time AND `end_time` > :time", array(':uniacid' => $uniacid, ':time' => $time));

        // 开播中更新为下播
        pdo_query('UPDATE ' . tablename('tapgo_tt_wxlive') . " SET `local_live_status`=-1 WHERE `uniacid` = :uniacid AND `end_time` < :time", array(':uniacid' => $uniacid, ':time' => $time));
    ...
    {
        $ret = pdo_get('tapgo_tt_wxlive', compact('id', 'room_id'));
        return $ret['name'];
    }
