    ...
    function property()
    {
        ...
        if (empty($id) && is_array($_GPC['ids'])) {
            foreach ($_GPC['ids'] as $item) {
                pdo_update("tapgo_tt_wxlive", array($type => $value), array("id" => $item, "uniacid" => $_W['uniacid']));
                plog('wxlive.room.edit', "修改直播{$typestr}状态   ID: {$item} {$statusstr} ");
            }
        } else {
            pdo_update("tapgo_tt_wxlive", array($type => $value), array("id" => $id, "uniacid" => $_W['uniacid']));
