...
class Back_TapgoTtPage extends PluginWebPage
{
    public function main()
    {
        ...
        $item = pdo_get('tapgo_tt_wxlive', compact('id', 'room_id'));
        if (empty($item)) {
            $this->message('直播间不存在~', '', 'error');
        }
        $page =  intval($_GPC['page']);
        $total = '';
        $list = pdo_getslice('tapgo_tt_wxlive_back', compact('live_id', 'room_id', 'uniacid'), [$page*20, 20], $total, $fields = array(), $keyfield = 'id,room_id');
    ...
    public function getBack()
    {
        ...
        if ($list['total'] > 0) {
            $ret = pdo_delete('tapgo_tt_wxlive_back', compact('room_id', 'uniacid'));
            ...
            foreach ($result as $item) {
                ...
                pdo_insert('tapgo_tt_wxlive_back', $item);
            }
        }
    ...
    public function delete()
    {
        ...
        global $_W;
        $id = $_GPC['id'];
        pdo_delete('tapgo_tt_wxlive_back', array('id' => $id));
