...
class Index_TapgoTtPage extends PluginMobileLoginPage {
    ...
    function main() {
        ...
        $share_res = pdo_fetch("SELECT * FROM ".tablename('tapgo_tt_bargain_account') ."WHERE id = :id",array(':id'=>$_W['uniacid']));
        ...
        if ($_W['ispost']){
            ...
            $res2 = pdo_fetchall("SELECT * FROM". tablename('tapgo_tt_bargain_goods') ."WHERE account_id = :account_id AND status ='0' ORDER BY id DESC",array(':account_id'=>$_W['uniacid']));
            ...
            foreach($res2 as $i=>$value) {
                ...
                $res[$i] = $res2[$i];
                $res3 = pdo_fetch("SELECT * FROM". tablename('tapgo_tt_goods') ."WHERE id = :id AND deleted = '0' AND status = '1' AND title LIKE :title ",array (':id'=>$res[$i]['goods_id'],':title'=>$keywords));
                if (empty($res3)){
                    unset($res[$i]);
                    continue;
                }
        ...
        foreach($res2 as $i=>$value) {
            ...
            $res[$i] = $res2[$i];
            $res3 = pdo_fetch("SELECT * FROM". tablename('tapgo_tt_goods') ."WHERE id = :id AND deleted = '0' AND status = '1'",array(':id'=>$res[$i]['goods_id']));
            if (empty($res3)){
                unset($res[$i]);
                continue;
            }
    ...
    function purchase(){
        ...
        $act = pdo_fetchall("SELECT * FROM ". tablename('tapgo_tt_bargain_actor') ." WHERE openid= :openid AND account_id = :account_id AND status = '1' ORDER BY id DESC",array(':openid'=>$_W['openid'],':account_id'=>$_W['uniacid']));//所有我发起的活动
        for ($i=0; $i < count($act); $i++) {
            $goods[$i] = pdo_fetchall("SELECT * FROM ". tablename('tapgo_tt_bargain_goods') ." WHERE id=:id",array(':id'=>$act[$i]['goods_id']));


            $ewei_detail = pdo_fetch("SELECT * FROM ". tablename('tapgo_tt_goods') ." WHERE id = :id AND status = '1'",array(':id'=>$goods[$i][0]['goods_id']));
    ...
    function act(){
        ...
        $act = pdo_fetchall("SELECT * FROM ". tablename('tapgo_tt_bargain_actor') ." WHERE openid= :openid AND account_id = :account_id AND status = '0' ORDER BY id DESC",array(':openid'=>$_W['openid'],':account_id'=>$_W['uniacid']));//所有我发起的活动
        for ($i=0; $i < count($act); $i++) {
            $goods[$i] = pdo_fetchall("SELECT * FROM ". tablename('tapgo_tt_bargain_goods') ." WHERE id=:id AND status = '0'",array(':id'=>$act[$i]['goods_id']));

            $ewei_detail = pdo_fetch("SELECT * FROM ". tablename('tapgo_tt_goods') ." WHERE id = :id AND status = '1' AND deleted='0'",array(':id'=>$goods[$i][0]['goods_id']));
            if (empty($ewei_detail)){
                unset($goods[$i]);
                continue;
            }
    ...
    function detail(){
        ...
        $res = pdo_fetch("SELECT * FROM ". tablename('tapgo_tt_bargain_goods') ." WHERE id = :id AND status='0'",array(':id'=>$id));
        if(!$res){
            include $this->template('bargain/lost');
            return;
        }
        $ewei_detail = pdo_fetch("SELECT * FROM ". tablename('tapgo_tt_goods') ." WHERE id = :id AND status = '1' AND deleted = '0'",array(':id'=>$res['goods_id']));
        if(!$ewei_detail){
            include $this->template('bargain/lost');
            return;
        }
       ...
       if(!empty($ewei_detail['thumb']) && empty($res['images'])){
        ...
        if (!empty($res['initiate'])) {
            $act_id = pdo_get('tapgo_tt_bargain_actor', array('goods_id' => $id, 'openid' => $_W['openid'],'status'=>0), 'id');
            if (!empty($act_id)) {
                $act_swi = $act_id;
            }
        }
    ...
    public function bargain(){
        ...
        $account_set = pdo_get('tapgo_tt_bargain_account',array('id'=>$_W['uniacid']),array('partin','rule','sharestyle'));

        $res = pdo_fetch("SELECT * FROM ". tablename('tapgo_tt_bargain_actor') ." WHERE id = :id",array(':id'=>$id));
        $res2 = pdo_fetch("SELECT * FROM ". tablename('tapgo_tt_bargain_goods') ." WHERE id = :id AND status='0'",array(':id'=>$res['goods_id']));
        $ewei_detail = pdo_fetch("SELECT * FROM ". tablename('tapgo_tt_goods') ." WHERE id = :id AND status = '1' AND bargain > 0 AND deleted=0",array(':id'=>$res2['goods_id']));
        ...
        $res3 = pdo_fetchall("SELECT * FROM ". tablename('tapgo_tt_bargain_record') ." WHERE actor_id = :actor_id ORDER BY id DESC",array(':actor_id'=>$id));
        $res4 = pdo_fetchall("SELECT * FROM ". tablename('tapgo_tt_bargain_actor') ." WHERE goods_id = {$res2['id']} AND account_id=:uniacid and bargain_price <= '{$account_set['partin']}' ORDER BY bargain_price ASC limit 20",array(':uniacid'=>$_W['uniacid']));

        $min_price = $res2['end_price'];
        $max_price = $res2['start_price'];
        if (pdo_fetch("SELECT * FROM ". tablename('tapgo_tt_bargain_actor') ." WHERE id = :id AND status = '1'",array(':id'=>$id))) {
            if ($trade_swi!=2) {
                $trade_swi = 1;
            }
        }elseif ($res2['stock']<=0) {
        ...
        if ($ajax == 151) {
            ...
            $account_set['partin'] *= -1;
            $myself_swi = 0;
            $myself_count = pdo_get('tapgo_tt_bargain_record',array('openid'=>$_W['openid'],'actor_id'=>$res['id']),'id');
    ...
    function join(){
        ...
        $res = pdo_fetch("SELECT * FROM ". tablename('tapgo_tt_bargain_goods') ." WHERE id = :id",array(':id'=>$goods_id));
        if ($res['act_times']>=$res['maximum']){
            $this->message('活动次数已到达上限,不能发起砍价',mobileUrl('bargain/detail',array('id'=>$goods_id)));
        }
        if(!empty($res['initiate'])){
            $count = pdo_get('tapgo_tt_bargain_actor',array('goods_id'=>$goods_id,'openid'=>$_W['openid'],'status'=>0,'order'=>0),'id');
            if(!empty($count['id'])){
                echo "<script>window.location.href = '".mobileUrl('bargain/bargain',array('id'=>$count['id']))."'</script>";                die();
            }
        }
        $goods_detail = pdo_fetch("SELECT * FROM ". tablename('tapgo_tt_goods') ." WHERE id = :id AND status='1'",array(':id'=>$res['goods_id']));
        ...
        if(!empty($user_info['openid'])){
            $if = pdo_insert('tapgo_tt_bargain_actor',$data);
            $id = pdo_insertid();
            pdo_query("UPDATE ". tablename('tapgo_tt_bargain_goods') ." SET act_times=act_times+1 WHERE id= :id",array(':id'=>$goods_id));
        }else{die('拒绝访问');}
    ...
    public function cut($actor_id,$time3,$min_price,$each_time,$total_time,$max_price,$probability_json){
        ...
        $min *= 100;
        $max *= 100;
        $record_res = pdo_fetchall("SELECT * FROM ". tablename('tapgo_tt_bargain_record') ." WHERE actor_id=:actor_id AND openid= :openid",array(':actor_id'=>$actor_id,':openid'=>$_W['openid']));
        $all_record = pdo_fetchall("SELECT * FROM ". tablename('tapgo_tt_bargain_record') ." WHERE actor_id= :actor_id",array(':actor_id'=>$actor_id));
        if (empty($actor_id)) {
            return "砍价失败！";
        } elseif ($time3 <= 0) {
            return "砍价已结束！";
        ...
        $now_price = pdo_fetch("SELECT * FROM ". tablename('tapgo_tt_bargain_actor') ." WHERE id = :id",array(':id'=>$actor_id));
        ...
        $res_ins = pdo_insert('tapgo_tt_bargain_record',$insert_data);
        $res_act = pdo_query("UPDATE ". tablename('tapgo_tt_bargain_actor') ." SET now_price = now_price + :now_price ,update_time = :update_time , bargain_price = bargain_price + :bargain_price WHERE id= :id",array(':now_price'=>$cut_price,':update_time'=>$time,':bargain_price'=>$cut_price,':id'=>$actor_id));
        ...
        if ($cut_price<=0) {
            ...
            pdo_query("UPDATE ". tablename('tapgo_tt_bargain_actor') ." SET bargain_times = bargain_times + 1 WHERE id = :id",array(':id'=>$actor_id));
    ...
    function rule(){
        ...
        $rule = pdo_get('tapgo_tt_bargain_goods',array('id'=>$id,'account_id'=>$_W['uniacid']),array('rule'));
        if (empty($rule['rule'])) {
            $rule = pdo_get('tapgo_tt_bargain_account', array('id' => $_W['uniacid']), array('rule'));
        }
    ...
    public function sendNotice(array $params) {
        ...
        if (!empty($templateid)) {
            ...
            if (!empty($template)) {
                ...
                if (is_error($ret)) {
                    $templatetype = pdo_fetch('select templateid  from ' . tablename('tapgo_tt_member_message_template_type') . ' where typecode=:typecode  limit 1', array(':typecode' => $tag));
