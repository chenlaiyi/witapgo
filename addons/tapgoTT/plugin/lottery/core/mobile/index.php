...
class index_TapgoTtPage extends PluginMobilePage {
    ...
    function main() {
        global $_W, $_GPC;
        $task_sql = 'SELECT COUNT(*) FROM '.tablename('tapgo_tt_lottery_join').'  WHERE lottery_num>0 and uniacid=:uniacid AND `join_user`=:join_user ';
    ...
    function getlotterylist(){
        ...
        $limit = ($page-1)*15;
        $task_sql = 'SELECT j.*,l.lottery_title,l.lottery_type,l.lottery_days FROM '.tablename('tapgo_tt_lottery_join').' as j left join '.tablename('tapgo_tt_lottery').' as l on j.lottery_id=l.lottery_id WHERE j.lottery_num>0 and j.uniacid=:uniacid AND j.`join_user`=:join_user AND l.`is_delete`=0 ORDER BY j.addtime DESC LIMIT '.$limit.',15';
        ...
        foreach ($lottery as $key=>$value){
            ...
            if($value['lottery_type']==1){
                $lottery[$key]['icon'] = '../addons/tapgo_ttv2/plugin/lottery/static/images/panicon.png';
            }elseif ($value['lottery_type']==2){
                $lottery[$key]['icon'] = '../addons/tapgo_ttv2/plugin/lottery/static/images/guaicon.png';
            }elseif ($value['lottery_type']==3){
                $lottery[$key]['icon'] = '../addons/tapgo_ttv2/plugin/lottery/static/images/gridicon.png';
            }
        }
        $task_sql = 'SELECT COUNT(*) FROM '.tablename('tapgo_tt_lottery_join').'  WHERE lottery_num>0 and uniacid=:uniacid AND `join_user`=:join_user ';
    ...
    function lottery_info(){
        ...
        $id = intval($_GPC['id']);
        if($id){
            $task_sql = 'SELECT * FROM '.tablename('tapgo_tt_lottery').' WHERE uniacid=:uniacid AND lottery_id=:id AND `is_delete`=0  ';
            $lottery = pdo_fetch($task_sql,array(':uniacid'=>$_W['uniacid'],':id'=>$id));
            $reward = unserialize($lottery['lottery_data']);
            $set = pdo_fetchcolumn('SELECT `data` FROM '.tablename('tapgo_tt_lottery_default') . " WHERE uniacid =:uniacid LIMIT 1",array(':uniacid'=>$_W['uniacid']));
            if(!empty($set)){
                $set = unserialize($set);
            }
            $where = '';
            ...
            //链表查询
            $log = pdo_fetchall('SELECT l.*,m.`nickname`,m.`avatar` FROM '.tablename('tapgo_tt_lottery_log').' AS l LEFT JOIN '.tablename('tapgo_tt_member').' AS m ON m.openid=l.join_user WHERE l.uniacid=:uniacid AND l.lottery_id=:lottery_id AND l.is_reward=1 '.$where.' order by l.log_id desc LIMIT 5',array(':uniacid'=>$_W['uniacid'],':lottery_id'=>$id));
            if(!empty($log)){
            ...
            $has_changes = pdo_fetchcolumn('select count(*) from '.tablename('tapgo_tt_lottery_join').'where uniacid=:uniacid AND lottery_id=:lottery_id  AND join_user=:join_user and lottery_num>0 '.$conditon,array(':uniacid'=>$_W['uniacid'],':join_user'=>$_W['openid'],':lottery_id'=>$id));
            //链表查询
            //用户头像
    ...
    function myreward(){
        ...
        //用户头像
        $member = m('member')->getMember($_W['openid'], true);
        $mylog = pdo_fetchcolumn('SELECT COUNT(*) FROM '.tablename('tapgo_tt_lottery_log').' WHERE uniacid=:uniacid  AND join_user=:join_user AND is_reward=1 ',array(':uniacid'=>$_W['uniacid'],':join_user'=>$_W['openid']));
    ...
    function myrewardpage(){
        ...
        $limit = ($page-1)*15;
        $mylog = pdo_fetchall('SELECT l.*,m.`nickname`,m.`avatar` FROM '.tablename('tapgo_tt_lottery_log').' AS l LEFT JOIN '.tablename('tapgo_tt_member').' AS m ON m.openid = l.join_user and l.uniacid = m.uniacid WHERE l.uniacid=:uniacid  AND l.join_user=:join_user AND l.is_reward=1 order by addtime desc LIMIT '.$limit.',15',array(':uniacid'=>$_W['uniacid'],':join_user'=>$_W['openid']));
        $count =  pdo_fetchcolumn('SELECT COUNT(*) FROM '.tablename('tapgo_tt_lottery_log').' WHERE uniacid=:uniacid  AND join_user=:join_user AND is_reward=1 ',array(':uniacid'=>$_W['uniacid'],':join_user'=>$_W['openid']));
        ...
        foreach ($mylog as $key=>$value){
            ...
            if (isset($lottery_data['credit'])){
                ...
                $mylog[$key]['icon'] ='../addons/tapgo_ttv2/plugin/lottery/static/images/jifen.png';
            }elseif(isset($lottery_data['money'])){
                $mylog[$key]['title'] = '奖金:'.$lottery_data['money']['num'];
                $mylog[$key]['rewarded'] = 1;
                $mylog[$key]['icon'] = '../addons/tapgo_ttv2/plugin/lottery/static/images/jiangjin.png';
            }elseif(isset($lottery_data['bribery'])){
                $mylog[$key]['title'] = '红包:'.$lottery_data['bribery']['num'];
                $mylog[$key]['rewarded'] = 1;
                $mylog[$key]['icon'] = '../addons/tapgo_ttv2/plugin/lottery/static/images/hongbao.png';
            }elseif(isset($lottery_data['goods'])){
                $goods = array_shift($lottery_data['goods']);
                $mylog[$key]['title'] = '特惠商品:'.$goods['title'];
                $mylog[$key]['rewarded'] = 0;
                $mylog[$key]['icon'] = '../addons/tapgo_ttv2/plugin/lottery/static/images/shangpin.png';
            }elseif(isset($lottery_data['coupon'])){
                $coupon = array_shift($lottery_data['coupon']);
                $mylog[$key]['title'] = '优惠券:'.$coupon['couponname'];
                $mylog[$key]['rewarded'] = 1;
                $mylog[$key]['icon'] = '../addons/tapgo_ttv2/plugin/lottery/static/images/quan.png';
            }
    ...
    function getlottery(){
        global $_W, $_GPC;
        $set_info = pdo_fetchcolumn('SELECT `data` FROM '.tablename('tapgo_tt_lottery_default') . " WHERE uniacid =:uniacid LIMIT 1",array(':uniacid'=>$_W['uniacid']));
    ...
    function getreward(){
        ...
        //检测是否已经抽过奖
        $has_changes = pdo_fetchcolumn('SELECT COUNT(*) FROM '.tablename('tapgo_tt_lottery_join').' WHERE uniacid=:uniacid AND lottery_id=:id AND join_user=:join_user AND lottery_num>0',array(':uniacid'=>$_W['uniacid'],':join_user'=>$_W['openid'],':id'=>$id));
        ...
        if(empty($has_changes)){
            ...
            $join_id = pdo_fetchcolumn('SELECT `id` FROM '.tablename('tapgo_tt_lottery_join').' WHERE uniacid=:uniacid AND lottery_id=:id AND join_user=:join_user AND lottery_num>0 order by addtime desc limit 1',array(':uniacid'=>$_W['uniacid'],':join_user'=>$_W['openid'],':id'=>$id));
        }
...
//如果总限制用完
        ...
        pdo_insert('tapgo_tt_lottery_log',$log_data);
        $res = pdo_query('UPDATE '.tablename('tapgo_tt_lottery_join').' SET lottery_num=lottery_num-1 WHERE uniacid=:uniacid AND lottery_id=:id AND join_user=:join_user and id=:join_id',array(':id'=>$id,':join_id'=>$join_id,':uniacid'=>$_W['uniacid'],':join_user'=>$_W['openid']));
        if($res===false){
            load()->func('logging');
    ...
    public function reward()
    {
        ...
        $res = pdo_update('tapgo_tt_lottery',$lottery_data,array('uniacid'=>$_W['uniacid'],'lottery_id'=>$id));
        if($res!==false){
            $info = array(
                'status'=>1,
    ...
    public function mygoods(){
        global $_W, $_GPC;
        $loglist = pdo_fetchall('SELECT l.* FROM '.tablename('tapgo_tt_lottery_log').' AS l WHERE l.uniacid=:uniacid  AND l.join_user=:join_user AND l.is_reward=1',array(':uniacid'=>$_W['uniacid'],':join_user'=>$_W['openid']));
        $goodslist =array();
        foreach ($loglist as $key=>$value){
            ...
            if(isset($value['goods'])&&!empty($value['goods'])){
                ...
                $goods['log_id']=$log_id;
                if(!empty($goods)){
                    $searchsql = 'SELECT thumb,marketprice FROM ' . tablename('tapgo_tt_goods') . ' WHERE uniacid= '.$_W['uniacid'].' and id='.$goods['id'].' and status=1 and deleted=0';
