...
class LotteryModel extends PluginModel {
    ...
    public function getGoods($param=''){
        ...
        $log = pdo_fetch('select * from '.tablename('tapgo_tt_lottery_log').' where log_id=:log_id and join_user=:join_user and is_reward=1',array(':log_id'=>$param['log_id'],':join_user'=>$param['openid']));
        if(empty($log)){
        ...
        if(isset($param['goods_num'])&&!empty($param['goods_num'])){
            ...
            if(!empty($goods_info['spec'])){
                ...
                if($goods_num>$goods_info['spec'][$param['goods_spec']]['total']){
                    ...
                    pdo_update('tapgo_tt_lottery_log',array('lottery_data'=>serialize($lottery_data)),array('log_id'=>$param['log_id']));
            ...
            $lottery = pdo_fetch('select lottery_days,is_goods from '.tablename('tapgo_tt_lottery').' where lottery_id=:lottery_id',array(':lottery_id'=>$log['lottery_id']));
    ...
    public function getLottery($openid,$type,$data){
        ...
        $lotterylist = pdo_fetchall('SELECT * FROM '.tablename('tapgo_tt_lottery').' WHERE uniacid='.$_W['uniacid'].' AND start_time<'.time().' AND  end_time>'.time().' AND is_delete=0 AND task_type='.$type.' ORDER BY addtime DESC');
            ...
            if($type==1){
                ...
                if(!empty($join_info['lottery_id'])){
                    for($i=1;$i<=$join_info['num'];$i++){
                        ...
                        pdo_insert('tapgo_tt_lottery_join',$join_data);
                    }
    ...
    public function getLotteryList($openid,$param=array()){
        ...
        if(!empty($lottery_list)){
            ...
            $url = $_W['siteroot'] . 'app/index.php?i=' . $_W['uniacid'] . '&c=entry&m=tapgo_ttv2&do=mobile&r=lottery.index.lottery_info&id='.$param['lottery_id'];
    ...
    public function reward($poster,$openid,$title,$lottery_id){
        ...
        if (isset($poster['bribery']) && $poster['bribery'] > 0) {
            ...
            if (!is_error($err)) {
                ...
                $log_id = pdo_fetchcolumn('SELECT log_id FROM '.tablename('tapgo_tt_lottery_log').' WHERE uniacid=:uniacid AND join_user=:join_user AND lottery_id=:lottery_id AND is_reward=1 ORDER BY addtime DESC LIMIT 1',array(':uniacid'=>$_W['uniacid'],':join_user'=>$openid,':lottery_id'=>$lottery_id));
                pdo_update('tapgo_tt_lottery_log', $upgrade, array('log_id' => $log_id));
            } else {//红包发送失败
...
//            dump($reward['goods']);
    ...
    public function check_isreward(){
        ...
        $changes = pdo_fetch('select * from '.tablename('tapgo_tt_lottery_join').' where uniacid=:uniacid and join_user=:join_user and addtime>'.$start_time.' and addtime<='.$end_time.' and lottery_num>0 limit 1',array(':uniacid'=>$_W['uniacid'],':join_user'=>$_W['openid']));
        if(!empty($changes)){
