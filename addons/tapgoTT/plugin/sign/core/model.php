...
class SignModel extends PluginModel
{
    ...
    public function getSet() {
        ...
        $set = pdo_fetch("select *  from " . tablename('tapgo_tt_sign_set') . " where uniacid=:uniacid limit 1 ", array(':uniacid'=>$_W['uniacid']));
    ...
    public function getCalendar($year=null, $month=null, $week=true)
    {
        ...
        //  2. 查出 签到记录并 合并
        $records = pdo_fetchall("select *  from " . tablename('tapgo_tt_sign_records') . " where openid=:openid and `type`=0 and `time` between :starttime and :endtime and uniacid=:uniacid ", array(':uniacid'=>$_W['uniacid'], ':openid'=>$_W['openid'], ':starttime'=>$date['firsttime'], ':endtime'=>$date['lasttime']));
    ...
    public function getSign($date=null)
    {
        ...
        //  根据系统设置 指定时间段查询
        $records = pdo_fetchall("select * from " . tablename('tapgo_tt_sign_records') . ' where openid=:openid and `type`=0 and uniacid=:uniacid '. $condition . ' order by `time` desc ', array(':uniacid'=>$_W['uniacid'], ':openid'=>$_W['openid']));
    ...
    public function getAdvAward()
    {
        ...
        //  根据系统设置 指定时间段查询
        $records = pdo_fetchall("select * from " . tablename('tapgo_tt_sign_records') . ' where openid=:openid and uniacid=:uniacid '. $condition . ' order by `time` asc ', array(':uniacid'=>$_W['uniacid'], ':openid'=>$_W['openid']));
        if(!empty($records)){
            foreach ($records as $item){
    ...
    public function updateSign($signinfo){
        ...
        $info = pdo_fetch("select id  from " . tablename('tapgo_tt_sign_user') . " where openid=:openid and uniacid=:uniacid limit 1 ", array(':openid'=>$_W['openid'], ':uniacid'=>$_W['uniacid']));
        $data = array(
            'openid'=>$_W['openid'],
        ...
        if(empty($info)){
            ...
            pdo_insert('tapgo_tt_sign_user', $data);
        }else{
            pdo_update('tapgo_tt_sign_user', $data, array('id'=>$info['id']));
        }
    }
}
