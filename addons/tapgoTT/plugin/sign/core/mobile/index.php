...
class Index_TapgoTtPage extends PluginMobileLoginPage {
    ...
    public function dosign() {
        ...
        pdo_insert('tapgo_tt_sign_records', $arr);
        $id = pdo_insertid();
    ...
    public function doreward() {
        ...
        //  根据系统设置 指定时间段查询
        $record = pdo_fetch("select * from " . tablename('tapgo_tt_sign_records') . ' where openid=:openid and `type`='.$type.' and `day`='.$day.' and uniacid=:uniacid '. $condition . ' limit 1 ', array(':uniacid'=>$_W['uniacid'], ':openid'=>$_W['openid']));
    ...
    public function getRecords()
    {
        ...
        $sql = 'SELECT COUNT(*) FROM ' . tablename('tapgo_tt_sign_records') . " log where 1 {$condition}";
        $total = pdo_fetchcolumn($sql, $params);
        $list = array();
        if (!empty($total)) {
            $sql = 'SELECT * FROM ' . tablename('tapgo_tt_sign_records') . ' where 1 ' . $condition . ' ORDER BY `time` DESC LIMIT ' . ($pindex - 1) * $psize . ',' . $psize;
    ...
    public function getRank() {
        ...
        if(!empty($type)){
            ...
            $sql = 'SELECT COUNT(*) FROM ' . tablename('tapgo_tt_sign_user') . " where 1 {$conditioncol}";
            $total = pdo_fetchcolumn($sql, $params);
            $list = array();
            if (!empty($total)) {
                $type  = 'su.'.$type;
                $sql = 'SELECT su.*, sm.nickname, sm.avatar FROM ' . tablename('tapgo_tt_sign_user') . ' su left join ' . tablename('tapgo_tt_member') . ' sm on sm.openid=su.openid where 1 ' . $condition . ' ORDER BY '.$type.' DESC LIMIT ' . ($pindex - 1) * $psize . ',' . $psize;
                $list = pdo_fetchall($sql, $params);
                if(!empty($list)){
                    foreach ($list as &$item){
