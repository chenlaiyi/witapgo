...
class CashierModel extends PluginModel 
{
	...
	public function paySuccess($log, $user) 
	{
		...
		pdo_update('tapgo_tt_cashier_pay_log', array('openid' => $log['openid'], 'payopenid' => $log['openid'], 'money' => $log['money'], 'status' => 1, 'paytime' => (0 < $log['paytime'] ? $log['paytime'] : time()), 'coupon' => $coupon), array('id' => $log['id']));
	...
	public function getOrderMoney($starttime = 0, $endtime = 0) 
	{
		...
		{
			$money = (double) pdo_fetchcolumn('SELECT SUM(money+deduction) FROM ' . tablename('tapgo_tt_cashier_pay_log') . ' WHERE uniacid=:uniacid AND status=1 AND cashierid=:cashierid ', array(':uniacid' => $_W['uniacid'], ':cashierid' => $_W['cashierid']));
		}
		else 
		{
			$money = (double) pdo_fetchcolumn('SELECT SUM(money+deduction) FROM ' . tablename('tapgo_tt_cashier_pay_log') . ' WHERE uniacid=:uniacid AND status=1 AND cashierid=:cashierid AND createtime BETWEEN :starttime AND :endtime', array(':uniacid' => $_W['uniacid'], ':cashierid' => $_W['cashierid'], ':starttime' => $starttime, ':endtime' => $endtime));
		}
		return $money;
	}
	...
	public function setStocks($orderid = 0) 
	{
		...
		$goods = pdo_fetchall('select og.goodsid,og.total,g.totalcnf,og.realprice,g.credit,og.optionid,g.total as goodstotal,og.optionid,g.sales,g.salesreal from ' . tablename('tapgo_tt_order_goods') . ' og ' . ' left join ' . tablename('tapgo_tt_goods') . ' g on g.id=og.goodsid ' . ' where ' . $condition . ' and og.uniacid=:uniacid ', $param);
		foreach ($goods as $g ) 
		{
			$stocktype = -1;
			if (!(empty($stocktype))) 
			...
			if (1 <= $order['status']) 
			{
				...
				$salesreal = pdo_fetchcolumn('select ifnull(sum(total),0) from ' . tablename('tapgo_tt_order_goods') . ' og ' . ' left join ' . tablename('tapgo_tt_order') . ' o on o.id = og.orderid ' . ' where og.goodsid=:goodsid and o.status>=1 and o.uniacid=:uniacid limit 1', array(':goodsid' => $g['goodsid'], ':uniacid' => $_W['uniacid']));
	...
	public function setSelfGoodsStocks($logid = 0) 
	{
		...
		$goods = pdo_fetchall('SELECT * FROM ' . tablename('tapgo_tt_cashier_pay_log_goods') . ' WHERE cashierid=:cashierid AND logid=:logid', array(':cashierid' => $_W['cashierid'], ':logid' => $logid));
		foreach ($goods as $g ) 
		{
