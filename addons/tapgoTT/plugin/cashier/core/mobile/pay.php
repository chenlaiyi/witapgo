...
class Pay_TapgoTtPage extends CashierMobilePage {
	...
	public function main()
    {
        ...
        $ret = pdo_get(
            'tapgo_tt_cashier_pay_log',array('uniacid'=>$_W['uniacid'],'openid'=>$_W['openid'])
        );

        if (empty($ret))
        {
            $url = mobileUrl('cashier/pay',array('cashierid'=>$cashierid));
            $redirect = 1;
        }
        if (!empty($_GPC['id'])){
            $id = $_GPC['id'];
            $qrid =authcode(base64_decode($id),'DECODE', "tapgo_ttv2_cashier");
            if (!empty($qrid)){
                $item = pdo_get("tapgo_tt_cashier_qrcode",array('uniacid'=>$_W['uniacid'],'cashierid'=>$_W['cashierid'],'id'=>$qrid));
    ...
    protected function paytype1()
    {
        ...
        if (is_array($config)){
            $config['return_url'] = $_W['siteroot'] . "addons/tapgo_ttv2/payment/alipay/return.php";
        }else{
            show_json(0);
        }
...
//        file_put_contents(TAPGO_TT_DATA . 'pay_error' . time() . '.json',json_encode(array(
//            'wechat' => $wechat,
//            'logid' => $this->log['id'],
//            'log' => $this->log
//        )));
