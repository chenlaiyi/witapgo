...
class PosteraModel extends PluginModel
{
    ...
    public function getQR($poster, $member)
    {
        ...
        $qr = pdo_fetch('select * from ' . tablename('tapgo_tt_postera_qr') . ' where openid=:openid and acid=:acid and posterid=:posterid limit 1', array(':openid' => $member['openid'], ':acid' => $acid, ':posterid' => $poster['id']));
        ...
        if (empty($qr)) {
            ...
            $ims_qrcode = array(
                ...
                'qrcid' => $scene_id,
                "model" => 0,
                "name" => "TAPGO_TT_POSTERA_QRCODE",
                "keyword" => 'TAPGO_TT_POSTERA',
                "expire" => $expire,
                "createtime" => time(),
                "status" => 1,
                'url' => $result['url'],
            ...
            pdo_insert('tapgo_tt_postera_qr', $qr);
            $qr['id'] = pdo_insertid();
        } else {
            ...
            if (time() > $qr['endtime'] || $qr['qrtime'] != ($poster['timestart'] . $poster['timeend'])) {
                ...
                pdo_update('tapgo_tt_postera_qr', array('ticket' => $ticket, 'qrimg' => $qrimg, 'url' => $result['url'], 'endtime' => $posterendtime, 'qrtime' => $poster['timestart'] . $poster['timeend']), array('id' => $qr['id']));
                $qr['ticket'] = $ticket;
    ...
    public function createPoster($poster, $member, $qr, $upload = true)
    {
        ...
        global $_W;
        $path = IA_ROOT . "/addons/tapgo_ttv2/data/postera/" . $_W['uniacid'] . "/";
        ...
        if (!empty($qr['goodsid'])) {
            $goods = pdo_fetch('select id,title,thumb,commission_thumb,marketprice,productprice from ' . tablename('tapgo_tt_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $qr['goodsid'], ':uniacid' => $_W['uniacid']));
            if (empty($goods)) {
        ...
        if ($qr['qrimg'] != $qr['current_qrimg'] || empty($qr['mediaid']) || empty($qr['createtime']) || $qr['createtime'] + 3600 * 24 * 3 - 7200 < time()) {
            ...
            $qr['mediaid'] = $mediaid;
            pdo_update('tapgo_tt_postera_qr', array('mediaid' => $mediaid, 'createtime' => time()), array('id' => $qr['id']));
        }
    ...
    public function getQRByTicket($ticket = '')
    {
        ...
        $qrs = pdo_fetchall('select * from ' . tablename('tapgo_tt_postera_qr') . ' where ticket=:ticket and acid=:acid limit 1', array(':ticket' => $ticket, ':acid' => $_W['acid']));
        $count = count($qrs);
        if ($count <= 0) {
            return false;
        }
    ...
    public function create_folder() {
        ...
        $path = IA_ROOT . "/addons/tapgo_ttv2/data/postera/";
        if (!is_dir($path)) {
            mkdir($path);
        }
    ...
    public function previewPoster($poster, $goodsId,$id)
    {
        ...
        if (!empty($goodsId)) {
            $goods = pdo_fetch('select id,title,thumb,commission_thumb,marketprice,productprice from ' . tablename('tapgo_tt_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $goodsId, ':uniacid' => $_W['uniacid']));
        ...
        if(!is_file($path . $file)){ 
            ...
            foreach ($data as $d) {
                ...
                if ($d['type'] == 'head') {
                    ...
                    $avatar = '../addons/tapgo_ttv2/plugin/sns/static/images/head.jpg';
                    ...
                    $src = '../addons/tapgo_ttv2/plugin/postera/static/images/img.jpg';
                    $src = tomedia($src);
                    $target = $this->mergeImage($target, $d, $src);
                } else if ($d['type'] == 'qr') {
                    $qrImg = '../addons/tapgo_ttv2/plugin/postera/static/images/qr.jpg';
