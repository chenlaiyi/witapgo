...
class Index_TapgoTtPage extends PluginWebPage {
	...
	function main() {
		...
		$list = pdo_fetchall("SELECT * FROM " . tablename('tapgo_tt_postera') . " WHERE 1 {$condition} ORDER BY isdefault desc,createtime desc LIMIT " . ($pindex - 1) * $psize . ',' . $psize, $params);
		foreach ($list as &$row) {
			$row['follows'] = pdo_fetchcolumn('select count(*) from ' . tablename('tapgo_tt_postera_log') . ' where posterid=:posterid and uniacid=:uniacid', array(':posterid' => $row['id'], ':uniacid' => $_W['uniacid']));
		}
		unset($row);
		$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('tapgo_tt_postera') . " where 1 {$condition} ", $params);
	...
	protected function post() {
		...
		$item = pdo_fetch("SELECT * FROM " . tablename('tapgo_tt_postera') . " WHERE id =:id and uniacid=:uniacid limit 1", array(':id' => $id, ':uniacid' => $_W['uniacid']));
		if (!empty($item)) {
		...
		if ($_W['ispost']) {
			...
			if($item['keyword2'] != $data['keyword2'] && !empty($keyword)){
				if($keyword['name']!='tapgo_ttv2:postera:' . $id){
					show_json(0, '关键字已存在!');
				}
			}
			if ($plugin_coupon) {
			...
			if ($data['isdefault'] == 1) {
				pdo_update('tapgo_tt_postera', array('isdefault' => 0), array('uniacid' => $_W['uniacid'], 'isdefault' => 1, 'type' => $data['type']));
			}
			if (!empty($id)) {
				pdo_update('tapgo_tt_postera', $data, array('id' => $id, 'uniacid' => $_W['uniacid']));
				...
				pdo_insert('tapgo_tt_postera', $data);
				$id = pdo_insertid();
			...
			$rule = pdo_fetch("select * from " . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name  limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'tapgo_ttv2', ':name' => "tapgo_ttv2:postera:" . $id));
			$rule_data = array(
				'uniacid' => $_W['uniacid'],
				'name' => 'tapgo_ttv2:postera:' . $id,
				'module' => 'tapgo_ttv2',
				'displayorder' => 0,
				'status' => $data['status']
			);
			...
			$ruleauto = pdo_fetch("select * from " . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name  limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'tapgo_ttv2', ':name' => "tapgo_ttv2:postera:auto"));
			if (empty($ruleauto)) {
				$rule_data = array(
					'uniacid' => $_W['uniacid'],
					'name' => 'tapgo_ttv2:postera:auto',
					'module' => 'tapgo_ttv2',
					'displayorder' => 0,
					'status' => 1
				);
		...
		if (!empty($item['goodsid'])) {
			$goods = set_medias(pdo_fetch('select id,title,thumb,commission_thumb,marketprice,productprice from ' . tablename('tapgo_tt_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $item['goodsid'], ':uniacid' => $_W['uniacid'])), 'thumb');
		}
		if (empty($item)) {
			$starttime = time();
	...
	function setdefault() {
		global $_W, $_GPC;
		$id = intval($_GPC['id']);
		$poster = pdo_fetch("SELECT * FROM " . tablename('tapgo_tt_postera') . " WHERE id = '$id'");
		if (empty($poster)) {
			show_json(0, '抱歉，海报不存在或是已经被删除！');
		}
		pdo_update('tapgo_tt_postera', array('isdefault' => 0), array('uniacid' => $_W['uniacid'], 'isdefault' => 1, 'type' => $poster['type']));
		pdo_update('tapgo_tt_postera', array('isdefault' => 1), array('uniacid' => $_W['uniacid'], 'id' => $poster['id']));
	...
	function delete() {
		...
		$posters = pdo_fetchall("SELECT id,title,keyword,keyword2 FROM " . tablename('tapgo_tt_postera') . " WHERE id in ( $id ) and uniacid=" . $_W['uniacid']);
		foreach ($posters as $poster) {
			...
			pdo_delete('tapgo_tt_postera', array('id' => $poster['id'], 'uniacid' => $_W['uniacid']));
			pdo_delete('tapgo_tt_postera_log', array('posterid' => $poster['id'], 'uniacid' => $_W['uniacid']));
	...
	function clear() {
		global $_W, $_GPC;
		load()->func('file');
		@rmdirs(IA_ROOT . "/addons/tapgo_ttv2/data/postera/" . $_W['uniacid']);
		@rmdirs(IA_ROOT . "/addons/tapgo_ttv2/data/qrcode/" . $_W['uniacid']);
		$acid = pdo_fetchcolumn("SELECT acid FROM " . tablename('account_wechats') . " WHERE `uniacid`=:uniacid LIMIT 1", array(':uniacid' => $_W['uniacid']));
		pdo_update('tapgo_tt_postera_qr', array('mediaid' => ''), array('acid' => $acid));
