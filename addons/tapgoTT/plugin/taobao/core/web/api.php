...
require_once(TAPGO_TT_PATH . "vendor/phpquery/phpQuery.php");

class Api_TapgoTtPage extends PluginWebPage
{
    ...
    private function api_taobao($cath_id = 0, $catch_url = "", $pageSource = "")
    {
        ...
        if( !empty($pageSource) ) 
        {
            ...
            {
                pdo_insert("tapgo_tt_goods_param", array( "goodsid" => $goodsid, "uniacid" => $_W["uniacid"], "title" => $key, "value" => $val ));
            }
            $spec_list = array(  );
            $div = pq("#J_isku div");
            ...
            if( !empty($hasoption) && !empty($spec_list) ) 
            {
                ...
                {
                    pdo_insert("tapgo_tt_goods_spec", array( "uniacid" => $_W["uniacid"], "goodsid" => $goodsid, "title" => $k ));
                    $specId = pdo_insertid();
                    foreach( $v["spec_item"] as $key => $val ) 
                    {
                        $title = $val;
                        $thumb = "";
                        pdo_insert("tapgo_tt_goods_spec_item", array( "specid" => $specId, "title" => $title, "thumb" => (empty($thumb) ? "" : $thumb), "show" => 1 ));
