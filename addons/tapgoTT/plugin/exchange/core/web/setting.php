...
class Setting_TapgoTtPage extends PluginWebPage
{
	...
	public function download()
	{
		global $_GPC;
		global $_W;
		$dir = IA_ROOT . ('/addons/tapgo_ttv2/data/qrcode/' . $_W['uniacid'] . '/exchange');
	...
	public function other()
	{
		...
		if ($_W['ispost']) {
			...
			if (pdo_get('tapgo_tt_exchange_setting', array('uniacid' => $_W['uniacid'])) == false) {
				pdo_insert('tapgo_tt_exchange_setting', array('uniacid' => $_W['uniacid'], 'mistake' => $mistake, 'freeze' => $freeze, 'grouplimit' => $grouplimit, 'alllimit' => $alllimit, 'no_qrimg' => (int) $_GPC['no_qrimg'], 'rule' => $_GPC['rule']));
			}
			else {
				pdo_update('tapgo_tt_exchange_setting', array('mistake' => $mistake, 'freeze' => $freeze, 'grouplimit' => $grouplimit, 'alllimit' => $alllimit, 'no_qrimg' => (int) $_GPC['no_qrimg'], 'rule' => $_GPC['rule'], 'coupon_templateid' => $coupon_templateid), array('uniacid' => $_W['uniacid']));
			}

			show_json(1, '保存成功');
		}
		else {
			$res = pdo_fetch('SELECT * FROM ' . tablename('tapgo_tt_exchange_setting') . ' WHERE uniacid = :uniacid ', array(':uniacid' => $_W['uniacid']));
		}
