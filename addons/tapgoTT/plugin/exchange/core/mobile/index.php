...
class Index_TapgoTtPage extends PluginMobileLoginPage
{
    public function main(){
        ...
        if (!empty($all)&&!empty($codetype)){
            $res['rule'] = pdo_fetchcolumn('select rule from '.tablename('tapgo_tt_exchange_setting').' where uniacid = '.$_W['uniacid']);
            ...
            $keyresult = pdo_fetch("SELECT groupid FROM ".tablename('tapgo_tt_exchange_code')." WHERE `key` = :code AND uniacid = :uniacid",array(':code'=>$key,':uniacid'=>$_W['uniacid']));
            if (empty($keyresult)){
        ...
        if (!empty($codetype)){
            $res = pdo_fetch("SELECT * FROM ".tablename('tapgo_tt_exchange_group')." WHERE id = :id AND uniacid = :uniacid",array(':uniacid'=>$_W['uniacid'],':id'=>$id));
    ...
    private function is_exchange($key){
        global $_W;
        $set = pdo_fetch("SELECT * FROM ".tablename('tapgo_tt_exchange_setting')." WHERE uniacid = :uniacid",array(':uniacid'=>$_W['uniacid']));
        $this->counterror($set);
        $time = strtotime('now');
        $time = date('Y-m-d');
        $time1 = $time." 00:00:00";
        ...
        if (empty($_W['openid'])){
            ...
            $exchangelimit = pdo_fetchcolumn("SELECT COUNT(*) FROM ".tablename('tapgo_tt_exchange_record')." WHERE openid =:openid AND uniacid = :uniacid AND `time` > :timea AND `time` <= :timeb",array(':uniacid'=>$_W['uniacid'],':openid'=>$_W['openid'],':timea'=>$time1,':timeb'=>$time2));
        ...
        if (empty($_SESSION['exchangeGroupId'])){
            ...
            $exchangelimit2 = pdo_fetchcolumn("SELECT COUNT(*) FROM ".tablename('tapgo_tt_exchange_record')." WHERE openid =:openid AND uniacid = :uniacid AND `time` > :timea AND `time` <= :timeb AND groupid = :groupid",array(':uniacid'=>$_W['uniacid'],':openid'=>$_W['openid'],':timea'=>$time1,':timeb'=>$time2,':groupid'=>$_SESSION['exchangeGroupId']));
        ...
        $return = array();
        $table1 = tablename('tapgo_tt_exchange_group');
        $table2 = tablename('tapgo_tt_exchange_code');
        ...
        if ($_SESSION['exchangeGroupId'] != $codeResult['groupid']){
            ...
            pdo_query("UPDATE ".tablename('tapgo_tt_exchange_query')." SET `errorcount` = `errorcount` + 1 WHERE openid = :openid",array(':openid'=>$_W['openid']));
            return $return;
        }
        ...
        if ($codeResult === false){
            ...
            pdo_query("UPDATE ".tablename('tapgo_tt_exchange_query')." SET `errorcount` = 0 AND `unfreeze`=0 WHERE openid = :openid",array(':openid'=>$_W['openid']));
        }
        ...
        if(!empty($codeResult['openid']) && $codeResult['openid'] != $_W['openid'] && !empty($groupResult['binding'])){
            ...
            pdo_query("UPDATE ".tablename('tapgo_tt_exchange_code')." SET openid = :openid , `count` = `count` + 1 WHERE openid != :openid AND uniacid = :uniacid AND `key`=:key",array(':openid'=>$_W['openid'],':uniacid'=>$_W['uniacid'],':key'=>$key));
        }
    ...
    function balance(){
        ...
        if ($exchange == '1'){
            ...
            if ($balance_res === 1) {
                ...
                if (empty($repeatcount)) pdo_update("tapgo_tt_exchange_code", array('status' => 2,), array('key' => $key, 'uniacid' => $_W['uniacid'], 'status' => 1, 'repeatcount'=>1));
                ...
                pdo_insert('tapgo_tt_exchange_record',$record);
    ...
    function coupon(){
        ...
        if ($exchange == '1'){
            ...
            $allCoupon = pdo_fetchall('SELECT * FROM ' . tablename('tapgo_tt_coupon') . ' WHERE ' . $condition . ' and uniacid=:uniacid and merchid=0', array(':uniacid' => $_W['uniacid']));
            ...
            $repeatcount = $this->model->setRepeatCount($key);
            if (empty($repeatcount)) pdo_update("tapgo_tt_exchange_code", array('status' => 2,), array('key' => $key, 'uniacid' => $_W['uniacid'], 'status' => 1, 'repeatcount'=>1));$info = m('member')->getInfo($_W['openid']);
            $record = array(
                'key'=>$key,
                'uniacid'=>$_W['uniacid'],
        ...
        if ($exchange == '1'){
            ...
            if ($goods_arr['goods'] != false){
                foreach ($goods_arr['goods'] as $k => $v){
                    $goods = pdo_fetch("SELECT * FROM ".tablename('tapgo_tt_goods')." WHERE id = :id AND uniacid = :uniacid",array(':id'=>$v,':uniacid'=>$_W['uniacid']));
                    if (!empty($goods))$goodsList[$k] = $goods;
                }
            }
            if ($goods_arr['option'] != false){
                foreach ($goods_arr['option'] as $k => $v){
                    $option = pdo_fetch("SELECT * FROM ".tablename('tapgo_tt_goods')." WHERE id = :id AND uniacid = :uniacid",array(':id'=>$k,':uniacid'=>$_W['uniacid']));
                    if (!empty($option)){
                        $optionList[$k] = $option;
                        $optionstr = implode('-',$v);
    ...
    function group(){
        ...
        if ($exchange == '1'){
            ...
            $balance_res = intval($balance_res);
            if ($balance_res === 1) {
                pdo_update("tapgo_tt_exchange_code", array('balancestatus' => 2,), array('key' => $key, 'uniacid' => $_W['uniacid']));
                $info = m('member')->getInfo($_W['openid']);

                $record_exist = pdo_fetchcolumn("SELECT COUNT(*) FROM ".tablename('tapgo_tt_exchange_record')." WHERE `key`=:key AND uniacid = :uniacid",array(':uniacid'=>$_W['uniacid'],'key'=>$key));
                if (empty($record_exist)){
                    ...
                    pdo_update('tapgo_tt_exchange_record',$record,array('key'=>$key,'uniacid'=>$_W['uniacid']));
                }
                if (($codeResult['scorestatus'] == 2 || empty($groupResult['score_type'])) && ($codeResult['redstatus'] == 2 || empty($groupResult['red_type'])) && ($codeResult['couponstatus'] == 2 || empty($groupResult['coupon_type'])) && ($codeResult['goodsstatus'] == 2 || empty($groupResult['type']))){
                    pdo_update("tapgo_tt_exchange_code",array('status'=>2),array('key'=>$key,'uniacid'=>$_W['uniacid']));
                    pdo_query("UPDATE " . tablename('tapgo_tt_exchange_group') . " SET `use` = `use` + 1 WHERE uniacid = :uniacid AND `id` = :id", array(':uniacid' => $_W['uniacid'], ':id' => $codeResult['groupid']));
                }
            ...
            $result = m('common')->sendredpack($params);
            if (!is_error($result)) {
                pdo_update("tapgo_tt_exchange_code", array('redstatus' => 2,), array('key' => $key, 'uniacid' => $_W['uniacid']));
                $info = m('member')->getInfo($_W['openid']);
                $record_exist = pdo_fetchcolumn("SELECT COUNT(*) FROM ".tablename('tapgo_tt_exchange_record')." WHERE `key`=:key AND uniacid = :uniacid",array(':uniacid'=>$_W['uniacid'],'key'=>$key));
                if (empty($record_exist)){
                    $record = array(
            ...
            $balance_res = intval($balance_res);
            if ($balance_res === 1) {
                pdo_update("tapgo_tt_exchange_code", array('scorestatus' => 2,), array('key' => $key, 'uniacid' => $_W['uniacid']));
                $info = m('member')->getInfo($_W['openid']);
                $record_exist = pdo_fetchcolumn("SELECT COUNT(*) FROM ".tablename('tapgo_tt_exchange_record')." WHERE `key`=:key AND uniacid = :uniacid",array(':uniacid'=>$_W['uniacid'],'key'=>$key));
                if (empty($record_exist)){
                    $record = array(
            ...
            $allCoupon = pdo_fetchall('SELECT * FROM ' . tablename('tapgo_tt_coupon') . ' WHERE ' . $condition . ' and uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
            if (empty($allCoupon[0])){
                show_json(0,"没有可兑换的优惠券");
                $this->message('没有可兑换的优惠券');
            }
            ...
            $this->model->sendMessage($resp, 1, $m);
            pdo_update("tapgo_tt_exchange_code", array('couponstatus' => 2,), array('key' => $key, 'uniacid' => $_W['uniacid']));
            $info = m('member')->getInfo($_W['openid']);
            $record_exist = pdo_fetchcolumn("SELECT COUNT(*) FROM ".tablename('tapgo_tt_exchange_record')." WHERE `key`=:key AND uniacid = :uniacid",array(':uniacid'=>$_W['uniacid'],'key'=>$key));
            if (empty($record_exist)){
                $record = array(
            ...
            if ($goods_arr['goods'] != false){
                foreach ($goods_arr['goods'] as $k => $v){
                    $goodsList[$k] = pdo_fetch("SELECT * FROM ".tablename('tapgo_tt_goods')." WHERE id = :id AND uniacid = :uniacid",array(':id'=>$v,':uniacid'=>$_W['uniacid']));
                }
            }
            if ($goods_arr['option'] != false){
                foreach ($goods_arr['option'] as $k => $v){
                    $optionList[$k] = pdo_fetch("SELECT * FROM ".tablename('tapgo_tt_goods')." WHERE id = :id AND uniacid = :uniacid",array(':id'=>$k,':uniacid'=>$_W['uniacid']));
                    $optionstr = implode('-',$v);
                    $optionList[$k]['optionstr'] = $optionstr;
                }
            }
    ...
    function modal(){
        ...
        $goods = pdo_fetch("SELECT * FROM ".tablename('tapgo_tt_goods')." WHERE id = :id AND uniacid = :uniacid",array(':id'=>$id,':uniacid'=>$_W['uniacid']));
        $optionstr = trim($_GPC['option']);
        $optionid = explode('-',$optionstr);
        $count = 0;
        foreach ($optionid as $k => $v){
            $option[$k] = pdo_fetch("SELECT * FROM ".tablename('tapgo_tt_goods_option')." WHERE id = :id AND uniacid = :uniacid",array(':id'=>$v,':uniacid'=>$_W['uniacid']));
        }
    ...
    function calculate(){
        ...
        global $_GPC,$_W;
        @session_start();
        pdo_delete("tapgo_tt_exchange_cart",array('openid'=>$_W['openid'],'selected'=>1));
        $key = $_SESSION['exchange_key'];
        $exchange = pdo_fetch("SELECT * FROM ".tablename('tapgo_tt_exchange_code')." WHERE `key` = :key AND uniacid = :uniacid",array(':key'=>$key,':uniacid'=>$_W['uniacid']));
        $group = pdo_fetch("SELECT * FROM ".tablename('tapgo_tt_exchange_group')." WHERE id = :id AND uniacid = :uniacid",array(':id'=>$exchange['groupid'],':uniacid'=>$_W['uniacid']));
        ...
        foreach($form as $val){
            ...
            $goods_diy = pdo_get('tapgo_tt_goods',array('id'=>$data['goods_id']));
        ...
        foreach ($goods_tmp as $gk => $gv){
            $price = pdo_fetch("SELECT * FROM ".tablename('tapgo_tt_goods')." WHERE id = :id AND uniacid=:uniacid",array(':id'=>$gv,':uniacid'=>$_W['uniacid']));
            pdo_insert('tapgo_tt_exchange_cart',array('uniacid'=>$_W['uniacid'],'openid'=>$_W['openid'],'goodsid'=>$price['id'],'marketprice'=>$price['marketprice'],'optionid'=>0,'merchid'=>$price['merchid'],'title'=>$price['title'],'groupid'=>$exchange['groupid'],'serial'=>$exchange['serial']));
            if ($price == false){
                continue;
            }
        ...
        foreach($option_str as $k =>$v){
            ...
            foreach ($tmp as $k2 => $v2){
                $price = pdo_fetch("SELECT * FROM ".tablename('tapgo_tt_goods_option')." WHERE id = :id AND uniacid=:uniacid",array(':id'=>$v2,':uniacid'=>$_W['uniacid']));
                pdo_insert('tapgo_tt_exchange_cart',array('uniacid'=>$_W['uniacid'],'openid'=>$_W['openid'],'goodsid'=>$price['goodsid'],'marketprice'=>$price['marketprice'],'optionid'=>$price['id'],'merchid'=>$price['merchid'],'title'=>$price['title'],'groupid'=>$exchange['groupid'],'serial'=>$exchange['serial']));
    ...
    function groupexchange(){
        global $_GPC,$_W;
        $key = trim($_GPC['key']);
        $code = pdo_fetch("SELECT * FROM ".tablename('tapgo_tt_exchange_code')." WHERE `key`=:key AND uniacid = :uniacid",array(':key'=>$key,':uniacid'=>$_W['uniacid']));
        if ($code == false){
            show_json(0,'没有这个兑换码');
        }
        $group = pdo_fetch("SELECT * FROM ".tablename('tapgo_tt_exchange_group')." WHERE id =:id AND uniacid = :uniacid",array(':id'=>$code['groupid'],':uniacid'=>$_W['uniacid']));
        if ($group == false){
            show_json(0,'没有这个兑换组');
        }
    ...
    function counterror($set){
        ...
        $query = pdo_fetch("SELECT * FROM ".tablename('tapgo_tt_exchange_query')." WHERE openid = :openid AND uniacid = :uniacid",array(':uniacid'=>$_W['uniacid'], ':openid'=>$_W['openid']));

        if (empty($query)){
            pdo_insert('tapgo_tt_exchange_query',array('uniacid'=>$_W['uniacid'],'openid'=>$_W['openid'],'querytime'=>time()));
        }
        if($query['querytime']<strtotime(date('Y-m-d',time()).' 00:00:00')){
            pdo_update('tapgo_tt_exchange_query',array('errorcount'=>0,'querytime'=>time()));
        }
        if($query['unfreeze'] > time()){
            show_json(0,'请'.($query['unfreeze']-time())."秒后再试");
        }
        if (!empty($set['mistake']) && $query['errorcount']>=$set['mistake']){
            pdo_update("tapgo_tt_exchange_query",array('unfreeze'=>time()+$set['freeze']*86400),array('uniacid'=>$_W['uniacid'],'openid'=>$_W['openid']));
            pdo_update('tapgo_tt_exchange_query',array('errorcount'=>0,'unfreeze'=>time()+$set['freeze']*86400),array('uniacid'=>$_W['uniacid'],'openid'=>$_W['openid']));
