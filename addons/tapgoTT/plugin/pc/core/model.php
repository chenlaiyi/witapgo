...
class PcModel extends PluginModel
{
	...
	{
		global $_W;
		$goods = pdo_fetchall('select id,thumb,title,marketprice,productprice from ' . tablename('tapgo_tt_goods') . ' where uniacid =:uniacid and `deleted` = 0 and status=1 and total > 0 and
	...
	{
		global $_W;
		$data = pdo_getall('tapgo_tt_category', array('uniacid' => $_W['uniacid'], 'enabled' => 1), array('id', 'parentid', 'name', 'level'), '', 'displayorder desc');
	...
	{
		global $_W;
		$settings = pdo_get('tapgo_tt_pc_adv', array('uniacid' => $_W['uniacid']));
	...
	{
		global $_W;
		$menus = pdo_getall('tapgo_tt_pc_menu', array('uniacid' => $_W['uniacid'], 'status' => 1), array('link', 'type', 'title', 'displayorder'), array(), 'displayorder desc');
		$result = array();
	...
	public function getGoodsGroupSettings()
	{
		global $_W;
		$floorLimit = 9;
		$settings = pdo_getall('tapgo_tt_pc_goods', array('uniacid' => $_W['uniacid'], 'status' => 1), array(), '', 'sort desc', $floorLimit);
	...
	public function fetchGoodsByType($id, $type)
	{
		...
		case static::GOODS_CATE:
			$cateId = $id;
			$goodsIds = pdo_fetchall('select id from ' . tablename('tapgo_tt_goods') . (' where uniacid = :uniacid  AND FIND_IN_SET(' . $cateId . ', cates) '), array(':uniacid' => $_W['uniacid']));
			$goodsIds = implode(',', array_column($goodsIds, 'id'));
			$goods = $this->fetchGoods($goodsIds);
			break;

		case static::GOODS_GROUP:
			$goodsIds = pdo_fetchcolumn('select goodsids from ' . tablename('tapgo_tt_goods_group') . ' where uniacid = :uniacid and id = :id', array(':uniacid' => $_W['uniacid'], ':id' => $id));
			$goods = $this->fetchGoods($goodsIds);
			break;
		}
	...
	{
		global $_W;
		$goods = pdo_fetchall('select ' . $fields . ' from ' . tablename('tapgo_tt_goods') . (' where uniacid =:uniacid and id in (' . $ids . ') limit ' . $limit), array(':uniacid' => $_W['uniacid']));
		array_walk($goods, function(&$value) {
			$value['url'] = mobileUrl('pc.goods.detail', array('id' => $value['id']), true);
		});
		return $goods;
	...
	public function getTemplateGlobalVariables()
	{
		...
		if (!empty($userinfo)) {
			$userinfo['cartcount'] = pdo_fetchcolumn('SELECT count(*) FROM ' . tablename('tapgo_tt_member_cart') . ' WHERE openid = :openid and deleted =0', array(':openid' => $userinfo['openid']));
		}
	...
	{
		$appFilePath = TAPGO_TT_PLUGIN . '/app/core/mobile/';
		...
		include $realPath;
		$className = ucfirst($className) . '_TapgoTtPage';
	...
	public function getBreadcrumb($goodsId)
	{
		...
		$goods = pdo_get('tapgo_tt_goods', array('id' => $goodsId), array('ccate', 'tcate', 'pcate', 'title'));
		$title = $goods['title'];
		unset($goods['title']);
		$categoriesIds = array_values($goods);
		$r = pdo_fetchall('select id,`name`,`level`,`parentid` from ' . tablename('tapgo_tt_category') . ' where uniacid = :uniacid and enabled = 1 and id in(' . implode(',', $categoriesIds) . ')', array(':uniacid' => $_W['uniacid']));
	...
	public function checkOpenid()
	{
		global $_W;
		global $_GPC;
		$key = '__tapgo_ttv2_member_session_' . $_W['uniacid'];
		...
		if (isset($_GPC[$key])) {
			...
			if (is_array($session)) {
				...
				if (is_array($member) && $session['tapgo_ttv2_member_hash'] == md5($member['pwd'] . $member['salt'])) {
					$GLOBALS['_W']['tapgo_ttv2_member_hash'] = md5($member['pwd'] . $member['salt']);
					$GLOBALS['_W']['tapgo_ttv2_member'] = $member;
					return $member;
				}
	...
	public function getOrderCount()
	{
		...
		if ($merch_plugin && $merch_data['is_openmerch']) {
			$statics = array('order_0' => pdo_fetchcolumn('select count(*) from ' . tablename('tapgo_tt_order') . ' where openid=:openid and status=0 and (isparent=1 or (isparent=0 and parentid=0)) and paytype<>3 and uniacid=:uniacid and istrade=0 and userdeleted=0', $params), 'order_1' => pdo_fetchcolumn('select count(*) from ' . tablename('tapgo_tt_order') . ' where openid=:openid and (status=1 or (status=0 and paytype=3)) and isparent=0 and refundid=0 and uniacid=:uniacid and istrade=0 and userdeleted=0', $params), 'order_2' => pdo_fetchcolumn('select count(*) from ' . tablename('tapgo_tt_order') . ' where openid=:openid and (status=2 or (status=1 and sendtype>0)) and isparent=0 and refundid=0 and uniacid=:uniacid and istrade=0 and userdeleted=0', $params), 'order_4' => pdo_fetchcolumn('select count(*) from ' . tablename('tapgo_tt_order') . ' where openid=:openid and refundstate>0 and isparent=0 and uniacid=:uniacid and istrade=0 and userdeleted=0', $params), 'order_3' => pdo_fetchcolumn('select count(*) from ' . tablename('tapgo_tt_order') . ' where openid=:openid and ismr=0 and uniacid=:uniacid and status=3 and userdeleted=0', $params));
		}
		else {
			$statics = array('order_0' => pdo_fetchcolumn('select count(*) from ' . tablename('tapgo_tt_order') . ' where openid=:openid and ismr=0 and status=0 and isparent=0 and paytype<>3 and uniacid=:uniacid and istrade=0 and userdeleted=0', $params), 'order_1' => pdo_fetchcolumn('select count(*) from ' . tablename('tapgo_tt_order') . ' where openid=:openid and ismr=0 and (status=1 or (status=0 and paytype=3)) and isparent=0 and refundid=0 and uniacid=:uniacid and istrade=0 and userdeleted=0', $params), 'order_2' => pdo_fetchcolumn('select count(*) from ' . tablename('tapgo_tt_order') . ' where openid=:openid and ismr=0 and (status=2 or (status=1 and sendtype>0)) and isparent=0 and refundid=0 and uniacid=:uniacid and istrade=0 and userdeleted=0', $params), 'order_4' => pdo_fetchcolumn('select count(*) from ' . tablename('tapgo_tt_order') . ' where openid=:openid and ismr=0 and refundstate>0 and isparent=0 and uniacid=:uniacid and istrade=0 and userdeleted=0', $params), 'order_5' => pdo_fetchcolumn('select count(*) from ' . tablename('tapgo_tt_order') . ' where openid=:openid and ismr=0 and uniacid=:uniacid and iscycelbuy=1 and status in(0,1,2) and userdeleted=0', $params), 'order_3' => pdo_fetchcolumn('select count(*) from ' . tablename('tapgo_tt_order') . ' where openid=:openid and ismr=0 and uniacid=:uniacid and status=3 and userdeleted=0', $params));
		}
	...
	public function getTemplateSetting()
	{
		global $_W;
		global $_GPC;
		$data = pdo_get('tapgo_tt_pc_template', array('uniacid' => $_W['uniacid']));
	...
	public function getUserFooterMark($goodsId, $rowNumber = 3)
	{
		...
		$goodsFooterMark = pdo_fetchall('select id,title,thumb,marketprice from ' . tablename('tapgo_tt_goods') . (' where id in (' . $footerMarkGoodsIDs . ')'));
	...
	public function getOrSetUserBrowseHistoryId($userId, $goodsId, $rowNumber)
	{
		...
		pdo_update('tapgo_tt_pc_browse_history', array('history' => $history), array('id' => $data['id']));
	...
	{
		return (bool) pdo_fetchcolumn('select count(*) from ' . tablename('tapgo_tt_goods') . ' where id = :id', array(':id' => $goodsId));
	}
}
