...
class IndexController extends PluginWebPage
{
	public function main()
	{
		global $_W;
		global $_GPC;
		$info = pdo_get('tapgo_tt_pc_template', array('uniacid' => $_W['uniacid']));
		...
		if (!$info) {
			...
			$data['status'] = 1;
			pdo_insert('tapgo_tt_pc_template', $data);
		}
	...
	public function typeSetting()
	{
		...
		if ($_W['ispost']) {
			...
			pdo_update('tapgo_tt_pc_template', array('setting' => $indexsort), array('uniacid' => $_W['uniacid']));
			show_json(1);
		}

		$tempinfo = pdo_get('tapgo_tt_pc_template', array('uniacid' => $_W['uniacid']), 'setting');
	...
	public function delete()
	{
		...
		$items = pdo_fetchall('SELECT id FROM ' . tablename('tapgo_tt_pc_template') . (' WHERE id in( ' . $id . ' ) AND uniacid=') . $_W['uniacid']);
		...
		foreach ($items as $item) {
			pdo_delete('tapgo_tt_pc_template', array('id' => $item['id']));
		}
	...
	public function action()
	{
		...
		if (!empty($id)) {
			pdo_update('tapgo_tt_pc_template', array('status' => 0), array('uniacid' => $_W['uniacid']));
			pdo_update('tapgo_tt_pc_template', array('status' => 1), array('uniacid' => $_W['uniacid'], 'id' => $id));
			show_json(1, array('url' => referer()));
		}
	}
}
