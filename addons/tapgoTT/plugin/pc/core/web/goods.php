...
class GoodsController extends PluginWebPage
{
	...
	{
		global $_W;
		$info = pdo_fetchall('SELECT * from' . tablename('tapgo_tt_pc_goods') . 'where uniacid=:uniacid', array('uniacid' => $_W['uniacid']));
	...
	public function post()
	{
		...
		$groups = pdo_fetchall('SELECT * FROM ' . tablename('tapgo_tt_goods_group') . ' WHERE uniacid=:uniacid  and  enabled =1 order by id desc', array('uniacid' => $_W['uniacid']));
		...
		if ($_W['ispost']) {
			...
			if (!empty($id)) {
				pdo_update('tapgo_tt_pc_goods', $data, array('id' => $id, 'uniacid' => $_W['uniacid']));
				show_json(1, array('message' => '修改成功', 'url' => webUrl('pc/goods/edit', array('id' => $id))));
			}
			else {
				pdo_insert('tapgo_tt_pc_goods', $data);
				$id = pdo_insertid();
		...
		$item = pdo_get('tapgo_tt_pc_goods', array('id' => $id, 'uniacid' => $_W['uniacid']));

		if ($item['goods_type'] == 0) {
			$goods_info = explode(',', $item['goods_info']);
			$goods = pdo_fetchall('SELECT id,thumb,title FROM ' . tablename('tapgo_tt_goods') . ' WHERE id IN (\'' . implode('\',\'', $goods_info) . ('\') AND uniacid=' . $_W['uniacid']));
		}
	...
	public function delete()
	{
		...
		$items = pdo_fetchall('SELECT id FROM ' . tablename('tapgo_tt_pc_goods') . (' WHERE id in( ' . $id . ' ) AND uniacid=') . $_W['uniacid']);
		...
		foreach ($items as $item) {
			pdo_delete('tapgo_tt_pc_goods', array('id' => $item['id']));
		}
