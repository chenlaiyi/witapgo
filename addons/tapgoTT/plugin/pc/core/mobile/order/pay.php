...
class Pay_TapgoTtPage extends MobileLoginPage 
{
	public function main() 
	{
		...
		if (empty($log)) 
		{
			$log = array('uniacid' => $uniacid, 'openid' => $member['uid'], 'module' => 'tapgo_ttv2', 'tid' => $order['ordersn'], 'fee' => $order['price'], 'status' => 0);
	...
	{
		@session_start();
		if (!(isset($_SESSION[TAPGO_TT_PREFIX . '_order_pay_complete']))) 
		{
			header('location: ' . mobileUrl('pc.order'));
			exit();
		}
		...
		else 
		{
			...
			if ($order['isverify']) 
			{
				...
				if (empty($storeids)) 
				{
					if (0 < $merchid) 
					{
						$stores = pdo_fetchall('select * from ' . tablename('tapgo_tt_merch_store') . ' where  uniacid=:uniacid and merchid=:merchid and status=1 order by displayorder desc,id desc', array(':uniacid' => $_W['uniacid'], ':merchid' => $merchid));
					}
					else 
					{
						$stores = pdo_fetchall('select * from ' . tablename('tapgo_tt_store') . ' where  uniacid=:uniacid and status=1 order by displayorder desc,id desc', array(':uniacid' => $_W['uniacid']));
					}
				}
				else if (0 < $merchid) 
				{
	...
	public function peerpay() 
	{
		...
		$ordergoods = pdo_fetch('select * from ' . tablename('tapgo_tt_order_goods') . ' where orderid=:id and uniacid=:uniacid limit 1', array(':id' => $orderid, ':uniacid' => $uniacid));
	...
	public function peerpaydetail() 
	{
		...
		$peerid = intval($_GPC['id']);
		$uniacid = $_W['uniacid'];
		$peerpay = pdo_fetch('select p.*,o.openid,o.address,o.id as oid from ' . tablename('tapgo_tt_order_peerpay') . ' p join ' . tablename('tapgo_tt_order') . ' o on o.id=p.orderid where p.orderid=:id and p.uniacid=:uniacid limit 1', array(':id' => $peerid, ':uniacid' => $uniacid));
		if (empty($peerpay)) 
		{
			header('location: ' . mobileUrl('pc.order'));
		}
