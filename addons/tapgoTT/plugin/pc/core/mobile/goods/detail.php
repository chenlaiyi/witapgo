...
require TAPGO_TT_PLUGIN . "pc/core/page_mobile.php";
class Detail_TapgoTtPage extends PcMobilePage 
{
	public function main() 
	{
		...
		$params = pdo_fetchall('SELECT * FROM ' . tablename('tapgo_tt_goods_param') . ' WHERE uniacid=:uniacid and goodsid=:goodsid AND title<>"" order by displayorder asc', array(':uniacid' => $uniacid, ':goodsid' => $goods['id']));
		$goods = set_medias($goods, 'thumb');
		$goods['canbuy'] = !(empty($goods['status'])) && empty($goods['deleted']);
		if (!(empty($goods['hasoption']))) 
		{
			$options = pdo_fetchall('select id,stock from ' . tablename('tapgo_tt_goods_option') . ' where goodsid=:goodsid and uniacid=:uniacid order by displayorder asc', array(':goodsid' => $goods['id'], ':uniacid' => $_W['uniacid']), 'stock');
...
//				pdo_query("delete from " . tablename("tapgo_tt_goods_option") . " where goodsid=" . $id);
		...
		if (empty($this->merch_user)) 
		{
			...
			{
				$statics = array('all' => pdo_fetchcolumn('select count(1) from ' . tablename('tapgo_tt_goods') . ' where uniacid=:uniacid ' . $sqlcon . ' and status=1 and deleted=0', $param), 'new' => pdo_fetchcolumn('select count(1) from ' . tablename('tapgo_tt_goods') . ' where uniacid=:uniacid ' . $sqlcon . ' and isnew=1 and status=1 and deleted=0', $param), 'discount' => pdo_fetchcolumn('select count(1) from ' . tablename('tapgo_tt_goods') . ' where uniacid=:uniacid ' . $sqlcon . ' and isdiscount=1 and status=1 and deleted=0', $param));
			}
			else 
			{
				$goodsids = explode(',', $shop['goodsids']);
				$statics = array('all' => count($goodsids), 'new' => pdo_fetchcolumn('select count(1) from ' . tablename('tapgo_tt_goods') . ' where uniacid=:uniacid ' . $sqlcon . ' and id in( ' . $shop['goodsids'] . ' ) and isnew=1 and status=1 and deleted=0', $param), 'discount' => pdo_fetchcolumn('select count(1) from ' . tablename('tapgo_tt_goods') . ' where uniacid=:uniacid ' . $sqlcon . ' and id in( ' . $shop['goodsids'] . ' ) and isdiscount=1 and status=1 and deleted=0', $param));
			}
		}
	...
	public function get_history() 
	{
		...
		$sql = 'SELECT f.id,f.goodsid,g.title,g.thumb,g.marketprice,g.productprice,f.createtime,g.merchid FROM ' . tablename('tapgo_tt_member_history') . ' f ' . ' left join ' . tablename('tapgo_tt_goods') . ' g on f.goodsid = g.id ' . ' where 1 ' . $condition . ' ORDER BY f.`id` DESC LIMIT ' . $psize;
