...
class Cart_TapgoTtPage extends PcMobileLoginPage 
{
	...
	public function select() 
	{
		...
		if (!(empty($id))) 
		{
			$data = pdo_fetch('select id,goodsid,optionid, total from ' . tablename('tapgo_tt_member_cart') . ' ' . ' where id=:id and uniacid=:uniacid and openid=:openid limit 1 ', array(':id' => $id, ':uniacid' => $_W['uniacid'], ':openid' => $_W['openid']));
			if (!(empty($data))) 
			{
	...
	public function add() 
	{
		...
		($total <= 0) && ($total = 1);
		$optionid = intval($_GPC['optionid']);
		$goods = pdo_fetch('select id,marketprice,diyformid,diyformtype,diyfields, isverify, type,merchid from ' . tablename('tapgo_tt_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		if (empty($goods)) 
		{
			show_json(0, '商品未找到');
		}
		...
		$data = pdo_fetch('select id,total,diyformid from ' . tablename('tapgo_tt_member_cart') . ' where goodsid=:id and openid=:openid and   optionid=:optionid  and deleted=0 and  uniacid=:uniacid   limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $_W['openid'], ':optionid' => $optionid, ':id' => $id));
		if (empty($data)) 
		{
		...
		$cartcount = pdo_fetchcolumn('select sum(total) from ' . tablename('tapgo_tt_member_cart') . ' where openid=:openid and deleted=0 and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $_W['openid']));
