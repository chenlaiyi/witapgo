...
class Single_TapgoTtPage extends PluginWebPage {
    ...
    function getorder(){
        ...
        if($_W['ispost']){
            ...
            $sql = "select distinct o.* , a.realname,a.mobile,a.province,a.city,a.area, d.dispatchname,m.nickname,r.status as refundstatus from " . tablename('tapgo_tt_order') . " o"
            ...
            foreach ($list as &$value) {
                ...
                //订单商品
                $order_goods = pdo_fetchall('select g.id,g.title,g.shorttitle,g.thumb,g.goodssn,og.optionid,g.unit,og.goodssn as option_goodssn, g.productsn,og.productsn as option_productsn, g.weight, og.total,og.price,og.optionname as optiontitle, og.realprice,og.id as ordergoodid,og.printstate,og.printstate2 from ' . tablename('tapgo_tt_order_goods') . ' og '
    ...
    function getprintTemp(){
        ...
        if($_W['ispost']){
            ...
            $tempSender = pdo_fetch("SELECT * FROM " . tablename('tapgo_tt_exhelper_senduser') . " WHERE id=:id and uniacid=:uniacid and merchid=0 limit 1", array(':id'=>$printUserId, ':uniacid' => $_W['uniacid']));

            if(!empty($tempSender['province'])){
                $tempSender["sendercity"]=$tempSender['province'].$tempSender['city'].$tempSender['area'];
            }

            $expTemp = pdo_fetch("SELECT * FROM " . tablename('tapgo_tt_exhelper_express') . " WHERE id=:id and uniacid=:uniacid and merchid=0 limit 1", array(':id'=>$printTempId, ':uniacid' => $_W['uniacid']));
    ...
    function getorderinfo(){
        ...
        if($_W['ispost']){
            ...
            $printTemp = pdo_fetch("SELECT id,type,expressname,express,expresscom FROM " . tablename('tapgo_tt_exhelper_express') . " WHERE id=:id and type=:type and uniacid=:uniacid and merchid=0 limit 1", array(':id'=>$temp_express,':type'=>1,':uniacid' => $_W['uniacid']));
            ...
            $orders = pdo_fetchall("SELECT id,ordersn,address,address_send,status,paytype,expresscom,expresssn,dispatchtype FROM " . tablename('tapgo_tt_order') . " WHERE id in( $in ) and (status=1 or (paytype=3 and status=0)) and uniacid=:uniacid and merchid=0 and isparent=0 order by ordersn desc ", array(':uniacid' => $_W['uniacid']));
            if(empty($orders)){
                exit;  // 订单信息为空
            }
            $paytype = array('0' => array('css' => 'default', 'name' => '未支付'),'1' => array('css' => 'danger', 'name' => '余额支付'),'11' => array('css' => 'default', 'name' => '后台付款'),'2' => array('css' => 'danger', 'name' => '在线支付'),
