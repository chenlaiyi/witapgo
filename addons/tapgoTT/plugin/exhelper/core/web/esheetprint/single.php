...
class Single_TapgoTtPage extends PluginWebPage {
    ...
    function getdata(){
        ...
        if($_W['ispost']){
            ...
            $sql = "select o.* ,a.realname ,m.nickname, d.dispatchname,m.nickname,r.status as refundstatus from " . tablename('tapgo_tt_order') . " o"
                . " left join " . tablename('tapgo_tt_order_refund') . " r on r.orderid=o.id and ifnull(r.status,-1)<>-1 and ifnull(r.status,-1)<>-2"
                . " left join " . tablename('tapgo_tt_member') . " m on m.openid=o.openid "
    ...
    function getorder(){
        ...
        if($_W['ispost']){
            ...
            $sql = "select o.* , a.realname,a.mobile,a.province,a.city,a.area,a.street, d.dispatchname,m.nickname,r.status as refundstatus from " . tablename('tapgo_tt_order') . " o"
                . " left join " . tablename('tapgo_tt_order_refund') . " r on r.orderid=o.id and ifnull(r.status,-1)<>-1"
                . " left join " . tablename('tapgo_tt_member') . " m on m.openid=o.openid "
                . " left join " . tablename('tapgo_tt_member_address') . " a on o.addressid = a.id "
                . " left join " . tablename('tapgo_tt_dispatch') . " d on d.id = o.dispatchid "
    ...
    function getorderinfo(){
        ...
        if($_W['ispost']){
            ...
            $sql = 'SELECT temp.id,temp.esheetname,esheet.express,esheet.name as expresscom  FROM ' . tablename('tapgo_tt_exhelper_esheet_temp') . ' temp '
                . ' left join ' . tablename('tapgo_tt_exhelper_esheet') . ' esheet on temp.esheetid = esheet.id '
    ...
    function getesheetTemp(){
        ...
        if($_W['ispost']){
            ...
            //电子面单模版
            $params = array(':uniacid' => $_W['uniacid'],':id'=>$temp_esheetid);
            $sql = 'SELECT *  FROM ' . tablename('tapgo_tt_exhelper_esheet_temp') . ' temp '
                . ' left join ' . tablename('tapgo_tt_exhelper_esheet') . ' esheet on temp.esheetid = esheet.id '
                . ' where temp.id=:id and temp.uniacid=:uniacid and temp.merchid=0 limit 1';
            ...
            if($result["ResultCode"] == "100" || $result['Success'] === true) {
                ...
                if (empty($_GPC['is_group'])){
                    ...
                    foreach ($orderids as $orderid){
                        ...
                        //如果是拼团
                        $orderinfo = pdo_fetch("SELECT * FROM " . tablename('tapgo_tt_groups_order') . " WHERE id=:orderid and status>-1 and uniacid=:uniacid and merchid=0 limit 1", array(':orderid'=>$orderids[0],':uniacid' => $_W['uniacid']));
