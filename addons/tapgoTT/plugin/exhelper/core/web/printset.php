...
class Printset_TapgoTtPage extends PluginWebPage {

	function main() {
		global $_W, $_GPC;
		
		$sys = pdo_fetch('select * from ' . tablename('tapgo_tt_exhelper_sys') . ' where uniacid=:uniacid and merchid=0 limit 1 ', array(':uniacid' => $_W['uniacid']));
		...
		if($_W['ispost']){
			...
			if(!empty($data)){
                ...
                if (empty($sys)) {
                    pdo_insert('tapgo_tt_exhelper_sys', $data);
                } else {
                    pdo_update('tapgo_tt_exhelper_sys', $data);
                    pdo_update('tapgo_tt_exhelper_sys', $data, array('uniacid' => $_W['uniacid'], 'merchid' => 0));
                }
 ...
 /*function test(){
        ...
        pdo_delete('tapgo_tt_exhelper_esheet');
        pdo_query('ALTER TABLE' . tablename('tapgo_tt_exhelper_esheet') . "AUTO_INCREMENT=1");
        $data = array();
        ...
        foreach ($data as $item) {
            pdo_insert('tapgo_tt_exhelper_esheet', $item);
        }
