...
class Goods_TapgoTtPage extends PluginWebPage {
	...
	function main() {
		...
		$sql = 'SELECT * FROM ' . tablename('tapgo_tt_creditshop_goods') . " where  1 and {$condition} ORDER BY displayorder DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize;
		$list = pdo_fetchall($sql, $params);
		$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('tapgo_tt_creditshop_goods') . " where 1 and {$condition}", $params);
		if($merch_plugin) {
		...
		$category = pdo_fetchall("select id,`name`,thumb from " . tablename('tapgo_tt_creditshop_category') . ' where uniacid=:uniacid order by displayorder desc', array(':uniacid' => $_W['uniacid']), 'id');
	...
	protected function post() {
		...
		$item = pdo_fetch("SELECT * FROM " . tablename('tapgo_tt_creditshop_goods') . " WHERE id =:id and uniacid=:uniacid limit 1", array(':uniacid' => $_W['uniacid'], ':id' => $id));
		...
		$category = pdo_fetchall("select id,`name`,thumb from " . tablename('tapgo_tt_creditshop_category') . ' where uniacid=:uniacid order by displayorder desc', array(':uniacid' => $_W['uniacid']));

		if(empty($item['goodstype']) || $item['goodstype']==0){
			$goods = pdo_fetch("SELECT * FROM " . tablename('tapgo_tt_goods') . " WHERE id =:id and status=1 and deleted=0 and uniacid=:uniacid limit 1", array(':uniacid' => $_W['uniacid'], ':id' => $item['goodsid']));
		}elseif($item['goodstype']==1){
			$coupon = pdo_fetch("SELECT * FROM " . tablename('tapgo_tt_coupon') . " WHERE id =:id and uniacid=:uniacid limit 1", array(':uniacid' => $_W['uniacid'], ':id' => $item['couponid']));
		}
		...
		$allspecs = pdo_fetchall("select * from " . tablename('tapgo_tt_creditshop_spec') . " where goodsid=:id order by displayorder asc", array(":id" => $id));
		foreach ($allspecs as &$s) {
			$s['items'] = pdo_fetchall("select a.id,a.specid,a.title,a.thumb,a.show,a.displayorder,a.valueId,a.virtual,b.title as title2 from " . tablename('tapgo_tt_creditshop_spec_item') . " a left join " . tablename('tapgo_tt_virtual_type') . " b on b.id=a.virtual  where a.specid=:specid order by a.displayorder asc", array(":specid" => $s['id']));
		}
		unset($s);
		...
		$options = pdo_fetchall("select * from " . tablename('tapgo_tt_creditshop_option') . " where goodsid=:id order by id asc", array(':id' => $id));
		//排序好的specs
		$specs = array();
		//找出数据库存储的排列顺序
		if (count($options) > 0) {
...
<a href="javascript:;" class="fa fa-angle-double-down" title="批量设置" onclick="setCol(\'option_money\');"></a></span></div></div></th>';
		...
		if ($_W['ispost']) {
			...
			if (!empty($id)) {
				...
				pdo_update('tapgo_tt_creditshop_goods', $data, array('id' => $id, 'uniacid' => $_W['uniacid']));
				...
				pdo_insert('tapgo_tt_creditshop_goods', $data);
				$id = pdo_insertid();
			...
			if($data['hasoption']){
				...
				for ($k = 0; $k < $len; $k++) {
					...
					if (is_numeric($get_spec_id)) {
						pdo_update("tapgo_tt_creditshop_spec", $a, array("id" => $get_spec_id));
						$spec_id = $get_spec_id;
					} else {
						pdo_insert("tapgo_tt_creditshop_spec", $a);
					...
					for ($n = 0; $n < $itemlen; $n++) {
						...
						if (is_numeric($get_item_id)) {
							pdo_update("tapgo_tt_creditshop_spec_item", $d, array("id" => $get_item_id));
							$item_id = $get_item_id;
						} else {
							pdo_insert("tapgo_tt_creditshop_spec_item", $d);
					...
					//删除其他的
					if (count($itemids) > 0) {
						pdo_query("delete from " . tablename('tapgo_tt_creditshop_spec_item') . " where uniacid={$_W['uniacid']} and specid=$spec_id and id not in (" . implode(",", $itemids) . ")");
					} else {
						pdo_query("delete from " . tablename('tapgo_tt_creditshop_spec_item') . " where uniacid={$_W['uniacid']} and specid=$spec_id");
					}
					//更新规格项id
					pdo_update("tapgo_tt_creditshop_spec", array("content" => serialize($itemids)), array("id" => $spec_id));
					$specids[] = $spec_id;
				}
				//删除其他的
				if (count($specids) > 0) {
					pdo_query("delete from " . tablename('tapgo_tt_creditshop_spec') . " where uniacid={$_W['uniacid']} and goodsid=$id and id not in (" . implode(",", $specids) . ")");
				} else {
					pdo_query("delete from " . tablename('tapgo_tt_creditshop_spec') . " where uniacid={$_W['uniacid']} and goodsid=$id");
				}
				//保存规格
				...
				for ($k = 0; $k < $len; $k++) {
					...
					if (empty($get_option_id)) {
						pdo_insert("tapgo_tt_creditshop_option", $a);
						$option_id = pdo_insertid();
					} else {
						pdo_update("tapgo_tt_creditshop_option", $a, array('id' => $get_option_id));
				...
				if (count($optionids) > 0 && $data['hasoption'] !== 0) {
					pdo_query("delete from " . tablename('tapgo_tt_creditshop_option') . " where goodsid=$id and id not in ( " . implode(',', $optionids) . ")");

					//更新最低价和最高价
					$sql = "update ".tablename('tapgo_tt_creditshop_goods')." g set
					g.mincredit = (select min(credit) from ".tablename('tapgo_tt_creditshop_option')." where goodsid = $id),
					g.minmoney = (select min(money) from ".tablename('tapgo_tt_creditshop_option')." where goodsid = $id),
					g.maxcredit = (select max(credit) from ".tablename('tapgo_tt_creditshop_option')." where goodsid = $id),
					g.maxmoney = (select max(money) from ".tablename('tapgo_tt_creditshop_option')." where goodsid = $id)
					where g.id = $id and g.hasoption=1";

					pdo_query($sql);
				} else {
					pdo_query("delete from " . tablename('tapgo_tt_creditshop_option') . " where goodsid=$id");
					$sql = "update ".tablename('tapgo_tt_creditshop_goods')." set minmoney = money,maxmoney = money,mincredit = credit,maxcredit = credit where id = $id and hasoption=0;";
					pdo_query($sql);
				}
				//总库存
				if ($data['hasoption'] !== 0) {
					pdo_update("tapgo_tt_creditshop_goods", array("total" => $totalstocks), array("id" => $id));
				}
			}
	...
	function delete() {
		...
		$items = pdo_fetchall("SELECT id,title FROM " . tablename('tapgo_tt_creditshop_goods') . " WHERE id in( $id ) AND uniacid=" . $_W['uniacid']);
		foreach ($items as $item) {
			pdo_update('tapgo_tt_creditshop_goods', array('deleted' => 1), array('id' =>$item['id'], 'uniacid' => $_W['uniacid']));
    ...
    function status() {
        ...
        $status = intval($_GPC['status']);
        $items = pdo_fetchall("SELECT id,status FROM " . tablename('tapgo_tt_creditshop_goods') . " WHERE id in( $id ) AND uniacid=" . $_W['uniacid']);
        foreach ($items as $item) {
            $status_update = pdo_update('tapgo_tt_creditshop_goods', array('status' => $status), array('id' => $item['id']));
            if(!$status_update){
    ...
    function deleted() {
        ...
        foreach ($items as $item) {
            pdo_delete('tapgo_tt_creditshop_goods', array('id' => $item['id']));
    ...
    function recycle() {
        ...
        foreach ($items as $item) {
            pdo_update('tapgo_tt_creditshop_goods', array('status' => 0,'deleted' => 0), array('id' => $item['id']));
	...
	function property() {
		...
		if (in_array($type, array('istop', 'isrecommand', 'status', 'displayorder','title'))) {
			...
			pdo_update("tapgo_tt_creditshop_goods", array($type => $value), array("id" => $id, "uniacid" => $_W['uniacid']));
			$statusstr = "";
			if ($type == 'istop') {
				$typestr = "置顶";
    ...
    /*function query(){
        ...
        $list = pdo_fetchall('SELECT *, money as minprice FROM ' . tablename('tapgo_tt_creditshop_goods') . " WHERE 1 {$condition} order by createtime desc", $params);
    ...
    function query(){
        ...
        $list = array();
        $list = pdo_fetchall('SELECT id,title,thumb,marketprice,productprice,share_title,share_icon,description,minprice,costprice,total,content,hasoption FROM ' . tablename('tapgo_tt_goods') . " WHERE 1 {$condition} order by createtime desc LIMIT ". ($pindex - 1) * $psize . ',' . $psize, $params);

        $list = set_medias($list,array('thumb','share_icon'));
        foreach ($list as $key => $value){
            if(intval($value['hasoption'])==1) {

                $allspecs = pdo_fetchall("select * from " . tablename('tapgo_tt_goods_spec') . " where goodsid=:id order by displayorder asc", array(":id" => $value['id']));
                foreach ($allspecs as &$s) {
                ...
                unset($s);
                $options = pdo_fetchall("select * from " . tablename('tapgo_tt_goods_option') . " where goodsid=:id order by id asc", array(':id' => $value['id']));
        ...
        $total = pdo_fetchcolumn('SELECT count(1) FROM ' . tablename('tapgo_tt_goods') . " WHERE 1 {$condition} ", $params);
