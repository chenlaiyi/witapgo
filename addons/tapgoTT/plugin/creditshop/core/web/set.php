...
class Set_TapgoTtPage extends PluginWebPage
{
	public function main()
	{
		...
		if ($_W['ispost']) {
			...
			if (!empty($keyword)) {
				if ($keyword['name'] != 'tapgo_ttv2:creditshop') {
					show_json(0, '关键字已存在!');
				}
			}

			$rule = pdo_fetch('select * from ' . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name  limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'tapgo_ttv2', ':name' => 'tapgo_ttv2:creditshop'));

			if (empty($rule)) {
				$rule_data = array('uniacid' => $_W['uniacid'], 'name' => 'tapgo_ttv2:creditshop', 'module' => 'tapgo_ttv2', 'displayorder' => 0, 'status' => 1);
