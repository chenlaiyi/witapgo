...
class Index_TapgoTtPage extends PluginMobilePage {
	...
	function getcontent(){
		...
		$article = pdo_fetch("SELECT article_content FROM " . tablename('tapgo_tt_article') . " WHERE id=:aid and article_state=1 and uniacid=:uniacid limit 1 ", array(':aid' => $aid, ':uniacid' => $_W['uniacid']));
		if(empty($article)){
			show_json(0, '文章不存在');
		}
