...
class Notify_TapgoTtPage extends PluginWebPage
{
    public function main()
    {
        ...
        $tmsg_list = pdo_fetchall("SELECT id, `name` FROM " . tablename("tapgo_tt_wxapp_tmessage") . "WHERE uniacid=:uniacid AND status=1", array(":uniacid" => $_W["uniacid"]));
    ...
    public function setTemplate()
    {
        ...
        if (strpos($tag, "wxapp_") !== false) {
            ...
            $templateId = pdo_fetchcolumn("select templateid from" . tablename("tapgo_tt_member_wxapp_message_template_default") . " where uniacid = :uniacid and typecode = :typecode", array(":uniacid" => $_W["uniacid"], ":typecode" => $typecode));
            ...
            if (!$templateId || !in_array($templateId, $templateIds)) {
                $defaultTemplate = pdo_fetch("select * from " . tablename("tapgo_tt_member_wxapp_message_template_type") . " where typecode = :typecode", array(":typecode" => $typecode));
                if (25 <= $templatenum) {
                ...
                if ($ret["errcode"] == 0) {
                    if (!$templateId) {
                        pdo_insert("tapgo_tt_member_wxapp_message_template_default", array("typecode" => $typecode, "uniacid" => $_W["uniacid"], "templateid" => $ret["template_id"], "datas" => "a:3:{i:0;a:2:{s:3:\"key\";s:17:\"{{keyword1.DATA}}\";s:5:\"value\";s:27:\"瓜分券活动完成通知\";}i:1;a:2:{s:3:\"key\";s:17:\"{{keyword2.DATA}}\";s:5:\"value\";s:26:\"[活动名称]活动完成\";}i:2;a:2:{s:3:\"key\";s:17:\"{{keyword3.DATA}}\";s:5:\"value\";s:52:\"您于[瓜分券领取时间]领取[瓜分券名称]\";}}"));
                    } else {
                        pdo_update("tapgo_tt_member_wxapp_message_template_default", array("templateid" => $ret["template_id"]), array("uniacid" => $_W["uniacid"], "typecode" => $typecode));
                    }
                }
            }
