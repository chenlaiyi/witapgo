...
class Index_TapgoTtPage extends PluginMobilePage
{
	public function main()
	{
		...
		if (!$this->model->validateActivity($_GPC['id'])) {
			if ($this->model->errno) {
				if ($isShare) {
					$takePartInUsers = pdo_fetchall('select openid from ' . tablename('tapgo_tt_friendcoupon_data') . (' where uniacid = ' . $_W['uniacid'] . ' and headerid = ' . $mid));
				}
				else {
					$takePartInUsers = pdo_fetchall('select openid from' . tablename('tapgo_tt_friendcoupon_data') . (' where uniacid = ' . $_W['uniacid'] . ' and activity_id = ' . $activity_id));
				}
...
', $activity['desc']);
		...
		if ($success) {
			$coupon = pdo_fetch('select id,couponid,openid,uniacid,used from ' . tablename('tapgo_tt_coupon_data') . ' where friendcouponid = :friendcouponid', array('friendcouponid' => $currentActivityInfo['id']));
		}
		...
		$_W['shopshare'] = array('title' => $activitySetting['title'], 'imgUrl' => $_W['siteroot'] . 'addons/tapgo_ttv2/static/images/friendcoupon.png', 'desc' => $share_user['nickname'] . '邀请您一起瓜分' . $activitySetting['coupon_money'] . '元', 'link' => mobileUrl('friendcoupon', $shareParams, true));
	...
	public function receive()
	{
		...
		if (!$couponActivities) {
			...
			foreach ($couponAmounts as $couponAmount) {
				...
				pdo_insert('tapgo_tt_friendcoupon_data', $data);
		...
		pdo_update('tapgo_tt_friendcoupon_data', array('openid' => $user['openid'], 'avatar' => $user['avatar'], 'nickname' => $user['nickname'], 'receive_time' => time()), array('id' => $headerid));
		pdo_update('tapgo_tt_friendcoupon', array('launches_count +=' => 1), array('id' => $activity['id']));
	...
	public function divide()
	{
		...
		if (!$currentActivityInfo) {
			$coupons = pdo_fetchall('select * from ' . tablename('tapgo_tt_friendcoupon_data') . ' where uniacid = :uniacid and activity_id = :activity_id and headerid = :headerid', array(':uniacid' => $_W['uniacid'], ':activity_id' => $activity['id'], ':headerid' => $mid));

			foreach ($coupons as $coupon) {
				if (!$coupon['openid']) {
					pdo_update('tapgo_tt_friendcoupon_data', array('openid' => $user['openid'], 'status' => 0, 'avatar' => $user['avatar'], 'nickname' => $user['nickname'], 'receive_time' => time()), array('id' => $coupon['id']));
					break;
				}
			}

			$overPlus = pdo_fetchcolumn('select count(1) from ' . tablename('tapgo_tt_friendcoupon_data') . ' where uniacid = :uniacid and headerid = :headerid and activity_id = :activity_id and openid = \'\'', array(':uniacid' => $_W['uniacid'], ':headerid' => $mid, ':activity_id' => $activity_id));

			if ($overPlus) {
				show_json(1, '瓜分成功!');
			}

			pdo_update('tapgo_tt_friendcoupon_data', array('status' => 1), array('activity_id' => $activity_id, 'headerid' => $mid));
			$sql = 'select f.*, fd.enough, fd.id as friendcouponid,fd.deduct,fd.openid,fd.activity_id,fd.receive_time,fd.deadline from' . tablename('tapgo_tt_friendcoupon_data') . ' fd ' . ' left join ' . tablename('tapgo_tt_friendcoupon') . ' f on fd.activity_id = f.id' . ' where fd.uniacid = :uniacid and fd.headerid = :headerid and fd.activity_id = :activity_id';
	...
	public function getTakePartInPeopleData($activity_id, $headerid)
	{
		...
		$headerid = $_GPC['mid'];
		$r = pdo_fetchall('select * from ' . tablename('tapgo_tt_friendcoupon_data') . ' where unaicid = :unaicid and headerid = :headerid and activity_id = :activity_id', array(':uniacid' => $_W['uniacid'], ':headerid' => $headerid, ':activity_id' => $activity_id));
	}
	...
	public function more()
	{
		...
		if (empty($mid)) {
			$currentTakePartActivity = pdo_fetch('select * from ' . tablename('tapgo_tt_friendcoupon_data') . ' where uniacid = :uniacid and openid = :openid and activity_id = :activity_id', array(':uniacid' => $_W['uniacid'], ':openid' => $_W['openid'], ':activity_id' => $activity_id));
			$mid = $currentTakePartActivity['headerid'];
		}

		$list = pdo_fetchall('select avatar,nickname,deduct from ' . tablename('tapgo_tt_friendcoupon_data') . ' where uniacid = :uniacid and activity_id = :activity_id and headerid = :headerid and openid <> \'\' limit ' . ($pindex - 1) . ',' . $psize, array(':uniacid' => $_W['uniacid'], ':headerid' => $mid, ':activity_id' => $activity_id));
