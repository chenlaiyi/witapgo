...
if ($_W['ispost']) {
	...
	if (empty($id)) {
		...
		$old_data = pdo_fetch('SELECT nocommission FROM ' . tablename('tapgo_tt_goods') . ' WHERE id=:id AND uniacid=:uniacid', array(':id' => $id, ':uniacid' => $_W['uniacid']));
...
if ($merchid == 0) {
	$details = pdo_fetchall('select detail_logo,detail_shopname,detail_btntext1, detail_btnurl1 ,detail_btntext2,detail_btnurl2,detail_totaltitle from ' . tablename('tapgo_tt_goods') . ' where uniacid=:uniacid and detail_shopname<>\'\' group by detail_shopname', array(':uniacid' => $_W['uniacid']));
