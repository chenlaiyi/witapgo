...
class Index_TapgoTtPage extends PluginWebPage
{
	public function main($goodsfrom = 'sale')
	{
		...
		$sql = 'SELECT g.id FROM ' . tablename('tapgo_tt_goods') . 'g' . $sqlcondition . $condition . $groupcondition;
		$total_all = pdo_fetchall($sql, $params);
		$total = count($total_all);
		unset($total_all);

		if (!empty($total)) {
			$sql = 'SELECT g.* FROM ' . tablename('tapgo_tt_goods') . 'g' . $sqlcondition . $condition . $groupcondition . ' ORDER BY g.`status` DESC, g.`displayorder` DESC,
	...
	public function change()
	{
		...
		$goods = pdo_fetch('select id,hasoption,marketprice,dowpayment from ' . tablename('tapgo_tt_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $id));
		...
		pdo_update('tapgo_tt_goods', array($type => $value), array('id' => $id));
