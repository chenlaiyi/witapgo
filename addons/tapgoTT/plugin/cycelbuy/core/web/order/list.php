...
class List_TapgoTtPage extends WebPage
{
    protected function orderData($status, $st)
    {
                                            ...
                                            else
                                            {
                                                ...
                                                else
                                                {
                                                    ...
                                                    else
                                                    {
                                                        ...
                                                        {
                                                            $sqlcondition = " inner join ( select  DISTINCT(og.orderid) from " . tablename("tapgo_tt_order_goods") . " og left join " . tablename("tapgo_tt_goods") . " g on g.id=og.goodsid where og.uniacid = '" . $uniacid . "' and (locate(:keyword,og.optionname)>0)) gs on gs.orderid=o.id";
                                                        }
                                                        else
                                                        {
        ...
        else
        {
            ...
            $sql = "select * from " . tablename("tapgo_tt_order") . " where uniacid = :uniacid and iscycelbuy = 1 and ismr=0 and deleted=0 and isparent=0 " . $status_condition . " GROUP BY id ORDER BY createtime DESC  ";
            if( empty($_GPC["export"]) ) 
            {
                $sql .= "LIMIT " . ($pindex - 1) * $psize . "," . $psize;
            }
        ...
        if( $condition != " o.uniacid = :uniacid and o.ismr=0 and o.deleted=0 and o.isparent=0" || !empty($sqlcondition) ) 
        {
            if( $searchfield == "member" ) 
            {
                $openidArr = pdo_fetchall("SELECT openid FROM " . tablename("tapgo_tt_member") . " WHERE uniacid = " . $_W["uniacid"] . $priceCondition);
                if( !empty($openidArr) ) 
                {
                    ...
                    $orderPrice = pdo_fetch("SELECT count(1) as count,sum(price) as sumprice FROM " . tablename("tapgo_tt_order") . " o WHERE o.uniacid = " . $_W["uniacid"] . " AND o.deleted=0 AND o.isparent=0 AND o.openid IN (" . $inOpenid . ")" . $priceStatus);
                }
                else
                {
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        $t = pdo_fetch("SELECT COUNT(DISTINCT(o.id)) as count, ifnull(sum(o.price),0) as sumprice   FROM " . tablename("tapgo_tt_order") . " o " . " left join " . tablename("tapgo_tt_order_refund") . " r on r.id =o.refundid " . " left join " . tablename("tapgo_tt_saler") . " s on s.openid = o.verifyopenid and s.uniacid=o.uniacid" . " left join " . tablename("tapgo_tt_member") . " sm on sm.openid = s.openid and sm.uniacid=s.uniacid" . " " . $sqlcondition . " WHERE " . $condition . " " . $statuscondition, $paras);
                        $total = $t["count"];
        ...
        {
            $t = pdo_fetch("SELECT COUNT(*) as count, ifnull(sum(price),0) as sumprice   FROM " . tablename("tapgo_tt_order") . " WHERE uniacid = :uniacid and ismr=0 and deleted=0 and isparent=0 and iscycelbuy = 1 " . $status_condition, $paras);
            $total = $t["count"];
            $totalmoney = $t["sumprice"];
        }
