...
class Detail_TapgoTtPage extends PluginWebPage
{
    public function main()
    {
        ...
        if( 0 < $item["sendtype"] ) 
        {
            ...
            {
                $order_goods[$key]["goods"] = pdo_fetchall("select g.id,g.title,g.thumb,og.sendtype,g.ispresell,og.realprice from " . tablename("tapgo_tt_order_goods") . " og " . " left join " . tablename("tapgo_tt_goods") . " g on g.id=og.goodsid " . " where og.uniacid=:uniacid and og.orderid=:orderid and og.sendtype=" . $value["sendtype"] . " ", array( ":uniacid" => $_W["uniacid"], ":orderid" => $id ));
            }
        ...
        $notArray = array(  );
        $periods = pdo_getall("tapgo_tt_cycelbuy_periods", array( "orderid" => $id ), array(  ), "", "id");
    ...
    public function option_title($orderid)
    {
        ...
        $sql = "select o.specs from " . tablename("tapgo_tt_order_goods") . " g join " . tablename("tapgo_tt_goods_option") . " o on o.id = g.optionid where g.orderid = :orderid";
