                    ...
                    else
                    {
                        ...
                        else
                        {
                            if( $refundstatus == 1 ) 
                            {
                                ...
                                $goods = pdo_fetchall("SELECT g.id,g.credit, o.total,o.realprice,g.isfullback FROM " . tablename("tapgo_tt_order_goods") . " o left join " . tablename("tapgo_tt_goods") . " g on o.goodsid=g.id " . " WHERE o.orderid=:orderid and o.uniacid=:uniacid", array( ":orderid" => $item["id"], ":uniacid" => $uniacid ));
                                $refundtype = 0;
                                ...
                                foreach( $goods as $g ) 
                                {
                                    $salesreal = pdo_fetchcolumn("select ifnull(sum(total),0) from " . tablename("tapgo_tt_order_goods") . " og " . " left join " . tablename("tapgo_tt_order") . " o on o.id = og.orderid " . " where og.goodsid=:goodsid and o.status>=1 and o.uniacid=:uniacid limit 1", array( ":goodsid" => $g["id"], ":uniacid" => $uniacid ));
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    if( $refundstatus == 2 ) 
                                    {
                                        ...
                                        pdo_update("tapgo_tt_order", array( "refundstate" => 0, "status" => -1, "refundtime" => $time ), array( "id" => $item["id"], "uniacid" => $uniacid ));
                                        $goods = pdo_fetchall("SELECT g.id,g.credit, o.total,o.realprice FROM " . tablename("tapgo_tt_order_goods") . " o left join " . tablename("tapgo_tt_goods") . " g on o.goodsid=g.id " . " WHERE o.orderid=:orderid and o.uniacid=:uniacid", array( ":orderid" => $item["id"], ":uniacid" => $uniacid ));
                                        $credits = m("order")->getGoodsCredit($goods);
