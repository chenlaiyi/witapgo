...
class Log_TapgoTtPage extends PluginWebPage
{
    ...
    protected function getData($type)
    {
        ...
        unset($row);
        $total = pdo_fetchcolumn("select count(log.id) from" . tablename("tapgo_tt_creditshop_log") . " log " . " left join " . tablename("tapgo_tt_member") . " m on m.openid = log.openid and m.uniacid=log.uniacid" . " left join " . tablename("tapgo_tt_store") . " s on s.id = log.storeid" . " left join " . tablename("tapgo_tt_member_address") . " a on a.id = log.addressid" . " left join " . tablename("tapgo_tt_creditshop_goods") . " g on g.id = log.goodsid" . " where 1 " . $condition, $params);
        foreach( $list as &$row ) 
        {
            $row["address"] = array(  );
            if( !empty($row["addressid"]) ) 
            {
                $row["address"] = pdo_fetch("select realname,mobile,address,province,city,area from " . tablename("tapgo_tt_member_address") . " where id=:id and uniacid=:uniacid limit 1", array( ":id" => $row["addressid"], ":uniacid" => $_W["uniacid"] ));
            }
            else
            {
    ...
    public function detail()
    {
        ...
        else
        {
            ...
            if( empty($address) ) 
            {
                $address = pdo_fetch("select realname,mobile,address,province,city,area from " . tablename("tapgo_tt_member_address") . " where id=:id and uniacid=:uniacid limit 1", array( ":id" => $log["addressid"], ":uniacid" => $_W["uniacid"] ));
            }
    ...
    public function doexchange()
    {
        ...
        if( $_W["ispost"] ) 
        {
            if( $goods["goodstype"] == 0 ) 
            {
                ...
                {
                    pdo_update("tapgo_tt_creditshop_log", array( "status" => 3, "usetime" => time(), "time_send" => time(), "expresscom" => $_GPC["expresscom"], "expresssn" => $_GPC["expresssn"], "express" => $_GPC["express"] ), array( "id" => $id ));
                }
