...
class Comment_TapgoTtPage extends PluginMobileLoginPage
{
    ...
    public function submit()
    {
        ...
        $log = pdo_fetch("select id,status,iscomment,logno from " . tablename("tapgo_tt_creditshop_log") . " where id=:id " . $condition . " and openid=:openid limit 1", array( ":id" => $logid, ":uniacid" => $uniacid, ":openid" => $openid ));
        if( empty($log) ) 
        {
            show_json(0, "兑换记录未找到");
        }
