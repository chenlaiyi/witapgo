...
class Detail_TapgoTtPage extends PluginMobilePage
{
    public function main()
    {
        ...
        if( p("task") ) 
        {
            ...
            $taskGoodsInfo = pdo_fetch("select * from " . tablename("tapgo_tt_task_reward") . " where id = :id and openid = :openid and senttime = 0 and gettime > 0", array( ":id" => $taskrewardgoodsid, ":openid" => $_W["openid"] ));
        ...
        foreach( $gifts as $key => &$value ) 
        {
            ...
            foreach( $gid as $ke => $val ) 
            {
                if( $val == $id ) 
                {
                    ...
                    {
                        $isgift = 1;
                        $gifts[$key]["gift"][$k] = pdo_fetch("select id,title,thumb,marketprice from " . tablename("tapgo_tt_goods") . " where uniacid = " . $uniacid . " and deleted = 0 and total > 0 and status = 2 and id = " . $val . " ");
        ...
        {
            $address_list = pdo_fetchall("select id,datavalue from " . tablename("tapgo_tt_member_address") . " where openid=:openid and uniacid=:uniacid", array( ":uniacid" => $uniacid, ":openid" => $openid ));
        ...
        {
            $order_goodscount = pdo_fetchcolumn("select ifnull(sum(og.total),0)  from " . tablename("tapgo_tt_order_goods") . " og " . " left join " . tablename("tapgo_tt_order") . " o on og.orderid=o.id " . " where og.goodsid=:goodsid and  o.status>=1 and o.openid=:openid  and og.uniacid=:uniacid ", array( ":goodsid" => $goods["id"], ":uniacid" => $uniacid, ":openid" => $openid ));
        ...
        {
            $statics = array( "all" => pdo_fetchcolumn("select count(1) from " . tablename("tapgo_tt_goods") . " where uniacid=:uniacid and merchid=:merchid and status=1 and deleted=0", array( ":uniacid" => $_W["uniacid"], ":merchid" => $goods["merchid"] )), "new" => pdo_fetchcolumn("select count(1) from " . tablename("tapgo_tt_goods") . " where uniacid=:uniacid and merchid=:merchid and isnew=1 and status=1 and deleted=0", array( ":uniacid" => $_W["uniacid"], ":merchid" => $goods["merchid"] )), "discount" => pdo_fetchcolumn("select count(1) from " . tablename("tapgo_tt_goods") . " where uniacid=:uniacid and merchid=:merchid and isdiscount=1 and status=1 and deleted=0", array( ":uniacid" => $_W["uniacid"], ":merchid" => $goods["merchid"] )) );
        }
        else
        {
            $goodsids = explode(",", $shop["goodsids"]);
            $statics = array( "all" => count($goodsids), "new" => pdo_fetchcolumn("select count(1) from " . tablename("tapgo_tt_goods") . " where uniacid=:uniacid and merchid=:merchid and id in( " . $shop["goodsids"] . " ) and isnew=1 and status=1 and deleted=0", array( ":uniacid" => $_W["uniacid"], ":merchid" => $goods["merchid"] )), "discount" => pdo_fetchcolumn("select count(1) from " . tablename("tapgo_tt_goods") . " where uniacid=:uniacid and merchid=:merchid and id in( " . $shop["goodsids"] . " ) and isdiscount=1 and status=1 and deleted=0", array( ":uniacid" => $_W["uniacid"], ":merchid" => $goods["merchid"] )) );
        }
    ...
    public function querygift()
    {
        ...
        $uniacid = $_W["uniacid"];
        $giftid = $_GPC["id"];
        $gift = pdo_fetch("select * from " . tablename("tapgo_tt_gift") . " where uniacid = " . $uniacid . " and status = 1 and id = " . $giftid . " ");
        show_json(1, $gift);
    }
    ...
    public function get_comment_list()
    {
        ...
        $list = pdo_fetchall("select * from " . tablename("tapgo_tt_order_comment") . " " . "  where goodsid=:goodsid and uniacid=:uniacid and deleted=0 and checked=0 " . $condition . " order by istop desc, createtime desc, id desc LIMIT " . ($pindex - 1) * $psize . "," . $psize, $params);
        foreach( $list as &$row ) 
        {
