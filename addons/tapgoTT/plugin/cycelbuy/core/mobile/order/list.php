...
class List_TapgoTtPage extends PluginMobilePage
{
    ...
    public function address()
    {
        ...
        if( !empty($orderid) ) 
        {
            $address = pdo_fetch("select * from " . tablename("tapgo_tt_cycelbuy_periods") . " where orderid=:orderid and status = 0 order by receipttime asc limit 1", array( ":orderid" => $orderid ));
            $address = iunserializer($address["address"]);
        }

        if( !empty($applyforid) ) 
        {
            $address = pdo_get("tapgo_tt_address_applyfor", array( "id" => $applyforid, "isdelete" => 0 ));
    ...
    public function submit()
    {
        ...
        $orderid = intval($_GPC["orderid"]);
        $order = pdo_fetch("SELECT openid,ordersn FROM " . tablename("tapgo_tt_order") . " WHERE id=:id AND uniacid=:uniacid", array( ":id" => $orderid, ":uniacid" => $_W["uniacid"] ));
        if( empty($order) ) 
        {
            show_json(0, "订单未找到");
        }
    ...
    public function applyfor()
    {
        ...
        $data = pdo_get("tapgo_tt_address_applyfor", array( "id" => $apply_id, "isdelete" => 0 ));
        if( empty($data) ) 
        {
    ...
    public function cancelApply()
    {
        ...
        global $_GPC;
        $id = $_GPC["applyid"];
        $result = pdo_update("tapgo_tt_address_applyfor", array( "isdelete" => 1 ), array( "id" => $id ));
        if( $result ) 
        {
            show_json(1);
        }
