...
class Index_TapgoTtPage extends PluginMobilePage
{
    ...
    public function detail()
    {
        ...
        if( $order["refundid"] != 0 ) 
        {
            $refund = pdo_fetch("SELECT *  FROM " . tablename("tapgo_tt_order_refund") . " WHERE orderid = :orderid and uniacid=:uniacid order by id desc", array( ":orderid" => $order["id"], ":uniacid" => $_W["uniacid"] ));
        }
        ...
        $goodsid_array = array(  );
        $goods = pdo_fetchall("select og.goodsid,og.price,g.title,g.thumb,g.status, g.cannotrefund, og.total,g.credit,og.optionid,\r\n            og.optionname as optiontitle,g.isverify,g.storeids,og.seckill,g.isfullback,\r\n            og.seckill_taskid" . $diyformfields . $condition1 . ",og.prohibitrefund  from " . tablename("tapgo_tt_order_goods") . " og " . " left join " . tablename("tapgo_tt_goods") . " g on g.id=og.goodsid " . " where " . $scondition . " and og.uniacid=:uniacid ", $param);
    ...
    public function express()
    {
        ...
        $order = pdo_fetch("select * from " . tablename("tapgo_tt_cycelbuy_periods") . " where id=:id  and orderid = :orderid and uniacid=:uniacid limit 1", array( ":id" => $id, ":orderid" => $orderid, ":uniacid" => $uniacid ));
        if( empty($order) ) 
        {
            header("location: " . mobileUrl("cycelbuy/order"));
            exit();
    ...
    public function cycledetail()
    {
        ...
        $sql = "SELECT * FROM " . tablename("tapgo_tt_cycelbuy_periods") . " WHERE orderid = :orderid AND uniacid = :uniacid ";
        ...
        $cycelbuy_periodic = pdo_fetchcolumn("select cycelbuy_periodic from " . tablename("tapgo_tt_order") . " where id=:orderid and uniacid=:uniacid limit 1", $params);
        include($this->template());
    }
