...
require(TAPGO_TT_PLUGIN . "cycelbuy/core/page_mobile.php");

class Refund_TapgoTtPage extends OrderMobilePage
{
    protected function globalData()
    {
        ...
        if( $order["status"] == 2 ) 
        {
            $goods = pdo_fetchall("select og.goodsid, og.price, og.total, og.optionname, g.cannotrefund, g.thumb, g.title,g.isfullback from" . tablename("tapgo_tt_order_goods") . " og left join " . tablename("tapgo_tt_goods") . " g on g.id=og.goodsid where og.orderid=" . $order["id"]);
            if( !empty($goods) ) 
            {
                foreach( $goods as $g ) 
                {
        ...
        $fullback_log = pdo_fetch("select * from " . tablename("tapgo_tt_fullback_log") . " where orderid = " . $orderid . " and uniacid = " . $uniacid . " ");
        if( $fullback_log ) 
        {
            $fullbackgoods = pdo_fetch("select refund from " . tablename("tapgo_tt_fullback_goods") . " where goodsid = " . $fullback_log["goodsid"] . " and uniacid = " . $uniacid . " ");
    ...
    public function main()
    {
        ...
        if( 0 < $order["refundstate"] ) 
        {
            if( !empty($refundid) ) 
            {
                $refund = pdo_fetch("select * from " . tablename("tapgo_tt_order_refund") . " where id=:id and uniacid=:uniacid and orderid=:orderid limit 1", array( ":id" => $refundid, ":uniacid" => $uniacid, ":orderid" => $orderid ));
