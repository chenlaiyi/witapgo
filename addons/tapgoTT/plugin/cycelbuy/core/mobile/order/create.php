...
class Create_TapgoTtPage extends PluginMobilePage
{
    ...
    public function caculatecoupon($contype, $couponid, $wxid, $wxcardid, $wxcode, $goodsarr, $totalprice, $discountprice, $isdiscountprice, $isSubmit = 0, $discountprice_array = array(  ), $merchisdiscountprice = 0)
    {
        ...
        if( $contype == 1 ) 
        {
            $sql = "select id,uniacid,card_type,logo_url,title, card_id,least_cost,reduce_cost,discount,merchid,limitgoodtype,limitgoodcatetype,limitgoodcateids,limitgoodids,merchid  from " . tablename("tapgo_tt_wxcard");
    ...
    public function submit()
    {
        ...
        foreach( $goods as $g ) 
        {
            ...
            if( !empty($optionid) ) 
            {
                $option = pdo_fetch("select id,title,marketprice,presellprice,goodssn,productsn,stock,`virtual`,weight,cycelbuy_periodic" . $sql_condition . " from " . tablename("tapgo_tt_goods_option") . " where id=:id and goodsid=:goodsid and uniacid=:uniacid  limit 1", array( ":uniacid" => $uniacid, ":goodsid" => $goodsid, ":id" => $optionid ));
                if( !empty($option) ) 
                {
                    if( empty($data["unite_total"]) ) 
                    {
