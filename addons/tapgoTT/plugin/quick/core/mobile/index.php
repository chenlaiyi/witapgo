...
class Index_TapgoTtPage extends PluginMobileLoginPage {
    ...
    public function main(){
        ...
        $cartcount = pdo_fetchcolumn('select ifnull(sum(total),0) from ' . tablename('tapgo_tt_member_cart') . ' where uniacid=:uniacid and openid=:openid and deleted=0 and isnewstore=0  and selected =1', array(':uniacid' => $_W['uniacid'], ':openid' => $_W['openid']));
    ...
    public function update() {
        ...
        $goods = pdo_fetch('select id,marketprice,diyformid,diyformtype,diyfields, isverify, `type`,merchid, cannotrefund from '.tablename('tapgo_tt_goods').' where id=:id and uniacid=:uniacid limit 1',array(':id'=>$goodsid,':uniacid'=>$_W['uniacid']));
        if (empty($goods)) {
            show_json(0, '商品未找到');
        }
    ...
    public function submit() {
        ...
        foreach ($list as &$g) {
            ...
            if( $seckillinfo && $seckillinfo['status']==0){
                ...
                if (!empty($optionid)) {
                    $option = pdo_fetch('select id,title,marketprice,goodssn,productsn,stock,`virtual`,weight from ' . tablename('tapgo_tt_goods_option') . ' where id=:id and goodsid=:goodsid and uniacid=:uniacid  limit 1', array(':uniacid' => $uniacid, ':goodsid' => $g['id'], ':id' => $optionid));
                    if (!empty($option)) {
                        if ($option['stock'] != -1) {
