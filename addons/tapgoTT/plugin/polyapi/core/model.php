...
class PolyapiModel extends PluginModel {
    ...
    public function get_key_info($array) {
        ...
        $item = pdo_fetch('select * from ' . tablename('tapgo_tt_polyapi_key') . ' where appkey=:appkey and token=:token limit 1', $params);
    ...
    public function GetOrder($array) {
        ...
        $sql = "select o.* , a.realname as arealname,a.mobile as amobile,a.province as aprovince ,a.city as acity , a.area as aarea,a.address as aaddress,
                  ...
                  r.rtype,r.status as rstatus,o.sendtype,o.is_cashier from " . tablename('tapgo_tt_order') . " o"
        ...
        $t = pdo_fetch(
            'SELECT COUNT(1) as count FROM ' . tablename('tapgo_tt_order') . " o "
            . " left join " . tablename('tapgo_tt_order_refund') . " r on r.id =o.refundid "
            . " left join " . tablename('tapgo_tt_member') . " m on m.openid=o.openid  and m.uniacid =  o.uniacid"
        ...
        if(!empty($list)) {
            foreach ($list as $k => $v){
                ...
                if ($v['parentid'] > 0) {
                    $parent_item = pdo_fetch("SELECT id,ordersn,ordersn2,price FROM " . tablename('tapgo_tt_order') . " WHERE id = :id and uniacid=:uniacid Limit 1", array(':id' => $v['parentid'], ':uniacid' => $uniacid));
                ...
                if (empty($v['addressid'])) {
                    ...
                    if (!is_array($user)) {
                        $user = pdo_fetch("SELECT * FROM " . tablename('tapgo_tt_member_address') . " WHERE id = :id and uniacid=:uniacid", array(':id' => $v['addressid'], ':uniacid' => $uniacid));
                    }
                ...
                $refundid = $v['refundid'];
                if (!empty($refundid)) {
                    $refund = pdo_fetch('select status from ' . tablename('tapgo_tt_order_refund') . ' where id=:id limit 1', array(':id' => $refundid));
                }
                $refund_info = "";
                ...
                $order_goods = pdo_fetchall('select g.id,g.title,g.thumb,g.goodssn,g.hasoption,og.goodssn as option_goodssn, g.productsn,og.productsn as option_productsn, og.total,
                    ...
                    og.diyformfields,op.specs,g.merchid,og.seckill,og.seckill_taskid,og.seckill_roomid,g.ispresell,og.id as ogid from ' . tablename('tapgo_tt_order_goods') . ' og '
    ...
    public function Send($array) {
        ...
        if (empty($IsSplit)) {
            ...
            pdo_update('tapgo_tt_order', $change_data, array('id' => $item['id'], 'uniacid' => $uniacid));
        } else {
            ...
            foreach($goodsid as $key => $value){
                pdo_update('tapgo_tt_order_goods', $change_data, array('orderid' => $item['id'],'goodsid'=>$value, 'uniacid' => $uniacid));
            }
            ...
            pdo_update('tapgo_tt_order', $senddata, array('id' => $item['id'], 'uniacid' => $uniacid));
        }
    ...
    public function DownloadProduct($array) {
        ...
        if (!empty($total)) {
            ...
            foreach($list as $k => &$v){
                ...
                if ($v['hasoption'] == 1) {
                    $options = pdo_fetchall("select * from " . tablename('tapgo_tt_goods_option') . " where goodsid=:id order by id asc", array(':id' => $v['id']));
    ...
    public function SyncStock($array) {
        ...
        $item = pdo_fetch("select * from " . tablename('tapgo_tt_goods') . ' where id=:id and uniacid=:uniacid Limit 1', array(':id' => $PlatProductID, ':uniacid' => $uniacid));
        ...
        if ($item['hasoption'] == 1) {
            ...
            pdo_update('tapgo_tt_goods_option', array('stock' => $Quantity), array('id' => $SkuID, 'goodsid' => $PlatProductID));
            pdo_update('tapgo_tt_goods', array('total' => $goods_total), array('id' => $PlatProductID));

        } else {
            pdo_update('tapgo_tt_goods', array('total' => $Quantity), array('id' => $PlatProductID));
        }
    ...
    public function getExpressInfo($LogisticType) {
        ...
        $express = pdo_fetch('select * from ' . tablename('tapgo_tt_express') . ' where code=:code limit 1', array(':code' => $LogisticType));
    ...
    public function getOrderData($ordersn, $uniacid) {
        ...
        $item = pdo_fetch('select * from ' . tablename('tapgo_tt_order') . ' where  ordersn=:ordersn and uniacid=:uniacid Limit 1', array(':ordersn' => $ordersn, ':uniacid' => $uniacid));
        return $item;
    }
    ...
    public function insert_data($data, $uniacid) {
        ...
        pdo_insert('tapgo_tt_polyapi_data', $insert_data);
    }
    ...
    public function get_data($uniacid) {
        ...
        $sql = "SELECT * FROM " . tablename('tapgo_tt_polyapi_data') . " WHERE uniacid=:uniacid order by id desc";
        $list = pdo_fetchall($sql, $params);
