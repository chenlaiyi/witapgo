...
class Set_TapgoTtPage extends PluginWebPage {
    ...
    function main()
    {
        ...
        $item = pdo_fetch('select * from ' . tablename('tapgo_tt_polyapi_key') . ' where uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid']));
        ...
        if ($_W['ispost']) {
            ...
            if (empty($item)) {
                ...
                pdo_insert('tapgo_tt_polyapi_key', $data);
            } else {
                $data['updatetime'] = $time;
                pdo_update('tapgo_tt_polyapi_key', $data, array('uniacid' => $_W['uniacid']));
            }
    ...
    function update()
    {
        ...
        $item = pdo_fetch('select code from ' . tablename('tapgo_tt_express') . ' where express=:express limit 1', array(':express' => 'shunfeng'));
        ...
        if (empty($item['code'])) {
            ...
            foreach ($data as $k => $v) {
                pdo_update('tapgo_tt_express', array('code' => $v), array('express' => $k));
