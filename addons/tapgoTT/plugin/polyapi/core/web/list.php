...
class List_TapgoTtPage extends PluginWebPage {
    ...
    protected  function orderData($status, $st){
        ...
        if ($condition != ' o.uniacid = :uniacid and o.ismr=0 and o.deleted=0 and o.isparent=0' || !empty($sqlcondition)){
            $t = pdo_fetch(
                'SELECT COUNT(1) as count, ifnull(sum(o.price),0) as sumprice   FROM ' . tablename('tapgo_tt_order') . " o "
                . " left join " . tablename('tapgo_tt_order_refund') . " r on r.id =o.refundid "
                . " left join " . tablename('tapgo_tt_member') . " m on m.openid=o.openid  and m.uniacid =  o.uniacid"
            ...
            $t = pdo_fetch(
                'SELECT COUNT(1) as count, ifnull(sum(price),0) as sumprice   FROM ' . tablename('tapgo_tt_order') . " WHERE uniacid = :uniacid and ismr=0 and deleted=0 and isparent=0 {$status_condition}", $paras);
        }
