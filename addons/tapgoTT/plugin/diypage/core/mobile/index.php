...
class Index_TapgoTtPage extends PluginMobilePage {
	...
	public function getmerch() {
        ...
        if($_W['ispost']){
            ...
            $merchs = pdo_fetchall("select id, merchname as `name`, logo as thumb, status, `desc`, address, tel, lng, lat from " . tablename('tapgo_tt_merch_user') . " where 1 {$condition} order by ".$orderby. $limitsql, $params);
    ...
    public function getInfo(){
	    ...
	    if($urlData[0] == 'goodsids' || $urlData[0] == 'category' || $urlData[0] == 'groups'){
	        ...
	        if($urlData[0] == 'category'){
                $pcate = $urlValue[0];
                $goodsql = 'SELECT id,displayorder,title,subtitle,thumb,marketprice,productprice,minprice,maxprice,isdiscount,isdiscount_time,isdiscount_discounts,sales,salesreal,hascommission,commission1_pay,commission,total,description,bargain,nocommission,`type`,ispresell,`virtual`,hasoption,video,buylevels,buygroups,checked FROM ' .tablename('tapgo_tt_goods') .' WHERE FIND_IN_SET('.$pcate.',cates) AND status > 0 AND deleted = 0 AND checked=0 AND uniacid ='.$_W['uniacid']. $where_tmp .  ' order by displayorder desc,id desc limit 0,'.$goodsnum;
                $list['list'] = pdo_fetchall($goodsql);

                $count = pdo_fetch('SELECT count(id) as count FROM ' .tablename('tapgo_tt_goods') .' WHERE FIND_IN_SET('.$pcate.',cates) AND status > 0 AND deleted = 0 AND uniacid ='.$_W['uniacid'] , $where_tmp);
...
//                    秒杀
	            ...
	            $sql = 'SELECT * FROM ' .tablename('tapgo_tt_goods_group') .' WHERE id = :id AND uniacid = :uniacid';
                $params = array(
                    ':uniacid' => $_W['uniacid'],
                    ':id' => $urlValue[0],
                );
                ...
                $goodsql = 'SELECT id,displayorder,title,subtitle,thumb,marketprice,productprice,minprice,maxprice,isdiscount,hascommission,nocommission,commission,commission1_rate,marketprice,commission1_pay,maxprice,isdiscount_time,isdiscount_discounts,sales,salesreal,total,description,bargain,`type`,ispresell,`virtual`,hasoption,video,buylevels,buygroups FROM ' .tablename('tapgo_tt_goods') .' WHERE id in('.$goodsid.') AND status > 0 AND deleted = 0 AND uniacid ='.$_W['uniacid']. $where_tmp .' limit 0,'.$goodsnum;
                $count = pdo_fetch('SELECT count(id) as count FROM ' .tablename('tapgo_tt_goods') .' WHERE id in('.$goodsid.') AND status > 0 AND checked=0 AND deleted = 0 AND uniacid ='.$_W['uniacid'] . $where_tmp);
                $list['list'] = pdo_fetchall($goodsql);
                $list['count'] = $count['count'];
                if(!empty($list)){
...
//                    秒杀
                ...
                if(!empty($goodsids)){
                    ...
                    $sql = 'SELECT id,displayorder,title,subtitle,thumb,marketprice,productprice,minprice,maxprice,hascommission,nocommission,commission,commission1_rate,marketprice,commission,commission1_pay,maxprice,isdiscount,isdiscount_time,isdiscount_discounts,sales,salesreal,total,description,bargain,`type`,ispresell,`virtual`,hasoption,video,buylevels,buygroups,showlevels FROM ' .tablename('tapgo_tt_goods') .' WHERE id in('.$goodsid.') AND uniacid ='.$_W['uniacid']. $where_tmp .' limit 0,'.$goodsnum;
                    $count = pdo_fetch('SELECT count(id) as count FROM ' .tablename('tapgo_tt_goods') .' WHERE id in('.$goodsid.') AND checked=0 AND uniacid ='.$_W['uniacid'] . $where_tmp);
...
//                    秒杀
            ...
            if(!empty($storesids)){
                ...
                $sql = 'SELECT id,displayorder,storename FROM ' .tablename('tapgo_tt_store') .' WHERE id in('.$storesid.') AND uniacid ='.$_W['uniacid']. $where_tmp . ' limit 0,'.$storenum;
                $count = pdo_fetch('SELECT count(id) as count FROM ' .tablename('tapgo_tt_store') .' WHERE id in('.$storesid.') AND uniacid ='.$_W['uniacid'] . $where_tmp);
                $list['list'] = pdo_fetchall($sql);
                $list['count'] = $count['count'];
                if(!empty($list)){
                    m('common')->sortArrayByKey($list['list'], 'displayorder');
