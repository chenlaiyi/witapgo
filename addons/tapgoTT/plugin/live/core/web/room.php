...
class Room_TapgoTtPage extends PluginWebPage
{
	public function main()
	{
		...
		$sql = 'SELECT l.*,lc.name FROM ' . tablename('tapgo_tt_live') . ' as l
		        left join ' . tablename('tapgo_tt_live_category') . (' as lc on lc.id = l.category
		        where  1 and ' . $condition . ' ORDER BY l.displayorder DESC,l.id DESC LIMIT ') . ($pindex - 1) * $psize . ',' . $psize;
		$list = pdo_fetchall($sql, $params);
		$total = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('tapgo_tt_live') . ' as l
		        left join ' . tablename('tapgo_tt_live_category') . (' as lc on lc.id = l.category
		...
		$category = pdo_fetchall('select id,`name`,thumb from ' . tablename('tapgo_tt_live_category') . ' where uniacid=:uniacid order by displayorder desc', array(':uniacid' => $_W['uniacid']), 'id');
	...
	protected function post()
	{
		...
		if (0 < $id) {
			$item = pdo_fetch('select * from ' . tablename('tapgo_tt_live') . ' where uniacid = ' . $uniacid . ' and id = ' . $id . ' ');
			$coupon = array();

			if (!empty($item['couponid'])) {
				$coupon = pdo_fetchall('select c.couponname,c.thumb,c.id,lc.coupontotal,lc.couponlimit from ' . tablename('tapgo_tt_live_coupon') . ' as lc
                        left join ' . tablename('tapgo_tt_coupon') . ' as c on c.id = lc.couponid
		...
		if ($_W['ispost']) {
			...
			if (!empty($data['couponid'])) {
				foreach ($_GPC['couponid'] as $key => $value) {
					if (strpos($couponid, $value) !== false) {
						pdo_update('tapgo_tt_live_coupon', array('coupontotal' => $_GPC['coupontotal' . $value . ''], 'couponlimit' => $_GPC['couponlimit' . $value . '']), array('uniacid' => $uniacid, 'roomid' => $id, 'couponid' => $value));
						...
						pdo_insert('tapgo_tt_live_coupon', $data_coupon);
					}
				}
			}
			...
			if (!empty($couponid)) {
				foreach ($couponid as $value) {
					pdo_delete('tapgo_tt_live_coupon', array('couponid' => $value, 'uniacid' => $uniacid, 'roomid' => $id));
				}
			}
			...
			if (!empty($id)) {
				...
				pdo_update('tapgo_tt_live', $data, array('id' => $id));
				plog('live.room.edit', '编辑直播间 ID: ' . $id . ' <br/>全返名称: ' . $data['title']);
			}
			else {
				$data['createtime'] = time();
				pdo_insert('tapgo_tt_live', $data);
				$id = pdo_insertid();
			...
			if (!empty($goodsarr)) {
				foreach ($goodsarr as $key => $value) {
					$good_data = pdo_fetch('select title,thumb,marketprice,goodssn,productsn,hasoption
                            from ' . tablename('tapgo_tt_goods') . ' where id = ' . $value['id'] . ' and uniacid = ' . $uniacid . ' ');
					...
					if (!empty($value['options'])) {
						pdo_delete('tapgo_tt_live_goods_option', array('goodsid' => $value['id'], 'uniacid' => $uniacid, 'liveid' => intval($id)));
						...
						foreach ($value['options'] as $options) {
							...
							pdo_insert('tapgo_tt_live_goods_option', $optionData);
						}

						$livegoods_option = pdo_fetch('select min(liveprice) as minliveprice,max(liveprice) as maxliveprice from ' . tablename('tapgo_tt_live_goods_option') . ' where goodsid = ' . $value['id'] . ' and liveid = ' . $id . ' ');
					...
					$live_goods = pdo_fetch('select * from ' . tablename('tapgo_tt_live_goods') . ' where goodsid = ' . $value['id'] . ' and uniacid = ' . $uniacid . ' and liveid = ' . $id . ' ');
					...
					if (empty($live_goods)) {
						...
						pdo_insert('tapgo_tt_live_goods', $live_goods_data);
					}
					else {
						pdo_update('tapgo_tt_live_goods', array('minliveprice' => floatval($minliveprice), 'liveprice' => floatval($minliveprice), 'maxliveprice' => floatval($maxliveprice)), array('uniacid' => $uniacid, 'goodsid' => intval($value['id']), 'liveid' => $id));
					}
				...
				if (!empty($goodsliveid)) {
					foreach ($goodsliveid as $value) {
						pdo_delete('tapgo_tt_live_goods', array('uniacid' => $uniacid, 'goodsid' => intval($value), 'liveid' => $id));
					}
				}
		...
		if ($isInvitation) {
			$invitation = pdo_fetchall('SELECT id,title FROM ' . tablename('tapgo_tt_invitation') . ' WHERE status = 1 and uniacid = ' . $uniacid . ' ORDER BY createtime desc');
		}
		...
		$category = pdo_fetchall('select * from ' . tablename('tapgo_tt_live_category') . ' where uniacid = ' . $uniacid . ' and enabled = 1 ');
		if (0 < $id && !empty($item['goodsid'])) {
			$goods = pdo_fetchall('select g.id,g.title,g.marketprice,lg.liveprice,lg.minliveprice,lg.maxliveprice,g.thumb,g.hasoption,lg.liveid,g.total  from ' . tablename('tapgo_tt_live_goods') . ' as lg 
            left join ' . tablename('tapgo_tt_goods') . ' as g on lg.goodsid = g.id and lg.uniacid = g.uniacid
			...
			foreach ($goods as $key => $value) {
				...
				if (0 < $value['hasoption']) {
					$goods[$key]['options'] = pdo_fetchall('SELECT go.id,lgo.liveprice FROM ' . tablename('tapgo_tt_live_goods_option') . ' as lgo
                    left join ' . tablename('tapgo_tt_goods_option') . ' as go on lgo.optionid=go.id and lgo.goodsid=go.goodsid and lgo.uniacid=go.uniacid and lgo.liveid = ' . $id . '
	...
	public function statistics()
	{
		...
		$end = intval($_GPC['end']);
		$room = pdo_fetch('select * from ' . tablename('tapgo_tt_live') . ' where uniacid = ' . $uniacid . ' and id = ' . $roomid . ' ');
		...
		$statistics_time = pdo_fetchall('select * from ' . tablename('tapgo_tt_live_status') . ' where roomid = ' . $roomid . ' and uniacid = ' . $uniacid . ' and endtime != \'\' order by id desc ');
		...
		$list = array();
		$order = array();
		$order = pdo_fetchall('select og.goodsid,og.price,og.total,g.title from ' . tablename('tapgo_tt_order') . ' as o 
                left join ' . tablename('tapgo_tt_order_goods') . ' as og on og.orderid = o.id 
                left join ' . tablename('tapgo_tt_goods') . ' as g on og.goodsid = g.id
	...
	public function hasoption()
	{
		...
		if (empty($id)) {
			$goods = pdo_fetch('select id,title,marketprice,hasoption from ' . tablename('tapgo_tt_goods') . '
                        where id = ' . $goodsid . ' and uniacid = ' . $uniacid . ' ');
		}
		else {
			$goods = pdo_fetch('select g.id,g.title,g.marketprice,lg.liveprice,g.hasoption from ' . tablename('tapgo_tt_live_goods') . ' as lg
                    left join ' . tablename('tapgo_tt_goods') . ' as g on g.id = lg.goodsid and g.uniacid = lg.uniacid
		...
		if ($goods['hasoption']) {
			...
			if (empty($id)) {
                ...
                FROM ' . tablename('tapgo_tt_goods_option') . ' as go 
                ...
                FROM ' . tablename('tapgo_tt_goods_option') . ' as go 
                left join ' . tablename('tapgo_tt_live_goods_option') . ' as lgo on lgo.optionid = go.id and lgo.goodsid = go.goodsid and lgo.uniacid = go.uniacid and lgo.liveid = ' . $id . '
                WHERE go.uniacid = :uniacid and go.goodsid = :goodsid group by go.id  ORDER BY go.displayorder DESC,go.id DESC ', $params);
			}
		}
		else {
	...
	public function deleted()
	{
		...
		$items = pdo_fetchall('SELECT id,title FROM ' . tablename('tapgo_tt_live') . (' WHERE id in( ' . $id . ' ) AND uniacid=') . $_W['uniacid']);

		foreach ($items as $item) {
			pdo_delete('tapgo_tt_live', array('id' => $item['id']));
			plog('live.room.deleted', '删除直播间<br/>ID: ' . $item['id'] . '<br/>直播间名称: ' . $item['title']);
			$favoritetotal = pdo_fetchcolumn('select count(1) from ' . tablename('tapgo_tt_live_favorite') . ' where roomid = ' . $id . ' ');

			if (0 < $favoritetotal) {
				pdo_delete('tapgo_tt_live_favorite', array('roomid' => $item['id']));
			}

			$viewtotal = pdo_fetchcolumn('select count(1) from ' . tablename('tapgo_tt_live_view') . ' where roomid = ' . $id . ' ');

			if (0 < $viewtotal) {
				pdo_delete('tapgo_tt_live_view', array('roomid' => $item['id']));
			}
	...
	public function console()
	{
		...
		$item = pdo_fetch('SELECT * FROM ' . tablename('tapgo_tt_live') . ' WHERE uniacid=:uniacid AND id=:id', array(':uniacid' => $_W['uniacid'], ':id' => $id));
		...
		if (in_array($type, array('status', 'displayorder', 'hot', 'recommend'))) {
			...
			if (empty($id) && is_array($_GPC['ids'])) {
				...
				foreach ($_GPC['ids'] as $item) {
					$property_update = pdo_update('tapgo_tt_live', array($type => $value), array('id' => $item, 'uniacid' => $_W['uniacid']));
					plog('live.room.edit', '修改直播' . $typestr . '状态   ID: ' . $item . ' ' . $statusstr . ' ');
				}
			}
			else {
				$property_update = pdo_update('tapgo_tt_live', array($type => $value), array('id' => $id, 'uniacid' => $_W['uniacid']));
