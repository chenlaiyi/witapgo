...
class LiveModel extends PluginModel
{
	...
	public function selectOrderPrice($day = 0, $roomid = 0)
	{
		...
		$sql = 'select id,price,createtime from ' . tablename('tapgo_tt_order') . ' where uniacid = :uniacid ' . $condition . ' and ismr=0 and isparent=0 and (status > 0 or ( status=0 and paytype=3)) and deleted=0 and createtime between :createtime1 and :createtime2';
	...
	public function getSet()
	{
		global $_W;
		$set = array();
		$set = pdo_fetch('select * from ' . tablename('tapgo_tt_live_setting') . ' where uniacid = :uniacid  ', array(':uniacid' => $_W['uniacid']));
		$plugin = pdo_fetch('select `name` from ' . tablename('tapgo_tt_plugin') . ' where `identity` = \'live\' and isv2 = 1 and status = 1 ');
	...
	public function sendLiveMessage($openid, $type, $roomid)
	{
		...
		$room = pdo_fetch('select * from ' . tablename('tapgo_tt_live') . ' where uniacid = ' . $_W['uniacid'] . ' and id = ' . $roomid . ' ');
...
感谢您的支持', 'color' => '#000000')
			...
			if ($type == 'liveroom') {
				...
				$url = str_replace('addons/tapgo_ttv2/plugin/live/task/', '', $_W['siteroot']);
				$url = $url . 'app/index.php?i=' . $_W['uniacid'] . '&c=entry&m=tapgo_ttv2&do=mobile&r=live.room&id=' . $room['id'];
				$tag = 'livefollow';
				$remark = '
<a href=\'' . $url . '\'>点击查看详情</a>';
...
感谢您的支持', 'color' => '#000000')
	...
	public function isLiving($roomid = 0)
	{
		...
		if (!empty($roomid)) {
			$live = pdo_fetch('SELECT id FROM ' . tablename('tapgo_tt_live') . ' WHERE id=:id AND uniacid=:uniacid  LIMIT 1', array(':id' => $roomid, ':uniacid' => $_W['uniacid']));
	...
	public function isFavorite($openid = NULL, $roomid = 0)
	{
		...
		if (!empty($openid) && !empty($roomid)) {
			$favorite = pdo_fetch('SELECT id FROM ' . tablename('tapgo_tt_live_favorite') . 'WHERE uniacid=:uniacid AND roomid=:roomid AND openid=:openid AND `deleted`=0 LIMIT 1', array(':uniacid' => $_W['uniacid'], ':roomid' => $roomid, ':openid' => $openid));
	...
	public function emoji2html($str)
	{
		...
		foreach ($emojiList as $index => $emoji) {
			if ($emoji == $str) {
				return '<img class="face" src="../addons/tapgo_ttv2/plugin/live/static/images/face/' . $index . '.gif?v=2" />';
			}
		}
	...
	public function handleRecords($roomid = 0, $manage = false)
	{
		...
		if ($manage) {
			$table_banned = 'tapgo_tt_live_banned_' . $roomid;
			$bannedArr = array();
		}
	...
	public function getLiveInfo($url = NULL, $type = 'auto')
	{
		...
		case 'douyu':
			...
			$resultArr = array('status' => $apiResult['data']['error'] == 0 ? 1 : 0, 'poster' => '../addons/tapgo_ttv2/static/images/nopic.png', 'hls_url' => $apiResult['data']['hls_url']);
	...
	{
		return 'tapgo_tt_live_' . $table . '_' . $roomid;
	}
	...
	public function isLiveGoods($goodsid = 0, $liveid = 0)
	{
		...
		$livegoods = pdo_fetch('SELECT * FROM ' . tablename('tapgo_tt_live_goods') . ' WHERE goodsid=:goodsid AND liveid=:liveid AND uniacid=:uniacid', array(':goodsid' => $goodsid, ':liveid' => $liveid, ':uniacid' => $_W['uniacid']));
	...
	public function getLiveOptions($goodsid = 0, $liveid = 0, $options = array())
	{
		...
		if (empty($options)) {
			$options = pdo_fetchall('SELECT id, marketprice FROM ' . tablename('tapgo_tt_goods_option') . ' WHERE goodsid=:goodsid AND uniacid=:uniacid ORDER BY displayorder ASC', array(':goodsid' => $goodsid, ':uniacid' => $_W['uniacid']));
		}
		...
		$liveOptions = pdo_fetchall('SELECT * FROM ' . tablename('tapgo_tt_live_goods_option') . ' WHERE goodsid=:goodsid AND liveid=:liveid AND uniacid=:uniacid', array(':goodsid' => $goodsid, ':liveid' => $liveid, ':uniacid' => $_W['uniacid']), 'optionid');
	...
	public function getAllGoods($liveid = 0)
	{
		...
		if (!empty($liveid)) {
			$goods = pdo_fetch('select goodsid from ' . tablename('tapgo_tt_live') . ' where id = :liveid and uniacid = :uniacid ', array(':liveid' => $liveid, ':uniacid' => $_W['uniacid']));
			$goodsid = array();
			...
			if (!empty($goods['goodsid'])) {
				...
				foreach ($goodsid as $key => $value) {
					$goodslist[$key] = pdo_fetch('SELECT lg.*,sg.id as id,lg.id as livegid,sg.marketprice,sg.thumb,sg.title FROM ' . tablename('tapgo_tt_live_goods') . ' lg LEFT JOIN ' . tablename('tapgo_tt_goods') . ' sg ON sg.id=lg.goodsid WHERE lg.liveid=:liveid AND lg.goodsid = :goodsid AND lg.uniacid=:uniacid', array(':liveid' => $liveid, ':goodsid' => $value, ':uniacid' => $_W['uniacid']));
				}
			}
		}
	...
	{
		$file = TAPGO_TT_CORE . 'socket/socket.config.php';
