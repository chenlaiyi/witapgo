...
class Follow_TapgoTtPage extends PluginMobileLoginPage
{
	...
	public function get_list()
	{
		...
		$sql = 'SELECT COUNT(1) FROM ' . tablename('tapgo_tt_live_favorite') . ' as lf
                    right join ' . tablename('tapgo_tt_live') . (' as l on l.id = lf.roomid and l.living = ' . $living . '
            		where 1 ' . $condition);
		$total = pdo_fetchcolumn($sql, $params);
		$list = array();

		if (!empty($total)) {
			$sql = 'SELECT lf.id,lf.roomid,l.title,l.thumb,l.livetime,l.subscribe,l.living,l.covertype,l.cover FROM ' . tablename('tapgo_tt_live_favorite') . ' as lf
                    right join ' . tablename('tapgo_tt_live') . (' as l on l.id = lf.roomid and l.living = ' . $living . '
