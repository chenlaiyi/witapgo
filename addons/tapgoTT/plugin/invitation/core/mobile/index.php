...
class Index_TapgoTtPage extends PluginMobilePage
{
	public function main()
	{
		...
		if ($item['type'] == 0) {
			...
			$room = pdo_fetch('SELECT id,title,introduce,invitation_id FROM ' . tablename('tapgo_tt_live') . ' where uniacid = :uniacid and id=:roomid ', array(':uniacid' => $_W['uniacid'], ':roomid' => $roomid));
	...
	protected function getTemp($item)
	{
		...
		foreach ($item['data']['selected'] as $tempid) {
			if (0 < intval($tempid)) {
				$templist[] = array('bg' => '../addons/tapgo_ttv2/plugin/invitation/static/templist/image_' . $tempid . '_bg.jpg', 'nail' => '../addons/tapgo_ttv2/plugin/invitation/static/templist/image_' . $tempid . '_nail.jpg');
			}
			else {
