...
ignore_user_abort();
set_time_limit(0); 
$sets = pdo_fetchall('select uniacid,refund from ' . tablename('tapgo_tt_groups_set') . " where {$where}");
...
foreach ($sets as $key => $value) {
    ...
    $times = $hours * 60 * 60;
    $orders = pdo_fetchall("select id,orderno,refundid,openid,credit,creditmoney,price,freight,status,pay_type,teamid,apppay,isborrow,borrowopenid,more_spec from " . tablename('tapgo_tt_groups_order') . "
    ...
    foreach ($orders as $k => $val) {
        ...
        $refund = pdo_fetch('select * from ' . tablename('tapgo_tt_groups_order_refund') . ' where id=:id limit 1', array(':id' => $val['refundid']));
        if(empty($refund)!=true){
            if($refund['refundstatus']==0){
                ...
                pdo_update('tapgo_tt_groups_order_refund', $change_refund, array('id' => $val['refundid']));
            }
        }
        pdo_update('tapgo_tt_groups_order', array('refundstate' => 0, 'status' => -1, 'refundtime' => time()), array('id' => $val['id'], 'uniacid' => $_W['uniacid']));
        $sales = pdo_fetch("select id,sales,stock from ".tablename('tapgo_tt_groups_goods')." where id = :id and uniacid = :uniacid ",array(':id'=>$val['goodid'], ':uniacid' => $uniacid));

        pdo_update('tapgo_tt_groups_goods', array('sales' => $sales['sales'] - 1,'stock'=>$sales['stock'] + 1), array('id' => $sales['id'], 'uniacid' => $uniacid));
        if( $val['more_spec'] == 1 ){
            $option = pdo_get( 'tapgo_tt_groups_order_goods' , array( 'uniacid' => $_W['uniacid'] , 'groups_order_id' => $val['id'] ) );
