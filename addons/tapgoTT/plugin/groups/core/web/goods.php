...
class Goods_TapgoTtPage extends PluginWebPage {
    function main() {
        ...
        $total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('tapgo_tt_groups_goods') . " AS g where 1 and {$condition}", $params);
        $pager = pagination2($total, $pindex, $psize);
        $category = pdo_fetchall("select id,name,thumb from " . tablename('tapgo_tt_groups_category') . ' where uniacid=:uniacid order by displayorder desc', array(':uniacid' => $_W['uniacid']), 'id');
    ...
    protected function post() {
        global $_W, $_GPC;
        $id = intval($_GPC['id']);
        $item = pdo_fetch("SELECT g.*,c.name as catename FROM " . tablename('tapgo_tt_groups_goods') . " as g
				left join ".tablename('tapgo_tt_groups_category')." as c on c.id = g.category
        ...
        $category = pdo_fetchall("select id,name,thumb from " . tablename('tapgo_tt_groups_category') . ' where uniacid=:uniacid order by displayorder desc', array(':uniacid' => $_W['uniacid']));
        ...
        if( !empty( $item['more_spec'] ) ){
            $specs = pdo_getall( 'tapgo_tt_groups_goods_option' , array( 'groups_goods_id' => $item['id'] ) );
        }

        $ladder = array();
        if( !empty( $item['is_ladder'] ) ){
            $ladder = pdo_getall( 'tapgo_tt_groups_ladder' , array( 'goods_id' => $item['id'] ) );
        }

        $dispatch_data = pdo_fetchall('select * from ' . tablename('tapgo_tt_dispatch') . ' where uniacid=:uniacid and enabled=1 order by displayorder desc', array(':uniacid' => $_W['uniacid']));
        if ($_W['ispost']) {
            $data = array(
                'uniacid' => $_W['uniacid'],
            ...
            if (!empty($id)) {
                $goods_update = pdo_update('tapgo_tt_groups_goods', $data, array('id' => $id, 'uniacid' => $_W['uniacid']));
                if(!$goods_update){
                    show_json(0,'商品编辑失败！');
                }
                if( !empty( $ladder ) ){
                    ...
                    foreach ( $ladder as $k => $v ){
                        if( $ladder_id[$k] ){
                            pdo_update( 'tapgo_tt_groups_ladder',$v , array( 'id' => $ladder_id[$k] ));
                        }else{
                            $v['goods_id'] = $id;
                            $v['uniacid'] = $_W['uniacid'];
                            pdo_insert( 'tapgo_tt_groups_ladder',$v );
                    ...
                    if( !empty($ladder_id_str) ){
                        pdo_query( "DELETE FROM ".tablename('tapgo_tt_groups_ladder')." WHERE id NOT IN({$ladder_id_str}) AND goods_id = :goods_id" , array(':goods_id' => $id) );
                    }
                ...
                $goods_insert = pdo_insert('tapgo_tt_groups_goods', $data);
                if(!$goods_insert){
                    show_json(0,'商品添加失败！');
                }
                $id = pdo_insertid();
                $gid = intval($data['gid']);
                if($gid){
                    pdo_update('tapgo_tt_goods', array('groupstype'=>1), array('id' => $gid, 'uniacid' => $_W['uniacid']));
                    if( !empty( $spec ) ){
    ...
    public function dispose_spec( $spec ,$groups_goods_id , $goods_id = 0)
    {
        ...
        foreach ( $spec as $k => $v ){
            ...
            if( empty($v['id']) ){
                pdo_insert( 'tapgo_tt_groups_goods_option' , $data );
            }else{
                pdo_update( 'tapgo_tt_groups_goods_option' , $data , array( 'id' => $v['id'] ) );
            }
    ...
    function total(){
        ...
        $total = pdo_fetchcolumn("select count(1) from ".tablename('tapgo_tt_groups_goods')." where ".$condition." ",$params);
    ...
    function query(){
        ...
        $ds = pdo_fetchall('SELECT id as gid,title,subtitle,thumb,thumb_url,marketprice,content,productprice,subtitle,goodssn,productsn,followtip,followurl
				FROM ' . tablename('tapgo_tt_goods') . " WHERE 1 {$condition} order by createtime desc LIMIT ". ($pindex - 1) * $psize . ',' . $psize, $params);
        foreach($ds as &$d) {
    ...
    function delete1() {
        ...
        $items = pdo_fetchall("SELECT id,title,gid FROM " . tablename('tapgo_tt_groups_goods') . " WHERE id in( $id ) AND uniacid=" . $_W['uniacid']);

        foreach ($items as $item) {
            pdo_delete('tapgo_tt_groups_goods', array('id' => $item['id']));
            pdo_delete( 'tapgo_tt_groups_goods_option' , array( 'groups_goods_id' => $item['id'] ) );
    ...
    function restore() {
        ...
        foreach ($items as $item) {
            pdo_update('tapgo_tt_groups_goods', array('deleted' => 0,'status'=>0), array('id' => $item['id']));
            if(intval($item['gid'])){
                pdo_update('tapgo_tt_goods', array('groupstype'=>1), array('id' => $item['gid'], 'uniacid' => $_W['uniacid']));
            }
    ...
    function delete() {
        ...
        $items = pdo_fetchall("SELECT id,title,gid FROM " . tablename('tapgo_tt_groups_goods') . " WHERE id in( $id ) AND uniacid=" . $_W['uniacid']);
        foreach ($items as $item) {
            $delete_update = pdo_update('tapgo_tt_groups_goods', array('deleted' => 1,'status'=>0), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
            if(!$delete_update){
                show_json(0,'删除商品失败！');
            }
            if(intval($item['gid'])){
                pdo_update('tapgo_tt_goods', array('groupstype'=>0), array('id' => $item['gid'], 'uniacid' => $_W['uniacid']));
            }
    ...
    function status() {
        ...
        $status = intval($_GPC['status']);
        $items = pdo_fetchall("SELECT id,status FROM " . tablename('tapgo_tt_groups_goods') . " WHERE id in( $id ) AND uniacid=" . $_W['uniacid']);
        foreach ($items as $item) {
            $status_update = pdo_update('tapgo_tt_groups_goods', array('status' => $status), array('id' => $item['id']));
            if(!$status_update){
    ...
    function property() {
        ...
        if (in_array($type, array('status', 'displayorder'))) {
            ...
            $property_update = pdo_update("tapgo_tt_groups_goods", array($type => $value), array("id" => $id, "uniacid" => $_W['uniacid']));
            if(!$property_update){
    ...
    public function get_spec()
    {
        ...
        $specArr = pdo_getall( 'tapgo_tt_goods_option',array('goodsid' => $goods_id),array('id','title','thumb','marketprice','stock','specs') );
    ...
    {
        $spec = pdo_getall( 'tapgo_tt_groups_goods_option' , array('groups_goods_id' => $goods_id) );
        if( !empty( $spec ) ){
            return pdo_delete( 'tapgo_tt_groups_goods_option',array('groups_goods_id' => $goods_id) );
        }
