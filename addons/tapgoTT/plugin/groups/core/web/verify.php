...
class Verify_TapgoTtPage extends PluginWebPage
{
                    ...
                    else
                    {
                        ...
                        else
                        {
                            if( $searchfield == "saler" ) 
                            {
                                ...
                                $sqlcondition = " left join " . tablename("tapgo_tt_saler") . " as s on s.openid = v.verifier and s.uniacid=v.uniacid\r\n\t\t\t\tleft join " . tablename("tapgo_tt_member") . " sm on sm.openid = s.openid and sm.uniacid=s.uniacid ";
                            }
                            else
                            {
                                if( $searchfield == "store" ) 
                                {
                                    ...
                                    $sqlcondition = " left join " . tablename("tapgo_tt_store") . " store on store.id = v.storeid and store.uniacid=o.uniacid ";
                                }
    ...
    public function detail()
    {
        ...
        $order = pdo_fetch("SELECT o.*,g.title,g.category,g.groupsprice,g.singleprice,g.thumb,g.thumb_url,g.id as gid FROM " . tablename("tapgo_tt_groups_order") . " as o\r\n\t\t\t\tleft join " . tablename("tapgo_tt_groups_goods") . " as g on g.id = o.goodid\r\n\t\t\t\tWHERE o.id = :orderid ", $params);
        ...
        if( $order["verifytype"] == 0 ) 
        {
            ...
            if( !empty($verify["verifier"]) ) 
            {
                ...
                $saler["salername"] = pdo_fetchcolumn("select salername from " . tablename("tapgo_tt_saler") . " where openid=:openid and uniacid=:uniacid limit 1 ", array( ":uniacid" => $_W["uniacid"], ":openid" => $verify["verifier"] ));
            }
