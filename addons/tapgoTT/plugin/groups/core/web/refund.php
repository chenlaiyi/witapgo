...
class Refund_TapgoTtPage extends PluginWebPage {
    protected function opData() {
        ...
        if (!empty($refundid)) {
            $refund = pdo_fetch('select * from ' . tablename('tapgo_tt_groups_order_refund') . ' where id=:id limit 1', array(':id' => $refundid));
    ...
    function main() {
        ...
        try{
            ...
            $list = pdo_fetchall("SELECT o.addressid,o.goodid,og.option_name as optiontitle,o.pay_type,o.price,o.creditmoney,o.is_team,o.refundid,ore.*,g.title,g.thumb,g.category,g.groupsprice,g.singleprice,c.name,g.thumb,g.thumb_url
                ...
                FROM " . tablename('tapgo_tt_groups_order_refund') . " as ore
            ...
            $total = pdo_fetchcolumn("SELECT COUNT(1) FROM " . tablename('tapgo_tt_groups_order_refund') . " as ore
                left join ".tablename('tapgo_tt_groups_order')." as o on o.id = ore.orderid
    ...
    function detail(){
        ...
        try{
            ...
            $refund = pdo_fetch("SELECT ore.*,o.price,o.credit,op.title as optiontitle,o.freight,o.paytime,o.pay_type,o.orderno,o.goodid,g.title,g.thumb,g.thumb_url,g.category,g.groupsprice
                FROM " . tablename('tapgo_tt_groups_order_refund') . " as ore
				right join ".tablename('tapgo_tt_groups_order')." as o on o.id = ore.orderid
				left join ".tablename('tapgo_tt_groups_goods')." as g on g.id = o.goodid
    ...
    function submit() {
        ...
        if ($_W['ispost']) {
            ...
            $setting = pdo_fetch("SELECT refundday FROM ".tablename('tapgo_tt_groups_set')." WHERE uniacid = :uniacid ",array(':uniacid'=>$_W['uniacid']));
            $refundtime = $item['finishtime'] + 3600*24*$setting['refundday'];


            if ($item['status'] == 3 && $refundtime < time()) {
                pdo_update('tapgo_tt_groups_order', array('refundstate' => 0), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
            ...
            if (empty($refund['refundno'])) { 
                ...
                pdo_update('tapgo_tt_groups_order_refund', array('refundno' => $refund['refundno']), array('id' => $refund['id']));
            }
            ...
            if ($refundstatus == 0) {
                ...
                if ($raid != 0){
                    $raddress = pdo_fetch('select * from '.tablename('tapgo_tt_refund_address').' where id=:id and uniacid=:uniacid limit 1',array(':id'=>$raid,':uniacid'=>$uniacid));
                }else{
                    $raddress = pdo_fetch('select * from '.tablename('tapgo_tt_refund_address').' where id=:id and uniacid=:uniacid limit 1',array(':id'=>$raid,':uniacid'=>$uniacid));
                }
                if (empty($raddress)) {
                    $raddress = pdo_fetch('select * from '.tablename('tapgo_tt_refund_address').' where uniacid=:uniacid order by id desc limit 1',array(':uniacid'=>$uniacid));
                }
                ...
                pdo_update('tapgo_tt_groups_order_refund', $change_refund, array('id' => $item['refundid']));
                ...
                pdo_update('tapgo_tt_groups_order_refund', $refund_data, array('id'=>$item['refundid'], 'uniacid' => $uniacid));

                $order_data = array();
                $order_data['refundstate'] = 0;
                $order_data['status'] = 3;
                $order_data['refundtime'] = $time;
                pdo_update('tapgo_tt_groups_order', $order_data, array('id'=>$item['id'], 'uniacid' => $uniacid));
                p('groups')->sendTeamMessage($item['id'], true);
                ...
                pdo_update('tapgo_tt_groups_order', array('refundstate' => 0, 'status' => -1, 'refundtime' => $time), array('id' => $item['id'], 'uniacid' => $uniacid));
                ...
                pdo_update('tapgo_tt_groups_order_refund', array('reply' => $refundcontent, 'refundstatus' => -1, 'endtime' => $time), array('id' => $item['refundid']));
                p('groups')->sendTeamMessage($item['id'], true);

                plog('groups.refund', "订单退款拒绝 ID: {$item['id']} 订单号: {$item['orderno']} 原因: {$refundcontent}");
                pdo_update('tapgo_tt_groups_order', array('refundstate' => 0), array('id' => $item['id'], 'uniacid' => $uniacid));
        ...
        $refund_address = pdo_fetchall('select * from '.tablename('tapgo_tt_refund_address').' where uniacid=:uniacid',array(':uniacid'=>$_W['uniacid']));
