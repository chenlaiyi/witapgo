...
class Esheet_TapgoTtPage extends PluginWebPage {
    ...
    function getorder(){
        ...
        if($_W['ispost']){
            ...
            $sql = "select o.* , a.realname,a.mobile,a.province,a.city,a.area,a.street, d.dispatchname,m.nickname,r.refundstatus as refundstatus from " . tablename('tapgo_tt_groups_order') . " o"
                . " left join " . tablename('tapgo_tt_groups_order_refund') . " r on r.orderid=o.id and ifnull(r.refundstatus,-1)<>-1"
                . " left join " . tablename('tapgo_tt_member') . " m on m.openid=o.openid "
                . " left join " . tablename('tapgo_tt_member_address') . " a on o.addressid = a.id "
                . " left join " . tablename('tapgo_tt_dispatch') . " d on d.id = o.dispatchid "
            ...
            foreach ($list as &$value) {
                ...
                if (empty($value['more_spec'])){
                    $order_goods = pdo_fetchall('select * from ' . tablename('tapgo_tt_groups_goods') . ' where uniacid=:uniacid and id=:goodid ', array(':uniacid' => $_W['uniacid'], ':goodid' => $value['goodid']));



                }else{

                    $order_goods = pdo_fetchall('select og.*,g.* from ' . tablename('tapgo_tt_groups_order_goods') . ' og LEFT JOIN ' . tablename('tapgo_tt_groups_goods') . ' g on g.id = og.groups_goods_id '.' where og.uniacid=:uniacid and og.groups_order_id=:orderid ', array(':uniacid' => $_W['uniacid'], ':orderid' => $value['id']));
