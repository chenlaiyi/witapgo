...
class Batch_TapgoTtPage extends PluginWebPage {
    ...
    function getdata(){
        ...
        if($_W['ispost']){
            ...
            $sql = "select o.* ,a.realname ,m.nickname, d.dispatchname,m.nickname,r.refundstatus as refundstatus from " . tablename('tapgo_tt_groups_order') . " o"
                . " left join " . tablename('tapgo_tt_groups_order_refund') . " r on r.orderid=o.id and ifnull(r.refundstatus,-1)<>-1 and ifnull(r.refundstatus,-1)<> -2"
            ...
            foreach($orders as $i=>$order){
                ...
                if (empty($order['more_spec'])){
                    $order_goods = pdo_fetchall('select * from ' . tablename('tapgo_tt_groups_goods') . ' where uniacid=:uniacid and id=:goodid ', array(':uniacid' => $_W['uniacid'], ':goodid' => $order['goodid']));

                }else{

                    $order_goods = pdo_fetchall('select og.*,g.* from ' . tablename('tapgo_tt_groups_order_goods') . ' og LEFT JOIN ' . tablename('tapgo_tt_groups_goods') . ' g on g.id = og.groups_goods_id '.' where og.uniacid=:uniacid and og.groups_order_id=:orderid ', array(':uniacid' => $_W['uniacid'], ':orderid' => $order['id']));
