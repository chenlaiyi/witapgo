...
class Orders_TapgoTtPage extends PluginMobileLoginPage {
    ...
    function detail(){
        global $_W,$_GPC;
        $data = pdo_fetch("SELECT * FROM ".tablename('tapgo_tt_groups_set')." WHERE uniacid = :uniacid ",array(':uniacid'=>$_W['uniacid']));
        $openid = $_W['openid'];
        $uniacid = $_W['uniacid'];
        $orderid = intval($_GPC['orderid']);
        $teamid = intval($_GPC['teamid']);
        $condition = " and openid=:openid  and uniacid=:uniacid and id = :orderid and teamid = :teamid ";
        $order = pdo_fetch("select * from " . tablename('tapgo_tt_groups_order') . "
        ...
        //如果维过权则取维权信息
        if($order['refundid'] !=0){
            $refund = pdo_fetch("SELECT *  FROM " . tablename('tapgo_tt_groups_order_refund') . " WHERE orderid = :orderid and uniacid=:uniacid order by id desc", array(':orderid' => $order['id'], ':uniacid' => $_W['uniacid']));
        }

//        多规格团
        if($order['more_spec'] ==1){
            $option = pdo_fetch("select * from " . tablename('tapgo_tt_groups_order_goods') . "
...
//        阶梯团
        if($order['is_ladder'] ==1){
            $ladder = pdo_fetch("select * from " . tablename('tapgo_tt_groups_ladder') . "
        ...
        //商品信息
        $good = pdo_fetch("select * from " . tablename('tapgo_tt_groups_goods') . '
        ...
        if(!empty($order['isverify'])){
            ...
            //核销次数
            $verifytotal = pdo_fetchcolumn('select count(1) from ' . tablename('tapgo_tt_groups_verify') . " where orderid = :orderid and openid = :openid and uniacid = :uniacid and verifycode = :verifycode ",
                array(':orderid'=>$order['id'],':openid'=>$order['openid'],':uniacid'=>$order['uniacid'],':verifycode'=>$order['verifycode']));
            if($order['verifytype']==0){
                $verify = pdo_fetch("select isverify from ". tablename('tapgo_tt_groups_verify') ." where orderid = :orderid and openid = :openid and uniacid = :uniacid and verifycode = :verifycode ",
                    array(':orderid'=>$order['id'],':openid'=>$order['openid'],':uniacid'=>$order['uniacid'],':verifycode'=>$order['verifycode']));
            }
    ...
    function express() {
        ...
        $order = pdo_fetch("select * from " . tablename('tapgo_tt_groups_order') . ' where id=:id and uniacid=:uniacid and openid=:openid limit 1'
        ...
        //商品信息
        $goods = pdo_fetch("select *  from " . tablename('tapgo_tt_groups_goods') . "  where id=:id and uniacid=:uniacid ", array(':uniacid' => $uniacid, ':id' => $order['goodid']));
    ...
    function cancel() {
        ...
        try{
            $orderid = intval($_GPC['id']);
            $order = pdo_fetch("select id,orderno,openid,status,credit,teamid,groupnum,creditmoney,price,freight,pay_type,discount,success from " . tablename('tapgo_tt_groups_order') . ' where id=:id and uniacid=:uniacid and openid=:openid limit 1'
                , array(':id' => $orderid, ':uniacid' => $_W['uniacid'], ':openid' => $_W['openid']));
            $total = pdo_fetchcolumn('select count(1) from ' . tablename('tapgo_tt_groups_order') . "  where teamid = :teamid  "
            ...
            pdo_update('tapgo_tt_groups_order', array('status' => -1, 'canceltime' => time()), array('id' => $order['id'], 'uniacid' => $_W['uniacid']));
            //模板消息
    ...
    function delete() {
        ...
        //删除订单
        $orderid = intval($_GPC['id']);
        $order = pdo_fetch("select id,status from " . tablename('tapgo_tt_groups_order') . ' where id=:id and uniacid=:uniacid and openid=:openid limit 1'
            , array(':id' => $orderid, ':uniacid' => $_W['uniacid'], ':openid' => $_W['openid']));
        if (empty($order)) {
            show_json(0, '订单未找到!');
        }
        ...
        pdo_update('tapgo_tt_groups_order', array('deleted' => 1), array('id' => $order['id'], 'uniacid' => $_W['uniacid']));
        show_json(1);
    }
    /*
     * 订单列表
    ...
    function get_list(){
        ...
        $orders = pdo_fetchall("select o.id,o.orderno,o.createtime,o.price,o.more_spec,o.is_ladder,o.freight,o.ladder_id,o.specs,o.creditmoney,o.goodid,o.teamid,o.status,o.is_team,o.success,o.teamid,o.openid,
				...
				left join ".tablename('tapgo_tt_groups_ladder')." as l on l.id = o.ladder_id
				left join ".tablename('tapgo_tt_groups_order_goods')." as p on p.groups_order_id = o.id
				where 1 {$condition} order by o.createtime desc LIMIT " . ($pindex - 1) * $psize . ',' . $psize, $params);
        $total = pdo_fetchcolumn('select count(1) from ' . tablename('tapgo_tt_groups_order') . " as o where 1 {$condition}", $params);
...
//          阶梯团
        ...
        if( $goods['is_ladder'] == 1 ){
            $ladder = pdo_get( 'tapgo_tt_groups_ladder',array('goods_id' => $goodid,'uniacid' => $_W['uniacid']) );
        }
        if($_GPC['ladder_id']>0){
            $ladder = pdo_get( 'tapgo_tt_groups_ladder',array('id' => $_GPC['ladder_id'],'uniacid' => $_W['uniacid']) );
        }
//多规格团
        if( $goods['more_spec'] == 1) {
            $option_id = $_GPC['options_id'];
            if(!empty($option_id)) {
                $option = pdo_fetch('select * from ' . tablename('tapgo_tt_groups_goods_option') . ' where goods_option_id=:goods_option_id and groups_goods_id=:groups_goods_id and uniacid=:uniacid  limit 1', array(':uniacid' => $uniacid, ':groups_goods_id' => $goods['id'], ':goods_option_id' => $option_id));
            }
        }
        //购买是否关注公众号
        ...
        if($type == 'groups'){
            $ordernum = pdo_fetchcolumn('select count(1) from ' . tablename('tapgo_tt_groups_order') . " as o
...
//			}
        ...
        if(!empty($teamid)){
            $orders = pdo_fetchall("select * from " . tablename('tapgo_tt_groups_order') . '
            ...
            $num = pdo_fetchcolumn('select count(1) from ' . tablename('tapgo_tt_groups_order') . " as o where teamid = :teamid and status > :status and goodid = :goodid and uniacid = :uniacid ",
        ...
        //团长优惠设置
        $set = pdo_fetch("select discount,headstype,headsmoney,headsdiscount from ".tablename('tapgo_tt_groups_set')."
        ...
        if(!empty($goods['isverify'])){
            ...
            while (1) {
                $count = pdo_fetchcolumn('select count(*) from ' . tablename('tapgo_tt_groups_order') . ' where verifycode=:verifycode and uniacid=:uniacid limit 1', array(':verifycode' => $verifycode, ':uniacid' => $_W['uniacid']));
                if ($count <= 0) {
                    break;
                }
                $verifycode = "PT".random(8, true);
            ...
            //默认地址
            $address = pdo_fetch('select * from ' . tablename('tapgo_tt_member_address') . '
				where openid=:openid and deleted=0 and isdefault=1  and uniacid=:uniacid limit 1'
                , array(':uniacid' => $uniacid, ':openid' => $openid));

        }
        /*积分抵扣*/
        $creditdeduct = pdo_fetch("SELECT creditdeduct,groupsdeduct,credit,groupsmoney FROM" . tablename('tapgo_tt_groups_set') .  "WHERE uniacid = :uniacid ",array(':uniacid'=>$uniacid));
        ...
        if ($_W['ispost']) {
            ...
            if(intval($_GPC['aid'])>0 && !$isverify){
                //默认地址
                $order_address = pdo_fetch('select * from ' . tablename('tapgo_tt_member_address') . ' where id=:id and openid=:openid and uniacid=:uniacid   limit 1'
            ...
            $order_insert = pdo_insert('tapgo_tt_groups_order', $data);
            if(!$order_insert){
                $this->message('生成订单失败！');
            }
            ...
            if(empty($teamid) && $type=='groups'){
                pdo_update('tapgo_tt_groups_order',array('teamid' => $orderid), array('id' => $orderid));
            }
//				多规格商品
            if(!empty($orderid) && $goods['more_spec'] ==1){
                ...
                pdo_insert('tapgo_tt_groups_order_goods', $_data);
            }
    ...
    function finish() {
        ...
        if ($order['refundstate'] > 0 && !empty($order['refundid'])) {
            ...
            pdo_update('tapgo_tt_groups_order_refund', $change_refund, array('id' => $order['refundid'], 'uniacid' => $_W['uniacid']));
        }

        pdo_update('tapgo_tt_groups_order', array('status' => 3, 'finishtime' => time(), 'refundstate' => 0), array('id' => $order['id'], 'uniacid' => $_W['uniacid']));
