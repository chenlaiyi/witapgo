...
class Goods_TapgoTtPage extends PluginMobileLoginPage
{
    public function main()
    {
        ...
        $uniacid = $_W["uniacid"];
        $id = intval($_GPC["id"]);
        $groupsset = pdo_fetch("select description,groups_description,discount,headstype,headsmoney,headsdiscount,followbar from " . tablename("tapgo_tt_groups_set") . "where uniacid = :uniacid ", array( ":uniacid" => $uniacid ));
        ...
        if( $goods["more_spec"] == 1 ) 
        {
            ...
            $specArr = pdo_getall("tapgo_tt_goods_spec", array( "goodsid" => $group_goods["gid"], "uniacid" => $_W["uniacid"] ), array( "id", "title" ), "", array( "displayorder" => "desc" ));
            foreach( $specArr as $k => $v ) 
            {
                $specArr[$k]["item"] = pdo_getall("tapgo_tt_goods_spec_item", array( "uniacid" => $_W["uniacid"], "specid" => $v["id"] ), array( "id", "specid", "title", "thumb" ), "", array( "displayorder" => "desc" ));
            }
        }
    ...
    public function openGroups()
    {
        ...
        $groupsset = pdo_fetch("SELECT followbar FROM " . tablename("tapgo_tt_groups_set") . " WHERE uniacid = :uniacid ", array( ":uniacid" => $uniacid ));
        $goods = pdo_fetch("select * from " . tablename("tapgo_tt_groups_goods") . "\r\n\t\t\t\t\twhere id = :id and status = :status and uniacid = :uniacid and deleted = 0 order by displayorder desc", array( ":id" => $id, ":uniacid" => $uniacid, ":status" => 1 ));
        $goods["fightnum"] = pdo_fetchcolumn("select count(1) from " . tablename("tapgo_tt_groups_order") . " where goodid = :goodid and uniacid = :uniacid and deleted = 0 and is_team = 1 and status > 0 ", array( ":goodid" => $goods["id"], ":uniacid" => $uniacid ));
        $goods["fightnum"] = $goods["teamnum"] + $goods["fightnum"];
        $goods = set_medias($goods, "thumb");
        ...
        {
            $ladder = pdo_getall("tapgo_tt_groups_ladder", array( "goods_id" => $id, "uniacid" => $_W["uniacid"] ));
            $info = pdo_fetchall("SELECT count(ladder_id) as order_num FROM " . tablename("tapgo_tt_groups_order") . " WHERE `uniacid` = :uniacid and `openid`!=:openid and success = 0 and is_team =1 and status>0 and ladder_id >0 and goodid = :goodid group by ladder_id", array( ":uniacid" => $_W["uniacid"], "openid" => $_W["openid"], "goodid" => $id ));
        ...
        if( $goods["more_spec"] == 1 && $_GPC["more_spec"] == 1 ) 
        {
            ...
            $order_num = pdo_fetchcolumn("SELECT count(id) as order_num FROM " . tablename("tapgo_tt_groups_order") . " WHERE `uniacid` = :uniacid and `openid`!=:openid and success = 0 and status>0 and more_spec =1 and is_team =1 and `goodid`=:goodid ", array( ":uniacid" => $_W["uniacid"], "openid" => $_W["openid"], "goodid" => $id ));
        }

        if( $goods["is_ladder"] == 0 && $goods["more_spec"] == 0 ) 
        {
            $order_num = pdo_fetchcolumn("SELECT count(id) as order_num FROM " . tablename("tapgo_tt_groups_order") . " WHERE `uniacid` = :uniacid and `openid`!=:openid and success = 0 and is_team =1 and status>0 and `goodid`=:goodid ", array( ":uniacid" => $_W["uniacid"], "openid" => $_W["openid"], "goodid" => $id ));
        }

        $teams = pdo_fetchall("select * from " . tablename("tapgo_tt_groups_goods") . " where deleted = 0 and status = 1 and uniacid = :uniacid order by sales desc limit 4", array( ":uniacid" => $uniacid ));
        foreach( $teams as $key => $value ) 
        {
            $value["fightnum"] = pdo_fetchcolumn("select count(1) from " . tablename("tapgo_tt_groups_order") . " where goodid = :goodid and uniacid = :uniacid and deleted = 0 and is_team = 1 and status > 0 ", array( ":goodid" => $value["id"], ":uniacid" => $uniacid ));
    ...
    public function goodsCheck()
    {
        ...
        try
        {
            ...
            if( $goods["is_ladder"] == 1 ) 
            {
                ...
                {
                    $info = pdo_fetchall("SELECT ladder_id,count(ladder_id) as order_num FROM " . tablename("tapgo_tt_groups_order") . " WHERE `uniacid` = :uniacid and `openid`!=:openid and success = 0 and status>0 and ladder_id >0 and goodid = :goodid group by ladder_id", array( ":uniacid" => $_W["uniacid"], "openid" => $_W["openid"], "goodid" => $id ));
    ...
    public function get_option()
    {
        ...
        if( !empty($specArr) ) 
        {
            ...
            $goods_option = pdo_get("tapgo_tt_groups_goods_option", array( "specs" => $spec_id, "uniacid" => $_W["uniacid"] ));
