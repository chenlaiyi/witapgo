...
class Pay_TapgoTtPage extends PluginMobileLoginPage {
    ...
    function main(){
        ...
        $order = pdo_fetch("select o.*,g.title,g.status as gstatus,g.deleted as gdeleted,g.stock,g.more_spec from " . tablename('tapgo_tt_groups_order') . ' as o
        ...
        if(!empty($teamid)){
            ...
            $num = pdo_fetchcolumn('select count(1) from ' . tablename('tapgo_tt_groups_order') . " as o where teamid = :teamid and status > :status and uniacid = :uniacid ",
    ...
    function complete() {
        ...
        if(is_h5app() && empty($orderid)){
            ...
            $orderid = pdo_fetchcolumn("select id from " . tablename('tapgo_tt_groups_order') . ' where orderno=:orderno and uniacid=:uniacid and openid=:openid limit 1', array(':orderno' => $ordersn, ':uniacid' => $uniacid, ':openid' => $openid));
        }
        ...
        $order = pdo_fetch("select * from " . tablename('tapgo_tt_groups_order') . ' where id = :orderid and uniacid=:uniacid and openid=:openid'
    ...
    function orderstatus()
    {
        ...
        $order = pdo_fetch("select status from " . tablename('tapgo_tt_groups_order') . ' where id=:id and uniacid=:uniacid limit 1'
            , array(':id' => $orderid, ':uniacid' => $uniacid));
        show_json(1,$order);
    }
