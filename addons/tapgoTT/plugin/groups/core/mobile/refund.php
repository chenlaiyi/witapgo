...
class Refund_TapgoTtPage extends PluginMobileLoginPage {
    ...
    function main() {
        ...
        try{
            ...
            if ($order['refundstate'] > 0) {
                if (!empty($refundid)) {
                    $refund = pdo_fetch("select * from " . tablename('tapgo_tt_groups_order_refund') . ' where id=:id and uniacid=:uniacid and orderid=:orderid limit 1'
                    ...
                    if (!empty($refund['refundaddress'])) {
                        ...
                        if($refund['refundaddressid']){
                            $refund['refundaddress'] = pdo_fetch('select * from '.tablename('tapgo_tt_member_address').' where id = :id and uniacid=:uniacid',
