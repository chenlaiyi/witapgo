...
class Index_TapgoTtPage extends PluginMobilePage
{
	public function main()
	{
		global $_W;
		global $_GPC;
		$info = pdo_fetch('SELECT * FROM ' . tablename('tapgo_tt_open_plugin') . ' WHERE plugin = :plugin', array(':plugin' => 'open_messikefu'));
		...
		if (!is_error($redis)) {
			if ($redis->get($redis_key)) {
				...
				pdo_update('tapgo_tt_open_plugin', array('expirtime' => time() + 172800), array('id' => $info['id']));
			}
		}
