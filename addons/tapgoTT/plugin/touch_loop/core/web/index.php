...
class Index_TapgoTtPage extends PluginWebPage
{
    ...
    public function delete()
    {
        ...
        foreach ($id as $item) {
            $list = pdo_delete('tapgo_tt_touch_loop_list', array('id' => $item, 'uniacid' => $_W['uniacid']));
        }
        show_json(1, '删除成功');
    }
    ...
    public function status()
    {
        ...
        foreach ($id as $item) {
            pdo_update('tapgo_tt_touch_loop_list', array('status' => $status), array('id' => $item));
        }
        show_json(1, '操作成功');
    }
    ...
    public function isrecommand()
    {
        ...
        foreach ($id as $item) {
            pdo_update('tapgo_tt_touch_loop_list', array('isrecommand' => $isrecommand), array('id' => $item));
        }
        show_json(1, '操作成功');
    }
    ...
    public function query()
    {
        ...
        $goods = pdo_fetchall('SELECT id,title,thumb,thumb_url,marketprice,total
            FROM ' . tablename('tapgo_tt_goods') . "
             WHERE 1 {$condition} and id not in (select goods_id from " . tablename('tapgo_tt_touch_loop_list') . " where uniacid = :uniacid) ORDER BY displayorder DESC,id DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize, $params);
        $total = pdo_fetchcolumn('SELECT count(*) 
            FROM ' . tablename('tapgo_tt_goods') . "
             WHERE 1 {$condition} and id not in (select goods_id from " . tablename('tapgo_tt_touch_loop_list') . " where uniacid = :uniacid) ORDER BY displayorder DESC,id DESC ", $params);
    ...
    public function displayorder()
    {
        ...
        pdo_update('tapgo_tt_touch_loop_list', compact('sort'), array('id' => $_GPC['id'], 'uniacid' => $_W['uniacid']));
        show_json(1, '操作成功');
    }
}
