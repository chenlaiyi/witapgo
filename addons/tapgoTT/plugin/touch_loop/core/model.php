...
{
    const LISTTABLENAME = 'tapgo_tt_touch_loop_list';
    ...
    public function getList($uniacid = 0, $params = array(), $page = 0)
    {
        ...
        $list = pdo_fetchall('select t.id,t.status,t.isrecommand, g.title,g.thumb,g.id as gid,t.sort,t.content from ' . tablename(self::LISTTABLENAME) . ' AS t LEFT JOIN ' . tablename('tapgo_tt_goods') . ' AS g on g.id = t.goods_id and g.uniacid = t.uniacid where t.uniacid = :uniacid  ' . $params['andWhere'] . ' order by ' . self::ORDERBY . ' limit ' . ($page - 1) * self::PAGENUM . ',' . self::PAGENUM, array(':uniacid' => $uniacid));
        $total = pdo_fetchcolumn('select count(t.goods_id) from ' . tablename(self::LISTTABLENAME) . ' AS t LEFT JOIN ' . tablename('tapgo_tt_goods') . ' AS g on g.id = t.goods_id and g.uniacid = t.uniacid where t.uniacid = :uniacid ' . $params['andWhere'] . ' order by ' . self::ORDERBY, array(':uniacid' => $uniacid));
    ...
    {
        $item = (pdo_get('tapgo_tt_touch_loop_list', compact('uniacid', 'id')));
        if (empty($item)) {
            return false;
        }
        $item['thumbs'] = unserialize($item['thumbs']);
        $goods = pdo_get('tapgo_tt_goods', array('id' => $item['goods_id'], 'uniacid' => $uniacid));
    ...
    public function buildText($params, $text)
    {
        ...
        $font = TAPGO_TT_LOCAL . 'static/fonts/msyh.ttf';
