...
class Chicken_TapgoTtPage extends PluginMobilePage {
    /**
     * 当前数据表名称
     * @var string
     */
    ...
    public function surprised() {
        global $_W;
        // 清理无用彩蛋
        $surprised = new Surprised_TapgoTtPage();
    ...
    public function feeding() {
        ...
        if ($egg > 0 && count($eggs) > 0) {
            foreach ($eggs as $value) {
                ...
                $this->eggLog($value);
                // 添加日志
                $presentation = new Presentation_TapgoTtPage();
            ...
            foreach ($data as $key => $value) {
                ...
                switch ($surprised['category']) {
                    case '优惠券':
                        ...
                        //如果优惠券数量为0,不产生优惠券彩蛋
                        $gettotal = pdo_fetchcolumn('select count(*) from ' . tablename('tapgo_tt_coupon_data') . ' where couponid=:couponid and uniacid=:uniacid limit 1', array(':couponid' => $surprised['value'], ':uniacid' => $_W['uniacid']));
    ...
    public function getSurprised($method = false) {
        ...
        if ($query['category'] === '优惠券') {
            ...
            $info = pdo_get($table, $where, $fields);
            $gettotal = pdo_fetchcolumn('select count(*) from ' . tablename('tapgo_tt_coupon_data') . ' where couponid=:couponid and uniacid=:uniacid limit 1', array(':couponid' => $surprisedInfo['value'], ':uniacid' => $_W['uniacid']));
            $left_count = $info['total'] -  $gettotal;
            $left_count = intval($left_count);
            if($info['total']!=-1 && $left_count <= 0 )
            {
