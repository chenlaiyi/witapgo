...
class Open_FarmModel extends PluginModel {
    ...
    public function getAllMemberLevel() {
        global $_W, $_GPC;
        $field = "id,levelname";
        $sql = " SELECT {$field} FROM " . tablename('tapgo_tt_member_level') . " WHERE `uniacid` = {$_W['uniacid']} ";
    ...
    public function getAllTask() {
        ...
        $field = "id,title";
        $sql = " SELECT {$field} FROM " . tablename('tapgo_tt_task_list') . " WHERE `uniacid` = {$_W['uniacid']} AND `starttime` <= \"{$currentTime}\" AND `endtime` >= \"{$currentTime}\" ";
        $return = pdo_fetchall($sql);
        return $return;
    }
    ...
    public function getAllGoods() {
        global $_W, $_GPC;
        $field = "id,title";
        $sql = " SELECT {$field} FROM " . tablename('tapgo_tt_goods') . " WHERE `uniacid` = {$_W['uniacid']} AND `status` <= 1 ";
        $return = pdo_fetchall($sql);
        return $return;
    }
