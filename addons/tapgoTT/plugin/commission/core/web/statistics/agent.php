...
class Agent_TapgoTtPage extends PluginWebPage
{
	public function main()
	{
		...
		$sql = 'select dm.*,dm.nickname,dm.avatar,l.levelname,p.nickname as parentname,p.avatar as parentavatar from ' . tablename('tapgo_tt_member') . ' dm ' . ' left join ' . tablename('tapgo_tt_member') . ' p on p.id = dm.agentid ' . ' left join ' . tablename('tapgo_tt_commission_level') . ' l on l.id = dm.agentlevel' . ' left join ' . tablename('mc_mapping_fans') . ('f on f.openid=dm.openid and f.uniacid=' . $_W['uniacid']) . ' where dm.uniacid = ' . $_W['uniacid'] . (' and dm.isagent =1  ' . $condition . ' ORDER BY dm.agenttime desc');
		$sql .= ' limit ' . ($pindex - 1) * $psize . ',' . $psize;
		$list = pdo_fetchall($sql, $params);
		$total = pdo_fetchcolumn('select count(dm.id) from' . tablename('tapgo_tt_member') . ' dm  ' . ' left join ' . tablename('tapgo_tt_member') . ' p on p.id = dm.agentid ' . ' left join ' . tablename('mc_mapping_fans') . 'f on f.openid=dm.openid' . ' where dm.uniacid =' . $_W['uniacid'] . (' and dm.isagent =1 ' . $condition), $params);
...
';
	...
	public function user()
	{
		...
		if ($hasagent) {
			$total = pdo_fetchcolumn('select count(dm.id) from' . tablename('tapgo_tt_member') . ' dm ' . ' left join ' . tablename('tapgo_tt_member') . ' p on p.id = dm.agentid ' . ' left join ' . tablename('mc_mapping_fans') . 'f on f.openid=dm.openid' . ' where dm.uniacid =' . $_W['uniacid'] . ('  ' . $condition), $params);
			$list = pdo_fetchall('select dm.*,p.nickname as parentname,p.avatar as parentavatar  from ' . tablename('tapgo_tt_member') . ' dm ' . ' left join ' . tablename('tapgo_tt_member') . ' p on p.id = dm.agentid ' . ' left join ' . tablename('mc_mapping_fans') . ('f on f.openid=dm.openid  and f.uniacid=' . $_W['uniacid']) . ' where dm.uniacid = ' . $_W['uniacid'] . ('  ' . $condition . '   ORDER BY dm.agenttime desc limit ') . ($pindex - 1) * $psize . ',' . $psize, $params);
...
';
	...
	public function check()
	{
		...
		$members = pdo_fetchall('SELECT id,openid,nickname,realname,mobile,status FROM ' . tablename('tapgo_tt_member') . (' WHERE id in( ' . $id . ' ) AND uniacid=') . $_W['uniacid']);
		$time = time();
	...
	public function agentblack()
	{
		...
		foreach ($members as $member) {
			...
			if ($agentblack == 1) {
				pdo_update('tapgo_tt_member', array('isagent' => 1, 'status' => 0, 'agentblack' => 1), array('id' => $member['id']));
				plog('commission.agent.agentblack', '设置黑名单 <br/>分销商信息:  ID: ' . $member['id'] . ' /  ' . $member['openid'] . '/' . $member['nickname'] . '/' . $member['realname'] . '/' . $member['mobile']);
			}
			else {
				pdo_update('tapgo_tt_member', array('isagent' => 1, 'status' => 1, 'agentblack' => 0), array('id' => $member['id']));
