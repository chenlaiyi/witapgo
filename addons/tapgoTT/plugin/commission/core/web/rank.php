...
class Rank_TapgoTtPage extends PluginWebPage
{
	...
	{
		global $_W;
		$result = pdo_fetchall('SELECT openid FROM ' . tablename('tapgo_tt_member') . ' WHERE uniacid = :uniacid AND status=1 AND agentblack=0', array(':uniacid' => $_W['uniacid']));
		...
		if (!empty($openid)) {
			if (is_array($openid)) {
				foreach ($openid as $val) {
					...
					pdo_update('tapgo_tt_member', $data, array('openid' => $val, 'uniacid' => $_W['uniacid']));
				}
			...
			$data = array('commission_total' => $member['commission_total']);
			pdo_update('tapgo_tt_member', $data, array('openid' => $openid, 'uniacid' => $_W['uniacid']));
			show_json(1, $data);
		}
