    ...
    function notice() {
        ...
        if (isset($data['openid1'])) {
            if (!empty($data['openid1'])) {
                ...
                @$salers1 = pdo_fetchall("select id,nickname,avatar,openid from " . tablename('tapgo_tt_member') . ' where openid in (' . implode(",", $openids1) . ") and uniacid={$_W['uniacid']}");
            }
        }
        //分销商申请通知商家高级
        $salers3 = array();
        if (isset($data['openid3'])) {
            if (!empty($data['openid3'])) {
                ...
                /*echo '<pre>';
                var_dump($salers3);die;*//*echo '<pre>';
                var_dump($salers3);die;*/
            }
        }
...
//                高级
        ...
        $template_lists= pdo_fetchall('SELECT id,title,typecode FROM ' . tablename('tapgo_tt_member_message_template') . ' WHERE uniacid=:uniacid ', array(':uniacid' => $_W['uniacid']));
...
//
        ...
        $data['register_bottom_content'] = m('common')->html_to_images($data['register_bottom_content']);
        $data['applycontent'] = m('common')->html_to_images($data['applycontent']);
        $goods = false;
        if (!empty($data['become_goodsid'])) {
            $goods = pdo_fetchall('select id,title,thumb from ' . tablename('tapgo_tt_goods') . ' where id IN('.implode(',',$data['become_goodsid']).') and uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
        }
    ...
    function query(){
        ...
        $goodsids = pdo_fetchall('SELECT goodsids FROM ' . tablename('tapgo_tt_commission_level') . " WHERE uniacid=:uniacid", array('uniacid'=>$_W['uniacid']));
        $newarray = array();
