...
class Register_TapgoTtPage extends CommissionMobileLoginPage {
	...
	function main() {
		...
		if ($_W['ispost']) {
			...
			if ($icode > 0) {
                //华仔定制邀请码，分销商手机号
			    if(p('offic')){
                    $iagent = pdo_fetch("select * from ".tablename('tapgo_tt_member')." where mobile = :mobile and uniacid = :uniacid limit 1",array(":mobile"=>$icode,":uniacid"=>intval($_W['uniacid'])));
		...
		if (empty($set['become'])) {
			...
			if (empty($member['status']) || empty($member['isagent'])) {
				...
				if (!empty($member['uid'])) {
					//更新会员
					if(!empty($mobile)) {
						$mc_memberdata = array('realname' => $data['realname'], 'mobile' => $data['mobile']);
					} else {
						$mc_memberdata = array('realname' => $data['realname']);
					}

					m('member')->mc_update($member['uid'],$mc_memberdata);
				}
				*/
			...
			if ($goodscount <= 0) {
				...
				if(empty($member['isagent'])){
                    pdo_update('tapgo_tt_member', array('applyagenttime'=>time()), array('id' => $member['id']));
                }
			} else {
    		     $to_check_agent = true;
				$status = 1;
        ...
        $set['register_bottom_content'] = m('common')->html_to_images($set['register_bottom_content']);
        $set['applycontent'] = m('common')->html_to_images($set['applycontent']);
