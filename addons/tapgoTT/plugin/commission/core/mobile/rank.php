...
class Rank_TapgoTtPage extends CommissionMobileLoginPage
{
	...
	public function main()
	{
		...
		case '0':
			$user = pdo_fetch('SELECT id,uid,credit1,nickname,avatar,commission_total FROM ' . tablename('tapgo_tt_member') . ' WHERE uniacid = :uniacid AND openid = :openid', array(':uniacid' => $_W['uniacid'], ':openid' => $_W['mopenid']));
	...
	protected function commissionTotal($pindex, $psize)
	{
		...
		$result = pdo_fetchall('SELECT id,uid,nickname,avatar,commission_total FROM ' . tablename('tapgo_tt_member') . ' WHERE uniacid = :uniacid ORDER BY commission_total DESC ' . $limit, array(':uniacid' => $_W['uniacid']));
