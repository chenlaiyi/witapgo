...
require TAPGO_TT_PLUGIN . 'commission/core/page_mobile.php';

class Share_TapgoTtPage extends CommissionMobilePage {
	...
	function main() {
		...
		$qrcode = "";
		$path = $_W['siteroot'] . "addons/tapgo_ttv2/data/poster/" . $_W['uniacid'] . '/';
		if (!empty($goodsid)) {
			$goods = pdo_fetch('select * from ' . tablename('tapgo_tt_goods') . ' where uniacid=:uniacid and id=:id limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $goodsid));
			...
			if (!empty($goods)) {
				...
				if(p('poster') && !is_file(IA_ROOT."/addons/tapgo_ttv2/data/poster/" . $_W['uniacid'] . '/'.$md5.'.jpg')){

					$poster = pdo_fetch('select bg,data from '.tablename('tapgo_tt_poster').' where uniacid=:uniacid and type=3 and isdefault=1',array(':uniacid'=>$_W['uniacid']));
					if(!empty($poster)) {
						$md5 = md5(json_encode(array(
		...
		if (!$share_goods) {
			...
			if(p('poster') && !is_file(IA_ROOT."/addons/tapgo_ttv2/data/poster/" . $_W['uniacid'] . '/'.$md5.'.jpg')){
					$poster = pdo_fetch('select bg,`data` from '.tablename('tapgo_tt_poster').' where uniacid=:uniacid and `type`=:type and isdefault=1 limit 1',array(':uniacid'=>$_W['uniacid'], ':type'=>empty($set['qrcode']) ? 2: 4));
