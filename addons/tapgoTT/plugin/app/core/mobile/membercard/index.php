...
class Index_TapgoTtPage extends AppMobilePage
{
    ...
    public function getlist()
    {
        ...
        if ($cate == 'my') {
            ...
            $all_total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('tapgo_tt_member_card') . "where {$condition}", $params);
            $my_total = $all['total'];
        } else {
            ...
            $my_total = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename('tapgo_tt_member_card_history') . " 
        ...
        if(!empty($list)){
            foreach ($list as $key => $val){
                ...
                if($card['is_card_points']){
                    $rightsnum += 1;
                    $send_point = false;
                    $buysend_point = pdo_fetch("SELECT * FROM " . tablename('tapgo_tt_member_card_buysend') . " 
                ...
                if($card['is_card_coupon']){
                    ...
                    if($card_coupon['couponids']){
                        ...
                        foreach ($card_coupons as $key1 => $val1){
                            ...
                            $buysend_coupon = pdo_fetch("SELECT * FROM " . tablename('tapgo_tt_member_card_buysend') . " 
