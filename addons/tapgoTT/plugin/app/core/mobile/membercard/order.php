...
class Order_TapgoTtPage extends AppMobilePage
{
    public function create_order()
    {
        ...
        $wait_pay = pdo_fetch("SELECT * FROM " . tablename("tapgo_tt_member_card_order") . " \r\n\t\t\t\tWHERE status =0 " . $condition . " limit 1", $params);
        if( $wait_pay ) 
        {
            $result = pdo_update("tapgo_tt_member_card_order", $data, array( "id" => $wait_pay["id"], "uniacid" => $_W["uniacid"] ));
            $order_id = $wait_pay["id"];
        }
        else
        {
            $result = pdo_insert("tapgo_tt_member_card_order", $data);
    ...
    public function pay($params = array(  ), $mine = array(  ))
    {
        ...
        $order = pdo_fetch("SELECT * FROM " . tablename("tapgo_tt_member_card_order") . " \r\n\t\t\t\tWHERE 1 " . $condition . " limit 1", $params);
        if( empty($order) ) 
        {
        ...
        $log = pdo_fetch("SELECT * FROM " . tablename("core_paylog") . "\r\n\t\t WHERE `uniacid`=:uniacid AND `module`=:module AND `tid`=:tid limit 1", array( ":uniacid" => $uniacid, ":module" => "tapgo_ttv2", ":tid" => $order["orderno"] ));
        ...
        if( empty($log) ) 
        {
            $log = array( "uniacid" => $uniacid, "openid" => $member["uid"], "module" => "tapgo_ttv2", "tid" => $order["orderno"], "fee" => $order["total"], "status" => 0 );
        ...
        if( !empty($set["pay"]["wxapp"]) && 0 < $order["total"] && $this->iswxapp ) 
        {
            ...
            if( !is_error($res) ) 
            {
                ...
                if( !empty($res["package"]) && strexists($res["package"], "prepay_id=") ) 
                {
                    ...
                    pdo_update("tapgo_tt_member_card_order", array( "wxapp_prepay_id" => $prepay_id ), array( "id" => $order_id, "uniacid" => $_W["uniacid"] ));
                }
