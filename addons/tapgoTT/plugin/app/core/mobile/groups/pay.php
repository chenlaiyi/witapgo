...
class Pay_TapgoTtPage extends AppMobilePage
{
    public function main()
    {
        ...
        if( !empty($teamid) ) 
        {
            ...
            $num = pdo_fetchcolumn("select count(1) from " . tablename("tapgo_tt_groups_order") . " as o where teamid = :teamid and status > :status and uniacid = :uniacid ", array( ":teamid" => $teamid, ":status" => 0, ":uniacid" => $uniacid ));
        ...
        if( empty($log) ) 
        {
            ...
            pdo_insert("tapgo_tt_groups_paylog", $log);
            $plid = pdo_insertid();
        }
        ...
        if( !empty($set["pay"]["wxapp"]) && 0 < $order["price"] + $order["freight"] && $this->iswxapp ) 
        {
            ...
            if( !is_error($res) ) 
            {
                ...
                if( !empty($res["package"]) && strexists($res["package"], "prepay_id=") ) 
                {
                    ...
                    pdo_update("tapgo_tt_groups_order", array( "wxapp_prepay_id" => $prepay_id ), array( "id" => $orderid, "uniacid" => $_W["uniacid"] ));
                }
    ...
    public function complete()
    {
        ...
        $order = pdo_fetch("select * from " . tablename("tapgo_tt_groups_order") . " where id = :orderid and uniacid=:uniacid and openid=:openid", array( ":orderid" => $orderid, ":uniacid" => $uniacid, ":openid" => $openid ));
        if( empty($order) ) 
        {
        ...
        if( $type == "credit" ) 
        {
            ...
            pdo_update("tapgo_tt_groups_order", array( "pay_type" => "credit", "status" => 1, "paytime" => time(), "starttime" => time() ), array( "id" => $orderid ));
        ...
        if( $type == "wechat" ) 
        {
            ...
            {
                p("groups")->payResult($log["tid"], $type);
                pdo_update("tapgo_tt_groups_order", array( "pay_type" => "wechat", "status" => 1, "paytime" => time(), "starttime" => time(), "apppay" => (is_h5app() ? 1 : 0) ), array( "id" => $orderid ));
    ...
    public function orderstatus()
    {
        ...
        $uniacid = $_W["uniacid"];
        $orderid = intval($_GPC["id"]);
        $order = pdo_fetch("select status from " . tablename("tapgo_tt_groups_order") . " where id=:id and uniacid=:uniacid limit 1", array( ":id" => $orderid, ":uniacid" => $uniacid ));
        show_json(1, $order);
    }
    ...
    public function checkorder()
    {
        ...
        $teamid = intval($_GPC["teamid"]);
        $isteam = intval($_GPC["isteam"]);
        $order = pdo_fetch("select o.*,g.title,g.status as gstatus,g.deleted as gdeleted,g.stock from " . tablename("tapgo_tt_groups_order") . " as o\r\n\t\t\t\tleft join " . tablename("tapgo_tt_groups_goods") . " as g on g.id = o.goodid\r\n\t\t\t\twhere o.id = :id and o.uniacid = :uniacid order by o.createtime desc", array( ":id" => $orderid, ":uniacid" => $uniacid ));
        if( empty($order) ) 
        {
            show_json(0, "订单未找到！");
        }
