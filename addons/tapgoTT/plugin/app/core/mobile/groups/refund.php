...
class Refund_TapgoTtPage extends AppMobilePage
{
    protected function globalData()
    {
        ...
        $verifytotal = pdo_fetchcolumn("select count(1) from " . tablename("tapgo_tt_groups_verify") . " where orderid = :orderid and openid = :openid and uniacid = :uniacid and verifycode = :verifycode ", array( ":orderid" => $order["id"], ":openid" => $order["openid"], ":uniacid" => $order["uniacid"], ":verifycode" => $order["verifycode"] ));
        if( $order["status"] == 0 ) 
        {
            $_err = "订单未付款，不能申请退款!";
        }
    ...
    public function main()
    {
        ...
        if( 0 < $order["refundstate"] ) 
        {
            if( !empty($refundid) ) 
            {
                $refund = pdo_fetch("select * from " . tablename("tapgo_tt_groups_order_refund") . " where id=:id and uniacid=:uniacid and orderid=:orderid limit 1", array( ":id" => $refundid, ":uniacid" => $uniacid, ":orderid" => $orderid ));
                ...
                else
                {
                    ...
                    {
                        $refund["refundaddress"] = pdo_fetch("select * from " . tablename("tapgo_tt_member_address") . " where id = :id and uniacid=:uniacid", array( ":id" => $refund["refundaddressid"], ":uniacid" => $_W["uniacid"] ));
                    }
                    else
                    {
                        $refund["refundaddress"] = pdo_fetch("select * from " . tablename("tapgo_tt_refund_address") . " where uniacid=:uniacid order by id desc limit 1", array( ":uniacid" => $uniacid ));
                    }
    ...
    public function submit()
    {
        ...
        if( $order["refundstate"] == 0 ) 
        {
            ...
            pdo_insert("tapgo_tt_groups_order_refund", $refund);
            $refundid = pdo_insertid();
            pdo_update("tapgo_tt_groups_order", array( "refundid" => $refundid, "refundstate" => $refundstate ), array( "id" => $orderid, "uniacid" => $uniacid ));
        }
        else
        {
            pdo_update("tapgo_tt_groups_order", array( "refundstate" => $refundstate ), array( "id" => $orderid, "uniacid" => $uniacid ));
            pdo_update("tapgo_tt_groups_order_refund", $refund, array( "id" => $refundid, "uniacid" => $uniacid ));
        }
    ...
    public function cancel()
    {
        ...
        pdo_update("tapgo_tt_groups_order_refund", $change_refund, array( "id" => $refundid, "uniacid" => $uniacid ));
        pdo_update("tapgo_tt_groups_order", array( "refundstate" => 0 ), array( "id" => $orderid, "uniacid" => $uniacid ));
        return app_json();
    }
    ...
    public function receive()
    {
        ...
        pdo_update("tapgo_tt_groups_order_refund", $refund_data, array( "id" => $refundid, "uniacid" => $uniacid ));
        $order_data = array(  );
        $order_data["refundstate"] = -1;
        $order_data["refundtime"] = $time;
        $order_data["status"] = 3;
        $order_data["finishtime"] = $time;
        pdo_update("tapgo_tt_groups_order", $order_data, array( "id" => $orderid, "uniacid" => $uniacid ));
        return app_json();
    }
