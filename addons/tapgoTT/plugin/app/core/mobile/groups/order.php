...
class Order_TapgoTtPage extends AppMobilePage
{
...
//        $grodiyform = array();
        ...
        if( $order['more_spec'] == 1 ){
            $order['option'] = pdo_get( 'tapgo_tt_groups_order_goods' ,array( 'groups_order_id' => $order['id'] )  );
        }

        //如果是阶梯团
        if( $order['is_ladder'] == 1 ){
            $order['ladder'] = pdo_get( 'tapgo_tt_groups_ladder' , array( 'id' => $order['ladder_id'] ) );
        }
        ...
        //商品是否禁止退换货
        $goodRefund = false;
        $groupsSet = pdo_fetch("select goodsid,refundday from " . tablename('tapgo_tt_groups_set') . 'where uniacid = :uniacid ',array(':uniacid' => $uniacid));
    ...
    public function create_order()
    {
        ...
        //规格
        $groups_option = pdo_get( 'tapgo_tt_groups_goods_option' , array('id' => $groups_option_id) );
        ...
        if( $goods['is_ladder'] == 1 && $type == 'groups' ){
            ...
            $is_ladder = 1;
            //查询阶梯数据
            $ladder = pdo_get( 'tapgo_tt_groups_ladder' , array('id' => $ladder_id) );

            $sql = 'select count(1) from ' . tablename('tapgo_tt_groups_order') . " as o
        ...
        if(!empty($teamid)){
            ...
            if( $goods['is_ladder'] == 0 ){
                $num = pdo_fetchcolumn('select count(1) from ' . tablename('tapgo_tt_groups_order') . " where teamid = :teamid and status > :status and goodid = :goodid and uniacid = :uniacid ",
                ...
                $num = pdo_fetchcolumn('select count(1) from ' . tablename('tapgo_tt_groups_order') . " where teamid = :teamid and status > :status and goodid = :goodid and uniacid = :uniacid and ladder_id = :ladder_id",
...
//自定义表单
        ...
        if ($_W['ispost'] == true) {
            ...
            if( !empty($goods['more_spec']) ){
                ...
                pdo_insert( 'tapgo_tt_groups_order_goods' , $groups_order_goods );
            }
    ...
    function cancel() {
        global $_W, $_GPC;
            ...
            pdo_update('tapgo_tt_groups_order', array('status' => -1, 'canceltime' => time() , 'cancel_reason' => $cancel_reason), array('id' => $order['id'], 'uniacid' => $_W['uniacid']));
            //模板消息
            p('groups')->sendTeamMessage($orderid);
