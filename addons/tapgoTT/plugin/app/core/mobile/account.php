...
class Account_TapgoTtPage extends AppMobilePage
{
    protected $key = "";
    protected $expire = 0;
    ...
    public function login()
    {
        ...
        $member = pdo_fetch("select id,openid,mobile,pwd,salt,avatar,nickname from " . tablename("tapgo_tt_member") . " where mobile=:mobile and uniacid=:uniacid limit 1", array( ":mobile" => $mobile, ":uniacid" => $_W["uniacid"] ));
        if( empty($member) ) 
        {
    ...
    public function register()
    {
        ...
        $key_time = "__tapgo_ttv2_member_verifycodesendtime_" . $_W["uniacid"];
        ...
        $member = pdo_fetch("select id,openid,mobile,pwd,salt from " . tablename("tapgo_tt_member") . " where mobile=:mobile and uniacid=:uniacid limit 1", array( ":mobile" => $mobile, ":uniacid" => $_W["uniacid"] ));
        ...
        if( empty($salt) ) 
        {
            ...
            while( 1 ) 
            {
                $count = pdo_fetchcolumn("select count(*) from " . tablename("tapgo_tt_member") . " where salt=:salt limit 1", array( ":salt" => $salt ));
                if( $count <= 0 ) 
                {
                    break;
                }
        ...
        {
            pdo_update("tapgo_tt_member", array( "openid" => $openid, "salt" => $salt, "pwd" => md5($pwd . $salt), "mobileverify" => 1 ), array( "id" => $member["id"] ));
        }
    ...
    public function changepwd()
    {
        ...
        pdo_update("tapgo_tt_member", array( "salt" => $salt, "pwd" => md5($pwd . $salt) ), array( "id" => $member["id"] ));
    ...
    public function changemobile()
    {
        ...
        $newmember = pdo_fetch("select id,openid,mobile,pwd,salt from " . tablename("tapgo_tt_member") . " where mobile=:mobile and uniacid=:uniacid limit 1", array( ":mobile" => $newmobile, ":uniacid" => $_W["uniacid"] ));
        if( empty($newmember) ) 
        {
            return app_error(AppError::$UserMobileExists);
        }

        pdo_update("tapgo_tt_member", array( "mobile" => $newmobile ), array( "id" => $member["id"] ));
