...
class Merch_TapgoTtPage extends AppMobilePage
{
	public function get_list()
	{
		...
		if ($merch_plugin && $merch_data['is_openmerch']) {
			...
			//增加城市选择
			if (pdo_fieldexists('tapgo_tt_merch_user', 'city')) {
				$tmp = pdo_fetchall("select distinct(province),city from ".tablename('tapgo_tt_merch_user')." where uniacid=:uniacid and status=1 order by province,city", array(':uniacid'=>$_W['uniacid']));
				if(!empty($tmp)) {
					$citysel = true;
	...
	public function get_detail()
	{
		...
		$merch_data = m('common')->getPluginset('merch');
		if ($merch_plugin && $merch_data['is_openmerch']) {
			$merch = pdo_fetch('select * from ' . tablename('tapgo_tt_merch_user') . ' where id=:merchid and uniacid=:uniacid Limit 1', array(':uniacid' => $_W['uniacid'], ':merchid' => $id));
			$merch['logo'] = tomedia($merch['logo']);
			$params = array(':merchid' => $id, ':uniacid' => $_W['uniacid']);
			$merch['allgoodsnum'] = pdo_fetchcolumn('select count(*) from ' . tablename('tapgo_tt_goods') . ' where uniacid=:uniacid and merchid=:merchid and status=1', $params);
			$merch['recgoodsnum'] = pdo_fetchcolumn('select count(*) from ' . tablename('tapgo_tt_goods') . ' where uniacid=:uniacid and merchid=:merchid and status=1 and isrecommand=1', $params);
			$merch['newgoodsnum'] = pdo_fetchcolumn('select count(*) from ' . tablename('tapgo_tt_goods') . ' where uniacid=:uniacid and merchid=:merchid and status=1 and isnew=1', $params);
			if(file_exists('../addons/tapgo_ttv2/storebg.png')) $merch['storebg'] = $_W['siteroot'].'/addons/tapgo_ttv2/storebg.png';
	...
	public function apply()
	{
		...
		$reg = pdo_fetch('select * from ' . tablename('tapgo_tt_merch_reg') . ' where openid=:openid and uniacid=:uniacid limit 1', array(':openid' => $_W['openid'], ':uniacid' => $_W['uniacid']));
		$user = false;
		if (!(empty($reg['status']))) 
		{
		...
		if ($_W['ispost']) 
		{
			...
			$usercount1 = pdo_fetchcolumn('select count(1) from ' . tablename('tapgo_tt_merch_reg') . ' where ' . $where1 . ' limit 1', $params1);
			$where2 = ' username=:username';
			$params2 = array(':username' => $uname);
			$usercount2 = pdo_fetchcolumn('select count(1) from ' . tablename('tapgo_tt_merch_account') . ' where ' . $where2 . ' limit 1', $params2);
			...
			{
				$data['applytime'] = time();
				pdo_insert('tapgo_tt_merch_reg', $data);
			}
			else 
			{
				pdo_update('tapgo_tt_merch_reg', $data, array('id' => $reg['id']));
			}
