...
require TAPGO_TT_PLUGIN . 'app/core/page_mobilev2.php';
class Sign_TapgoTtPage extends AppMobilePage
{
	...
	public function getRecords()
	{
		...
		$sql = 'SELECT COUNT(*) FROM ' . tablename('tapgo_tt_sign_records') . ' log where ' . $condition;
		$total = pdo_fetchcolumn($sql, $params);
		$list = array();

		if (!empty($total)) {
			$sql = 'SELECT * FROM ' . tablename('tapgo_tt_sign_records') . ' where ' . $condition . ' ORDER BY `time` DESC LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize;
