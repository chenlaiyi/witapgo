...
class Index_TapgoTtPage extends AppMobilePage
{
    ...
    public function get_goods()
    {
        ...
        $sql = "select tg.id,tg.goodsid, tg.price, g.title,g.thumb,g.thumb_url,g.hasoption,g.marketprice,g.productprice,tg.commission1,tg.commission2,tg.commission3,tg.total from " . tablename("tapgo_tt_seckill_task_goods") . " tg  \n                  left join " . tablename("tapgo_tt_goods") . " g on tg.goodsid = g.id \n                  where tg.taskid=:taskid and tg.roomid=:roomid and tg.timeid=:timeid and tg.uniacid=:uniacid  group by tg.goodsid order by tg.displayorder asc ";
        ...
        foreach( $goods as &$g ) 
        {
            ...
            if( $g["hasoption"] ) 
            {
                $total = 0;
                $count = 0;
                $options = pdo_fetchall("select tg.id,tg.goodsid,tg.optionid,tg.price,g.title,g.marketprice,g.productprice,tg.commission1,tg.commission2,tg.commission3,tg.total from " . tablename("tapgo_tt_seckill_task_goods") . "  tg  left join " . tablename("tapgo_tt_goods") . " g on tg.goodsid = g.id  where tg.timeid=:timeid and tg.taskid=:taskid and tg.timeid=:timeid  and tg.goodsid=:goodsid and  tg.uniacid =:uniacid ", array( ":timeid" => $time["id"], ":taskid" => $taskid, ":goodsid" => $g["goodsid"], ":uniacid" => $_W["uniacid"] ));
    ...
    public function deleteSeckill()
    {
        ...
        foreach( $keys as $key ) 
        {
            ...
            if( !empty($orders) ) 
            {
                ...
                {
                    $o = pdo_fetch("select  id,openid,deductcredit2,ordersn,isparent,deductcredit,deductprice,status   from " . tablename("tapgo_tt_order") . " where id=:id  limit 1", array( ":id" => $orderid ));
