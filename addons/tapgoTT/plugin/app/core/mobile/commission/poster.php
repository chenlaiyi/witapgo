...
class Poster_TapgoTtPage extends Base_TapgoTtPage
{
    public function main()
    {
        global $_W;
        $list = pdo_fetchall("SELECT id FROM " . tablename("tapgo_tt_wxapp_poster") . " WHERE uniacid=:uniacid AND status=1 ORDER BY displayorder ASC, id DESC", array( ":uniacid" => $_W["uniacid"] ));
        if( empty($list) ) 
        {
    ...
    public function getimage()
    {
        ...
        $poster = pdo_fetch("SELECT * FROM " . tablename("tapgo_tt_wxapp_poster") . " WHERE id=:id AND uniacid=:uniacid LIMIT 1", array( ":id" => $id, ":uniacid" => $_W["uniacid"] ));
        if( empty($poster) ) 
        {
        ...
        $path = IA_ROOT . "/addons/tapgo_ttv2/data/poster_wxapp/commission/" . $_W["uniacid"] . "/";
        if( !is_dir($path) ) 
        {
            load()->func("file");
            mkdirs($path);
    ...
    {
        global $_W;
        return $_W["siteroot"] . "addons/tapgo_ttv2/data/poster_wxapp/commission/" . $_W["uniacid"] . "/" . $filename . "?v=" . time();
    }
