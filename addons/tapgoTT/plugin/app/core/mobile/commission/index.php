...
class Index_TapgoTtPage extends Base_TapgoTtPage {
    ...
    public function main() {
        ...
        //提现明细
        $member['applycount'] = pdo_fetchcolumn('select count(id) from ' . tablename('tapgo_tt_commission_apply') . ' where mid=:mid and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':mid' => $member['id']));
...
//        分销中心团队分红入口
        ...
        $posercount = pdo_fetchcolumn('SELECT COUNT(id) FROM '. tablename('tapgo_tt_wxapp_poster'). ' WHERE uniacid=:uniacid AND status=1', array(':uniacid'=>$_W['uniacid']));
    ...
    public function withdraw() {
        ...
        $agentid = $member['id'];
        if (!empty($agentid)) {
            $data = pdo_fetch('select sum(deductionmoney) as sumcharge from ' . tablename('tapgo_tt_commission_log') .' where mid=:mid and uniacid=:uniacid  limit 1', array(':uniacid' => $_W['uniacid'], ':mid' => $agentid));
    ...
    public function apply() {
        ...
        if ($level >= 1) {
            ...
            $level1_orders = pdo_fetchall('select distinct o.id from ' . tablename('tapgo_tt_order') . ' o '
        ...
        if ($level >= 2) {
            if ($member['level1'] > 0) {
                ...
                $level2_orders = pdo_fetchall('select distinct o.id from ' . tablename('tapgo_tt_order') . ' o '
                    . ' left join  ' . tablename('tapgo_tt_order_goods') . ' og on og.orderid=o.id '
        ...
        if ($level >= 3) {
            if ($member['level2'] > 0) {
                $level3_orders = pdo_fetchall('select distinct o.id from ' . tablename('tapgo_tt_order') . ' o '
                    . ' left join  ' . tablename('tapgo_tt_order_goods') . ' og on og.orderid=o.id '
        ...
        foreach ($orderids as $o) {
            ...
            $goods = pdo_fetchall("SELECT "
                ...
                . "og.content1,og.content2,og.content3 from " . tablename('tapgo_tt_order_goods') . " og"
        ...
        if($this->set['cashother'] == 1) {
            ...
            if($this->set['cashcard'] == 1) {
                ...
                $banklist = pdo_fetchall("SELECT * FROM " . tablename('tapgo_tt_commission_bank') . " WHERE 1 {$condition}  ORDER BY displayorder DESC", $params);
            }
        }
...
//            unset($_SESSION['commission_apply_token']);
            ...
            //申请的ids
            foreach ($orderids as $o) {
                pdo_update('tapgo_tt_order_goods', array('status' . $o['level'] => 1, 'applytime' . $o['level'] => $time), array('orderid' => $o['orderid'], 'uniacid' => $_W['uniacid']));
            }
            ...
            pdo_insert('tapgo_tt_commission_apply', $apply);
...
//        }
        ...
        if ($_W['ispost']) {
            ...
            if ($template_flag == 1) {
                ...
                pdo_update('tapgo_tt_member', $m_data, array('id' => $member['id']));
                ...
                pdo_update('tapgo_tt_member', $data, array('id' => $member['id']));
                if ($become_check == 1) {
        ...
        if (empty($set['become'])) {
            ...
            $goods = pdo_fetchall('select id,title,thumb,marketprice from' . tablename('tapgo_tt_goods') . ' where id IN ('.implode(',',$set['become_goodsid']).') and uniacid=:uniacid ', array(':uniacid' => $_W['uniacid']));
            $goodscount = pdo_fetchcolumn('select count(*) from ' . tablename('tapgo_tt_order_goods') . ' og '
        ...
        $res = array(
            'set'=>array(
                'texts' => $this->set['texts'],
                'regbg' => empty($set['regbg'])? $_W['siteroot'].'addons/tapgo_ttv2/plugin/commission/template/mobile/default/static/images/bg.png':$set['regbg'],
                'become' => (int)$set['become'],
                'open_protocol' => $set['open_protocol'],
                'applytitle' => $set['applytitle'],
                'applycontent' => m('common')->html_to_images($set['applycontent']),
                ...
                'register_bottom_content' => m('common')->html_to_images($set['register_bottom_content']),
