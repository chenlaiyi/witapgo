	...
	public function main()
	{
		...
		if (!$friendcouponModel->validateActivity($_GPC['id'])) {
			if ($friendcouponModel->errno) {
				if ($isShare) {
					$takePartInUsers = pdo_fetchall('select openid from ' . tablename('tapgo_tt_friendcoupon_data') . (' where uniacid = ' . $_W['uniacid'] . ' and headerid = ' . $share_id));
				}
				else {
					$takePartInUsers = pdo_fetchall('select openid from' . tablename('tapgo_tt_friendcoupon_data') . (' where uniacid = ' . $_W['uniacid'] . ' and activity_id = ' . $activitySetting['id']));
				}
...
', $activitySetting['desc']);
	...
	public function receive()
	{
		...
		pdo_update('tapgo_tt_friendcoupon_data', array('openid' => $user['openid'], 'avatar' => $user['avatar'], 'nickname' => $user['nickname'], 'receive_time' => time(), 'form_id' => $form_id), array('id' => $headerid));
	...
	public function divide()
	{
		...
		if (!$currentActivityInfo) {
			$coupons = pdo_fetchall('select * from ' . tablename('tapgo_tt_friendcoupon_data') . ' where uniacid = :uniacid and activity_id = :activity_id and headerid = :headerid', array(':uniacid' => $_W['uniacid'], ':activity_id' => $activity['id'], ':headerid' => $share_id));

			foreach ($coupons as $coupon) {
				if (!$coupon['openid']) {
					pdo_update('tapgo_tt_friendcoupon_data', array('openid' => $user['openid'], 'status' => 0, 'avatar' => $user['avatar'], 'nickname' => $user['nickname'], 'receive_time' => time(), 'form_id' => $form_id), array('id' => $coupon['id']));
					break;
				}
			}

			$overPlus = pdo_fetchcolumn('select count(1) from ' . tablename('tapgo_tt_friendcoupon_data') . ' where uniacid = :uniacid and headerid = :headerid and activity_id = :activity_id and openid = \'\'', array(':uniacid' => $_W['uniacid'], ':headerid' => $share_id, ':activity_id' => $activity_id));

			if ($overPlus) {
				return app_json(array('message' => '瓜分成功！'));
			}

			pdo_update('tapgo_tt_friendcoupon_data', array('status' => 1), array('activity_id' => $activity_id, 'headerid' => $share_id));
			$sql = 'select f.*, fd.enough, fd.id as friendcouponid,fd.deduct,fd.form_id,fd.openid,fd.enough from' . tablename('tapgo_tt_friendcoupon_data') . ' fd ' . ' left join ' . tablename('tapgo_tt_friendcoupon') . ' f on fd.activity_id = f.id' . ' where fd.uniacid = :uniacid and fd.headerid = :headerid and fd.activity_id = :activity_id';
			...
			foreach ($coupons as $coupon) {
				...
				if ($sendResult['errcode'] === 0) {
					pdo_update('tapgo_tt_friendcoupon_data', array('is_send' => 1), array('id' => $coupon['friendcouponid']));
				}
			}
	...
	public function more()
	{
		...
		if (empty($share_id)) {
			$currentTakePartActivity = pdo_fetch('select * from ' . tablename('tapgo_tt_friendcoupon_data') . ' where uniacid = :uniacid and openid = :openid and activity_id = :activity_id', array(':uniacid' => $_W['uniacid'], ':openid' => $openid, ':activity_id' => $activity_id));
			$share_id = $currentTakePartActivity['headerid'];
		}

		$list = pdo_fetchall('select avatar,nickname,deduct from ' . tablename('tapgo_tt_friendcoupon_data') . ' where uniacid = :uniacid and activity_id = :activity_id and headerid = :headerid and openid <> \'\' limit ' . ($pindex - 1) . ',' . $psize, array(':uniacid' => $_W['uniacid'], ':headerid' => $share_id, ':activity_id' => $activity_id));
