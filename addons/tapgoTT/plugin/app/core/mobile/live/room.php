...
class Room_TapgoTtPage extends Base_TapgoTtPage
{
    ...
    function get_list()
    {
        ...
        $list = pdo_fetchall('SELECT * FROM ' . tablename('tapgo_tt_wxlive') . " WHERE 1 AND {$condition}" . " LIMIT " . ($pindex - 1) * $psize . ',' . $psize, $params);
        ...
        if (!empty($list)) {
            foreach ($list as &$row) {
                ...
                if ($row['local_live_status'] == -1){
                    $replay = pdo_fetchall('SELECT * FROM ' . tablename('tapgo_tt_wxlive_back') . " WHERE room_id=" .$row['room_id']);
                    if (!empty($replay)){
                        $row['replay_status'] = 1;
                    }
                }
        ...
        $total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('tapgo_tt_wxlive') . " WHERE 1 and {$condition}", $params);
    ...
    function get_replay()
    {
        ...
        $list = pdo_fetchall('SELECT * FROM ' . tablename('tapgo_tt_wxlive_back') . " WHERE room_id= " .$_GPC['id']." AND uniacid=".$uniacid);
