...
class Index_TapgoTtPage extends AppMobilePage
{
    ...
    public function main()
    {
        ...
        $item = pdo_fetch("SELECT * FROM" . tablename("tapgo_tt_quick") . " WHERE id=:id AND uniacid=:uniacid", array(':id' => $id, ':uniacid' => $_W['uniacid']));
            ...
            if (!empty($data['datas'])) {
                foreach ($data['datas'] as $index => $d) {
                    ...
                    if ($datatype == 0) {
                        ...
                        if (!empty($groupid)) {
                            $group = pdo_fetch("SELECT * FROM " . tablename('tapgo_tt_goods_group') . " WHERE id=:id and uniacid=:uniacid and merchid=:merchid and enabled=1 limit 1 ", array(':id' => $groupid, ':uniacid' => $_W['uniacid'], ':merchid' => $merchid));
            ...
            if (!empty($data['datas'])) {
                foreach ($data['datas'] as $index => $d) {
                    ...
                    if ($d['datatype'] == 2 && !empty($d['groupid'])) {
                        $group = pdo_fetch("SELECT * FROM " . tablename('tapgo_tt_goods_group') . " WHERE id=:id and uniacid=:uniacid and merchid=:merchid and enabled=1 limit 1 ", array(':id' => $d['groupid'], ':uniacid' => $_W['uniacid'], ':merchid' => $merchid));
                        $goodsids = $group['goodsids'];
    ...
    public function clearCart()
    {
        ...
        $quickid = intval($_GPC['quickid']);
        $tablename = empty($quickid) ? "tapgo_tt_member_cart" : "tapgo_tt_quick_cart";
    ...
    public function update()
    {
        ...
        $goods = pdo_fetch('select id,maxbuy,usermaxbuy,minbuy,total,marketprice,diyformid,diyformtype,diyfields, isverify, `type`,merchid, cannotrefund from ' . tablename('tapgo_tt_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $goodsid, ':uniacid' => $_W['uniacid']));
        if (empty($goods)) {
            return app_error(AppError::$OrderCreateNoGoods);
        }
        if ($optionid > 0) {
            $optionInfo = pdo_fetch('select id,stock from' . tablename('tapgo_tt_goods_option') . 'where id=:id limit 1', array(":id" => $optionid));
        }
