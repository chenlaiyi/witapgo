...
class Index_TapgoTtPage extends AppMobilePage {
    ...
    public function getareas(){
        global $_W;
        $set = m('util')->get_area_config_set();
        $path = TAPGO_TT_PATH."static/js/dist/area/Area.xml";
        $path_full = TAPGO_TT_STATIC. "js/dist/area/Area.xml";
        if(!empty($set['new_area'])){
            $path = TAPGO_TT_PATH."static/js/dist/area/AreaNew.xml";
            $path_full = TAPGO_TT_STATIC. "js/dist/area/AreaNew.xml";
        }
    ...
    public function getstreet(){
        ...
        if(!empty($citycode) && !empty($areacode)){
            ...
            $path = TAPGO_TT_STATIC."js/dist/area/list/".$city2."/".$citycode.".xml";
