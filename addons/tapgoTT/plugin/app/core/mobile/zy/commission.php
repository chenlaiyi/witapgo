...
class Commission_TapgoTtPage extends AppMobilePage
{
	public function shopset()
	{
		...
		if ($_GPC['save']==1) {
			...
			if (empty($shop['id'])) {
				pdo_insert('tapgo_tt_commission_shop', $shopdata);
			}
			else {
				pdo_update('tapgo_tt_commission_shop', $shopdata, array('id' => $shop['id']));
			}
	...
	public function rank()
	{
		...
		case '0':
			$user = pdo_fetch('SELECT id,uid,credit1,nickname,avatar,commission_total FROM ' . tablename('tapgo_tt_member') . ' WHERE uniacid = :uniacid AND openid = :openid', array(':uniacid' => $_W['uniacid'], ':openid' => $_W['openid']));
			$user['paiming'] = pdo_fetchcolumn('SELECT count(*) FROM ' . tablename('tapgo_tt_member') . ' WHERE uniacid = :uniacid AND commission_total >= :commission_total', array(':uniacid' => $_W['uniacid'], ':commission_total' => $user['commission_total']));
			$commission_title = '累计佣金';
			break;

		case '1':
			$user = pdo_fetch('SELECT id,uid,credit1,nickname,avatar,commission_total FROM ' . tablename('tapgo_tt_member') . ' WHERE uniacid = :uniacid AND openid = :openid', array(':uniacid' => $_W['uniacid'], ':openid' => $_W['openid']));
			$user['commission_total'] = pdo_fetchcolumn('SELECT SUM(commission_pay) FROM ' . tablename('tapgo_tt_commission_apply') . ' WHERE uniacid = :uniacid AND mid = :mid and status=3 ', array(':uniacid' => $_W['uniacid'], ':mid' => $user['id']));
			if(!$user['commission_total']) $user['commission_total'] = '0.00';
			$result = pdo_fetchall('SELECT c.id,c.mid,SUM(c.commission_pay)  as commission_pay,m.nickname,m.avatar FROM ' . tablename('tapgo_tt_commission_apply') . ' c LEFT JOIN ' . tablename('tapgo_tt_member') . ' m ON c.mid=m.id WHERE c.uniacid = :uniacid AND c.status=3  GROUP BY c.mid ORDER BY commission_pay DESC LIMIT ' . intval($commission_rank['num']), array(':uniacid' => $_W['uniacid']));
			$paiming = 0;
	...
	protected function commissionTotal($pindex, $psize)
	{
		...
		$result = pdo_fetchall('SELECT id,uid,nickname,avatar,commission_total FROM ' . tablename('tapgo_tt_member') . ' WHERE uniacid = :uniacid ORDER BY commission_total DESC,id ' . $limit, array(':uniacid' => $_W['uniacid']));
//print_r($this->commission_rank);exit;
		$total = pdo_fetchcolumn('SELECT count(*) FROM ' . tablename('tapgo_tt_member') . ' WHERE uniacid = :uniacid ', array(':uniacid' => $_W['uniacid']));
		if (!empty($result)) {
			$result_tmp = array();
	...
	protected function commissionPay($pindex, $psize)
	{
		...
		$total = pdo_fetchcolumn('SELECT count(*) FROM ' . tablename('tapgo_tt_commission_apply') . ' c LEFT JOIN ' . tablename('tapgo_tt_member') . ' m ON c.mid=m.id WHERE c.uniacid = :uniacid AND c.status=3', array(':uniacid' => $_W['uniacid']));
		if (!empty($result)) {
			$result_tmp = array();
	...
	protected function rankIcon($index=0){
		if($index>=1 && $index<=3) return tomedia('../addons/tapgo_ttv2/template/mobile/default/static/images/rank_list_icon_'.$index.'.png');
		else return '';
	}
	public function myshop()
	{
		...
		if (empty($shop['selectgoods'])) {
			$goodscount = pdo_fetchcolumn('select count(*) from ' . tablename('tapgo_tt_goods') . ' where uniacid=:uniacid and status=1 and deleted=0', array(':uniacid' => $_W['uniacid']));
		}
		else {
		...
		//$banners = pdo_fetchall('select id,bannername,link,thumb from ' . tablename('tapgo_tt_banner') . ' where uniacid=:uniacid and enabled=1 and iswxapp=0 order by displayorder desc', array(':uniacid' => $uniacid));
		...
		/*if (!empty($_W['shopset']['shop']['indexrecommands'])) {
			...
			if (!empty($goodids)) {
				$indexrecommands = pdo_fetchall('select id, title, thumb, marketprice, productprice ,minprice, total from ' . tablename('tapgo_tt_goods') . ' where id in( ' . $goodids . ' ) and uniacid=:uniacid and status=1 order by displayorder desc', array(':uniacid' => $uniacid));
			}
		}*/

		$goodsstyle = $_W['shopset']['shop']['goodsstyle'];
		$notices = pdo_fetchall('select id, title, link, thumb from ' . tablename('tapgo_tt_notice') . ' where uniacid=:uniacid and status=1 order by displayorder desc limit 5', array(':uniacid' => $uniacid));
		$shareid = $mid;
		if (($member['isagent'] == 1) && ($member['status'] == 1)) {
			$shareid = $member['id'];
		}
