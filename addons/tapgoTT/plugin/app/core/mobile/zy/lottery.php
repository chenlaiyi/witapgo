...
class Lottery_TapgoTtPage extends AppMobilePage
{
	public function get_list()
	{
		...
		$banner = empty($set['thumb'])?'../addons/tapgo_ttv2/plugin/lottery/static/images/lottery_banner.png':$set['thumb'];
		$banner = tomedia($banner);
		$task_sql = 'SELECT lottery_id,lottery_title,lottery_icon,lottery_cannot,lottery_type,task_type,task_data,start_time,end_time FROM ' . tablename('tapgo_tt_lottery') . ' WHERE uniacid=:uniacid AND `is_delete`=0 and start_time<='.time().' and end_time>='.time().' ORDER BY lottery_id DESC';
		...
		foreach ($lottery as $key => $value) {
			...
			if(empty($value['lottery_icon'])){
				if ($value['lottery_type'] == 1) {
					$lottery[$key]['lottery_icon'] = '../addons/tapgo_ttv2/plugin/lottery/static/images/panicon.png';
				}
				else if ($value['lottery_type'] == 2) {
					$lottery[$key]['lottery_icon'] = '../addons/tapgo_ttv2/plugin/lottery/static/images/guaicon.png';
				}
				else {
					if ($value['lottery_type'] == 3) {
						$lottery[$key]['lottery_icon'] = '../addons/tapgo_ttv2/plugin/lottery/static/images/gridicon.png';
					}
				}
			}
			...
			$lottery[$key]['total'] = 0;
			if(!empty($_W['openid'])){
				$task_sql = 'SELECT SUM(lottery_num) FROM ' . tablename('tapgo_tt_lottery_join') . '  WHERE uniacid=:uniacid AND lottery_id=:lottery_id and `join_user`=:join_user and lottery_num>0';
		...
		return app_json(array('banner'=>$banner, 'list'=>$lottery));
	}
	public function lottery_info()
	{
		...
		if ($id) {
			...
			$lottery['has_changes'] = intval(pdo_fetchcolumn('select sum(lottery_num) from ' . tablename('tapgo_tt_lottery_join') . 'where uniacid=:uniacid AND lottery_id=:lottery_id  AND join_user=:join_user and lottery_num>0 ' . $conditon, array(':uniacid' => $_W['uniacid'], ':join_user' => $_W['openid'], ':lottery_id' => $id)));
			//$member = m('member')->getMember($_W['openid'], true);
		}
		return app_json(array('data'=>$lottery,'intro'=>$intro));
	}
	...
	public function lottery_reward()
	{
		...
		return app_json(array('data'=>$reward));
	}
	public function getreward()
	{
		...
		$has_changes = intval(pdo_fetchcolumn('SELECT SUM(lottery_num) FROM ' . tablename('tapgo_tt_lottery_join') . ' WHERE uniacid=:uniacid AND lottery_id=:id AND join_user=:join_user AND lottery_num>0', array(':uniacid' => $_W['uniacid'], ':join_user' => $_W['openid'], ':id' => $id)));
	...
	public function myrewardpage()
	{
		...
		return app_json(array('list' => $mylog, 'total' => $count, 'pagesize' => $pagesize));
	}
