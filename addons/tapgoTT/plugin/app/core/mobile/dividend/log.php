...
class Log_TapgoTtPage extends Base_TapgoTtPage {
    ...
    function get_list() {
        ...
        $dividendcount = pdo_fetchcolumn('select sum(dividend) from '.tablename('tapgo_tt_dividend_apply') ." where mid=:mid and uniacid=:uniacid and status>-1 limit 1",
...
//            'set'=>$this->set,
    ...
    function orders() {
        ...
        $list = pdo_fetchall("select id,headsid, ordersn,price,goodsprice, dispatchprice,createtime, paytype,dividend,dividend_status,dividend_content from " . tablename('tapgo_tt_order')
            . " where  id in ( " . $apply['orderids'] . " ) LIMIT " . ($pindex - 1) * $psize . ',' . $psize);
        $total = pdo_fetchcolumn("select count(*) from " . tablename('tapgo_tt_order')
        ...
        foreach ($list as &$row) {
            ...
            $orderpay = 0;
            $goods = pdo_fetchall("SELECT og.id,g.thumb,og.price,og.realprice, og.total,g.title,o.paytype,og.optionname from " . tablename('tapgo_tt_order_goods') . " og"
