...
class Index_TapgoTtPage extends Base_TapgoTtPage
{
    ...
    public  function register(){
        ...
        if ($set['condition'] == 1) { //一级下线人数
            $status = 1;
            $membercount = pdo_fetchcolumn('select count(*) from ' . tablename('tapgo_tt_member') . " where  agentid = ".$member['id']." and id <> ".$member['id']."  and uniacid=:uniacid  limit 1", array(':uniacid' => $_W['uniacid']));
            ...
            $status = 1;
            $commissiondownlinecount = pdo_fetchcolumn('select count(*) from ' . tablename('tapgo_tt_member') . " where  agentid = ".$member['id']." and isagent = 1 and id <> ".$member['id']."  and uniacid=:uniacid  limit 1", array(':uniacid' => $_W['uniacid']));
...
//                }
        ...
        $result = array(
            'set'=>array(
                ...
                'regbg' => empty($set['regbg'])? $_W['siteroot'].'addons/tapgo_ttv2/plugin/dividend/template/mobile/default/static/images/banner.jpg':$set['regbg'],
    ...
    public function main(){
        ...
        //提现笔数
        $member['applycount'] = pdo_fetchcolumn('select count(id) from ' . tablename('tapgo_tt_dividend_apply') . ' where mid=:mid and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':mid' => $member['id']));
