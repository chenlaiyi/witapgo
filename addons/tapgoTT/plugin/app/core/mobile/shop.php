...
class Shop_TapgoTtPage extends AppMobilePage
{
    public function get_shopindex()
    {
        ...
        $advs = pdo_fetchall("select id,advname,link,thumb from " . tablename("tapgo_tt_adv") . " where uniacid=:uniacid" . $appsql . " and enabled=1 order by displayorder desc", array( ":uniacid" => $uniacid ));
        $advs = set_medias($advs, "thumb");
        $navs = pdo_fetchall("select id,navname,url,icon from " . tablename("tapgo_tt_nav") . " where uniacid=:uniacid" . $appsql . " and status=1 order by displayorder desc", array( ":uniacid" => $uniacid ));
        $navs = set_medias($navs, "icon");
        $cubes = ($this->iswxapp ? $_W["shopset"]["shop"]["cubes_wxapp"] : $_W["shopset"]["shop"]["cubes"]);
        $cubes = set_medias($cubes, "img");
        $banners = pdo_fetchall("select id,bannername,link,thumb from " . tablename("tapgo_tt_banner") . " where uniacid=:uniacid" . $appsql . " and enabled=1 order by displayorder desc", array( ":uniacid" => $uniacid ));
        ...
        if( !empty($_W["shopset"]["shop"]["indexrecommands"]) ) 
        {
            ...
            if( !empty($goodids) ) 
            {
                $indexrecommands = pdo_fetchall("select id, title, thumb, marketprice,ispresell,presellprice, productprice, minprice, total from " . tablename("tapgo_tt_goods") . " where id in( " . $goodids . " ) and uniacid=:uniacid and status=1 order by instr('" . $goodids . "',id),displayorder desc", array( ":uniacid" => $uniacid ));
        ...
        $notices = pdo_fetchall("select id, title, link, thumb from " . tablename("tapgo_tt_notice") . " where uniacid=:uniacid" . $appsql . " and status=1 order by displayorder desc limit 5", array( ":uniacid" => $uniacid ));
    ...
    public function get_nopayorder()
    {
        ...
        {
            $order = pdo_fetch("select id,price  from " . tablename("tapgo_tt_order") . " where uniacid=:uniacid and status = 0 and paytype<>3 and openid=:openid order by createtime desc limit 1", array( ":uniacid" => $_W["uniacid"], ":openid" => $_W["openid"] ));
            if( !empty($order) ) 
            {
    ...
    public function get_hasnewcoupon()
    {
        ...
        pdo_update("tapgo_tt_member", array( "hasnewcoupon" => 0 ), array( "openid" => $openid, "uniacid" => $_W["uniacid"] ));
