...
class Index_TapgoTtPage extends AppMobilePage
{
    ...
    public function get_list()
    {
        ...
        $sql = "SELECT bg.* ,g.id as gid,g.title,g.subtitle,g.thumb,g.marketprice,g.productprice,g.minprice,g.maxprice,g.isdiscount,g.isdiscount_time,g.isdiscount_discounts,g.sales,g.salesreal,g.total,g.description,g.bargain,g.`type` as gtype,g.ispresell,g.`virtual`,g.hasoption,g.video FROM " . tablename("tapgo_tt_bargain_goods") . " bg " . " LEFT JOIN " . tablename("tapgo_tt_goods") . " g ON g.id = bg.goods_id " . " WHERE " . $condition . " ORDER BY bg.id DESC LIMIT " . ($pindex - 1) * $pagesize . "," . $pagesize;
        $list = pdo_fetchall($sql, $params);
        $data = date("Y-m-d h:i:s");
        $sqlcount = "SELECT bg.id,bg.start_time,bg.end_time FROM " . tablename("tapgo_tt_bargain_goods") . " bg " . " LEFT JOIN " . tablename("tapgo_tt_goods") . " g ON g.id = bg.goods_id " . " WHERE " . $condition . "  ORDER BY bg.id DESC ";
    ...
    public function get_detail()
    {
        ...
        if (!empty($res)) {
            $goods = pdo_fetch("SELECT * FROM " . tablename("tapgo_tt_goods") . " WHERE id = :id AND status = '1' AND deleted = '0'", array(":id" => $res["goods_id"]));
            if (!empty($goods)) {
    ...
    public function bargain()
    {
        ...
        if (!$res || !$res2) {
            ...
            $res3 = pdo_fetchall("SELECT * FROM " . tablename("tapgo_tt_bargain_record") . " WHERE actor_id = :actor_id ORDER BY id DESC LIMIT 0,10", array(":actor_id" => $id));
            if (!empty($res3)) {
                foreach ($res3 as $res3k => $res3v) {
                    $res3[$res3k]["bargain_time"] = date("m/d  H:i", strtotime($res3v["bargain_time"]));
                }
            }
            $res4 = pdo_fetchall("SELECT * FROM " . tablename("tapgo_tt_bargain_actor") . " WHERE goods_id = " . $res2["id"] . " AND account_id=:uniacid and bargain_price <= '" . $account_set["partin"] . "' ORDER BY bargain_price ASC LIMIT 10", array(":uniacid" => $_W["uniacid"]));
            if (!empty($res4)) {
    ...
    public function join()
    {
        ...
        $res = pdo_fetch("SELECT * FROM " . tablename("tapgo_tt_bargain_goods") . " WHERE id = :id", array(":id" => $bargain_id));
        if ($res["maximum"] <= $res["act_times"]) {
            return app_error(1, "活动次数已到达上限,不能发起砍价");
        }
        if (!empty($res["initiate"])) {
            $count = pdo_get("tapgo_tt_bargain_actor", array("goods_id" => $bargain_id, "openid" => $_W["openid"], "status" => 0, "order" => 0), "id");
            if (!empty($count["id"])) {
        ...
        if (!empty($user_info["openid"])) {
            ...
            pdo_query("UPDATE " . tablename("tapgo_tt_bargain_goods") . " SET act_times=act_times+1 WHERE id= :id", array(":id" => $bargain_id));
            if ($result_id) {
    ...
    public function act()
    {
        ...
        $mid = (int) $_GPC["mid"];
        $mid = m("member")->getMid();
        $act = pdo_fetchall("SELECT * FROM " . tablename("tapgo_tt_bargain_actor") . " WHERE openid= :openid AND account_id = :account_id AND status = '0' ORDER BY id DESC", array(":openid" => $_W["openid"], ":account_id" => $_W["uniacid"]));
    ...
    public function purchase()
    {
        ...
        $mid = (int) $_GPC["mid"];
        $mid = m("member")->getMid();
        $act = pdo_fetchall("SELECT * FROM " . tablename("tapgo_tt_bargain_actor") . " WHERE openid= :openid AND account_id = :account_id AND status = '1' ORDER BY id DESC", array(":openid" => $_W["openid"], ":account_id" => $_W["uniacid"]));
        for ($i = 0; $i < count($act); $i++) {
    ...
    private function get_bargain_share()
    {
        ...
        global $_GPC;
        $id = $_GPC["id"];
        $actor = pdo_get("tapgo_tt_bargain_actor", array("id" => $id, "account_id" => $_W["uniacid"]));
        $share_goods = pdo_get("tapgo_tt_bargain_goods", array("id" => $actor["goods_id"], "account_id" => $_W["uniacid"]));
