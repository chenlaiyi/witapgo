...
class Detail_TapgoTtPage extends AppMobilePage
{
    ...
    function option(){
        ...
        $goods = pdo_fetch("select id,thumb,credit,money,total,title from ".tablename('tapgo_tt_creditshop_goods')." where id= :id and uniacid = :uniacid ",array(':id'=>$id,':uniacid'=>$uniacid));
        ...
        unset($spec);
        $options = pdo_fetchall('select * from ' . tablename('tapgo_tt_creditshop_option') . ' where goodsid=:goodsid and uniacid=:uniacid order by displayorder asc', array(':goodsid' => $id, ':uniacid' => $uniacid));
        if (!empty($specs))
        {
    ...
    function pay($a=array(), $b=array()){
        ...
        if ($goods['money'] > 0 || floatval($goods['dispatch'])>0) {
            //删除以前无效的记录
            // pdo_delete('tapgo_tt_creditshop_log',array('goodsid'=>$id, 'openid'=>$openid,'status'=>0,'paystatus'=>0));
            ...
            //删除以前无效的记录
            // pdo_delete('tapgo_tt_creditshop_log',array('goodsid'=>$id, 'openid'=>$openid,'status'=>0));
        }
        $dispatchstatus =  0;
        ...
        $address = false;
        if (!empty($addressid)) {
            $address = pdo_fetch('select id,realname,mobile,address,province,city,area from ' . tablename('tapgo_tt_member_address') . '
        ...
        if ($needpay) {
            if($paytype == "credit"){
                ...
                //支付方式
                pdo_update('tapgo_tt_creditshop_log', array('paytype' => $paystatus), array('id' => $logid));
...
//        }
    ...
    function express()
    {
        ...
        $order = pdo_fetch("select expresscom,expresssn,addressid,status,express from " . tablename('tapgo_tt_creditshop_order') . ' where id=:id and uniacid=:uniacid and openid=:openid limit 1'
        ...
        //商品信息
        $goods = pdo_fetchall("select og.goodsid,og.price,g.title,g.thumb,og.total,g.credit,og.optionid,og.optionname as optiontitle,g.isverify,g.storeids  from " . tablename('tapgo_tt_order_goods') . " og "
            . " left join " . tablename('tapgo_tt_creditshop_order') . " g on g.id=og.goodsid "
