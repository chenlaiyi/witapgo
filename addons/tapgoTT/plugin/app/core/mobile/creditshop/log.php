...
class Log_TapgoTtPage extends AppMobilePage
{
...
//        $credit = m('member')->getCredit($openid, 'credit1');
        ...
        if (!empty($total)) {
            $sql = 'SELECT log.id,log.logno,log.goodsid,log.goods_num,log.status,log.eno,log.paystatus,g.title,g.type,g.thumb,log.credit,log.money,log.dispatch,g.isverify,g.goodstype,log.addressid,log.storeid,'
                ...
                . ' left join ' . tablename('tapgo_tt_creditshop_goods') . ' g on log.goodsid = g.id '
                . ' left join ' . tablename('tapgo_tt_creditshop_option') . ' op on op.id = log.optionid '
    ...
    public function detail(){
        ...
        if(!empty($log['addressid'])){
            $address = pdo_fetch('select * from ' . tablename('tapgo_tt_member_address') . ' where id=:id and openid=:openid and uniacid=:uniacid limit 1'
        ...
        if($goods['type']==1 && $goods['goodstype']==0 && empty($log['addressid']) && $goods['isverify']==0){
            //默认地址
            $address = pdo_fetch('select * from ' . tablename('tapgo_tt_member_address') . ' where openid=:openid and deleted=0 and isdefault=1  and uniacid=:uniacid limit 1'
        ...
        if(!empty($goods['isverify'])){
            //核销次数
            $verifytotal = pdo_fetchcolumn("select count(1) from " . tablename('tapgo_tt_creditshop_verify') . " where logid = :id and openid=:openid ".$condition." and verifycode = :verifycode ",
                array(':id'=>$id,':openid'=>$log['openid'],':uniacid'=>$log['uniacid'],':verifycode'=>$log['eno']));
            if($goods['verifytype']==0){
                $verify = pdo_fetch("select isverify from ". tablename('tapgo_tt_creditshop_verify') ." where logid = :id and openid=:openid ".$condition." and verifycode = :verifycode ",
    ...
    public function finish() {
        ...
        $log = pdo_fetch("select * from " . tablename('tapgo_tt_creditshop_log') . ' where id=:id '.$condition.' and openid=:openid limit 1'
    ...
    function paydispatch(){
        ...
        if($paytype == 'wechat'){
            ...
            if(empty($dispatchno)){
                ...
                pdo_update('tapgo_tt_creditshop_log',array('dispatchno'=>$dispatchno,'addressid'=>$addressid,'dispatch'=>$dispatchprice),array('id'=>$log['id']));
            }
            $tid = $dispatchno;
            $payinfo = array(
                'openid' => $_W['openid_wa'],
    ...
    function paydispatchresult(){
        ...
        pdo_update('tapgo_tt_creditshop_log',array('dispatchstatus'=>1),array('id'=>$log['id']));
        return app_json();
    }
    ...
    function express()
    {
        ...
        $order = pdo_fetch("select expresscom,expresssn,addressid,status,express from " . tablename('tapgo_tt_creditshop_log') . ' where id=:id and uniacid=:uniacid and openid=:openid limit 1'
        ...
        //商品信息
        $goods = pdo_fetchall("select og.goodsid,g.thumb  from " . tablename('tapgo_tt_creditshop_log') . " og "
            . " left join " . tablename('tapgo_tt_creditshop_goods') . " g on g.id=og.goodsid "
