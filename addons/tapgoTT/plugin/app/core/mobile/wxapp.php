...
require TAPGO_TT_PLUGIN . "app/core/error_code.php";
require TAPGO_TT_PLUGIN . "app/core/wxapp/wxBizDataCrypt.php";
class Wxapp_TapgoTtPage extends Page
{
    ...
    public function auth()
    {
      ...
      $open=substr($openid,7);
        ...
        if ($errCode == 0) {
            ...
            if (empty($member)) {
                $member = array("uniacid" => $_W["uniacid"], "uid" => 0, "openid" => $openid, "nickname" => !empty($data["nickName"]) ? $data["nickName"] : "", "avatar" => !empty($data["avatarUrl"]) ? $data["avatarUrl"] : "", "gender" => !empty($data["gender"]) ? $data["gender"] : "-1", "openid_wa" => $data["openId"], "comefrom" => "sns_wa", "createtime" => time(), "status" => 0);
                ...
                pdo_update("tapgo_tt_member", $updateData, array("id" => $member["id"], "uniacid" => $member["uniacid"]));
                $data["id"] = $member["id"];
...
function app_json($result = NULL, $openid)
{
    ...
    $ret["error"] = 0;
    $key = time() . "@" . $openid;
    $auth = array("authkey" => base64_encode(authcode($key, "ENCODE", "tapgo_ttv2_wxapp")));
