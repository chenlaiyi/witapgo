...
class Index_TapgoTtPage extends AppMobileAuthPage {
    ...
    public function home() {
        global $_W;
        $notices = pdo_fetchall("SELECT * FROM " . tablename('tapgo_tt_system_copyright_notice') . " WHERE status=1 ORDER BY displayorder ASC,createtime DESC LIMIT 10" );
        $order = $this->order(0);
        ...
        if($open_redis){
            ...
            if(!$member_count){
                $member_count = pdo_fetchcolumn('select count(*) from ' . tablename('tapgo_tt_member') . ' where uniacid=:uniacid', array(':uniacid'=>$_W['uniacid']));
    ...
    private function selectOrderPrice($day=0) {
        ...
        $sql = 'select id,price,createtime from '.tablename('tapgo_tt_order').' where uniacid = :uniacid and ismr=0 and isparent=0 and (status > 0 or ( status=0 and paytype=3)) and deleted=0 and createtime between :createtime1 and :createtime2';
        $param = array(
            ':uniacid'=>$_W['uniacid'],
