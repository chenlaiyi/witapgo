...
class Login_TapgoTtPage extends AppMobilePage
{
    ...
    public function submit()
    {
        ...
        if( $type == "wechat" ) 
        {
            ...
            $openid = $arr["openid"];
            $roleuser = pdo_fetch("SELECT id, uid, uniacid, username, status, openid_wa FROM" . tablename("tapgo_tt_perm_user") . "WHERE openid_wa=:openid LIMIT 1", array( ":openid" => $openid ));
            if( empty($roleuser) ) 
            {
        ...
        else
        {
            ...
            if( $role == "operator" ) 
            {
                $roleuser = pdo_fetch("SELECT id, uid, uniacid FROM" . tablename("tapgo_tt_perm_user") . "WHERE uid=:uid LIMIT 1", array( ":uid" => $account["uid"] ));
                if( empty($roleuser) ) 
                {
                    return app_error(AppError::$UserLoginFail, "此账号没有管理权限");
                }
