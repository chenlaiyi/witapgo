...
require TAPGO_TT_PLUGIN . 'app/core/page_auth_mobile.php';

class Order_TapgoTtPage extends AppMobileAuthPage{
    ...
    public function get_list() {
        ...
        if (!empty($_GPC['searchfield']) && !empty($_GPC['keyword'])) {
            ...
            if ($searchfield == 'ordersn') {
                ...
                $sqlcondition = " left join " . tablename('tapgo_tt_store') . " store on store.id = o.verifystoreid and store.uniacid=o.uniacid";
            } else if ($searchfield == 'goodstitle') {
                $sqlcondition =  " inner join ( select  og.orderid from " . tablename('tapgo_tt_order_goods') . " og left join " . tablename('tapgo_tt_goods') . " g on g.id=og.goodsid where og.uniacid = '$uniacid' and (locate(:keyword,g.title)>0)) gs on gs.orderid=o.id";
            } else if ($searchfield == 'goodssn') {
                $sqlcondition =  " inner join ( select og.orderid from " . tablename('tapgo_tt_order_goods') . " og left join " . tablename('tapgo_tt_goods') . " g on g.id=og.goodsid where og.uniacid = '$uniacid' and (((locate(:keyword,g.goodssn)>0)) or (locate(:keyword,og.goodssn)>0))) gs on gs.orderid=o.id";
            } else if ($searchfield == 'goodsoptiontitle') {
                $sqlcondition =  " inner join ( select  og.orderid from " . tablename('tapgo_tt_order_goods') . " og left join " . tablename('tapgo_tt_goods') . " g on g.id=og.goodsid where og.uniacid = '$uniacid' and (locate(:keyword,og.optionname)>0)) gs on gs.orderid=o.id";
        ...
        if ($condition != ' o.uniacid = :uniacid and o.deleted=0 and o.isparent=0' || !empty($sqlcondition)){
            $sql = "select o.* , a.realname as arealname,a.mobile as amobile,a.province as aprovince ,a.city as acity , a.area as aarea,a.address as aaddress,
                  ...
                  r.rtype,r.status as rstatus,o.sendtype from " . tablename('tapgo_tt_order') . " o"
                . " left join " . tablename('tapgo_tt_order_refund') . " r on r.id =o.refundid "
                . " left join " . tablename('tapgo_tt_member') . " m on m.openid=o.openid and m.uniacid =  o.uniacid "
                . " left join " . tablename('tapgo_tt_member_address') . " a on a.id=o.addressid "
                . " left join " . tablename('tapgo_tt_dispatch') . " d on d.id = o.dispatchid "
                . " left join " . tablename('tapgo_tt_saler') . " s on s.openid = o.verifyopenid and s.uniacid=o.uniacid"
                . " left join " . tablename('tapgo_tt_member') . " sm on sm.openid = s.openid and sm.uniacid=s.uniacid"
            ...
            $sql = "select * from " . tablename('tapgo_tt_order') . " where uniacid = :uniacid and deleted=0 and isparent=0 {$status_condition} GROUP BY id ORDER BY createtime DESC  ";
            if (empty($_GPC['export'])) {
                $sql.="LIMIT " . ($pindex - 1) * $psize . ',' . $psize;
            }
            $list = pdo_fetchall($sql, $paras);
            if (!empty($list)){
                ...
                $openid_array = pdo_fetchall("SELECT openid,nickname,id as mid,realname as mrealname,mobile as mmobile FROM " .tablename('tapgo_tt_member')." WHERE openid IN ({$openid}) AND uniacid={$_W['uniacid']}",array(),'openid');
                ...
                $verifyopenid_array = pdo_fetchall("SELECT sm.id as salerid,sm.nickname as salernickname,sm.openid,s.salername FROM " .tablename('tapgo_tt_saler')." s LEFT JOIN ".tablename('tapgo_tt_member')." sm ON sm.openid = s.openid and sm.uniacid=s.uniacid WHERE s.openid IN ({$verifyopenid})",array(),'openid');
                foreach ($list as $key=>&$value){
        ...
        if (!empty($list)) {
            ...
            foreach ($list as &$value) {
                ...
                $order_goods = pdo_fetchall('select g.id,g.title,g.thumb,g.goodssn,og.goodssn as option_goodssn, g.productsn,og.productsn as option_productsn, og.total,
                    ...
                    og.diyformfields,op.specs,g.merchid,og.seckill,og.seckill_taskid,og.seckill_roomid,g.ispresell from ' . tablename('tapgo_tt_order_goods') . ' og '
                    . ' left join ' . tablename('tapgo_tt_goods') . ' g on g.id=og.goodsid '
                    . ' left join ' . tablename('tapgo_tt_goods_option') . ' op on og.optionid = op.id '
                ...
                if (!empty($agentid) && $level > 0) {
                    ...
                    if ($value['agentid'] == $agentid) {
                        $value['level'] = 1;
                        $level1_commissions = pdo_fetchall('select commission1,commissions  from ' . tablename('tapgo_tt_order_goods') . ' og '
                        ...
                        $value['level'] = 2;
                        if ($agent['level2'] > 0) {
                            $level2_commissions = pdo_fetchall('select commission2,commissions  from ' . tablename('tapgo_tt_order_goods') . ' og '
                                . ' left join  ' . tablename('tapgo_tt_order') . ' o on o.id = og.orderid '
                        ...
                        $value['level'] = 3;
                        if ($agent['level3'] > 0) {
                            $level3_commissions = pdo_fetchall('select commission3,commissions from ' . tablename('tapgo_tt_order_goods') . ' og '
                                . ' left join  ' . tablename('tapgo_tt_order') . ' o on o.id = og.orderid '
        ...
        if ($condition != ' o.uniacid = :uniacid and and o.deleted=0 and o.isparent=0' || !empty($sqlcondition)){
            $t = pdo_fetch(
                'SELECT COUNT(*) as count, ifnull(sum(o.price),0) as sumprice   FROM ' . tablename('tapgo_tt_order') . " o "
                . " left join " . tablename('tapgo_tt_order_refund') . " r on r.id =o.refundid "
                . " left join " . tablename('tapgo_tt_member') . " m on m.openid=o.openid  and m.uniacid =  o.uniacid"
                . " left join " . tablename('tapgo_tt_member_address') . " a on o.addressid = a.id "
                . " left join " . tablename('tapgo_tt_saler') . " s on s.openid = o.verifyopenid and s.uniacid=o.uniacid"
                . " left join " . tablename('tapgo_tt_member') . " sm on sm.openid = s.openid and sm.uniacid=s.uniacid"
                . " $sqlcondition WHERE $condition $statuscondition", $paras);
        }else{
            $t = pdo_fetch(
                'SELECT COUNT(*) as count, ifnull(sum(price),0) as sumprice   FROM ' . tablename('tapgo_tt_order') . " WHERE uniacid = :uniacid and deleted=0 and isparent=0 {$status_condition}", $paras);
        }
    ...
    public function get_detail() {
        ...
        $item = pdo_fetch("SELECT * FROM " . tablename('tapgo_tt_order') . " WHERE id = :id and uniacid=:uniacid", array(':id' => $id, ':uniacid' => $_W['uniacid']));
        if(empty($item)){
        ...
        $order_goods = array();
        if($item['sendtype']>0){
            $order_goods = pdo_fetchall("SELECT orderid,goodsid,sendtype,expresssn,expresscom,express,sendtime FROM " . tablename('tapgo_tt_order_goods') . "
            WHERE orderid = ".$id." and sendtime > 0 and uniacid=".$_W['uniacid']." and sendtype > 0 group by sendtype ");
            foreach($order_goods as $key => $value){
                $order_goods[$key]['goods'] = pdo_fetchall('select g.id,g.title,g.thumb,og.sendtype,g.ispresell from ' . tablename('tapgo_tt_order_goods') . ' og '
                    . ' left join ' . tablename('tapgo_tt_goods') . ' g on g.id=og.goodsid '
        ...
        // 配送方式
        $dispatch = pdo_fetch("SELECT * FROM " . tablename('tapgo_tt_dispatch') . " WHERE id = :id and uniacid=:uniacid and merchid=0", array(':id' => $item['dispatchid'], ':uniacid' => $_W['uniacid']));
        ...
        if (empty($item['addressid'])) {
            ...
            if (!is_array($user_address)) {
                $user_address = pdo_fetch("SELECT * FROM " . tablename('tapgo_tt_member_address') . " WHERE id = :id and uniacid=:uniacid", array(':id' => $item['addressid'], ':uniacid' => $_W['uniacid']));
            }
        ...
        if (com('verify')) {
            ...
            if (!empty($item['verifyopenid'])) {
                ...
                $saler['salername'] = pdo_fetchcolumn('select salername from ' . tablename('tapgo_tt_saler') . ' where openid=:openid and uniacid=:uniacid limit 1 ', array(':uniacid' => $_W['uniacid'], ':openid' => $item['verifyopenid']));
            }
            if (!empty($item['verifystoreid'])) {
                $store = pdo_fetch('select * from ' . tablename('tapgo_tt_store') . ' where id=:storeid limit 1 ', array(':storeid' => $item['verifystoreid']));
            }
            //核销
            if ($item['isverify']) {
                    ...
                    if (empty($item['dispatchtype'])) {
                        foreach ($verifyinfo as &$v) {
                            if ($v['verified'] || $item['verifytype']==1) {
                                $v['storename'] = pdo_fetchcolumn('select storename from ' . tablename('tapgo_tt_store') . " where id=:id limit 1", array(':id' => $v['verifystoreid']));
                                if (empty($v['storename'])) {
                                    $v['storename'] = "总店";
                                }
                                $v['nickname'] = pdo_fetchcolumn('select nickname from ' . tablename('tapgo_tt_member') . " where openid=:openid and uniacid=:uniacid limit 1", array(':openid' => $v['verifyopenid'], ':uniacid' => $_W['uniacid']));
                                $v['salername'] = pdo_fetchcolumn('select salername from ' . tablename('tapgo_tt_saler') . " where openid=:openid and uniacid=:uniacid limit 1", array(':openid' => $v['verifyopenid'], ':uniacid' => $_W['uniacid']));
                            }
                        }
                        unset($v);
                    }
    ...
    public function remarksaler() {
        ...
        if($_W['ispost']){
            ...
            pdo_update('tapgo_tt_order', array('remarksaler' => $remarksaler), array('id' => $orderid, 'uniacid' => $_W['uniacid']));
    ...
    public function payorder() {
        ...
        if (!empty($item['virtual']) && com('virtual')) {
            ...
            //确认付款先改状态，再设置库存
            pdo_update('tapgo_tt_order', array(
                'status' => 1,
                'paytype' => 11,
                'paytime' => time()
                //,'remark' => $_GPC['remark']
    ...
    public function send() {
        ...
        if($_W['ispost']){
            if($item['city_express_state'] ==0){
                ...
                if(!empty($sendtype)){
                    ...
                    $ogoods = array();
                    $ogoods = pdo_fetchall("select sendtype from ".tablename('tapgo_tt_order_goods')."
                    ...
                    if($goodsids){
                        foreach($goodsids as $key => $value){
                            pdo_update('tapgo_tt_order_goods', $data, array('orderid' => $item['id'],'goodsid'=>$value, 'uniacid' => $_W['uniacid']));
                        }
                    }
                    //查询是否有未发货包裹
                    $send_goods = pdo_fetch("select * from ".tablename('tapgo_tt_order_goods')."
                    where orderid = ".$item['id']." and sendtype = 0 and uniacid = ".$_W['uniacid']." limit 1 ");
                    //如果包裹都发货，修改订单状态
                    if(empty($send_goods)){
                        $senddata['status'] = 2 ;
                    }
                    pdo_update('tapgo_tt_order', $senddata, array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
                }else{
                    $data['status'] = 2;
                    pdo_update('tapgo_tt_order', $data, array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
                }
            }else{
            ...
            if (!empty($item['refundid'])) {
                $refund = pdo_fetch('select * from ' . tablename('tapgo_tt_order_refund') . ' where id=:id limit 1', array(':id' => $item['refundid']));
                if (!empty($refund)) {
                    pdo_update('tapgo_tt_order_refund', array('status' => -1, 'endtime' => $time), array('id' => $item['refundid']));
                    pdo_update('tapgo_tt_order', array('refundstate' => 0), array('id' => $item['id']));
                }
            }
            if ($item['paytype'] == 3) {
                //处理订单库存
        ...
        if($item['sendtype']>0){
            //未发货商品
            $noshipped = pdo_fetchall('select g.id,g.title,g.thumb,og.sendtype,g.ispresell,og.optionname,og.total from ' . tablename('tapgo_tt_order_goods') . ' og '
                . ' left join ' . tablename('tapgo_tt_goods') . ' g on g.id=og.goodsid '
            ...
            for($i=1;$i<=$item['sendtype'];$i++){
                $row = array(
                    'sendtype'=>$i,
                    'goods'=>pdo_fetchall('select g.id,g.title,g.thumb,og.sendtype,g.ispresell,og.optionname,og.total from ' . tablename('tapgo_tt_order_goods') . ' og '
                        . ' left join ' . tablename('tapgo_tt_goods') . ' g on g.id=og.goodsid '
        ...
        $order_goods = pdo_fetchall('select g.id,g.title,g.thumb,og.sendtype,g.ispresell,og.optionname,og.total from ' . tablename('tapgo_tt_order_goods') . ' og '
            . ' left join ' . tablename('tapgo_tt_goods') . ' g on g.id=og.goodsid '
        ...
        if (!is_array($address)) {
            $address = pdo_fetch("SELECT * FROM " . tablename('tapgo_tt_member_address') . " WHERE id = :id and uniacid=:uniacid", array(':id' => $item['addressid'], ':uniacid' => $_W['uniacid']));
        }
    ...
    public function sendcancel() {
            ...
            if($item['sendtype']>0){
                ...
                if(is_array($sendtype)){
                    foreach ($sendtype as $key=>$value){
                        $data['sendtype'] = 0;
                        pdo_update('tapgo_tt_order_goods', $data, array('orderid' => $item['id'],'sendtype'=>$value, 'uniacid' => $_W['uniacid']));
                        $order = pdo_fetch("select sendtype from ".tablename('tapgo_tt_order')." where id = ".$item['id']." and uniacid = ".$_W['uniacid']." ");
                        pdo_update('tapgo_tt_order', array('sendtype'=>$order['sendtype']-1,'status'=>1), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
                    }
                }
            }else{
                $data['status'] = 1;
        ...
        $bundles = array();
        if($sendtype>0){
            $sendgoods = pdo_fetchall('select g.id,g.title,g.thumb,og.sendtype,g.ispresell,og.optionname,og.total from ' . tablename('tapgo_tt_order_goods') . ' og '
                . ' left join ' . tablename('tapgo_tt_goods') . ' g on g.id=og.goodsid '
            ...
            if($item['sendtype']>0){
                for($i=1;$i<=intval($item['sendtype']);$i++){
                    $bundles[$i]['goods'] = pdo_fetchall('select g.id,g.title,g.thumb,og.sendtype,g.ispresell,og.optionname,og.total from ' . tablename('tapgo_tt_order_goods') . ' og '
                        . ' left join ' . tablename('tapgo_tt_goods') . ' g on g.id=og.goodsid '
    ...
    public function changeaddress() {
        ...
        if (empty($item['addressid'])) {
            ...
            if (!is_array($user)) {
                $user = pdo_fetch("SELECT * FROM " . tablename('tapgo_tt_member_address') . " WHERE id = :id and uniacid=:uniacid", array(':id' => $item['addressid'], ':uniacid' => $_W['uniacid']));
            }
        ...
        if($_W['ispost']){
            ...
            pdo_update('tapgo_tt_order', array('address' => $address_array), array('id' => $orderid, 'uniacid' => $_W['uniacid']));
    ...
    public function changeexpress() {
        ...
        if($_W['ispost']){
            ...
            if($item['sendtype']>0){
                ...
                if(is_array($sendtype)){
                    foreach ($sendtype as $key=>$value){
                        pdo_update('tapgo_tt_order_goods', $change_data, array('orderid' => $orderid,'sendtype'=>$value, 'uniacid' => $_W['uniacid']));
                    }
                }
            }else{
                pdo_update('tapgo_tt_order', $change_data, array('id' => $orderid, 'uniacid' => $_W['uniacid']));
            }
        ...
        $bundles = array();
        if($sendtype>0){
            $sendgoods = pdo_fetchall('select g.id,g.title,g.thumb,og.sendtype,g.ispresell from ' . tablename('tapgo_tt_order_goods') . ' og '
                . ' left join ' . tablename('tapgo_tt_goods') . ' g on g.id=og.goodsid '
                . ' where og.uniacid=:uniacid and og.orderid=:orderid and og.sendtype='.$sendtype.' ', array(':uniacid' => $_W['uniacid'], ':orderid' => $item['id']));
            $sendgoods = set_medias($sendgoods, 'thumb');
        }else{
            if($item['sendtype']>0){
                for($i=1;$i<=intval($item['sendtype']);$i++){
                    $bundle = array(
                        'goods' => pdo_fetchall('select g.id,g.title,og.optionname,og.total,g.thumb,og.sendtype,g.ispresell from ' . tablename('tapgo_tt_order_goods') . ' og '
                            . ' left join ' . tablename('tapgo_tt_goods') . ' g on g.id=og.goodsid '
                            . ' where og.uniacid=:uniacid and og.orderid=:orderid and og.sendtype='.$i.' ', array(':uniacid' => $_W['uniacid'], ':orderid' => $item['id'])),
                        'sendtype'=>$i
                    );
                    ...
                    /*
                    $bundles[$i]['goods'] = pdo_fetchall('select g.id,g.title,og.optionname,og.total,g.thumb,og.sendtype,g.ispresell from ' . tablename('tapgo_tt_order_goods') . ' og '
                        . ' left join ' . tablename('tapgo_tt_goods') . ' g on g.id=og.goodsid '
                        . ' where og.uniacid=:uniacid and og.orderid=:orderid and og.sendtype='.$i.' ', array(':uniacid' => $_W['uniacid'], ':orderid' => $item['id']));
                    $bundles[$i]['sendtype'] = $i;
                    if(empty($bundles[$i]['goods'])){
    ...
    public function finish() {
        ...
        pdo_update('tapgo_tt_order', array('status' => 3, 'finishtime' => time()), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
    ...
    public function fetch() {
        ...
        pdo_update('tapgo_tt_order', $d, array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
        ...
        if (!empty($item['refundid'])) {
            $refund = pdo_fetch('select * from ' . tablename('tapgo_tt_order_refund') . ' where id=:id limit 1', array(':id' => $item['refundid']));
            if (!empty($refund)) {
                pdo_update('tapgo_tt_order_refund', array('status' => -1), array('id' => $item['refundid']));
                pdo_update('tapgo_tt_order', array('refundstate' => 0), array('id' => $item['id']));
            }
        }
    ...
    public function changeprice() {
        ...
        if ($_W['ispost']) {
            ...
            foreach ($changegoodsprice as $ogid => $change) {
                $og = pdo_fetch('select price,realprice from ' . tablename('tapgo_tt_order_goods') . ' where id=:ogid and uniacid=:uniacid limit 1', array(':ogid' => $ogid, ':uniacid' => $_W['uniacid']));
                if (!empty($og)) {
            ...
            if (!empty($orderupdate)) {
                pdo_update('tapgo_tt_order', $orderupdate, array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
            }
            ...
            //修改商品价
            foreach ($changegoodsprice as $ogid => $change) {
                $og = pdo_fetch('select price,realprice,changeprice from ' . tablename('tapgo_tt_order_goods') . ' where id=:ogid and uniacid=:uniacid limit 1', array(':ogid' => $ogid, ':uniacid' => $_W['uniacid']));
                if (!empty($og)) {
                    $realprice = $og['realprice'] + $change; //这次的变化
                    $changeprice = $og['changeprice'] + $change; //累计的变化
                    pdo_update('tapgo_tt_order_goods', array('realprice' => $realprice, 'changeprice' => $changeprice), array('id' => $ogid));
                }
            }
