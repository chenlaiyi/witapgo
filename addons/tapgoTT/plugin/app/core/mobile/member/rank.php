...
class Rank_TapgoTtPage extends MobilePage
{
    ...
    protected function getRank($update = false)
    {
        ...
        if( empty($rank_cache) || $rank_cache["time"] < TIMESTAMP || $update ) 
        {
            ...
            $result = pdo_fetchall("SELECT sm.id,sm.uid,m.credit1,sm.nickname,sm.avatar,sm.openid FROM " . tablename("tapgo_tt_member") . " sm RIGHT JOIN " . tablename("mc_members") . " m ON m.uid=sm.uid WHERE sm.uniacid = :uniacid ORDER BY m.credit1 DESC LIMIT " . $num, array( ":uniacid" => $_W["uniacid"] ));
            $result1 = pdo_fetchall("SELECT id,uid,credit1,nickname,avatar,openid FROM " . tablename("tapgo_tt_member") . " WHERE uniacid = :uniacid AND uid=0 ORDER BY credit1 DESC LIMIT " . $num, array( ":uniacid" => $_W["uniacid"] ));
    ...
    protected function creditChange($uid, $credittype = "credit1", $day = 7)
    {
        ...
        $sql = "select sum(num) from " . tablename("mc_credits_record") . " where uniacid = :uniacid and uid = :uid and credittype = :credittype and `module` = \"tapgo_ttv2\" and createtime between :createtime1 and :createtime2";
    ...
    public function order_rank()
    {
        ...
        $this->orderStatus();
        $user = m("member")->getMember($_W["openid"], true);
        $result_one = pdo_fetch("SELECT SUM(price) as price,openid FROM " . tablename("tapgo_tt_order") . " WHERE uniacid = :uniacid AND openid = :openid AND status = 3 GROUP BY openid ", array( ":uniacid" => $_W["uniacid"], ":openid" => $user["openid"] ));
        $result_all = pdo_fetchall("SELECT SUM(price) as price,openid FROM " . tablename("tapgo_tt_order") . " WHERE uniacid = :uniacid AND status = 3 GROUP BY openid HAVING price>=:price ORDER BY price DESC", array( ":uniacid" => $_W["uniacid"], ":price" => $result_one["price"] ));
    ...
    public function ajaxorderpage()
    {
        ...
        $result = pdo_fetchall("SELECT m.id,m.uid,m.nickname,m.avatar,SUM(o.price) as price FROM " . tablename("tapgo_tt_member") . " as m JOIN " . tablename("tapgo_tt_order") . " as o ON m.openid = o.openid AND m.uniacid = o.uniacid WHERE o.uniacid = :uniacid AND o.status = 3 GROUP BY o.openid ORDER BY price DESC " . $limit, array( ":uniacid" => $_W["uniacid"] ));
        show_json(1, array( "list" => $result, "stop" => $stop ));
    }
    ...
    protected function orderChange($openid, $day = 7)
    {
        ...
        $sql = "select sum(price) from " . tablename("tapgo_tt_order") . " where uniacid = :uniacid and openid = :openid and status = 3 and createtime between :createtime1 and :createtime2";
