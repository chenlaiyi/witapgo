...
class Cart_TapgoTtPage extends AppMobilePage {
    ...
    public function get_cart() {
        ...
        $sql = 'SELECT f.id,f.total,g.status,f.goodsid,g.isdiscount,g.isdiscount_time,g.isdiscount_time_start,g.total as stock, g.discounts,g.isdiscount_discounts,g.isnodiscount,g.deleted,o.stock as optionstock, g.maxbuy,g.title,g.thumb,ifnull(o.marketprice, g.marketprice) as marketprice,'
            ...
            . ' ,f.selected FROM ' . tablename('tapgo_tt_member_cart') . ' f '
    ...
    public function add() {
        ...
        $goods = pdo_fetch('select id,marketprice,`type`,total,diyformid,diyformtype,diyfields, isverify,merchid,cannotrefund,hasoption from '.tablename('tapgo_tt_goods').' where id=:id and uniacid=:uniacid limit 1',array(':id'=>$id,':uniacid'=>$_W['uniacid']));
        if (empty($goods)) {
    ...
    public function update(){
        ...
        $data = pdo_fetch("select * from " . tablename('tapgo_tt_member_cart') . " where id=:id and uniacid=:uniacid and openid=:openid limit 1 ", array(':id' => $id, ':uniacid' => $_W['uniacid'],':openid'=>$_W['openid']));

        if (empty($data)) {
            return app_error(AppError::$NotInCart);
        }
        $goods =pdo_fetch('select id,maxbuy,minbuy,total,unit from '.tablename('tapgo_tt_goods').' where id=:id and uniacid=:uniacid and status=1 and deleted=0',array(':id'=>$data['goodsid'],':uniacid'=>$_W['uniacid']));
        if(empty($goods)){
            return app_error(AppError::$GoodsNotFound);
        }
        ...
        pdo_update('tapgo_tt_member_cart', $arr, array('id' => $id, 'uniacid' => $_W['uniacid'],'openid'=>$_W['openid']));
        return app_json();
    }
    ...
    public function select(){
        ...
        if(!empty($id)){
            ...
            pdo_update('tapgo_tt_member_cart', array('selected' => $select), array('uniacid' => $_W['uniacid'],'openid'=>$_W['openid']));
        }
    ...
    public function count()
    {
        ...
        $cartcount = (int)pdo_fetchcolumn('select ifnull(sum(total),0) from ' . tablename('tapgo_tt_member_cart') . ' where uniacid=:uniacid and openid=:openid and deleted=0 and selected =1', $params);
