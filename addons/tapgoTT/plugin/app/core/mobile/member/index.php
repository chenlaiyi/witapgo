...
class Index_TapgoTtPage extends AppMobilePage
{
	public function main()
	{
		...
		if ($merch_plugin && $merch_data['is_openmerch']) {
			$statics = array('order_0' => pdo_fetchcolumn('select count(*) from ' . tablename('tapgo_tt_order') . ' where openid=:openid and status=0  and uniacid=:uniacid limit 1', $params), 'order_1' => pdo_fetchcolumn('select count(*) from ' . tablename('tapgo_tt_order') . ' where openid=:openid and status=1 and refundid=0 and uniacid=:uniacid limit 1', $params), 'order_2' => pdo_fetchcolumn('select count(*) from ' . tablename('tapgo_tt_order') . ' where openid=:openid and status=2 and refundid=0 and uniacid=:uniacid limit 1', $params), 'order_4' => pdo_fetchcolumn('select count(*) from ' . tablename('tapgo_tt_order') . ' where openid=:openid and refundstate=1 and uniacid=:uniacid limit 1', $params), 'cart' => pdo_fetchcolumn('select ifnull(sum(total),0) from ' . tablename('tapgo_tt_member_cart') . ' where uniacid=:uniacid and openid=:openid and deleted=0 ', $params), 'favorite' => pdo_fetchcolumn('select count(*) from ' . tablename('tapgo_tt_member_favorite') . ' where uniacid=:uniacid and openid=:openid and deleted=0 ', $params));
		}
		 else {
			$statics = array('order_0' => pdo_fetchcolumn('select count(*) from ' . tablename('tapgo_tt_order') . ' where openid=:openid and ismr=0 and status=0  and uniacid=:uniacid and isparent=0 limit 1', $params), 'order_1' => pdo_fetchcolumn('select count(*) from ' . tablename('tapgo_tt_order') . ' where openid=:openid and ismr=0 and status=1 and refundid=0 and uniacid=:uniacid and isparent=0 limit 1', $params), 'order_2' => pdo_fetchcolumn('select count(*) from ' . tablename('tapgo_tt_order') . ' where openid=:openid and ismr=0 and status=2 and refundid=0 and uniacid=:uniacid and isparent=0 limit 1', $params), 'order_4' => pdo_fetchcolumn('select count(*) from ' . tablename('tapgo_tt_order') . ' where openid=:openid and ismr=0 and refundstate=1 and uniacid=:uniacid and isparent=0 limit 1', $params), 'cart' => pdo_fetchcolumn('select ifnull(sum(total),0) from ' . tablename('tapgo_tt_member_cart') . ' where uniacid=:uniacid and openid=:openid and deleted=0 and selected = 1', $params), 'favorite' => pdo_fetchcolumn('select count(*) from ' . tablename('tapgo_tt_member_favorite') . ' where uniacid=:uniacid and openid=:openid and deleted=0 and `type`=0', $params));
		}
		...
		if ($plugin_membercard) {
			...
			$card_history = pdo_fetch('SELECT * FROM ' . tablename('tapgo_tt_member_card_history') . ' ' . "\n\t\t\t\t" . 'WHERE ' . $card_condition . ' limit 1', $params);

			if ($card_history) {
				$hasmembercard = true;
				$hasbuycardnum = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('tapgo_tt_member_card_history') . ' ' . "\n\t\t\t\t" . 'WHERE ' . $card_condition . ' limit 1', $params);
			}
			...
			$allcardnum = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('tapgo_tt_member_card') . 'where ' . $allcard_condition, $allcard_params);
		}
	...
	public function get_member()
	{
		...
		$member = pdo_get('tapgo_tt_member', array('uniacid' => $_W['uniacid'], 'openid' => $openid));

		if (empty($member)) {
			$member = pdo_get('tapgo_tt_member', array('uniacid' => $_W['uniacid'], 'openid' => 'sns_wa_' . $openid));
		}
