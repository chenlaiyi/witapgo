...
class Recharge_TapgoTtPage extends AppMobilePage {
    function main() {
    ...
    function submit() {
        ...
        $sql = "DELETE FROM " . tablename('tapgo_tt_member_log') . " WHERE openid= '{$_W['openid']}' AND status = 0 AND uniacid = {$_W['uniacid']} AND createtime < (unix_timestamp()-86400)";
        pdo_fetch($sql);
        ...
        pdo_insert('tapgo_tt_member_log', $log);
        $logid = pdo_insertid();
        $type = $_GPC['type'];
...
protected function refine(&$openid)
    ...
    function wechat_complete() {
        ...
        $log = pdo_fetch('SELECT * FROM ' . tablename('tapgo_tt_member_log') . ' WHERE `id`=:id and `uniacid`=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $logid));
        if(empty($log)){
            $logno = intval($_GPC['logno']);
            $log = pdo_fetch('SELECT * FROM ' . tablename('tapgo_tt_member_log') . ' WHERE `logno`=:logno and `uniacid`=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':logno' => $logno));
        }
    ...
    function alipay_complete() {
        ...
        if (!empty($log)) {
            ...
            if($alisign){
                if (empty($log['status'])){
                    pdo_update('tapgo_tt_member_log', array('status' => 1, 'rechargetype' =>'alipay', 'apppay'=>2), array('id' => $logid));
