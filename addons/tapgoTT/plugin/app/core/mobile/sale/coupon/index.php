...
class Index_TapgoTtPage extends AppMobilePage
{
            ...
            foreach ($list as $key => &$row) {
                ...
                if ($row['islimitlevel'] == 1) {
                    ...
                    if (!empty($row['limitmemberlevels']) || $row['limitmemberlevels'] == '0') {
                        //会员等级
                        $level1 = pdo_fetchall('select * from ' . tablename('tapgo_tt_member_level') . ' where uniacid=:uniacid and  id in (' . $coupon['limitmemberlevels'] . ') ', array(':uniacid' => $_W['uniacid']));
                    ...
                    if ((!empty($coupon['limitagentlevels']) || $coupon['limitagentlevels'] == '0') && $hascommission) {
                        //分销商等级
                        $level2 = pdo_fetchall('select * from ' . tablename('tapgo_tt_commission_level') . ' where uniacid=:uniacid and id  in (' . $coupon['limitagentlevels'] . ') ', array(':uniacid' => $_W['uniacid']));
                    ...
                    if ((!empty($coupon['limitpartnerlevels']) || $coupon['limitpartnerlevels'] == '0') && $hasglobonus) {
                        //股东等级
                        $level3 = pdo_fetchall('select * from ' . tablename('tapgo_tt_globonus_level') . ' where uniacid=:uniacid and  id in(' . $coupon['limitpartnerlevels'] . ') ', array(':uniacid' => $_W['uniacid']));
                    ...
                    if ((!empty($coupon['limitaagentlevels']) || $coupon['limitaagentlevels'] == '0') && $hasabonus) {
                        //区域代理
                        $level4 = pdo_fetchall('select * from ' . tablename('tapgo_tt_abonus_level') . ' where uniacid=:uniacid and  id in (' . $coupon['limitaagentlevels'] . ') ', array(':uniacid' => $_W['uniacid']));
    ...
    public function pay($a = array(), $b = array()) {
        ...
        if($goodsdetail){
            ...
            if ($coupon['money'] > 0) {
                //删除以前无效的记录
                pdo_delete('tapgo_tt_coupon_log', array('couponid' => $id, 'openid' => $openid, 'status' => 0,'paystatus'=>0));
                $needpay = true;
                //找出上次支付但未参加的记录（例如断电，断网等特殊情况)
                $lastlog = pdo_fetch('select * from ' . tablename('tapgo_tt_coupon_log') . ' where couponid=:couponid and openid=:openid  and status=0 and paystatus=1 and uniacid=:uniacid limit 1', array(':couponid' => $id, ':openid' => $openid, ':uniacid' => $_W['uniacid']));
                if (!empty($lastlog)) {
                    return app_json(array('logid' => $lastlog['id']));
                }
            }else{
                //删除以前无效的记录
                pdo_delete('tapgo_tt_coupon_log', array('couponid' => $id, 'openid' => $openid, 'status' => 0));
            }
        }
        ...
        if($goodsdetail){
            ...
            if ($needpay) {
                ...
                //支付方式
                pdo_update('tapgo_tt_coupon_log', array('paytype' => $useweixin ? 1 : 0), array('id' => $logid));
    ...
    public function payresult($a=array()){
        ...
        $log = pdo_fetch('select id,logno,status,paystatus,couponid from ' . tablename('tapgo_tt_coupon_log') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $logid, ':uniacid' => $_W['uniacid']));
        if(empty($log)){
    ...
    public function getCouponCate() {
        ...
        $param = array();
        $param[':uniacid'] = $_W['uniacid'];
        $sql = 'select * from ' . tablename('tapgo_tt_coupon_category') . ' where uniacid=:uniacid';
        if ($is_openmerch == 0) {
            $sql .= ' and merchid=0';
        } else {
            if (!empty($_GPC['merchid'])) {
    ...
    public function getlist() {
        ...
        $sql = "select id,timelimit,coupontype,timedays,timestart,timeend,thumb,couponname,enough,backtype,deduct,discount,backmoney,backcredit,backredpack,bgcolor,thumb,credit,money,getmax,merchid,total as t,tagtitle,settitlecolor,titlecolor from " . tablename('tapgo_tt_coupon') . " c ";
    ...
    public function getdetail() {
        ...
        if ($coupon['limitgoodtype'] != 0) {
            ...
            $goods = pdo_fetchall('select `title` from ' . tablename('tapgo_tt_goods') . ' where uniacid=:uniacid ' . $where, array(':uniacid' => $_W['uniacid']), 'id');
        }
        if ($coupon['limitgoodcatetype'] != 0) {
            if (!empty($coupon['limitgoodcateids'])) {
                $where = 'and id in(' . $coupon['limitgoodcateids'] . ')';
            }
            $category = pdo_fetchall('select `name`  from ' . tablename('tapgo_tt_category') . ' where uniacid=:uniacid   ' . $where, array(':uniacid' => $_W['uniacid'],), 'id');
        }
        //分类限制
