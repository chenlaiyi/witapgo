...
class My_TapgoTtPage extends AppMobilePage
{
    ...
    public function getlist()
    {
        ...
        $psize = 10;
        $time = time();
        $sql = "select d.id,d.couponid,d.gettime,c.timelimit,c.coupontype,c.timedays,c.timestart,c.timeend,c.thumb,c.couponname,c.enough,c.backtype,c.deduct,c.discount,c.backmoney,c.backcredit,c.backredpack,c.bgcolor,c.thumb,c.merchid,c.tagtitle,c.settitlecolor,c.titlecolor from " . tablename('tapgo_tt_coupon_data') . " d";
        ...
        pdo_update('tapgo_tt_coupon_data', array('isnew' => 0), array('uniacid' => $_W['uniacid'], 'openid' => $_W['openid']));
        if (empty($coupons)) {
            $coupons = array();
        }
        foreach ($coupons as $i => &$row) {
            ...
            $row['iconurl'] = $_W['siteroot']."addons/tapgo_ttv2/template/mobile/default/static/images/coupon/".$row['imgname'].".png";
    ...
    public function getdetail()
    {
        ...
        $data = pdo_fetch('select * from ' . tablename('tapgo_tt_coupon_data') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
        if (empty($data)) {
            if (empty($coupon)) {
        ...
        $num = pdo_fetchcolumn('select ifnull(count(*),0) from ' . tablename('tapgo_tt_coupon_data') . ' where couponid=:couponid and openid=:openid and uniacid=:uniacid and used=0 '
    ...
    public function showcoupon2()
    {
        ...
        $data = pdo_fetch('select c.*  from ' . tablename('tapgo_tt_coupon_data') . '  cd inner join  ' . tablename('tapgo_tt_coupon') . ' c on cd.couponid = c.id  where cd.id=:id and cd.uniacid=:uniacid and coupontype =0  limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
        if (empty($data)) {
            if (empty($coupon)) {
                return app_error(AppError::$CouponRecordNotFound);
            }
        ...
        if ($data['limitgoodcatetype'] == 1 && !empty($data['limitgoodcateids'])) {
            ...
            if (count($limitcateids) > 0) {
                ...
                foreach ($limitcateids as $cateid) {
                    ...
                    $table .= 'select * from ' . tablename('tapgo_tt_goods') . ' where FIND_IN_SET(' . $cateid . ',cates)';
                }
                $table .= ') g';
            } else {
