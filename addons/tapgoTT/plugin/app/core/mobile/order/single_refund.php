...
class Single_Refund_TapgoTtPage extends AppMobilePage {
    ...
    protected function globalData() {
        ...
        $refund = pdo_fetch("select id,orderid from ".tablename('tapgo_tt_order_refund')." where uniacid = :uniacid and orderid = :orderid and ordergoodsids in (:goodsid) and status > 0 limit 1 ",array(':uniacid'=>$uniacid,':orderid'=>$order['id'],':goodsid'=>$order_goodsid));
    ...
    function cancel() {
        ...
        pdo_update('tapgo_tt_order_goods', array('single_refundstate' => 0), array('id' => $order_goodsid, 'uniacid' => $uniacid));
