<?php
//云硕科技解密修复、qq960327091、淘宝店铺：https://yscms.taobao.com/
if (!defined('IN_IA')) {
	exit('Access Denied');
}

require_once TAPGO_TT_PLUGIN . 'app/core/page_mobile.php';
class Pay_TapgoTtPage extends AppMobilePage
{
	const AFTER_PAY = 1;
		...
		if (!empty($set['pay']['wxapp']) && 0 < $order['price'] && $this->iswxapp) {
			...
			if (!is_error($res)) {
				...
				if (!empty($res['package']) && strexists($res['package'], 'prepay_id=')) {
					...
					pdo_update('tapgo_tt_order', array('wxapp_prepay_id' => $prepay_id), array('id' => $orderid, 'uniacid' => $_W['uniacid']));
				}
			}
			else {
				$wechat['payinfo'] = $res;
...
protected function refine(&$openid)
	...
	public function complete()
	{
		...
		if ($type == 'wechat') {
			...
			if (!is_error($payquery)) {
				...
				@session_start();
				$_SESSION[TAPGO_TT_PREFIX . '_order_pay_complete'] = 1;
				pdo_update('tapgo_tt_order', array('apppay' => 2), array('id' => $order['id']));
				return $this->success($orderid);
			}
	...
	protected function success($orderid)
	{
		...
		if (!empty($order['virtual']) && !empty($order['virtual_str'])) {
			...
			$result['virtualtemp'] = pdo_fetch('SELECT linktext, linkurl FROM ' . tablename('tapgo_tt_virtual_type') . ' WHERE id=:id AND uniacid=:uniacid LIMIT 1', array(':id' => $order['virtual'], ':uniacid' => $_W['uniacid']));
		}
	...
	protected function creditpay_log($openid = '', $fee = 0, $orderid = 0)
	{
		...
		$order = pdo_fetch('select id,ordersn from ' . tablename('tapgo_tt_order') . ' where id=:id AND uniacid=:uniacid LIMIT 1', array(':id' => $orderid, ':uniacid' => $uniacid));
		...
		pdo_insert('tapgo_tt_member_log', $log_data);
	}
