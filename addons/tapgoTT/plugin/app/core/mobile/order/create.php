...
class Create_TapgoTtPage extends AppMobilePage
{
    ...
    public function main()
    {
        ...
        if( !$packageid ) 
        {
            ...
            if( empty($id) ) 
            {
                ...
                {
                    $sql = "SELECT c.goodsid,c.total,g.maxbuy,g.type,g.issendfree,g.isnodiscount,g.ispresell,g.presellprice as gpprice,o.presellprice,g.preselltimeend,g.presellsendstatrttime,g.presellsendtime,g.presellsendtype" . ",g.weight,o.weight as optionweight,g.title,g.thumb,ifnull(o.marketprice, g.marketprice) as marketprice,o.title as optiontitle,c.optionid,g.isfullback," . " g.storeids,g.isverify,g.isforceverifystore,g.deduct,g.manydeduct,g.manydeduct2,g.virtual,o.virtual as optionvirtual,discounts," . " g.deduct2,g.ednum,g.edmoney,g.edareas,g.edareas_code,g.diyformtype,g.diyformid,diymode,g.dispatchtype,g.dispatchid,g.dispatchprice,g.minbuy " . " ,g.isdiscount,g.isdiscount_time,g.isdiscount_time_start,g.isdiscount_discounts,g.cates, " . " g.virtualsend,invoice,o.specs,g.merchid,g.checked,g.merchsale," . " g.buyagain,g.buyagain_islong,g.buyagain_condition, g.buyagain_sale" . " FROM " . tablename("tapgo_tt_member_cart") . " c " . " left join " . tablename("tapgo_tt_goods") . " g on c.goodsid = g.id " . " left join " . tablename("tapgo_tt_goods_option") . " o on c.optionid = o.id " . " where c.openid=:openid and c.selected=1 and  c.deleted=0 and c.uniacid=:uniacid  order by c.id desc";
                    $goods = pdo_fetchall($sql, array( ":uniacid" => $uniacid, ":openid" => $openid ));
                }
            ...
            else
            {
                ...
                if( !empty($optionid) ) 
                {
                    $option = pdo_fetch("select * from " . tablename("tapgo_tt_goods_option") . " where id=:id and goodsid=:goodsid and uniacid=:uniacid  limit 1", array( ":uniacid" => $uniacid, ":goodsid" => $id, ":id" => $optionid ));
                    if( !empty($option) ) 
                    {
                        $data["optionid"] = $optionid;
            ...
            foreach( $goods as &$g ) 
            {
                ...
                else
                {
                    ...
                    if( 0 < $g["usermaxbuy"] ) 
                    {
                        $order_goodscount = pdo_fetchcolumn("select ifnull(sum(og.total),0)  from " . tablename("tapgo_tt_order_goods") . " og " . " left join " . tablename("tapgo_tt_order") . " o on og.orderid=o.id " . " where og.goodsid=:goodsid and  o.status>=1 and o.openid=:openid  and og.uniacid=:uniacid ", array( ":goodsid" => $g["goodsid"], ":uniacid" => $uniacid, ":openid" => $openid ));
            ...
            if( $isverify ) 
            {
                ...
                if( empty($storeids) ) 
                {
                    if( 0 < $merchid ) 
                    {
                        $stores = pdo_fetchall("select * from " . tablename("tapgo_tt_merch_store") . " where  uniacid=:uniacid and merchid=:merchid and status=1 and type in(2,3)", array( ":uniacid" => $_W["uniacid"], ":merchid" => $merchid ));
                    }
                    else
                    {
                        $stores = pdo_fetchall("select * from " . tablename("tapgo_tt_store") . " where  uniacid=:uniacid and status=1 and type in(2,3)", array( ":uniacid" => $_W["uniacid"] ));
                    }
                ...
                else
                {
                    if( 0 < $merchid ) 
                    {
                        $stores = pdo_fetchall("select * from " . tablename("tapgo_tt_merch_store") . " where id in (" . implode(",", $storeids) . ") and uniacid=:uniacid and merchid=:merchid and status=1 and type in(2,3)", array( ":uniacid" => $_W["uniacid"], ":merchid" => $merchid ));
                    }
                    else
                    {
                        $stores = pdo_fetchall("select * from " . tablename("tapgo_tt_store") . " where id in (" . implode(",", $storeids) . ") and uniacid=:uniacid and status=1 and type in(2,3)", array( ":uniacid" => $_W["uniacid"] ));
                    }
                ...
                if( $isforceverifystore ) 
                {
                    ...
                    if( 0 < $merch_id ) 
                    {
                        $store_list = pdo_fetchall("select * from " . tablename("tapgo_tt_merch_store") . " where " . $storeids_condition . "  uniacid=:uniacid and merchid=:merchid and status=1 and type in(2,3) order by displayorder desc,id desc", array( ":uniacid" => $_W["uniacid"], ":merchid" => $merch_id ));
                    }
                    else
                    {
                        $store_list = pdo_fetchall("select * from " . tablename("tapgo_tt_store") . " where  " . $storeids_condition . "  uniacid=:uniacid and status=1 and type in(2,3) order by displayorder desc,id desc", array( ":uniacid" => $_W["uniacid"] ));
                    }
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        $gift = array(  );
                        $giftdata = pdo_fetch("select giftgoodsid,activity,orderprice from " . tablename("tapgo_tt_gift") . " where uniacid = " . $uniacid . " and id = " . $giftid . " and status = 1 and starttime <= " . time() . " and endtime >= " . time() . " ");
                    ...
                    else
                    {
                        ...
                        foreach( $gifts as $key => $value ) 
                        {
                            ...
                            {
                                $giftinfo = pdo_fetch("select id,title,thumb,marketprice,total from " . tablename("tapgo_tt_goods") . " where uniacid = " . $uniacid . " and status = 2  and id = " . $val . " ");
                                if( $giftinfo ) 
                                {
                                    $isgift = 1;
        ...
        else
        {
            ...
            {
                $goods[$key] = pdo_fetch("select id,title,thumb,marketprice,merchid,dispatchtype,dispatchid,dispatchprice from " . tablename("tapgo_tt_goods") . "\n                            where id = " . $value["goodsid"] . " and uniacid = " . $uniacid . " ");
                if( $is_openmerch == 1 ) 
                {
        ...
        $result["include_dispatch"] = ($saleset["include_dispatch"] ? $saleset["include_dispatch"] : 1);
        if( $saleset["dispatchnodeduct"] == 1 ) 
        {
            $tmp_dispatch_price = $dispatch_price;
        }

        if( $result["realprice"] - $result["seckill_price"] - $result["merch_enoughdeduct"] - $result["enoughdeduct"] - $result["discountprice"] - $tmp_dispatch_price < $result["deductcredit2"] ) 
        {
            $result["deductcredit2"] = $result["realprice"] - $result["seckill_price"] - $result["merch_enoughdeduct"] - $result["enoughdeduct"] - $result["discountprice"] - $tmp_dispatch_price;
        }
    ...
    public function caculate()
    {
        ...
        if( 0 < $packageid ) 
        {
            ...
            if( is_array($goodsarr) ) 
            {
                ...
                {
                    $goods[$key] = pdo_fetch("select id,title,thumb,marketprice from " . tablename("tapgo_tt_goods") . "\n                            where id = " . $value["goodsid"] . " and uniacid = " . $uniacid . " ");
                    $option = array(  );
                    $packagegoods = array(  );
                    if( 0 < $value["optionid"] ) 
                    {
        ...
        else
        {
            if( is_array($goodsarr) ) 
            {
                ...
                foreach( $goodsarr as &$g ) 
                {
                    ...
                    if( !empty($optionid) ) 
                    {
                        $option = pdo_fetch("select id,title,marketprice,presellprice,goodssn,productsn,stock,`virtual`,weight,cycelbuy_periodic from " . tablename("tapgo_tt_goods_option") . " where id=:id and goodsid=:goodsid and uniacid=:uniacid  limit 1", array( ":uniacid" => $uniacid, ":goodsid" => $goodsid, ":id" => $optionid ));
                        if( !empty($option) ) 
                        {
                            $data["optionid"] = $optionid;
                            $data["optiontitle"] = $option["title"];
        ...
        if( $couponid ) 
        {
            ...
            $saleset["dispatchnodeduct"] = 1;
            if( $saleset["dispatchnodeduct"] ) 
            {
                $tmp_dispatch = $dispatch_price;
            }

            if( $realprice - $coupon_deductprice - $tmp_dispatch < $deductcredit2 ) 
            {
                $deductcredit2 = $realprice - $coupon_deductprice - $tmp_dispatch;
            }

            if( $realprice - $coupon_deductprice - $tmp_dispatch < $deductmoney ) 
            {
                $deductmoney = $realprice - $coupon_deductprice - $tmp_dispatch;
            }
        ...
        $return_array["include_dispatch"] = ($saleset["dispatchnodeduct"] ? $saleset["dispatchnodeduct"] : 1);
    ...
    public function submit()
    {
        ...
        if( $giftid ) 
        {
            ...
            if( $giftdata["giftgoodsid"] ) 
            {
                ...
                foreach( $giftgoodsid as $key => $value ) 
                {
                    $giftinfo = pdo_fetch("select id as goodsid,title,thumb,status from " . tablename("tapgo_tt_goods") . " where uniacid = " . $uniacid . " and status = 2 and id = " . $value . " and deleted = 0 ");
                    $gift[$key] = $giftinfo;
                    if( $giftinfo ) 
                    {
                        $canmerge = true;
        ...
        foreach( $goods as $g ) 
        {
            ...
            else
            {
                ...
                {
                    $order_goodscount = pdo_fetchcolumn("select ifnull(sum(og.total),0)  from " . tablename("tapgo_tt_order_goods") . " og " . " left join " . tablename("tapgo_tt_order") . " o on og.orderid=o.id " . " where og.goodsid=:goodsid and  o.status>=1 and o.openid=:openid  and og.uniacid=:uniacid ", array( ":goodsid" => $data["goodsid"], ":uniacid" => $uniacid, ":openid" => $openid ));
            ...
            if( !empty($optionid) ) 
            {
                ...
                if( $packageid ) 
                {
                    $packageoption = pdo_fetch("select packageprice from " . tablename("tapgo_tt_package_goods_option") . "\n                            where uniacid = " . $uniacid . " and goodsid = " . $goodsid . " and optionid = " . $optionid . " and pid = " . $packageid . " ");
            ...
            else
            {
                if( $packageid ) 
                {
                    $pg = pdo_fetch("select packageprice from " . tablename("tapgo_tt_package_goods") . "\n                                where uniacid = " . $uniacid . " and goodsid = " . $goodsid . " and pid = " . $packageid . " ");
            ...
            else
            {
                if( 0 < $_GPC["fromquick"] ) 
                {
                    $cartdata = pdo_fetch("select id,diyformdataid,diyformfields,diyformdata from " . tablename("tapgo_tt_quick_cart") . " " . " where goodsid=:goodsid and optionid=:optionid and openid=:openid and deleted=0 order by id desc limit 1", array( ":goodsid" => $data["goodsid"], ":optionid" => intval($data["optionid"]), ":openid" => $openid ));
                    if( !empty($cartdata) ) 
                    {
                        $data["diyformdataid"] = $cartdata["diyformdataid"];
                        $data["diyformdata"] = $cartdata["diyformdata"];
        ...
        if( is_array($carrier) ) 
        {
            ...
            pdo_update("tapgo_tt_member", $up, array( "id" => $member["id"], "uniacid" => $_W["uniacid"] ));
            if( !empty($member["uid"]) ) 
            {
                load()->model("mc");
        ...
        else
        {
            ...
            {
                $vgoods = $allgoods[0];
                pdo_update("tapgo_tt_goods", array( "sales" => $vgoods["sales"] + $vgoods["total"] ), array( "id" => $vgoods["goodsid"] ));
            }
        ...
        if( $pluginc ) 
        {
            ...
            if( intval($set["become_child"]) == 1 ) 
            {
                $parant_member = $memberModel->getMember($_GPC["mid"]);
                ...
                {
                    pdo_update("tapgo_tt_member", array( "agentid" => $parant_member["id"], "childtime" => time(), "fixagentid" => 1 ), array( "uniacid" => $_W["uniacid"], "id" => $member["id"] ));
                }
