...
class Index_TapgoTtPage extends AppMobilePage
{
    ...
    function get_list()
    {
        ...
        foreach ($list as &$row) {
            ...
            //所有商品
            $sql = "SELECT og.id,og.goodsid,og.total,g.title,g.thumb,g.type, og.price,og.optionname as optiontitle,op.marketprice,g.marketprice as gprice,og.optionid,op.specs,g.merchid,g.status,og.single_refundid,og.single_refundstate FROM " . tablename('tapgo_tt_order_goods') . " og "
...
//            print_r($row['goods']);exit;
            ...
            if ($row['refundstate'] > 0 && !empty($row['refundid'])) {
                ...
                $refund = pdo_fetch("select * from " . tablename('tapgo_tt_order_refund') . ' where id=:id and uniacid=:uniacid and orderid=:orderid limit 1'
    ...
    function detail()
    {
        ...
        $order = pdo_fetch("select * from " . tablename('tapgo_tt_order') . ' where id=:id and uniacid=:uniacid and openid=:openid limit 1'
        ...
        $goods = pdo_fetchall("select og.id as ordergoodsid,og.single_refundstate,og.sendtime,g.id,g.type, og.goodsid,og.price,g.title,g.thumb,g.status,og.total,g.credit,og.optionid,og.optionname as optiontitle,g.isverify,g.isfullback,g.refund,g.cannotrefund,g.returngoods,g.exchange,g.storeids{$diyformfields}  from " . tablename('tapgo_tt_order_goods') . " og "
        ...
        if($order['sendtype']>0 && $order['status']>=1){
            ...
            $order_goods = pdo_fetchall("select orderid,goodsid,sendtype,expresscom,expresssn,express,sendtime from ".tablename('tapgo_tt_order_goods')."
    ...
    function express()
    {
        ...
        if(!empty($cycelid)){
            $order = pdo_fetch("select expresscom,expresssn,addressid,status,express,address from ".tablename('tapgo_tt_cycelbuy_periods') ." where id=:id and uniacid=:uniacid",array(':id'=>$cycelid,':uniacid'=>$uniacid));

        }else{
            $order = pdo_fetch("select expresscom,expresssn,addressid,status,express,sendtype,address,carrier from " . tablename('tapgo_tt_order') . ' where id=:id and uniacid=:uniacid and openid=:openid limit 1'
                , array(':id' => $orderid, ':uniacid' => $uniacid, ':openid' => $openid));
            if (empty($order)) {
                return app_error(AppError::$OrderNotFound);
            }
        ...
        if(!empty($order['sendtype']) && $sendtype == 0){
            for($i=1;$i<=intval($order['sendtype']);$i++){
                ...
                $goods_arr=pdo_fetchall("select g.title,g.thumb,og.total,og.optionname as optiontitle,og.expresssn,og.express,
                    og.sendtype,og.expresscom,og.sendtime from " . tablename('tapgo_tt_order_goods') . " og "
                    . " left join " . tablename('tapgo_tt_goods') . " g on g.id=og.goodsid "
        ...
        $goods = pdo_fetchall("select og.goodsid,og.price,g.title,g.thumb,og.total,g.credit,og.optionid,og.optionname as optiontitle,g.isverify,og.expresssn,og.express,
            og.sendtype,og.expresscom,og.sendtime,g.storeids from " . tablename('tapgo_tt_order_goods') . " og "
            . " left join " . tablename('tapgo_tt_goods') . " g on g.id=og.goodsid "
...
//        }
    ...
    function cycel_orderList(){
        ...
        foreach ($list as &$row) {
            ...
            //所有商品
            $sql = "SELECT og.id,og.goodsid,og.total,g.title,g.thumb,og.price,og.optionname as optiontitle,og.optionid,op.specs,g.merchid,g.status,og.single_refundid,og.single_refundstate FROM " . tablename('tapgo_tt_order_goods') . " og "
                . " left join " . tablename('tapgo_tt_goods') . " g on og.goodsid = g.id "
                . " left join " . tablename('tapgo_tt_goods_option') . " op on og.optionid = op.id "
                . " where $scondition order by og.id asc";
