...
class Index_TapgoTtPage extends AppMobilePage {
    ...
    function main() {
        ...
        if(is_array($page['data']['page']) && !empty($page['data']['page']['diyadv'])){
            $startadvitem = pdo_fetch('SELECT * FROM '. tablename('tapgo_tt_wxapp_startadv'). ' WHERE id=:id AND uniacid=:uniacid', array(
        ...
        if(!empty($dataurl)){
            if(strpos($dataurl,'/pages/') === false){
                ...
                if($dataParams[0] == 'category'){
                    ...
                    $goodsql = 'SELECT id,displayorder,title,subtitle,thumb,marketprice,hascommission,nocommission,commission,commission1_rate,commission1_rate,marketprice,commission1_pay,maxprice,productprice,minprice,maxprice,isdiscount,isdiscount_time,isdiscount_discounts,sales,salesreal,total,description,bargain,`type`,ispresell,`virtual`,hasoption,video FROM ' .tablename('tapgo_tt_goods') .' WHERE FIND_IN_SET('.$pcate.',cates) AND status = 1 AND deleted = 0 AND uniacid ='.$_W['uniacid'].' ORDER BY displayorder DESC'.' limit 0,'.$goodsnum;
                    $count = pdo_fetch('SELECT count(id) as count FROM ' .tablename('tapgo_tt_goods') .' WHERE FIND_IN_SET('.$pcate.',cates) AND status = 1 AND deleted = 0 AND uniacid ='.$_W['uniacid']);
                    ...
                    foreach($list['list'] as $k => $v){
                        ...
                        if($v['hasoption']==1) {
                            $pricemax = array();
                            $options = pdo_fetchall('select * from ' . tablename('tapgo_tt_goods_option') . ' where goodsid=:goodsid and                               uniacid=:uniacid order by displayorder asc', array(':goodsid' => $v['id'], ':uniacid' => $_W['uniacid']));
...
//                    秒杀
                ...
                elseif($dataParams[0] == 'groups'){
                    ...
                    $goodsql = 'SELECT id,displayorder,title,subtitle,thumb,hascommission,nocommission,commission,commission1_rate,commission1_rate,marketprice,commission1_pay,maxprice,marketprice,productprice,minprice,maxprice,isdiscount,isdiscount_time,isdiscount_discounts,sales,salesreal,total,description,bargain,`type`,ispresell,`virtual`,hasoption,video FROM ' .tablename('tapgo_tt_goods') .' WHERE id in('.$goodsid.') AND status = 1 AND deleted = 0 AND uniacid ='.$_W['uniacid'].' ORDER BY displayorder DESC'.' limit 0,'.$goodsnum;
                    $count = pdo_fetch('SELECT count(id) as count FROM ' .tablename('tapgo_tt_goods') .' WHERE id in('.$goodsid.') AND status = 1 AND deleted = 0 AND uniacid ='.$_W['uniacid']);
                    $list['list'] = pdo_fetchall($goodsql);
                    $list['count'] = $count['count'];
                    foreach($list['list'] as $k => $v){
                        $list['list'][$k]['thumb'] = tomedia($v['thumb']);
...
//                    秒杀
                    ...
                    if(!empty($goodsids)){
                        ...
                        $sql = 'SELECT id,showlevels,displayorder,title,subtitle,thumb,marketprice,hascommission,nocommission,commission,commission1_rate,commission1_rate,marketprice,commission1_pay,maxprice,productprice,minprice,maxprice,isdiscount,isdiscount_time,isdiscount_discounts,sales,salesreal,total,description,bargain,`type`,ispresell,`virtual`,hasoption,video FROM ' .tablename('tapgo_tt_goods') .' WHERE id in('.$goodsid.') AND uniacid ='.$_W['uniacid'].' ORDER BY displayorder DESC'.' limit 0,'.$goodsnum;
                        $count = pdo_fetch('SELECT count(id) as count FROM ' .tablename('tapgo_tt_goods') .' WHERE id in('.$goodsid.') AND uniacid ='.$_W['uniacid']);
...
//                    秒杀
                    ...
                    if(!empty($storesids)){
                        ...
                        $sql = 'SELECT id,storename,displayorder FROM ' .tablename('tapgo_tt_store') .' WHERE id in('.$storesid.') AND uniacid ='.$_W['uniacid'].' ORDER BY displayorder DESC'.' limit 0,'.$storenum;
                        $count = pdo_fetch('SELECT count(id) as count FROM ' .tablename('tapgo_tt_store') .' WHERE id in('.$storesid.') AND uniacid ='.$_W['uniacid']);
                        $list['list'] = pdo_fetchall($sql);
                        $list['count'] = $count['count'];
