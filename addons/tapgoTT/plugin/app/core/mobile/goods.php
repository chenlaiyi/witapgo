...
require TAPGO_TT_PLUGIN . 'app/core/page_mobile.php';

class Goods_TapgoTtPage extends AppMobilePage
{
    ...
    public function get_detail() {
        ...
        {
            $goods['end_video_url'] = parseTencentVideo($goods['video_url']);

        }else
        {
            $goods['end_video_url'] = tomedia($goods['video_url']);
        }
...
//        }
        ...
        //参数
        $goods['params'] = pdo_fetchall("SELECT * FROM " . tablename('tapgo_tt_goods_param') . " WHERE uniacid=:uniacid and goodsid=:goodsid order by displayorder asc", array(':uniacid' => $uniacid, ":goodsid" => $goods['id']));
        ...
        $isfullback = false;
        if($goods['isfullback']){
            $fullbackgoods = pdo_fetch("SELECT * FROM ".tablename('tapgo_tt_fullback_goods')." WHERE uniacid = :uniacid and goodsid = :goodsid and status =1 limit 1 ",array(':uniacid'=>$uniacid,':goodsid'=>$id));
            ...
            if($goods['hasoption']==1){
                $fullprice = pdo_fetch("select min(allfullbackprice) as minfullprice,max(allfullbackprice) as maxfullprice,min(allfullbackratio) as minfullratio
                            ...
                            from ".tablename('tapgo_tt_goods_option')." where goodsid = :goodsid",array(':goodsid'=>$id));
        ...
        $grftarray = array();
        $i = 0;
        $gifts = pdo_fetchall("select id,goodsid,giftgoodsid,thumb,title from ".tablename('tapgo_tt_gift')." where uniacid = :uniacid and activity = 2 and status = 1 and starttime <= :starttime and endtime >= :endtime ",array(':uniacid'=>$uniacid,':starttime'=>time(),':endtime'=>time()));
            ...
            foreach ($gid as $ke => $val){
                if($val==$id){
                    ...
                    foreach($giftgoods as $k => $val){
                        $giftdata= pdo_fetch("select id,title,thumb,marketprice from ".tablename('tapgo_tt_goods')." where uniacid = :uniacid and deleted = 0 and total > 0 and status = 2 and id = :id ",array(':uniacid'=>$uniacid,':id'=>$val));
                        if(!empty($giftdata)){
                            $isgift = 1;
        ...
        if($goods['isdiscount'] && $goods['isdiscount_time']>=time() && $goods['isdiscount_time_start'] < time()){
            ...
            if ($goods['hasoption']){
                $max = pdo_fetch(" SELECT marketprice FROM ".tablename('tapgo_tt_goods_option'). " WHERE uniacid=:uniacid AND goodsid=:goodsid ORDER BY marketprice DESC ",array(':uniacid'=>$_W['uniacid'],':goodsid'=>$goods['id']));
                $min = pdo_fetch(" SELECT marketprice FROM ".tablename('tapgo_tt_goods_option'). " WHERE uniacid=:uniacid AND goodsid=:goodsid ORDER BY marketprice ASC ",array(':uniacid'=>$_W['uniacid'],':goodsid'=>$goods['id']));
        ...
        // 旧标签
        $labelstyle = pdo_fetch("SELECT id,uniacid,style FROM " . tablename('tapgo_tt_goods_labelstyle') . " WHERE uniacid=:uniacid LIMIT 1", array(
            ':uniacid'=>$uniacid
        ));
...
//                    秒杀
        ...
        if ($goods['isverify'] == 2) {
            ...
            if (empty($storeids)) {
                //全部门店
                if ($merchid > 0) {
                    $stores = pdo_fetchall('select * from ' . tablename('tapgo_tt_merch_store') . ' where  uniacid=:uniacid and merchid=:merchid and status=1 ', array(':uniacid' => $_W['uniacid'], ':merchid' => $merchid));
                } else {
                    $stores = pdo_fetchall('select * from ' . tablename('tapgo_tt_store') . ' where  uniacid=:uniacid and status=1', array(':uniacid' => $_W['uniacid']));
                }
            } else {
                if ($merchid > 0) {
                    $stores = pdo_fetchall('select * from ' . tablename('tapgo_tt_merch_store') . ' where id in (' . implode(',', $storeids) . ') and uniacid=:uniacid and merchid=:merchid and status=1', array(':uniacid' => $_W['uniacid'], ':merchid' => $merchid));
                } else {
                    $stores = pdo_fetchall('select * from ' . tablename('tapgo_tt_store') . ' where id in (' . implode(',', $storeids) . ') and uniacid=:uniacid and status=1', array(':uniacid' => $_W['uniacid']));
                }
            }
        }
        ...
        //套餐
        $package_goods = array();
        $package_goods = pdo_fetch("select pg.id,pg.pid,pg.goodsid,p.displayorder,p.title from ".tablename('tapgo_tt_package_goods')." as pg
...
//      秒杀价显示   lgt
//            $productprice = pdo_fetch("select max(productprice) as productprice,min(productprice) as minProductprice from ".tablename('tapgo_tt_goods_option')." where goodsid = :goodsid",array(':goodsid'=>$id));
//            if(!empty($productprice)){
...
//            $minprice = $maxprice = $goods['marketprice'] = $seckillinfo['price'];  //秒杀价格显示问题 lgt
        ...
        if ($touch_loop) {
            ...
            $info = pdo_get('tapgo_tt_touch_loop_list', array('goods_id' => $goods['id'], 'uniacid' => $uniacid, 'status' => 1));
            if (!empty($info)) {
    ...
    public function get_content() {
        ...
        //商品
        $goods = pdo_fetch("select content from " . tablename('tapgo_tt_goods') . " where id=:id and uniacid=:uniacid limit 1", array(':id' => $id, ':uniacid' => $_W['uniacid']));
    ...
    public function get_picker() {
        ...
        if( $seckill){
            ...
            {
                $option_tmp = pdo_get('tapgo_tt_goods_option',array('id'=>$op['optionid'],'goodsid'=>$op['goodsid'],'uniacid'=>$op['uniacid']));
...
//                exit();
        ...
        if (!empty($goods) && $goods['hasoption']) {
            ...
            foreach($specs as &$spec) {
                $spec['items'] = pdo_fetchall('select *  from '.tablename('tapgo_tt_goods_spec_item')." where specid=:specid and `show`=1 order by displayorder asc",array(':specid'=>$spec['id']));
            }
            unset($spec);
            $options = pdo_fetchall('select *  from ' . tablename('tapgo_tt_goods_option') . ' where goodsid=:goodsid and uniacid=:uniacid order by displayorder asc', array(':goodsid' => $id, ':uniacid' => $_W['uniacid']));
        }
...
//                获取会员等级
            ...
            if(!empty($member_levelid)) {
                $member_level = pdo_fetch('select * from ' . tablename('tapgo_tt_member_level') . ' where id=:id and uniacid=:uniacid and enabled=1 limit 1', array(':id' => $member_levelid, ':uniacid' => $_W['uniacid']));
        ...
        if($sale_plugin){
            ...
            $gifts = array();
            $giftgoods = array();
            $gifts = pdo_fetchall("select id,goodsid,giftgoodsid,thumb,title from ".tablename('tapgo_tt_gift')." where uniacid = ".$_W['uniacid']." and activity = 2 and status = 1 and starttime <= ".time()." and endtime >= ".time()."  ");
            foreach($gifts as $key => &$value){
                $gid = explode(",",$value['goodsid']);
                foreach ($gid as $ke => $val){
                    if($val==$id){
                        $giftgoods = explode(",",$value['giftgoodsid']);
                        foreach($giftgoods as $k => $val){
                            $giftdata =  pdo_fetch("select id,title,thumb,marketprice,total from ".tablename('tapgo_tt_goods')." where uniacid = ".$_W['uniacid']." and deleted = 0 and total>0  and status = 2 and id = ".$val." ");
                            if(!empty($giftdata)){
                                $isgift = 1;
                                $gifts[$key]['gift'][$k] = $giftdata;
