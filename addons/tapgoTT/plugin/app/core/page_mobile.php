...
require_once TAPGO_TT_PLUGIN . 'app/core/error_code.php';
$iswxapp = false;
$openid = '';
...
class AppMobilePage extends PluginMobilePage {
	...
	private function checkUniacid() {
        ...
        $bind = pdo_fetch('SELECT * FROM '. tablename('tapgo_tt_wxapp_bind'). ' WHERE wxapp=:wxapp LIMIT 1', array(':wxapp'=>$_W['uniacid']));
        if(!empty($bind)){
