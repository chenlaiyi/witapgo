...
define('TAPGO_TT_NEW_AUTH_WXAPP', 'http://renren.weapp.cc/'); // http://newapps.cwlwl.com/
!defined('SITE_ID') && define("SITE_ID",base64_encode($_SERVER["HTTP_HOST"]));
class Newrelease_TapgoTtPage extends PluginWebPage
{
	...
	public function main()
	{
		...
		if (is_error($auth)) {
			...
			$authUrl = TAPGO_TT_NEW_AUTH_WXAPP . 'auth/auth?id=' . $auth['id'];
			...
			$log = pdo_fetchall('select * from ' . tablename('tapgo_tt_upwxapp_log') . ' where uniacid=:uniacid and type=1 order by id desc', array(':uniacid' => $_W['uniacid']));
			$test_code = IA_ROOT . '/addons/tapgo_ttv2/plugin/app/static/images/test_code_' . $_W['uniacid'] . '.jpg';
			$version_time = 0;
			...
			$wxcode = IA_ROOT . '/addons/tapgo_ttv2/plugin/app/static/images/wxcode_' . $_W['uniacid'] . '.jpg';
	...
	public function upload()
	{
		...
		$last_log = pdo_fetch('select * from ' . tablename('tapgo_tt_upwxapp_log') . ' where uniacid=:uniacid and type=1 order by id desc limit 1', array(':uniacid' => $_W['uniacid']));
		@session_start();
		...
		if (empty($ticket)) {
			$need_scan = 1;
			load()->func('communication');
			$res = ihttp_get(TAPGO_TT_NEW_AUTH_WXAPP . 'generate/getqrcode');
	...
	public function getstatus()
	{
		...
		$res = ihttp_get(TAPGO_TT_NEW_AUTH_WXAPP . 'generate/getstatus?uuid=' . $uuid);
		$content = json_decode($res['content'], true);
	...
	public function getticket()
	{
		...
		$res = ihttp_get(TAPGO_TT_NEW_AUTH_WXAPP . 'generate/getticket?code=' . $code);
		$content = json_decode($res['content'], true);
	...
	public function submit()
	{
		...
		$diy_str = '';
		$list = pdo_fetchall('SELECT `data` FROM ' . tablename('tapgo_tt_wxapp_page') . ' WHERE uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
		...
		load()->func('communication');
		$appid = $app_set['appid'];
		$request = ihttp_request(TAPGO_TT_NEW_AUTH_WXAPP . 'generate/upload?id=' . $auth['id'], array('version' => $version, 'describe' => $describe, 'tabBar' => $tabBar, 'ticket' => $ticket,'appid' => $appid , 'site_id' => SITE_ID,'uniacid' => $_W['uniacid'], 'appIds' => $appIds,'is_goods' => $isGoods,'is_live' => $isLive), array('Content-Type' => 'application/x-www-form-urlencoded'));
		...
		if ($content['status'] != 1) {
			...
			pdo_insert('tapgo_tt_upwxapp_log', $data);
			$wxcode = IA_ROOT . '/addons/tapgo_ttv2/plugin/app/static/images/test_code_' . $_W['uniacid'] . '.jpg';
	...
	public function uploadstatus()
	{
		...
		load()->func('communication');
		$response = ihttp_get(TAPGO_TT_NEW_AUTH_WXAPP . 'generate/uploadstatus?ticket=' . $ticket);
