...
load()->model('miniapp');
class Tongrelease_TapgoTtPage extends PluginWebPage {
    private $key = 'asdf734JH3464tr56GJ';
    public function main() {
        global $_W;
        $error                   = NULL;
        $log                     = pdo_fetchall('select * from ' . tablename('tapgo_tt_upwxapp_log') . ' where uniacid=:uniacid and type=1 order by id desc', array(
            ':uniacid' => $_W['uniacid']
        ));
        $test_code               = IA_ROOT . '/addons/tapgo_ttv2/plugin/app/static/images/test_code_' . $_W['uniacid'] . '.jpg';
        $app_info                = $this->miniapp_app_info('tapgo_ttv2');
    ...
    public function upload() {
        ...
        $last_log = pdo_fetch('select * from ' . tablename('tapgo_tt_upwxapp_log') . ' where uniacid=:uniacid and type=1 order by id desc limit 1', array(
            ':uniacid' => $_W['uniacid']
        ));
        @session_start();
    ...
    public function uploadstatus() {
        ...
        $path = IA_ROOT . '/addons/tapgo_ttv2/plugin/app/static/images';
        if (!is_dir($path)) {
    ...
    function miniapp_code_generate($version_id, $user_version, $is_goods = 0) {
        ...
        if ($app_info['name'] == 'tapgo_ttv2') {
            $app_set        = pdo_get('tapgo_tt_sysset', array(
                'uniacid' => $_W['uniacid']
            ));
	...
	class XcApi {
    private $url = 'http://manage.bdou.cc/XcxApi/Wxa/auth/auth?id=';
    ...
    public function url($api, $method, $params = array(), $dataType = 'json') {
        ...
        $url  = $_url.'/'.$api . '/' . $method . '/' . 'index?access_token=' . $access_token;
        if (!empty($dataType)) {
        ...
        $url .= "&appid=" . $this->appid . "&appname=tapgo_ttv2";
        if (strlen($url) > 2800) {
