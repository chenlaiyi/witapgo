...
class Goods_TapgoTtPage extends PluginWebPage
{
	public function main()
	{
		...
		$sql = 'SELECT g.id FROM ' . tablename('tapgo_tt_goods') . 'g' . $sqlcondition . $condition . ' group by g.id';
		$total_all = pdo_fetchall($sql, $params);
		$total = count($total_all);
		unset($total_all);

		if (!empty($total)) {
			$sql = 'SELECT g.id, g.title, g.createtime, g.marketprice, g.thumb FROM ' . tablename('tapgo_tt_goods') . 'g' . $sqlcondition . $condition . ' group by g.id ORDER BY g.`status` DESC, g.`displayorder` DESC,
