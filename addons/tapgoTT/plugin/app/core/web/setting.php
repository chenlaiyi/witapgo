...
class Setting_TapgoTtPage extends PluginWebPage {
    public function main() {
        ...
        $subscribe_list = pdo_fetchall("select * from ".tablename('tapgo_tt_wxapp_subscribe').' where uniacid = :uniacid ', array(':uniacid' => $_W['uniacid']));
        // 判断分销是否有插件，是否开启
        $commission = false;
        if(p('commission') ){
    ...
    public function setTemplate()
    {
        ...
        $subscribe = pdo_fetchall("select * from ".tablename('tapgo_tt_wxapp_subscribe').' where uniacid = :uniacid and type=:type ', array(':uniacid' => $_W['uniacid'], ':type' => $type));
        if (!empty($subscribe)) {
            return show_json(1);
        }
        ...
        // 没错误插入数据库
        pdo_insert('tapgo_tt_wxapp_subscribe', $ins);
    ...
    public function deleteTemplate()
    {
        ...
        $subscribe = pdo_fetch("select * from ".tablename('tapgo_tt_wxapp_subscribe').' where uniacid = :uniacid and type=:type ', array(':uniacid' => $_W['uniacid'], ':type' => $type));
        if (empty($subscribe)) {
        ...
        // 没错误插入数据库
        pdo_delete('tapgo_tt_wxapp_subscribe', array('id'=>$subscribe['id']));
