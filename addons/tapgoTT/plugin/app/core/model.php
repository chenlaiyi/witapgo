...
if(!class_exists('AppModel')) {
    ...
    class AppModel extends PluginModel {
        ...
        public function wxpay($params, $type = 0){
            ...
            $package['device_info'] = "tapgo_ttv2";
            ...
            $package['notify_url'] = $_W['siteroot'] . "addons/tapgo_ttv2/payment/wechat/notify.php";
        ...
        public function getTMessage($id){
            ...
            $item = pdo_fetch('SELECT * FROM ' . tablename('tapgo_tt_wxapp_tmessage') . ' WHERE id=:id AND uniacid=:uniacid AND status=1 LIMIT 1', array(
                ':id' => $id,
                ':uniacid' => $_W['uniacid']
            ));
        ...
        public function getGlobal(){
            ...
            $path = IA_ROOT . "/addons/tapgo_ttv2/data/global";
                    ...
                    foreach ($page['data']['items'] as $itemid => &$item) {
                        ...
                        if($mobile) {
                            ...
                            if($item['id'] == 'richtext') {
                                ...
                                $notices = pdo_fetchall("select id, title, link, thumb from " . tablename('tapgo_tt_notice') . " where uniacid=:uniacid and status=1 order by displayorder desc limit {$limit}", array(':uniacid' => $_W['uniacid']));
                                $item['data'] = array();
                                ...
                                if(!empty($item['data'])) {
                                    ...
                                    if($item['params']['saleout'] == 0 && !empty($_W['shopset']['shop']['saleout'])) {
                                        ...
                                        if(empty($item['params']['saleout'])) {
                                            $item['params']['saleout'] = tomedia('../addons/tapgo_ttv2/plugin/diypage/static/images/default/saleout-2.png');
                                        }
                                    } elseif($item['params']['saleout'] == 1 && !empty($item['style']['saleoutstyle'])) {
                                        $item['params']['saleout'] = tomedia('../addons/tapgo_ttv2/plugin/diypage/static/images/default/saleout-' . $item['style']['saleoutstyle'] . '.png');
                                    }
                                } else {
                                    unset($page['data']['items'][$itemid]);
                                }
...
//                                        }
                                            ...
                                            foreach ($merchids as $merchid) {
                                                foreach ($merchs as $index => $merch) {
                                                    if ($merch['id'] == $merchid) {
                                                        ...
                                                        $childid = 'C' . $childid;
                                                        if (empty($merch['logo'])){
                                                            $merch['logo'] = tomedia('addons/tapgo_ttv2/plugin/diypage/static/images/default/logo.jpg');
                                                        }
                                                        $item['data'][$childid] = array(
                            ...
                            //                            }
                                ...
                                {
                                    $item['params']['turl'] = parseTencentVideo($item['params']['shortVideourl']);
                                    $item['params']['default'] = $_W['siteroot'] . '/addons/tapgo_ttv2/static/images/';
                                    $item['params']['shortPoster'] = tomedia($item['params']['shortPoster']);
                                }else
                                {
                                    $item['params']['turl'] = tomedia($item['params']['shortOrignUrl']);
                                ...
                                $item['params']['turl'] = tomedia($item['params']['shortVideourl']);
                                $item['params']['default'] = $_W['siteroot'] . '/addons/tapgo_ttv2/static/images/';
                                $item['params']['shortPoster'] = tomedia($item['params']['shortPoster']);
                            }
                        } else {
            ...
            if(strexists($str, "r=commission")) {
                ...
                if($plugin_commissiom) {
                    ...
                    if(strexists($str, "r=commission.down")) {
                        ...
                        $level1 = pdo_fetchcolumn('select count(*) from ' . tablename('tapgo_tt_member') . ' where agentid=:agentid and uniacid=:uniacid limit 1'
                            , array(":agentid" => $member['id'], ':uniacid' => $_W['uniacid']));
                        if($plugin_commissiom_set['level'] >= 2 && count($member['level1_agentids']) > 0) {
                            $level2 = pdo_fetchcolumn('select count(*) from ' . tablename('tapgo_tt_member') . " where agentid in( " . implode(',', array_keys($member['level1_agentids'])) . ") and uniacid=:uniacid limit 1"
                                , array(':uniacid' => $_W['uniacid']));
                        }
                        if($plugin_commissiom_set['level'] >= 3 && count($member['level2_agentids']) > 0) {
                            $level3 = pdo_fetchcolumn('select count(*) from ' . tablename('tapgo_tt_member') . " where agentid in( " . implode(',', array_keys($member['level2_agentids'])) . ") and uniacid=:uniacid limit 1"
                                , array(':uniacid' => $_W['uniacid']));
                        }
...
array_walk($goods, function (&$goodsItem) {
                    ...
                    $group = pdo_fetch("SELECT * FROM " . tablename('tapgo_tt_goods_group') . " WHERE id=:id and uniacid=:uniacid limit 1 ", array(':id' => $item['params']['groupid'], ':uniacid' => $_W['uniacid']));
                    if(!empty($group) && !empty($group['goodsids'])) {
                        ...
                        $item['params']['goodsnum'] = !empty($item['params']['goodsnum']) ? $item['params']['goodsnum'] : 20;
                        $goods = pdo_fetchall("select id, title, subtitle, thumb, minprice, sales, salesreal, total, showlevels, showgroups, bargain, productprice,hascommission,nocommission,commission,commission1_rate,commission1_rate,marketprice,commission1_pay,maxprice,video,`type` from " . tablename('tapgo_tt_goods') . " where id in( {$group['goodsids']} ) and status=1 and `deleted`=0 and `status`=1 and uniacid=:uniacid " . $orderby . " limit {$item['params']['goodsnum']}", array(':uniacid' => $_W['uniacid']));
                    }
                }
...
//                    秒杀
                    ...
                    if(empty($item['params']['goodsdata'])) {
                        ...
                        foreach ($item['data'] as $childid => $childgoods) {
                            $current_good= pdo_get('tapgo_tt_goods',array('id'=>$childgoods['gid'],'uniacid'=>$_W['uniacid']));
                            if ($mobile)
                            {
...
//                    秒杀
        ...
        public function getAuth(){
            ...
            */
            ...
            load()->func('communication');
            $request = ihttp_get(TAPGO_TT_AUTH_WXAPP . 'xcxapi/auth-xcx-info/validate?site_id=' . SITE_ID . '&uniacid=' . $_W['uniacid'].$uri);
		...
		global $_W;
            ...
            load()->func('communication');
            $request = ihttp_get(TAPGO_TT_AUTH_WXAPP . 'xcxapi/auth-xcx-info/view?site_id=' . SITE_ID . '&uniacid=' . $_W['uniacid'].$uri);
        ...
        public function getReleaseList(){
            ...
            load()->func('communication');
            $request = ihttp_get(TAPGO_TT_AUTH_WXAPP . 'xcxapi/auth-xcx-info/list?site_id=' . SITE_ID . '&uniacid=' . $_W['uniacid']);
        ...
        public function getReleaseLog($page = 1, $psize = 10){
            load()->func('communication');
            $request = ihttp_get(TAPGO_TT_AUTH_URL . "wxapp_log?pindex={$page}&psize={$psize}");
            $content = json_decode($request['content'], true);
            return $content;
        }
        ...
        public function apiFile(){
            $path1 = __DIR__ . '/mobile/tapgo_ttv2_api.php';
            $path2 = IA_ROOT . '/app/tapgo_ttv2_api.php';
        ...
        public function createAuth(){
            ...
            $auth = get_auth();
            load()->func('communication');
            $resp = ihttp_request(TAPGO_TT_AUTH_URL . 'app', array('ip' => $ip, 'id' => $id, 'code' => $auth['code'], 'domain' => $domain, 'appid' => $_W['account']['key']), null, 1);
        ...
        public function diyMenu($menuid){
            ...
            $menu = pdo_fetch("select * from " . tablename('tapgo_tt_diypage_menu') . " where id=:id and uniacid=:uniacid limit 1", array(':id' => $id, ':uniacid' => $_W['uniacid']));
            if(empty($menu)) {
                return $this->defaultMenu($menuid);
            }
        ...
        public function getUrl($url){
            ...
            //  如果 站内链接 则转换为
            if(strexists($url, './index.php?') && strexists($url, 'tapgo_ttv2') && strexists($url, 'mobile')) {
                $parse = parse_url($url);
                ...
                //  不在此模块 直接返回URL
                if($newVars['m'] != 'tapgo_ttv2' || $newVars['do'] != 'mobile') {
        ...
        public function getIconUnicode($class = null){
            $file = TAPGO_TT_PLUGIN . 'app/static/iconfont.json';
            if(!file_exists($file)) {
                return '';
            }
        ...
        public function alipay_build($params, $config = array()){
            ...
            $arr = array(
                ...
                'version' => '1.0',
                'notify_url' => $_W['siteroot'] . 'addons/tapgo_ttv2/payment/alipay/notify.php',
        ...
        public function sendSubscribeMessage($openid, $msgdata, $type)
        {
            ...
            $templateid = pdo_fetchcolumn("select templateid from ".tablename('tapgo_tt_wxapp_subscribe')." where uniacid={$_W['uniacid']} and `type` = '{$type}'");
            if (!empty($templateid)) {
