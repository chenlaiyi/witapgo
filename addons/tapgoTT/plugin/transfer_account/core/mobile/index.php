...
//        }
    ...
    public function sdl()
    {
        ...
        if ($search == $this->model->getW('tapgo_ttv2_member')['id']) {
    ...
    public function getList()
    {
        ...
        if ($listType == 'in') {
            ...
            $leftJoin = ' left join '.tablename('tapgo_tt_member'). ' m on m.openid=log.from_user';
          
        } else {
            $condition .= ' and from_user=:from_user ';
            $params[':from_user'] = $_W['openid'];
            $leftJoin = ' left join '.tablename('tapgo_tt_member'). ' m on m.openid=log.to_user';
        }
        ...
        $list = pdo_fetchall("select log.*,m.nickname, m.id uid from " . tablename('tapgo_tt_transfer_log') . " log {$leftJoin}  where 1 {$condition} order by log.id desc LIMIT " . ($pindex - 1) * $psize . ',' . $psize , $params);

        $total = pdo_fetchcolumn('select count(*) from ' . tablename('tapgo_tt_transfer_log') . "log  $leftJoin where 1 {$condition}", $params);
    ...
    public function getuser()
    {
        ...
        if ($uid == $this->model->getW('tapgo_ttv2_member')['id']) {
            show_json(-1, '不能转给自己');
        }
