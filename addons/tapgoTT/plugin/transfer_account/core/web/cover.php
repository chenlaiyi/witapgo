...
class Cover_TapgoTtPage extends PluginWebPage {
    ...
    function main() {
        ...
        $rule = pdo_fetch("select * from " . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'cover', ':name' => "tapgo_ttv2用户转账入口设置"));
        if (!empty($rule)) {
        ...
        if ($_W['ispost']) {
            ...
            if(!empty($keyword)){
                if($keyword['name']!='tapgo_ttv2用户转账入口设置'){
                    show_json(0, '关键字已存在!');
                }
            }
            if (!empty($rule)) {
