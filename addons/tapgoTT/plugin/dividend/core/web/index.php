...
class Index_TapgoTtPage extends DividendWebPage
{
	...
	public function init()
	{
		...
		$count = pdo_fetchcolumn('SELECT count(*) FROM ' . tablename('tapgo_tt_member') . ' WHERE uniacid=:uniacid AND agentid>0', array(':uniacid' => $_W['uniacid']));
		...
		if ($_W['isajax']) {
			...
			$list = pdo_fetchall('SELECT id,agentid FROM ' . tablename('tapgo_tt_member') . ' WHERE uniacid=:uniacid AND agentid>0  ORDER BY id asc limit ' . ($pindex - 1) * $psize . ',' . $psize, array(':uniacid' => $_W['uniacid']));
	...
	public function refresh()
	{
		...
		$data['init'] = 0;
		m('common')->updatePluginset(array('dividend' => $data));
		pdo_delete('tapgo_tt_commission_relation');
