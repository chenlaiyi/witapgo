...
class Apply_TapgoTtPage extends DividendWebPage
{
...
else
{
    if( !cv("dividend.apply.view" . $status) ) 
    {
        ...
        $sql = "select a.*, m.nickname,m.avatar,m.realname,m.mobile,m.agentlevel,a.realname as applyrealname,a.sendmoney from " . tablename("tapgo_tt_dividend_apply") . " a " . " left join " . tablename("tapgo_tt_member") . " m on m.id = a.mid" . " where 1 " . $condition . " ORDER BY " . $orderby . " desc ";
        ...
        if( $status == 3 ) 
        {
            $realmoney_total = (double) pdo_fetchcolumn("select sum(a.realmoney) from " . tablename("tapgo_tt_dividend_apply") . " a " . " left join " . tablename("tapgo_tt_member") . " m on m.id = a.mid" . " where 1 " . $condition, $params);
        }
        ...
        if( $_GPC["export"] == "1" ) 
        {
            ...
            foreach( $list as &$row ) 
            {
                ...
                $orders = pdo_fetchall("select id,agentid, ordersn,price,goodsprice,dispatchprice,createtime,paytype,dividend,dividend_status from " . tablename("tapgo_tt_order") . " where  id in ( " . $row["orderids"] . " );");
                $totaldividend = 0;
                $totalpay = 0;
                $passmoney = 0;
                $ordergoodscount = 0;
                ...
                {
                    $goods = pdo_fetchall("SELECT og.id,g.thumb,og.price,og.realprice, og.total,g.title,o.paytype,og.optionname from " . tablename("tapgo_tt_order_goods") . " og" . " left join " . tablename("tapgo_tt_goods") . " g on g.id=og.goodsid  " . " left join " . tablename("tapgo_tt_order") . " o on o.id=og.orderid  " . " where og.uniacid = :uniacid and og.orderid=:orderid  order by og.createtime  desc ", array( ":uniacid" => $_W["uniacid"], ":orderid" => $order["id"] ));
        ...
        $total = pdo_fetchcolumn("select count(a.id) from" . tablename("tapgo_tt_dividend_apply") . " a " . " left join " . tablename("tapgo_tt_member") . " m on m.uid = a.mid" . " where 1 " . $condition, $params);
    ...
    protected function applyData()
    {
        ...
        global $_GPC;
        $id = intval($_GPC["id"]);
        $apply = pdo_fetch("select * from " . tablename("tapgo_tt_dividend_apply") . " where uniacid=:uniacid and id=:id limit 1", array( ":uniacid" => $_W["uniacid"], ":id" => $id ));
        if( empty($apply) ) 
        {
            if( $_W["isajax"] ) 
            {
...
else
{
    ca("dividend.apply.view" . $apply["status"]);
        ...
        $list = pdo_fetchall("select id,headsid, ordersn,price,goodsprice, dispatchprice,discountprice,deductprice,deductcredit2,deductenough,couponprice,createtime, paytype,dividend,dividend_status,dividend_content from " . tablename("tapgo_tt_order") . " where  id in ( " . $apply["orderids"] . " );");
        $totaldividend = 0;
        $totalmoney = 0;
        $totalpay = 0;
        foreach( $list as &$row ) 
        ...
        unset($row);
        $totalcount = $total = pdo_fetchcolumn("select count(*) from " . tablename("tapgo_tt_order") . " o " . " left join " . tablename("tapgo_tt_member") . " m on o.openid = m.openid " . " left join " . tablename("tapgo_tt_member_address") . " a on a.id = o.addressid " . " where o.id in ( " . $apply["orderids"] . " );");
        $set_array = array(  );
    ...
    public function check()
    {
        ...
        foreach( $list as $order ) 
        {
            ...
            if( !empty($update) ) 
            {
                pdo_update("tapgo_tt_order", $update, array( "id" => $order["id"] ));
            }

        }
        if( $isAllUncheck ) 
        {
            pdo_update("tapgo_tt_dividend_apply", array( "status" => -1, "invalidtime" => $time ), array( "id" => $id, "uniacid" => $_W["uniacid"] ));
        }
        else
        {
            pdo_update("tapgo_tt_dividend_apply", array( "status" => 2, "checktime" => $time ), array( "id" => $id, "uniacid" => $_W["uniacid"] ));
            $rmoney = $paydividend;
            $dmoney = 0;
            if( !empty($set_array["charge"]) ) 
            {
    ...
    public function cancel()
    {
        ...
        foreach( $list as $row ) 
        {
            pdo_update("tapgo_tt_order", array( "dividend_checktime" => 0, "dividend_status" => 1 ), array( "id" => $row["id"] ));
        }
        pdo_update("tapgo_tt_dividend_apply", array( "status" => 1, "checktime" => 0, "invalidtime" => 0 ), array( "id" => $id, "uniacid" => $_W["uniacid"] ));
    ...
    public function refuse()
    {
        ...
        foreach( $list as $row ) 
        {
            pdo_update("tapgo_tt_order", array( "dividend_checktime" => 0, "dividend_status" => 0 ), array( "id" => $row["id"] ));
        }
        pdo_update("tapgo_tt_dividend_apply", array( "status" => -2, "checktime" => 0, "invalidtime" => 0, "invalidtime" => time() ), array( "id" => $id, "uniacid" => $_W["uniacid"] ));
    ...
    public function pay($params = array(  ), $mine = array(  ))
    {
        ...
        if( $apply["type"] < 2 ) 
        {
            ...
            if( !empty($data["paytype"]["commission"]) && $apply["type"] == 1 ) 
            {
                if( strpos($member["openid"], "sns_wa_") === false ) 
                {
                    ...
                    pdo_update("tapgo_tt_dividend_apply", array( "sendmoney" => $result["sendmoney"], "senddata" => json_encode($result["senddata"]) ), array( "id" => $apply["id"] ));
        ...
        foreach( $list as $row ) 
        {
            ...
            {
                pdo_update("tapgo_tt_order", array( "dividend_paytime" => $time, "dividend_status" => 3 ), array( "id" => $row["id"] ));
            }

        }
        pdo_update("tapgo_tt_dividend_apply", array( "status" => 3, "paytime" => $time, "dividend_pay" => $totalpay, "realmoney" => $realmoney, "deductionmoney" => $deductionmoney ), array( "id" => $id, "uniacid" => $_W["uniacid"] ));
        ...
        pdo_insert("tapgo_tt_dividend_log", $log);
        $mdividend = $totalpay;
