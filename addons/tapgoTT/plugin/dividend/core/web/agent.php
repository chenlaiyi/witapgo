...
class Agent_TapgoTtPage extends DividendWebPage
{
    public function main()
    {
        ...
        $sql = "select dm.*,dm.nickname,dm.avatar,p.id as pid,p.nickname as parentname,p.avatar as parentavatar  from " . tablename("tapgo_tt_member") . " dm " . " left join " . tablename("tapgo_tt_member") . " p on p.id = dm.headsid " . " where dm.uniacid = " . $_W["uniacid"] . " and dm.isagent =1 and dm.isheads = 1 " . $condition . " ORDER BY dm.agenttime desc";
        ...
        $total = pdo_fetchcolumn("select count(dm.id) from" . tablename("tapgo_tt_member") . " dm  " . " left join " . tablename("tapgo_tt_member") . " p on p.id = dm.headsid " . " where dm.uniacid =" . $_W["uniacid"] . " and dm.isagent =1 and dm.isheads = 1 " . $condition, $params);
        foreach( $list as &$row ) 
        {
    ...
    public function delete()
    {
        ...
        {
            pdo_update("tapgo_tt_member", array( "isheads" => 0, "headsstatus" => 0 ), array( "id" => $member["id"] ));
            plog("dividend.agent.delete", "取消队长资格 <br/>队长信息:  ID: " . $member["id"] . " /  " . $member["openid"] . "/" . $member["nickname"] . "/" . $member["realname"] . "/" . $member["mobile"]);
            $initData = pdo_fetch("select * from " . tablename("tapgo_tt_dividend_init") . " where headsid = :headsid and uniacid = :uniacid", array( ":headsid" => $member["id"], ":uniacid" => $_W["uniacid"] ));
            if( !empty($initData) ) 
            {
                pdo_update("tapgo_tt_dividend_init", array( "status" => 0 ), array( "headsid" => $member["id"] ));
                $dividend = pdo_fetch("select id,isheads,headsid,headsstatus from " . tablename("tapgo_tt_member") . " where id = :id", array( ":id" => $member["agentid"] ));
                $dividend_init = pdo_fetch("select * from " . tablename("tapgo_tt_dividend_init") . " where headsid = :headsid", array( ":headsid" => $member["agentid"] ));
                if( !empty($dividend["isheads"]) && !empty($dividend["headsstatus"]) && !empty($dividend_init["status"]) ) 
                {
                    pdo_update("tapgo_tt_member", array( "headsid" => $member["agentid"] ), array( "id" => $member["id"], "uniacid" => $_W["uniacid"] ));
                    ...
                    if( !empty($data) ) 
                    {
                        ...
                        pdo_update("tapgo_tt_member", array( "headsid" => $member["agentid"] ), array( "id" => $ids ));
                    }
    ...
    public function user()
    {
        ...
        if( $hasheads ) 
        {
            $total = pdo_fetchcolumn("select count(dm.id) from" . tablename("tapgo_tt_member") . " dm " . " left join " . tablename("mc_mapping_fans") . "f on f.openid=dm.openid" . " where dm.uniacid =" . $_W["uniacid"] . " and dm.headsid = " . $headsid . " " . $condition, $params);
            $sql = "select dm.*  from " . tablename("tapgo_tt_member") . " dm " . " left join " . tablename("mc_mapping_fans") . "f on f.openid=dm.openid  and f.uniacid=" . $_W["uniacid"] . " where dm.uniacid = " . $_W["uniacid"] . " and dm.headsid = " . $headsid . "  " . $condition . "   ORDER BY dm.createtime desc";
            if( empty($_GPC["export"]) ) 
            {
                $sql .= " limit " . ($pindex - 1) * $psize . "," . $psize;
            }
    ...
    public function check()
    {
        ...
        $members = pdo_fetchall("SELECT id,openid,agentid,nickname,realname,mobile,status,headsstatus FROM " . tablename("tapgo_tt_member") . " WHERE id in( " . $id . " ) AND uniacid=" . $_W["uniacid"]);
        $time = time();
        foreach( $members as $member ) 
        {
            ...
            if( $status == 1 ) 
            {
                ...
                pdo_update("tapgo_tt_member", array( "headsstatus" => 1, "headstime" => $time, "headsid" => 0 ), array( "id" => $member["id"], "uniacid" => $_W["uniacid"] ));
                ...
                if( empty($initData) ) 
                {
                    pdo_insert("tapgo_tt_dividend_init", $heads_data);
                }
            ...
            {
                pdo_update("tapgo_tt_member", array( "headsstatus" => 0, "headstime" => 0 ), array( "id" => $member["id"], "uniacid" => $_W["uniacid"] ));
    ...
    public function init()
    {
        ...
        foreach( $members as $member ) 
        {
            ...
            $data = pdo_fetchall("select  r.id,r.pid,m.isheads,m.headsstatus from " . tablename("tapgo_tt_commission_relation") . " r " . " left join " . tablename("tapgo_tt_member") . " m on m.id = r.id where  r.pid=:pid and m.uniacid=:uniacid", array( ":pid" => $member["id"], ":uniacid" => $_W["uniacid"] ));
            if( !empty($data) ) 
            {
                $heads = array(  );
                $later = array(  );
                ...
                if( !empty($heads) ) 
                {
                    $later = pdo_fetchall("select id from " . tablename("tapgo_tt_commission_relation") . " where pid in (" . implode(",", $heads) . ")");
                }
                ...
                if( !empty($ids) ) 
                {
                    ...
                    pdo_update("tapgo_tt_member", array( "headsid" => $member["id"] ), array( "id" => $ids ));
                }

            }

            pdo_update("tapgo_tt_dividend_init", array( "status" => 1 ), array( "headsid" => $member["id"] ));
        }
        show_json(1);
    }
