...
class Apply_TapgoTtPage extends DividendMobileLoginPage 
{
	public function main() 
	{
		...
		$dividend_ok = 0;
		$orders = pdo_fetchall('select id,dividend from ' . tablename('tapgo_tt_order') . ' where headsid=:headsid and status>=3  and dividend_status=0 and (' . $time . ' - finishtime > ' . $day_times . ') and uniacid=:uniacid  group by id', array(':uniacid' => $_W['uniacid'], ':headsid' => $member['id']));
		foreach ($orders as $o ) 
		{
		...
		if ($this->set['cashother'] == 1) 
		{
			...
			if ($this->set['cashcard'] == 1) 
			{
				...
				$banklist = pdo_fetchall('SELECT * FROM ' . tablename('tapgo_tt_dividend_bank') . ' WHERE 1 ' . $condition . '  ORDER BY displayorder DESC', $params);
			}
		}
		if (!(empty($last_data))) 
		{
		...
		if ($_W['ispost']) 
		{
			...
			{
				$orderids[] = $o['id'];
				pdo_update('tapgo_tt_order', array('dividend_status' => 1, 'dividend_applytime' => $time), array('id' => $o['id'], 'uniacid' => $_W['uniacid']));
			}
			...
			pdo_insert('tapgo_tt_dividend_apply', $apply);
