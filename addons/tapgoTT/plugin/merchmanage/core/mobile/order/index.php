...
require TAPGO_TT_PLUGIN . 'merchmanage/core/inc/page_merchmanage.php';
class Index_TapgoTtPage extends MerchmanageMobilePage
{
	...
	public function detail()
	{
		...
		if (0 < $item['sendtype']) {
			...
			foreach ($order_goods as $key => $value ) {
				$order_goods[$key]['goods'] = pdo_fetchall('select g.id,g.title,g.thumb,og.sendtype,g.ispresell from ' . tablename('tapgo_tt_order_goods') . ' og ' . ' left join ' . tablename('tapgo_tt_goods') . ' g on g.id=og.goodsid ' . ' where og.uniacid=:uniacid and og.orderid=:orderid and og.sendtype=' . $value['sendtype'] . ' ', array(':uniacid' => $_W['uniacid'], ':orderid' => $id));
			}
		...
		if ($_W['ispost']) {
			pdo_update('tapgo_tt_order', array('remark' => trim($_GPC['remark'])), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
		...
		$refund = pdo_fetch('SELECT * FROM ' . tablename('tapgo_tt_order_refund') . ' WHERE orderid = :orderid and uniacid=:uniacid order by id desc', array(':orderid' => $item['id'], ':uniacid' => $_W['uniacid']));
		$diyformfields = '';
	...
	public function getlist()
	{
		...
		unset($value);
		if (($condition != ' o.uniacid = :uniacid and and o.deleted=0 and o.isparent=0') || !(empty($sqlcondition))) {
			$t = pdo_fetch('SELECT COUNT(*) as count, ifnull(sum(o.price),0) as sumprice   FROM ' . tablename('tapgo_tt_order') . ' o ' . ' left join ' . tablename('tapgo_tt_order_refund') . ' r on r.id =o.refundid ' . ' left join ' . tablename('tapgo_tt_member') . ' m on m.openid=o.openid  and m.uniacid =  o.uniacid' . ' left join ' . tablename('tapgo_tt_member_address') . ' a on o.addressid = a.id ' . ' left join ' . tablename('tapgo_tt_saler') . ' s on s.openid = o.verifyopenid and s.uniacid=o.uniacid' . ' left join ' . tablename('tapgo_tt_member') . ' sm on sm.openid = s.openid and sm.uniacid=s.uniacid' . ' ' . $sqlcondition . ' WHERE ' . $condition . ' ' . $statuscondition, $paras);
		}
		 else {
			$t = pdo_fetch('SELECT COUNT(*) as count, ifnull(sum(price),0) as sumprice   FROM ' . tablename('tapgo_tt_order') . ' WHERE uniacid = :uniacid and deleted=0 and isparent=0 ' . $status_condition, $paras);
		}
