...
class Index_TapgoTtPage  extends  MerchmanageMobilePage // PluginMobilePage
          //extends MerchmanageMobilePage
{
	public function main()
	{
		...
		$merchshop = pdo_fetch('select * from '.tablename('tapgo_tt_merch_user').' where id ="'.$merchid.'"');
	...
	protected function selectOrderPrice($day = 0)
	{
		...
		$sql = 'select id,price,createtime from ' . tablename('tapgo_tt_order') . ' where uniacid = :uniacid and ismr=0 and isparent=0 and (status > 0 or ( status=0 and paytype=3)) and merchid =:merchid and deleted=0 and createtime between :createtime1 and :createtime2';
