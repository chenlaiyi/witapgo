...
class Index_TapgoTtPage extends MerchmanageMobilePage 
{
	...
	public function get_list()
	{
		...
		$sql = 'select b.* from ' . tablename('tapgo_tt_merch_bill') . ' b ' . ' left join ' . tablename('tapgo_tt_merch_user') . ' u on b.merchid = u.id' . ' where 1 ' . $condition . ' ORDER BY applytime desc ';
		
		$sql .= '  limit ' . (($pindex - 1) * $psize) . ',' . $psize;
		
		$list = pdo_fetchall($sql, $params);

		$total = pdo_fetchcolumn('select count(b.id) from' . tablename('tapgo_tt_merch_bill') . ' b ' . ' left join ' . tablename('tapgo_tt_merch_user') . ' u on b.merchid = u.id' . ' where 1 ' . $condition, $params);
