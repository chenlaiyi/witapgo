...
if(!pdo_fieldexists('tapgo_tt_merch_user', 'can_import')) {
    pdo_query("ALTER TABLE ".tablename('tapgo_tt_merch_user')." ADD `can_import` tinyint(3) NOT NULL DEFAULT 0;"); 
}
if(!pdo_fieldexists('tapgo_tt_merch_user', 'can_edit')) {
    pdo_query("ALTER TABLE ".tablename('tapgo_tt_merch_user')." ADD `can_edit` tinyint(3) NOT NULL DEFAULT 0;");
}
if(!pdo_fieldexists('tapgo_tt_goods', 'import_id')) {
    pdo_query("ALTER TABLE ".tablename('tapgo_tt_goods')." ADD `import_id` int(11) NOT NULL DEFAULT 0;");
}
if (!pdo_tableexists('tapgo_tt_domain_bindings')) {
    pdo_query("CREATE TABLE " . tablename('tapgo_tt_domain_bindings') . " (
...
{
    copy(TAPGO_TT_PLUGIN . 'merch/core/inc/merchant.php',IA_ROOT . '/web/merchant.php');
...
class MerchModel extends PluginModel
{
    ...
    {
        global $_W;
        return pdo_fetchall('select id,groupname from ' . tablename('tapgo_tt_merch_group') . ' where uniacid=:uniacid and status=1 order by isdefault desc , id asc', array(':uniacid' => $_W['uniacid']), 'id');
    }
    ...
    public function getCategory($data = array())
    {
        ...
        return pdo_fetchall('select ' . $res['field'] . ' from ' . tablename('tapgo_tt_merch_category') . $res['condition'], $res['params'], $res['column']);
    }
    ...
    public function getCategorySwipe($data = array())
    {
        ...
        $res = $this->build($condition, $params, $data);
        return pdo_fetchall('select ' . $res['field'] . ' from ' . tablename('tapgo_tt_merch_category_swipe') . $res['condition'], $res['params'], $res['column']);
    }
    ...
    public function getMerch($data = array())
    {
        ...
        $res = $this->build($condition, $params, $data);
        return pdo_fetchall('select ' . $res['field'] . ' from ' . tablename('tapgo_tt_merch_user') . $res['condition'], $res['params'], $res['column']);
    }
    ...
    public function updateSet($values = array(), $merchid = 0)
    {
        ...
        pdo_update('tapgo_tt_merch_user', array('sets' => iserializer($sets)), array('id' => $merchid));
    ...
    public function refreshSet($merchid = 0)
    {
        ...
        $merch_set = pdo_fetch('select sets from ' . tablename('tapgo_tt_merch_user') . ' where uniacid=:uniacid and id=:id limit 1 ', array(':uniacid' => $_W['uniacid'], ':id' => $merchid));
    ...
    public function getGoodsTotals()
    {
        global $_W;
        return array(
            "sale" => pdo_fetchcolumn('select count(1) from ' . tablename('tapgo_tt_goods') . " where checked=0 and status=1 and deleted=0 and total>0 and uniacid=:uniacid and merchid=:merchid", array(':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid'])),
            "out" => pdo_fetchcolumn('select count(1) from ' . tablename('tapgo_tt_goods') . " where checked=0 and  status=1 and deleted=0 and total=0 and uniacid=:uniacid and merchid=:merchid", array(':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid'])),
            "check" => pdo_fetchcolumn('select count(1) from ' . tablename('tapgo_tt_goods') . " where checked=1 and deleted=0 and uniacid=:uniacid and merchid=:merchid", array(':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid'])),
            "stock" => pdo_fetchcolumn('select count(1) from ' . tablename('tapgo_tt_goods') . " where checked=0 and status=0 and deleted=0 and uniacid=:uniacid  and merchid=:merchid", array(':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid'])),
            "cycle" => pdo_fetchcolumn('select count(1) from ' . tablename('tapgo_tt_goods') . " where deleted=1 and uniacid=:uniacid and merchid=:merchid", array(':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid'])),
        );
    }
    ...
    function getListUser($list, $return = 'all')
    {
        ...
        if (!is_array($list)) {
            ...
            if (!empty($merch)) {
                ...
                $merch_user = pdo_fetchall("select * from " . tablename('tapgo_tt_merch_user') . " where uniacid=:uniacid and id in(" . implode(',', $merch_ids) . ")", array(':uniacid' => $_W['uniacid']), 'id');
    ...
    function getListUserOne($merchid)
    {
        ...
        if ($merchid) {
            $merch_user = pdo_fetch("select * from " . tablename('tapgo_tt_merch_user') . " where uniacid=:uniacid and id={$merchid}", array(':uniacid' => $_W['uniacid']));
            return $merch_user;
        }
        return false;
    }
    ...
    private function check($permtype = '')
    {
        ...
        $user = pdo_fetch('select u.status as userstatus,r.status as rolestatus,r.perms as roleperms,u.roleid from ' . tablename('tapgo_tt_merch_account') . ' u '
            . ' left join ' . tablename('tapgo_tt_merch_perm_role') . ' r on u.roleid = r.id '
    ...
    public function log($type = '', $op = '')
    {
        ...
        pdo_insert('tapgo_tt_merch_perm_log', $log);
    }
    ...
    public function select_operator($merchid = 0)
    {
        global $_W;
        $merchid = empty($merchid) ? $_W['merchid'] : intval($merchid);
        $total = pdo_fetchcolumn("SELECT count(*) FROM " . tablename('tapgo_tt_merch_account') . " WHERE 1  and uniacid = :uniacid and merchid = :merchid and isfounder<>1", array(':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid']));
        return $total;
    }
    ...
    public function getUserTotals()
    {
        ...
        $totals = array(
            'reg0' => pdo_fetchcolumn('select count(*) from ' . tablename('tapgo_tt_merch_reg') . ' where uniacid=:uniacid and status=0 limit 1', array(':uniacid' => $_W['uniacid'])),
            'reg_1' => pdo_fetchcolumn('select count(*) from ' . tablename('tapgo_tt_merch_reg') . ' where uniacid=:uniacid and status=-1 limit 1', array(':uniacid' => $_W['uniacid'])),
            'user0' => pdo_fetchcolumn('select count(*) from ' . tablename('tapgo_tt_merch_user') . ' where uniacid=:uniacid and status=0 limit 1', array(':uniacid' => $_W['uniacid'])),
            'user1' => pdo_fetchcolumn('select count(*) from ' . tablename('tapgo_tt_merch_user') . ' where uniacid=:uniacid and status=1 limit 1', array(':uniacid' => $_W['uniacid'])),
            'user2' => pdo_fetchcolumn('select count(*) from ' . tablename('tapgo_tt_merch_user') . ' where uniacid=:uniacid and status=2 limit 1', array(':uniacid' => $_W['uniacid'])),
            'user3' => pdo_fetchcolumn('select count(*) from ' . tablename('tapgo_tt_merch_user') . ' where uniacid=:uniacid and status=1 and TIMESTAMPDIFF(DAY,now(),FROM_UNIXTIME(accounttime))<=30  limit 1', array(':uniacid' => $_W['uniacid'])),
        );
        return $totals;
    }
    ...
    public function getClearTotals()
    {
        ...
        $totals = array(
            'status0' => pdo_fetchcolumn('select count(*) from ' . tablename('tapgo_tt_merch_clearing') . ' where uniacid=:uniacid and status=0 limit 1', array(':uniacid' => $_W['uniacid'])),
            'status1' => pdo_fetchcolumn('select count(*) from ' . tablename('tapgo_tt_merch_clearing') . ' where uniacid=:uniacid and status=1 limit 1', array(':uniacid' => $_W['uniacid'])),
            'status2' => pdo_fetchcolumn('select count(*) from ' . tablename('tapgo_tt_merch_clearing') . ' where uniacid=:uniacid and status=2 limit 1', array(':uniacid' => $_W['uniacid'])),
        );
        return $totals;
    }
    ...
    public function getMerchOrderTotals($type = 0)
    {
        ...
        $sql = "select sum(o.price) as totalmoney from " . tablename('tapgo_tt_order') . ' o '
            . " left join " . tablename('tapgo_tt_merch_user') . " u on u.id = o.merchid "
            . " where 1 {$condition} ";

        $price = pdo_fetch($sql, $params);
        $totalmoney = round($price['totalmoney'], 2);

        $sql = "select count(o.id) as totalcount from " . tablename('tapgo_tt_order') . ' o '
            . " left join " . tablename('tapgo_tt_merch_user') . " u on u.id = o.merchid "
            . " where 1 {$condition} ";
    ...
    public function getMerchOrderTotalPrice($merchid)
    {
        ...
        //待审核金额 status1
        $sql = "select *  from " . tablename('tapgo_tt_merch_bill')
        ...
        if (count($status1orderids) > 0) {
            $status1order = pdo_fetchall("SELECT id FROM " . tablename('tapgo_tt_order') . " WHERE id in(" . join(',', $status1orderids) . ") AND uniacid=" . $_W['uniacid']);
            $commission1 = 0;
        ...
        if (count($status2orderids) > 0) {
            $status2order = pdo_fetchall("SELECT id FROM " . tablename('tapgo_tt_order') . " WHERE id in(" . join(',', $status2orderids) . ") AND uniacid=" . $_W['uniacid']);
            $commission2 = 0;
        ...
        if (count($status3orderids) > 0) {
            $status3order = pdo_fetchall("SELECT id FROM " . tablename('tapgo_tt_order') . " WHERE id in(" . join(',', $status3orderids) . ") AND uniacid=" . $_W['uniacid']);
            $commission3 = 0;
    ...
    public function getMerchPrice($merchid, $flag = 0)
    {
        ...
        $orderids = array();
        $sql = "select {$con} from " . tablename('tapgo_tt_merch_user') . " u "
            . " left join " . tablename('tapgo_tt_order') . " o on u.id=o.merchid"
            . " where 1 {$condition} limit 1";
        $list = pdo_fetch($sql, $params);

        $merchcouponprice = pdo_fetchcolumn("select sum(o.couponprice) from " . tablename('tapgo_tt_merch_user') . " u "
            . " left join " . tablename('tapgo_tt_order') . " o on u.id=o.merchid"
            . " where o.couponmerchid>0 {$condition} limit 1", $params);

        $refundlist = pdo_fetchall("select o.id,r.orderprice,r.applyprice from " . tablename('tapgo_tt_merch_user') . " u "
            . " left join " . tablename('tapgo_tt_order') . " o on u.id=o.merchid"
            . " left join " . tablename('tapgo_tt_order_refund') . " r on o.refundid=r.id"
...
FROM 
	" . tablename('tapgo_tt_order') . " AS o 
	LEFT JOIN " . tablename('tapgo_tt_order_single_refund') . " AS refund ON o.id = refund.orderid 
	INNER JOIN " . tablename('tapgo_tt_order_goods') . " AS og ON refund.ordergoodsid = og.id 
	AND o.id = og.orderid where o.uniacid = :uniacid and o.merchid = :merchid  and refund.operatetime<:operatetime and o.status = 3 and o.finishtime < :finishtime and o.merchapply<=0 ";
        ...
        if ($flag > 0) {
            $sql = "select o.id,o.agentid,comprice.commission_price,comprice.order_id from " . tablename('tapgo_tt_merch_user') . " u "
                . " left join " . tablename('tapgo_tt_order') . " o on u.id=o.merchid"
                . " left join " . tablename('tapgo_tt_merch_commission_orderprice') . " comprice on o.id=comprice.order_id"
                . " where 1 {$condition}";
            ...
            if (!empty($order)) {
                foreach ($order as $k => $v) {
                    ...
                    if (!empty($v['order_id'])) {
                        ...
                        pdo_insert('tapgo_tt_merch_commission_orderprice', array('order_id' => $v['id'], 'commission_price' => $commission_price));
...
FROM
	".tablename('tapgo_tt_order_single_refund')." AS refund
	INNER JOIN
	".tablename('tapgo_tt_order_goods')." AS og
	ON 
...
//exit();
    ...
    public function getMerchCreditList($merchid, $orderid = 0, $flag = 0, $creditrate = 0, $isbillcredit = 1)
    {
        ...
        $sql = "select {$con} from " . tablename('tapgo_tt_merch_user') . " u "
            . " left join " . tablename('tapgo_tt_creditshop_log') . " o on u.id=o.merchid"
            . " where 1 {$condition}";
        $order = pdo_fetchall($sql, $params);
        $_order = array();
        if (!empty($order)) {
...
//  积分商城计算总金额
    ...
    public function getMerchCredit($merchid, $flag = 0)
    {
        ...
        $_sql = "select iscreditmoney,creditrate,iscredit from" . tablename('tapgo_tt_merch_user') . "where 1 {$_condition}";
        $iscredit = pdo_fetch($_sql, $params);

        if ($flag > 0) {
            $sql = "select o.id,u.iscreditmoney,o.dispatch,o.money,o.credit from " . tablename('tapgo_tt_merch_user') . " u "
                . " left join " . tablename('tapgo_tt_creditshop_log') . " o on u.id=o.merchid"
                . " where 1 {$condition}";
            $order = pdo_fetchall($sql, $params);
...
//订单金额
    ...
    public function getOneApply($id)
    {
        ...
        $sql = "select b.*,u.merchname,u.realname,u.mobile,u.iscreditmoney from " . tablename('tapgo_tt_merch_bill') . " b "
            . " left join " . tablename('tapgo_tt_merch_user') . " u on b.merchid = u.id"
            . " where 1 {$condition} Limit 1";
    ...
    public function getMerchApplyTotals($merchid = 0)
    {
        ...
        $totals['status1'] = pdo_fetchcolumn(
            'SELECT COUNT(1) FROM ' . tablename('tapgo_tt_merch_bill')
    ...
    public function getAllUserTotals()
    {
        ...
        $totals = pdo_fetchcolumn('select count(1) from ' . tablename('tapgo_tt_merch_user') . ' where uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
        return $totals;
    }
    ...
    public function getMaxMerchUser()
    {
        ...
        $data = pdo_fetch('select datas from ' . tablename('tapgo_tt_perm_plugin') . ' where acid=:acid Limit 1', array(':acid' => $_W['uniacid']));
        $max_merch = 0;
        if (!empty($data['datas'])) {
    ...
    public function tempData($type, $merchid = 0)
    {
        ...
        $sql = 'SELECT id,expressname,expresscom,isdefault FROM ' . tablename('tapgo_tt_exhelper_express') . " where  1 and {$condition} ORDER BY isdefault desc, id DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize;
        $list = pdo_fetchall($sql, $params);
        $total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('tapgo_tt_exhelper_express') . " where 1 and {$condition}", $params);
    ...
    public function setDefault($id, $type, $merchid = 0)
    {
        ...
        $item = pdo_fetch("SELECT id,expressname,type FROM " . tablename('tapgo_tt_exhelper_express') . " WHERE id=:id and type=:type AND uniacid=:uniacid and merchid=:merchid", array(":id" => $id, ':type' => $type, ":uniacid" => $_W['uniacid'], ':merchid' => $merchid));
        if (!empty($item)) {
            pdo_update('tapgo_tt_exhelper_express', array('isdefault' => 0), array('type' => $type, 'uniacid' => $_W['uniacid'], 'merchid' => $merchid));
            pdo_update('tapgo_tt_exhelper_express', array('isdefault' => 1), array('id' => $id, 'merchid' => $merchid));
            if ($type == 1) {
    ...
    public function tempDelete($id, $type, $merchid = 0)
    {
        ...
        $items = pdo_fetchall("SELECT id,expressname FROM " . tablename('tapgo_tt_exhelper_express') . " WHERE id in( $id ) and type=:type and uniacid=:uniacid and merchid=:merchid", array(':type' => $type, ':uniacid' => $_W['uniacid'], ':merchid' => $merchid));
        foreach ($items as $item) {
            pdo_delete('tapgo_tt_exhelper_express', array('id' => $item['id'], 'uniacid' => $_W['uniacid'], 'merchid' => $merchid));
    ...
    public function getTemp($merchid = 0)
    {
        ...
        // 查询发件人模板
        $temp_sender = pdo_fetchall("SELECT id,isdefault,sendername,sendertel FROM " . tablename('tapgo_tt_exhelper_senduser') . " WHERE uniacid=:uniacid and merchid=:merchid order by isdefault desc ", array(':uniacid' => $_W['uniacid'], ':merchid' => $merchid));
        // 查询 快递单模板
        $temp_express = pdo_fetchall("SELECT id,type,isdefault,expressname FROM " . tablename('tapgo_tt_exhelper_express') . " WHERE type=1 and uniacid=:uniacid and merchid=:merchid order by isdefault desc ", array(':uniacid' => $_W['uniacid'], ':merchid' => $merchid));
        // 查询发货单模板
        $temp_invoice = pdo_fetchall("SELECT id,type,isdefault,expressname FROM " . tablename('tapgo_tt_exhelper_express') . " WHERE type=2 and uniacid=:uniacid and merchid=:merchid order by isdefault desc ", array(':uniacid' => $_W['uniacid'], ':merchid' => $merchid));
    ...
    public function getPluginsetByMerch($key = '')
    {
        ...
        $set = pdo_fetch("select * from " . tablename('tapgo_tt_sysset') . ' where uniacid=:uniacid limit 1', array(':uniacid' => $uniacid));
        if (empty($set)) {
            $set = array();
        }
    ...
    public function getPluginList($merchid = 0)
    {
        ...
        if (p('goodshelper') && $perm && $perm->is_perm_plugin('goodshelper')) {
