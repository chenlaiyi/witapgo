...
class User_TapgoTtPage extends PluginWebPage
{
    ...
    function main()
    {
        ...
        $sql = "select  u.*,g.groupname  from " . tablename('tapgo_tt_merch_user') . "  u "
            . " left join  " . tablename('tapgo_tt_merch_group') . " g on u.groupid = g.id "
            . " where u.uniacid=:uniacid {$condition} ORDER BY {$sortfield} desc";
        if (empty($_GPC['export'])) {
            $sql .= " limit " . ($pindex - 1) * $psize . ',' . $psize;
        }
        $list = pdo_fetchall($sql, $params);
        $total = pdo_fetchcolumn("select count(*) from" . tablename('tapgo_tt_merch_user') . " u  "
            . " left join  " . tablename('tapgo_tt_merch_group') . " g on u.groupid = g.id "
...
//        unset($row);
    ...
    protected function post()
    {
        ...
        $item = pdo_fetch('select * from ' . tablename('tapgo_tt_merch_user') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
...
//            新用户积分提现默认为空
        ...
        if(!empty($item['accountid'])){
            $account = pdo_fetch('select * from ' . tablename('tapgo_tt_merch_account') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $item['accountid'], ':uniacid' => $_W['uniacid']));
        }
        ...
        if ($_W['ispost']) {
            ...
            if ($checkUser) {
                ...
                $usercount = pdo_fetchcolumn('select count(*) from ' . tablename('tapgo_tt_merch_account') . " where {$where} limit 1", $params);
            ...
            if (empty($account) || empty($account['salt']) || !empty($_GPC['pwd'])) {
                $salt = random(8);
                while (1) {
                    $saltcount = pdo_fetchcolumn('select count(*) from ' . tablename('tapgo_tt_merch_account') . ' where salt=:salt limit 1', array(':salt' => $salt));
                    if ($saltcount <= 0) {
                        break;
                    }
                    $salt = random(8);
            ...
            $data = array(
                ...
                'can_import' => (int) $_GPC['can_import'],
                'can_edit' => (int) $_GPC['can_edit'],
            ...
            if (empty($item)) {
                $item['applytime'] = time();
                pdo_insert('tapgo_tt_merch_user', $data);
                $id = pdo_insertid();
                $account['merchid'] = $id;

                //主帐号
                pdo_insert('tapgo_tt_merch_account', $account);
                $accountid = pdo_insertid();
                pdo_update('tapgo_tt_merch_user', array('accountid' => $accountid), array('id' => $id));
                ...
                pdo_update('tapgo_tt_merch_user', $data, array('id' => $id));

                //主帐号
                if (!empty($item['accountid'])) {
                    pdo_update('tapgo_tt_merch_account', $account, array('id' => $item['accountid']));
                } else {
                    pdo_insert('tapgo_tt_merch_account', $account);
                    $accountid = pdo_insertid();
                    pdo_update('tapgo_tt_merch_user', array('accountid' => $accountid), array('id' => $id));
    ...
    function status()
    {
        ...
        $items = pdo_fetchall("SELECT id,merchname FROM " . tablename('tapgo_tt_merch_user') . " WHERE id in( $id ) AND uniacid=" . $_W['uniacid']);

        foreach ($items as $item) {
            pdo_update('tapgo_tt_merch_user', array('status' => intval($_GPC['status'])), array('id' => $item['id']));
    ...
    function delete()
    {
        ...
        $items = pdo_fetchall("SELECT * FROM " . tablename('tapgo_tt_merch_user') . " WHERE id in( $id ) AND uniacid=" . $_W['uniacid']);
        foreach ($items as $item) {
            pdo_update('tapgo_tt_goods', $change_data, array('merchid' => $item['id'], 'uniacid' =>$uniacid));

            pdo_delete('tapgo_tt_merch_reg', array('id' => $item['regid']));
            pdo_delete('tapgo_tt_merch_account', array('merchid' => $item['id'], 'uniacid' =>$uniacid));

            pdo_delete('tapgo_tt_merch_user', array('id' => $item['id'], 'uniacid' =>$uniacid));
    ...
    function query() {
        ...
        $ds = pdo_fetchall('SELECT * FROM ' . tablename('tapgo_tt_merch_user') . " WHERE {$condition} order by id asc", $params);
        include $this->template();
        exit;
    }
    ...
    public  function querymerchs(){
        ...
        $ds = pdo_fetchall('SELECT id,merchname as title ,logo as thumb FROM ' . tablename('tapgo_tt_merch_user') . " WHERE 1 {$condition} order by id desc", $params);
