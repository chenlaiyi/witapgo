...
class Merch_TapgoTtPage extends PluginWebPage 
{
	...
	public function ajax_user_data() 
	{
		...
		$merch = pdo_fetch('select * from ' . tablename('tapgo_tt_merch_user') . ' where id = :id', array(':id' => $merchid));
		...
		$sql = 'select u.id,u.merchname,u.payrate,sum(o.price) price,sum(o.goodsprice) goodsprice,sum(o.dispatchprice) dispatchprice,sum(o.discountprice) discountprice,sum(o.deductprice) deductprice,sum(o.deductcredit2) deductcredit2,sum(o.isdiscountprice) isdiscountprice,sum(o.deductenough) deductenough,sum(o.merchdeductenough) merchdeductenough,sum(o.couponmerchid) couponmerchid from ' . tablename('tapgo_tt_merch_user') . ' u ' . ' left join ' . tablename('tapgo_tt_order') . ' o on u.id=o.merchid' . ' where 1 ' . $condition . ' limit 1';
		$list = pdo_fetch($sql, $params);
		$merchcouponprice = pdo_fetchcolumn('select sum(o.couponprice) from ' . tablename('tapgo_tt_merch_user') . ' u ' . ' left join ' . tablename('tapgo_tt_order') . ' o on u.id=o.merchid' . ' where o.couponmerchid>0 ' . $condition . ' limit 1', $params);
