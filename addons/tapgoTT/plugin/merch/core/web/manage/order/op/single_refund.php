...
class Single_Refund_TapgoTtPage extends MerchWebPage {
    ...
    function submit() {
        ...
        if ($_W['ispost']) {
            ...
            if ($refundstatus == 0) {
                ...
                if ($raid == 0) {
                    $raddress = pdo_fetch('select * from '.tablename('tapgo_tt_refund_address').' where isdefault=1 and uniacid=:uniacid and merchid=:merchid limit 1',array(':uniacid'=>$uniacid,':merchid'=>$_W['merchid']));
                } else {
                    $raddress = pdo_fetch('select * from '.tablename('tapgo_tt_refund_address').' where id=:id and uniacid=:uniacid and merchid=:merchid limit 1',array(':id'=>$raid,':uniacid'=>$uniacid,':merchid'=>$_W['merchid']));
                }

                if (empty($raddress)) {
                    $raddress = pdo_fetch('select * from '.tablename('tapgo_tt_refund_address').' where uniacid=:uniacid and merchid=:merchid order by id desc limit 1',array(':uniacid'=>$uniacid,':merchid'=>$_W['merchid']));
                }
        ...
        $refund_address = pdo_fetchall('select * from '.tablename('tapgo_tt_refund_address').' where uniacid=:uniacid and merchid=:merchid',array(':uniacid'=>$_W['uniacid'],':merchid'=>$_W['merchid']));
    ...
    function refund_after($item,$time,$shopset){
        ...
        $order_goods=pdo_fetchall("select single_refundid,single_refundstate,single_refundtime from ".tablename('tapgo_tt_order_goods')." where orderid=:orderid",array(':orderid'=>$item['id']));
        ...
        if($refund_num==count($order_goods)){
            ...
            pdo_update('tapgo_tt_order', array('status' => -1,'canceltime' => $time), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
            plog('order.op.close', "订单关闭 ID: {$item['id']} 订单号: {$item['ordersn']}");
        }

        if(empty($apply_refund_num)){
            pdo_update('tapgo_tt_order', array('refundtime' => $time), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
        }
