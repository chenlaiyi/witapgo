...
class Op_TapgoTtPage extends MerchWebPage {
    ...
    function delete() {
        ...
        pdo_update('tapgo_tt_order', array('deleted' => 1), array('id' => $orderid, 'uniacid' => $_W['uniacid'], 'merchid' => $_W['merchid']));
    ...
    protected function opData() {
        ...
        global $_W, $_GPC;
        $id = intval($_GPC['id']);
        $item = pdo_fetch("SELECT * FROM " . tablename('tapgo_tt_order') . " WHERE id = :id and uniacid=:uniacid and merchid = :merchid", array(':id' => $id, ':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid']));
        if (empty($item)) {
            if ($_W['isajax']) {
                show_json(0, "未找到订单!");
            }
    ...
    function changeprice() {
        ...
        if ($_W['ispost']) {
            ...
            foreach ($changegoodsprice as $ogid => $change) {
                $og = pdo_fetch('select price,realprice from ' . tablename('tapgo_tt_order_goods') . ' where id=:ogid and uniacid=:uniacid and merchid = :merchid limit 1', array(':ogid' => $ogid, ':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid']));
                if (!empty($og)) {
            ...
            if (!empty($orderupdate)) {
                pdo_update('tapgo_tt_order', $orderupdate, array('id' => $item['id'], 'uniacid' => $_W['uniacid'], 'merchid' => $_W['merchid']));
            }
            $dividend = p('dividend');
            ...
            //修改商品价
            foreach ($changegoodsprice as $ogid => $change) {
                $og = pdo_fetch('select price,realprice,changeprice from ' . tablename('tapgo_tt_order_goods') . ' where id=:ogid and uniacid=:uniacid and merchid = :merchid limit 1', array(':ogid' => $ogid, ':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid']));
                if (!empty($og)) {
        ...
        //订单商品
        $order_goods = pdo_fetchall('select og.id,g.title,g.thumb,g.goodssn,og.goodssn as option_goodssn, g.productsn,og.productsn as option_productsn, og.total,og.price,og.optionname as optiontitle, og.realprice,og.oldprice from ' . tablename('tapgo_tt_order_goods') . ' og '
    ...
    function close() {
        ...
        if ($_W['ispost']) {
            ...
            if ($item['refundstate'] > 0 && !empty($item['refundid'])) {
                ...
                pdo_update('tapgo_tt_order_refund', $change_refund, array('id' => $item['refundid'], 'uniacid' => $_W['uniacid'], 'merchid' => $_W['merchid']));
            }
    ...
    function fetch() {
        ...
        if ($item['isverify'] == 1) {
            ...
            if(empty($verifyinfo)){
                $verifyinfo[] = array(
                    'verifyopenid'  => $item['openid'],
                    'verifystoreid' => $item['storeid'],
                    'verifytime'    => $time
                );
            }else{
        ...
        /*$verify = pdo_fetch("select * from ".tablename('tapgo_tt_order')." where id = :id and uniacid = :uniacid and merchid = :merchid ",array(':id'=>$item['id'],':uniacid'=>$_W['uniacid'],'merchid'=>$_W['merchid']));
        show_json(0,array('data'=>$d,'item'=>$verify));
        show_json(0,array('data'=>$d,'item'=>$d['verifyinfo']));*/
        pdo_update(
            'tapgo_tt_order', $d, array('id' => $item['id'], 'uniacid' => $_W['uniacid'], 'merchid' => $_W['merchid'])
        );
    ...
    function remarksaler() {
        ...
        if ($_W['ispost']) {
            pdo_update('tapgo_tt_order', array('remarksaler' => $_GPC['remark']), array('id' => $item['id'], 'uniacid' => $_W['uniacid'], 'merchid' => $_W['merchid']));
    ...
    function changeexpress() {
        ...
        if ($_W['ispost']) {
            ...
            if (!empty($expresssn)) {
                ...
                pdo_update('tapgo_tt_order', $change_data, array('id' => $id, 'uniacid' => $_W['uniacid'], 'merchid' => $_W['merchid']));
    ...
    function changeaddress() {
        ...
        if ($_W['ispost']) {
            ...
            if (!empty($id)) {
                ...
                $item = pdo_fetch("SELECT id, ordersn, address FROM " . tablename('tapgo_tt_order') . " WHERE id = :id and uniacid=:uniacid and merchid = :merchid", array(':id' => $id, ':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid']));
                ...
                pdo_update('tapgo_tt_order', array('address' => $address_array), array('id' => $id, 'uniacid' => $_W['uniacid'], 'merchid' => $_W['merchid']));
