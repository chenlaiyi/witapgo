...
class List_TapgoTtPage extends MerchWebPage {
    ...
    protected  function orderData($status, $st){
        ...
        if (!empty($_GPC['searchfield']) && !empty($_GPC['keyword'])) {
            ...
            if ($searchfield == 'ordersn') {
                ...
                $sqlcondition = " left join " . tablename('tapgo_tt_merch_store') . " store on (store.id = o.verifystoreid or store.id = vol.storeid) and store.uniacid=o.uniacid";
        ...
        $sql = "select distinct o.* ,a.realname as arealname,a.mobile as amobile,a.province as aprovince ,a.city as acity , a.area as aarea,a.address as aaddress,a.street as astreet,d.dispatchname,m.nickname,m.id as mid,m.realname as mrealname,m.mobile as mmobile,sm.id as salerid,sm.nickname as salernickname,s.salername,r.rtype,r.status as rstatus from " . tablename('tapgo_tt_order') . " o"
            ...
            . " left join " . tablename('tapgo_tt_verifyorder_log') . " vol on vol.orderid=o.id "
            . " left join " . tablename('tapgo_tt_merch_saler') . " s on (s.openid = o.verifyopenid or s.id=vol.salerid) and s.uniacid=o.uniacid and s.merchid=o.merchid"
        ...
        if ($_GPC['export'] == 1) {
            ...
            $columns = array(
                ...
                array('title' => '主商城满额立减', 'field' => 'deductenough', 'width' => 12),
                array('title' => '多商户满额立减', 'field' => 'merchdeductenough', 'width' => 12),
                array('title' => '主商城优惠券优惠', 'field' => 'couponprice', 'width' => 12),
                array('title' => '多商户优惠券优惠', 'field' => 'merchcouponprice', 'width' => 12),
            ...
            foreach ($list as &$row) {
                $row['merchcouponprice'] = 0;
                if ($row['couponmerchid'] > 0)
                {
                    $merchcouponprice = pdo_fetchcolumn("select sum(o.couponprice) from " . tablename('tapgo_tt_merch_user') . " u "
                        ...
                        . " where o.couponmerchid>0  and o.id = :id limit 1", array(':id'=>$row['id']));
                    $row['merchcouponprice'] = '-'. $merchcouponprice;
                    $row['couponprice'] = 0;
                }
                if ($row['merchcouponprice'] > 0)
                {
                    $row['couponprice'] = 0;
                }
                ...
                if (!empty($row['verifyopenid']) && $row['verifytype'] == 0) {
                    ...
                    $salermember = pdo_fetch("SELECT * FROM ".tablename('tapgo_tt_merch_saler')." s left join ".tablename('tapgo_tt_member')." m on m.openid=s.openid
        ...
        if ($searchfield == 'member') {
            ...
            if (!empty($openidArr)) {
                ...
                $orderPrice = pdo_fetch('SELECT count(1) as count,sum(price) as sumprice FROM '.tablename('tapgo_tt_order').' WHERE uniacid = '.$_W['uniacid'].' AND merchid = '.$merchid.' AND deleted=0 AND isparent=0 AND openid IN ('.$inOpenid.')'.$priceStatus);
            }else{
            ...
            $orderPrice = pdo_fetch('SELECT count(1) as count,sum(o.price) as sumprice FROM '.tablename('tapgo_tt_order').
                ' o left join '.tablename('tapgo_tt_member_address').' a on o.addressid = a.id WHERE o.uniacid = '.$_W['uniacid'].'  AND o.deleted=0 AND o.isparent=0 AND o.merchid = '.$merchid.$priceCondition.$statuscondition);
            $totalmoney = $orderPrice['sumprice'];
            $total = $orderPrice['count'];
            ...
            $t = pdo_fetch(
                'SELECT count(DISTINCT(o.id)) as count,sum(o.price) as sumprice FROM ' . tablename('tapgo_tt_order') . " o "
                . " left join " . tablename('tapgo_tt_order_refund') . " r on r.id =o.refundid "
                . " left join " . tablename('tapgo_tt_verifyorder_log') . " vol on vol.orderid=o.id "
                . " left join " . tablename('tapgo_tt_merch_saler') . " s on (s.openid = o.verifyopenid or s.id=vol.salerid) and s.uniacid=o.uniacid and s.merchid=o.merchid"
                . " left join " . tablename('tapgo_tt_member') . " sm on sm.openid = s.openid and sm.uniacid=s.uniacid"
        ...
        $stores = pdo_fetchall('select id,storename from ' . tablename('tapgo_tt_merch_store') . ' where uniacid=:uniacid and merchid = :merchid', array(':uniacid' => $uniacid, ':merchid' => $_W['merchid']));
