...
class Refund_TapgoTtPage extends MerchWebPage {
    ...
    protected function opData() {
        ...
        $item = pdo_fetch("SELECT * FROM " . tablename('tapgo_tt_order') . " WHERE id = :id and uniacid=:uniacid and merchid = :merchid  Limit 1", array(':id' => $id, ':uniacid' => $_W['uniacid'], 'merchid' => $_W['merchid']));
        if (empty($item)) {
            if ($_W['isajax']) {
                show_json(0, "未找到订单!");
            }
    ...
    function submit() {
        ...
        if ($_W['ispost']) {
            ...
            if ($refund['status'] < 0 || $refund['status'] == 1) {
                pdo_update('tapgo_tt_order', array('refundstate' => 0), array('id' => $item['id'], 'uniacid' => $_W['uniacid'], 'merchid' => $_W['merchid']));
            ...
            if ($refundstatus == 0) {
                ...
                if ($raid == 0) {
                    $raddress = pdo_fetch('select * from '.tablename('tapgo_tt_refund_address').' where isdefault=1 and uniacid=:uniacid and merchid = :merchid limit 1',array(':uniacid'=>$uniacid, ':merchid' => $merchid));
                } else {
                    $raddress = pdo_fetch('select * from '.tablename('tapgo_tt_refund_address').' where id=:id and uniacid=:uniacid and merchid = :merchid limit 1',array(':id'=>$raid,':uniacid'=>$uniacid, ':merchid' => $merchid));
                }

                if (empty($raddress)) {
                    $raddress = pdo_fetch('select * from '.tablename('tapgo_tt_refund_address').' where uniacid=:uniacid and merchid = :merchid order by id desc limit 1',array(':uniacid'=>$uniacid, ':merchid' => $merchid));
                }
                ...
                pdo_update('tapgo_tt_order', $order_data, array('id'=>$item['id'], 'uniacid' => $uniacid, 'merchid' => $merchid));
                m('notice')->sendOrderMessage($item['id'], true);
                ...
                m('notice')->sendOrderMessage($item['id'], true);
                pdo_update('tapgo_tt_order', array('refundstate' => 0, 'status' => -1, 'refundtime' => $time), array('id' => $item['id'], 'uniacid' => $uniacid, 'merchid' => $merchid));
                foreach ($goods as $g) {
                ...
                pdo_update('tapgo_tt_order', array('refundstate' => 0), array('id' => $item['id'], 'uniacid' => $uniacid, 'merchid' => $merchid));
                ...
                $goods = pdo_fetchall("SELECT g.id,g.credit, o.total,o.realprice FROM " . tablename('tapgo_tt_order_goods') .
                    " o left join " . tablename('tapgo_tt_goods') . " g on o.goodsid=g.id " . " WHERE o.orderid=:orderid and o.uniacid=:uniacid and o.merchid = :merchid", array(':orderid' => $item['id'], ':uniacid' => $uniacid, ':merchid' => $merchid));
                $credits = m('order')->getGoodsCredit($goods);
                if($credits>0){
        ...
        $refund_address = pdo_fetchall('select * from '.tablename('tapgo_tt_refund_address').' where uniacid=:uniacid and merchid = :merchid',array(':uniacid'=>$_W['uniacid'], ':merchid' => $merchid));
    ...
    function main() {
        ...
        $goods = pdo_fetchall("SELECT g.*, o.goodssn as option_goodssn, o.productsn as option_productsn,o.total,g.type,o.optionname,o.optionid,o.price as orderprice,o.realprice,o.changeprice,o.oldprice,o.commission1,o.commission2,o.commission3,o.commissions{$diyformfields} FROM " . tablename('tapgo_tt_order_goods') .
            " o left join " . tablename('tapgo_tt_goods') . " g on o.goodsid=g.id " . " WHERE o.orderid=:orderid and o.uniacid=:uniacid and o.merchid=:merchid", array(':orderid' => $id, ':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid']));
        foreach ($goods as &$r) {
    ...
    function changeRefundStatus($id, $refundprice) {
        pdo_update('tapgo_tt_order_peerpay_payinfo', array('refundstatus' => 1, 'refundprice' => $refundprice), array('id' => $id));
