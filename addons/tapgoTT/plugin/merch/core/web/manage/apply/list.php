...
class List_TapgoTtPage extends MerchWebPage {
    ...
    protected  function applyData($status, $st){
        ...
        $sql = "select b.* from " . tablename('tapgo_tt_merch_bill') . " b "
        ...
        $total = pdo_fetchcolumn("select count(b.id) from" . tablename('tapgo_tt_merch_bill') . " b "
            . " left join " . tablename('tapgo_tt_merch_user') . " u on b.merchid = u.id"
    ...
    protected function post() {
        ...
        $total = pdo_fetchcolumn("select count(o.id) from " . tablename('tapgo_tt_merch_user') . " u "
...
//            foreach ($orderids as $item)
            ...
            pdo_insert('tapgo_tt_merch_bill', $insert);
            $billid = pdo_insertid();
            ...
            foreach ($lists as $k => $v) {
                ...
                tapgo_tt_order_single_refund')." where orderid = :id and uniacid = :uniacid",array(':uniacid'=>$_W['uniacid'],':id'=>$v['id']));
                $orderid = $v['id'];
                $insert_data = array();
                ...
                pdo_insert('tapgo_tt_merch_billo', $insert_data);

                $change_order_data = array();
                $change_order_data['merchapply'] = 1;

                pdo_update('tapgo_tt_order', $change_order_data, array('id' => $orderid));
            }
    ...
    function getLastApply($merchid, $applytype = -1)
    {
        ...
        $params = array(':uniacid' => $_W['uniacid'],':merchid'=>$merchid);
        $sql = "select applytype,alipay,bankname,bankcard,applyrealname from " . tablename('tapgo_tt_merch_bill') . " where merchid=:merchid and uniacid=:uniacid";
