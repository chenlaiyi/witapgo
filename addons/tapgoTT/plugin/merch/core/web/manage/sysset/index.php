...
class Index_TapgoTtPage extends MerchWebPage {
    ...
    function shop() {
        global $_W, $_GPC;
        $item = pdo_fetch('select * from ' . tablename('tapgo_tt_merch_user') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $_W['uniaccount']['merchid'], ':uniacid' => $_W['uniacid']));
        ...
        if ($_W['ispost']) {
            ...
            pdo_update('tapgo_tt_merch_user', $data, array('id' => $_W['uniaccount']['merchid']));
    ...
    function notice() {
        ...
        if(empty($data)){
            $merchid =  $_W['merchid'];
            if($merchid){
                $merch_user_set = pdo_get('tapgo_tt_merch_user',array('id' => $merchid));
