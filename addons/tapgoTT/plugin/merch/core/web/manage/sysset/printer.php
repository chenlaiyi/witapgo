...
class Printer_TapgoTtPage extends MerchWebPage
{
	...
	protected function post() {
		...
		$id = intval($_GPC['id']);
		if (!empty($id)) {
			$list = pdo_fetch('SELECT * FROM ' . tablename('tapgo_tt_member_printer_template') . ' WHERE id=:id and uniacid=:uniacid and merchid=:merchid', array(':id' => $id, ':uniacid' => $_W['uniacid'],':merchid' => $_W['merchid']));
	...
	function delete() {
		...
		$items = pdo_fetchall("SELECT id,title FROM " . tablename('tapgo_tt_member_printer_template') . " WHERE id in( $id ) AND uniacid=:uniacid AND merchid=:merchid",array(':uniacid' => $_W['uniacid'],':merchid' => $_W['merchid']));
    ...
    protected function printer_post()
    {
        ...
        if (!empty($id)) {
            $list = pdo_fetch('SELECT * FROM ' . tablename('tapgo_tt_member_printer') . ' WHERE id=:id and uniacid=:uniacid and merchid=:merchid', array(':id' => $id, ':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid']));
            $print_data = json_decode($list['print_data'],true);
    ...
    function printer_delete() {
        ...
        $items = pdo_fetchall("SELECT id,title FROM " . tablename('tapgo_tt_member_printer') . " WHERE id in( $id ) AND uniacid={$_W['uniacid']} AND merchid={$_W['merchid']}");
        foreach ($items as $item) {
    ...
    function set(){
        ...
        $order_template = pdo_fetchall('SELECT * FROM ' . tablename('tapgo_tt_member_printer_template') . ' WHERE uniacid=:uniacid AND merchid=:merchid', array(':uniacid' => $_W['uniacid'],':merchid' => $_W['merchid']));
        include $this->template();
    }
