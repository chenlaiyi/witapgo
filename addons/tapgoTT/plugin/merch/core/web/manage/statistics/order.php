...
class Order_TapgoTtPage extends MerchWebPage
{
	public function main()
	{
		...
		$sql = 'select o.*, a.realname as addressname,m.realname from ' . tablename('tapgo_tt_order') . ' o ' . ' left join ' . tablename('tapgo_tt_member') . ' m on o.openid = m.openid ' . ' left join ' . tablename('tapgo_tt_member_address') . ' a on a.id = o.addressid ' . ' where 1 ' . $condition . ' ';
		...
		foreach ($list as &$row ) {
			...
			$row['goods'] = pdo_fetchall('SELECT g.thumb,og.price,og.total,og.realprice,g.title,og.optionname from ' . tablename('tapgo_tt_order_goods') . ' og' . ' left join ' . tablename('tapgo_tt_goods') . ' g on g.id=og.goodsid  ' . ' where og.uniacid = :uniacid and og.orderid=:orderid order by og.createtime  desc ', array(':uniacid' => $_W['uniacid'], ':orderid' => $row['id']));
			$totalmoney += $row['price'];
		}

		unset($row);
		$condition = rtrim($condition, 'group by o.id');
		$count = pdo_fetch('select count(o.id) as count from ' . tablename('tapgo_tt_order') . ' o ' . ' left join ' . tablename('tapgo_tt_member') . ' m on o.openid = m.openid ' . ' left join ' . tablename('tapgo_tt_member_address') . ' a on a.id = o.addressid ' . ' where 1 ' . $condition, $params);
