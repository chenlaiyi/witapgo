...
class Goods_TapgoTtPage extends MerchWebPage 
{
	public function main() 
	{
		...
		$sql = 'select og.price,og.total,o.createtime,o.ordersn,g.title,g.thumb,g.goodssn,op.goodssn as optiongoodssn from ' . tablename('tapgo_tt_order_goods') . ' og ' . ' left join ' . tablename('tapgo_tt_order') . ' o on o.id = og.orderid ' . ' left join ' . tablename('tapgo_tt_goods') . ' g on g.id = og.goodsid ' . ' left join ' . tablename('tapgo_tt_goods_option') . ' op on op.id = og.optionid ' . ' where 1 ' . $condition . ' order by ' . $orderby . ' desc ';
