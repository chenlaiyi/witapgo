...
class Recommand_TapgoTtPage extends MerchWebPage 
{
	public function main() 
	{
		...
		if (!(empty($goodsids))) 
		{
			$goods = pdo_fetchall('select id,title,thumb from ' . tablename('tapgo_tt_goods') . ' where id in (' . $goodsids . ') and status=1 and deleted=0 and uniacid=' . $_W['uniacid'] . ' and merchid=' . $_W['merchid'] . ' order by instr(\'' . $goodsids . '\',id)');
		}
