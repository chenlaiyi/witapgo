...
class Goods_selector_TapgoTtPage extends MerchWebPage
{
	/**
     * 商品选择器
     */
	...
	public function op()
	{
		...
		$id = intval($_GPC['id']);
		$sql = 'select * from ' . tablename('tapgo_tt_goods') . ' where id = ' . $id;
		$goods = pdo_fetch($sql);
		...
		if ($_GPC['nooption'] == 1) {
			$sql = 'select * from ' . tablename('tapgo_tt_goods_option') . ' where goodsid = ' . $id;
