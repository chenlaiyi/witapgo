...
class Index_TapgoTtPage extends MerchWebPage
{
    ...
    function fetch()
    {
        ...
        $type = $_GPC['type'];
        $cates = $_GPC['cate'];
        $goodsTotal = pdo_fetchcolumn('select count(id) from ' . tablename('tapgo_tt_goods') . ' where uniacid = :uniacid and merchid = :merchid',array(':uniacid'=>$_W['uniacid'],'merchid'=>$_W['merch_user']['id']));
...
//        $set = m('common')->getSysset('goodshelper');
    ...
    function set()
    {
        ...
        if ($_W['ispost']) {
            ...
            pdo_update('tapgo_tt_merch_user', compact('apikey'), array('id' => $_W['merch_user']['id'], 'uniacid' => $_W['uniacid']));
            show_json(1);
        }
