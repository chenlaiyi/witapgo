...
class Index_TapgoTtPage extends MerchWebPage
{
    ...
    function main($goodsfrom = '')
    {
        ...
        $sql = 'SELECT COUNT(g.`id`) FROM ' . tablename('tapgo_tt_goods') . 'g' . $sqlcondition . $condition . $groupcondition;
        $total = pdo_fetchcolumn($sql, $params);
        $list = array();
        if (!empty($total)) {
            $sql = 'SELECT g.* FROM ' . tablename('tapgo_tt_goods') . 'g' . $sqlcondition . $condition . $groupcondition . ' ORDER BY g.`status` DESC, g.`merchdisplayorder` DESC,
        ...
        $shopset = m('common')->getSysset('category');
//        echo json_encode([$value['allcates'],$category]);
//        exit();
    ...
    function change()
    {
        ...
        $goods = pdo_fetch('select id,hasoption,checked from ' . tablename('tapgo_tt_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $id));
        if (empty($goods)) {
            show_json(0, array('message' => '参数错误'));
        }
       //如果修改了产品名称，商户又不是商品免审核，则需要重新审核
        ...
        pdo_update('tapgo_tt_goods', array($type => $value,'checked'=>$checked), array('id' => $id));
    ...
    function query()
    {
        ...
        $ds = pdo_fetchall('SELECT id,title,thumb,marketprice,productprice,share_title,share_icon,description,minprice FROM ' . tablename('tapgo_tt_goods') . " WHERE 1 {$condition} order by createtime desc", $params);
    ...
    public function import()
    {
        ...
        $_W['import'] = 1;
        if ($_W['ispost']) {
            $merchgroup = pdo_get('tapgo_tt_merch_group', array('id' => $_W['merch_user']['groupid']));
...
//            pdo_begin();
            foreach ($ids as $id) {
                $goods = pdo_get('tapgo_tt_goods', array('id' => $id));

                if (empty($goods)) {
                    continue;
                }
                $options = pdo_getall('tapgo_tt_goods_option', array('goodsid' => $id, 'uniacid' => $_W['uniacid']));
                ...
                pdo_insert('tapgo_tt_goods', $goods);
                $newid = pdo_insertid();

                //获取所有的params
                $params = pdo_getall('tapgo_tt_goods_param', array('goodsid' => $id));

                foreach ($params as $param) {
                    $param['goodsid'] = $newid;
                    unset($param['id']);
                    //参数入库
                    pdo_insert('tapgo_tt_goods_param', $param);
                }
                //获取sku
                $spec = pdo_getall('tapgo_tt_goods_spec', array('goodsid' => $id, 'uniacid' => $_W['uniacid']));
                ...
                foreach ($spec as $specitem) {
                    ...
                    pdo_insert('tapgo_tt_goods_spec', $specitem);

                    $new_specid = pdo_insertid();

                    //查询所有的item
                    $specitems = pdo_getall('tapgo_tt_goods_spec_item', array('specid' => $sid));
                    ...
                    foreach ($specitems as $sitems) {
                        ...
                        pdo_insert('tapgo_tt_goods_spec_item', $sitems);
                        ...
                        foreach ($options as &$op) {
                            ...
                            if (!strexists($op['specs'],'_') && $op['specs'] == $iid)
                            {
                                $op['specs'] = $itemsid;
                            }else{
                            ...
                            $op['goodsid'] = $newid;
//                            pdo_insert('tapgo_tt_goods_option', $op);

                        }

                        unset($op);
                    }
                    pdo_update('tapgo_tt_goods_spec',array('content'=>serialize($ids)),array('id'=>$new_specid));
                }
            }
