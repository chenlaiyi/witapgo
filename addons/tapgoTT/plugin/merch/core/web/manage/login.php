...
class Login_TapgoTtPage extends Page
{
...
//        exit();
        ...
        if ($_W['ispost']) {
            ...
            $account = pdo_fetch("select * from " . tablename('tapgo_tt_merch_account') . " where uniacid=:uniacid and username=:username limit 1", array(':uniacid' => $_W['uniacid'], ':username' => $username));
            if (empty($account)) {
                show_json(0, '用户未找到!');
            }
            ...
            $user = pdo_fetch('select status,accounttime from ' . tablename('tapgo_tt_merch_user') . ' where uniacid=:uniacid and accountid=:accountid limit 1', array(':uniacid' => $_W['uniacid'], ':accountid' => $account['id']));
            ...
            pdo_update('tapgo_tt_merch_account', $status, array('id' => $account['id']));
            $url = $_W['siteroot'] . "web/merchant.php?c=site&a=entry&i={$account['uniacid']}&m=tapgo_ttv2&do=web&r=shop";
            show_json(1, array('url' => $url));

        }
        $submitUrl = $_W['siteroot'] . "web/merchant.php?c=site&a=entry&i={$_COOKIE[$_W['config']['cookie']['pre'].'__uniacid']}&m=tapgo_ttv2&do=web&r=login";
