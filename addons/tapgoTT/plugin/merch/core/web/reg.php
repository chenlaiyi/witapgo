...
class Reg_TapgoTtPage extends PluginWebPage
{
    ...
    function main() {
        ...
        $sql = "select  *  from " . tablename('tapgo_tt_merch_reg') . "   where uniacid=:uniacid {$condition} ORDER BY  applytime desc";
        if (empty($_GPC['export'])) {
            $sql.=" limit " . ($pindex - 1) * $psize . ',' . $psize;
        }
        $list = pdo_fetchall($sql, $params);
        $total = pdo_fetchcolumn("select count(*) from" . tablename('tapgo_tt_merch_reg') . "  where uniacid = :uniacid {$condition}", $params);
    ...
    function detail()
    {
        ...
        $id = intval($_GPC['id']);
        $item = pdo_fetch('select * from '.tablename('tapgo_tt_merch_reg').' where id=:id and uniacid=:uniacid limit 1',array(':id'=>$id,':uniacid'=>$_W['uniacid']));
        if(empty($item)){
            if($_W['ispost']){
        ...
        if($_W['ispost']){
            ...
            pdo_update('tapgo_tt_merch_reg',$item,array('id'=>$item['id']));


            if($status==1){
                $usercount = pdo_fetch('select * from '.tablename('tapgo_tt_merch_user').' where regid=:regid and uniacid=:uniacid limit 1',array(':regid'=>$item['id'],':uniacid'=>$_W['uniacid']));
                ...
                if(empty($usercount)){
                    ...
                    $user['regid'] =$item['id'];
                    $user['status'] = 0;
                    pdo_insert('tapgo_tt_merch_user',$user);
                    ...
                    unset($user['reason']);
                    $user['status'] = 0;
                    pdo_update('tapgo_tt_merch_user',$user,array('uniacid'=>$_W['uniacid'],'regid'=>$item['id']));
                    pdo_update('tapgo_tt_merch_reg',$item,array('id'=>$item['id']));
                    m('notice')->sendMerchSuccess($item['id']);
                    if(ca('merch.user.edit')){
    ...
    function delete()
    {
        ...
        $regs = pdo_fetchall("SELECT id,merchname FROM " . tablename('tapgo_tt_merch_reg') . " WHERE id in( $id ) AND uniacid=" . $_W['uniacid']);
        foreach ($regs as $reg) {
            pdo_delete('tapgo_tt_merch_reg', array('id' => $reg['id']));
