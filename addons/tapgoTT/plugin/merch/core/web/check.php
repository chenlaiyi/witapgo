...
class Check_TapgoTtPage extends PluginWebPage {
    ...
    protected function main($status = 1)
    {
        ...
        $sql = "select b.*,u.merchname,u.realname,u.mobile from " . tablename('tapgo_tt_merch_bill') . " b "
        ...
        $total = pdo_fetchcolumn("select COUNT(1) from " . tablename('tapgo_tt_merch_bill') . " b "
            . " left join " . tablename('tapgo_tt_merch_user') . " u on b.merchid = u.id"
    ...
    function detail() {
        ...
        if (empty($item['applytype'])) {
            $merch_user = pdo_fetch("select * from " . tablename('tapgo_tt_merch_user') . " where uniacid=:uniacid and id={$item['merchid']}", array(':uniacid'=>$_W['uniacid']));
        ...
        $select=array();
        $select_order = pdo_fetchall("SELECT order_id FROM " . tablename('tapgo_tt_merch_bill_select') . " WHERE bill_id =:bill_id",array(':bill_id'=>$id));
    ...
    function merchpay()
    {
        ...
        pdo_update('tapgo_tt_merch_bill', $change_data, array('id' => $id));

        $orderids = iunserializer($item['passorderids']);
        foreach ($orderids as $key => $orderid) {
            pdo_update('tapgo_tt_order', array('merchapply' => 3), array('id' => $orderid));
        }
        show_json(1);
    }
        ...
        if ($type == 1) {
            ...
            foreach ($orderids as $key => $orderid) {
                if (in_array($orderid, $bpid)) {
                    pdo_update('tapgo_tt_order', array('merchapply' => 2), array('id' => $orderid));
                } else {
                    pdo_update('tapgo_tt_order', array('merchapply' => -1), array('id' => $orderid));
                }
            }
        } else if ($type == -1){
            $change_data = array();
    ...
    function query() {
        ...
        $sql = "select id,merchname,logo,realname,mobile from " . tablename('tapgo_tt_merch_user') . " where 1 {$condition} ORDER BY id ASC";
    ...
    public function status()
    {
        ...
        if ($id)
        {
            pdo_update('tapgo_tt_merch_clearing',array('status'=>$status),array('id'=>$id,'uniacid'=>$_W['uniacid']));
            show_json(1);
        }
        show_json(0);
    }
    ...
    public function delete()
    {
        ...
        if ($id)
        {
            pdo_query('DELETE FROM '.tablename('tapgo_tt_merch_clearing').' WHERE id=:id AND status<>2 AND uniacid=:uniacid',array(':id'=>$id,':uniacid'=>$_W['uniacid']));
            show_json(1);
        }
        show_json(0);
    }
    ...
    function change_bill_select()
    {
        ...
        if(!empty($id)){
            if($bpid==-1){
                if(empty($type)){
                    pdo_delete('tapgo_tt_merch_bill_select', array('bill_id' => $id));
                }else{
                    $bill = pdo_fetch("SELECT orderids FROM ".tablename('tapgo_tt_merch_bill')." WHERE id = :id limit 1", array(':id' => $id));
                    $orderids = iunserializer($bill['orderids']);
                    $orderids = array_unique($orderids);
                    foreach ($orderids as $ids){
                        pdo_insert('tapgo_tt_merch_bill_select', array('bill_id'=>$id,'order_id'=>intval($ids)),true);
                    }
                }
            }else{
                if(empty($type)){
                    pdo_delete('tapgo_tt_merch_bill_select', array('order_id' => $bpid));
                }else{
                    pdo_insert('tapgo_tt_merch_bill_select', array('bill_id'=>$id,'order_id'=>$bpid),true);
                }
            }
        }
       show_json(0);
