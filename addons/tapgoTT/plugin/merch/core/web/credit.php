...
class Credit_TapgoTtPage extends PluginWebPage 
{
	protected function main($status = 1) 
	{
		...
		$sql = 'select b.*,u.merchname,u.realname,u.mobile from ' . tablename('tapgo_tt_merch_bill') . ' b ' . ' left join ' . tablename('tapgo_tt_merch_user') . ' u on b.merchid = u.id' . ' where 1 ' . $condition . ' ORDER BY b.id desc ';
		...
		$_sql = 'select iscreditmoney,creditrate from ' . tablename('tapgo_tt_merch_user') . 'where 1 ' . $_condition;
		...
		$total = pdo_fetchcolumn('select COUNT(1) from ' . tablename('tapgo_tt_merch_bill') . ' b ' . ' left join ' . tablename('tapgo_tt_merch_user') . ' u on b.merchid = u.id' . ' where 1 ' . $condition, $params);
	...
	public function confirm() 
	{
		...
		if ($type == 1) 
		{
			...
			foreach ($orderids as $key => $orderid ) 
			{
				...
				{
					pdo_update('tapgo_tt_creditshop_log', array('merchapply' => 2), array('id' => $orderid));
				}
				else 
				{
					pdo_update('tapgo_tt_creditshop_log', array('merchapply' => -1), array('id' => $orderid));
				}
			}
		}
		else if ($type == -1) 
