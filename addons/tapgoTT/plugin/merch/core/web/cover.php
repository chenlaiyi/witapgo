...
class Cover_TapgoTtPage extends PluginWebPage 
{
	...
	protected function _cover($key, $name, $url) 
	{
		global $_W;
		global $_GPC;
		$rule = pdo_fetch('select * from ' . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'cover', ':name' => 'tapgo_ttv2' . $name . '入口设置'));
		$keyword = false;
		$cover = false;
		if (!(empty($rule))) 
		{
		...
		if ($_W['ispost']) 
		{
			...
			if (!(empty($keyword1))) 
			{
				if ($keyword1['name'] != 'tapgo_ttv2' . $name . '入口设置') 
				{
					show_json(0, '关键字已存在!');
				}
			}
			...
			$rule_data = array('uniacid' => $_W['uniacid'], 'name' => 'tapgo_ttv2' . $name . '入口设置', 'module' => 'cover', 'displayorder' => 0, 'status' => intval($data['status']));
			...
			$cover_data = array('uniacid' => $_W['uniacid'], 'rid' => $rid, 'module' => 'tapgo_ttv2', 'title' => trim($data['title']), 'description' => trim($data['desc']), 'thumb' => save_media($data['thumb']), 'url' => $url);
