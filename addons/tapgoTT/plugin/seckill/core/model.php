...
class SeckillModel extends PluginModel
{
	public function get_prefix()
	{
		...
		return 'tapgo_ttv2_' . $_W['setting']['site']['key'] . '_' . $_W['uniacid'] . '_' . $_W['account']['key'] . '_seckill_';
	}
	...
	public function setTaskCache($id)
	{
		...
		$task = pdo_fetch('select * from ' . tablename('tapgo_tt_seckill_task') . ' where id=:id limit 1', array(':id' => $id));
		redis()->delete($redis_prefix . 'info_' . $id);
		redis()->hMset($redis_prefix . 'info_' . $id, $task);
		$allrooms = pdo_fetchall('select * from ' . tablename('tapgo_tt_seckill_task_room') . ' where taskid=:taskid and enabled=1 and uniacid=:uniacid order by `displayorder` desc', array(':taskid' => $id, ':uniacid' => $_W['uniacid']));
		...
		$alltimes = pdo_fetchall('select * from ' . tablename('tapgo_tt_seckill_task_time') . ' where taskid=:taskid and uniacid=:uniacid order by `time` asc', array(':taskid' => $id, ':uniacid' => $_W['uniacid']));
		$redisgoods = array();

		foreach ($alltimes as &$time) {
			$goods = pdo_fetchall('select * from ' . tablename('tapgo_tt_seckill_task_goods') . ' where taskid=:taskid and timeid=:timeid and uniacid=:uniacid order by displayorder asc', array(':taskid' => $id, ':timeid' => $time['id'], ':uniacid' => $_W['uniacid']));
		...
		foreach ($keys as $key) {
			...
			if (!empty($orders)) {
				...
				foreach ($orders as $orderid) {
					$o = pdo_fetch('select  id,openid,deductcredit2,ordersn,isparent,deductcredit,deductprice   from ' . tablename('tapgo_tt_order') . ' where id=:id  limit 1', array(':id' => $orderid));
					if (!empty($o) && ($o['status'] == 0)) {
						...
						pdo_query('update ' . tablename('tapgo_tt_order') . ' set status=-1,canceltime=' . time() . ' where id=' . $o['id']);
					}
				}
			}
		}
	...
	public function orderRefund($item)
	{
		...
		$goods = pdo_fetchall('SELECT g.id,g.credit, o.total,o.realprice FROM ' . tablename('tapgo_tt_order_goods') . ' o left join ' . tablename('tapgo_tt_goods') . ' g on o.goodsid=g.id ' . ' WHERE o.orderid=:orderid and o.uniacid=:uniacid', array(':orderid' => $item['id'], ':uniacid' => $item['uniacid']));
		...
		foreach ($goods as $g) {
			$salesreal = pdo_fetchcolumn('select ifnull(sum(total),0) from ' . tablename('tapgo_tt_order_goods') . ' og ' . ' left join ' . tablename('tapgo_tt_order') . ' o on o.id = og.orderid ' . ' where og.goodsid=:goodsid and o.status>=1 and o.uniacid=:uniacid limit 1', array(':goodsid' => $g['id'], ':uniacid' => $item['uniacid']));
	...
	public function setOrderPay($orderid)
	{
		...
		$redis_prefix = $this->get_prefix();
		$date = date('Y-m-d');
		$order = pdo_fetch('select id,ordersn, price,openid,status, paytype, deductcredit2, couponid,isparent,merchid,agentid,createtime from ' . tablename('tapgo_tt_order') . ' where  id=:id and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $orderid));
		...
		$goods = pdo_fetchall('select uniacid,total,goodsid,optionid,seckill_timeid,seckill_taskid from  ' . tablename('tapgo_tt_order_goods') . ' where orderid=' . $order['id'] . ' and  seckill=1 ');
	...
	public function getTaskSeckillInfo()
	{
		...
		$dbgoods = pdo_fetchall('select id,thumb,marketprice from ' . tablename('tapgo_tt_goods') . ' where id in (' . implode(',', $goodsids) . ') and uniacid=' . $_W['uniacid'], array(), 'id');
		$goods = array();
		...
		foreach ($times as $time) {
			...
			foreach ($goods as $g) {
				if (in_array($g['goodsid'], $goodsids) && ($g['roomid'] != $roomid)) {
					...
					$goodstitle = pdo_fetchcolumn('select title from ' . tablename('tapgo_tt_goods') . ' where id=:id limit 1', array(':id' => $g['goodsid']));
