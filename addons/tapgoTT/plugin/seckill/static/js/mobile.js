define(['core', 'tpl', 'swiper','./timer.js'], function (core, tpl,s,timerUtil) {
    ...
    model.getCurrentTime = function () {
        return +new Date($.ajax({url: '../addons/tapgo_ttv2/map.json', async: false}).getResponseHeader("Date")) / 1000
    };
    ...
    model.initTimer = function (timeid) {
        ...
        if (status != -1) {
            $.ajax({
                url: '../addons/tapgo_ttv2/map.json',
                complete: function (x) {
                    try{
    ...
    model.setTimer = function (timeid) {
        ...
        if(model.lasttime%10==0){
            ...
            var status = parseInt(slide.data('status')), starttime = slide.data('starttime'), endtime = slide.data('endtime');
            $.ajax({
                url: '../addons/tapgo_ttv2/map.json',cache:false,complete: function (x) {
