...
class Cardmanage_TapgoTtPage extends PluginWebPage
{
	public function main()
	{
		...
		$sql = 'SELECT * FROM ' . tablename('tapgo_tt_member_card') . ('
	...
	protected function post()
	{
		...
		global $_GPC;
		$id = intval($_GPC['id']);
		$item = pdo_fetch('SELECT * FROM ' . tablename('tapgo_tt_member_card') . '
		...
		if ($_W['ispost']) {
			...
			if (!empty($id)) {
				...
				$goods_update = pdo_update('tapgo_tt_member_card', $update_data, array('id' => $id, 'uniacid' => $_W['uniacid']));
				...
				$goods_insert = pdo_insert('tapgo_tt_member_card', $data);
			...
			if ($_GPC['cardmodel'] == 2 || $item['cardmodel'] == 2) {
				$sql = 'UPDATE ' . tablename('tapgo_tt_goods') . ' SET membercardpoint = 1 WHERE id in (' . $goodsids . ')';
				pdo_fetch($sql);
				$goodsids_del = '';
				...
				if (!empty($goodsids_old)) {
					...
					$sql = 'UPDATE ' . tablename('tapgo_tt_goods') . ' SET membercardpoint = 0 WHERE id in (' . $goodsids_del . ')';
					pdo_fetch($sql);
				}
			}
		...
		if (!empty($item['goodsids'])) {
			$sql = 'SELECT id,maxbuy,title,thumb,total FROM ' . tablename('tapgo_tt_goods') . ' WHERE id in (' . $item['goodsids'] . ')';
	...
	public function status()
	{
		...
		$item = pdo_fetch('SELECT id,name,status FROM ' . tablename('tapgo_tt_member_card') . (' WHERE id = \'' . $id . '\' AND uniacid=') . $_W['uniacid'] . '');
		...
		$result = pdo_update('tapgo_tt_member_card', array('status' => $status), array('id' => $id, 'uniacid' => $_W['uniacid']));
	...
	public function delete()
	{
		...
		global $_GPC;
		$id = intval($_GPC['id']);
		$item = pdo_fetch('SELECT id,name,goodsids FROM ' . tablename('tapgo_tt_member_card') . (' WHERE id = \'' . $id . '\' AND uniacid=') . $_W['uniacid'] . '');

		if (empty($item)) {
			show_json(0, '抱歉,会员卡不存在或者已经被删除');
		}

		pdo_update('tapgo_tt_member_card', array('isdelete' => 1, 'del_time' => time()), array('id' => $id, 'uniacid' => $_W['uniacid']));
		pdo_update('tapgo_tt_member_card_history', array('isdelete' => 1, 'del_time' => time()), array('member_card_id' => $id, 'uniacid' => $_W['uniacid']));
		$goodsids = $item['goodsids'];
		pdo_update('tapgo_tt_goods', array('membercardpoint' => 0), array());
		pdo_query('update ' . tablename('tapgo_tt_goods') . ' set membercardpoint=0 where find_in_set(id, :goodsids) and uniacid=:uniacid', array(':goodsids' => $goodsids, ':uniacid' => $_W['uniacid']));
	...
	public function querycoupon($couponid = array())
	{
		...
		foreach ($couponid as $ck => $cv) {
			...
			$cpsql = 'SELECT * FROM ' . tablename('tapgo_tt_coupon') . $where;
