...
class Getrecord_TapgoTtPage extends PluginWebPage
{
	public function main()
	{
		...
		$sql = 'SELECT mch.*,mc.create_time,m.uid,m.avatar,m.nickname,m.id as mid FROM  
            ' . tablename('tapgo_tt_member_card_history') . ' as mch 
            LEFT JOIN   ' . tablename('tapgo_tt_member_card') . ' as mc on mch.member_card_id=mc.id 
		...
		$total = pdo_fetchcolumn('SELECT count(DISTINCT mch.id)  FROM   
            ' . tablename('tapgo_tt_member_card_history') . ' mch 
