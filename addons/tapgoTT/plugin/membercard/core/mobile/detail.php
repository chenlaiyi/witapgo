...
class Detail_TapgoTtPage extends PluginMobilePage
{
	...
	public function membercard_complete()
	{
		...
		if ($log['status'] != 1) {
			...
			$orderid = pdo_fetchcolumn('select id from ' . tablename('tapgo_tt_member_card_order') . ' where orderno=:orderno and uniacid=:uniacid', array(':orderno' => $log['tid'], ':uniacid' => $_W['uniacid']));
			...
			if (!empty($orderid)) {
				...
				pdo_update('tapgo_tt_member_card_order', $data_alipay, array('id' => $orderid));
			}
		}
