...
class Pay_TapgoTtPage extends PluginMobileLoginPage
{
	public function main()
	{
		...
		$order = pdo_fetch('select o.*,c.name,c.status as gstatus,c.isdelete as gdeleted,c.stock from ' . tablename('tapgo_tt_member_card_order') . ' as o
				left join ' . tablename('tapgo_tt_member_card') . ' as c on c.id = o.member_card_id
	...
	public function complete()
	{
		...
		if (is_h5app() && empty($orderid)) {
			...
			$orderid = pdo_fetchcolumn('select id from ' . tablename('tapgo_tt_member_card_order') . ' where orderno=:orderno and uniacid=:uniacid and openid=:openid limit 1', array(':orderno' => $orderno, ':uniacid' => $uniacid, ':openid' => $openid));
		}
		...
		$order = pdo_fetch('select * from ' . tablename('tapgo_tt_member_card_order') . ' where id = :orderid and uniacid=:uniacid and openid=:openid', array(':orderid' => $orderid, ':uniacid' => $uniacid, ':openid' => $openid));
		...
		$set['pay']['weixin'] = !empty($set['pay']['weixin_sub']) ? 1 : $set['pay']['weixin'];
		$set['pay']['weixin_jie'] = !empty($set['pay']['weixin_jie_sub']) ? 1 : $set['pay']['weixin_jie'];
		$order_goods = pdo_fetch('select * from  ' . tablename('tapgo_tt_member_card') . '
		...
		if ($type == 'credit') {
			...
			pdo_update('tapgo_tt_member_card_order', array('paytype' => 'credit', 'status' => 1, 'paytime' => time(), 'finishtime' => time()), array('id' => $orderid));
			...
			if ($type == 'wechat') {
				...
				if (!is_error($payquery) || !is_error($payqueryBorrow)) {
					...
					pdo_update('tapgo_tt_member_card_order', array('paytype' => 'wechat', 'status' => 1, 'paytime' => time(), 'finishtime' => time(), 'apppay' => is_h5app() ? 1 : 0), array('id' => $orderid));
	...
	public function orderstatus()
	{
		...
		$uniacid = $_W['uniacid'];
		$orderid = intval($_GPC['id']);
		$order = pdo_fetch('select status from ' . tablename('tapgo_tt_member_card_order') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $orderid, ':uniacid' => $uniacid));
		show_json(1, $order);
	}
