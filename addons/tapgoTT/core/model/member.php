...
class Member_TapgoTtModel {
    ...
    public function getInfo($openid = '')
    {
        ...
        $uid = intval($openid);
        if ($uid == 0) {
            $info = pdo_fetch('select * from ' . tablename('tapgo_tt_member') . ' where openid=:openid and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
            if (empty($info)) {
                ...
                if (!empty($condition)) {
                    $info = pdo_fetch('select * from ' . tablename('tapgo_tt_member') . ' where ' . $condition . '  and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
                    if (!empty($info)) {
            ...
            $info = pdo_fetch('select * from ' . tablename('tapgo_tt_member') . ' where id=:id  and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $uid));
        }
        if (!empty($info['uid'])) {
            //会员余额积分信息
            load()->model('mc');
    ...
    public function getMember($openid = '')
    {
        ...
        if ($uid == 0) {
            ...
            $info = pdo_fetch('select * from ' . tablename('tapgo_tt_member') . ' where id=:id and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $openid));
        }
        ...
        if (!empty($info)) {
            ...
            if(strpos($info['avatar'],'132132')){
                ...
                pdo_update('tapgo_tt_member', $upgrade2, array('id' => $info['id']));
            }
        ...
        if (empty($info['uid'])) {
            ...
            if ($followed) {
                ...
                if (!empty($uid)) {
                    ...
                    if (!empty($upgrade)) {
                        pdo_update('tapgo_tt_member', $upgrade, array('id' => $info['id']));
                    }
                }
            }
        }
    ...
    public function getMobileMember($mobile) {
        global $_W;
        $info = pdo_fetch('select * from ' . tablename('tapgo_tt_member') . ' where mobile=:mobile and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':mobile' => $mobile));
        if (!empty($info)){
            $info = $this->updateCredits($info);
        }
        return $info;
    ...
    public function setCredit($openid = '', $credittype = 'credit1', $credits = 0, $log = array()) {
        ...
        if (!empty($uid)) {
            ...
            //如果未关注
            $value = pdo_fetchcolumn("SELECT {$credittype} FROM " . tablename('tapgo_tt_member') . " WHERE  uniacid=:uniacid and openid=:openid limit 1", array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
            $newcredit = $credits + $value;
            if ($newcredit <= 0) {
                $newcredit = 0;
            }
            pdo_update('tapgo_tt_member', array($credittype => $newcredit), array('uniacid' => $_W['uniacid'], 'openid' => $openid));
        ...
        //新增商城积分(余额)记录表
        /*$member_log_table_flag = pdo_tableexists('tapgo_tt_member_credit_record');
        if($member_log_table_flag){
            $log_data['openid'] = $openid;
            $log_data['presentcredit'] = $a;
            pdo_insert('tapgo_tt_member_credit_record', $log_data);
        }*/
...
//        }
    ...
    public function getCredit($openid = '', $credittype = 'credit1') {
        ...
        if (!empty($uid)) {
            ...
            //如果未关注
            $item = pdo_fetch("SELECT {$credittype} FROM " . tablename('tapgo_tt_member') . " WHERE openid=:openid and uniacid=:uniacid limit 1", array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
            if(empty($item)){
                $item = pdo_fetch("SELECT {$credittype} FROM " . tablename('tapgo_tt_member') . " WHERE openid_wa=:openid and uniacid=:uniacid limit 1", array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
            }
    ...
    public function getCredits($openid = '', $credittypes = array('credit1', 'credit2')) {
        ...
        if (!empty($uid)) {
            ...
            $item = pdo_fetch("SELECT {$types} FROM " . tablename('tapgo_tt_member') . " WHERE openid=:openid and uniacid=:uniacid limit 1", array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
            if (empty($item)){
                $item = pdo_fetch("SELECT {$types} FROM " . tablename('tapgo_tt_member') . " WHERE openid=:openid and uniacid=:uniacid limit 1", array(':uniacid' => $_W['uniacid'], ':openid' => $oldOpenid));
            }
            if(empty($item)){
                $item = pdo_fetch("SELECT {$types} FROM " . tablename('tapgo_tt_member') . " WHERE openid_wa=:openid and uniacid=:uniacid limit 1", array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
            }
            //如果未关注
//            return pdo_fetch("SELECT {$types} FROM " . tablename('tapgo_tt_member') . " WHERE  (openid=:openid or openid_wa=:openid) and uniacid=:uniacid limit 1", array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
            if(empty($item)){
	...
	public function checkMember() {
		...
		//如果你把下面这个if注释了,你就倒霉了
		if (empty($openid) && !TAPGO_TT_DEBUG) {
		...
		if (empty($member) && !empty($openid)) {
			...
			if (!empty($upgrade)) {
				pdo_update('tapgo_tt_member', $upgrade, array('id' => $member['id']));
			}
            $this->deleterepeat($openid);
		}
		//分销商
    ...
    function getLevels($all=true) {
        ...
        return pdo_fetchall('select * from ' . tablename('tapgo_tt_member_level') . ' where uniacid=:uniacid'.$condition.' order by level asc', array(':uniacid' => $_W['uniacid']));
    }
    ...
    function getLevel($openid) {
        ...
        if (!empty($member) && !empty($member['level'])) {
            $level = pdo_fetch('select * from ' . tablename('tapgo_tt_member_level') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $member['level'], ':uniacid' => $_W['uniacid']));
            if (!empty($level)) {
                return $level;
            }
        }
    ...
    function getOneGoodsLevel($openid, $goodsid) {
        ...
        $data = array();
        $levels = pdo_fetchall('select * from ' . tablename('tapgo_tt_member_level') . ' where uniacid=:uniacid and buygoods=1 and level and level > :level order by level asc', array(':uniacid' => $uniacid, ':level' => $level));
    ...
    function getGoodsLevel($openid, $orderid) {
        ...
        $order_goods = pdo_fetchall("select goodsid from " . tablename('tapgo_tt_order_goods') . " where orderid=:orderid and uniacid=:uniacid", array(':uniacid' => $uniacid, ':orderid' => $orderid));
    ...
    function upgradeLevel($openid, $orderid = 0, $status = 3) {
        ...
        $level = false;
        if (empty($leveltype)) {
            /*$ordermoney = pdo_fetchcolumn('select ifnull( sum(og.realprice),0) from ' . tablename('tapgo_tt_order_goods') . ' og '
            ...
            //根据订单支付金额来判断   -- yqj --
            $ordermoney = pdo_fetchcolumn('select sum(price) from ' . tablename('tapgo_tt_order') . " where uniacid=:uniacid and openid=:openid and status=:status limit 1", array(':uniacid' => $_W['uniacid'], ':openid' => $member['openid'], ':status' => $status));
            $ordermoney = floatval($ordermoney);
            $level = pdo_fetch('select * from ' . tablename('tapgo_tt_member_level') . " where uniacid=:uniacid  and enabled=1 and {$ordermoney} >= ordermoney and ordermoney>0  order by level desc limit 1", array(':uniacid' => $_W['uniacid']));
        } else if ($leveltype == 1) {
            $ordercount = pdo_fetchcolumn('select count(*) from ' . tablename('tapgo_tt_order') . ' where openid=:openid and status=:status and uniacid=:uniacid ', array(':uniacid' => $_W['uniacid'], ':openid' => $member['openid'],':status' => $status));
            $level = pdo_fetch('select * from ' . tablename('tapgo_tt_member_level') . " where uniacid=:uniacid and enabled=1 and {$ordercount} >= ordercount and ordercount>0  order by level desc limit 1", array(':uniacid' => $_W['uniacid']));
        }
        ...
        if ($canupgrade) {
            //会员升级
            pdo_update('tapgo_tt_member', array('level' => $level['id']), array('id' => $member['id']));
    ...
    function upgradeLevelByLevelId($openid, $LevelID) {
        ...
        $level = pdo_fetch('select *  from ' . tablename('tapgo_tt_member_level') . " where uniacid=:uniacid and enabled=1 and id=:id",  array(':uniacid' => $_W['uniacid'],':id' =>$LevelID));
    ...
    function getGroups() {
        global $_W;
        return pdo_fetchall('select * from ' . tablename('tapgo_tt_member_group') . ' where uniacid=:uniacid order by id asc', array(':uniacid' => $_W['uniacid']));
    }
    ...
    function setGroups($openid,$group_ids,$reason = ''){
        ...
        $old_group_ids = pdo_getcolumn('tapgo_tt_member',$condition,'groupid');
        ...
        pdo_update('tapgo_tt_member', array('groupid'=>$group_ids), $condition);
        foreach ($diff_ids as $groupid){
            pdo_insert('tapgo_tt_member_group_log',
    ...
    public function checkMemberSNS($sns){
        ...
        if (empty($member)){
            ...
            pdo_insert('tapgo_tt_member', $data);
            if(method_exists(m('member'),'memberRadisCountDelete')) {
                m('member')->memberRadisCountDelete(); //清除会员统计radis缓存
            }
            return pdo_insertid();
        ...
        elseif(empty($member['bindsns']) || $member['bindsns']==$sns){
            pdo_update('tapgo_tt_member', $data, array('id'=>$member['id'], 'uniacid'=>$_W['uniacid']));
            return $member['id'];
        }
    }
    ...
    public function deleterepeat($openid = '')
    {
        ...
        if (!empty($uid)) {
            $result = pdo_fetchall("SELECT id,openid,createtime FROM ".tablename('tapgo_tt_member')." WHERE uniacid=:uniacid and openid=:openid order by createtime DESC",array('uniacid' => $_W['uniacid'],'openid' => $uid));
            $count = count($result);
            if ($count > 1){
                pdo_delete('tapgo_tt_member',array('id'=>$result['0']['id'],'uniacid'=>$_W['uniacid']));
            }
        }
