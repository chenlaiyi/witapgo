...
class Notice_TapgoTtModel
{
    ...
    public function sendMerchSuccess($id)
    {
        ...
        $merch = pdo_get("tapgo_tt_merch_reg", array("id" => $id, "uniacid" => $_W["uniacid"]));
        $template = pdo_get("tapgo_tt_member_message_template_default", array("typecode" => "march_type_success"));
    ...
    public function sendMerchFail($id)
    {
        ...
        $template = pdo_get("tapgo_tt_member_message_template_default", array("typecode" => "march_type_fail"));
    ...
    public function sendTeamMessage($orderid = "0", $delRefund = false)
    {
        ...
        $order = pdo_fetch("select * from " . tablename("tapgo_tt_groups_order") . " where id=:id limit 1", array(":id" => $orderid));
        if (empty($order)) {
            return NULL;
        }
        $openid = $order["openid"];
    ...
    {
        global $_W;
        $order = pdo_fetch("select * from " . tablename("tapgo_tt_order") . " where id=:id limit 1", array(":id" => $orderid));
        if (empty($order)) {
            return NULL;
        }
        $openid = $order["openid"];
        ...
        $order_goods = pdo_fetchall("select g.id,g.title,og.realprice,og.total,og.price,og.optionname as optiontitle,g.noticeopenid,g.noticetype from " . tablename("tapgo_tt_order_goods") . " og " . " left join " . tablename("tapgo_tt_goods") . " g on g.id=og.goodsid " . " where " . $scondition . " and og.uniacid=:uniacid ", $param);
        $goods = "";
        $goodsname = "";
        $goodsnum = 0;
        ...
        if (!empty($order["storeid"])) {
            if (0 < $order["merchid"]) {
                $store = pdo_fetch("select * from " . tablename("tapgo_tt_merch_store") . " where id=:id and uniacid=:uniacid and merchid = :merchid limit 1", array(":id" => $order["storeid"], ":uniacid" => $_W["uniacid"], ":merchid" => $order["merchid"]));
            } else {
                $store = pdo_fetch("select * from " . tablename("tapgo_tt_store") . " where id=:id and uniacid=:uniacid limit 1", array(":id" => $order["storeid"], ":uniacid" => $_W["uniacid"]));
            }
        }
        $buyerinfo = "";
        $buyerinfo_name = "";
        ...
        if (!empty($order["address"])) {
            ...
            if (is_array($address) && empty($address) || !is_array($address)) {
                $address = pdo_fetch("select id,realname,mobile,address,province,city,area from " . tablename("tapgo_tt_member_address") . " where id=:id and uniacid=:uniacid limit 1", array(":id" => $order["addressid"], ":uniacid" => $_W["uniacid"]));
            }
            if (!empty($address)) {
        ...
        if ($order["status"] == 0) {
            ...
            pdo_update("tapgo_tt_order", array("willcancelmessage" => 1), array("id" => $orderid));
    ...
    public function sendOrderMessage($orderid = "0", $delRefund = false, $raid = NULL)
    {
        ...
        $order_goods = pdo_fetchall("select g.id,g.title,og.realprice,og.total,og.price,og.optionname as optiontitle,g.noticeopenid,g.noticetype,og.sendtype,og.expresscom,og.expresssn,og.sendtime from " . tablename("tapgo_tt_order_goods") . " og " . " left join " . tablename("tapgo_tt_goods") . " g on g.id=og.goodsid " . " where " . $scondition . " and og.uniacid=:uniacid ", $param);
        $goods = "";
        $goodsname = "";
        $goodsnum = 0;
        foreach ($order_goods as $og) {
        ...
        if ($delRefund) {
            ...
            if (empty($order["refundid"]) && $order["refundstate"] == 3) {
                $order["single_refund"] = pdo_fetch("select id from " . tablename("tapgo_tt_order_single_refund") . " where orderid=:id limit 1", array(":id" => $order[id]));
            }
            if (!empty($order["refundid"]) || !empty($order["single_refund"])) {
                $refund = pdo_fetch("select * from " . tablename("tapgo_tt_order_refund") . " where id=:id limit 1", array(":id" => $order["refundid"]));
                if (empty($refund)) {
                    $refund = pdo_fetch("select * from " . tablename("tapgo_tt_order_single_refund") . " where id=:id limit 1", array(":id" => $order["single_refund"]["id"]));
                }
                if (empty($refund)) {
                    return NULL;
                }
                        ...
                        if ($refund["status"] == 3) {
                            if ($refund["rtype"] == 2 || $refund["rtype"] == 1) {
                                if (0 < $order["merchid"]) {
                                    ...
                                    if (empty($raid)) {
                                        $salerefund = pdo_fetch("select * from " . tablename("tapgo_tt_refund_address") . " where uniacid=:uniacid and isdefault=1 limit 1", array(":uniacid" => $_W["uniacid"]));
                                    } else {
                                        $salerefund = pdo_fetch("select * from " . tablename("tapgo_tt_refund_address") . " where uniacid=:uniacid and id=:id limit 1", array(":uniacid" => $_W["uniacid"], ":id" => $raid));
                                    }
            ...
            if ($order["status"] == -1) {
                ...
                if ($order["status"] == 0 && $order["paytype"] == 3) {
                    ...
                    if ($order["status"] == 1 && !empty($order["istrade"])) {
                        $item = pdo_fetch("select og.trade_time,g.title,s.storename,s.mobile,p.nickname,s.id as storeid  from " . tablename("tapgo_tt_order_goods") . "  og\n\t\t\t\t left join " . tablename("tapgo_tt_order") . " o  on  og.orderid = o.id\n\t\t\t\t left join " . tablename("tapgo_tt_goods") . " g  on  og.goodsid = g.id\n\t\t\t\t left join " . tablename("tapgo_tt_store") . " s  on  o.storeid = s.id\n\t\t\t\t left join " . tablename("tapgo_tt_newstore_people") . " p  on  og.peopleid = p.id\n\t\t\t\t where o.id =:id limit 1", array(":id" => $order["id"]));
                        ...
                        if (empty($notice["o2o_snorder_close_advanced"])) {
                            ...
                            $salers = pdo_fetchall("select *   from " . tablename("tapgo_tt_saler") . " sr  inner join " . tablename("tapgo_tt_store") . " s on s.id = sr.storeid  where  s.id=:id and sr.getnotice =1", array(":id" => $item["storeid"]));
    ...
    public function sendMemberLogMessage($log_id = "", $channel = 0, $isback = false)
    {
        ...
        $log_info = pdo_fetch("select * from " . tablename("tapgo_tt_member_log") . " where id=:id and uniacid=:uniacid limit 1", array(":id" => $log_id, ":uniacid" => $_W["uniacid"]));
    ...
    public function sendStockWarnMessage($goodsid, $optionid)
    {
        ...
        $goods = pdo_fetch("select  * from " . tablename("tapgo_tt_goods") . " where id=:id and uniacid=:uniacid", array(":id" => $goodsid, ":uniacid" => $_W["uniacid"]));
        if (empty($goods)) {
            return NULL;
        }
        $goodtitle = $goods["title"];
        if (!empty($optionid)) {
            $option = pdo_fetch("select  *  from " . tablename("tapgo_tt_goods_option") . " where id=:id and uniacid=:uniacid and goodsid = :goodsid", array(":uniacid" => $_W["uniacid"], ":goodsid" => $goodsid, ":id" => $optionid));
            if (!empty($option)) {
    ...
    public function sendNotice(array $params)
    {
        ...
        if (!empty($templateid)) {
            $template = pdo_fetch("select * from " . tablename("tapgo_tt_member_message_template") . " where id=:id and uniacid=:uniacid limit 1", array(":id" => $templateid, ":uniacid" => $_W["uniacid"]));
            if (!empty($template)) {
                ...
                if (is_error($ret)) {
                    $templatetype = pdo_fetch("select templateid  from " . tablename("tapgo_tt_member_message_template_default") . " where typecode=:typecode and uniacid=:uniacid  limit 1", array(":typecode" => $tag, ":uniacid" => $_W["uniacid"]));
    ...
    public function settemplateidbyback($tag, $uniacid = 0)
    {
        ...
        $templatetype = pdo_fetch("select `name`,templatecode,content  from " . tablename("tapgo_tt_member_message_template_type") . " where typecode=:typecode  limit 1", array(":typecode" => $tag));
        ...
        foreach ($result["template_list"] as $key => $value) {
            if (str_replace("\n", "", $value["content"]) == $content) {
                $issnoet = false;
                $defaulttemp = pdo_fetch("select 1  from " . tablename("tapgo_tt_member_message_template_default") . " where typecode=:typecode and uniacid=:uniacid  limit 1", array(":typecode" => $tag, ":uniacid" => $uniacid));
                if (empty($defaulttemp)) {
                    pdo_insert("tapgo_tt_member_message_template_default", array("typecode" => $tag, "uniacid" => $uniacid, "templateid" => $value["template_id"]));
                } else {
                    pdo_update("tapgo_tt_member_message_template_default", array("templateid" => $value["template_id"]), array("typecode" => $tag, "uniacid" => $uniacid));
                }
                return true;
            }
        }
    ...
    {
        global $_W;
        $item = pdo_fetch("select vgl.*,vg.limitnum,s.storename,sa.salername,sa.openid as saleropenid,o.openid,g.title,o.ordersn  from " . tablename("tapgo_tt_verifygoods_log") . "   vgl\n\t\tleft  join " . tablename("tapgo_tt_store") . " s on s.id = vgl.storeid\n\t\tleft  join " . tablename("tapgo_tt_saler") . " sa on sa.id = vgl.salerid\n\t\tinner  join " . tablename("tapgo_tt_verifygoods") . " vg on vg.id = vgl.verifygoodsid\n\t\tinner  join " . tablename("tapgo_tt_order_goods") . " og on vg.ordergoodsid = og.id\n\t\tinner  join " . tablename("tapgo_tt_order") . " o on o.id = og.orderid\n\t\tleft  join " . tablename("tapgo_tt_goods") . " g on og.goodsid = g.id\n\t  where  vgl.id =:id  and vgl.uniacid=:uniacid  limit 1", array(":id" => $verifygoodlogid, ":uniacid" => $_W["uniacid"]));
        $verifygoodlogs = pdo_fetchall("select *  from " . tablename("tapgo_tt_verifygoods_log") . "    where verifygoodsid =:id  ", array(":id" => $item["verifygoodsid"]));
        $verifynum = 0;
    ...
    public function sendOrderWillVerifyMessage($orderid = "0", $time)
    {
        ...
        global $_GPC;
        $url = $this->getUrl("order/detail", array("id" => $orderid));
        $orders = pdo_fetch("select g.type,o.id,o.openid,g.title,og.total from " . tablename("tapgo_tt_order") . " o LEFT JOIN" . tablename("tapgo_tt_order_goods") . " og on og.orderid=o.id\n        LEFT JOIN " . tablename("tapgo_tt_goods") . " g on g.id=og.goodsid \n        LEFT JOIN " . tablename("tapgo_tt_verifygoods") . " vg on vg.orderid=o.id where  o.uniacid=" . $_W["uniacid"] . " and o.id=" . $orderid . " and willcloseverifymessage <>1");
        if (empty($orders)) {
            return NULL;
        }
        pdo_update("tapgo_tt_order", array("willcloseverifymessage" => 1), array("id" => $orderid));
        $member = m("member")->getMember($orders["openid"]);
        $time = date("Y-m-d H:i:s", $time);
        $datas = array(array("name" => "商品名称", "value" => $orders["title"]), array("name" => "核销时间", "value" => $time));
        $remark = "\n请尽快处理，逾期将作废，如有疑问请联系在线客服。";
        $text = "你好，您有即将逾期的商品，请尽快前往提货或使用！\n\n提货商品：[商品名称]\n到期时间：[核销时间] \n" . $remark;
        $message = array("first" => array("value" => "你好，您有即将逾期的商品，请尽快前往提货或使用！", "color" => "#ff0000"), "keyword1" => array("title" => "提货商品", "value" => $orders["title"], "color" => "#000000"), "keyword2" => array("title" => "到期时间", "value" => $time, "color" => "#000000"), "remark" => array("value" => "\n请尽快处理，逾期将作废！如有任何疑问可与我们联系，感谢您的支持", "color" => "#000000"));
        $this->sendNotice(array("openid" => $member["openid"], "tag" => "willcancelverify", "default" => $message, "cusdefault" => $text, "url" => $url, "datas" => $datas));
        com_run("sms::callsms", array("tag" => "backpoint_ok", "datas" => $datas, "mobile" => $member["mobile"]));
    }
}
