...
class Util_TapgoTtModel
{
	...
	public function get_area_config_data($uniacid = 0)
	{
		...
		$sql = 'select * from ' . tablename('tapgo_tt_area_config') . ' where uniacid=:uniacid limit 1';
	...
	public function location($lat, $lng)
	{
		...
		if (empty($key)) {
			$key = 'F51571495f717ff1194de02366bb8da9';
		}
	...
	private function getExpressBird($express, $expresssn, $mobile = '', $express_set)
	{
		...
		if (!empty($express_set['express_bird_cache']) && 0 < $express_set['express_bird_cache']) {
			...
			$cache = pdo_fetch('SELECT * FROM' . tablename('tapgo_tt_express_cache') . 'WHERE express=:express AND expresssn=:expresssn LIMIT 1', array('express' => $express, 'expresssn' => $expresssn));
		...
		$expressInfo = pdo_fetch('SELECT * FROM' . tablename('tapgo_tt_express') . 'WHERE express=:express LIMIT 1', array('express' => $express));
		...
		if ($expressData['Reason'] && 0 < strlen($expressData['Reason'])) {
			$list = array();
		}

		if (0 < $express_set['express_bird_cache'] && !empty($list)) {
			if (empty($cache)) {
				pdo_insert('tapgo_tt_express_cache', array('expresssn' => $expresssn, 'express' => $express, 'lasttime' => time(), 'datas' => iserializer($list)));
			}
			else {
				pdo_update('tapgo_tt_express_cache', array('lasttime' => time(), 'datas' => iserializer($list)), array('id' => $cache['id']));
			}
		}
	...
	private function getExpressAli($express, $expresssn, $mobile = '', $express_set)
	{
		...
		if (in_array($expressData['State'], $errorCode)) {
			$list = array();
		}
