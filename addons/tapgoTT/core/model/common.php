...
class Common_TapgoTtModel
{
    ...
    public function updateSysset($values, $uniacid = 0)
    {
        ...
        $setdata = pdo_fetch("select * from " . tablename("tapgo_tt_sysset") . " where uniacid=:uniacid limit 1", array( ":uniacid" => $uniacid ));
        if( empty($setdata) ) 
        {
            $res = pdo_insert("tapgo_tt_sysset", array( "sets" => iserializer($values), "uniacid" => $uniacid ));
        ...
        else
        {
            ...
            $res = pdo_update("tapgo_tt_sysset", array( "sets" => iserializer($sets) ), array( "id" => $setdata["id"] ));
            if( $res ) 
            {
                $setdata["sets"] = $sets;
            }
    ...
    public function deleteSysset($key, $uniacid = 0)
    {
        ...
        $setdata = pdo_fetch("select id, sets from " . tablename("tapgo_tt_sysset") . " where uniacid=:uniacid limit 1", array( ":uniacid" => $uniacid ));
        if( !empty($setdata) ) 
        {
            $sets = iunserializer($setdata["sets"]);
            $sets = (is_array($sets) ? $sets : array(  ));
            ...
            pdo_update("tapgo_tt_sysset", array( "sets" => iserializer($sets) ), array( "id" => $setdata["id"] ));
        }
    ...
    public function updatePluginset($values, $uniacid = 0)
    {
        ...
        if( empty($setdata) ) 
        {
            $res = pdo_insert("tapgo_tt_sysset", array( "plugins" => iserializer($values), "uniacid" => $uniacid ));
        ...
        else
        {
            ...
            $res = pdo_update("tapgo_tt_sysset", array( "plugins" => iserializer($plugins) ), array( "id" => $setdata["id"] ));
            if( $res ) 
            {
    ...
    public function alipay_build($params, $alipay = array(  ), $type = 0, $openid = "")
    {
        ...
        if( 0 < $pay["alipay_id"] ) 
        {
            ...
            if( empty($type) ) 
            {
                $config["notify_url"] = $_W["siteroot"] . "addons/tapgo_ttv2/payment/alipay/notify.php";
        ...
        if( empty($type) ) 
        {
            $set["notify_url"] = $_W["siteroot"] . "addons/tapgo_ttv2/payment/alipay/notify.php";
    ...
    public function publicAliPay($params = array(  ), $return = NULL)
    {
        ...
        $url = (TAPGO_TT_DEBUG ? "https://openapi.alipaydev.com/gateway.do" : "https://openapi.alipay.com/gateway.do");
        if( $return !== NULL ) 
        {
            return $public;
        }
    ...
    public function public_build($isapp = false)
    {
        ...
        {
            $payments = pdo_fetch("SELECT * FROM " . tablename("tapgo_tt_payment") . " WHERE uniacid=:uniacid AND id=:id", array( ":uniacid" => $_W["uniacid"], ":id" => $set["weixin_id"] ));
            if( empty($payments) ) 
            {
                error(-1, "支付参数不存在!");
            }
     ...
     * @return array
     */
    public function unionpay_build($params, $unionpay, $type = 0){
        global $_W;
        ...
        $unionpay = [
            ...
            'mch_id' => $data['sub_mch_id'],
            'is_raw' => 1,
            'sub_appid' => $data['sub_appid'],
            'key' => $data['apikey']
        ];
        $params["title"] = $this->changeTitle(trim($params["title"]));
        return $this->unionpay_jspay($params, $unionpay, $type = 0);
    }

    public function unionpay_jspay($params, $unionpay, $type = 0){
        global $_W;
        load()->func("communication");
        $Opt = array(  );
        $package = array(  );
        $tid=trim($params["tid"]).rand(10000,99999);
        
        $package["service"] = "pay.weixin.jspay";
        $package["mch_id"] = trim($unionpay["mch_id"]);
        $package["is_raw"] = trim($unionpay["is_raw"]);
        $package["out_trade_no"] = $tid;
        $package["body"] = trim($params["title"]);
        $package["sub_openid"] = (empty($params["openid"]) ? trim($_W["openid"]) : trim($params["openid"]));
        $package["total_fee"] = $params["fee"] * 100;
        $package["mch_create_ip"] = CLIENT_IP;
        ...
        $string1 .= "key=" . $unionpay["key"];
        $package["sign"] = strtoupper(md5(trim($string1)));
        $dat = array2xml($package);
        $response = ihttp_request("https://qra.95516.com/pay/gateway", $dat);
        ...
        $txt =json_encode($package);
        fwrite($myfile, $txt);
        fclose($myfile);
        
        $myfile = fopen("/www/wwwroot/w.itapgo.com/data/newfile7.txt", "w") or die("Unable to open file!");
        $txt =$response;
        fwrite($myfile, $txt);
        fclose($myfile);
        ...
        $xml=json_decode($xml->pay_info,true);
  
        $Opt["appId"] = $xml['appId'];
        $Opt["timeStamp"] = $xml['timeStamp'];
        $Opt["nonceStr"] = $xml['nonceStr'];
        $Opt["package"] = $xml['package'];
        $Opt["signType"] = $xml['signType'];
        $Opt['paySign'] = $xml['paySign'];
        pdo_update('tapgo_tt_order', array("ordersn3"=>$tid), array('ordersn'=>trim($params["tid"])));
    ...
    public function wechat_child_build($params, $wechat, $type = 0)
    {
        ...
        $package["nonce_str"] = random(32);
        $package["body"] = trim($params["title"]);
        $package["device_info"] = (isset($params["device_info"]) ? "tapgo_ttv2:" . $params["device_info"] : "tapgo_ttv2");
        ...
        $package["notify_url"] = (empty($params["notify_url"]) ? $_W["siteroot"] . "addons/tapgo_ttv2/payment/wechat/notify.php" : $params["notify_url"]);
        $package["trade_type"] = "JSAPI";
    ...
    public function sendredpack($params)
    {
        ...
        {
            $payment = pdo_fetch("SELECT * FROM " . tablename("tapgo_tt_payment") . " WHERE uniacid=:uniacid AND id=:id", array( ":uniacid" => $_W["uniacid"], ":id" => $set["weixin_id"] ));
            if( empty($payment) ) 
            {
                error(-1, "支付参数不存在!");
            }
        ...
        {
            $payment = pdo_fetch("SELECT * FROM " . tablename("tapgo_tt_payment") . " WHERE uniacid=:uniacid AND `type`='0'", array( ":uniacid" => $_W["uniacid"] ));
        }
    ...
    public function createNO($table, $field, $prefix)
    {
        ...
        while( 1 ) 
        {
            $count = pdo_fetchcolumn("select count(*) from " . tablename("tapgo_tt_" . $table) . " where " . $field . "=:billno limit 1", array( ":billno" => $billno ));
            if( $count <= 0 ) 
            {
                break;
            }
    ...
    public function getSec($uniacid = 0)
    {
        ...
        $set = pdo_fetch("select sec from " . tablename("tapgo_tt_sysset") . " where uniacid=:uniacid limit 1", array( ":uniacid" => $uniacid ));
        if( empty($set) ) 
        {
            $set = array(  );
        }
        ...
        if( 0 < $data["alipay_id"] ) 
        {
            $paymentalis = pdo_fetch("SELECT alipay_sec FROM " . tablename("tapgo_tt_payment") . " WHERE id = :id and uniacid=:uniacid and paytype = 1 ", array( ":uniacid" => $_W["uniacid"], ":id" => $data["alipay_id"] ));
    ...
    public function paylog($log = "")
    {
        ...
        if( !empty($openpaylog) ) 
        {
            $path = IA_ROOT . "/addons/tapgo_ttv2/data/paylog/" . $_W["uniacid"] . "/" . date("Ymd");
            if( !is_dir($path) ) 
            {
                load()->func("file");
                @mkdirs($path, "0777");
    ...
    public function getAreas()
    {
        ...
        if( !empty($new_area) ) 
        {
            $file = IA_ROOT . "/addons/tapgo_ttv2/static/js/dist/area/AreaNew.xml";
        }
        else
        {
            $file = IA_ROOT . "/addons/tapgo_ttv2/static/js/dist/area/Area.xml";
        }
    ...
    {
        return pdo_fetchall("SELECT  a.uniacid,a.name FROM " . tablename("tapgo_tt_sysset") . " s  " . " left join " . tablename("account_wechats") . " a on a.uniacid = s.uniacid" . " left join " . tablename("account") . " acc on acc.uniacid = a.uniacid where acc.isdeleted=0 GROUP BY uniacid");
    }
    ...
    public function getCopyright($ismanage = false)
    {
        ...
        {
            $copyrights = pdo_fetchall("select *  from " . tablename("tapgo_tt_system_copyright"));
        ...
        if( !empty($merchid) ) 
        {
            $merch = pdo_fetch("select logo from " . tablename("tapgo_tt_merch_user") . " where id = :id and uniacid = :uniacid ", array( ":id" => $merchid, ":uniacid" => $_W["uniacid"] ));
            if( !empty($merch["logo"]) ) 
            {
