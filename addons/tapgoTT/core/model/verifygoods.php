...
class Verifygoods_TapgoTtModel
{
    /**
     *
     * @param type $orderid
    ...
    {
        global $_W;
        $verifygoods = pdo_fetchall("select  *   from  " . tablename("tapgo_tt_verifygoods") . " where  orderid=:orderid ", array( ":orderid" => $orderid ));
        ...
        if( empty($goods["verifygoodstype"]) ) 
        {
            foreach( $ordergoods as $ordergood ) 
            {
                ...
                for( $i = 0; $i < $total; $i++ ) 
                {
                    ...
                    pdo_insert("tapgo_tt_verifygoods", $data);
                }
            }
        }
        else
    ...
    public function search($verifycode)
    {
        global $_W;
        global $_GPC;
        $verifygood = pdo_fetch("select *  from " . tablename("tapgo_tt_verifygoods") . " where uniacid=:uniacid and  verifycode=:verifycode and invalid =0  limit 1 ", array( ":uniacid" => $_W["uniacid"], ":verifycode" => $verifycode ));
        if( empty($verifygood) ) 
        {
        ...
        $saler = pdo_fetch("select * from " . tablename("tapgo_tt_saler") . " where  status=1  and  openid=:openid and uniacid=:uniacid limit 1", array( ":uniacid" => $_W["uniacid"], ":openid" => $_W["openid"] ));
        if( empty($saler) ) 
        {
    ...
    public function allow($verifycode, $times)
    {
        ...
        $order = pdo_fetch("select * from " . tablename("tapgo_tt_order") . " where id=:id and uniacid=:uniacid  limit 1", array( ":id" => $verifygood["orderid"], ":uniacid" => $verifygood["uniacid"] ));
        if( empty($order) ) 
        {
        ...
        {
            $verifygoodlogs = pdo_fetchall("select *  from " . tablename("tapgo_tt_verifygoods_log") . "    where verifygoodsid =:id  ", array( ":id" => $verifygood["id"] ));
            $verifynum = 0;
    ...
    public function complete($verifycode, $times, $remarks)
    {
        ...
        global $_GPC;
        $redis = redis();
        $redisKey = "self_commission_task_stage_" . $_W["uniacid"] . "_" . $verifycode;
        $existsKey = $redis->get($redisKey);
        if( $existsKey ) 
        {
            return error("操作频繁，请重试");
        }

        $redis->setex($redisKey, 10, $verifycode);
        ...
        pdo_insert("tapgo_tt_verifygoods_log", $data);
        $logid = pdo_insertid();
        m("notice")->sendVerifygoodMessage($logid);
        pdo_query("update " . tablename("tapgo_tt_verifygoods") . " set used=:used , verifycode=null ,codeinvalidtime=0 where id=:id", array( ":id" => $verifygood["id"], ":used" => $used ));
        ...
        if( $isonlyverifygood ) 
        {
            $status = pdo_fetchcolumn("select status  from " . tablename("tapgo_tt_order") . " where uniacid=:uniacid and id=:id  limit 1 ", array( ":uniacid" => $_W["uniacid"], ":id" => $verifygood["orderid"] ));
            $verifytypexzx = pdo_fetchall("select verifytype  from " . tablename("tapgo_tt_order") . " where uniacid=:uniacid and id=:id  limit 1 ", array( ":uniacid" => $_W["uniacid"], ":id" => $verifygood["orderid"] ));
    ...
    public function finishorder($id, $arr = array(  ))
    {
        ...
        pdo_update("tapgo_tt_order", array( "status" => 3, "sendtime" => time(), "finishtime" => time(), "verifyinfo" => iserializer($verifyinfo) ), array( "id" => $item["id"], "uniacid" => $_W["uniacid"] ));
        if( $item["status"] == 2 ) 
        {
    ...
    public function verifyLog($data, $type = 0)
    {
        ...
        $result["type"] = $type;
        $ret = pdo_insert("tapgo_tt_saler_verify_log", $result);
        if( empty($ret) ) 
        {
