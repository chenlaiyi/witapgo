...
class Order_TapgoTtModel
{
    ...
    function fullback($orderid)
    {
        global $_W;
        $uniacid = $_W['uniacid'];
        $order_goods = pdo_fetchall("select o.openid,og.optionid,og.goodsid,og.price,og.total,og.id from " . tablename("tapgo_tt_order_goods") . " as og
                    left join " . tablename('tapgo_tt_order') . " as o on og.orderid = o.id
                    where og.uniacid = " . $uniacid . " and og.orderid = " . $orderid . " ");
        /*$order = pdo_fetch('select o.id,o.ordersn,o.openid,og.optionid,og.goodsid,og.price from ' . tablename('tapgo_tt_order') . ' as o
                left join '.tablename('tapgo_tt_order_goods').' as og on og.orderid = o.id
                where  o.id=:id and o.uniacid=:uniacid limit 1', array(':uniacid' => $uniacid, ':id' => $orderid));*/
        foreach ($order_goods as $key => $value) {
            if ($value['optionid'] > 0) {
                $goods = pdo_fetch("select g.hasoption,g.id,go.goodsid,go.isfullback from " . tablename('tapgo_tt_goods') . ' as g
                left join ' . tablename('tapgo_tt_goods_option') . ' as go on go.goodsid = :id and go.id = ' . $value['optionid'] . '
                 where g.id=:id and g.uniacid=:uniacid limit 1'
                    , array(':id' => $value['goodsid'], ':uniacid' => $uniacid));
            } else {
                $goods = pdo_fetch("select * from " . tablename('tapgo_tt_goods') . ' where id=:id and uniacid=:uniacid limit 1'
                    , array(':id' => $value['goodsid'], ':uniacid' => $uniacid));
            }
            if ($goods['isfullback'] > 0) {
                $fullbackgoods = pdo_fetch("SELECT id,minallfullbackallprice,maxallfullbackallprice,minallfullbackallratio,maxallfullbackallratio,`day`,
                          ...
                          FROM " . tablename('tapgo_tt_fullback_goods') . " WHERE uniacid = " . $uniacid . " and status=1 and goodsid = " . $value['goodsid'] . " limit 1");
                //return $fullbackgoods;
                $condition = '';
                if (!empty($fullbackgoods) && $goods['hasoption'] && $value['optionid'] > 0) {
                    $option = pdo_fetch('select id,title,allfullbackprice,allfullbackratio,fullbackprice,fullbackratio,`day` from ' . tablename('tapgo_tt_goods_option') . ' 
                ...
                if (!empty($fullbackgoods)) {
                    ...
                    $has_record = pdo_fetch("select id from " . tablename('tapgo_tt_fullback_log') . " where {$condition}", $params);
                    //防止重复记录数据
                    if (empty($has_record)) {
                        pdo_insert('tapgo_tt_fullback_log', $data);
                        $fullbackid = pdo_insertid();
                        pdo_update('tapgo_tt_order_goods',array('fullbackid'=>$fullbackid),array('id'=>$value['id'],'uniacid'=>$_W['uniacid']));
                    }
                }
            }
    ...
    function fullbackstop($orderid)
    {
        ...
        $fullback_log = pdo_fetch("select * from " . tablename('tapgo_tt_fullback_log') . " where uniacid = " . $uniacid . " and orderid = " . $orderid . " ");
        ...
        pdo_update('tapgo_tt_fullback_log', array('isfullback' => 1), array('id' => $fullback_log['id'], 'uniacid' => $uniacid));
    ...
    public function payResult($params)
    {
        ...
        $order = pdo_fetch('select id,uniacid,ordersn, price,openid,dispatchtype,addressid,carrier,status,isverify,deductcredit2,`virtual`,isvirtual,couponid,isvirtualsend,isparent,paytype,merchid,agentid,createtime,buyagainprice,istrade,tradestatus,iscycelbuy from ' . tablename('tapgo_tt_order') . ' where  ordersn=:ordersn and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':ordersn' => $ordersn));
        ...
        if (!empty($ispeerpay)) {
            $peerpay_info = (float)pdo_fetchcolumn("select SUM(price) price from " . tablename('tapgo_tt_order_peerpay_payinfo') . ' where pid=:pid limit 1', array(':pid' => $ispeerpay['id']));
            if ($peerpay_info < $ispeerpay['peerpay_realprice']) {
                return;
            }
            pdo_update('tapgo_tt_order', array('status' => 0), array('id' => $order['id']));
            $order['status'] = 0;
            $result = pdo_update('tapgo_tt_order_peerpay', array('status' => 1), array('id' => $ispeerpay['id']));
        ...
        if ($params['from'] == 'return') {
            ...
            if (empty($order['dispatchtype'])) {
                $address = pdo_fetch('select realname,mobile,address from ' . tablename('tapgo_tt_member_address') . ' where id=:id limit 1', array(':id' => $order['addressid']));
            }
            ...
            if ($params['type'] == 'cash') {
                ...
                if ($order['isparent'] == 1) {
                    ...
                    //订单状态
                    pdo_update('tapgo_tt_order', $change_data, array('id' => $order['id']));

                    //处理子订单状态
                    $this->setChildOrderPayResult($order, 0, 0);

                    //小票打印
                    $merchSql = 'SELECT id,merchid FROM ' . tablename('tapgo_tt_order') . ' WHERE uniacid = ' . intval($order['uniacid']) . ' AND parentid = ' . intval($order['id']);
                    $merchData = pdo_fetchall($merchSql);
                    foreach ($merchData as $mk => $mv) {
                        //打印机打印
                        file_put_contents(TAPGO_TT_DATA . 'print1.json',1111);
...
//                                }
    ...
    function setChildOrderPayResult($order, $time, $type)
    {
        ...
        if (!empty($list)) {
            ...
            foreach ($list as $k => $v) {
                //订单状态
                if ($v['status'] == 0) {
                    pdo_update('tapgo_tt_order', $change_data, array('id' => $v['id']));
                }
            }
        }
    }
    ...
    function setOrderPayType($orderid, $paytype, $ordersn = '')
    {
        ...
        pdo_update('tapgo_tt_order', $change_data, array('id' => $orderid));
        $peerpay = pdo_fetch('select id from ' . tablename('tapgo_tt_order_peerpay') . ' where orderid=:orderid and uniacid=:uniacid', array('orderid' => $orderid, 'uniacid' => $_W['uniacid']));
        if (!empty($orderid)) {
            pdo_update('tapgo_tt_order', array('paytype' => $paytype), array('parentid' => $orderid));
            pdo_update('tapgo_tt_order_peerpay_payinfo', array('paytype' => $paytype), array('pid' => $peerpay['id'], 'openid' => $_W['openid']));
        }
    }
    ...
    function getChildOrder($orderid)
    {
        ...
        $list = pdo_fetchall('select id,ordersn,status,finishtime,couponid,merchid  from ' . tablename('tapgo_tt_order') . ' where  parentid=:parentid and uniacid=:uniacid', array(':parentid' => $orderid, ':uniacid' => $_W['uniacid']));
        return $list;
    }
    ...
    function payVirtualSend($orderid = 0, $ispeerpay = false)
    {
        ...
        $order = pdo_fetch('select id,uniacid,ordersn, price,openid,dispatchtype,addressid,carrier,status,isverify,deductcredit2,`virtual`,isvirtual,couponid,isvirtualsend,isparent,paytype,merchid,agentid,createtime,buyagainprice,istrade,tradestatus,iscycelbuy, wxapp_allow_subscribe from ' . tablename('tapgo_tt_order') . ' where  id=:id and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $orderid));
        $order_goods = pdo_fetch("select g.virtualsend,g.virtualsendcontent from " . tablename('tapgo_tt_order_goods') . " og "
            . " left join " . tablename('tapgo_tt_goods') . " g on g.id=og.goodsid "
            . " where og.orderid=:orderid and og.uniacid=:uniacid limit 1", array(':uniacid' => $order['uniacid'], ':orderid' => $orderid));
        $time = time();
        //自动完成
        pdo_update('tapgo_tt_order', array('virtualsend_info' => $order_goods['virtualsendcontent'], 'status' => '3', 'paytime' => $time, 'sendtime' => $time, 'finishtime' => $time), array('id' => $orderid));
        ...
        if (!empty($order['wxapp_allow_subscribe'])) {
            ...
            if (in_array('autosend', $template)) {
                ...
                $goods = pdo_fetchall("select og.goodsid,og.price,g.title,g.thumb,og.total,g.credit,og.optionid,og.optionname as optiontitle,g.isverify,g.storeids, og.realprice from " . tablename('tapgo_tt_order_goods') . " og "
                    . " left join " . tablename('tapgo_tt_goods') . " g on g.id=og.goodsid "
                ...
                if (p('app')) {
                    if (strexists('sns_wa', $order['openid'])) {
                        ...
                        $openid = pdo_fetchcolumn("select openid_wa from " . tablename('tapgo_tt_member') . " where openid='{$order['openid']}'");
                    }
    ...
    function afterPayResult($order, $ispeerpay = false)
    {
        ...
        if (p('task')) {
            ...
            $goodslist = pdo_fetchall("SELECT goodsid FROM " . tablename('tapgo_tt_order_goods') . " WHERE orderid = :orderid AND uniacid = :uniacid", array(':orderid' => $order['id'], ':uniacid' => $order['uniacid']));
...
//            p('task')->checkTaskProgress($order['price'],'order_full','',$order['openid']);//??这个需要移动到确认收货
            ...
            //首次购物
            if (pdo_fetchcolumn("select count(*) from " . tablename('tapgo_tt_order') . " where openid = '{$order['openid']}' and uniacid = {$order['uniacid']}") == 1) {
    ...
    {
        global $_W;
        $order = pdo_fetch('select id,ordersn,price,openid,dispatchtype,addressid,carrier,status from ' . tablename('tapgo_tt_order') . ' where id=:id limit 1', array(':id' => $orderid));
        $goods = pdo_fetchall("select og.goodsid,og.total,g.totalcnf,og.realprice,g.money,og.optionid,g.total as goodstotal,og.optionid,g.sales,g.salesreal from " . tablename('tapgo_tt_order_goods') . " og "
            . " left join " . tablename('tapgo_tt_goods') . " g on g.id=og.goodsid "
    ...
    function setStocksAndCredits($orderid = '', $type = 0, $flag = false)
    {
        ...
        $tempflag = false;
        $order = pdo_fetch('select id,ordersn,price,openid,dispatchtype,addressid,carrier,status,isparent,paytype,isnewstore,storeid,istrade,status from ' . tablename('tapgo_tt_order') . ' where id=:id limit 1', array(':id' => $orderid));
        ...
        $goods = pdo_fetchall("select og.goodsid,og.seckill,og.total,g.totalcnf,og.realprice,g.credit,og.optionid,g.total as goodstotal,og.optionid,g.sales,g.salesreal,g.type from " . tablename('tapgo_tt_order_goods') . " og "
            . " left join " . tablename('tapgo_tt_goods') . " g on g.id=og.goodsid "
        ...
        foreach ($goods as $g) {
            ...
            if ($newstoreid > 0) {
                ...
                $goods_item = pdo_fetch("select total as goodstotal from" . tablename('tapgo_tt_goods') . " where id=:id and uniacid=:uniacid limit 1", array(":id" => $g['goodsid'], ':uniacid' => $_W['uniacid']));
            ...
            if (!empty($stocktype)) {
                ...
                if (!empty($g['optionid'])) {
                    ...
                    if (!empty($option) && $option['stock'] != -1) {
                        ...
                        if ($stock != -1) {
                            ...
                            if ($newstoreid > 0) {
                                pdo_update('tapgo_tt_newstore_goods_option', array('stock' => $stock), array('uniacid' => $_W['uniacid'], 'goodsid' => $g['goodsid'], 'id' => $store_goods_option['id']));
                            } else {
                                pdo_update('tapgo_tt_goods_option', array('stock' => $stock), array('uniacid' => $_W['uniacid'], 'goodsid' => $g['goodsid'], 'id' => $g['optionid']));
                            }
                        }
                    }
                }
                if (((!empty($g['goodstotal']) && $stocktype == -1) || $stocktype == 1) && $g['goodstotal'] != -1) {
                    ...
                    if ($totalstock != -1) {
                        ...
                        if ($newstoreid > 0) {
                            pdo_update('tapgo_tt_newstore_goods', array('stotal' => $totalstock), array('uniacid' => $_W['uniacid'], 'id' => $store_goods['id']));
                        } else {
                            pdo_update('tapgo_tt_goods', array('total' => $totalstock), array('uniacid' => $_W['uniacid'], 'id' => $g['goodsid']));
                        }
                    }
                }
            }
...
//                if ($g['totalcnf'] != 1) {
//                    pdo_update('tapgo_tt_goods', array('sales' => $g['sales'] + $g['total']), array('uniacid' => $_W['uniacid'], 'id' => $g['goodsid']));
//                }
            } elseif ($type == 1) {
                //真实销量付款才加
                if ($order['status'] >= 1) {
...
//                    }
                    //实际销量
                    $salesreal = pdo_fetchcolumn('select ifnull(sum(total),0) from ' . tablename('tapgo_tt_order_goods') . ' og '
                        . ' left join ' . tablename('tapgo_tt_order') . ' o on o.id = og.orderid '
                        . ' where og.goodsid=:goodsid and o.status>=1 and o.uniacid=:uniacid limit 1', array(':goodsid' => $g['goodsid'], ':uniacid' => $_W['uniacid']));
                    pdo_update('tapgo_tt_goods', array('salesreal' => $salesreal), array('id' => $g['goodsid']));

                    //支付成功,如果商品设置了送积分营销活动,则增加一条记录
                    $table_flag = pdo_tableexists('tapgo_tt_order_buysend');
                    if ($credits > 0 && $table_flag) {
                        ...
                        $send_record = pdo_fetch('SELECT * FROM ' . tablename('tapgo_tt_order_buysend') . ' WHERE orderid = :orderid AND uniacid = :uniacid AND openid = :openid', array(':orderid' => $orderid, ':uniacid' => $_W['uniacid'], ':openid' => $order['openid']));
                        if ($send_record) {
                            pdo_update('tapgo_tt_order_buysend', $send_data, array('id' => $send_record['id']));
                        } else {
                            pdo_insert('tapgo_tt_order_buysend', $send_data);
                        }
                    }
                }
            }
    ...
    function getTotals($merch = 0)
    {
        ...
        $totals['all'] = pdo_fetchcolumn(
            'SELECT COUNT(1) FROM ' . tablename('tapgo_tt_order') . ""
    ...
    function getGoodsDiscountPrice($g, $level, $type = 0, $memberCardId = 0)
    {
        ...
        // 判断会员等级状态
        if (!empty($level['id'])) {
            $level = pdo_fetch('select * from ' . tablename('tapgo_tt_member_level') . ' where id=:id and uniacid=:uniacid and enabled=1 limit 1', array(':id' => $level['id'], ':uniacid' => $_W['uniacid']));
            ...
            if (is_array($discounts)) {
                ...
                if (!isset($discounts['type']) || empty($discounts['type'])) {
                    ...
                    if (!empty($discounts[$key])) {
                        ...
                        $dd = floatval(intval($discounts[$key . '_pay']) * intval($total)); //设置的会员折扣
...
//            $g['proportion'] = round($g['ggprice'] / $grprice, 2);
        ...
        if ($is_exchange) {//兑换中心
            if (is_array($postage)) {//按件计算运费
                ...
                pdo_update('tapgo_tt_order', array('dispatchprice' => $order['dispatchprice'],
...
//                if(empty($address['lng']) || empty($address['lat'])){
                ...
                if ($data['status'] == 1 && $data['count'] > 0) {
                    ...
                    pdo_update('tapgo_tt_member_address', $addres, array('id' => $addres['id'], 'uniacid' => $_W['uniacid']));
...
//                }
    ...
    function changeParentOrderPrice($parent_order)
    {
        ...
        $id = $parent_order['id'];
        $item = pdo_fetch("SELECT price,ordersn2,dispatchprice,changedispatchprice FROM " . tablename('tapgo_tt_order') . " WHERE id = :id and uniacid=:uniacid", array(':id' => $id, ':uniacid' => $_W['uniacid']));
        ...
        if (!empty($item)) {
            ...
            if (!empty($orderupdate)) {
                pdo_update('tapgo_tt_order', $orderupdate, array('id' => $id, 'uniacid' => $_W['uniacid']));
            }
        }
    }
    ...
    function getOrderCommission($orderid, $agentid = 0)
    {
        ...
        if (empty($agentid)) {
            $item = pdo_fetch('select agentid from ' . tablename('tapgo_tt_order') . ' where id=:id and uniacid=:uniacid Limit 1', array('id' => $orderid, ':uniacid' => $_W['uniacid']));
            if (!empty($item)) {
    ...
    function checkOrderGoods($orderid)
    {
        ...
        $item = pdo_fetch("select * from " . tablename('tapgo_tt_order') . "  where  id = :id and uniacid=:uniacid limit 1", array(":id" => $orderid, ":uniacid" => $uniacid));

        if ((empty($order['isnewstore']) || empty($order['storeid'])) && empty($order['istrade'])) {

            $order_goods = pdo_fetchall('select og.id,g.title, og.goodsid,og.optionid,g.total as stock,og.total as buycount,g.status,g.deleted,g.maxbuy,g.usermaxbuy,g.istime,g.timestart,g.timeend,g.buylevels,g.buygroups,g.totalcnf,og.seckill from  ' . tablename('tapgo_tt_order_goods') . ' og '
                . ' left join ' . tablename('tapgo_tt_goods') . ' g on og.goodsid = g.id '
                . ' where og.orderid=:orderid and og.uniacid=:uniacid ', array(':uniacid' => $_W['uniacid'], ':orderid' => $orderid));
                ...
                if ($seckillinfo && $seckillinfo['status'] == 0 || !empty($ispeerpay)) {
                    ...
                    if ($data['totalcnf'] == 1) {
                        if (!empty($data['optionid'])) {
                            $option = pdo_fetch('select id,title,marketprice,goodssn,productsn,stock,`virtual` from ' . tablename('tapgo_tt_goods_option') . ' where id=:id and goodsid=:goodsid and uniacid=:uniacid  limit 1', array(':uniacid' => $uniacid, ':goodsid' => $data['goodsid'], ':id' => $data['optionid']));
                            if (!empty($option)) {
                                if ($option['stock'] != -1) {
            ...
            if (p('newstore')) {
                $sql = "select g.id,g.title,ng.gstatus,g.deleted"
                    . " from " . tablename('tapgo_tt_order_goods') . " og left join  " . tablename('tapgo_tt_goods') . " g  on g.id=og.goodsid and g.uniacid=og.uniacid"
                    . " inner join " . tablename('tapgo_tt_newstore_goods') . " ng on ng.goodsid = g.id AND ng.storeid=" . $item['storeid']
    ...
    {//查询是否是代付订单,如果不是返回false,如果是返回代付订单内容
        global $_W;
        $sql = "SELECT p.*,o.openid FROM " . tablename('tapgo_tt_order_peerpay') . " AS p JOIN " . tablename('tapgo_tt_order') . " AS o ON p.orderid = o.id WHERE p.orderid = :orderid  AND (p.status = 0 OR p.status=1) AND o.status >= 0 LIMIT 1";
        $query = pdo_fetch($sql, array(':orderid' => $orderid));
        return $query;
    }
    ...
    public function peerStatus($param)
    {
        global $_W;
        if (!empty($param['tid'])) {
            $sql = "SELECT id FROM " . tablename('tapgo_tt_order_peerpay_payinfo') . " WHERE tid = :tid";
            $id = pdo_fetchcolumn($sql, array(':tid' => $param['tid']));
            if ($id) return $id;
        }
        pdo_insert('tapgo_tt_order_peerpay_payinfo', $param);
    ...
    {
        global $_W;
        $num = pdo_fetchcolumn('select SUM(og.total)  from ' . tablename('tapgo_tt_order_goods') . ' og
		 inner join ' . tablename('tapgo_tt_goods') . ' g on og.goodsid = g.id
    ...
    {
        global $_W;
        $num = pdo_fetchcolumn('select COUNT(1)  from ' . tablename('tapgo_tt_order_goods') . ' og
		 inner join ' . tablename('tapgo_tt_goods') . ' g on og.goodsid = g.id
        ...
        if ($num > 0) {
            ...
            $num2 = pdo_fetchcolumn('select COUNT(1)  from ' . tablename('tapgo_tt_order_goods') . ' og
             inner join ' . tablename('tapgo_tt_goods') . ' g on og.goodsid = g.id
             where og.uniacid=:uniacid  and og.orderid =:orderid and g.type=5', array(':uniacid' => $_W['uniacid'], ':orderid' => $orderid));
            $num2 = intval($num2);
    ...
    {
        global $_W;
        $num = pdo_fetchcolumn('select COUNT(1)  from ' . tablename('tapgo_tt_verifygoods_log') . ' vl
		 inner join ' . tablename('tapgo_tt_verifygoods') . ' v on vl.verifygoodsid = v.id
    ...
    public function getOrderVirtual($order = array())
    {
        ...
        $ordervirtual = array();
        $virtual_type = pdo_fetch('select fields from ' . tablename('tapgo_tt_virtual_type') . ' where id=:id and uniacid=:uniacid and merchid = :merchid limit 1 ', array(':id' => $order['virtual'], ':uniacid' => $_W['uniacid'], ':merchid' => $order['merchid']));
...
//        $url = 'http://newopen.qa.imdada.cn/api/order/addOrder';
        ...
        $cityexpress = pdo_fetch("SELECT * FROM " . tablename('tapgo_tt_city_express') . " WHERE uniacid=:uniacid AND merchid=:merchid", array(":uniacid" => $_W['uniacid'], ":merchid" => 0));
    ...
    function CheckoodsStock($orderid = '', $type = 0, $flg = true)
    {
        ...
        $goods = pdo_fetchall("select og.goodsid,og.total,g.totalcnf,og.realprice,g.credit,og.optionid,g.total as goodstotal,og.optionid,g.sales,g.salesreal,g.type from " . tablename('tapgo_tt_order_goods') . " og "
            . " left join " . tablename('tapgo_tt_goods') . " g on g.id=og.goodsid "
            . " where $condition and og.uniacid=:uniacid ", $param);
