		...
		if (function_exists('redis')) {
			...
			if (!is_error($redis)) {
				if (stripos($uniacid, 'global') !== false) {
					//return 'tapgo_ttv2_syscache_' . $_W['setting']['site']['key'] . '_global_' . $key;
				}
				...
				//return 'tapgo_ttv2_syscache_' . $_W['setting']['site']['key'] . '_' . $uniacid . '_' . $account_key . '_' . $key;
			}
		}

		return TAPGO_TT_PREFIX . md5($uniacid . '_new_' . $key);
	}
