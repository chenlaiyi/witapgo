<?php
/*WEMECMS  QQ4424986*/
class Finance_TapgoTtModel
{
    
    
    public function unipay($openid = '', $paytype = 0, $money = 0, $trade_no = '', $desc = '', $return = true)
	{
		global $_W;
		global $_GPC;
		...
		if (!empty($set['weixin_id'])) {
			...
			$payment = pdo_fetch('SELECT * FROM ' . tablename('tapgo_tt_payment') . ' WHERE uniacid=:uniacid AND  type=0  AND paytype=0 AND sub_appid=:appid', array(':uniacid' => $_W['uniacid'], ':appid' => $_W['account']['key']));
		}
        ...
        $order = pdo_get('tapgo_tt_order', array('ordersn' => $trade_no));
        
		$url = 'https://qra.95516.com/pay/gateway';
		load()->func("communication");
        $Opt = array(  );
        $package = array(  );
        $package["service"] = "unified.trade.refund";
        $package["mch_id"] = trim($unionpay["mch_id"]);
        $package["op_user_id"] = trim($unionpay["mch_id"]);
        $package["out_trade_no"] = $desc;
        $package["out_refund_no"] = $order['ordersn3'];
        
        $package["total_fee"] = $money;
        $package["refund_fee"] = $money;
	...
	public function pay($openid = '', $paytype = 0, $money = 0, $trade_no = '', $desc = '', $return = true)
	{
		...
		$xml = array2xml($pars);
		$extras = array();
		$errmsg = '未上传完整的微信支付证书q，请到【系统设置】->【支付方式】中上传!';
	...
	public function refund($openid, $out_trade_no, $out_refund_no, $totalmoney, $refundmoney = 0, $app = false, $refund_account = false, $normal = false)
	{
		...
		$xml = array2xml($pars);
		$extras = array();
		$errmsg = '未上传完整的微信支付证书2，请到【系统设置】->【支付方式】中上传!';
	...
	public function wxapp_refund($openid, $out_trade_no, $out_refund_no, $totalmoney, $refundmoney = 0, $app = false, $refund_account = false)
	{
		...
		if (is_array($certs)) {
			...
			$certfile = IA_ROOT . '/addons/tapgo_ttv2/cert/' . random(64);
			file_put_contents($certfile, $certs['cert']);
			$keyfile = IA_ROOT . '/addons/tapgo_ttv2/cert/' . random(64);
			file_put_contents($keyfile, $certs['key']);
			$extras['CURLOPT_SSLCERT'] = $certfile;
			$extras['CURLOPT_SSLKEY'] = $keyfile;

			if (!empty($certs['root'])) {
				$rootfile = IA_ROOT . '/addons/tapgo_ttv2/cert/' . random(64);
				file_put_contents($rootfile, $certs['root']);
				$extras['CURLOPT_CAINFO'] = $rootfile;
			}
		}
	...
	public function AlipayRefund($params, $batch_no, $config)
	{
		...
		$parameter = array('service' => 'refund_fastpay_by_platform_pwd', 'partner' => $config['partner'], '_input_charset' => 'UTF-8', 'notify_url' => isset($params['notify_url']) ? $params['notify_url'] : $_W['siteroot'] . 'addons/tapgo_ttv2/payment/alipay/notify.php', 'seller_user_id' => $config['partner'], 'seller_email' => $config['account'], 'refund_date' => date('Y-m-d H:i:s'), 'batch_no' => $batch_no, 'batch_num' => '1', 'detail_data' => $params['trade_no'] . '^' . $params['refund_price'] . '^' . $params['refund_reason']);
