	...
	protected function orderData($status, $st, $index)
	{
		...
		if (!empty($_GPC['searchfield']) && !empty($_GPC['keyword'])) {
			...
			if ($searchfield == 'ordersn') {
				...
				if (p('merch')) {
					$sqlcondition = ' left join ' . tablename('tapgo_tt_store') . ' store on store.id = o.verifystoreid and store.uniacid=o.uniacid left join ' . tablename('tapgo_tt_merch_store') . ' merstore on merstore.id = o.verifystoreid and merstore.uniacid=o.uniacid';
		...
		if ($condition != ' o.uniacid = :uniacid and o.ismr=0 and o.deleted=0 and o.isparent=0 and o.istrade=0 ' || !empty($sqlcondition)) {
			if ($searchfield == 'member') {
				...
				$sql = 'select o.* ,sm.groupid, a.realname as arealname,a.mobile as amobile,a.province as aprovince ,a.city as acity , a.area as aarea, a.street as astreet,a.address as aaddress,
                      d.dispatchname,
                      r.rtype,r.status as rstatus,o.sendtype,o.city_express_state from ' . tablename('tapgo_tt_order') . ' o' . ' left join ' . tablename('tapgo_tt_order_refund') . ' r on r.id =o.refundid ' . ' left join ' . tablename('tapgo_tt_member_address') . ' a on a.id=o.addressid ' . ' left join ' . tablename('tapgo_tt_dispatch') . ' d on d.id = o.dispatchid ' . ' left join ' . tablename('tapgo_tt_saler') . ' s on s.openid = o.verifyopenid and s.uniacid=o.uniacid' . ' left join ' . tablename('tapgo_tt_member') . ' sm on sm.openid = s.openid and sm.uniacid=s.uniacid' . (' ' . $sqlcondition . ' where ' . $condition . ' ' . $statuscondition . ' ORDER BY ' . $orderbuy . ' DESC  ');
			}
			else if ($searchfield == 'mid') {
				$paras[':keyword'] = htmlspecialchars_decode($_GPC['keyword'], ENT_QUOTES);
				$condition .= ' AND (m.id=:keyword)';
		...
		if (!empty($list)) {
			...
			foreach ($list as &$value) {
				...
				if (!empty($value['refundid']) && !empty($_GPC['export'])) {
					$refund_apply = pdo_fetch('select id,status,applyprice from ' . tablename('tapgo_tt_order_refund') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $value['refundid'], ':uniacid' => $_W['uniacid']));
		...
		if ($_GPC['export'] == 1) {
			...
			foreach ($list as &$row) {
				...
				if (!empty($row['verifystoreid'])) {
					if (0 < $row['merchid']) {
						$row['storeinfo'] = pdo_fetchcolumn('select storename from ' . tablename('tapgo_tt_merch_store') . ' where id=:storeid limit 1 ', array(':storeid' => $row['verifystoreid']));
					}
					else {
						$row['storeinfo'] = pdo_fetchcolumn('select storename from ' . tablename('tapgo_tt_store') . ' where id=:storeid limit 1 ', array(':storeid' => $row['verifystoreid']));
					}
				}
				else {
					if (!empty($row['verifyinfo'])) {
				...
				if (!empty($row['storeid'])) {
					if (0 < $row['merchid']) {
						$row['pickname'] = pdo_fetchcolumn('select storename from ' . tablename('tapgo_tt_merch_store') . ' where id=:storeid limit 1 ', array(':storeid' => $row['storeid']));
					}
					else {
						$row['pickname'] = pdo_fetchcolumn('select storename from ' . tablename('tapgo_tt_store') . ' where id=:storeid limit 1 ', array(':storeid' => $row['storeid']));
					}
				}
					...
					if (is_array($verifyinfo)) {
						if (empty($row['dispatchtype'])) {
							...
							if ($v['verified'] || $row['verifytype'] == 1) {
								$v['storename'] = pdo_fetchcolumn('select storename from ' . tablename('tapgo_tt_merch_store') . ' where id=:id limit 1', array(':id' => $v['verifystoreid']));
								...
								$v['salername'] = pdo_fetchcolumn('select salername from ' . tablename('tapgo_tt_merch_saler') . ' where openid=:openid and uniacid=:uniacid and merchid = :merchid limit 1', array(':openid' => $v['verifyopenid'], ':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid']));
		...
		if ($condition != ' o.uniacid = :uniacid and o.ismr=0 and o.deleted=0 and o.isparent=0' || !empty($sqlcondition)) {
			if ($searchfield == 'member') {
				...
				if (!empty($openidArr)) {
					...
					$orderPrice = pdo_fetch('SELECT count(1) as count,sum(price) as sumprice FROM ' . tablename('tapgo_tt_order') . ' o WHERE o.uniacid = ' . $_W['uniacid'] . ' AND o.deleted=0 AND o.isparent=0 AND o.openid IN (' . $inOpenid . ')' . $timeCondition . $statuscondition);
				}
				else {
				...
				$orderPrice = pdo_fetch('SELECT count(1) as count,sum(o.price) as sumprice FROM ' . tablename('tapgo_tt_order') . ' o left join ' . tablename('tapgo_tt_member_address') . ' a on o.addressid = a.id WHERE o.uniacid = ' . $_W['uniacid'] . '  AND o.deleted=0 AND o.isparent=0 ' . $priceCondition . $statuscondition);
				$totalmoney = $orderPrice['sumprice'];
				$total = $orderPrice['count'];
				...
				if ($status === '' && empty($_GPC['keyword']) && empty($_GPC['time'])) {
					...
					if (!is_error($redis)) {
						if (false && $redis->get($redis_key_total) != false && $redis->get($redis_key_totalmoney) != false) {
							...
							if (empty($agentid)) {
								$t = pdo_fetch('SELECT COUNT(DISTINCT(o.id)) as count, ifnull(sum(o.price),0) as sumprice   FROM ' . tablename('tapgo_tt_order') . ' o ' . ' left join ' . tablename('tapgo_tt_order_refund') . ' r on r.id =o.refundid ' . ' left join ' . tablename('tapgo_tt_saler') . ' s on s.openid = o.verifyopenid and s.uniacid=o.uniacid' . ' left join ' . tablename('tapgo_tt_member') . ' m on m.openid = s.openid and m.uniacid=s.uniacid' . (' ' . $sqlcondition . ' WHERE ' . $condition . ' ' . $statuscondition), $paras);
								$total = $t['count'];
					...
					$t = pdo_fetch('SELECT COUNT(DISTINCT(o.id)) as count, ifnull(sum(o.price),0) as sumprice   FROM ' . tablename('tapgo_tt_order') . ' o ' . ' left join ' . tablename('tapgo_tt_order_refund') . ' r on r.id =o.refundid ' . ' left join ' . tablename('tapgo_tt_saler') . ' s on s.openid = o.verifyopenid and s.uniacid=o.uniacid' . ' left join ' . tablename('tapgo_tt_member') . ' m on m.openid = o.openid and m.uniacid=o.uniacid' . (' ' . $sqlcondition . ' WHERE ' . $condition . ' ' . $statuscondition), $paras);
					$total = $t['count'];
					$totalmoney = $t['sumprice'];
				}
			}
