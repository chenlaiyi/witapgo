...
class Dispatch_TapgoTtPage extends WebPage
{
	...
	protected function post()
	{
		...
		if ($_W['ispost']) {
			...
			if ($data['isdefault']) {
				pdo_update('tapgo_tt_dispatch', array('isdefault' => 0), array('uniacid' => $_W['uniacid'], 'merchid' => 0));
			}

			if (!empty($id)) {
				plog('shop.dispatch.edit', '修改配送方式 ID: ' . $id);
				pdo_update('tapgo_tt_dispatch', $data, array('id' => $id));
			}
			else {
		...
		$dispatch = pdo_fetch('SELECT * FROM ' . tablename('tapgo_tt_dispatch') . (' WHERE id = \'' . $id . '\' and merchid=0 and uniacid = \'' . $_W['uniacid'] . '\''));
	...
	public function delete()
	{
		...
		$items = pdo_fetchall('SELECT id,dispatchname FROM ' . tablename('tapgo_tt_dispatch') . (' WHERE id in( ' . $id . ' ) AND merchid=0 AND uniacid=') . $_W['uniacid']);
	...
	public function displayorder()
	{
		...
		$item = pdo_fetchall('SELECT id,dispatchname FROM ' . tablename('tapgo_tt_dispatch') . (' WHERE id in( ' . $id . ' ) AND merchid=0 AND uniacid=') . $_W['uniacid']);
