...
class Log_TapgoTtPage extends WebPage
{
	protected function main($type = 0)
	{
		...
		$sql = 'select log.id,log.openid,log.logno,log.type,log.status,log.rechargetype,log.sendmoney,log.money,log.createtime,log.realmoney,log.deductionmoney,log.charge,log.remark,log.alipay,log.bankname,log.bankcard,log.realname as applyrealname,log.applytype,m.nickname,m.id as mid,m.avatar,m.level,m.groupid,m.realname,m.mobile,g.groupname,l.levelname from ' . tablename('tapgo_tt_member_log') . ' log ' . ' left join ' . tablename('tapgo_tt_member') . ' m on m.openid = log.openid ' . ' left join ' . tablename('tapgo_tt_member_group') . ' g on g.id = m.groupid ' . ' left join ' . tablename('tapgo_tt_member_level') . ' l on l.id = m.level ' . (' where 1 ' . $condition . ' ' . $condition1 . ' GROUP BY log.id ORDER BY log.createtime DESC ');
		...
		if ($condition1 != '') {
			$condition_member = pdo_fetchall('SELECT openid FROM ' . tablename('tapgo_tt_member') . ('m WHERE m.uniacid = :uniacid ' . $condition1), array(':uniacid' => $_W['uniacid'], ':keyword' => '%' . $_GPC['keyword'] . '%'));
			$condition_member2 = pdo_fetchall('SELECT CONCAT(\'sns_wa_\',openid_wa) FROM ' . tablename('tapgo_tt_member') . (' m WHERE m.uniacid = :uniacid ' . $condition1), array(':uniacid' => $_W['uniacid'], ':keyword' => '%' . $_GPC['keyword'] . '%'));
		...
		$total = pdo_fetchcolumn('select count(*) from ' . tablename('tapgo_tt_member_log') . ' log ' . (' where 1 ' . $condition . ' ' . $member_sql), $params);
