...
paytype,expresssn,refundstate,dispatchtype,verifyinfo,merchid,isparent,iscycelbuy,cycelbuy_periodic,userdeleted' . $s_string . '
 from ' . tablename('tapgo_tt_order') . (' where 1 ' . $condition . ' order by createtime desc LIMIT ') . ($pindex - 1) * $psize . ',' . $psize, $params);
		...
		foreach ($list as &$row) {
			...
			$sql = 'SELECT og.id,og.goodsid,og.total,g.title,g.thumb,g.type,g.status,og.price,og.title as gtitle,og.optionname as optiontitle,og.optionid,op.specs,g.merchid,og.seckill,og.seckill_taskid,
                ...
                FROM ' . tablename('tapgo_tt_order_goods') . ' og ' . ' left join ' . tablename('tapgo_tt_goods') . ' g on og.goodsid = g.id ' . ' left join ' . tablename('tapgo_tt_goods_option') . ' op on og.optionid = op.id ' . (' where ' . $scondition . ' order by og.id asc');
...
', '<br/>', $order['virtual_str']);
		...
		if (!empty($order['virtual']) && !empty($order['virtual_str'])) {
			...
			$virtualtemp = pdo_fetch('SELECT linktext, linkurl,description FROM ' . tablename('tapgo_tt_virtual_type') . ' WHERE id=:id AND uniacid=:uniacid LIMIT 1', array(':id' => $order['virtual'], ':uniacid' => $_W['uniacid']));
		}
	...
	public function express()
	{
		...
		$goods = pdo_fetchall('select og.goodsid,og.price,g.title,g.thumb,og.total,g.credit,og.optionid,og.optionname as optiontitle,g.isverify,og.expresssn,og.express,
            ...
            from ' . tablename('tapgo_tt_order_goods') . ' og ' . ' left join ' . tablename('tapgo_tt_goods') . ' g on g.id=og.goodsid ' . ' where og.orderid=:orderid and og.single_refundtime=0 ' . $condition . ' and og.uniacid=:uniacid ', array(':uniacid' => $uniacid, ':orderid' => $orderid));
