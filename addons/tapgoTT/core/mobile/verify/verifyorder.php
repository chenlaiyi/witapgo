...
if (!pdo_tableexists('tapgo_tt_saler_verify_log'))
{
    pdo_query("CREATE TABLE " . tablename('tapgo_tt_saler_verify_log') ." (
...
class Verifyorder_TapgoTtPage extends MobilePage
{
...
//        订单号，核销员，核销门店，核销码，会员姓名，收件人姓名，快递单号，商品名称，商品编码
        if (!empty($_GPC['searchfield']) && !empty($_GPC['keyword'])) {
            ...
            if ($searchfield == 'ordersn') {
                ...
                $sqlcondition = " inner join ( select DISTINCT(og.orderid) from " . tablename('tapgo_tt_order_goods') . " og left join " . tablename('tapgo_tt_goods') . " g on g.id=og.goodsid where og.uniacid = '$uniacid' and (locate(:keyword,g.title)>0)) gs on gs.orderid=o.id";
            } else if ($searchfield == 'goodssn') {
                $sqlcondition = " inner join ( select DISTINCT(og.orderid) from " . tablename('tapgo_tt_order_goods') . " og left join " . tablename('tapgo_tt_goods') . " g on g.id=og.goodsid where og.uniacid = '$uniacid' and (((locate(:keyword,g.goodssn)>0)) or (locate(:keyword,og.goodssn)>0))) gs on gs.orderid=o.id";
            }
        }
        ...
        if ($condition != ' o.uniacid = :uniacid and o.ismr=0 and o.deleted=0 and o.isparent=0 AND ( o.status = 3 and (isverify=1 or o.istrade=1)) and o.verifyopenid = :openid' || !empty($sqlcondition)) {
            if ($this->merchPluginExists()) {
                $sql = "select o.* , a.realname as arealname,a.mobile as amobile,a.province as aprovince ,a.city as acity , a.area as aarea, a.street as astreet,a.address as aaddress, d.dispatchname,m.nickname,m.id as mid,m.realname as mrealname,m.mobile as mmobile,sm.id as salerid,sm.nickname as salernickname,s.salername,
                  r.rtype,r.status as rstatus,o.sendtype,0 as isverifygoods_order from " . tablename('tapgo_tt_order') . " o"
                    ...
                    . " left join " . tablename('tapgo_tt_merch_saler') . " ms on ms.openid = o.verifyopenid and ms.uniacid=o.uniacid"
            ...
            if ($this->merchPluginExists()) {
                $sql = "SELECT o.*,0 as isverifygoods_order FROM" . tablename("tapgo_tt_order") . " o "
                    . " LEFT JOIN " . tablename("tapgo_tt_saler") . " s ON o.verifyopenid=s.openid"
                    . " LEFT JOIN " . tablename("tapgo_tt_merch_saler") . " ms ON o.verifyopenid=ms.openid"
...
//                }
        ...
        if ($condition != ' o.uniacid = :uniacid and o.ismr=0 and o.deleted=0 and o.isparent=0' || !empty($sqlcondition)) {
            $t = pdo_fetch(
                'SELECT COUNT(DISTINCT(o.id)) as count, ifnull(sum(o.price),0) as sumprice   FROM ' . tablename('tapgo_tt_order') . " o "
                . " left join " . tablename('tapgo_tt_order_refund') . " r on r.id =o.refundid "
                . " left join " . tablename('tapgo_tt_member') . " m on m.openid=o.openid  and m.uniacid =  o.uniacid"
            ...
            $t = pdo_fetch(
                'SELECT COUNT(*) as count, ifnull(sum(price),0) as sumprice   FROM ' . tablename('tapgo_tt_order') . " WHERE uniacid = :uniacid and ismr=0 and deleted=0 and isparent=0 {$status_condition}", $paras);
        }
    ...
    public function orderDataVerify()
    {
        ...
        //当前核销员
        $saler = pdo_fetch('select id,openid from ' . tablename('tapgo_tt_saler') . ' where status=1  and openid=:openid and uniacid=:uniacid limit 1', array(
...
//        订单号，核销员，核销门店，核销码，会员姓名，联系人，快递单号，商品名称，商品编码
        ...
        if ($condition != ' log.salerid = :salerid and log.uniacid = :uniacid ' || !empty($sqlcondition)) {
            ...
            $sql = "select o.* ,1 as isverifygoods_order,g.id as verifygoodsid from " . tablename('tapgo_tt_verifygoods_log') . " log "
                . " left join  " . tablename('tapgo_tt_verifygoods') . " g on log.verifygoodsid = g.id "
                . " left join  " . tablename('tapgo_tt_order') . " o on g.orderid = o.id  "
                . " left join " . tablename('tapgo_tt_member') . " m on m.openid=o.openid and m.uniacid =  o.uniacid "
            ...
            $sql = "select o.* ,1 as isverifygoods_order,g.id as verifygoodsid from " . tablename('tapgo_tt_verifygoods_log') . " log "
                . " left join  " . tablename('tapgo_tt_verifygoods') . " g on log.verifygoodsid = g.id "
                . " left join  " . tablename('tapgo_tt_order') . " o on g.orderid = o.id where log.uniacid = :uniacid and log.salerid=:salerid GROUP BY o.id ORDER BY log.verifydate DESC  ";
        ...
        if (!empty($list)) {
            ...
            foreach ($list as &$value) {
                ...
                //核销次数统计
                $verifygoodlogs = pdo_fetchall('select *  from ' . tablename('tapgo_tt_verifygoods_log') . '    where verifygoodsid =:id  ', array(':id' => $value['verifygoodsid']));
                $verifynum = 0;
                foreach ($verifygoodlogs as $verifygoodlog) {
                    $verifynum += intval($verifygoodlog['verifynum']);
                }
                $verifygood = pdo_fetch('select *  from ' . tablename('tapgo_tt_verifygoods') . '    where id =:id  ', array(':id' => $value['verifygoodsid']));
        ...
        if ($condition != ' o.uniacid = :uniacid and o.ismr=0 and o.deleted=0 and o.isparent=0' || !empty($sqlcondition)) {
            ...
            $t = pdo_fetch(
                'SELECT COUNT(*) as count, ifnull(sum(price),0) as sumprice   FROM ' . tablename('tapgo_tt_order') . " WHERE uniacid = :uniacid and ismr=0 and deleted=0 and isparent=0", $paras);
        }
    ...
    protected function selectOrderPrice($day = 0)
    {
        ...
        if ($this->merchPluginExists()) {
            ...
            $sql = "SELECT count(o.id) FROM" . tablename("tapgo_tt_order") . " o"
                . " LEFT JOIN " . tablename("tapgo_tt_saler") . " s ON o.verifyopenid=s.openid"
                . " LEFT JOIN " . tablename("tapgo_tt_merch_saler") . " ms ON o.verifyopenid=ms.openid"
        ...
        $sql = "SELECT sum(verifynum) FROM" . tablename("tapgo_tt_verifygoods_log") . "    WHERE 
...
//            ':openid' => $_W['openid'], ':uniacid' => $_W['uniacid']
//        ));
//        $merch_saler =  pdo_fetchall('select id,openid from ' . tablename('tapgo_tt_merch_saler') . ' where status=1  and openid=:openid and uniacid=:uniacid limit 1', array(
//            ':openid' => $_W['openid'], ':uniacid' => $_W['uniacid']
//        ));

//        $param['salerid'] = $saler['id'];
        $sql = "select count(*) from " . tablename('tapgo_tt_saler_verify_log') . " where uniacid = :uniacid and openid = :openid  and  verify_time > :createtime1 and verify_time < :createtime2 and type = 0";
        // 判断 merch_saler表是否存在
...
//        $sql= "SELECT sum(verifynum) FROM". tablename("tapgo_tt_verifygoods_log")."    WHERE
//          salerid=:salerid and verifydate between :createtime1 and :createtime2 and uniacid=:uniacid limit 1";
        $sql = "select count(*) from " . tablename('tapgo_tt_saler_verify_log') . " where uniacid = :uniacid and openid = :openid  and  verify_time > :createtime1 and verify_time < :createtime2 and type = 1";
