...
class Index_TapgoTtPage extends MobilePage {
	...
	function qrcode() {
        ...
        $order = pdo_fetch("select istrade from " . tablename('tapgo_tt_order') . ' where id=:id and uniacid=:uniacid limit 1'
	...
	function select() {
		...
		$order = pdo_fetch("select id,verifyinfo from " . tablename('tapgo_tt_order') . ' where id=:id and uniacid=:uniacid limit 1'
			, array(':id' => $orderid, ':uniacid' => $_W['uniacid']));
		if (empty($order)) {
			show_json(0);
		}
		...
		unset($v);
		$res = pdo_update('tapgo_tt_order', array('verifyinfo' => iserializer($verifyinfo)), array('id' => $orderid));
        if (empty($res)) {
            show_json(0);
        }
        show_json(1);
	...
	function check() {
		...
		$orderid = intval($_GPC['id']);
		$order = pdo_fetch("select id,status,isverify,verified from " . tablename('tapgo_tt_order') . ' where id=:id and uniacid=:uniacid and openid=:openid limit 1'
