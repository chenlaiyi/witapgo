...
class Verifygoods_TapgoTtPage extends MobilePage {
	...
	function  detail() {
		...
		$item = pdo_fetch('select vg.*,g.id as goodsid ,g.title,g.subtitle,g.thumb,vg.storeid from ' . tablename('tapgo_tt_verifygoods') . '   vg
		...
		$saler = pdo_fetch('select * from ' . tablename('tapgo_tt_saler') . ' where  status=1  and openid=:openid and uniacid=:uniacid limit 1', array(
		...
		{
			$verifygoodlogs = pdo_fetchall('select *  from ' . tablename('tapgo_tt_verifygoods_log') . '    where verifygoodsid =:id  ', array(':id' => $item['id']));
			$verifynum = 0;
