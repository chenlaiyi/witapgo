...
class Index_TapgoTtPage extends MobilePage 
{
	...
	public function detail() 
	{
		...
		if( empty($verifycode) || $item["codeinvalidtime"] < time() ) 
		{
			...
			while( 1 ) 
			{
				$count = pdo_fetchcolumn("select count(*) from " . tablename("tapgo_tt_verifygoods") . " where verifycode=:verifycode and uniacid=:uniacid limit 1", array( ":verifycode" => $verifycode, ":uniacid" => $_W["uniacid"] ));
				if( $count <= 0 ) 
				{
					break;
				}
			...
			pdo_update("tapgo_tt_verifygoods", $data, array( "id" => $item["id"] ));
		}
		...
		$goodsstore = pdo_fetch("select * from " . tablename("tapgo_tt_store") . " where id=:id and uniacid=:uniacid limit 1", array( ":id" => $item["storeid"], ":uniacid" => $_W["uniacid"] ));
		include($this->template());
	}
