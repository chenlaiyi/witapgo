...
class Cart_TapgoTtPage extends MobileLoginPage {
    ...
    function get_list(){
        ...
        $sql = 'SELECT f.id,f.total,f.goodsid,g.status,g.total as stock,g.preselltimeend,g.presellprice as gpprice,g.hasoption, o.stock as optionstock,g.presellprice,g.ispresell, g.maxbuy,g.title,g.thumb,ifnull(o.marketprice, g.marketprice) as marketprice,'
            ...
            . ' ,f.selected,g.type,g.intervalfloor,g.intervalprice  FROM ' . tablename('tapgo_tt_member_cart') . ' f '
            . ' left join ' . tablename('tapgo_tt_goods') . ' g on f.goodsid = g.id '
            . ' left join ' . tablename('tapgo_tt_goods_option') . ' o on f.optionid = o.id '
        ...
        foreach ($list as $index => &$g) {
            ...
            if (!empty($g['optionid'])) {
                ...
                $optionData = pdo_fetch('SELECT * FROM '.tablename('tapgo_tt_goods_option').' WHERE uniacid = '.intval($_W['uniacid']).' AND id = '.intval($g['optionid']));
                if(empty($optionData) || $optionData == false){
                    pdo_update('tapgo_tt_member_cart',array('deleted'=>1),array('id'=>$g['id']));
                }
            }

            if ($g['status'] == 0 || $g['stock'] == 0 || ($g['hasoption'] == 1 && $g['optionstock'] == 0) ) {
                $g['selected'] = false;
                pdo_update('tapgo_tt_member_cart', array('selected' => 0), array('id' => $g['id']));
            }
    ...
    function select(){
        ...
        if(!empty($id)){
            $data = pdo_fetch("select id,goodsid,optionid, total from " . tablename('tapgo_tt_member_cart') . " "
                . " where id=:id and uniacid=:uniacid and openid=:openid limit 1 ", array(':id' => $id, ':uniacid' => $_W['uniacid'],':openid'=>$_W['openid']));
            if (!empty($data)) {
                pdo_update('tapgo_tt_member_cart', array('selected' => $select), array('id' => $id, 'uniacid' => $_W['uniacid']));
            }
        } else {
            $cartGoods = pdo_fetchall(
                "select c.id as cartid,g.id as goodsid, g.total as stock, g.status, c.optionid, o.stock as optionstock from " . tablename('tapgo_tt_member_cart') . " c"
                . " left join " . tablename('tapgo_tt_goods') . " g on g.id = c.goodsid"
                . " left join " . tablename('tapgo_tt_goods_option') . " o on o.id = c.optionid"
            ...
            $ret = implode(',', $ret);
            pdo_query("update ". tablename('tapgo_tt_member_cart') . " set selected = {$select} where id in ({$ret})");
        }
    ...
    function update(){
        ...
        $data = pdo_fetch("select id,goodsid,optionid,total,selected from " . tablename('tapgo_tt_member_cart') . " "
            . " where id=:id and uniacid=:uniacid and openid=:openid limit 1 ", array(':id' => $id, ':uniacid' => $_W['uniacid'],':openid'=>$_W['openid']));
        ...
        pdo_update('tapgo_tt_member_cart', array('total' => $goodstotal,'optionid'=>$optionid), array('id' => $id, 'uniacid' => $_W['uniacid'],'openid'=>$_W['openid']));
    ...
    function add() {
        ...
        $total <= 0 && $total = 1;
        $optionid = intval($_GPC['optionid']);
        $goods = pdo_fetch('select id,marketprice,diyformid,diyformtype,diyfields, isverify, `type`,merchid, cannotrefund,hasoption from '.tablename('tapgo_tt_goods').' where id=:id and uniacid=:uniacid limit 1',array(':id'=>$id,':uniacid'=>$_W['uniacid']));
        if (empty($goods)) {
            show_json(0, '商品未找到');
        }
        ...
        $gift = pdo_fetch("select * from ".tablename('tapgo_tt_gift')." where uniacid = ".$_W['uniacid']." and id = ".$giftid." and starttime >= ".time()." and endtime <= ".time()." and status = 1 ");
        ...
        $data = pdo_fetch("select id,total,diyformid from " . tablename('tapgo_tt_member_cart') . ' where goodsid=:id and openid=:openid and   optionid=:optionid  and deleted=0 and  uniacid=:uniacid   limit 1', array(
            ':uniacid' => $_W['uniacid'],
            ':openid' => $_W['openid'],
            ':optionid' => $optionid,
            ':id' => $id
        ...
        if (empty($data)) {
            ...
            pdo_insert('tapgo_tt_member_cart', $data);
            ...
            $data['total']+=$total;
            pdo_update('tapgo_tt_member_cart', $data, array('id' => $data['id']));
        }
        $cartcount = pdo_fetchcolumn('select sum(total) from ' . tablename('tapgo_tt_member_cart') . ' where openid=:openid and deleted=0 and uniacid=:uniacid limit 1', array(
            ':uniacid' => $_W['uniacid'],
            ':openid' => $_W['openid']
        ));
    ...
    function addwholesale() {
        ...
        $goods = pdo_fetch('select id,marketprice,diyformid,diyformtype,diyfields, isverify, `type`,merchid, cannotrefund from '.tablename('tapgo_tt_goods').' where id=:id and uniacid=:uniacid limit 1',array(':id'=>$id,':uniacid'=>$_W['uniacid']));
        if (empty($goods)) {
            show_json(0, '商品未找到');
        }
    ...
    {
        global $_W, $_GPC;
        $listCount = pdo_fetch("select count(*) sum from" .tablename('tapgo_tt_member_cart') . " where uniacid = :uniacid and openid = :openid and deleted = 0", array('uniacid'=>$_W['uniacid'], 'openid'=>$_W['openid']));
    ...
    function remove(){
        ...
        $sql = "update " . tablename('tapgo_tt_member_cart') . ' set deleted=1 where uniacid=:uniacid and openid=:openid and id in (' . implode(',', $ids) . ')';
    ...
    function tofavorite(){
        ...
        foreach ($ids as $id) {
            $goodsid = pdo_fetchcolumn('select goodsid from ' . tablename('tapgo_tt_member_cart') . ' where id=:id and uniacid=:uniacid and openid=:openid limit 1 ', array(':id' => $id, ':uniacid' => $uniacid, ':openid' => $openid));
            if (!empty($goodsid)) {
                $fav = pdo_fetchcolumn('select count(*) from ' . tablename('tapgo_tt_member_favorite') . ' where goodsid=:goodsid and uniacid=:uniacid and openid=:openid and deleted=0 limit 1 ', array(':goodsid' => $goodsid, ':uniacid' => $uniacid, ':openid' => $openid));
                if ($fav <= 0) {
                    $fav = array(
                        'uniacid' => $uniacid,
                        'goodsid' => $goodsid,
                    ...
                    pdo_insert('tapgo_tt_member_favorite', $fav);
                }
            }
        }
    ...
    private function cartGoodsCheckedStatus($id, $checked = false)
    {
        ...
        pdo_update('tapgo_tt_member_cart', array('selected' => $selected), array('id'=>$id));
    }
    ...
    function submit(){
        ...
        $sql = 'SELECT f.id,f.total,f.goodsid,g.total as stock, o.stock as optionstock, g.hasoption,g.maxbuy,g.title,g.thumb,ifnull(o.marketprice, g.marketprice) as marketprice,'
            ...
            . ' ,f.selected,g.status,g.deleted as goodsdeleted,g.type,g.intervalfloor,g.intervalprice  FROM ' . tablename('tapgo_tt_member_cart') . ' f '
            . ' left join ' . tablename('tapgo_tt_goods') . ' g on f.goodsid = g.id '
            . ' left join ' . tablename('tapgo_tt_goods_option') . ' o on f.optionid = o.id '
            . ' where 1 ' . $condition . ' ORDER BY `id` DESC ';
        $list = pdo_fetchall($sql, $params);
        ...
        $array = pdo_fetchall('select og.optionid  from ' . tablename('tapgo_tt_order_goods') . ' og '
            . ' left join ' . tablename('tapgo_tt_order') . ' o on og.orderid=o.id '
        ...
        foreach ($list as &$g) {
            ...
            if( ($g['stock'] - $g['total'] < 0) && ( !$g['optionid'] || ($g['optionid'] && $g['stock'] != -1))) {
                pdo_update('tapgo_tt_member_cart', array('selected' => 0, 'total'=> $g['stock']), array('id'=>$g['id']));
            ...
            if( $seckillinfo && $seckillinfo['status']==0){
                ...
                if (!empty($g['optionid'])) {
                    $option = pdo_fetch('select id,title,marketprice,goodssn,productsn,stock,`virtual`,weight from ' . tablename('tapgo_tt_goods_option') . ' where id=:id and goodsid=:goodsid and uniacid=:uniacid  limit 1', array(':uniacid' => $uniacid, ':goodsid' => $g['goodsid'], ':id' => $g['optionid']));
                    if (!empty($option)) {
                        if ($option['stock'] != -1) {
