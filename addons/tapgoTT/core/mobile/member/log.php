...
class Log_TapgoTtPage extends MobileLoginPage
{
    ...
    public function get_list()
    {
        ...
        if( 0 < $uid ) 
        {
            $r = pdo_fetchall("select m.uid,m.mobile,m.nickname,r.remark title,r.num money,r.createtime from " . tablename("tapgo_tt_member_credit_record") . "r left join " . tablename("tapgo_tt_member") . " m on m.uid = r.uid where 1 " . $credit_condition . ($pindex - 1) * $psize . "," . $psize);
            foreach( $r as &$item ) 
            {
        ...
        $list = pdo_fetchall("select * from " . tablename("tapgo_tt_member_log") . " where 1 " . $condition . " order by createtime desc LIMIT " . ($pindex - 1) * $psize . "," . $psize, $params);
        $total = pdo_fetchcolumn("select count(*) from " . tablename("tapgo_tt_member_log") . " where 1 " . $condition, $params);
        foreach( $list as &$row ) 
        {
