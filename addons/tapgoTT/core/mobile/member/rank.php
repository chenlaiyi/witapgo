...
class Rank_TapgoTtPage extends MobileLoginPage {
	...
	protected function creditChange($uid,$credittype='credit1',$day=7,$openid='')
	{
		...
		if($uid){
            $sql = 'select sum(num) from ' . tablename('mc_credits_record') . ' where uniacid = :uniacid and uid = :uid and credittype = :credittype and `module` = "tapgo_ttv2" and createtime between :createtime1 and :createtime2 and uid<>0';
            $param = array(
                ':uniacid' => $_W['uniacid'],
                ':uid' => (int)$uid,
            ...
            $sql = 'select sum(num) from ' . tablename('tapgo_tt_member_credit_record') . ' where uniacid = :uniacid and openid = :openid and credittype = :credittype and createtime between :createtime1 and :createtime2';
            $param = array(
                ':uniacid' => $_W['uniacid'],
                ':openid' => $openid,
                ':createtime1' => $createtime1,
	...
	public function order_rank()
	{
		...
		$result_one = pdo_fetch("select a.openid, sum(price) as price,a.uniacid,a.status
			from (
			(select openid, sum(price) price,uniacid,status from ".tablename('tapgo_tt_order')." where uniacid = :uniacid and status >= 1 group by openid)
			union  all (select openid, sum(price+freight) price,uniacid,status from ".tablename('tapgo_tt_groups_order')." where uniacid = :uniacid and status >= 1 group by openid)
	...
	public function ajaxorderpage()
	{
		...
		$sql  = "SELECT m.id,m.uid,m.nickname,m.avatar,m.openid,"
            . "(select ifnull(sum(o.price),0) from  " . tablename('tapgo_tt_order') . " o where o.openid=m.openid and o.status>=1 {$condition})  as price"
            . " from " . tablename('tapgo_tt_member') . " m  "
	...
	protected function orderChange($openid,$day=7)
	{
		...
		$sql = 'select sum(price) from ' . tablename('tapgo_tt_order') . ' where uniacid = :uniacid and openid = :openid and status >= 1 and createtime between :createtime1 and :createtime2';
		$param = array(
			':uniacid' => $_W['uniacid'],
			':openid' => $openid,
			':createtime1' => $createtime1,
