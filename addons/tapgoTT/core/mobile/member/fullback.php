...
class Fullback_TapgoTtPage extends MobileLoginPage
{
	public function main()
	{
		...
		$info = pdo_fetchall('SELECT * FROM' . tablename('tapgo_tt_fullback_log') . $condition, $params);
		$alldata = array();
		$allday = array();
		$fullbackday = array();
		$createtime = array();
	...
	public function get_list()
	{
		...
		$list = array();
		$list = pdo_fetchall('select fl.*,g.thumb,g.title from ' . tablename('tapgo_tt_fullback_log') . ' as fl
            left join ' . tablename('tapgo_tt_goods') . ' as g on g.id = fl.goodsid
            left join ' . tablename('tapgo_tt_order_goods') . (' as og on og.orderid = fl.orderid and og.goodsid = fl.goodsid 
            where 1 ' . $condition . ' group by fl.id order by fl.createtime desc LIMIT ') . ($pindex - 1) * $psize . ',' . $psize, $params);
		$total = pdo_fetchcolumn('select count(1) from ' . tablename('tapgo_tt_fullback_log') . (' as fl
		...
		foreach ($list as &$row) {
			...
			if (0 < $row['optionid']) {
				$optionname = pdo_get('tapgo_tt_order_goods', array('optionid' => $row['optionid']), array('optionname'));
