...
class Address_TapgoTtPage extends MobileLoginPage {
    ...
    function main() {
        ...
        $sql = 'SELECT * FROM ' . tablename('tapgo_tt_member_address') . ' where 1 ' . $condition . ' ORDER BY `id` DESC';
    ...
    function post() {
        ...
        if(!empty($id) || !empty($wx_address_info)){
            ...
            if(empty($address['datavalue'])){
                ...
                $path = TAPGO_TT_PATH."static/js/dist/area/AreaNew.xml";
                ...
                if($province_code!=0 &&$citys_code!=0&&$county_code!=0){
                    ...
                    if(empty($wx_address_info)){
                        pdo_update('tapgo_tt_member_address', $address, array('id' => $id, 'uniacid' => $_W['uniacid'], 'openid' => $_W['openid']));
                    }
                }
            }
    ...
    function selector() {
        ...
        $sql = 'SELECT * FROM ' . tablename('tapgo_tt_member_address') . ' where 1 ' . $condition . ' ORDER BY isdefault desc, id DESC ';
        $list = pdo_fetchall($sql, $params);
        include $this->template();
        exit;
    }
