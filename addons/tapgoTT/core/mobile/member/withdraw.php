...
class Withdraw_TapgoTtPage extends MobileLoginPage {
    ...
    function submit() {
        ...
        pdo_insert('tapgo_tt_member_log', $apply);
        $logid = pdo_insertid();
        if (empty($logid)) {
    ...
    function getLastApply($openid, $applytype = -1)
    {
        ...
        $sql = "select applytype,alipay,bankname,bankcard,realname from " . tablename('tapgo_tt_member_log') . " where openid=:openid and uniacid=:uniacid";
