...
class Refund_TapgoTtPage extends MobileLoginPage {
    ...
    protected function globalData() {
        ...
        $order = pdo_fetch("select id,status,price,refundid,goodsprice,dispatchprice,deductprice,deductcredit2,finishtime,isverify,`virtual`,refundstate,merchid,random_code,iscycelbuy,paytype from " . tablename('tapgo_tt_order') . ' where id=:id and uniacid=:uniacid and openid=:openid limit 1'
        ...
        $refund = pdo_fetch("select * from ".tablename('tapgo_tt_order_refund')." where uniacid = :uniacid and orderid = :orderid and status > 0 limit 1 ",array(':uniacid'=>$uniacid,':orderid'=>$orderid));
    ...
    function cancel() {
        ...
        pdo_update('tapgo_tt_order_refund', $change_refund, array('id' => $refundid, 'uniacid' => $uniacid));
        pdo_update('tapgo_tt_order', array('refundstate' => 0), array('id' => $orderid, 'uniacid' => $uniacid));
        show_json(1);
    }
    ...
    function receive(){
        ...
        pdo_update('tapgo_tt_order_refund', $refund_data, array('id'=>$refundid, 'uniacid' => $uniacid));
        ...
        pdo_update('tapgo_tt_order', $order_data, array('id'=>$orderid, 'uniacid' => $uniacid));
        show_json(1);
