...
class Pay_TapgoTtPage extends MobileLoginPage
{
    const AFTER_PAY = 1;
    ...
    public function main()
    {
        ...
        if( !empty($ispeerpay) ) 
        {
            if( pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename("tapgo_tt_order_peerpay_payinfo") . " WHERE openid = :openid AND pid = :pid", array( ":openid" => $_W["openid"], ":pid" => $ispeerpay["id"] )) ) 
            {
                $this->message("每人只能代付一次");
            }

            $peerpayMessage = trim($_GPC["peerpaymessage"]);
            $peerpay_info = (double) pdo_fetchcolumn("select SUM(price) from " . tablename("tapgo_tt_order_peerpay_payinfo") . " where pid=:pid limit 1", array( ":pid" => $ispeerpay["id"] ));
            ...
            $openid = pdo_fetchcolumn("SELECT openid FROM " . tablename("tapgo_tt_order") . " WHERE id = :id AND uniacid = :uniacid LIMIT 1", array( ":id" => $orderid, ":uniacid" => $_W["uniacid"] ));
        }

        $order = pdo_fetch("select * from " . tablename("tapgo_tt_order") . " where id=:id and uniacid=:uniacid and openid=:openid limit 1", array( ":id" => $orderid, ":uniacid" => $uniacid, ":openid" => $openid ));
        ...
        {
            $order_goods = pdo_fetchall("select og.id,g.title, og.goodsid,og.optionid,g.total as stock,og.total as buycount,g.status,g.deleted,g.maxbuy,g.usermaxbuy,g.istime,g.timestart,g.timeend,g.buylevels,g.buygroups,g.totalcnf,og.seckill from  " . tablename("tapgo_tt_order_goods") . " og " . " left join " . tablename("tapgo_tt_goods") . " g on og.goodsid = g.id " . " where og.orderid=:orderid and og.uniacid=:uniacid ", array( ":uniacid" => $_W["uniacid"], ":orderid" => $orderid ));
        ...
        else
        {
            if( p("newstore") ) 
            {
                $sql = "select og.id,g.title, og.goodsid,og.optionid,ng.stotal as stock,og.total as buycount,ng.gstatus as status,ng.deleted,g.maxbuy,g.usermaxbuy,g.istime,g.timestart,g.timeend,g.buylevels,g.buygroups,g.totalcnf,og.seckill,og.storeid " . " from " . tablename("tapgo_tt_order_goods") . " og left join  " . tablename("tapgo_tt_goods") . " g  on g.id=og.goodsid and g.uniacid=og.uniacid" . " inner join " . tablename("tapgo_tt_newstore_goods") . " ng on g.id=ng.goodsid   " . " where og.orderid=:orderid and og.uniacid=:uniacid and ng.storeid=:storeid";
                ...
                foreach( $order_goods as $data ) 
                {
                    ...
                    if( $data["totalcnf"] == 1 ) 
                    {
                        if( !empty($data["optionid"]) ) 
                        {
                            $option = pdo_fetch("select id,marketprice,stock from " . tablename("tapgo_tt_newstore_goods_option") . " where optionid=:optionid and goodsid=:goodsid and uniacid=:uniacid and storeid=:storeid  limit 1", array( ":uniacid" => $uniacid, ":goodsid" => $data["goodsid"], ":optionid" => $data["optionid"], ":storeid" => $data["storeid"] ));
                            if( !empty($option) && $option["stock"] != -1 && empty($option["stock"]) ) 
                            {
                                if( $_W["ispost"] ) 
                                {
        ...
        $log = pdo_fetch("SELECT * FROM " . tablename("core_paylog") . " WHERE `uniacid`=:uniacid AND `module`=:module AND `tid`=:tid limit 1", array( ":uniacid" => $uniacid, ":module" => "tapgo_ttv2", ":tid" => $order["ordersn"] ));
        if( !empty($ispeerpay) ) 
        {
        }
        else
        ...
        $seckill_goods = pdo_fetchall("select goodsid,optionid,seckill from  " . tablename("tapgo_tt_order_goods") . " where orderid=:orderid and uniacid=:uniacid and seckill=1 ", array( ":uniacid" => $_W["uniacid"], ":orderid" => $orderid ));
        if( !empty($log) && $log["status"] == "0" ) 
        {
        ...
        if( empty($log) ) 
        {
            $log = array( "uniacid" => $uniacid, "openid" => trim($member["uid"]), "module" => "tapgo_ttv2", "tid" => trim($order["ordersn"]), "fee" => $order["price"], "status" => 0 );
        ...
        if( is_weixin() ) 
        {
            ...
            if( !empty($order["ordersn2"]) ) 
            {
                ...
                if( $redis->get($key) === false ) 
                {
                    ...
                    {
                        $var = $var + 1;
                        pdo_update("tapgo_tt_order", array( "ordersn2" => $var ), array( "id" => $orderid, "uniacid" => $uniacid, "openid" => $openid ));
                    }
            ...
            $datasss = m('common')->getSysset('pay');
            $datasss = pdo_fetch('SELECT * FROM ' . tablename('tapgo_tt_payment') . ' WHERE id=:id', array(':id' => $datasss['weixin_id']));
            
            if($datasss['type']==5){
            
                pdo_update('tapgo_tt_order', array('isunion' => 1), array('ordersn'=>$params['tid']));
            ...
            $unionpay = m("common")->unionpay_build($params,$unionpay,0);
        ...
        $data = pdo_fetch('SELECT * FROM ' . tablename('tapgo_tt_payment') . ' WHERE id=:id', array(':id' => $data['weixin_id']));
        
        if($data['type']==5){
            $wechat['success']=true;
            $unionpay['success']=true;
            $payinfo = array( "orderid" => $orderid, "ordersn" => trim($log["tid"]), "credit" => $credit, "alipay" => $alipay, "wechat" => $unionpay, "cash" => $cash, "money" => $order["price"], "unionpay" => $unionpay);
         
        }else{
            $payinfo = array( "orderid" => $orderid, "ordersn" => trim($log["tid"]), "credit" => $credit, "alipay" => $alipay, "wechat" => $wechat, "cash" => $cash, "money" => $order["price"], "unionpay" => $unionpay);
        }
    ...
    public function orderstatus()
    {
        ...
        $uniacid = $_W["uniacid"];
        $orderid = intval($_GPC["id"]);
        $order = pdo_fetch("select status from " . tablename("tapgo_tt_order") . " where id=:id and uniacid=:uniacid limit 1", array( ":id" => $orderid, ":uniacid" => $uniacid ));
        if( 1 <= $order["status"] ) 
        {
            @session_start();
            $_SESSION[TAPGO_TT_PREFIX . "_order_pay_complete"] = 1;
            show_json(1);
        }
    ...
    public function complete()
    {
        ...
        if( !empty($ispeerpay) ) 
        {
            ...
            $openid = pdo_fetchcolumn("select openid from " . tablename("tapgo_tt_order") . " where id=:orderid and uniacid=:uniacid limit 1", array( ":orderid" => $orderid, ":uniacid" => $uniacid ));
            $peerpay_info = (double) pdo_fetchcolumn("select SUM(price) price from " . tablename("tapgo_tt_order_peerpay_payinfo") . " where pid=:pid limit 1", array( ":pid" => $ispeerpay["id"] ));
        }
        else
        {
        ...
        if( is_h5app() && empty($orderid) ) 
        {
            ...
            $orderid = pdo_fetchcolumn("select id from " . tablename("tapgo_tt_order") . " where ordersn=:ordersn and uniacid=:uniacid and openid=:openid limit 1", array( ":ordersn" => $ordersn, ":uniacid" => $uniacid, ":openid" => $openid ));
        }
        ...
        {
            $package = pdo_fetch("SELECT * FROM " . tablename("tapgo_tt_package") . " WHERE uniacid = " . $uniacid . " and id = " . $order["packageid"] . " ");
            if( empty($package) ) 
            {
                show_json(0, "未找到套餐！");
            }
        ...
        if( $type == "cash" && empty($ispeerpay) ) 
        {
            ...
            pdo_update("tapgo_tt_order", array( "cashtime" => time() ), array( "id" => $order["id"] ));
            pdo_update("tapgo_tt_order", array( "cashtime" => time() ), array( "parentid" => $order["id"] ));
        ...
        else
        {
            if( $type == "wechat" ) 
            {
                ...
                if( !is_error($payquery) || !is_error($payquery_jie) || !empty($ispeerpay) ) 
                {
                    ...
                    if( is_h5app() ) 
                    {
                        pdo_update("tapgo_tt_order", array( "apppay" => 1 ), array( "id" => $order["id"] ));
                    }
                    ...
                    if( !empty($ispeerpay) ) 
                    {
                        ...
                        pdo_update("tapgo_tt_order_peerpay_payinfo", $udata, array( "pid" => $ispeerpay["id"], "openid" => $_W["openid"] ));
                        m("order")->setOrderPayType($order["id"], 21, $gpc_ordersn);
                    }
    ...
    public function success()
    {
        ...
        $order = pdo_fetch("select * from " . tablename("tapgo_tt_order") . " where id=:id and uniacid=:uniacid and openid=:openid limit 1", array( ":id" => $orderid, ":uniacid" => $uniacid, ":openid" => $openid ));
        @session_start();
        if( empty($_SESSION[TAPGO_TT_PREFIX . "_order_pay_complete"]) ) 
        {
            if( empty($order["istrade"]) ) 
            {
                header("location: " . mobileUrl("order"));
        ...
        unset($_SESSION[TAPGO_TT_PREFIX . "_order_pay_complete"]);
        ...
        if( !empty($ispeerpay) ) 
        {
            ...
            $peerprice = pdo_fetch("SELECT `price` FROM " . tablename("tapgo_tt_order_peerpay_payinfo") . " WHERE uid = :uid ORDER BY id DESC LIMIT 1", array( ":uid" => $peerpayuid["id"] ));
            $share = false;
            if( com("coupon") ) 
            {
        ...
        else
        {
            ...
            $goods = pdo_fetchall("select og.goodsid,og.price,g.title,g.thumb,og.total,g.credit,og.optionid,og.optionname as optiontitle,g.isverify,g.storeids from " . tablename("tapgo_tt_order_goods") . " og " . " left join " . tablename("tapgo_tt_goods") . " g on g.id=og.goodsid " . " where og.orderid=:orderid and og.uniacid=:uniacid ", array( ":uniacid" => $uniacid, ":orderid" => $orderid ));
            $address = false;
            if( !empty($order["addressid"]) ) 
            {
                ...
                if( !is_array($address) ) 
                {
                    $address = pdo_fetch("select * from  " . tablename("tapgo_tt_member_address") . " where id=:id limit 1", array( ":id" => $order["addressid"] ));
                }
            ...
            if( !empty($order["storeid"]) ) 
            {
                if( 0 < $merchid ) 
                {
                    $store = pdo_fetch("select * from  " . tablename("tapgo_tt_merch_store") . " where id=:id limit 1", array( ":id" => $order["storeid"] ));
                }
                else
                {
                    $store = pdo_fetch("select * from  " . tablename("tapgo_tt_store") . " where id=:id limit 1", array( ":id" => $order["storeid"] ));
                }
            ...
            if( $order["isverify"] ) 
            {
                ...
                {
                    $stores = pdo_fetchall("select * from " . tablename("tapgo_tt_store") . " where id = :id  and uniacid=:uniacid and status=1  order by displayorder desc,id desc", array( ":uniacid" => $_W["uniacid"], ":id" => $order["storeid"] ));
                }
                else
                {
                    if( empty($storeids) ) 
                    ...
                    else
                    {
                        if( 0 < $merchid ) 
                        {
                            $stores = pdo_fetchall("select * from " . tablename("tapgo_tt_merch_store") . " where id in (" . implode(",", $storeids) . ") and uniacid=:uniacid and merchid=:merchid and status=1 order by displayorder desc,id desc", array( ":uniacid" => $_W["uniacid"], ":merchid" => $merchid ));
                        }
                        else
                        {
                            $stores = pdo_fetchall("select * from " . tablename("tapgo_tt_store") . " where id in (" . implode(",", $storeids) . ") and uniacid=:uniacid and status=1 order by displayorder desc,id desc", array( ":uniacid" => $_W["uniacid"] ));
                        }
        ...
        if( !empty($order["virtual"]) && !empty($order["virtual_str"]) ) 
        {
            ...
            $virtualtemp = pdo_fetch("SELECT linktext, linkurl FROM " . tablename("tapgo_tt_virtual_type") . " WHERE id=:id AND uniacid=:uniacid LIMIT 1", array( ":id" => $order["virtual"], ":uniacid" => $_W["uniacid"] ));
        }
    ...
    public function peerpay()
    {
        ...
        $peerpay = (int) pdo_fetchcolumn("select orderid from " . tablename("tapgo_tt_order_peerpay") . " where orderid=:id and uniacid=:uniacid limit 1", array( ":id" => $orderid, ":uniacid" => $uniacid ));
        if( !empty($peerpay) ) 
        {
        ...
        $order = pdo_fetch("select * from " . tablename("tapgo_tt_order") . " where id=:id and uniacid=:uniacid limit 1", array( ":id" => $orderid, ":uniacid" => $uniacid ));
        if( 100 <= $order["ordersn2"] ) 
        {
            $order["ordersn2"] = 0;
            pdo_update("tapgo_tt_order", array( "ordersn2" => 0 ), array( "id" => $orderid, "uniacid" => $uniacid ));
        }
        ...
        if( $_W["ispost"] ) 
        {
            ...
            $res = pdo_insert("tapgo_tt_order_peerpay", $data);
    ...
    public function peerpayshare()
    {
        ...
        $uniacid = $_W["uniacid"];
        $peerpay = pdo_fetch("select p.*,o.openid from " . tablename("tapgo_tt_order_peerpay") . " p join " . tablename("tapgo_tt_order") . " o on o.id=p.orderid where p.orderid=:id and p.uniacid=:uniacid limit 1", array( ":id" => $peerid, ":uniacid" => $uniacid ));
        if( empty($peerpay) ) 
        {
            header("location: " . mobileUrl("order"));
            exit();
        ...
        $peerpay_info = (double) pdo_fetchcolumn("select SUM(price) price from " . tablename("tapgo_tt_order_peerpay_payinfo") . " where pid=:pid limit 1", array( ":pid" => $peerpay["id"] ));
        ...
        $ordergoods = pdo_fetch("select * from " . tablename("tapgo_tt_order_goods") . " where orderid=:id and uniacid=:uniacid limit 1", array( ":id" => $peerpay["orderid"], ":uniacid" => $uniacid ));
        $goods = pdo_fetch("select * from " . tablename("tapgo_tt_goods") . " where id=:id and uniacid=:uniacid limit 1", array( ":id" => $ordergoods["goodsid"], ":uniacid" => $uniacid ));
    ...
    public function peerpaydetail()
    {
        ...
        $peerid = intval($_GPC["id"]);
        $uniacid = $_W["uniacid"];
        $peerpay = pdo_fetch("select p.*,o.openid,o.address,o.isparent,o.id as oid from " . tablename("tapgo_tt_order_peerpay") . " p join " . tablename("tapgo_tt_order") . " o on o.id=p.orderid where p.orderid=:id and p.uniacid=:uniacid limit 1", array( ":id" => $peerid, ":uniacid" => $uniacid ));
        if( empty($peerpay) ) 
        {
            header("location: " . mobileUrl("order"));
        }
        ...
        $goods = pdo_fetchall("SELECT g.id,og.goodsid,og.total,g.title,g.thumb,g.type,g.status,og.price,og.title as gtitle,og.optionname as optiontitle,og.optionid,op.specs,g.merchid,og.seckill,og.seckill_taskid,\n                og.sendtype,og.expresscom,og.expresssn,og.express,og.sendtime,og.finishtime,og.remarksend\n                FROM " . tablename("tapgo_tt_order_goods") . " og " . " left join " . tablename("tapgo_tt_goods") . " g on og.goodsid = g.id " . " left join " . tablename("tapgo_tt_goods_option") . " op on og.optionid = op.id " . " where " . $scondition . " and og.uniacid=:uniacid order by og.id asc", array( ":id" => $peerpay["orderid"], ":uniacid" => $uniacid ));
        ...
        {
            $message = pdo_fetchall("SELECT * FROM " . tablename("tapgo_tt_order_peerpay_payinfo") . " WHERE pid = :pid ORDER BY id DESC LIMIT 3", array( ":pid" => $peerpay["id"] ));
