...
class Pay_Alipay_TapgoTtPage extends MobilePage
{
	...
	public function complete()
	{
		...
		if (is_h5app()) {
			...
			if (!m('finance')->isAlipayNotify($_GET)) {
				$log = pdo_fetch('SELECT * FROM ' . tablename('core_paylog') . ' WHERE `uniacid`=:uniacid AND `module`=:module AND `tid`=:tid limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'tapgo_ttv2', ':tid' => $tid));
		...
		if ($log['status'] != 1) {
			...
			$orderid = pdo_fetchcolumn('select id from ' . tablename('tapgo_tt_order') . ' where ordersn=:ordersn and uniacid=:uniacid', array(':ordersn' => $log['tid'], ':uniacid' => $_W['uniacid']));
			...
			if (!empty($orderid)) {
				...
				pdo_update('tapgo_tt_order', $data_alipay, array('id' => $orderid));
			}
	...
	public function recharge_complete()
	{
		...
		$log = pdo_fetch('SELECT * FROM ' . tablename('tapgo_tt_member_log') . ' WHERE `logno`=:logno and `uniacid`=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':logno' => $logno));
		if (!empty($log) && empty($log['status'])) {
			pdo_update('tapgo_tt_member_log', array('status' => 1, 'rechargetype' => 'alipay', 'apppay' => is_h5app() ? 1 : 0, 'transid' => $transid), array('id' => $log['id']));
