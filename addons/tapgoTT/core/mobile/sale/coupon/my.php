...
class My_TapgoTtPage extends MobileLoginPage
{
	...
	public function detail()
	{
		...
		$num = pdo_fetchcolumn('select ifnull(count(*),0) from ' . tablename('tapgo_tt_coupon_data') . ' where couponid=:couponid and openid=:openid and uniacid=:uniacid and used=0 ', array(':couponid' => $coupon['id'], ':openid' => $_W['openid'], ':uniacid' => $_W['uniacid']));
		...
		if ($data['limitgoodcatetype'] == 1 && !empty($data['limitgoodcateids'])) {
			...
			if (0 < count($limitcateids)) {
				...
				foreach ($limitcateids as $cateid) {
					...
					$table .= 'select * from ' . tablename('tapgo_tt_goods') . ' where FIND_IN_SET(' . $cateid . ',cates) and deleted =0';
				}
