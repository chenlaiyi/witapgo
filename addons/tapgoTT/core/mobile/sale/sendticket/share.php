...
class Share_TapgoTtPage extends MobileLoginPage {
    ...
    function enjoy(){
        ...
        $openid = $_W['openid'];
        if (!empty($_GPC['sid'])) {
            $asql = 'SELECT * FROM '.tablename('tapgo_tt_sendticket_share').' WHERE uniacid = '.intval($_W['uniacid']).' AND id = '.intval($_GPC['sid']).' AND status = 1 AND (expiration = 0 OR (expiration = 1 AND endtime >= '.TIMESTAMP.'))';
        ...
        $csql = 'SELECT * FROM '.tablename('tapgo_tt_coupon_data').' WHERE uniacid = '.intval($_W['uniacid']).' AND shareident = "'.trim($_GPC['ident']).'" AND openid = "'.$openid.'"';
        ...
        if (!empty($clist)) {
            ...
            if(!empty($cpid)){
                ...
                foreach ($cpid as $ck => $cv) {
                    $couponsql = 'SELECT * FROM '.tablename('tapgo_tt_coupon').' WHERE uniacid = '.intval($_W['uniacid']).' AND id = '.$cv;
        ...
        $storesql = 'SELECT * FROM '.tablename('tapgo_tt_sysset').' WHERE uniacid = '.intval($_W['uniacid']);
    ...
    function getCoupon($cpids){
        global $_W;
        $sql = 'SELECT * FROM '.tablename('tapgo_tt_coupon').' WHERE  uniacid = '.intval($_W['uniacid']).' AND id IN ('.$cpids.')';
    ...
    function sendTicket($openid, $couponid,$gettype=0,$num,$ident='') {
        ...
        for($i = 0;$i<$num;$i++){
            ...
            $datas = pdo_insert('tapgo_tt_coupon_data', $data);
        }
    ...
    function shareCoupon(){
        ...
        if($shareOpenid == $openid){
            ...
            pdo_update('tapgo_tt_order',$data,array('id'=>intval($_GPC['orderid'])));
    ...
    function shareSuccess(){
        ...
        $openid = $_W['openid'];
            ...
            $orderSql = 'SELECT isshare FROM '.tablename('tapgo_tt_order').' WHERE uniacid = '.intval($_W['uniacid']).' AND id = '.intval($_GPC['orderid']);
            ...
            $couponDataSql = 'SELECT * FROM '.tablename('tapgo_tt_coupon_data').' WHERE uniacid = '.intval($_W['uniacid']).' AND shareident = "'.$_GPC['ident'].'"';
