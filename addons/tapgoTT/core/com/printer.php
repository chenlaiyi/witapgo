...
class Printer_TapgoTtComModel extends ComModel
{
    const PRINTRT_365 = 0;
        ...
        if ($type=='0' || $type=='1'){
            switch ($code){
                ...
                case self::CONTENT_CODE:
                    if ($type == 1)
                    {
                        $res = bar_code($str) . '<BR>';break;
                    }else
                    {
                        $res = '<QR>'. $str.'</QR><BR>';break;
                    }
                case self::CONTENT_QRCODE:
                    $res = '<QR>'.$str.'</QR>';break;
                case self::CONTENT_BR:
                    if ($type == 1)
                    {
                        $res = bar_code($str) . '<BR>';break;
                    }else
                    {
                        $res = '<BR3>'.$str.'</BR3>';break;
                    }
                default:
                    $res = $str.'<BR>';break;
            }
    ...
    public function printer($params,$templateid,$printerid,$type=0)
    {
        ...
        $printer = pdo_fetch("SELECT * FROM ".tablename('tapgo_tt_member_printer')." WHERE uniacid=:uniacid AND id=:id",array(':uniacid'=>$_W['uniacid'],':id'=>$printerid));
        if (empty($printer)){
            return error(-1,'小票打印机未找到');
        }
        $template = pdo_fetch("SELECT * FROM ".tablename('tapgo_tt_member_printer_template')." WHERE uniacid=:uniacid AND id=:id",array(':uniacid'=>$_W['uniacid'],':id'=>$templateid));
        if (empty($printer)){
    ...
    public function buildContent($template,$type=0)
    {
        ...
        $tel = $this->params['mobile'];
        if ($template['tel_code_type'] > 0)
        {
            switch ($template['tel_code_type'])
            {
                case 1:
                    $code = self::CONTENT_BR;
                    break;
                case 2:
                    $code = self::CONTENT_QRCODE;
                    break;
            }
            $str .= "手机号:" . $this->style_list_code($tel,$code,$type) . "\n" ;
        }


        if ($template['ordersn_code_type'] > 0)
        {
            $str .=  '订单码:'.$this->style_list_code($ordersn ,self::CONTENT_QRCODE,$type) . "\n" ;
        }
...
//        $str = $this->getStr($selfMessage);
    ...
    public function getPrinterSet($merchid=0)
    {
        ...
        if (!empty($data['order_printer'])){
            $data['order_printer'] = pdo_fetchall('SELECT id,title FROM ' . tablename('tapgo_tt_member_printer') . ' WHERE uniacid=:uniacid AND id IN ('.$data['order_printer'].')', array(':uniacid' => $_W['uniacid']));
        }else{
    ...
    public function getStorePrinterSet($store)
    {
        ...
        if (!empty($store['order_printer'])){
            $store['order_printer'] = pdo_fetchall('SELECT id,title FROM ' . tablename('tapgo_tt_member_printer') . ' WHERE uniacid=:uniacid AND id IN ('.$store['order_printer'].')', array(':uniacid' => $_W['uniacid']));
        }else{
...
//        $this->params = $params;
    ...
    public function sendOrderMessage($orderid = 0,$verify = array())
    {
        ...
        $order = pdo_fetch('select * from ' . tablename('tapgo_tt_order') . ' where id=:id and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $orderid));
        ...
        if($status == 3 && !empty($order['virtual']) && com('virtual')){
            //虚拟卡密自动发货的时候,支付完成算付款打印
            $status = 2;
        }
        /*if($status == 3 && !empty($order['cashtime']) && $order['paytype'] == 3){
            //如果使用货到付款,那么订单完成的时候,算付款打印
            $status = 2;
        }*/
        if($status == 1 && $order['paytype'] == 3){
            //如果使用货到付款,那么点击货到付款的时候,算付款打印
            $status = 2;
        }
        ...
        $order_goods = pdo_fetchall('select g.id,g.title,g.shorttitle,og.realprice,og.total,og.price,og.optionname as optiontitle,g.noticeopenid,g.noticetype,og.goodssn,og.productsn from ' . tablename('tapgo_tt_order_goods') . ' og '
        ...
        if (!empty($order['address'])) {
            ...
            if ((is_array($address) && empty($address)) || !is_array($address)) {
                ...
                if (!is_array($address)) {
                    $address = pdo_fetch('select id,realname,mobile,address,province,city,area from ' . tablename('tapgo_tt_member_address') . ' where id=:id and uniacid=:uniacid limit 1'
        ...
        if (!empty($order['verifystoreid'])){
            $store = pdo_fetch('select * from ' . tablename('tapgo_tt_store') . " where id=:id and uniacid=:uniacid limit 1", array(':id' => $order['verifystoreid'], ':uniacid' => $_W['uniacid']));
            //获取打印设置
            $StorePrinterSet = $this->getStorePrinterSet($store);
        }
        ...
        $params = array(
            ...
            'data' => $goods,
            'discount' => (float)$order['goodsprice']-$order['price']+(float)$order['dispatchprice'],//各种优惠金额
        ...
        $myfile = fopen("/www/wwwroot/w.itapgo.com/data/newfile6.txt", "w") or die("Unable to open file!");
        $txt =json_encode($res);
        fwrite($myfile, $txt);
        fclose($myfile);
        return $res;
    ...
    public function sendCashierMessage($log,$templateid,$printerid,$operator)
    {
        ...
        if (!empty($log['isgoods'])){
            $goodsAll = pdo_fetchall("SELECT g.title,g.goodssn,og.price,og.total FROM ".tablename('tapgo_tt_cashier_goods')." g LEFT JOIN ".tablename('tapgo_tt_cashier_pay_log_goods')." og ON g.id=og.goodsid WHERE og.cashierid=:cashierid AND og.logid=:logid",array(':cashierid'=>$log['cashierid'],':logid'=>$log['id']));
        ...
        if (!empty($log['orderid'])){
            $order_goods = pdo_fetchall('select g.id,g.title,g.shorttitle,og.realprice,og.total,og.price,og.optionname as optiontitle,g.noticeopenid,g.noticetype from ' . tablename('tapgo_tt_order_goods') . ' og '
                . ' left join ' . tablename('tapgo_tt_goods') . ' g on g.id=og.goodsid '
