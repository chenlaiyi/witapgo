...
class Coupon_TapgoTtComModel extends ComModel {

    protected function getUrl($do, $query = null) {

        $url = mobileUrl($do, $query, true);
        if (strexists($url, '/addons/tapgo_tt/')) {
            $url = str_replace("/addons/tapgo_tt/", '/', $url);
        }
    ...
    function get_last_count($couponid = 0) {
        global $_W;
        $coupon = pdo_fetch('SELECT id,total FROM ' . tablename('tapgo_tt_coupon') . ' WHERE id=:id and uniacid=:uniacid ', array(':id' => $couponid, ':uniacid' => $_W['uniacid']));
        if (empty($coupon)) {
            return 0;
        }
        if ($coupon['total'] == -1) {
        ...
        $gettotal = pdo_fetchcolumn('select count(*) from ' . tablename('tapgo_tt_coupon_data') . ' where couponid=:couponid and uniacid=:uniacid ', array(':couponid' => $couponid, ':uniacid' => $_W['uniacid']));
    ...
    function creditshop($logid = 0) {
        ...
        $log = pdo_fetch('SELECT * FROM ' . tablename('tapgo_tt_creditshop_log') . ' WHERE `id`=:id and `uniacid`=:uniacid  limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $logid));
        if (!empty($log)) {
            ...
            pdo_insert('tapgo_tt_coupon_log', $couponlog);
            ...
            $coupon = pdo_fetch('select * from ' . tablename('tapgo_tt_coupon') . ' where id=:id limit 1', array(':id' => $log['couponid']));
            ...
            pdo_update('tapgo_tt_creditshop_log', array('status' => 3), array('id' => $logid));
        }
    }
    ...
    function getAvailableCoupons($type, $money = 0, $merch_array,$goods_array=array()){
        ...
        if (!empty($merch_array)) {
            foreach ($merch_array as $key => $value) {
                ...
                if ($merchid > 0) {
                    ...
                    $sql = "select d.id,d.couponid,d.gettime,c.timelimit,c.timedays,c.timestart,c.timeend,c.thumb,c.couponname,c.enough,c.backtype,c.deduct,c.discount,c.backmoney,c.backcredit,c.backredpack,c.bgcolor,c.thumb,c.merchid,c.limitgoodcatetype,c.limitgoodtype,c.limitgoodcateids,c.limitgoodids  from " . tablename('tapgo_tt_coupon_data') . " d";
                    $sql.=" left join " . tablename('tapgo_tt_coupon') . " c on d.couponid = c.id";
        ...
        if (!empty($goods_array)) {
            foreach ($goods_array as $key => $value) {
                ...
                $sql = "select id,cates,marketprice,merchid,`type`,discounts,istime,isdiscount,timestart,timeend,isdiscount_time,isdiscount_discounts   from " . tablename('tapgo_tt_goods') ;
                ...
                foreach($goodlist as $good)
                {
                    ...
                    if($row['enough']>0&&$p==1)
                    {
                        if($good['optionid']>0&&$good['type']!=4)
                        {
                            ...
                            $sql = "select  marketprice  from " . tablename('tapgo_tt_goods_option') ;
                            $sql.=" where uniacid=:uniacid and id =:id order by id desc LIMIT 1 "; 
    ...
    function payResult($logno) {
        ...
        $log = pdo_fetch('SELECT * FROM ' . tablename('tapgo_tt_coupon_log') . ' WHERE `logno`=:logno and `uniacid`=:uniacid  limit 1', array(':uniacid' => $_W['uniacid'], ':logno' => $logno));
        if (empty($log)) {
        ...
        if (empty($log['status'])) {
            ...
            $update['status'] = 1;
            pdo_update('tapgo_tt_coupon_log', $update, array('id' => $log['id']));
            $data = array(
                'uniacid' => $_W['uniacid'],
                'merchid' => $log['merchid'],
                'openid' => $log['openid'],
    ...
    function sendMessage($coupon, $send_total, $member, $account = null) {
        ...
        if(strexists($siteroot,'/addons/tapgoTT/')){
            $replace_str =  array("/addons/tapgoTT/");
    ...
    function useRechargeCoupon($log) {
        ...
        $data = pdo_fetch('select id,openid,couponid,used from ' . tablename('tapgo_tt_coupon_data') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $log['couponid'], ':uniacid' => $_W['uniacid']));
        if (empty($data)) {
            return;
        }
        if (!empty($data['used'])) {
        ...
        $coupon = pdo_fetch('select enough,backcredit,backmoney,backredpack,couponname from ' . tablename('tapgo_tt_coupon') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $data['couponid'], ':uniacid' => $_W['uniacid']));
        if (empty($coupon)) {
            return;
        }
        ...
        pdo_update('tapgo_tt_coupon_data', array('used' => 1, 'usetime' => time(), 'ordersn' => $log['logno']), array('id' => $data['id']));
    ...
    function consumeCouponCount($openid, $money = 0, $merch_array,$goods_array) {
        ...
        $param[':openid'] = $openid;
        $param[':uniacid'] = $_W['uniacid'];
        $sql = "select d.id,d.couponid,c.enough,c.merchid,c.limitgoodtype,c.limitgoodcatetype,c.limitgoodcateids,c.limitgoodids,c.limitdiscounttype  from " . tablename('tapgo_tt_coupon_data') . " d";
        $sql.=" left join " . tablename('tapgo_tt_coupon') . " c on d.couponid = c.id";
        ...
        if (!empty($merch_array)) {
            foreach ($merch_array as $key => $value) {
                ...
                if ($merchid > 0) {
                    ...
                    $param[':merchid'] = $merchid;
                    $sql = "select d.id,d.couponid,c.enough,c.merchid,c.limitgoodtype,c.limitgoodcatetype,c.limitgoodcateids,c.limitgoodids  from " . tablename('tapgo_tt_coupon_data') . " d";
                    $sql.=" left join " . tablename('tapgo_tt_coupon') . " c on d.couponid = c.id";
    ...
    function rechargeCouponCount($openid, $money = 0) {
        ...
        $sql = "select count(*) from " . tablename('tapgo_tt_coupon_data') . " d "
            . "  left join " . tablename('tapgo_tt_coupon') . " c on d.couponid = c.id "
    ...
    function useConsumeCoupon($orderid = 0) {
        ...
        $order = pdo_fetch('select ordersn,createtime,couponid from ' . tablename('tapgo_tt_order') . ' where id=:id and status>=0 and uniacid=:uniacid limit 1', array(':id' => $orderid, ':uniacid' => $_W['uniacid']));
        if (empty($order)) {
            return;
        }
        ...
        pdo_update('tapgo_tt_coupon_data', array('used' => 1, 'usetime' => $order['createtime'], 'ordersn' => $order['ordersn']), array('id' => $order['couponid']));
    ...
    function returnConsumeCoupon($order) {
        global $_W;
        if (!is_array($order)) {
            $order = pdo_fetch('select id,openid,ordersn,createtime,couponid,status,finishtime from ' . tablename('tapgo_tt_order') . ' where id=:id and status>=0 and uniacid=:uniacid limit 1', array(':id' => intval($order), ':uniacid' => $_W['uniacid']));
        }
        ...
        if (!empty($coupon['used']) && ($coupon['ordersn'] == $order['ordersn'])) {
            pdo_update('tapgo_tt_coupon_data', array('used' => 0, 'usetime' => 0, 'ordersn' => ''), array('id' => $order['couponid']));
    ...
    function backConsumeCoupon($orderid) {
        ...
        if (!is_array($orderid)) {
            $order = pdo_fetch('select id,openid,ordersn,createtime,couponid,couponmerchid,status,finishtime,`virtual`,isparent,parentid,coupongoodprice  from ' . tablename('tapgo_tt_order') . ' where id=:id and status>=0 and couponid >0 and uniacid=:uniacid limit 1', array(':id' => intval($orderid), ':uniacid' => $_W['uniacid']));
        }
        ...
        if ($isparent == 1 || $parentid != 0) {
            ...
            if ($isparent == 1) {
                ...
                if ($couponmerchid > 0) {
                    $sql = 'select id,openid,ordersn,createtime,couponid,couponmerchid,status,finishtime,`virtual`,isparent,parentid from ' . tablename('tapgo_tt_order') . ' where parentid=:parentid and couponmerchid=:couponmerchid and status>=0 and uniacid=:uniacid limit 1';
        ...
        if ($parentid != 0 && $couponmerchid == 0) {
            if ($all_done == 1) {
                $sql = 'select id,openid,ordersn,createtime,couponid,couponmerchid,status,finishtime,`virtual`,isparent,parentid from ' . tablename('tapgo_tt_order') . ' where id=:id and status>=0 and uniacid=:uniacid limit 1';
        ...
        if ($canback) {
            ...
            if($parentid>0)
            {
                $ordermoney = pdo_fetchcolumn('select coupongoodprice from ' . tablename('tapgo_tt_order') . ' where id=:id and status>=0 and couponid >0 and uniacid=:uniacid limit 1', array(':id' => intval($parentid), ':uniacid' => $_W['uniacid']));
            }else
            {
            ...
            if($ordermoney==0)
            {
                $sql = 'select ifnull( sum(og.realprice),0) from ' . tablename('tapgo_tt_order_goods') . ' og ';
                $sql .= ' left join ' . tablename('tapgo_tt_order') . ' o on';
            ...
            pdo_update('tapgo_tt_coupon_data', array('back' => 1, 'backtime' => time()), array('id' => $order['couponid']));
    ...
    function getCoupon($couponid = 0) {
        ...
        return pdo_fetch('select * from ' . tablename('tapgo_tt_coupon') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $couponid, ':uniacid' => $_W['uniacid']));
    }

    function getCouponByDataID($dataid = 0) {
        global $_W;
        $data = pdo_fetch('select id,openid,couponid,used,back,backtime,ordersn from ' . tablename('tapgo_tt_coupon_data') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $dataid, ':uniacid' => $_W['uniacid']));
        if (empty($data)) {
            return false;
        }
        $coupon = pdo_fetch('select * from ' . tablename('tapgo_tt_coupon') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $data['couponid'], ':uniacid' => $_W['uniacid']));
        if (empty($coupon)) {
            return false;
        }
    ...
    function setCoupon($row, $time, $withOpenid = true) {
        ...
        if ($withOpenid) {
            ...
            if ($row['getmax'] > 0) {
                $gets = pdo_fetchcolumn('select count(*) from ' . tablename('tapgo_tt_coupon_data') . ' where couponid=:couponid and openid=:openid and uniacid=:uniacid and gettype=1 limit 1', array(':couponid' => $row['id'], ':openid' => $openid, ':uniacid' => $_W['uniacid']));
    ...
    function addtaskdata($orderid)
    {
        ...
        $order = pdo_fetch('select id,openid,price  from ' . tablename('tapgo_tt_order') . ' where id=:id   and uniacid=:uniacid limit 1', array(':id' => intval($orderid), ':uniacid' => $_W['uniacid']));
        ...
        {
            $price = $order['price'];
            $sendtasks = pdo_fetch('select id,couponid,sendnum,num,sendpoint  from ' . tablename('tapgo_tt_coupon_sendtasks') . '
            ...
            if(!empty($sendtasks))
            {
                ...
                pdo_insert('tapgo_tt_coupon_taskdata', $data);

                $num = intval($sendtasks['num'])-intval($sendtasks['sendnum']);
                pdo_update('tapgo_tt_coupon_sendtasks', array('num' => $num), array('id' => $sendtasks['id']));
            }
        }
        ...
        if($pdata['isopengoodssendtask'] == 1)
        {
            ...
            from '. tablename('tapgo_tt_coupon_goodsendtask').' gst
            inner join ' . tablename('tapgo_tt_order_goods') . ' og on og.goodsid =gst.goodsid  and (orderid=:orderid or parentorderid=:orderid)
            ...
            foreach($goodssendtasks as $task)
            {
                if ($task['starttime'] <= TIMESTAMP && $task['endtime'] >= TIMESTAMP){
                    ...
                    pdo_update('tapgo_tt_coupon_goodsendtask', array('num' => $num), array('id' => $task['taskid']));
                }
            }
    ...
    function sendcouponsbytask($orderid) {
        ...
        if (!is_array($orderid)) {
            $order = pdo_fetch('select id,openid,ordersn,createtime,status,finishtime,`virtual`,isvirtualsend,isparent,parentid  from ' . tablename('tapgo_tt_order') . ' where id=:id and status>=0  and uniacid=:uniacid limit 1', array(':id' => intval($orderid), ':uniacid' => $_W['uniacid']));
        }
        if (empty($order)) {
            return;
        }
        ...
        if($order['status'] == 2){
            $sql = 'SELECT goodsid FROM '.tablename('tapgo_tt_order_goods').' WHERE uniacid=:uniacid AND orderid=:orderid ';
            $goodsids = pdo_fetchall($sql,array(':uniacid'=>$_W['uniacid'],':orderid'=>$order['id']));
            foreach($goodsids as $gidk => $gidv){
                $gtype = pdo_fetch('SELECT type FROM '.tablename('tapgo_tt_goods').' WHERE uniacid=:uniacid AND id=:id',array(':uniacid'=>$_W['uniacid'],':id'=>$gidv['goodsid']));
                if($gtype['type'] == 5){
        ...
        if ($isonlyverifygoods || $order['status'] == 1)
        {
            ...
            if($parentid>0)
            {
                $num = pdo_fetchcolumn('select 1 from '.tablename('tapgo_tt_order') .'
    ...
    function getOrderSendCoupons($orderid,$sendpoint,$tasktype,$openid)
    {
        ...
        if($sendpoint ==2)
        {
            $taskdata = pdo_fetchall('select id, couponid, sendnum  from ' . tablename('tapgo_tt_coupon_taskdata') . '
    ...
    function posterbylist($list ,$openid,$gettype)
    {
        ...
        foreach($list  as $taskdata)
        {
            ...
            $num+=$couponnum;
            $merchid = pdo_fetchcolumn("select merchid from " . tablename('tapgo_tt_coupon') . ' where id=:couponid limit 1', array(':couponid' => $taskdata['couponid']));
            ...
            for ($i = 1; $i <= $couponnum; $i++) {
                ...
                pdo_insert('tapgo_tt_coupon_sendshow', $data);
            }

            pdo_update('tapgo_tt_coupon_taskdata', array('status' => 1), array('id' => $taskdata['id']));
        }
    ...
    function posterbyusesendtask($couponid,$openid)
    {
        ...
        $list = pdo_fetchall('select  *  from ' . tablename('tapgo_tt_coupon_usesendtasks') . '
        ...
        foreach($list  as $taskdata)
        {
            ...
            pdo_update('tapgo_tt_coupon_usesendtasks', array('num' => $num2), array('id' => $taskdata['id']));
        }
    ...
    function getCoupons() {
        ...
        return pdo_fetchall('select * from ' . tablename('tapgo_tt_coupon') . ' where  (timelimit = 0  or  (timelimit =1 and timestart<={$time} && timeend>={$time})) and uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
    }
    ...
    function getInfo(){
        ...
        $osql = 'SELECT `id` FROM '.tablename('tapgo_tt_order').$condition;
    ...
    function insertDraw($openid,$cpinfo){
        ...
        $drawsql = 'SELECT * FROM '.tablename('tapgo_tt_sendticket_draw').' WHERE  openid = :openid AND uniacid = :uniacid ';
        $drawparpams = array(
            ':uniacid' => intval($_W['uniacid']),
            ':openid' => trim($openid),
        );
        ...
        if (empty($drawdata)) {
            ...
            $insert = pdo_insert('tapgo_tt_sendticket_draw',$data);
            return $insert;
        } else {
            return false;
        }
    ...
    function getCanGetCouponNum($merchid=0){
        ...
        $sql = "select id,timelimit,coupontype,timedays,timestart,timeend,couponname,enough,backtype,deduct,discount,backmoney,backcredit,backredpack,bgcolor,thumb,credit,money,getmax,merchid,total as t,tagtitle,settitlecolor,titlecolor  from " . tablename('tapgo_tt_coupon');
    ...
    function sethasnewcoupon($openid,$hasnewcoupon=0){
        global $_W;
        pdo_update('tapgo_tt_member', array('hasnewcoupon'=>$hasnewcoupon), array('openid'=>$openid,'uniacid'=>$_W['uniacid']));
    }
