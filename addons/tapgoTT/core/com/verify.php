...
class Verify_TapgoTtComModel extends ComModel {
    ...
    public function allow($orderid, $times = 0,$verifycode = '',$openid = '') {
        ...
        if (empty($merchid)) {
            $saler = pdo_fetch('select * from ' . tablename('tapgo_tt_saler') . ' where openid=:openid and uniacid=:uniacid and status=1 limit 1', array(
                ':uniacid' => $_W['uniacid'], ':openid' => $openid
            ));
        } else {
            if ($merch_plugin) {
                $saler = pdo_fetch('select * from ' . tablename('tapgo_tt_merch_saler') . ' where openid=:openid and uniacid=:uniacid and status=1 and merchid=:merchid limit 1', array(
        ...
        $allgoods = pdo_fetchall("select og.goodsid,og.price,g.title,g.thumb,og.total,g.credit,og.optionid,o.title as optiontitle,g.isverify,g.storeids,g.status".$sqlstr." from " . tablename('tapgo_tt_order_goods') . " og "
            . " left join " . tablename('tapgo_tt_goods') . " g on g.id=og.goodsid "
            . " left join " . tablename('tapgo_tt_goods_option') . " o on o.id=og.optionid "
    ...
    public function verify($orderid = 0, $times = 0,$verifycode = '',$openid = '') {
        ...
        if ($order['isverify'] || $order['istrade']) {
            if ($order['verifytype'] == 0 || $order['verifytype'] == 3) {
                ...
                pdo_insert('tapgo_tt_verifyorder_log', $verifyorder_log);
                if ($newstore_plugin && !empty($order['istrade'])) {
                    ...
                    pdo_insert('tapgo_tt_newstore_trade_log', $insert_data);
                }
...
//
                if ($order['status'] != 3) {
                    pdo_update('tapgo_tt_order', array('status' => 3, 'sendtime' => $current_time, 'finishtime' => $current_time), array('id' => $order['id']));
                ...
                if ($order['status'] != 3) {
                    pdo_update('tapgo_tt_order', array('status' => 3, 'sendtime' => $current_time, 'finishtime' => $current_time, 'verifytime' => $current_time, 'verified' => 1, 'verifyopenid' => $openid, 'verifystoreid' => $saler['storeid']), array('id' => $order['id']));
...
//                    $this->finish(array('status' => 3, 'sendtime' => $current_time, 'finishtime' => $current_time, 'verifytime' => $current_time, 'verified' => 1, 'verifyopenid' => $openid, 'verifystoreid' => $saler['storeid']),$order);
                ...
                pdo_update('tapgo_tt_order', array('verifyinfo' => iserializer($verifyinfo)), array('id' => $order['id']));
                $verifyorder_log = array();
                $verifyorder_log['uniacid'] = $_W['uniacid'];
                $verifyorder_log['orderid'] = $order['id'];
                $verifyorder_log['salerid'] = $saler['id'];
            ...
            pdo_update('tapgo_tt_order', array('status' => 3, 'fetchtime' => $current_time,'sendtime'=>$current_time, 'finishtime' => $current_time,'verifytime' => $current_time, 'verified' => 1, 'verifyopenid' => $openid, 'verifystoreid' => $saler['storeid']), array('id' => $order['id']));
...
//        m('member')->upgradeLevel($openid, $order['id']);
    ...
    public function getSalerInfo($openid, $merchid = 0) {
        ...
        if (empty($merchid)) {
            $table_name = tablename('tapgo_tt_saler');
        } else {
            $table_name = tablename('tapgo_tt_merch_saler');
        ...
        $sql = "SELECT m.id as salerid,m.nickname as salernickname,s.salername FROM {$table_name}  s "
            . " left join " . tablename('tapgo_tt_member') . " m on s.openid=m.openid and m.uniacid = s.uniacid "
    ...
    public function getStoreInfo($storeid, $merchid = 0) {
        ...
        if (empty($merchid)) {
            $table_name = tablename('tapgo_tt_store');
        } else {
            $table_name = tablename('tapgo_tt_merch_store');
