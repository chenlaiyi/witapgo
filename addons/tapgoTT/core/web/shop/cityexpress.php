...
class Cityexpress_TapgoTtPage extends WebPage
{
	public function main()
	{
		...
		$makeExpress = m('common')->getPluginset('makeexpress');
		...
		if ($_W['ispost']) {
			...
			if (!empty($cityexpress)) {
				...
				pdo_update('tapgo_tt_city_express', $data, array('id' => $cityexpress['id']));
			}
			else {
				pdo_insert('tapgo_tt_city_express', $data);
				$id = pdo_insertid();
