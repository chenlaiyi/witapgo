...
class Index_TapgoTtPage extends ComWebPage 
{
	...
	public function enoughfree() 
	{
		...
		{
			$goods = pdo_fetchall('SELECT id,uniacid,title,thumb FROM ' . tablename('tapgo_tt_goods') . ' WHERE uniacid=:uniacid AND id IN (' . implode(',', $data['goodsids']) . ')', array(':uniacid' => $_W['uniacid']));
		}
