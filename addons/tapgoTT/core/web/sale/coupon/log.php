...
class Log_TapgoTtPage extends ComWebPage {
    ...
    function main() {
        ...
        if (!empty($couponid)) {
            $coupon = pdo_fetch('select * from ' . tablename('tapgo_tt_coupon') . ' where id=:id and uniacid=:uniacid and merchid=0 limit 1', array(':id' => $couponid, ':uniacid' => $_W['uniacid']));
        ...
        $sql = 'SELECT d.*, c.coupontype,c.couponname,m.nickname,m.avatar,m.realname,m.mobile FROM ' . tablename('tapgo_tt_coupon_data') . " d "
            . " left join " . tablename('tapgo_tt_coupon') . " c on d.couponid = c.id "
            . " left join " . tablename('tapgo_tt_member') . " m on m.openid = d.openid and m.uniacid = d.uniacid "
        ...
        $total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('tapgo_tt_coupon_data') . " d "
            . " left join " . tablename('tapgo_tt_coupon') . " c on d.couponid = c.id "
            . " left join " . tablename('tapgo_tt_member') . " m on m.openid = d.openid and m.uniacid = d.uniacid "
