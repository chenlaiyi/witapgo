...
class Goods_rank_detail_TapgoTtPage extends WebPage {
    ...
    function main() {
        ...
        $sql .= "FROM ".tablename('tapgo_tt_order_goods')." og ";
        $sql .= "LEFT JOIN ".tablename('tapgo_tt_order')." o ON og.orderid = o.id ";
        $sql .= "LEFT JOIN ".tablename('tapgo_tt_order_refund')." re ON o.id = re.orderid AND re.STATUS = 1 ";
        $sql .= "LEFT JOIN ".tablename('tapgo_tt_goods')." g ON g.id = og.goodsid ";
        ...
        if (empty($list)) {
            ...
            $sql .= "FROM " . tablename('tapgo_tt_order_goods') . " og ";
            $sql .= "LEFT JOIN ".tablename('tapgo_tt_order')." o ON og.orderid = o.id  ";
            $sql .= "LEFT JOIN ".tablename('tapgo_tt_order_refund')." re ON og.orderid = re.orderid AND re.STATUS = 1 ";
            $sql .= "LEFT JOIN ".tablename('tapgo_tt_goods')." g ON g.id = og.goodsid ";
        ...
        $groups = pdo_fetchall("SELECT id,name,goodsids FROM " . tablename('tapgo_tt_goods_group') . " WHERE 1 {$con}  ORDER BY id DESC", $params_g);
