...
class Refund_TapgoTtPage extends WebPage {
...
//                    $item['openid'] = $member['openid_wa'];
                ...
                if ($item['paytype'] == 1) {
                    ...
                    if ($item['apppay']==2){
                        ...
                        //微信支付，走退款 接口
                        $order=pdo_get('tapgo_tt_order', array('id' => $item['id']));
                        if($order['isunion']==1){
                            
                            $result = m('finance')->unipay($item['openid'], 1, $pay_refund_price * 100, $refund['refundno'], $item['ordersn']);
...
//                }
        ...
        //查询退货地址
        $refund_address = pdo_fetchall('select * from '.tablename('tapgo_tt_refund_address').' where uniacid=:uniacid and merchid=0',array(':uniacid'=>$_W['uniacid']));
    ...
    function main() {
        ...
        $goods = pdo_fetchall("SELECT g.*, o.goodssn as option_goodssn, o.productsn as option_productsn,o.total,g.type,o.optionname,o.optionid,o.price as orderprice,o.realprice,o.changeprice,o.oldprice,o.commission1,o.commission2,o.commission3,o.commissions {$diyformfields} FROM " . tablename('tapgo_tt_order_goods') .
            " o left join " . tablename('tapgo_tt_goods') . " g on o.goodsid=g.id " . " WHERE o.orderid=:orderid and o.uniacid=:uniacid", array(':orderid' => $id, ':uniacid' => $_W['uniacid']));
        foreach ($goods as &$r) {
