...
class Export_TapgoTtPage extends WebPage 
{
	...
	public function main() 
	{
		...
		if ($_GPC['export'] == 1) 
		{
			...
			$sql = 'select o.* , a.realname as arealname,m.level,a.mobile as amobile,a.province as aprovince ,a.city as acity,a.area as aarea, a.street as astreet,a.address as aaddress,d.dispatchname,m.nickname,m.id as mid,m.realname as mrealname,m.mobile as mmobile,m.groupid,m.uid,sm.id as salerid,sm.nickname as salernickname,s.salername from ' . tablename('tapgo_tt_order') . ' o' . ' left join ' . tablename('tapgo_tt_order_refund') . ' r on r.id =o.refundid ' . ' left join ' . tablename('tapgo_tt_member') . ' m on m.openid=o.openid and m.uniacid =  o.uniacid ' . ' left join ' . tablename('tapgo_tt_member_address') . ' a on a.id=o.addressid ' . ' left join ' . tablename('tapgo_tt_dispatch') . ' d on d.id = o.dispatchid ' . ' left join ' . tablename('tapgo_tt_member') . ' sm on sm.openid = o.verifyopenid and sm.uniacid=o.uniacid' . ' left join ' . tablename('tapgo_tt_saler') . ' s on s.openid = o.verifyopenid and s.uniacid=o.uniacid' . ' where ' . $condition . ' ' . $statuscondition . ' ORDER BY o.createtime DESC,o.status DESC  ';
			$list = pdo_fetchall($sql, $paras);
			$userlevel = pdo_fetchall('SELECT * FROM ' . tablename('tapgo_tt_member_level') . ' WHERE uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
			if (!(empty($userlevel))) 
			{
			...
			$list_group = pdo_fetchall('SELECT * FROM ' . tablename('tapgo_tt_member_group') . ' WHERE uniacid=:uniacid', array(':uniacid' => $_W['uniacid']), 'id');
			...
			foreach ($list as &$value ) 
			{
				...
				if (!(empty($value['storeid']))) 
				{
					if (0 < $value['merchid']) 
					{
						$value['pickname'] = pdo_fetchcolumn('select storename from ' . tablename('tapgo_tt_merch_store') . ' where id=:storeid limit 1 ', array(':storeid' => $value['storeid']));
					}
					else 
					{
						$value['pickname'] = pdo_fetchcolumn('select storename from ' . tablename('tapgo_tt_store') . ' where id=:storeid limit 1 ', array(':storeid' => $value['storeid']));
					}
				}
				if (!(empty($value['verifystoreid']))) 
				{
					if (0 < $value['merchid']) 
					{
						$value['storeinfo'] = pdo_fetchcolumn('select storename from ' . tablename('tapgo_tt_merch_store') . ' where id=:storeid limit 1 ', array(':storeid' => $value['verifystoreid']));
					}
					else 
					{
						$value['storeinfo'] = pdo_fetchcolumn('select storename from ' . tablename('tapgo_tt_store') . ' where id=:storeid limit 1 ', array(':storeid' => $value['verifystoreid']));
					}
				}
		...
		$stores = pdo_fetchall('select id,storename from ' . tablename('tapgo_tt_store') . ' where uniacid=:uniacid ', array(':uniacid' => $_W['uniacid']));
