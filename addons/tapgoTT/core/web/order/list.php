<?php
/*
 * @ PHP 5.6 解密时间 2020/11/29 11点35分
 */
...
class List_TapgoTtPage extends WebPage
{
    protected function orderData($status, $st, $index)
    {
        ...
        if ($order_type !== "") {
            ...
            if ($order_type == "general") {
                if (in_array($searchfield, array("goodstitle", "goodssn", "goodsoptiontitle"))) {
                    ...
                    $fullback_sqlcondition = " inner join ( select DISTINCT(og.orderid) from " . tablename("tapgo_tt_order_goods") . " og  where og.uniacid = '" . $uniacid . "' and og.fullbackid=0) gs on gs.orderid=o.id";
                }
            } else {
                if ($order_type == "fullback") {
                    ...
                    if (in_array($searchfield, array("goodstitle", "goodssn", "goodsoptiontitle"))) {
                        ...
                        $fullback_sqlcondition = " inner join ( select DISTINCT(og.orderid) from " . tablename("tapgo_tt_order_goods") . " og  where og.uniacid = '" . $uniacid . "' and og.fullbackid<>0 " . $fullback_goods_where . ") gs on gs.orderid=o.id";
                    }
                }
            }
        }
                                    ...
                                    if ($searchfield == "saler") {
                                        ...
                                        if ($searchfield == "verifycode") {
                                            ...
                                            if ($searchfield == "store") {
                                                if (p("merch")) {
                                                    $sqlcondition = " left join " . tablename("tapgo_tt_store") . " store on ( store.id = vgl.storeid or store.id = vol.storeid or store.id = o.verifystoreid) and store.uniacid=o.uniacid and o.ismerch=0\n                    left join " . tablename("tapgo_tt_merch_store") . " merstore on (  merstore.id = o.verifystoreid or merstore.id = vgl.storeid or merstore.id = vol.storeid )and merstore.uniacid=o.uniacid and o.ismerch=1";
                                                    $condition .= " AND (locate(:keyword,store.storename)>0 or locate(:keyword,merstore.storename)>0)";
                                                } else {
                                                    $sqlcondition = " left join " . tablename("tapgo_tt_store") . " store on ( store.id = o.verifystoreid or store.id = vgl.storeid or store.id = vol.storeid ) and store.uniacid=o.uniacid ";
                                                    $condition .= " AND (locate(:keyword,store.storename)>0)";
                                                }
                                            } else {
                                                if ($searchfield == "goodstitle") {
                                                    $sqlcondition = " inner join ( select DISTINCT(og.orderid) from " . tablename("tapgo_tt_order_goods") . " og left join " . tablename("tapgo_tt_goods") . " g on g.id=og.goodsid where og.uniacid = '" . $uniacid . "' and (locate(:keyword,g.title)>0) " . $fullback_where . ") gs on gs.orderid=o.id";
                                                } else {
                                                    if ($searchfield == "goodssn") {
                                                        $sqlcondition = " inner join ( select DISTINCT(og.orderid) from " . tablename("tapgo_tt_order_goods") . " og left join " . tablename("tapgo_tt_goods") . " g on g.id=og.goodsid where og.uniacid = '" . $uniacid . "' and (((locate(:keyword,g.goodssn)>0)) or (locate(:keyword,og.goodssn)>0)) " . $fullback_where . ") gs on gs.orderid=o.id";
                                                    } else {
                                                        if ($searchfield == "goodsoptiontitle") {
                                                            $sqlcondition = " inner join ( select  DISTINCT(og.orderid) from " . tablename("tapgo_tt_order_goods") . " og left join " . tablename("tapgo_tt_goods") . " g on g.id=og.goodsid where og.uniacid = '" . $uniacid . "' and (locate(:keyword,og.optionname)>0) " . $fullback_where . ") gs on gs.orderid=o.id";
                                                        } else {
                                                            if ($searchfield == "merch") {
                                                                if ($merch_plugin) {
                                                                    $condition .= " AND (locate(:keyword,merch.merchname)>0)";
                                                                    $sqlcondition = " left join " . tablename("tapgo_tt_merch_user") . " merch on merch.id = o.merchid and merch.uniacid=o.uniacid";
                                                                }
                                                            } else {
                                                                if ($searchfield == "selfget") {
                                                                    $condition .= " AND (locate(:keyword,store.storename)>0)";
                                                                    $sqlcondition = " left join " . tablename("tapgo_tt_store") . " store on store.id = o.storeid and store.uniacid=o.uniacid";
                                                                }
                                                            }
                                                        }
                                                    }
        ...
        if ($condition != " o.uniacid = :uniacid and o.ismr=0 and o.deleted=0 and o.isparent=0 and o.istrade=0 " || !empty($sqlcondition)) {
            if ($searchfield == "member") {
                ...
                $sql = "select distinct o.* , a.realname as arealname,a.mobile as amobile,a.province as aprovince ,a.city as acity , a.area as aarea, a.street as astreet,a.address as aaddress,\n                      d.dispatchname,m.nickname,m.id as mid,m.realname as mrealname,m.mobile as mmobile,m.uid,m.groupid,\n                      r.rtype,r.status as rstatus,o.sendtype,o.city_express_state from " . tablename("tapgo_tt_order") . " o" . " left join " . tablename("tapgo_tt_order_refund") . " r on r.id =o.refundid " . " left join " . tablename("tapgo_tt_member") . " m on m.openid=o.openid and m.uniacid =  o.uniacid " . " left join " . tablename("tapgo_tt_member_address") . " a on a.id=o.addressid " . " left join " . tablename("tapgo_tt_dispatch") . " d on d.id = o.dispatchid " . " " . $sqlcondition . " where " . $condition . " " . $statuscondition . " ORDER BY " . $orderbuy . " DESC  ";
            } else {
                if ($searchfield == "saler") {
                    $sql = "select distinct o.* ,m.groupid, a.realname as arealname,a.mobile as amobile,a.province as aprovince ,a.city as acity , a.area as aarea, a.street as astreet,a.address as aaddress,\n                      d.dispatchname,\n                      r.rtype,r.status as rstatus,o.sendtype,o.city_express_state from " . tablename("tapgo_tt_order") . " o" . " left join " . tablename("tapgo_tt_order_refund") . " r on r.id =o.refundid " . " left join " . tablename("tapgo_tt_member") . " m on m.openid = o.openid and m.uniacid=o.uniacid" . " left join " . tablename("tapgo_tt_member_address") . " a on a.id=o.addressid " . " left join " . tablename("tapgo_tt_dispatch") . " d on d.id = o.dispatchid " . " left join " . tablename("tapgo_tt_verifygoods") . " vg on vg.orderid = o.id " . " left join " . tablename("tapgo_tt_verifygoods_log") . " vgl on vgl.verifygoodsid = vg.id" . " left join " . tablename("tapgo_tt_verifyorder_log") . " vol on vol.orderid=o.id " . " left join " . tablename("tapgo_tt_saler") . " s on (s.id = vgl.salerid or s.id=vol.salerid or s.openid=o.verifyopenid) and s.uniacid=o.uniacid and o.ismerch=0" . " left join " . tablename("tapgo_tt_member") . " sm on sm.openid = s.openid and sm.uniacid=s.uniacid" . " left join " . tablename("tapgo_tt_merch_saler") . " ms on (ms.id = vgl.salerid or ms.id=vol.salerid or ms.openid=o.verifyopenid) and ms.uniacid=o.uniacid and o.ismerch=1" . " " . $sqlcondition . " where " . $condition . " " . $statuscondition . " ORDER BY " . $orderbuy . " DESC  ";
                } else {
                    if ($searchfield == "store") {
                        $sql = "select distinct o.* ,m.groupid, a.realname as arealname,a.mobile as amobile,a.province as aprovince ,a.city as acity , a.area as aarea, a.street as astreet,a.address as aaddress,\n                      d.dispatchname,\n                      r.rtype,r.status as rstatus,o.sendtype,o.city_express_state from " . tablename("tapgo_tt_order") . " o" . " left join " . tablename("tapgo_tt_order_refund") . " r on r.id =o.refundid " . " left join " . tablename("tapgo_tt_member") . " m on m.openid = o.openid and m.uniacid=o.uniacid" . " left join " . tablename("tapgo_tt_member_address") . " a on a.id=o.addressid " . " left join " . tablename("tapgo_tt_dispatch") . " d on d.id = o.dispatchid " . " left join " . tablename("tapgo_tt_verifygoods") . " vg on vg.orderid = o.id " . " left join " . tablename("tapgo_tt_verifygoods_log") . " vgl on vgl.verifygoodsid = vg.id" . " left join " . tablename("tapgo_tt_verifyorder_log") . " vol on vol.orderid=o.id " . " " . $sqlcondition . " where " . $condition . " " . $statuscondition . " ORDER BY " . $orderbuy . " DESC  ";
                    } else {
                        if ($searchfield == "mid") {
                            $paras[":keyword"] = htmlspecialchars_decode($_GPC["keyword"], ENT_QUOTES);
                            $condition .= " AND (m.id=:keyword)";
                            $sql = "select distinct m.groupid,o.* , a.realname as arealname,a.mobile as amobile,o.merchdeductenough,a.province as aprovince ,a.city as acity , a.area as aarea, a.street as astreet,a.address as aaddress,\n                      d.dispatchname,m.nickname,m.id as mid,m.realname as mrealname,m.mobile as mmobile,m.uid,\n                      r.rtype,r.status as rstatus,o.sendtype,o.city_express_state from " . tablename("tapgo_tt_order") . " o" . " left join " . tablename("tapgo_tt_order_refund") . " r on r.id =o.refundid " . " left join " . tablename("tapgo_tt_member") . " m on m.openid=o.openid and m.uniacid =  o.uniacid " . " left join " . tablename("tapgo_tt_member_address") . " a on a.id=o.addressid " . " left join " . tablename("tapgo_tt_dispatch") . " d on d.id = o.dispatchid " . " " . $sqlcondition . " where " . $condition . " " . $statuscondition . " ORDER BY " . $orderbuy . " DESC  ";
                        } else {
                            $sql = "select  m.groupid,o.* ,a.realname as arealname,o.merchdeductenough,a.mobile as amobile,a.province as aprovince ,a.city as acity , a.area as aarea, a.street as astreet,a.address as aaddress,\n                  r.rtype,r.status as rstatus,o.sendtype,o.city_express_state from " . tablename("tapgo_tt_order") . " o" . " left join " . tablename("tapgo_tt_order_refund") . " r on r.id =o.refundid " . " left join " . tablename("tapgo_tt_member") . " m on m.openid=o.openid and m.uniacid =  o.uniacid " . " left join " . tablename("tapgo_tt_member_address") . " a on a.id=o.addressid " . " " . $sqlcondition . " where " . $condition . " " . $statuscondition . " ORDER BY " . $orderbuy . " DESC  ";
                        }
                    }
                }
            }
            ...
            $sql = "select * from " . tablename("tapgo_tt_order") . " where uniacid = :uniacid and ismr=0 and deleted=0 and isparent=0 " . $status_condition . " GROUP BY id ORDER BY createtime DESC  ";
            $sql .= "LIMIT " . ($pindex - 1) * $psize . "," . $psize;
            $list = pdo_fetchall($sql, $paras);
            if (!empty($list)) {
                $refundid = "";
                ...
                $refundid_array = pdo_fetchall("SELECT id,rtype,status as rstatus FROM " . tablename("tapgo_tt_order_refund") . " WHERE id IN (" . $refundid . ")", array(), "id");
                $openid_array = pdo_fetchall("SELECT openid,nickname,id,groupid as mid,realname as mrealname,uid,mobile as mmobile FROM " . tablename("tapgo_tt_member") . " WHERE openid IN (" . $openid . ") AND uniacid=" . $_W["uniacid"], array(), "openid");
                $addressid_array = pdo_fetchall("SELECT id,realname as arealname,mobile as amobile,province as aprovince ,city as acity , area as aarea,address as aaddress FROM " . tablename("tapgo_tt_member_address") . " WHERE id IN (" . $addressid . ")", array(), "id");
                $dispatchid_array = pdo_fetchall("SELECT id,dispatchname FROM " . tablename("tapgo_tt_dispatch") . " WHERE id IN (" . $dispatchid . ")", array(), "id");
                $verifyopenid_array = pdo_fetchall("SELECT m.id as salerid,m.nickname as salernickname,m.openid,s.salername FROM " . tablename("tapgo_tt_saler") . " s LEFT JOIN " . tablename("tapgo_tt_member") . " m ON m.openid = s.openid and m.uniacid=s.uniacid WHERE s.openid IN (" . $verifyopenid . ")", array(), "openid");
                foreach ($list as $key => &$value) {
        ...
        $gdata = pdo_fetchall("select * from " . tablename("tapgo_tt_member_group") . " where uniacid = " . $uniacid);
        ...
        if (!empty($list)) {
            ...
            foreach ($list as &$value) {
                ...
                if (!empty($agentid) && 0 < $level) {
                    ...
                    if ($value["agentid"] == $agentid) {
                        $value["level"] = 1;
                        $level1_commissions = pdo_fetchall("select commission1,commissions  from " . tablename("tapgo_tt_order_goods") . " og " . " left join  " . tablename("tapgo_tt_order") . " o on o.id = og.orderid " . " where og.orderid=:orderid and o.agentid= " . $agentid . "  and o.uniacid=:uniacid", array(":orderid" => $value["id"], ":uniacid" => $uniacid));
                        ...
                        if (in_array($value["agentid"], array_keys($agent["level1_agentids"]))) {
                            $value["level"] = 2;
                            if (0 < $agent["level2"]) {
                                $level2_commissions = pdo_fetchall("select commission2,commissions  from " . tablename("tapgo_tt_order_goods") . " og " . " left join  " . tablename("tapgo_tt_order") . " o on o.id = og.orderid " . " where og.orderid=:orderid and  o.agentid in ( " . implode(",", array_keys($agent["level1_agentids"])) . ")  and o.uniacid=:uniacid", array(":orderid" => $value["id"], ":uniacid" => $uniacid));
                            ...
                            if (in_array($value["agentid"], array_keys($agent["level2_agentids"]))) {
                                $value["level"] = 3;
                                if (0 < $agent["level3"]) {
                                    $level3_commissions = pdo_fetchall("select commission3,commissions from " . tablename("tapgo_tt_order_goods") . " og " . " left join  " . tablename("tapgo_tt_order") . " o on o.id = og.orderid " . " where og.orderid=:orderid and  o.agentid in ( " . implode(",", array_keys($agent["level2_agentids"])) . ")  and o.uniacid=:uniacid", array(":orderid" => $value["id"], ":uniacid" => $uniacid));
            ...
            $omember = pdo_fetchall("select openid,nickname,id as mid,realname as mrealname,mobile as mmobile,uid from " . tablename("tapgo_tt_member") . " where openid in(" . $oopenid . ") and uniacid = :uniacid", array(":uniacid" => $_W["uniacid"]), "openid");
            $verifyopenid_array = pdo_fetchall("SELECT m.id as salerid,m.nickname as salernickname,m.openid,s.salername FROM " . tablename("tapgo_tt_saler") . " s LEFT JOIN " . tablename("tapgo_tt_member") . " m ON m.openid = s.openid and m.uniacid=s.uniacid WHERE s.openid IN (" . $verifyopenid . ") and s.uniacid = :uniacid", array(":uniacid" => $_W["uniacid"]), "openid");
        ...
        if (!empty($temp_refund)) {
            $refund_applys = pdo_fetchall("select id,status,applyprice,orderid from " . tablename("tapgo_tt_order_refund") . " where id in (" . implode(",", $temp_refund) . ") and uniacid=:uniacid ", array(":uniacid" => $_W["uniacid"]));
        }
        foreach ($list as $key => &$value) {
        ...
        if ($_GPC["export"] == 1) {
            ...
            foreach ($list as &$row) {
                ...
                foreach ($row["goods"] as $k => $g) {
                    ...
                    $row["groupname"] = "";
                    $row["channel"] = "";
                    $row["groupid"] = array_filter(explode(",", $row["groupid"]));
                    ...
                    $res_multi_arr = pdo_fetchall("select applyprice,rtype,status from " . tablename("tapgo_tt_order_single_refund") . " where id=:id and uniacid=:uniacid limit 1", array(":id" => $g["single_refundid"], ":uniacid" => $_W["uniacid"]));
            ...
            if (!empty($temp_verify_orderids)) {
                $sql = "select og.orderid,s.storename,sa.salername,sa.id,m.nickname  from " . tablename("tapgo_tt_verifygoods_log") . "   vgl\n                                 left join " . tablename("tapgo_tt_verifygoods") . " vg on vg.id = vgl.verifygoodsid\n                                 left join " . tablename("tapgo_tt_store") . " s  on s.id = vgl.storeid\n                                 left join " . tablename("tapgo_tt_saler") . " sa  on sa.id = vgl.salerid\n                                 left join " . tablename("tapgo_tt_order_goods") . " og on vg.ordergoodsid = og.id\n                                 left join " . tablename("tapgo_tt_member") . " m on m.openid = sa.openid\n                                 where  og.orderid in (" . implode(",", $temp_verify_orderids) . ") ORDER BY vgl.verifydate DESC ";
            ...
            if ($index == 1) {
                ...
                $filename = "订单数据-" . $filename . ".csv";
                $savepath = TAPGO_TT_DATA . "order/" . $filename;
                file_delete($savepath);
            }
            m("excel")->exportCSV($exportlist, array("title" => "订单数据", "columns" => $columns), TAPGO_TT_DATA . "order/", $index, $exflag);
            unset($exportlist);
            unset($list);
            if (!$exflag) {
                $pindex++;
            ...
            if ($condition != " o.uniacid = :uniacid and o.ismr=0 and o.deleted=0 and o.isparent=0 " || !empty($sqlcondition)) {
                if ($searchfield == "member") {
                    ...
                    $openidArr = pdo_fetchall("SELECT openid FROM " . tablename("tapgo_tt_member") . " WHERE uniacid = " . $_W["uniacid"] . $priceCondition1);
                    if (!empty($openidArr)) {
                        foreach ($openidArr as $openid) {
                            $openids[] = $openid["openid"];
                        }
                        $inOpenid = "'" . implode("','", $openids) . "'";
                        $orderPrice = pdo_fetch("SELECT COUNT(DISTINCT(o.id)) as count,sum(price) as sumprice FROM " . tablename("tapgo_tt_order") . " o WHERE o.uniacid = " . $_W["uniacid"] . " AND o.deleted=0 AND o.isparent=0 and o.iscycelbuy=0 AND o.openid IN (" . $inOpenid . ")" . $timeCondition . $statuscondition);
                    } else {
                    ...
                    if ($searchfield == "address") {
                        $orderPrice = pdo_fetch("SELECT COUNT(DISTINCT(o.id)) as count,sum(o.price) as sumprice FROM " . tablename("tapgo_tt_order") . " o left join " . tablename("tapgo_tt_member_address") . " a on o.addressid = a.id WHERE o.uniacid = " . $_W["uniacid"] . "  AND o.deleted=0 AND o.isparent=0 and o.iscycelbuy=0 " . $priceCondition . $statuscondition);
                        $totalmoney = $orderPrice["sumprice"];
                        $total = $orderPrice["count"];
                        ...
                        if ($searchfield == "location") {
                            $orderPrice = pdo_fetch("SELECT COUNT(DISTINCT(o.id)) as count,sum(o.price) as sumprice FROM " . tablename("tapgo_tt_order") . " o left join " . tablename("tapgo_tt_member_address") . " a on o.addressid = a.id WHERE o.uniacid = " . $_W["uniacid"] . " AND o.deleted=0 AND o.isparent=0 " . $priceCondition . $statuscondition);
                            $totalmoney = $orderPrice["sumprice"];
                            ...
                            if ($searchfield == "saler") {
                                $t = pdo_fetch("SELECT COUNT(DISTINCT(o.id)) as count, ifnull(sum(o.price),0) as sumprice   FROM " . tablename("tapgo_tt_order") . " o " . " left join " . tablename("tapgo_tt_order_refund") . " r on r.id =o.refundid " . " left join " . tablename("tapgo_tt_verifygoods") . " vg on vg.orderid = o.id" . " left join " . tablename("tapgo_tt_verifygoods_log") . " vgl on vgl.verifygoodsid = vg.id" . " left join " . tablename("tapgo_tt_verifyorder_log") . " vol on vol.orderid=o.id " . " left join " . tablename("tapgo_tt_saler") . " s on (s.id = vgl.salerid or s.id=vol.salerid) and s.uniacid=o.uniacid and o.ismerch=0" . " left join " . tablename("tapgo_tt_merch_saler") . " ms on (ms.id = vgl.salerid or ms.id=vol.salerid) and ms.uniacid=o.uniacid and o.ismerch=1" . " left join " . tablename("tapgo_tt_member") . " sm on sm.openid = s.openid and sm.uniacid=s.uniacid" . " " . $sqlcondition . " WHERE " . $condition . " " . $statuscondition, $paras);
                                $total = (int) $t["count"];
                                $totalmoney = $t["sumprice"];
                            } else {
                                if ($searchfield == "store") {
                                    $t = pdo_fetch("SELECT COUNT(DISTINCT(o.id)) as count, ifnull(sum(o.price),0) as sumprice   FROM " . tablename("tapgo_tt_order") . " o " . " left join " . tablename("tapgo_tt_order_refund") . " r on r.id =o.refundid " . " left join " . tablename("tapgo_tt_verifygoods") . " vg on vg.orderid = o.id" . " left join " . tablename("tapgo_tt_verifygoods_log") . " vgl on vgl.verifygoodsid = vg.id" . " left join " . tablename("tapgo_tt_verifyorder_log") . " vol on vol.orderid=o.id " . " " . $sqlcondition . " WHERE " . $condition . " " . $statuscondition, $paras);
                                    $total = (int) $t["count"];
                                    $totalmoney = $t["sumprice"];
                                } else {
                                    if ($searchfield == "mid") {
                                        $t = pdo_fetch("SELECT COUNT(DISTINCT(o.id)) as count, ifnull(sum(o.price),0) as sumprice   FROM " . tablename("tapgo_tt_order") . " o " . " left join " . tablename("tapgo_tt_order_refund") . " r on r.id =o.refundid " . " left join " . tablename("tapgo_tt_member") . " m on m.openid = o.openid and m.uniacid=o.uniacid" . " " . $sqlcondition . " WHERE " . $condition . " " . $statuscondition, $paras);
                                        $total = (int) $t["count"];
                                        $totalmoney = $t["sumprice"];
                                    } else {
                                        if ($status === "" && empty($_GPC["keyword"]) && empty($_GPC["time"])) {
                                            $redis = redis();
                                            if (!empty($heads)) {
                                                $redis_key_total = $_W["uniacid"] . "_tapgo_tt_order_list_total" . $heads;
                                                $redis_key_totalmoney = $_W["uniacid"] . "_tapgo_tt_order_list_totalmoney" . $heads;
                                            }
                                            if (!is_error($redis)) {
                                                if (false && $redis->get($redis_key_total) != false && $redis->get($redis_key_totalmoney) != false) {
                                                    ...
                                                    $sumprice = 0;
                                                    if (empty($agentid)) {
                                                        $t = pdo_fetch("SELECT COUNT(DISTINCT(o.id)) as count, ifnull(sum(o.price),0) as sumprice   FROM " . tablename("tapgo_tt_order") . " o " . " left join " . tablename("tapgo_tt_order_refund") . " r on r.id =o.refundid " . " " . $sqlcondition . " WHERE " . $condition . " " . $statuscondition, $paras);
                                                        $total = (int) $t["count"];
                                                        $totalmoney = $t["sumprice"];
                                                    } else {
                                                        if (p("commission")) {
                ...
                $t = pdo_fetch("SELECT COUNT(*) as count, ifnull(sum(price),0) as sumprice   FROM " . tablename("tapgo_tt_order") . " WHERE uniacid = :uniacid and ismr=0 and deleted=0 and isparent=0 " . $status_condition, $paras);
                $total = $t["count"];
                $totalmoney = $t["sumprice"];
            }
            $pager = pagination2($total, $pindex, $psize);
            $stores = pdo_fetchall("select id,storename from " . tablename("tapgo_tt_store") . " where uniacid=:uniacid ", array(":uniacid" => $uniacid));
            $r_type = array("退款", "退货退款", "换货");
            load()->func("tpl");
    ...
    public function updateChildOrderPay()
    {
        ...
        $sql = "select id,parentid from " . tablename("tapgo_tt_order") . " where parentid>0 and status>0 and paytype=0 and uniacid=:uniacid";
        $list = pdo_fetchall($sql, $params);
        if (!empty($list)) {
            foreach ($list as $k => $v) {
                $params[":orderid"] = $v["parentid"];
                $sql1 = "select paytype from " . tablename("tapgo_tt_order") . " where id=:orderid and status>0 and paytype>0 and uniacid=:uniacid";
                $item = pdo_fetch($sql1, $params);
                if (0 < $item["paytype"]) {
                    pdo_update("tapgo_tt_order", array("paytype" => $item["paytype"]), array("id" => $v["id"]));
                }
            }
        }
    }
