...
class Op_TapgoTtPage extends WebPage {
    ...
    function delete() {
        ...
        pdo_update('tapgo_tt_order', array('deleted' => 1), array('id' => $orderid, 'uniacid' => $_W['uniacid']));
    ...
    protected function opData() {
        ...
        $order_goods = pdo_fetchall('select single_refundstate from ' . tablename('tapgo_tt_order_goods'). ' where orderid=:orderid and uniacid=:uniacid', array(':uniacid' => $_W['uniacid'], ':orderid' => $item['id']));
    ...
    function pay($a = array(), $b = array()) {
        ...
        if (!empty($item['virtual']) && com('virtual')) {
            ...
            //确认付款先改状态，再设置库存
            pdo_update('tapgo_tt_order', $updateorder, array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
    ...
    function close() {
        ...
        if ($_W['ispost']) {
            ...
            if ($item['refundstate'] > 0 && !empty($item['refundid'])) {
                ...
                pdo_update('tapgo_tt_order_refund', $change_refund, array('id' => $item['refundid'], 'uniacid' => $_W['uniacid']));
            }
            ...
            pdo_update('tapgo_tt_order', array('status' => -1, 'refundstate' => 0, 'canceltime' => $time, 'remarkclose' => $_GPC['remark']), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));

            if (!empty($item['virtual']) && $item['virtual'] != 0) {

                $goodsid = pdo_fetch('SELECT goodsid FROM '.tablename('tapgo_tt_order_goods').' WHERE uniacid = '.$_W['uniacid'].' AND orderid = '.$item['id']);
                ...
                //未付款卡密变为未使用
                pdo_query("update " . tablename('tapgo_tt_virtual_data') . ' set openid="",usetime=0,orderid=0,ordersn="",price=0,merchid='.$item['merchid'].' where typeid=' . intval($typeid).' and orderid = '.$item["id"]);

                //模板减少使用数据
                pdo_query("update " . tablename('tapgo_tt_virtual_type') . " set usedata=usedata-".$vkeynum." where id=" .intval($typeid));
        ...
        if (!empty($item['wxapp_allow_subscribe'])) {
            ...
            if (in_array('receive', $template)) {
                ...
                if (strexists('sns_wa_', $item['openid'])) {
                    ...
                    $openid = pdo_fetchcolumn("select openid_wa from ".tablename('tapgo_tt_member')." where openid='{$item['openid']}'");
                }
    ...
    function sendcancel() {
        ...
        if ($_W['ispost']) {
            ...
            if($item['sendtype']>0){
                ...
                $data['sendtype'] = 0;
                pdo_update('tapgo_tt_order_goods', $data, array('orderid' => $item['id'],'sendtype'=>$sendtype, 'uniacid' => $_W['uniacid']));
        ...
        if($sendtype>0){
            ...
            if($item['sendtype']>0){
                for($i=1;$i<=intval($item['sendtype']);$i++){
                    $bundles[$i]['goods'] = pdo_fetchall('select g.id,g.title,g.thumb,og.sendtype,g.ispresell from ' . tablename('tapgo_tt_order_goods') . ' og '
                        . ' left join ' . tablename('tapgo_tt_goods') . ' g on g.id=og.goodsid '
    ...
    function fetch() {
        ...
        pdo_update(
            'tapgo_tt_order', $d, array('id' => $item['id'], 'uniacid' => $_W['uniacid'])
        );
    ...
    function send() {
            ...
            if($item['city_express_state']==0){
                    ...
                    if(intval($_GPC['sendtype'])==1 || $item['sendtype'] > 0){
                        ...
                        foreach($goodsid as $key => $value){
                            pdo_update('tapgo_tt_order_goods', $data, array('id'=>$value, 'uniacid' => $_W['uniacid']));
                        }
                        //查询是否有未发货包裹
                ...
                if($cityexpress['express_type']==1){
                    $ret = pdo_fetchall('select goodsid , total , optionid from ' . tablename('tapgo_tt_order_goods') . ' where orderid = :orderid and uniacid = :uniacid',array(':uniacid'=>$_W['uniacid'],':orderid'=>$item['id']));
                    $weight = 0;
                foreach ($ret as $key=>$val)
                {
                    if ($val['optionid'] > 0)
                    {
                        $weight += pdo_fetchcolumn(' select weight*' . $val['total'] . ' from ' . tablename('tapgo_tt_goods_option') . ' where goodsid=:id and id = :optionid',array(':id'=>$val['goodsid'],':optionid'=>$val['optionid']));
                    }else
                    {
                        $weight += (pdo_fetchcolumn("select weight* ".$val['total']." from " . tablename('tapgo_tt_goods'). ' where id=:id',array(':id'=>$val['goodsid'])));
                    }
                }
                    if ($weight == 0)
                    {
        ...
        if($item['sendtype']>0){
            //未发货商品
            $noshipped = pdo_fetchall('select og.id,g.title,g.thumb,og.sendtype,g.ispresell from ' . tablename('tapgo_tt_order_goods') . ' og '
                . ' left join ' . tablename('tapgo_tt_goods') . ' g on g.id=og.goodsid '
            ...
            for($i=1;$i<=$item['sendtype'];$i++){
                ...
                $shipped[$i]['goods'] = pdo_fetchall('select g.id,g.title,g.thumb,og.sendtype,g.ispresell from ' . tablename('tapgo_tt_order_goods') . ' og '
                    . ' left join ' . tablename('tapgo_tt_goods') . ' g on g.id=og.goodsid '
        ...
        $order_goods = pdo_fetchall('select og.id,g.title,g.thumb,og.sendtype,g.ispresell from ' . tablename('tapgo_tt_order_goods') . ' og '
            . ' left join ' . tablename('tapgo_tt_goods') . ' g on g.id=og.goodsid '
    ...
    function remarksaler() {
        ...
        if ($_W['ispost']) {
            pdo_update('tapgo_tt_order', array('remarksaler' => $_GPC['remark']), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
        ...
        if ($_W['ispost']) {
            ...
            if (!empty($expresssn)) {
                ...
                if($item['sendtype']>0){
                    ...
                    pdo_update('tapgo_tt_order_goods', $change_data, array('orderid' => $id,'sendtype'=>$sendtype, 'uniacid' => $_W['uniacid']));
                }else{
                    pdo_update('tapgo_tt_order', $change_data, array('id' => $id, 'uniacid' => $_W['uniacid']));
                }
    ...
    function changeaddress() {
        ...
        if ($_W['ispost']) {
            ...
            if (!empty($id)) {
                ...
                $item = pdo_fetch("SELECT id, ordersn, address,openid FROM " . tablename('tapgo_tt_order') . " WHERE id = :id and uniacid=:uniacid", array(':id' => $id, ':uniacid' => $_W['uniacid']));
                ...
                pdo_update('tapgo_tt_order', array('address' => $address_array), array('id' => $id, 'uniacid' => $_W['uniacid']));
    ...
    function upload_invoice(){
        ...
        if (!$_W['ispost']){
            $invoice = pdo_fetch('select invoicename,invoice_img from '.tablename('tapgo_tt_order').' where id = :order_id and uniacid = :uniacid limit 1',array(':order_id'=>$order_id, ':uniacid'=>$_W['uniacid']));
        ...
        $update_ret = pdo_update('tapgo_tt_order', array('invoice_img'=>$invoice_img), array('id'=>$order_id));
    ...
    function peerpay() {
        ...
        $sql = "SELECT * FROM ".tablename('tapgo_tt_order_peerpay_payinfo')." where pid=:pid";
