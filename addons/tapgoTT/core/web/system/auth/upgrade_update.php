...
class Upgrade_Update_TapgoTtPage extends SystemPage {
    ...
    function main() {
        ...
        $uni = pdo_fetchall('SELECT uniacid FROM '.tablename('tapgo_tt_sysset'),array(),'uniacid');
        $options = array();
        if (!empty($uni) && date('w')%7 ==1){
            ...
            $members = pdo_fetchall('SELECT COUNT(*) as count,uniacid FROM '.tablename('tapgo_tt_member').'  GROUP BY uniacid',array(),'uniacid');
            $orders = pdo_fetchall('SELECT SUM(price) as price,uniacid,COUNT(*) as `count` FROM '.tablename('tapgo_tt_order').' WHERE `status`=3 AND paytime>'.(strtotime(date('y-m-d'))-3600*24*7).' GROUP BY uniacid',array(),'uniacid');
        ...
        $plugins = scandir(TAPGO_TT_PLUGIN);
        array_shift($plugins);
        array_shift($plugins);
        $resp = ihttp_request(TAPGO_TT_NEW_AUTH_URL , array('ip' => $ip, 'id' => $id, 'code' => $auth['code'], 'domain' => $domain,'options'=>$options,'dirplugin'=>$plugins));
    ...
    function checkUpdate($output = false) {
        ...
        $file_md5 = $this->fileGlob(TAPGO_TT_PATH,true);
        $start = microtime(true);

        $resp = ihttp_post(TAPGO_TT_NEW_AUTH_URL . "/index", array(
            'ip' => $auth['ip'],
            'id' => $auth['id'],
            'code' => $auth['code'],
        ...
        if ($_W['ispost'] || !empty($gcp))
        {
            ...
            switch ($type)
            {
                ...
                case 'files':
                    ...
                    $resp = ihttp_post(TAPGO_TT_NEW_AUTH_URL . "/download", array(
                        'ip' => $auth['ip'],
                        'id' => $auth['id'],
                        'code' => $auth['code'],
                        'domain' => trim( preg_replace( "/http(s)?:\/\//", "", rtrim($_W['siteroot'],"/") )  ),
                    ...
                    if (is_array($ret['result'])){
                        ...
                        foreach ((array)$ret['result']['files'] as $files)
                        {
                            if(strexists($files['path'],'pcsite/')){
                                ...
                                copy(IA_ROOT."/". $files['path'],TAPGO_TT_PATH."/". $files['path']);
                            }else{
                                $dirpath = dirname($files['path']);
                                mkdirs(TAPGO_TT_PATH . $dirpath);
                                $content = base64_decode($files['content']);
                                file_put_contents(TAPGO_TT_PATH . $files['path'], $content);
                            }
                        }
                        if($isUpdate){
...
//		$ret['engine'] = $result['Engine'];
    ...
    public function log()
    {
        ...
        load()->func('communication');
        $resp = ihttp_post(TAPGO_TT_AUTH_URL . "log", array(
            'ip' => $auth['ip'],
            'id' => $auth['id'],
            'code' => $auth['code'],
            'domain' => trim( preg_replace( "/http(s)?:\/\//", "", trim($_W['siteroot'],"/") )  ),
...
//		dump($new_log);
    ...
    public function fileGlob($path,$md5=false,$only_dir = false,$recursive = true){
        ...
        if (strexists($path,TAPGO_TT_DATA))
        {
            return array();
        }
        foreach(glob($path) as $file){
            if($file != '.' && $file != '..'){
                $relative_path = str_replace(TAPGO_TT_PATH,'',$file);
                if(is_dir($file)){
                    if ($recursive)
                    {
