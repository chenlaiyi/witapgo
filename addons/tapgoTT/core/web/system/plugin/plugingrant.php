...
class Plugingrant_TapgoTtPage extends SystemPage {
	...
	function main() {
        ...
        $list = pdo_fetchall("SELECT gl.*,w.name as wname,p.name as pname,p.thumb,p.iscom,p.identity FROM " . tablename('tapgo_tt_system_plugingrant_log') . " as gl
                    left join ".tablename('account_wechats')." as w on w.uniacid = gl.uniacid
                    left join ".tablename('tapgo_tt_plugin')." as p on p.id = gl.pluginid
			        WHERE 1 {$condition} ORDER BY gl.id desc LIMIT " . ($pindex - 1) * $psize . ',' . $psize, $params);


        $total = pdo_fetchcolumn("SELECT COUNT(1) FROM " . tablename('tapgo_tt_system_plugingrant_log') . " as gl
                    left join ".tablename('account_wechats')." as w on w.uniacid = gl.uniacid
                    left join ".tablename('tapgo_tt_plugin')." as p on p.id = gl.pluginid
	...
	protected function post() {
		global $_W, $_GPC;
		$id = intval($_GPC['id']);
		$item = pdo_fetch("SELECT * FROM " . tablename('tapgo_tt_system_plugingrant_log') . " WHERE id =:id limit 1", array(':id' => $id));
		...
		if ($_W['ispost']) {
            ...
            foreach ($pluginid as $value) {
                ...
                $data['pluginid'] = $value;
                if ($data['isperm'] > 0) {
                    $lastitem = pdo_fetch("SELECT MAX(permendtime) as permendtime,permlasttime FROM " . tablename('tapgo_tt_system_plugingrant_log') . "
        ...
        if(!empty($item) && $item['pluginid']>0){
            $plugin = pdo_fetch('select * from ' . tablename('tapgo_tt_plugin') . ' where id = '.$item['pluginid'].' ');
	...
	function delete() {
		global $_W, $_GPC;
		$id = intval($_GPC['id']);
		$item = pdo_fetch("SELECT id FROM " . tablename('tapgo_tt_system_plugingrant_log') . " WHERE id = '$id'");
		if (empty($item)) {
			message('抱歉，权限设置不存在或是已经被删除！', webUrl('system/plugin/plugingrant', array('op' => 'display')), 'error');
		}

		$log = pdo_fetch("select * from " . tablename('tapgo_tt_system_plugingrant_log') . "where id = :id", array(':id'    => $id));
        $currentTime = time();
        ...
        pdo_update('tapgo_tt_system_plugingrant_log', array('deleted'=>'1'), array('id'  => $id));
		show_json(1, array(
	...
	function queryplugin() {
        ...
        $plugins = pdo_fetchall('select id,`name` as title,thumb,`desc` from ' . tablename('tapgo_tt_plugin') . ' where 1 '.$condition.' order by displayorder asc LIMIT '. ($pindex - 1) * $psize . ',' . $psize,$params);

        $total = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('tapgo_tt_plugin') . " WHERE 1 ".$condition." ", $params);
