...
class Pluginsetting_TapgoTtPage extends SystemPage {

	function main() {
		global $_W, $_GPC;
        $data = pdo_fetch("select * from ".tablename('tapgo_tt_system_plugingrant_setting')." where 1 = 1 limit 1 ");
        load()->model('user');
        ...
        if ($_W['ispost']) {
            ...
            if(!empty($data)){
                pdo_update("tapgo_tt_system_plugingrant_setting",$data_set,array('id'=>$data['id']));
                plog('system.plugin.pluginsetting.edit', '修改授权管理基本设置');
            }else{
                pdo_insert("tapgo_tt_system_plugingrant_setting",$data_set);
