...
class Payment_TapgoTtPage extends WebPage
{
	public $paytype = array('微信支付', '微信支付子商户', '借用微信支付', '借用微信支付子商户', '中信银行全付通','银联支付');
	public $paytypeali = array('支付宝2.0');
	public $paytypeunion = array('银联支付');
	...
	public function main()
	{
		...
		$list = pdo_fetchall('SELECT * FROM ' . tablename('tapgo_tt_payment') . (' WHERE 1 ' . $condition . '  ORDER BY id asc limit ') . ($pindex - 1) * $psize . ',' . $psize, $params);
		$total = pdo_fetchcolumn('SELECT count(*) FROM ' . tablename('tapgo_tt_payment') . (' WHERE 1 ' . $condition), $params);
		$pager = pagination($total, $pindex, $psize);
		$payment = $this->paytype;
		$paytypeali = $this->paytypeali;
		$paytypeunion= $this->paytypeunion;

		if (p('qpay')) {
			$payment[4] = '中信银行全付通';
		}

		$count = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('tapgo_tt_payment') . ' WHERE  uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
	...
	protected function post()
	{
		...
		if ($_W['ispost']) {
			...
			if (empty($id)) {
				...
				pdo_insert('tapgo_tt_payment', $data);
				$id = pdo_insertid();
				plog('sysset.payment.add', '添加支付信息一条 ID: ' . $id . ' 标题: ' . $data['title'] . ' ');
			}
			else {
				pdo_update('tapgo_tt_payment', $data, array('id' => $id));
		...
		if (!empty($id)) {
			$data = pdo_fetch('SELECT * FROM ' . tablename('tapgo_tt_payment') . ' WHERE id=:id and uniacid=:uniacid ', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		}
	...
	public function delete()
	{
		...
		$items = pdo_fetchall('SELECT id,title FROM ' . tablename('tapgo_tt_payment') . (' WHERE id in( ' . $id . ' ) AND uniacid=') . $_W['uniacid']);
		$count = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('tapgo_tt_payment') . ' WHERE  uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));

		if (count($items) < $count) {
			foreach ($items as $item) {
				pdo_delete('tapgo_tt_payment', array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
	...
	public function query()   
	{
		...
		$ds = pdo_fetchall('SELECT id,title FROM ' . tablename('tapgo_tt_member_printer_template') . (' WHERE 1 ' . $condition . ' order by id asc'), $params);
