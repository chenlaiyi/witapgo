...
class Index_TapgoTtPage extends ComWebPage
{
	...
	public function set()
	{
		global $_W;
		global $_GPC;
		$item = pdo_fetch('SELECT * FROM ' . tablename('tapgo_tt_sms_set') . ' WHERE uniacid=:uniacid ', array(':uniacid' => $_W['uniacid']));
		...
		if ($_W['ispost']) {
			...
			if (empty($item)) {
				...
				pdo_insert('tapgo_tt_sms_set', $arr);
				$id = pdo_insertid();
			}
			 else {
				pdo_update('tapgo_tt_sms_set', $arr, array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
			}
