...
class Templatetool_TapgoTtPage extends WebPage
{
	...
	{
		$items = pdo_fetchall('select typecode from ' . tablename('tapgo_tt_member_message_template_type') . ' where templatecode is not null');
		$typecode = array();
	...
	public function settemplateid()
	{
		...
		foreach ($result['template_list'] as $key => $value) {
			...
			if ($valuecontent == $content) {
				$issnoet = false;
				$defaulttemp = pdo_fetch('select 1  from ' . tablename('tapgo_tt_member_message_template_default') . ' where typecode=:typecode and uniacid=:uniacid  limit 1', array(':typecode' => $tag, ':uniacid' => $_W['uniacid']));

				if (empty($defaulttemp)) {
					pdo_insert('tapgo_tt_member_message_template_default', array('typecode' => $tag, 'uniacid' => $_W['uniacid'], 'templateid' => $value['template_id']));
				}
				else {
					pdo_update('tapgo_tt_member_message_template_default', array('templateid' => $value['template_id']), array('typecode' => $tag, 'uniacid' => $_W['uniacid']));
				}
