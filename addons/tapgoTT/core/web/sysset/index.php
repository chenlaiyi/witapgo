...
class Index_TapgoTtPage extends WebPage {
	...
	function settemplateid()
	{
		...
		$issnoet = true;
        $defaulttemp = pdo_fetch('select *  from ' . tablename('tapgo_tt_member_message_template_default') . ' where typecode=:typecode and uniacid=:uniacid  limit 1',
        ...
        if (in_array($templatetype['templatename'], $templateTiele)) {
            ...
            if(empty($defaulttemp)) {
                pdo_insert('tapgo_tt_member_message_template_default',
                    array('typecode' =>$tag,'uniacid' => $_W['uniacid'],'templateid'=>$template_list[$found_key]['template_id']));
            } else {
                pdo_update('tapgo_tt_member_message_template_default', array('templateid'=>$template_list[$found_key]['template_id']),
                    array('typecode' =>$tag,'uniacid' => $_W['uniacid']));
            }
            ...
            else
            {
                ...
                {
                    pdo_update('tapgo_tt_member_message_template_default', array('templateid'=>$result['template_id']),
                        array('typecode' =>$tag,'uniacid' => $_W['uniacid']));
                }
            }
        }
	...
	function payset() {
		...
		if ($_W['ispost']) {
            ...
            $data['yinlian'] = intval($inputdata['yinlian']);
