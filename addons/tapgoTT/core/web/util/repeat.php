...
class Repeat_TapgoTtPage extends WebPage 
{
	public function main() 
	{
		global $_W;
		global $_GPC;
		$init = pdo_tableexists('tapgo_tt_member_tmp');
		if ($init) 
		{
			...
			$join = ' join ' . tablename('tapgo_tt_member_tmp') . ' tmp on tmp.openid=dm.openid ';
			if ($_GPC['followed'] != '') 
			{
				if ($_GPC['followed'] == 2) 
				{
			...
			$sql = 'select * from ' . tablename('tapgo_tt_member') . ' dm ' . $join . ' where 1 ' . $condition . '  ORDER BY dm.openid DESC,dm.createtime DESC ';
	...
	public function delete() 
	{
		...
		foreach ($members as $member ) 
		{
			...
			{
				$m = pdo_fetch('SELECT * FROM ' . tablename('tapgo_tt_member') . ' WHERE openid=\'' . $member['openid'] . '\' AND id <> ' . $member['id'] . ' AND uniacid=' . $_W['uniacid'] . ' limit 1');
				if (!(empty($m))) 
				{
					if (!(empty($member['credit1']))) 
					{
						...
						pdo_update('tapgo_tt_member', array('credit1' => $credit1), array('id' => $m['id'], 'uniacid' => $_W['uniacid']));
					}
					if (!(empty($member['credit2']))) 
					{
						...
						pdo_update('tapgo_tt_member', array('credit2' => $credit2), array('id' => $m['id'], 'uniacid' => $_W['uniacid']));
					}
					pdo_update('tapgo_tt_member', array('agentid' => $m['id']), array('agentid' => $member['id'], 'uniacid' => $_W['uniacid']));
					...
					pdo_delete('tapgo_tt_member_tmp', array('openid' => $member['openid'], 'uniacid' => $member['uniacid']));
	...
	public function init() 
	{
		...
		if ($init) 
		{
			pdo_query('DROP TABLE ' . tablename('tapgo_tt_member_tmp'));
		}
		pdo_run('CREATE TABLE ' . tablename('tapgo_tt_member_tmp') . ' (`uniacid` int(11) DEFAULT \'0\',`openid` varchar(50) DEFAULT \'\') ENGINE=MyISAM DEFAULT CHARSET=utf8;');
		pdo_query('INSERT INTO  ' . tablename('tapgo_tt_member_tmp') . ' SELECT uniacid,openid FROM ' . tablename('tapgo_tt_member') . ' WHERE uniacid=:uniacid  GROUP BY openid HAVING COUNT(openid)>1', array(':uniacid' => $_W['uniacid']));
		show_json(1);
	}
	public function upmember() 
	{
		...
		global $_GPC;
		$id = ((is_array($_GPC['ids']) ? implode(',', $_GPC['ids']) : 0));
		$members = pdo_fetchall('select id,openid,nickname,realname,avatar from ' . tablename('tapgo_tt_member') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);
		foreach ($members as $member ) 
		{
			pdo_update('tapgo_tt_member', array('nickname' => '', 'realname' => '', 'avatar' => ''), array('id' => $member['id']));
		}
		show_json(1);
	}
}
