...
class Open_TapgoTtPage extends WebPage
{
	public function main()
	{
		...
		$key = pdo_fetch('SELECT * FROM ' . tablename('tapgo_tt_open_plugin') . ' WHERE plugin = :plugin', array(':plugin' => $plugin));
		...
		if ($_W['ispost']) {
			...
			if ($key) {
				...
				$num = pdo_get('tapgo_tt_open_plugin', array('plugin' => $plugin), 'status');

				if (!empty($num)) {
					pdo_update('tapgo_tt_open_plugin', array('status' => 1, 'expirtime' => time() + 172800), array('plugin' => $plugin));
				...
				pdo_insert('tapgo_tt_open_plugin', $data);
				$id = pdo_insertid();
	...
	private function checkOpen($key = '', $plugin = '', $domain = '')
	{
		...
		$resp = ihttp_post(TAPGO_TT_AUTH_WXAPP . '/grant', $data);
