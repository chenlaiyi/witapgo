...
class Selecturl_TapgoTtPage extends WebPage
{
	...
	public function main()
	{
		...
		if (0 < $syscate['level']) {
			$categorys = pdo_fetchall('SELECT id,name,parentid FROM ' . tablename('tapgo_tt_category') . ' WHERE enabled=:enabled and uniacid= :uniacid  ', array(':uniacid' => $_W['uniacid'], ':enabled' => '1'));
		}

		$groups = pdo_fetchall('SELECT id,name FROM ' . tablename('tapgo_tt_goods_group') . ' WHERE enabled=:enabled AND merchid = 0 AND  uniacid= :uniacid ', array(':uniacid' => $_W['uniacid'], ':enabled' => '1'));
		$storeList = pdo_fetchall('SELECT id,storename FROM ' . tablename('tapgo_tt_store') . ' WHERE status = 1 AND uniacid = :uniacid ', array(':uniacid' => $_W['uniacid']));
		...
		if ($platform) {
			$customs = pdo_fetchall('select id,`name` from ' . tablename('tapgo_tt_wxapp_page') . ' where uniacid = :uniacid and `type` = 20 and status = 1 ', array(':uniacid' => $_W['uniacid']));
			...
			if (!empty($customs)) {
				...
				foreach ($customs as $key => $value) {
					if ($type == 'topmenu') {
						...
						$diypage = pdo_fetch('SELECT * FROM ' . tablename('tapgo_tt_wxapp_page') . ' WHERE id=:id AND uniacid = :uniacid', array(':id' => $value['id'], ':uniacid' => $_W['uniacid']));
        ...
        if (p('open_messikefu')) {
            $key = pdo_fetch("SELECT id FROM " . tablename('tapgo_tt_open_plugin') . " WHERE `status` =1 and plugin = :plugin", array(':plugin' => 'open_messikefu'));
 ...
 //       }
	...
	public function query()
	{
		...
		if (!empty($kw) && !empty($type)) {
			if ($type == 'good') {
				$list = pdo_fetchall('SELECT id,title,productprice,marketprice,thumb,sales,unit,minprice FROM ' . tablename('tapgo_tt_goods') . ' WHERE uniacid= :uniacid and status=:status and deleted=0 AND title LIKE :title ', array(':title' => '%' . $kw . '%', ':uniacid' => $_W['uniacid'], ':status' => '1'));
				$list = set_medias($list, 'thumb');
			}
			else if ($type == 'goods_data_diy') {
				if ($kw == 'all') {
					$list = pdo_fetchall('SELECT id,title,productprice,marketprice,thumb,sales,unit,minprice FROM ' . tablename('tapgo_tt_goods') . ' WHERE uniacid= :uniacid and status=:status and deleted=0 ', array(':uniacid' => $_W['uniacid'], ':status' => '1'));
					$list = set_medias($list, 'thumb');
				}
				else {
					$list = pdo_fetchall('SELECT id,title,productprice,marketprice,thumb,sales,unit,minprice FROM ' . tablename('tapgo_tt_goods') . ' WHERE uniacid= :uniacid and status=:status and deleted=0 AND title LIKE :title ', array(':title' => '%' . $kw . '%', ':uniacid' => $_W['uniacid'], ':status' => '1'));
				...
				$list = pdo_fetchall('select id,article_title from ' . tablename('tapgo_tt_article') . ' where article_title LIKE :title and article_state=1 and uniacid=:uniacid ', array(':uniacid' => $_W['uniacid'], ':title' => '%' . $kw . '%'));
			}
			else if ($type == 'coupon') {
				$list = pdo_fetchall('select id,couponname,coupontype from ' . tablename('tapgo_tt_coupon') . ' where couponname LIKE :title and uniacid=:uniacid ', array(':uniacid' => $_W['uniacid'], ':title' => '%' . $kw . '%'));
			}
			else if ($type == 'groups') {
				$list = pdo_fetchall('select id,title from ' . tablename('tapgo_tt_groups_goods') . ' where title LIKE :title and status=1 and deleted=0 and uniacid=:uniacid ', array(':uniacid' => $_W['uniacid'], ':title' => '%' . $kw . '%'));
			}
			else if ($type == 'sns') {
				$list_board = pdo_fetchall('select id,title from ' . tablename('tapgo_tt_sns_board') . ' where title LIKE :title and status=1 and enabled=0 and uniacid=:uniacid order by id desc ', array(':uniacid' => $_W['uniacid'], ':title' => '%' . $kw . '%'));
				$list_post = pdo_fetchall('select id,title from ' . tablename('tapgo_tt_sns_post') . ' where title LIKE :title and checked=1 and deleted=0 and uniacid=:uniacid order by id desc ', array(':uniacid' => $_W['uniacid'], ':title' => '%' . $kw . '%'));
				$list = array();
				...
				if ($type == 'creditshop') {
					$list = pdo_fetchall('select id, thumb, title, price, credit, money from ' . tablename('tapgo_tt_creditshop_goods') . ' where title LIKE :title and status=1 and deleted=0 and uniacid=:uniacid ', array(':uniacid' => $_W['uniacid'], ':title' => '%' . $kw . '%'));
				}
			}
		}
