...
class Verify_TapgoTtPage extends WebPage {
    ...
    function main() {
        ...
        if ($_W['ispost']) {
            ...
            if($verifytype==2)
            {
                ...
                $verifygood = pdo_fetch('select *  from ' . tablename('tapgo_tt_verifygoods') . " where uniacid=:uniacid and  verifycode=:verifycode  limit 1 ", array(':uniacid' => $_W['uniacid'], ':verifycode' => $verifycode));
                if (empty($verifygood)) {
    ...
    function verifygoods() {
        ...
        $verifygood = pdo_fetch('select vg.*,g.id as goodsid ,g.title,g.subtitle,g.thumb  from ' . tablename('tapgo_tt_verifygoods') . '   vg
        ...
        if (!empty($verifygood['storeid'])) {
            $store = pdo_fetch('select * from ' . tablename('tapgo_tt_store') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $verifygood['storeid'], ':uniacid' => $_W['uniacid']));
        }
    ...
    function log() {
        ...
        $sql = 'select vg.*,g.id as goodsid ,g.title,g.verifytype,g.thumb,o.ordersn,vgl.verifydate,vgl.verifynum,o.paytime,s.storename,sa.salername,vgl.remarks,o.openid,o.carrier,o.price  from ' . tablename('tapgo_tt_verifygoods_log') . '   vgl
