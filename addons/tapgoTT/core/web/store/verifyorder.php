...
class Verifyorder_TapgoTtPage extends WebPage
{
	protected function orderData()
	{
		...
		if (p('merch')) {
			$leftsql = ' left join ' . tablename('tapgo_tt_merch_saler') . ' ms on ms.openid = o.verifyopenid and ms.uniacid=o.uniacid';
			$leftsql .= ' left join ' . tablename('tapgo_tt_merch_store') . ' mstore on mstore.id = o.verifystoreid';
		}
		...
		if ($searchtime == 'finish') {
			...
			$leftsql .= ' left join ' . tablename('tapgo_tt_verifygoods') . ' vg on o.id = vg.orderid';
			$leftsql .= ' left join ' . tablename('tapgo_tt_verifygoods_log') . ' vgl on vgl.verifygoodsid = vg.id';
		}
		...
		$sql = 'select o.* , a.realname as arealname,a.mobile as amobile,a.province as aprovince ,a.city as acity , a.area as aarea, a.street as astreet,a.address as aaddress,
                  ...
                  r.rtype,r.status as rstatus,o.sendtype,store.storename' . $field . ' from ' . tablename('tapgo_tt_order') . ' o' . ' left join ' . tablename('tapgo_tt_order_refund') . ' r on r.id =o.refundid ' . ' left join ' . tablename('tapgo_tt_member') . ' m on m.openid=o.openid and m.uniacid =  o.uniacid ' . ' left join ' . tablename('tapgo_tt_member_address') . ' a on a.id=o.addressid ' . ' left join ' . tablename('tapgo_tt_dispatch') . ' d on d.id = o.dispatchid ' . ' left join ' . tablename('tapgo_tt_saler') . ' s on s.openid = o.verifyopenid and s.uniacid=o.uniacid' . ' left join ' . tablename('tapgo_tt_member') . ' sm on sm.openid = s.openid and sm.uniacid=s.uniacid' . ' left join ' . tablename('tapgo_tt_store') . ' store on store.id = o.verifystoreid' . $leftsql . (' ' . $sqlcondition . ' where ' . $condition . ' AND o.status =3 GROUP BY o.id ORDER BY o.createtime DESC  ');
...
';
		...
		$t = pdo_fetch('SELECT COUNT(DISTINCT(o.id)) as count, ifnull(sum(o.price),0) as sumprice   FROM ' . tablename('tapgo_tt_order') . ' o ' . ' left join ' . tablename('tapgo_tt_order_refund') . ' r on r.id =o.refundid ' . ' left join ' . tablename('tapgo_tt_member') . ' m on m.openid=o.openid  and m.uniacid =  o.uniacid' . ' left join ' . tablename('tapgo_tt_member_address') . ' a on o.addressid = a.id ' . ' left join ' . tablename('tapgo_tt_saler') . ' s on s.openid = o.verifyopenid and s.uniacid=o.uniacid' . ' left join ' . tablename('tapgo_tt_member') . ' sm on sm.openid = s.openid and sm.uniacid=s.uniacid' . ' left join ' . tablename('tapgo_tt_store') . ' store on store.id = o.verifystoreid' . $leftsql . (' ' . $sqlcondition . ' WHERE ' . $condition . ' AND o.status =3'), $paras);
		$total = $t['count'];
