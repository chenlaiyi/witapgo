...
class Log_TapgoTtPage extends WebPage {
...
//        $sql = "select log.id,m.id as mid, g.realname,m.avatar,m.weixin,log.logno,log.type,log.status,log.rechargetype,log.sendmoney,m.nickname,m.mobile,g.groupname,log.money,log.createtime,l.levelname,log.realmoney,log.deductionmoney,log.charge,log.remark,log.alipay,log.bankname,log.bankcard,log.realname as applyrealname,log.applytype from " . tablename('tapgo_tt_member_log') . " log "
//            . " left join " . tablename('tapgo_tt_member') . " m on m.openid=log.openid"
//            . " left join " . tablename('tapgo_tt_member_group') . " g on m.groupid=g.id"
//            . " left join " . tablename('tapgo_tt_member_level') . " l on m.level =l.id"
//            . " where 1 {$condition} ORDER BY log.createtime DESC ";

//        $sql = "select log.id,log.openid,log.logno,log.type,log.status,log.rechargetype,log.sendmoney,log.money,log.createtime,log.realmoney,log.deductionmoney,log.charge,log.remark,log.alipay,log.bankname,log.bankcard,log.realname as applyrealname,log.applytype from " . tablename('tapgo_tt_member_log') . " log "
//            . " where 1 {$condition} {$member_sql} ORDER BY log.createtime DESC ";

        $sql = "select log.id,log.bankopen,log.openid,log.logno,log.type,log.status,log.rechargetype,log.sendmoney,log.money,log.createtime,log.realmoney,log.deductionmoney,log.charge,log.remark,log.alipay,log.bankname,log.bankcard,log.realname as applyrealname,log.applytype,m.nickname,m.id as mid,m.avatar,m.level,m.groupid,m.realname,m.mobile,g.groupname,l.levelname from " . tablename('tapgo_tt_member_log') . " log "
            ." left join ".tablename('tapgo_tt_member')." m on m.openid = log.openid "
            ." left join ".tablename('tapgo_tt_member_group')." g on g.id = m.groupid "
            ." left join ".tablename('tapgo_tt_member_level')." l on l.id = m.level "
...
//        print_r($sql);die();
        ...
        if (!empty($list)) {
            ...
            $members_sql = "select id as mid, realname,avatar,weixin,nickname,mobile,openid,openid_wa from " . tablename('tapgo_tt_member') . " m where uniacid=:uniacid and (openid IN ('".implode("','",array_unique($openids))."') OR openid_wa IN ('".implode("','",array_unique($openids))."'))";
        ...
        if ($condition1 != '')
        {
            $condition_member = pdo_fetchall("SELECT openid FROM ".tablename('tapgo_tt_member')."m WHERE m.uniacid = :uniacid {$condition1}", $total_params);
            
            $condition_member2 = pdo_fetchall("SELECT CONCAT('sns_wa_',openid_wa) FROM ".tablename('tapgo_tt_member')." m WHERE m.uniacid = :uniacid {$condition1}", $total_params);
        ...
        $total = pdo_fetchcolumn("select count(*) from " . tablename('tapgo_tt_member_log') . " log "
    ...
    function refund($tid=0, $fee = 0, $reason = '') {
        ...
        $set = $_W['shopset']['shop'];
        $id = intval($_GPC['id']);
        $log = pdo_fetch('select * from ' . tablename('tapgo_tt_member_log') . ' where id=:id and uniacid=:uniacid limit 1', array(
        ...
        if ($log['rechargetype'] == 'wechat') {
            ...
            if ($log['apppay']==2){
                ...
                if($log['isuni']==1){
                    $result = m('finance')->unipay($log['openid'], 1, $log['money'] * 100, $out_refund_no, $log['logno']);
                }else{
    ...
    function wechat() {
        ...
        if (!empty($data['paytype']['withdraw'])){
            if(strpos($log['openid'],'sns_wa_')===false){
                ...
                pdo_update('tapgo_tt_member_log',array(
    ...
    function refuse() {
        ...
        $member=pdo_fetchall('SELECT * FROM '.tablename('tapgo_tt_member').' WHERE uniacid =:uniacid AND openid=:openid',array(':uniacid' => $_W['uniacid'],':openid'=>$log['openid']));
        //模板消息
        m('notice')->sendMemberLogMessage($log['id']);
