...
class Credit_TapgoTtPage extends WebPage
{
    ...
    protected function main($type = 'credit1')
    {
        ...
        $params = array(':uniacid' => $_W['uniacid'], ':module1' => 'tapgo_ttv2', ':module2' => 'tapgo_tt',':credittype' => $type);
        ...
        if($_GPC['groupid'] || $_GPC['level'] || $_GPC['keyword']){
            ...
            if($type == 'credit1'){
                /* $sql = "select log.*,m.id as mid, m.realname,m.avatar,m.nickname,m.avatar, m.mobile, m.weixin,u.username,g.groupname,l.levelname  from " . tablename('mc_credits_record') . " log "
                     ...
                     . " left join " . tablename('tapgo_tt_member') . " m on m.uid=log.uid"
               ...
               /* $table1 = "select log.id,log.num,log.presentcredit,log.createtime,log.remark,log.credittype,m.id as mid,m.openid, m.realname,m.nickname,m.avatar, m.mobile, m.weixin,u.username,g.groupname,l.levelname from " . tablename('mc_credits_record') . " log "
                ...
                $table2 = "select log.id,log.num,log.presentcredit,log.createtime,log.remark,log.credittype,m.id as mid,m.openid, m.realname,m.nickname,m.avatar, m.mobile, m.weixin,u.username,g.groupname,l.levelname  from " . tablename('tapgo_tt_member_credit_record') . " log "
                    . " left join " . tablename('users') . " u on  log.operator=u.uid and log.operator<>0 and log.operator<>log.uid"
               ...
               /* $table1 = "select log.id,log.num,log.presentcredit,log.createtime,log.remark,log.credittype,m.id as mid,m.openid, m.realname,m.nickname,m.avatar, m.mobile, m.weixin,u.username,g.groupname,l.levelname  from " . tablename('mc_credits_record') . " log "
                    ...
                    . " left join " . tablename('tapgo_tt_member_level') . " l on m.level =l.id"
                    . " where 1 {$condition} and log.uid<>0";*/
//                $table2 = "select log.id,log.money,log.createtime,log.remark,'credit2' as credittype,m.id as mid,m.openid, m.realname,m.nickname,m.avatar, m.mobile, m.weixin,log.rechargetype as username,g.groupname,l.levelname from " . tablename('tapgo_tt_member_log') . "as log "
//                    . " inner join " . tablename('tapgo_tt_member') . " m on m.openid=log.openid"
//                    . " left join " . tablename('tapgo_tt_member_group') . " g on m.groupid=g.id"
//                    . " left join " . tablename('tapgo_tt_member_level') . " l on m.level =l.id"
//                    . " where m.uid=0 and log.status=1 {$condition1}";
           ...
           /* $table1 = "select log.id,log.num,log.presentcredit,log.createtime,log.remark,log.credittype,u.username,log.uid,'xxx' as openid from " . tablename('mc_credits_record') . " log "
                . " left join " . tablename('users') . " u on  log.operator=u.uid and log.operator<>0 and log.operator<>log.uid"
                . " where 1 {$condition} and log.uid<>0";*/
            $table2 = "select log.id,log.num,log.presentcredit,log.createtime,log.remark,log.credittype,u.username,'0' as uid,log.openid  from " . tablename('tapgo_tt_member_credit_record') . " log "
                . " left join " . tablename('users') . " u on  log.operator=u.uid and log.operator<>0 and log.operator<>log.uid"
                . " where 1 {$condition} ";
            //$sql = "select * from (".$table1." UNION ALL ".$table2.") as main order by createtime desc";
            //正常显示列表的时候 , 使用咱们自己的表(tapgo_tt_member_credit_record)记录
            $sql = "select log.id,log.num,log.presentcredit,log.createtime,log.remark,log.credittype,u.username,'0' as uid,log.openid  from " . tablename('tapgo_tt_member_credit_record') . " log "
                . " left join " . tablename('users') . " u on  log.operator=u.uid and log.operator<>0 and log.operator<>log.uid"
                . " where 1 {$condition} order by log.createtime desc";
        }
        ...
        if($list && $search_flag == 0){
            foreach ($list as $key=>$val){
                $member = array();
                if($val['uid']){
                    $member = pdo_fetch("select * from " . tablename('tapgo_tt_member') ." where uniacid=:uniacid and uid=:uid",array(':uniacid'=>$_W['uniacid'],':uid'=>$val['uid']));
                }else{
                    $member = pdo_fetch("select * from " . tablename('tapgo_tt_member') ." where uniacid=:uniacid and openid=:openid",array(':uniacid'=>$_W['uniacid'],':openid'=>$val['openid']));
                }
                $groupname = pdo_fetchcolumn("select groupname from ". tablename('tapgo_tt_member_group') ." where uniacid=:uniacid and id=:id",array(':uniacid'=>$_W['uniacid'],':id'=>$member['groupid']));
                $levelname = pdo_fetchcolumn("select levelname from ". tablename('tapgo_tt_member_level') ." where uniacid=:uniacid and id=:id",array(':uniacid'=>$_W['uniacid'],':id'=>$member['level']));
                ...
                $list[$key]['nickname'] = trim($member['nickname'],'=');
        ...
        if($type == 'credit1'){
            ...
            if(empty($search_flag)){
                $total_sql = "select count(*) as ccc  from " . tablename('tapgo_tt_member_credit_record') . " log "
                    . " left join " . tablename('users') . " u on  log.operator=u.uid and log.operator<>0 and log.operator<>log.uid"
                    . " where 1 {$condition}";
