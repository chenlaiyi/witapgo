...
class Index_TapgoTtPage extends AppMobilePage
{
	...
	public function get_list()
	{
		...
		$list = pdo_fetchall('select id,ordersn,price,userdeleted,isparent,refundstate,paytype,status,addressid,refundid,isverify,dispatchtype,verifytype,verifyinfo,verifycode,iscomment,iscycelbuy,verified,createtime from ' . tablename('tapgo_tt_order') . (' where 1 ' . $condition . ' order by createtime desc LIMIT ') . ($pindex - 1) * $psize . ',' . $psize, $params);
		$total = pdo_fetchcolumn('select count(*) from ' . tablename('tapgo_tt_order') . (' where 1 ' . $condition), $params);
		...
		foreach ($list as &$row) {
			...
			$sql = 'SELECT og.id,og.goodsid,og.total,g.title,g.thumb,g.type, og.price,og.optionname as optiontitle,op.marketprice,g.marketprice as gprice,og.optionid,op.specs,g.merchid,g.status,og.single_refundid,og.single_refundstate FROM ' . tablename('tapgo_tt_order_goods') . ' og ' . ' left join ' . tablename('tapgo_tt_goods') . ' g on og.goodsid = g.id ' . ' left join ' . tablename('tapgo_tt_goods_option') . ' op on og.optionid = op.id ' . (' where ' . $scondition . ' order by og.id asc');
			...
			foreach ($goods as &$r) {
				...
				if ($r['status'] == 2) {
					$giftinfo = pdo_fetch('select id,title,thumb,marketprice,total from ' . tablename('tapgo_tt_goods') . ' where uniacid = ' . $uniacid . ' and status = 2  and id = ' . $r['goodsid'] . ' ');
			...
			if (0 < $row['refundstate'] && !empty($row['refundid'])) {
				$refund = pdo_fetch('select * from ' . tablename('tapgo_tt_order_refund') . ' where id=:id and uniacid=:uniacid and orderid=:orderid limit 1', array(':id' => $row['refundid'], ':uniacid' => $uniacid, ':orderid' => $row['id']));
			...
			if ($row['isonlyverifygoods']) {
				$row['canverify'] = false;
				$verifygood = pdo_fetch('select * from ' . tablename('tapgo_tt_verifygoods') . ' where orderid=:orderid limit 1', array(':orderid' => $row['id']));

				if (!empty($verifygood)) {
					$row['verifygoods_id'] = $verifygood['id'];
					$verifynum = pdo_fetchcolumn('select sum(verifynum) from ' . tablename('tapgo_tt_verifygoods_log') . '    where verifygoodsid =:id  ', array(':id' => $verifygood['id']));
	...
	public function detail()
	{
		...
		if (0 < $order['seckilldiscountprice']) {
			...
			$page = pdo_fetch('SELECT * FROM ' . tablename('tapgo_tt_wxapp_page') . $where . ' LIMIT 1 ', $params);
		...
		$gn = 0;
		$nog = 0;
		$goods = pdo_fetchall('select og.id as ordergoodsid,og.single_refundstate,og.sendtime,g.id,g.type, og.goodsid,og.price,g.title,g.thumb,g.status,og.total,g.credit,og.optionid,og.optionname as optiontitle,g.isverify,g.isfullback,g.refund,g.cannotrefund,g.returngoods,g.exchange,g.storeids' . $diyformfields . '  from ' . tablename('tapgo_tt_order_goods') . ' og ' . ' left join ' . tablename('tapgo_tt_goods') . ' g on g.id=og.goodsid ' . (' where ' . $scondition . ' and og.uniacid=:uniacid '), $param);
		$goods = set_medias($goods, array('thumb'));
		$goodsrefund = true;
		...
		if (!empty($goods)) {
			...
			foreach ($goods as &$g) {
				if ($g['isfullback']) {
					$isfullback = true;
					$fullbackgoods = pdo_fetch('SELECT * FROM ' . tablename('tapgo_tt_fullback_goods') . ' WHERE goodsid = :goodsid and uniacid = :uniacid  limit 1 ', array(':goodsid' => $g['goodsid'], ':uniacid' => $uniacid));

					if ($g['optionid']) {
						$option = pdo_fetch('select `day`,allfullbackprice,fullbackprice,allfullbackratio,fullbackratio,isfullback
                      from ' . tablename('tapgo_tt_goods_option') . ' where id = :id and uniacid = :uniacid ', array(':id' => $g['optionid'], ':uniacid' => $uniacid));
		...
		if ($order['isonlyverifygoods']) {
			...
			$verifygood = pdo_fetch('select * from ' . tablename('tapgo_tt_verifygoods') . ' where orderid=:orderid limit 1', array(':orderid' => $order['id']));
		...
		foreach ($gift as $kek => $val) {
			$giftinfo = pdo_fetch('select id,title,thumb,marketprice,total from ' . tablename('tapgo_tt_goods') . ' where uniacid = ' . $uniacid . ' and status = 2  and id = ' . $val['id'] . ' ');
			if (empty($giftinfo) || $giftinfo['total'] == 0) {
				unset($gift[$kek]);
			}
		}
		...
		if (!empty($order['virtual']) && !empty($order['virtual_str'])) {
			...
			$result['virtualtemp'] = pdo_fetch('SELECT linktext, linkurl,description FROM ' . tablename('tapgo_tt_virtual_type') . ' WHERE id=:id AND uniacid=:uniacid LIMIT 1', array(':id' => $order['virtual'], ':uniacid' => $_W['uniacid']));
		}

		if ($order['iscycelbuy'] == 1) {
			$cycelSql = 'SELECT * FROM ' . tablename('tapgo_tt_cycelbuy_periods') . ' WHERE orderid=:orderid AND uniacid=:uniacid';
			$cycelParams = array(':orderid' => $order['id'], ':uniacid' => $_W['uniacid']);
			$cycelData = pdo_fetchall($cycelSql, $cycelParams);
			$cycelUnderway = pdo_fetch('SELECT count(*) as count FROM ' . tablename('tapgo_tt_cycelbuy_periods') . ' WHERE orderid=' . $order['id'] . ' AND status<=1 AND uniacid=' . $_W['uniacid']);
	...
	public function express()
	{
		...
		if (!empty($cycelid)) {
			$order = pdo_fetch('select expresscom,expresssn,addressid,status,express,sendtype,address,carrier from ' . tablename('tapgo_tt_cycelbuy_periods') . ' where id=:id and uniacid=:uniacid', array(':id' => $cycelid, ':uniacid' => $uniacid));
		}
		else {
			$order = pdo_fetch('select expresscom,expresssn,addressid,status,express,sendtype,address,carrier from ' . tablename('tapgo_tt_order') . ' where id=:id and uniacid=:uniacid and openid=:openid limit 1', array(':id' => $orderid, ':uniacid' => $uniacid, ':openid' => $openid));
		...
		if (!empty($order['sendtype']) && $sendtype == 0) {
			...
			while ($i <= intval($order['sendtype'])) {
				...
				$goods_arr = pdo_fetchall('select g.title,g.thumb,og.total,og.optionname as optiontitle,og.expresssn,og.express,
                    og.sendtype,og.expresscom,og.sendtime from ' . tablename('tapgo_tt_order_goods') . ' og ' . ' left join ' . tablename('tapgo_tt_goods') . ' g on g.id=og.goodsid ' . ' where og.orderid=:orderid and og.sendtype = ' . $i . ' and og.uniacid=:uniacid ', array(':uniacid' => $uniacid, ':orderid' => $orderid));
		...
		$goods = pdo_fetchall('select og.goodsid,og.price,g.title,g.thumb,og.total,g.credit,og.optionid,og.optionname as optiontitle,g.isverify,og.expresssn,og.express,
            og.sendtype,og.expresscom,og.sendtime,g.storeids from ' . tablename('tapgo_tt_order_goods') . ' og ' . ' left join ' . tablename('tapgo_tt_goods') . ' g on g.id=og.goodsid ' . ' where og.orderid=:orderid and og.single_refundtime=0 ' . $condition . ' and og.uniacid=:uniacid ', array(':uniacid' => $uniacid, ':orderid' => $orderid));
	...
	public function cycelbuy_list()
	{
		...
		$sql = 'SELECT id,cycelbuy_periodic,refundstate FROM ' . tablename('tapgo_tt_order') . ' WHERE id=:orderid AND uniacid =:uniacid ';
		$orderData = pdo_fetch($sql, array(':uniacid' => $_W['uniacid'], ':orderid' => $orderid));
		$cycelSql = 'SELECT * FROM ' . tablename('tapgo_tt_cycelbuy_periods') . ' WHERE orderid=:orderid AND uniacid=:uniacid';
		$cycelParams = array(':orderid' => $orderid, ':uniacid' => $_W['uniacid']);
		$cycelData = pdo_fetchall($cycelSql, $cycelParams);
		$cycelUnderway = pdo_fetch('SELECT count(*) as count FROM ' . tablename('tapgo_tt_cycelbuy_periods') . ' WHERE orderid=' . $orderid . ' AND status<=1 AND uniacid=' . $_W['uniacid']);
		$notStart = false;
		$status0 = 0;
		$status2 = 0;
		$period_index = 1;
		...
		if (!empty($cycelData)) {
			...
			$existApply = '';
			$existApply = pdo_get('tapgo_tt_address_applyfor', array('orderid' => $orderid, 'uniacid' => $_W['uniacid'], 'isdelete' => 0));
		...
		$applyfor = pdo_get('tapgo_tt_address_applyfor', array('orderid' => $orderid, 'uniacid' => $_W['uniacid']));
	...
	public function getCycelbuyDate()
	{
		...
		$cycelSql = 'SELECT * FROM ' . tablename('tapgo_tt_cycelbuy_periods') . ' WHERE id=:cycelid AND uniacid=:uniacid';
	...
	public function do_deferred()
	{
		...
		$order = pdo_get('tapgo_tt_order', array('id' => $order_id));
		...
		$data = pdo_fetchall('select * from ' . tablename('tapgo_tt_cycelbuy_periods') . ' where ' . $condition, $param);

		foreach ($data as $k => $v) {
			$receipttime = $time + $interval * $k;
			pdo_update('tapgo_tt_cycelbuy_periods', array('receipttime' => $receipttime), array('id' => $v['id']));
		}
	...
	public function address()
	{
		...
		$orderid = intval($_GPC['id']);
		$cycelid = intval($_GPC['cycelid']);
		$sql = 'SELECT id,cycelbuy_periodic FROM ' . tablename('tapgo_tt_order') . ' WHERE id=:orderid AND uniacid =:uniacid ';
		$orderData = pdo_fetch($sql, array(':uniacid' => $_W['uniacid'], ':orderid' => $orderid));
		$ordersn = $orderData['ordersn'];

		if (!empty($orderid)) {
			$address = pdo_fetch('select * from ' . tablename('tapgo_tt_cycelbuy_periods') . ' where id=:id and orderid=:orderid and status = 0', array(':orderid' => $orderid, ':id' => $cycelid));
			$address = iunserializer($address['address']);
		}

		if (!empty($applyforid)) {
			$new_address = pdo_fetch('select * from ' . tablename('tapgo_tt_address_applyfor') . ' where cycleid = :cycleid and isdelete = 0 order by createtime desc limit 1', array(':cycleid' => $cycelid));
	...
	public function addressSubmit()
	{
		...
		if (!empty($applyid)) {
			$res = pdo_update('tapgo_tt_address_applyfor', $new_data, array('id' => $applyid));
		}
		else {
			$is_submit = pdo_get('tapgo_tt_address_applyfor', array('orderid' => $orderid, 'isdelete' => 0, 'isdispose' => 0));

			if ($is_submit) {
				return app_json(1, '请勿重复提交');
			}

			$address = pdo_fetch('select * from ' . tablename('tapgo_tt_cycelbuy_periods') . ' where orderid=:orderid and (status = 0 or status = 1) order by receipttime asc limit 1', array(':orderid' => $orderid));
			$new_data['old_address'] = $address['address'];
			$res = pdo_insert('tapgo_tt_address_applyfor', $new_data);
		}
	...
	public function confirm_receipt()
	{
		...
		$order = pdo_fetch('select * from ' . tablename('tapgo_tt_order') . ' where uniacid=:uniacid and id=:id  limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $orderid));
		$last_periods = pdo_fetch('select * from ' . tablename('tapgo_tt_cycelbuy_periods') . ' where uniacid=:uniacid and orderid=:orderid order by id desc  limit 1', array(':uniacid' => $_W['uniacid'], ':orderid' => $orderid));

		if (!empty($last_periods)) {
			if ($last_periods['id'] == $id) {
				pdo_update('tapgo_tt_order', array('status' => 3, 'finishtime' => time()), array('id' => $orderid, 'status' => 2));
				$result = pdo_update('tapgo_tt_cycelbuy_periods', array('status' => 2, 'finishtime' => time()), array('orderid' => $orderid, 'uniacid' => $_W['uniacid']));
				...
				$result = pdo_update('tapgo_tt_cycelbuy_periods', array('status' => 2, 'finishtime' => time()), array('id' => $id, 'uniacid' => $_W['uniacid']));
			}
	...
	public function cycel_orderList()
	{
		...
		$com_verify = com('verify');
		$list = pdo_fetchall('select id,ordersn,price,userdeleted,isparent,refundstate,paytype,status,addressid,refundid,isverify,dispatchtype,verifytype,verifyinfo,verifycode,iscomment from ' . tablename('tapgo_tt_order') . (' where 1 ' . $condition . ' order by createtime desc LIMIT ') . ($pindex - 1) * $psize . ',' . $psize, $params);
		$total = pdo_fetchcolumn('select count(*) from ' . tablename('tapgo_tt_order') . (' where 1 ' . $condition), $params);
		$refunddays = intval($_W['shopset']['trade']['refunddays']);
	...
	public function express_number()
	{
		...
		if ($_GPC['submit']) {
			...
			$res = pdo_update('tapgo_tt_order_refund', $refund, array('id' => $refundid, 'uniacid' => $uniacid));
		...
		$sql = 'SELECT * FROM ' . tablename('tapgo_tt_order_refund') . ' WHERE id = :id AND uniacid = :uniacid';
	...
	public function single_express_number()
	{
		...
		if ($_GPC['submit']) {
			...
			$refund = array('status' => 4, 'express' => trim($_GPC['express']), 'expresscom' => trim($_GPC['expresscom']), 'expresssn' => trim($_GPC['express_number']), 'sendtime' => time());
			$res = pdo_update('tapgo_tt_order_single_refund', $refund, array('id' => $refundid, 'uniacid' => $uniacid));
		...
		$sql = 'SELECT * FROM ' . tablename('tapgo_tt_order_single_refund') . ' WHERE id = :id AND uniacid = :uniacid';
		$params = array(':id' => $refundid, ':uniacid' => $uniacid);
		$refund = pdo_fetch($sql, $params);
		$express_list = m('express')->getExpressList();
		$index = '';
