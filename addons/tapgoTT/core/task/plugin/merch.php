...
if (!empty($sets)) {
	foreach ($sets as $set) {
		$sql = 'SELECT id,accounttime FROM ' . tablename('tapgo_tt_merch_user') . ' WHERE accounttime <= ' . time() . ' AND uniacid = :uniacid';
		$params = array(':uniacid' => $set['uniacid']);
		$merchUsers = pdo_fetchall($sql, $params);

		if (!empty($merchUsers)) {
			foreach ($merchUsers as $merchUser) {
				pdo_update('tapgo_tt_goods', array('status' => 0), array('merchid' => $merchUser['id'], 'uniacid' => $set['uniacid']));
			}
		}
	}
}
