...
foreach ($sets as $set) {
    ...
    $logs = pdo_fetchall("select id,`day`,fullbackday,openid,priceevery,price,fullbacktime,goodsid,optionid from ".tablename('tapgo_tt_fullback_log')." where uniacid = ".$_W['uniacid']." and isfullback = 0 and (fullbacktime =0 or fullbacktime < ".strtotime('-1 days').") and fullbackday < day ");
    $today = strtotime(date("Y-m-d"),time());
    foreach ($logs as $key => $value){

        // 查询当前log下的log_map 有大于当前时间的跳过
        $mapCount = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('tapgo_tt_fullback_log_map') . ' WHERE logid=:logid AND fullback_time >= :fullback_time', array(
            ':logid' => $value['id'],
        ...
        if(($value['day']-$value['fullbackday'])>1){
            ...
            if($count>=1){
                ...
                if($value['day']-$value['fullbackday']<=$count){
                    ...
                    $success = pdo_update('tapgo_tt_fullback_log', array('fullbackday'=>$value['day'],'fullbacktime'=>$today,'isfullback'=>1), array('id' => $value['id']));
                    if ($success){
                    ...
                    $success = pdo_update('tapgo_tt_fullback_log', array('fullbackday'=>$value['fullbackday'],'fullbacktime'=>$today), array('id' => $value['id']));
                    if ($success){
                        $result = m('member')->setCredit($value['openid'], 'credit2', $value['priceevery'], array('0', $_W['shopset']['shop']['name'] . '全返余额' . $value['priceevery']));
                    }
                }
            ...
            $success = pdo_update('tapgo_tt_fullback_log', array('fullbackday'=>$value['day'],'fullbacktime'=>time(),'isfullback'=>1), array('id' => $value['id']));
        ...
        if($count>1 && $success){
            for ($i = 1; $i <= $count-1; $i++) {
                ...
                $logdata['day'] = 0;
                pdo_insert('tapgo_tt_fullback_log_map',$logdata);
            }
        }elseif ($success){
            $logdata = array();
            $logdata['uniacid'] =  $_W['uniacid'];
