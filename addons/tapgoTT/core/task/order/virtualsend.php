...
ignore_user_abort();
set_time_limit(0);
$logs = pdo_fetchall('select * from ' . tablename('tapgo_tt_virtual_send_log') . ' where uniacid=:uniacid and status=0 limit 10', array(':uniacid' => $_W['uniacid']));
if ($logs) 
{
	foreach ($logs as $log ) 
	{
		...
		if ($log['sendtime'] < time()) 
		{
			...
			if ($res) 
			{
				pdo_update('tapgo_tt_virtual_send_log', array('status' => 1), array('id' => $log['id'], 'uniacid' => $_W['uniacid']));
			}
		}
	}
}
