...
if ($operation == 'display') {
    ...
    $schoollist = pdo_fetchall("SELECT id,title,typeid,cityid,areaid,logo,address,tel,weid FROM " . tablename('wx_school_index') . " $where $condition   order by ssort desc,id desc LIMIT " . ($pindex - 1) * $psize . ",{$psize}");
...
}elseif($operation == 'getBgShowInfo'){
    ...
    include $this->template ( 'web/bgshowinfo' );
    die;
}elseif($operation == 'setBgShowInfo'){
    $data = array(
        'jsandxs' => $_GPC['jsandxs'],
        'kq' => $_GPC['kq'],
        'jhgn' => $_GPC['jhgn'],
        'cw' => $_GPC['cw'],
        'isshow' => $_GPC['isshow'],
    );
    $bgShowInfo = serialize($data);
    if(pdo_update('wx_school_schoolset',array('bgshowinfo'=>$bgShowInfo),array('schoolid'=>$_GPC['schoolid']))){
        $data ['result'] = true;
        $data['msg'] = '设置成功';
    }else{
        $data ['result'] = false;
