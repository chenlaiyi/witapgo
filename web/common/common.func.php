...
function check_w7_request() {
	global $_W;

	if ($_W['isw7_request']) {
		return true;
	}
	$notice = '最新版系统功能已经迁移到控制台，请前往控制台操作。';
	$redirect = 'https://user.w7.cc/v2/console/' . $_W['setting']['site']['key'] . '/go-license-console';
	$extend_buttons = array(
		'status_store_button' => array(
			'url' => $redirect,
	...
	if ($_W['isajax']) {
		...
		message($notice, $redirect, 'error', '', $extend_buttons);
	}
}