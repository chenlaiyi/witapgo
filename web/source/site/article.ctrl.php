...
if ($do == 'display') {
	...
	if (checksubmit('submit')) {
		...
		if (!empty($keyword)) {
			$result = pdo_getall('rule_keyword', array('uniacid !=' => $_W['uniacid'], 'content IN' => $keyword), array('rid'));
			if (!empty($result)) {
				$keywords = array();
				...
				foreach ($rules as $rule) {
					...
					$ruleurl .= "<a href='./index.php?c=platform&a=reply&do=post&rid=" . $rule['id'] . "' target='_blank'><strong class='text-danger'>" . $name . "</strong></a>&nbsp;";
				}
				itoast('该关键字已存在于 ' . $ruleurl . ' 规则中.', '', '');
			}
