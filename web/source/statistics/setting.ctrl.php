...
if ('display' == $do) {
	...
	$highest_api_visit = empty($statistics_setting['founder']) ? '不限' : $statistics_setting['founder'];
	$month_use = 0;
	...
	$total_remain_num = empty($statistics_setting['use']) ? (intval($highest_api_visit) + $order_num - $month_use) : (intval($highest_api_visit) + $order_num - $statistics_setting['use'] - $month_use);
	$total_remain_num = empty($statistics_setting['founder']) ? '不限' : max(0, $total_remain_num);
}
if ('edit_setting' == $do) {
	$type = trim($_GPC['type']);
