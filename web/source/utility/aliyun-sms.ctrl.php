...
if ($do == 'send_code') {
	...
	if($receiver_type == 'email') {
		...
		$cloud_sms = $_W['setting']['cloud_sms_info'];
		if(empty($cloud_sms['type'])){
			iajax(-1, '平台已关闭短信功能');
		}
		$data = array();
		if($cloud_sms['type'] ==1){
			switch($_GPC['tpl']){
				case 'find_pass':
					$alisms_code_tpl = $cloud_sms['alisms']['find_pass_tpl'];
					break;
				case 'register':
					$alisms_code_tpl = $cloud_sms['alisms']['user_bind_tpl'];
					break;
				case 'over':
					$alisms_code_tpl = $cloud_sms['alisms']['user_over_tpl'];
					break;
				default: $alisms_code_tpl = $cloud_sms['alisms']['find_pass_tpl']; break;
			}
			$data['type'] = 'alisms';
			$data['tpl'] = $alisms_code_tpl;
			$data['pars'] = $cloud_sms['alisms'];
			$data['receiver'] = $receiver;
			$data['TemplateParam'] = array("code" => $code);
		}elseif($cloud_sms['type'] ==2){
			
		}

		load()->classs('cloudapi');
		$api = new CloudApi();
		$result = $api->post('sms', 'send_code', $data, 'json');
	}
