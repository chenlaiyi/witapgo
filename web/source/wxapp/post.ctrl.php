...
if ('post' == $do) {
	...
	if ($is_submit) {
		...
		if (!empty($_GPC['choose']['modules'])) {
			...
			$_GPC['quickmenu'] = safe_gpc_array($_GPC['quickmenu']);
			$quickmenu = array(
		...
		if ($isedit) {
			...
			$add_uni_modules_sql = '';
			foreach ($uni_modules as $module_name) {
				$add_uni_modules_sql .= '(' . $uniacid . ',"' . $module_name . '"),';
			}
			$add_uni_modules_sql = 'INSERT INTO ' . tablename('uni_modules') . ' (`uniacid`, `module_name`) VALUES ' . rtrim($add_uni_modules_sql, ',');
			pdo_query($add_uni_modules_sql);
		}
