...
if ('install' == $do) {
	...
	if (APPLICATION_TYPE_TEMPLATES == $installed_module['application_type'] || APPLICATION_TYPE_TEMPLATES == $application_type) {
		...
		$module = $manifest;
		//$module['version'] = $packet['version'];//by 壹锋源码
	...
	if (APPLICATION_TYPE_TEMPLATES == $installed_module['application_type'] || APPLICATION_TYPE_TEMPLATES == $application_type) {
		...
		if (!empty($module_info['version']['cloud_setting']) || !empty($manifest['application']['cloud_setting'])) {
			$module['settings'] = 2;
		}
		...
		if (empty($code)) {
			...
			$module_support_name_arr = $manifest['platform']['supports'];
			foreach ($module_support_name_arr as &$support) {
				if ('app' == $support) {
					$support = 'account';
				}
