...
if ('sms' == $do) {
	...
	if (!empty($_GPC['mobile'])) {
		$timewhere = empty($timewhere) ? ' WHERE `mobile` LIKE :mobile ' : $timewhere . ' AND `mobile` LIKE :mobile ';
		$params[':mobile'] = "%{$_GPC['mobile']}%";
	}
	$pindex = max(1, intval($_GPC['page']));
	$psize = 40;
	$sql = 'SELECT * FROM' . tablename('core_sendsms_log') . $timewhere . ' ORDER BY id DESC LIMIT ' . ($pindex - 1) * $psize . ',' . $psize;
	$list = pdo_fetchall($sql, $params);
	$total = pdo_fetchcolumn('SELECT COUNT(*) FROM' . tablename('core_sendsms_log') . $timewhere, $params);
	$pager = pagination($total, $pindex, $psize);
	if ($_W['isajax']) {
		$message = array(
			'list' => $list,
